...
React Native companion app for Dobius. Monitor worktrees, view terminal output, and send commands from your phone.

Local development uses two processes:

- Dobius desktop/Electron from the repo root. This hosts the mobile WebSocket RPC server on port `6768`.
...
## Pair With Desktop Dobius

1. Open Dobius desktop.
...
dobius fill --element @e1 --value "ls" --json
...
```bash
cd mobile
DOBIUS_MOBILE_WS_URL=ws://127.0.0.1:6768 pnpm exec tsx scripts/test-subscribe.ts <deviceToken> <serverPublicKeyB64>
```
...
```bash
cd mobile
DOBIUS_MOBILE_WS_URL=ws://127.0.0.1:6768 pnpm exec tsx scripts/repro-terminal-colors.ts \
...
Develop the mobile app without a running Dobius desktop instance:
...
## Connecting to Real Dobius

1. Start Dobius desktop with WebSocket transport enabled
2. In Dobius, go to Settings > Mobile and scan the QR code with this app
