...
import {
  ...
  trustedDobiusHooksWithSetupApproval,
...
type DobiusYamlTrustPrompt = WorkspaceCreateArgs & {
  repoId: string
  repoName: string
  scriptContent: string
  contentHash: string
...
type ProjectRepoNotInDobiusPrompt = {
  owner: string
  repo: string
  url: string | null
}
...
export default function MobileTasksScreen() {
  ...
  const [trustedDobiusHooks, setTrustedDobiusHooks] = useState<PersistedTrustedDobiusHooks>({})
  const [dobiusYamlTrustPrompt, setDobiusYamlTrustPrompt] = useState<DobiusYamlTrustPrompt | null>(null)
  ...
  const [projectRepoNotInDobius, setProjectRepoNotInDobius] =
    useState<ProjectRepoNotInDobiusPrompt | null>(null)
  ...
  const persistSetupHookTrust = useCallback(
    async (repoId: string, contentHash: string, alwaysTrust: boolean): Promise<void> => {
      ...
      const next = trustedDobiusHooksWithSetupApproval({
        trust: trustedDobiusHooks,
        repoId,
        contentHash,
        alwaysTrust
      })
      const response = await client.sendRequest('ui.set', { trustedDobiusHooks: next })
      if (!isSuccess(response)) {
        throw new Error(response.error.message)
      }
      setTrustedDobiusHooks(next)
    },
    [client, trustedDobiusHooks]
  )
  ...
  useEffect(() => {
    if (!client || connState !== 'connected') {
      ...
      setProjectRepoNotInDobius(null)
    ...
    const hydrateTaskState = async (): Promise<void> => {
      ...
      if (!status.capabilities?.includes(MOBILE_TASKS_CAPABILITY)) {
        ...
        setMergeMethodProjectRow(null)
        resetWorkspaceCreateState()
        setError('Update Dobius desktop to use Tasks on mobile.')
        setTaskStateHydrated(false)
        return
      }
      ...
      const uiState = isSuccess(uiResponse)
        ...
        : null
      setTrustedDobiusHooks(uiState?.trustedDobiusHooks ?? {})
    ...
    async (project: GitHubProjectRef, options: { viewNumber?: number } = {}): Promise<void> => {
      ...
      try {
        ...
        if (explicitView && explicitView.layout !== 'TABLE_LAYOUT') {
          throw new Error("Dobius doesn't support this GitHub Project layout yet.")
        }
    ...
    async (
      ...
      try {
        ...
        if (
          ...
          // Why: desktop prompts before running repo-owned dobius.yaml hooks. Mobile
          ...
          setDobiusYamlTrustPrompt({
            ...
            previouslyApproved: wasSetupHookPreviouslyApproved(trustedDobiusHooks, targetRepo.id)
          })
          return
        }
  ...
  const createWorkspaceFromProjectRow = useCallback(
    async (row: GitHubProjectRow): Promise<void> => {
      ...
      if (!kind || !row.content.number || !row.content.url) {
        setError('Add the project item repository to Dobius before creating a workspace.')
        return
      }
      if (!repo) {
        const slug = splitRepositorySlug(row.content.repository)
        setProjectRepoNotInDobius({
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {!tasksSupported ? (
        tasksUnsupported ? (
          <View style={styles.centered}>
            <Text style={styles.emptyText}>Update Dobius desktop</Text>
      ...
      <PickerModal
        ...
        onSelect={(viewId) => {
          ...
          if (view && view.layout !== 'TABLE_LAYOUT') {
            setGithubProjectError("Dobius doesn't support this GitHub Project layout yet.")
            return
          }
      ...
      <BottomDrawer
        visible={taskUiReady && dobiusYamlTrustPrompt != null}
        onClose={() => setDobiusYamlTrustPrompt(null)}
        zIndex={TASK_SECONDARY_DRAWER_Z_INDEX + 1}
      >
        {dobiusYamlTrustPrompt ? (
          <View>
            <View style={styles.sheetHeader}>
              <Text style={styles.sheetTitle}>
                {dobiusYamlTrustPrompt.previouslyApproved
                  ? `${dobiusYamlTrustPrompt.repoName}'s setup script changed`
                  : `Run setup from ${dobiusYamlTrustPrompt.repoName}?`}
              </Text>
              <Text style={styles.sheetSubtitle}>
                This repository's dobius.yaml runs on your machine before the workspace starts. Only
            ...
            <View style={styles.setupPromptBox}>
              <View style={styles.detailSectionHeader}>
                <Text style={styles.detailSectionTitle}>
                  {dobiusYamlTrustPrompt.previouslyApproved ? 'New setup script' : 'Setup script'}
                </Text>
              </View>
              <Text style={styles.setupPromptCommand}>{dobiusYamlTrustPrompt.scriptContent}</Text>
            </View>

            <View style={styles.actionGroup}>
              <Pressable
                style={styles.actionRow}
                disabled={creatingKey === dobiusYamlTrustPrompt.item.key}
                onPress={() =>
                  void (async () => {
                    try {
                      await persistSetupHookTrust(
                        dobiusYamlTrustPrompt.repoId,
                        dobiusYamlTrustPrompt.contentHash,
                        false
                      )
                      setDobiusYamlTrustPrompt(null)
                      await createWorkspace(
                        dobiusYamlTrustPrompt.item,
                        dobiusYamlTrustPrompt.repoIdOverride,
                        'run',
                        dobiusYamlTrustPrompt.agentOverride,
                        dobiusYamlTrustPrompt.workspaceNameOverride,
                        dobiusYamlTrustPrompt.noteOverride,
                        dobiusYamlTrustPrompt.baseBranchOverride,
                        dobiusYamlTrustPrompt.branchNameOverride,
                        dobiusYamlTrustPrompt.sparseCheckoutOverride,
                        dobiusYamlTrustPrompt.contentHash
                      )
                    } catch (err) {
              ...
              <Pressable
                ...
                disabled={creatingKey === dobiusYamlTrustPrompt.item.key}
                onPress={() => {
                  const prompt = dobiusYamlTrustPrompt
                  setDobiusYamlTrustPrompt(null)
                  void createWorkspace(
                    prompt.item,
      ...
      <BottomDrawer
        visible={taskUiReady && projectRepoNotInDobius != null}
        onClose={() => {
          setProjectRepoNotInDobius(null)
        }}
      >
        {projectRepoNotInDobius ? (
          <View>
            <View style={styles.sheetHeader}>
              <Text style={styles.sheetTitle}>Repository not in Dobius</Text>
              <Text style={styles.sheetSubtitle}>
                {projectRepoNotInDobius.owner}/{projectRepoNotInDobius.repo} is not added to Dobius. Add
                this repository from the desktop app, then refresh mobile Tasks.
              </Text>
            </View>

            <View style={styles.actionGroup}>
              {projectRepoNotInDobius.url ? (
                <Pressable
                  style={styles.actionRow}
                  onPress={() => {
                    if (projectRepoNotInDobius.url) {
                      void Linking.openURL(projectRepoNotInDobius.url)
                    }
                  }}
                >
              ...
              {projectRepoNotInDobius.url ? <View style={styles.actionSeparator} /> : null}
              <Pressable
                style={styles.actionRow}
                onPress={() =>
                  void copyTextToClipboard(
                    `project-repo:${projectRepoNotInDobius.owner}/${projectRepoNotInDobius.repo}`,
                    `${projectRepoNotInDobius.owner}/${projectRepoNotInDobius.repo}`
                  )
                }
              >
            ...
            <View style={styles.actionGroup}>
              ...
              projectRowItem.content.state !== 'MERGED' ? (
                <>
                  ...
                  {!projectRowHostedRepo ? (
                    <Text style={styles.emptyInlineText}>
                      Merge requires this repository in Dobius.
                    </Text>
                  ) : null}
                </>
              ) : null}
