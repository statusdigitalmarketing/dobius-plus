#!/usr/bin/env node
/**
 * Start Dobius Mobile server and load it in the iOS emulator.
...
const DOBIUS_CLI = process.env.DOBIUS_CLI || 'dobius'
...
// Execute dobius CLI command
async function dobius(args, options = {}) {
  const { stdout, stderr } = await execFileAsync(DOBIUS_CLI, args, {
...
async function getWorktree() {
  ...
  try {
    const { stdout } = await dobius(['worktree', 'current', '--json'])
...
async function attachEmulator(worktree, device, runtime) {
  ...
  try {
    await dobius(['emulator', 'attach', device.udid, '--worktree', worktree, '--focus', '--json'], {
      cwd: worktree,
...
function devClientUrlForMetroUrl(url) {
  return `exp+dobius-mobile://expo-development-client/?url=${encodeURIComponent(url)}`
}
...
async function startMetro(worktree) {
  ...
  return new Promise((resolve, reject) => {
    ...
    rl.on('line', (line) => {
      ...
      const urlMatch = line.match(/exp\+dobius-mobile:\/\/expo-development-client\/\?url=([^\s]+)/)
...
async function openPairingUrlInSimulator(pairingUrl, deviceUdid, runtime, worktree) {
  ...
  await dobius(['emulator', 'tap', '0.5', '0.56', '--worktree', worktree, '--json'], {
    cwd: worktree,
    env: runtime?.env || process.env,
    timeout: 30000
  })
...
async function takeScreenshot(
  deviceUdid,
  outputPath = path.join(os.tmpdir(), 'dobius-mobile-ios.png')
) {
...
// Main function
async function main() {
  log(colors.bright + 'Starting Dobius Mobile in Emulator\n' + colors.reset)
  ...
  try {
    ...
    // Why: emulator helpers are worktree-scoped in Dobius; attach is idempotent
