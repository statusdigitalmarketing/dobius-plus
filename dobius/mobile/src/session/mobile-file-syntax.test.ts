...
describe('mobile file syntax highlighting', () => {
  ...
  it('emits semantic syntax segments for highlighted code', () => {
    const result = highlightMobileCode('const label: string = "Dobius"', 'typescript')
  ...
  it('falls back to plain text when segment caps would create too many React Native nodes', () => {
    const result = highlightMobileCode('const label: string = "Dobius"', 'typescript', 1_000, 1)

    expect(result.highlighted).toBe(false)
    expect(result.segments).toEqual([{ text: 'const label: string = "Dobius"', kind: 'plain' }])
  })
