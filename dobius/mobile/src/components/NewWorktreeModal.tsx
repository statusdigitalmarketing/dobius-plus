...
import type { PersistedTrustedDobiusHooks, TuiAgent } from '../../../src/shared/types'
      ...
      try {
        ...
        if (uiResponse.ok) {
          const result = (uiResponse as RpcSuccess).result as {
            ui?: { trustedDobiusHooks?: PersistedTrustedDobiusHooks }
          }
          setTrustedDobiusHooks(result.ui?.trustedDobiusHooks ?? {})
        }
      } catch {
  ...
  async function handleCreate(options: CreateOptions = {}) {
    ...
    try {
      ...
      if (
        ...
        !isSetupHookTrusted(trustedDobiusHooks, selectedRepo.id, setupTrust.contentHash)
      ) {
        // Why: desktop prompts before running repo-owned dobius.yaml setup hooks.
        ...
        setSetupTrustPrompt({
          ...
          previouslyApproved: wasSetupHookPreviouslyApproved(trustedDobiusHooks, selectedRepo.id)
        })
        return
      }
                      ...
                      {setupSource && (
                        <View style={styles.sourceBadge}>
                          <Text style={styles.sourceBadgeText}>
                            {setupSource === 'dobius.yaml' ? 'DOBIUS.YAML' : 'HOOKS'}
                          </Text>
                        </View>
                      )}
                    </View>
        ...
        {setupTrustPrompt ? (
          <View>
            <View style={styles.trustHeader}>
              ...
              <Text style={styles.subtitle}>
                This repository's dobius.yaml runs before the workspace starts. Only run it if you
