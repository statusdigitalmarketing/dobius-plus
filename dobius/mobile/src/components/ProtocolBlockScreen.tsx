...
const RELEASES_URL = 'https://github.com/statusdigitalmarketing/dobius-plus/releases'
// No Dobius+ iOS app is published yet — fall back to the releases page.
const IOS_APP_STORE_URL = RELEASES_URL
...
export function ProtocolBlockScreen({ verdict }: Props) {
  ...
  const title = isMobileTooOld ? 'Update Dobius Mobile' : 'Update Dobius on your computer'
  const body = isMobileTooOld
    ? `This desktop needs a newer Dobius Mobile app. Update Dobius Mobile from ${mobileUpdateTarget.storeName}, then try this host again.`
    : 'This paired desktop app is too old for your current Dobius Mobile app. Update Dobius on your computer, then try this host again.'
  const recoveryNote =
