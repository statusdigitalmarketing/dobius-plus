...
describe('terminal autocomplete preference', () => {
  ...
  it('defaults to disabled when unset', async () => {
    ...
    expect(AsyncStorage.getItem).toHaveBeenCalledWith('dobius:terminalAutocompleteEnabled')
  })
  ...
  it('persists the selected value', async () => {
    ...
    expect(AsyncStorage.setItem).toHaveBeenCalledWith('dobius:terminalAutocompleteEnabled', 'true')

    await saveTerminalAutocompleteEnabled(false)

    expect(AsyncStorage.setItem).toHaveBeenCalledWith('dobius:terminalAutocompleteEnabled', 'false')
  })
})
...
describe('terminal live input disabled handles preference', () => {
  ...
  it('persists disabled handles per host and worktree', async () => {
    ...
    expect(AsyncStorage.setItem).toHaveBeenCalledWith(
      'dobius:terminalLiveInputDisabled:host%2Fone:folder%3AC%3A%5Crepo',
...
describe('terminal link open mode preference', () => {
  ...
  it('defaults to Dobius browser when unset', async () => {
    vi.mocked(AsyncStorage.getItem).mockResolvedValue(null)

    await expect(loadTerminalLinkOpenMode()).resolves.toBe('dobius-browser')
    expect(AsyncStorage.getItem).toHaveBeenCalledWith('dobius:terminalLinkOpenMode')
  })
  ...
  it('falls back to Dobius browser when storage cannot be read', async () => {
    vi.mocked(AsyncStorage.getItem).mockRejectedValue(new Error('storage unavailable'))
  ...
  it('persists the selected mode', async () => {
    ...
    expect(AsyncStorage.setItem).toHaveBeenCalledWith('dobius:terminalLinkOpenMode', 'phone-browser')
  })
})
