...
import Foundation
import ImageIO
import DobiusComputerUseMacOSCore
import ScreenCaptureKit

private let providerName = "dobius-computer-use-macos"
    ...
    private func buildSnapshot(
        ...
        guard accessibilityTrusted() else {
            ...
            throw ProviderError.coded(
                "permission_denied",
                "Accessibility permission is required for Dobius+ Computer Use. Run `dobius computer permissions` or open Settings > Computer Use, grant Accessibility to Dobius+ Computer Use, then retry."
            )
        }
        ...
        let screenshotStatus: ScreenshotStatus = if screenshot != nil {
            ...
            .failed("Screen Recording permission is required for Dobius+ Computer Use; grant permission or pass --no-screenshot to inspect accessibility state only.")
...
private func focusedWindow(appElement: AXUIElement, app: AppDescriptor, visibleWindowCount: Int, allowRecovery: Bool) throws -> AXUIElement {
    ...
    let permissionHint = visibleWindowCount > 0
        ? " The app has visible windows, so macOS Accessibility may need Dobius+ Computer Use toggled off and on again in System Settings."
        : ""
...
private struct WindowCapture {
    ...
    private static func captureImage(windowId: CGWindowID, bounds: CGRect) -> CapturedImage? {
        if ProcessInfo.processInfo.environment["DOBIUS_COMPUTER_USE_SCK_SCREENSHOTS"] == "1",
...
private final class PermissionWindowController: NSWindowController {
    ...
    convenience init(initialPermission: PermissionKind? = nil, terminateWhenDragAssistantCloses: Bool = false) {
        ...
        window.title = "Enable Dobius+ Computer Use"
...
private enum PermissionKind: CaseIterable {
    ...
    var dragInstruction: String {
        switch self {
        case .accessibility:
            "Drag Dobius+ Computer Use into the list above to allow Accessibility."
        case .screenshots:
            "Drag Dobius+ Computer Use into the list above to allow Screenshots."
        }
    }
...
private final class PermissionView: NSView {
    ...
    private func build() {
        ...
        let title = label(ready ? "Computer Use is Ready" : "Enable Dobius+ Computer Use", size: 22, weight: .bold)
        let subtitle = label(
            ready ? "Dobius can use local apps when you ask." : "Grant permissions so Dobius can use apps when you ask.",
            size: 12,
            weight: .regular
        )
    ...
    private func permissionRow(permission: PermissionKind, action: @escaping () -> Void) -> NSView {
        ...
        objc_setAssociatedObject(button, "dobius-action", target, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
private final class PermissionDragAssistantController: NSWindowController {
    ...
    convenience init(
        ...
        window.title = "Drag Dobius+ Computer Use"
...
private final class PermissionDragAssistantView: NSView {
    ...
    private func build() {
        ...
        objc_setAssociatedObject(closeButton, "dobius-action", target, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
private final class DraggableAppTile: NSView, NSDraggingSource {
    ...
    private func build() {
        ...
        let title = NSTextField(labelWithString: "Dobius+ Computer Use")
...
private func isAuthorizedAgentPeer(_ pid: pid_t) -> Bool {
    ...
    if isTrustedDobiusApplication(pid) {
        return true
    }
    guard let parentPid = parentProcessId(pid) else { return false }
    return isTrustedDobiusApplication(parentPid)
}

private func isTrustedDobiusApplication(_ pid: pid_t) -> Bool {
    guard let app = NSRunningApplication(processIdentifier: pid),
    ...
    // Dobius-owned bundle ids; the sidecar peer check must still authorize them.
    return bundleId == "com.statusdigitalmarketing.dobius-plus" ||
        bundleId.hasPrefix("com.statusdigitalmarketing.dobius-plus.dev.") ||
        bundleId == "com.github.Electron"
}
...
private func runAgent(socketPath: String, token: String?) {
    ...
    setenv("DOBIUS_COMPUTER_USE_SCK_SCREENSHOTS", "1", 1)
    app.run()
}
...
private func runStdio() {
    fputs("Dobius+ Computer Use provider must be launched by Dobius in app-agent mode.\n", stderr)
    exit(13)
}
...
if arguments.first == "--agent" {
    guard arguments.count >= 2 else {
        fputs("usage: dobius-computer-use-macos --agent <socket-path> --token-file <token-path>\n", stderr)
        exit(2)
    }
    ...
    guard let token, !token.isEmpty else {
        fputs("dobius-computer-use-macos --agent requires a non-empty --token-file\n", stderr)
        exit(2)
    }
    ...
    guard arguments.count >= 2 else {
        fputs("usage: dobius-computer-use-macos --permission-status-file <path>\n", stderr)
        exit(2)
    }
