...
public static class DobiusDesktopWin32 {
...
function Write-DobiusJson($Payload) {
    $Payload | ConvertTo-Json -Depth 100 -Compress
}

function New-DobiusFrame([double]$X, [double]$Y, [double]$Width, [double]$Height) {
    if ($Width -le 0 -or $Height -le 0) { return $null }
    [pscustomobject]@{ x = $X; y = $Y; width = $Width; height = $Height }
}

function Read-DobiusOperation([string]$Path) {
    Get-Content -Raw -Encoding UTF8 -Path $Path | ConvertFrom-Json
}

function ConvertTo-DobiusLParam([int]$X, [int]$Y) {
    [IntPtr]((($Y -band 0xffff) -shl 16) -bor ($X -band 0xffff))
}

function ConvertTo-DobiusWheelParam([int]$Delta) {
    [IntPtr](($Delta -band 0xffff) -shl 16)
}

function Get-DobiusWindowProcesses {
    @(Get-Process | Where-Object { $_.MainWindowHandle -ne 0 } | Sort-Object ProcessName, Id)
}

function Find-DobiusProcess([string]$Query) {
    $needle = ""
...
function Assert-DobiusProcessAllowed($Process) {
...
function Test-DobiusBrowserProcess($Process) {
...
function Get-DobiusRootElement($Process) {
...
function Get-DobiusWindowFrame($Process, $RootElement) {
    $rect = New-Object DobiusDesktopWin32+RECT
    if ([DobiusDesktopWin32]::GetWindowRect([IntPtr]$Process.MainWindowHandle, [ref]$rect)) {
        return New-DobiusFrame $rect.Left $rect.Top ($rect.Right - $rect.Left) ($rect.Bottom - $rect.Top)
    }

    try {
        $bounds = $RootElement.Current.BoundingRectangle
        if (-not $bounds.IsEmpty) {
            return New-DobiusFrame $bounds.X $bounds.Y $bounds.Width $bounds.Height
        }
    } catch {}
    $null
}

function Get-DobiusWindowId($Process) {
    [int64]$Process.MainWindowHandle
}

function Get-DobiusAppName($Process) {
...
function New-DobiusAppRecord($Process) {
    [pscustomobject]@{
        name = Get-DobiusAppName $Process
...
function Assert-DobiusWindowTarget($Process, $WindowId, $WindowIndex) {
    if ($null -ne $WindowIndex -and [int]$WindowIndex -ne 0) {
        throw "windowNotFound(`"$WindowIndex`")"
    }
    if ($null -ne $WindowId -and [int64]$WindowId -ne (Get-DobiusWindowId $Process)) {
        throw "windowNotFound(`"$WindowId`")"
    }
}

function Restore-DobiusWindow($Process) {
    if ($Process.MainWindowHandle -eq 0) { return }
    [void][DobiusDesktopWin32]::ShowWindow([IntPtr]$Process.MainWindowHandle, 9)
    [void][DobiusDesktopWin32]::SetForegroundWindow([IntPtr]$Process.MainWindowHandle)
}

function Test-DobiusWindowFocused([IntPtr]$WindowHandle) {
    [DobiusDesktopWin32]::GetForegroundWindow() -eq $WindowHandle
}

function Wait-DobiusWindowFocused([IntPtr]$WindowHandle, [int]$TimeoutMilliseconds) {
    $stopwatch = [Diagnostics.Stopwatch]::StartNew()
    while ($stopwatch.ElapsedMilliseconds -lt $TimeoutMilliseconds) {
        if (Test-DobiusWindowFocused $WindowHandle) { return $true }
        Start-Sleep -Milliseconds 50
    }
    Test-DobiusWindowFocused $WindowHandle
}

function Assert-DobiusKeyboardFocus([IntPtr]$WindowHandle, $Operation) {
    if (Test-DobiusWindowFocused $WindowHandle) { return }
    if ([bool]$Operation.restoreWindow) {
        if (Wait-DobiusWindowFocused $WindowHandle 500) { return }
...
function Get-DobiusElementFrame($Element, $WindowFrame) {
    try {
        ...
        New-DobiusFrame ($bounds.X - $WindowFrame.x) ($bounds.Y - $WindowFrame.y) $bounds.Width $bounds.Height
    } catch {
        $null
    }
}

function Get-DobiusProperty($Element, [string]$Name) {
    try { [string]$Element.Current.$Name } catch { "" }
}

function Get-DobiusRuntimeId($Element) {
    try { @($Element.GetRuntimeId()) } catch { @() }
}

function Test-DobiusSensitiveElement($Element) {
    try {
        if ($Element.Current.IsPassword) { return $true }
    } catch {}
    $controlType = try { [string]$Element.Current.ControlType.ProgrammaticName } catch { "" }
    $parts = @(
        (Get-DobiusProperty $Element "LocalizedControlType"),
        $controlType,
        (Get-DobiusProperty $Element "Name"),
        (Get-DobiusProperty $Element "AutomationId"),
        (Get-DobiusProperty $Element "ClassName")
    )
...
function Get-DobiusValueText($Element) {
    try {
        if (Test-DobiusSensitiveElement $Element) { return "[redacted]" }
...
function Get-DobiusActions($Element) {
...
function Get-DobiusMeaningfulActions($Actions) {
    $noisy = @("Invoke", "ScrollToVisible", "ShowMenu")
    @($Actions | Where-Object { $noisy -notcontains $_ })
}

function Format-DobiusSnapshotText([string]$Text) {
    if ([string]::IsNullOrWhiteSpace($Text)) { return "" }
    (($Text -replace "\s+", " ").Trim())
}

function Format-DobiusValueSegment([string]$RoleKey, [string]$Title, [string]$Value) {
    $clean = Format-DobiusSnapshotText $Value
...
function Test-DobiusSuppressChildren([string]$RoleKey, [string]$Title, [string]$Value, [string]$Summary) {
    $hasCompactLabel = -not [string]::IsNullOrWhiteSpace($Title) -or -not [string]::IsNullOrWhiteSpace((Format-DobiusSnapshotText $Value)) -or -not [string]::IsNullOrWhiteSpace((Format-DobiusSnapshotText $Summary))
...
function Get-DobiusTextSnippets($Element, [int]$Limit = 6, [int]$MaxDepth = 3) {
    $values = New-Object System.Collections.Generic.List[string]
    $seen = New-Object System.Collections.Generic.HashSet[string]

    function Visit-DobiusText($Node, [int]$Depth) {
        ...
        if ($role -match "text|link|label") {
            foreach ($raw in @((Get-DobiusProperty $Node "Name"), (Get-DobiusValueText $Node))) {
        ...
        try {
            ...
            for ($i = 0; $i -lt $children.Count; $i++) {
                Visit-DobiusText $children.Item($i) ($Depth + 1)
                if ($values.Count -ge $Limit) { return }
            }
        } catch {}
    }

    Visit-DobiusText $Element 0
    @($values.ToArray())
}

function Test-DobiusPlainTextSubtree($Element, [int]$MaxDepth = 4) {
    $script:sawDobiusText = $false
    $allowed = @("pane", "group", "custom", "unknown", "text", "link", "image")

    function Visit-DobiusPlainText($Node, [int]$Depth) {
        if ($Depth -gt $MaxDepth) { return $false }
        $role = try { [string]$Node.Current.LocalizedControlType } catch { "" }
        $roleKey = $role.ToLowerInvariant()
        if ($allowed -notcontains $roleKey) { return $false }
        if ($roleKey -match "text|link") { $script:sawDobiusText = $true }
        if (@(Get-DobiusMeaningfulActions @(Get-DobiusActions $Node)).Count -gt 0) { return $false }
        try {
            $children = $Node.FindAll([Windows.Automation.TreeScope]::Children, [Windows.Automation.Condition]::TrueCondition)
            for ($i = 0; $i -lt $children.Count; $i++) {
                if (-not (Visit-DobiusPlainText $children.Item($i) ($Depth + 1))) { return $false }
            }
        } catch {}
        return $true
    }

    (Visit-DobiusPlainText $Element 0) -and $script:sawDobiusText
}

function New-DobiusElementRecord($Element, [int]$Index, $WindowFrame) {
    $controlType = try { [string]$Element.Current.ControlType.ProgrammaticName } catch { "" }
    $nativeWindowHandle = try { [int64]$Element.Current.NativeWindowHandle } catch { 0 }
    [pscustomobject]@{
        index = $Index
        runtimeId = @(Get-DobiusRuntimeId $Element)
        automationId = Get-DobiusProperty $Element "AutomationId"
        name = Get-DobiusProperty $Element "Name"
        controlType = $controlType
        localizedControlType = Get-DobiusProperty $Element "LocalizedControlType"
        className = Get-DobiusProperty $Element "ClassName"
        value = Get-DobiusValueText $Element
        isSelected = Test-DobiusElementSelected $Element
        nativeWindowHandle = $nativeWindowHandle
        frame = Get-DobiusElementFrame $Element $WindowFrame
        actions = @(Get-DobiusActions $Element)
    }
}

function Test-DobiusElementSelected($Element) {
    try {
...
function Render-DobiusTree($RootElement, $WindowFrame, [bool]$CompactBrowserTabs = $false) {
    ...
    function Visit-DobiusNode($Node, [int]$Depth) {
        ...
        $record = New-DobiusElementRecord $Node $records.Count $WindowFrame
        $children = @()
        try {
            $children = @($Node.FindAll([Windows.Automation.TreeScope]::Children, [Windows.Automation.Condition]::TrueCondition))
        } catch {}
        $meaningfulActions = @(Get-DobiusMeaningfulActions $record.actions)
        ...
        $roleKey = $role.ToLowerInvariant()
        $snippets = @(Get-DobiusTextSnippets $Node 8 4)
        $genericSummary = $null
        if (($roleKey -in @("pane", "group", "custom", "unknown")) -and [string]::IsNullOrWhiteSpace($title) -and [string]::IsNullOrWhiteSpace($record.value) -and $snippets.Count -ge 2 -and (Test-DobiusPlainTextSubtree $Node)) {
        ...
        if (($roleKey -in @("pane", "group", "custom", "unknown")) -and [string]::IsNullOrWhiteSpace($title) -and [string]::IsNullOrWhiteSpace($record.value) -and $meaningfulActions.Count -eq 0 -and $null -eq $genericSummary -and $children.Count -le 1) {
            for ($i = 0; $i -lt $children.Count; $i++) {
                Visit-DobiusNode $children.Item($i) $Depth
            }
            return
        }
        ...
        $line = "$($record.index) $role $(Format-DobiusSnapshotText $title)".TrimEnd()
        $line += Format-DobiusValueSegment $roleKey $title $record.value
        if (-not [string]::IsNullOrWhiteSpace($genericSummary) -and $genericSummary -ne $title) {
            $line += ", Text: " + (Format-DobiusSnapshotText $genericSummary)
        } elseif ($roleKey -in @("row", "data item", "list item")) {
            $rowSummary = @((Get-DobiusTextSnippets $Node 6 3)) -join " "
            if (-not [string]::IsNullOrWhiteSpace($rowSummary) -and $rowSummary -ne $title) {
                $line += ", Text: " + (Format-DobiusSnapshotText $rowSummary)
            }
        }
        ...
        if (-not [string]::IsNullOrWhiteSpace($genericSummary) -or (Test-DobiusSuppressChildren $roleKey $title $record.value $genericSummary)) { return }
        $childLineStart = $lines.Count
        for ($i = 0; $i -lt $children.Count; $i++) {
            Visit-DobiusNode $children.Item($i) ($Depth + 1)
        }
        if ($CompactBrowserTabs) {
            Compress-DobiusRenderedBrowserTabs $records $lines $childLineStart ($Depth + 1)
        }
    }

    Visit-DobiusNode $RootElement 0
    [pscustomobject]@{ elements = @($records.ToArray()); lines = @($lines.ToArray()); truncation = $truncation }
}

function Compress-DobiusRenderedBrowserTabs($Records, $Lines, [int]$StartLine, [int]$Depth) {
    $tabLineIndexes = New-Object System.Collections.Generic.List[int]
    for ($lineIndex = $StartLine; $lineIndex -lt $Lines.Count; $lineIndex++) {
        if (Test-DobiusDirectRenderedBrowserTabLine ([string]$Lines[$lineIndex]) $Depth) {
    ...
    foreach ($lineIndex in $tabLineIndexes) {
        if (Test-DobiusActiveRenderedBrowserTabLine ([string]$Lines[$lineIndex]) $Depth $recordsByIndex) {
    ...
    for ($i = $tabLineIndexes.Count - 1; $i -ge 0; $i--) {
        ...
        $recordIndex = Get-DobiusRenderedElementIndex ([string]$Lines[$lineIndex]) $Depth
...
function Test-DobiusDirectRenderedBrowserTabLine([string]$Line, [int]$Depth) {
    $indent = "`t" * $Depth
...
function Test-DobiusActiveRenderedBrowserTabLine([string]$Line, [int]$Depth, $RecordsByIndex) {
    if ($Line.Contains("(selected")) { return $true }
    $recordIndex = Get-DobiusRenderedElementIndex $Line $Depth
    ...
    [bool]$record.isSelected -or (Format-DobiusSnapshotText $record.value) -eq "1"
}

function Get-DobiusRenderedElementIndex([string]$Line, [int]$Depth) {
    $text = $Line.Substring(("`t" * $Depth).Length)
    if ($text -match "^(\d+)") { return [int]$Matches[1] }
    $null
}

function ConvertTo-DobiusPngBytes([System.Drawing.Image]$Image) {
    $stream = $null
    try {
...
function New-DobiusScreenshotPayload([byte[]]$Bytes, [int]$Width, [int]$Height, [double]$Scale) {
    [pscustomobject]@{
...
function Resize-DobiusBitmap([System.Drawing.Bitmap]$Source, [int]$Width, [int]$Height) {
    $resized = $null
    $graphics = $null
    try {
...
function Get-DobiusBoundedScreenshotPayload([System.Drawing.Bitmap]$Bitmap) {
    $originalWidth = [int][Math]::Max(1, $Bitmap.Width)
    $originalHeight = [int][Math]::Max(1, $Bitmap.Height)
    $pngBytes = ConvertTo-DobiusPngBytes $Bitmap
    if ($pngBytes.Length -le $MaxScreenshotPngBytes) {
        return New-DobiusScreenshotPayload $pngBytes $originalWidth $originalHeight 1.0
    }
    ...
    while ($scale -ge $MinScreenshotScale) {
        ...
        $resized = $null
        try {
            $resized = Resize-DobiusBitmap $Bitmap $width $height
            $candidateBytes = ConvertTo-DobiusPngBytes $resized
            if ($candidateBytes.Length -le $MaxScreenshotPngBytes) {
                return New-DobiusScreenshotPayload $candidateBytes $width $height ($width / [double]$originalWidth)
            }
        } finally {
            if ($null -ne $resized) { $resized.Dispose() }
        }
...
function Get-DobiusScreenshot([bool]$IncludeScreenshot, $WindowFrame) {
    ...
    try {
        ...
        Get-DobiusBoundedScreenshotPayload $bitmap
    } catch {
        $null
    } finally {
        if ($null -ne $graphics) { $graphics.Dispose() }
...
function New-DobiusSnapshot([string]$Query, [bool]$IncludeScreenshot, $WindowId = $null, $WindowIndex = $null, [bool]$RestoreWindow = $false) {
    $process = Find-DobiusProcess $Query
    if ($RestoreWindow) { Restore-DobiusWindow $process }
    Assert-DobiusWindowTarget $process $WindowId $WindowIndex
    $root = Get-DobiusRootElement $process
    $windowFrame = Get-DobiusWindowFrame $process $root
    $tree = Render-DobiusTree $root $windowFrame (Test-DobiusBrowserProcess $process)
    $screenshot = Get-DobiusScreenshot $IncludeScreenshot $windowFrame

    [pscustomobject]@{
        snapshotId = [guid]::NewGuid().ToString()
        app = New-DobiusAppRecord $process
        windowTitle = $process.MainWindowTitle
        windowId = Get-DobiusWindowId $process
...
function Get-DobiusAppList {
    @(Get-DobiusWindowProcesses | ForEach-Object {
        New-DobiusAppRecord $_
    })
}

function Get-DobiusWindowList([string]$Query) {
    $process = Find-DobiusProcess $Query
    $root = Get-DobiusRootElement $process
    $windowFrame = Get-DobiusWindowFrame $process $root
    $x = $null
    ...
    [pscustomobject]@{
        ...
        windows = @([pscustomobject]@{
            ...
            isOffscreen = $false
            screenIndex = $null
            platform = [pscustomobject]@{ backend = "uia"; nativeWindowHandle = Get-DobiusWindowId $process }
        })
    }
}

function Get-DobiusHandshake {
    [pscustomobject]@{
        platform = "win32"
        provider = "dobius-computer-use-windows"
...
function Test-DobiusSameRuntimeId($Left, $Right) {
...
function Find-DobiusElement($RootElement, $Record) {
    ...
    try {
        ...
        for ($i = 0; $i -lt $descendants.Count; $i++) {
            ...
            if (Test-DobiusSameRuntimeId @($candidate.GetRuntimeId()) @($Record.runtimeId)) {
                return $candidate
            }
        }
    } catch {}
...
function Invoke-DobiusPrimaryAction($Element) {
    foreach ($pattern in @(
...
function Invoke-DobiusNamedAction($Element, [string]$Action) {
    $wanted = ""
...
function Set-DobiusElementValue($Element, [string]$Value) {
    try {
        $pattern = $Element.GetCurrentPattern([Windows.Automation.ValuePattern]::Pattern)
...
function Get-DobiusRequiredNumber($Value, [string]$Name) {
...
function Get-DobiusPositiveInteger($Value, [string]$Name) {
...
function Get-DobiusPositiveNumber($Value, [string]$Name) {
    if ($null -eq $Value) { $Value = 1 }
    $number = Get-DobiusRequiredNumber $Value $Name
    if ($number -le 0) { throw "$Name must be a positive number" }
    $number
}

function Get-DobiusRequiredString($Value, [string]$Name) {
    if ($null -eq $Value) { throw "$Name is required" }
    $text = [string]$Value
...
function Get-DobiusScreenPoint($Operation, $WindowFrame) {
    if ($null -ne $Operation.element) {
        throw "stale element frame; run get-app-state again and use a fresh element index"
    }
    $x = Get-DobiusRequiredNumber $Operation.x "x"
    $y = Get-DobiusRequiredNumber $Operation.y "y"
    @{
...
function Get-DobiusElementScreenPoint($Element) {
...
function Send-DobiusMouseClick([IntPtr]$WindowHandle, [int]$ScreenX, [int]$ScreenY, [string]$Button, [int]$Count) {
    [void][DobiusDesktopWin32]::SetForegroundWindow($WindowHandle)
    [void][DobiusDesktopWin32]::SetCursorPos($ScreenX, $ScreenY)
    ...
    for ($i = 0; $i -lt (Get-DobiusPositiveInteger $Count "click_count"); $i++) {
        [DobiusDesktopWin32]::mouse_event($down, 0, 0, 0, [UIntPtr]::Zero)
        Start-Sleep -Milliseconds 35
        [DobiusDesktopWin32]::mouse_event($up, 0, 0, 0, [UIntPtr]::Zero)
    }
}

function Send-DobiusDrag([IntPtr]$WindowHandle, $From, $To) {
    [void][DobiusDesktopWin32]::SetForegroundWindow($WindowHandle)
    $startX = [int]$From.x
    $startY = [int]$From.y
    $endX = [int]$To.x
    $endY = [int]$To.y
    [void][DobiusDesktopWin32]::SetCursorPos($startX, $startY)
    [DobiusDesktopWin32]::mouse_event($MouseEvents.LeftDown, 0, 0, 0, [UIntPtr]::Zero)
    for ($step = 1; $step -le 12; $step++) {
        $x = [int][Math]::Round($startX + (($endX - $startX) * $step / 12))
        $y = [int][Math]::Round($startY + (($endY - $startY) * $step / 12))
        [void][DobiusDesktopWin32]::SetCursorPos($x, $y)
        Start-Sleep -Milliseconds 20
    }
    [DobiusDesktopWin32]::mouse_event($MouseEvents.LeftUp, 0, 0, 0, [UIntPtr]::Zero)
}

function Send-DobiusText([IntPtr]$WindowHandle, [string]$Text) {
    [void][DobiusDesktopWin32]::SetForegroundWindow($WindowHandle)
    $hasNonAscii = $false
    ...
    if ($hasNonAscii) {
        foreach ($character in $Text.ToCharArray()) {
            [void][DobiusDesktopWin32]::PostMessage($WindowHandle, $WindowsMessages.Char, [IntPtr][int][char]$character, [IntPtr]::Zero)
            Start-Sleep -Milliseconds 8
        }
        return
    }
    [System.Windows.Forms.SendKeys]::SendWait((ConvertTo-DobiusSendKeysText $Text))
}

function Get-DobiusVirtualKey([string]$Key) {
...
function Send-DobiusKey([IntPtr]$WindowHandle, [string]$Key) {
    [void][DobiusDesktopWin32]::SetForegroundWindow($WindowHandle)
    [System.Windows.Forms.SendKeys]::SendWait((ConvertTo-DobiusSendKeysKey $Key))
}

function Get-DobiusModifierVirtualKey([string]$Modifier) {
...
function Send-DobiusHotkey([IntPtr]$WindowHandle, [string]$KeySpec) {
    ...
    if ($parts.Count -gt 1) {
        foreach ($modifier in $parts[0..($parts.Count - 2)]) {
            $prefix += ConvertTo-DobiusSendKeysModifier $modifier
        }
    }
    [void][DobiusDesktopWin32]::SetForegroundWindow($WindowHandle)
    [System.Windows.Forms.SendKeys]::SendWait($prefix + (ConvertTo-DobiusSendKeysKey $key))
}

function ConvertTo-DobiusSendKeysText([string]$Text) {
...
function ConvertTo-DobiusSendKeysKey([string]$Key) {
    switch ($Key.ToLowerInvariant()) {
        ...
        default {
            if ($Key.Length -eq 1) { return (ConvertTo-DobiusSendKeysText $Key) }
            throw "Unsupported key: $Key"
        }
    }
}

function ConvertTo-DobiusSendKeysModifier([string]$Modifier) {
    switch ($Modifier.ToLowerInvariant()) {
...
function Send-DobiusPasteText([IntPtr]$WindowHandle, [string]$Text) {
    $previous = $null
    $hadPrevious = $false
    ...
    try {
        ...
        Send-DobiusHotkey $WindowHandle "Ctrl+v"
    } finally {
        if ($hadPrevious) {
...
function Invoke-DobiusOperation($Operation) {
    $includeScreenshot = -not [bool]$Operation.noScreenshot
    if ($Operation.tool -eq "handshake") {
        return [pscustomobject]@{ ok = $true; capabilities = Get-DobiusHandshake }
    }
    if ($Operation.tool -eq "list_apps") {
        return [pscustomobject]@{ ok = $true; apps = @(Get-DobiusAppList) }
    }
    if ($Operation.tool -eq "list_windows") {
        $list = Get-DobiusWindowList $Operation.app
        return [pscustomobject]@{ ok = $true; app = $list.app; windows = @($list.windows) }
    }
    if ($Operation.tool -eq "get_app_state") {
        return [pscustomobject]@{ ok = $true; snapshot = New-DobiusSnapshot $Operation.app $includeScreenshot $Operation.windowId $Operation.windowIndex ([bool]$Operation.restoreWindow) }
    }

    $process = Find-DobiusProcess $Operation.app
    if ([bool]$Operation.restoreWindow) { Restore-DobiusWindow $process }
    Assert-DobiusWindowTarget $process $Operation.windowId $Operation.windowIndex
    $root = Get-DobiusRootElement $process
    $windowFrame = if ($null -ne $Operation.windowBounds) { $Operation.windowBounds } else { Get-DobiusWindowFrame $process $root }
    $element = Find-DobiusElement $root $Operation.element
    $fromElement = Find-DobiusElement $root $Operation.fromElement
    $toElement = Find-DobiusElement $root $Operation.toElement
    $handle = [IntPtr]$process.MainWindowHandle
    if ($Operation.tool -in @("type_text", "press_key", "hotkey", "paste_text")) {
        Assert-DobiusKeyboardFocus $handle $Operation
    }
    $action = $null
    ...
    switch ($Operation.tool) {
        "click" {
            ...
            Restore-DobiusWindow $process
            $handledByPattern = $false
            $clickCount = Get-DobiusPositiveInteger $Operation.click_count "click_count"
            if ($null -ne $element -and $Operation.mouse_button -ne "right" -and $Operation.mouse_button -ne "middle" -and $clickCount -le 1) {
                $handledByPattern = Invoke-DobiusPrimaryAction $element
            }
            if (-not $handledByPattern) {
                $point = Get-DobiusElementScreenPoint $element
                if ($null -eq $point) { $point = Get-DobiusScreenPoint $Operation $windowFrame }
                Send-DobiusMouseClick $handle $point.x $point.y $Operation.mouse_button $clickCount
        ...
        "perform_secondary_action" {
            ...
            if (-not (Invoke-DobiusNamedAction $element $Operation.action)) {
        ...
        "scroll" {
            $delta = 120 * [int][Math]::Ceiling((Get-DobiusPositiveNumber $Operation.pages "pages"))
            ...
            $point = Get-DobiusElementScreenPoint $element
            if ($null -eq $point) { $point = Get-DobiusScreenPoint $Operation $windowFrame }
            [void][DobiusDesktopWin32]::SetForegroundWindow($handle)
            [void][DobiusDesktopWin32]::SetCursorPos([int]$point.x, [int]$point.y)
            [DobiusDesktopWin32]::mouse_event($MouseEvents.Wheel, 0, 0, $delta, [UIntPtr]::Zero)
            $action = [pscustomobject]@{ path = "synthetic"; actionName = "scroll"; fallbackReason = $null }
        }
        "drag" {
            $from = Get-DobiusElementScreenPoint $fromElement
            ...
            if ($null -eq $from) {
                $from = @{
                    x = $windowFrame.x + (Get-DobiusRequiredNumber $Operation.from_x "from_x")
                    y = $windowFrame.y + (Get-DobiusRequiredNumber $Operation.from_y "from_y")
                }
            }
            $to = Get-DobiusElementScreenPoint $toElement
            ...
            if ($null -eq $to) {
                $to = @{
                    x = $windowFrame.x + (Get-DobiusRequiredNumber $Operation.to_x "to_x")
                    y = $windowFrame.y + (Get-DobiusRequiredNumber $Operation.to_y "to_y")
                }
            }
            Send-DobiusDrag $handle $from $to
            $action = [pscustomobject]@{ path = "synthetic"; actionName = "drag"; fallbackReason = $null }
        }
        "type_text" {
            Send-DobiusText $handle (Get-DobiusRequiredString $Operation.text "text")
        ...
        "press_key" {
            Send-DobiusKey $handle (Get-DobiusRequiredString $Operation.key "key")
        ...
        "hotkey" {
            Send-DobiusHotkey $handle (Get-DobiusRequiredString $Operation.key "key")
        ...
        "paste_text" {
            Send-DobiusPasteText $handle (Get-DobiusRequiredString $Operation.text "text")
        ...
        "set_value" {
            if ($null -eq $element -or -not (Set-DobiusElementValue $element ([string]$Operation.value))) {
    ...
    try {
        $snapshot = New-DobiusSnapshot $Operation.app $includeScreenshot $Operation.windowId $Operation.windowIndex
    } catch {
        ...
        $snapshot = New-DobiusSnapshot $Operation.app $includeScreenshot $null $null
    }
    [pscustomobject]@{ ok = $true; action = $action; snapshot = $snapshot }
}

try {
    $operation = Read-DobiusOperation $OperationPath
    Write-DobiusJson (Invoke-DobiusOperation $operation)
} catch {
    Write-DobiusJson ([pscustomobject]@{ ok = $false; error = [string]$_.Exception.Message })
}
