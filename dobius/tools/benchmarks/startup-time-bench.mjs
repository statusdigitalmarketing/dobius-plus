 ...
 * parses `DOBIUS_STARTUP_DIAGNOSTICS=1` milestone lines from stderr, and
 ...
 *     [--exe <path-to-packaged-Dobius>] [--timeout-ms 240000]
 *
 ...
 * tiny. Layout mirrors Chromium cache dirs plus a few Dobius-owned dirs.
 */
  ...
  for (let i = 0; i < githubRepos; i++) {
    ...
    const remote = spawnSync(
      ...
      [
        ...
        'add',
        'origin',
        `https://github.com/dobius-bench/bench-gh-repo-${i}.git`
      ],
      { stdio: 'ignore' }
    )
...
function writePersistedStateFixture(fixtureDir, { stateProfile, sessionTabs, githubRepos }) {
  const dataPath = join(fixtureDir, 'dobius-data.json')
...
function buildLaunchEnvironment({ fixtureDir, githubRepos, ghShimDir }) {
  const env = {
    ...process.env,
    DOBIUS_STARTUP_DIAGNOSTICS: '1',
    DOBIUS_E2E_USER_DATA_DIR: fixtureDir,
    DOBIUS_E2E_HEADLESS: '1'
  }
  if (ghShimDir) {
