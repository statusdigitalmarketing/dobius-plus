...
function createLocalRepoFixture() {
  const baseDir = mkdtempSync(path.join(os.tmpdir(), 'dobius-termperf-'))
  const repoPath = path.join(baseDir, 'repo')
  mkdirSync(repoPath, { recursive: true })
  git(repoPath, 'init', '--initial-branch=main')
  git(repoPath, 'config', 'user.email', 'bench@dobius.local')
  git(repoPath, 'config', 'user.name', 'Dobius Bench')
...
async function setupWorkspaces(page, fixture) {
  return await runWithTimeout(
    'fixture registration in Dobius',
    () =>
      page.evaluate(
...
async function main() {
  ...
  try {
    ...
    if (launched) {
      ...
      // Main-process phase attribution (requires DOBIUS_PTY_SPAWN_TIMING=1 in the
