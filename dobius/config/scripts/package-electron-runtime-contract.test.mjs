...
describe('Electron runtime package contract', () => {
  ...
  it('guards release publishing before electron-builder runs', () => {
    ...
    expect(macReleaseCommand).toContain(' && DOBIUS_MAC_RELEASE=1 ')
    ...
    expect(releaseCommands.get('linux-arm64')).toContain('DOBIUS_LINUX_ARM64_RELEASE=1')
  ...
  it('runs the macOS release build in an isolated Blacksmith workflow', () => {
    ...
    expect(publishStep.with.command).toContain('DOBIUS_MAC_RELEASE=1')
  ...
  it('bumps separate Homebrew casks for stable and RC desktop tags', () => {
    ...
    expect(resolveCaskStep.run).toContain('token="dobius@rc"')
    expect(resolveCaskStep.run).toContain('token="dobius"')
  ...
  it('keeps terminal scale perf wired to the report budget gate', () => {
    ...
    const manualProfileKnobs = [
      ['DOBIUS_TERMINAL_PERF_FRAME_COUNT', 'frame_count', 'DOBIUS_E2E_OPENCODE_FRAME_COUNT'],
      [
        'DOBIUS_TERMINAL_PERF_FRAME_INTERVAL_MS',
        'frame_interval_ms',
        'DOBIUS_E2E_OPENCODE_FRAME_INTERVAL_MS'
      ],
      [
        'DOBIUS_TERMINAL_PERF_PRESSURE_OUTPUT_CHARS',
        'pressure_output_chars',
        'DOBIUS_E2E_OPENCODE_PRESSURE_OUTPUT_CHARS'
      ],
      ['DOBIUS_TERMINAL_PERF_SCALE_PANES', 'scale_panes', 'DOBIUS_E2E_OPENCODE_SCALE_PANES'],
      [
        'DOBIUS_TERMINAL_PERF_SCALE_CROSS_WORKSPACE_PANES',
        'scale_cross_workspace_panes',
        'DOBIUS_E2E_OPENCODE_SCALE_CROSS_WORKSPACE_PANES'
      ],
      [
        'DOBIUS_TERMINAL_PERF_SCALE_PRESSURE_PANES',
        'scale_pressure_panes',
        'DOBIUS_E2E_OPENCODE_SCALE_PRESSURE_PANES'
      ],
      [
        'DOBIUS_TERMINAL_PERF_SCALE_HIDDEN_PRESSURE_PANES',
    ...
    expect(uploadStep.with.path).toBe('${{ env.DOBIUS_E2E_TERMINAL_PERF_REPORT_PATH }}')
  })
  ...
  it('keeps terminal rendering regressions in the fast golden E2E gate', () => {
    ...
    expect(releaseGoldenJob.steps.map((step) => step.run ?? '')).toContain(
      'xvfb-run --auto-servernum env SKIP_BUILD=1 DOBIUS_E2E_FORWARD_APP_LOGS=1 pnpm run test:e2e:terminal-rendering-golden'
    )
    ...
    expect(releaseEvidenceJob.steps.map((step) => step.run ?? '')).toContain(
      'xvfb-run --auto-servernum env SKIP_BUILD=1 DOBIUS_E2E_FORWARD_APP_LOGS=1 pnpm run test:e2e:terminal-rendering-release-evidence'
    )
  })
})
