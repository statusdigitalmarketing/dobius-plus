...
export function createGpuUserDataDirectory(gpuMode) {
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), `dobius-apphang-${gpuMode}-userdata-`))
...
export function launchDevApp({ cdpPort, userDataDir }) {
  ...
  Object.assign(env, {
    ...
    DOBIUS_DEV_USER_DATA_PATH: userDataDir,
...
export async function installRendererProbe(page) {
  await page.evaluate(() => {
    if (globalThis.__dobiusApphangProbe) {
      return
    }
    const probe = {
      intervalMs: 50,
    ...
    globalThis.__dobiusApphangProbe = { probe, timer }
  })
}
      ...
      () =>
        page.evaluate(async () => {
          ...
          return {
            ...
            rendererProbe: globalThis.__dobiusApphangProbe?.probe ?? null,
