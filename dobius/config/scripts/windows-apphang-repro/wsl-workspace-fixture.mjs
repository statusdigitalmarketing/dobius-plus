...
set -euo pipefail
base="$(mktemp -d /tmp/dobius-apphang-repro.XXXXXX)"
repo="$base/repo"
mkdir -p "$repo"
cd "$repo"
git init -q
...
mkdir -p src docs
printf '# Dobius Windows AppHang repro\n' > README.md
...
printf 'plain folder for Dobius AppHang repro\n' > "$plain/README.txt"
...
export function createCompletedOnboardingProfile(userDataDir) {
  ...
  writeFileSync(path.join(userDataDir, 'dobius-data.json'), `${JSON.stringify(profile, null, 2)}\n`)
}
