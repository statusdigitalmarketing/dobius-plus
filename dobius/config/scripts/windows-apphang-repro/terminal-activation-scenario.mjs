...
async function readRendererProbe(page) {
  return await runWithTimeout(
    'renderer probe read',
    () => page.evaluate(() => globalThis.__dobiusApphangProbe?.probe ?? null),
    rendererActionTimeoutMs
  )
}
...
async function resetRendererProbe(page) {
  ...
  await runWithTimeout(
    ...
    () =>
      page.evaluate(() => {
        const probe = globalThis.__dobiusApphangProbe?.probe
        if (!probe) {
          return
        }
...
async function runActivationCycle(page, target, args) {
  ...
  const marker = `DOBIUS_APPHANG_${target.index}_${Date.now()}`
