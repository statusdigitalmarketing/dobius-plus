...
describe('rebuild-native-deps Electron install fallback', () => {
  it('continues non-strict postinstall when Electron retry download fails', () => {
    ...
    try {
      ...
      const result = runRebuildScript(projectDir, {
        ...
        DOBIUS_STRICT_ELECTRON_INSTALL: ''
      })
  ...
  it('fails strict postinstall when Electron retry download fails', () => {
    ...
    try {
      ...
      const result = runRebuildScript(projectDir, {
        npm_lifecycle_event: 'postinstall',
        DOBIUS_STRICT_ELECTRON_INSTALL: '1'
      })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      try {
        ...
        writeFakeElectronRebuild(projectDir, { logPathEnv: 'DOBIUS_REBUILD_TEST_LOG' })
        writeFakeLoadableNodePty(projectDir)
        writeNodePtyPatchFile(projectDir)

        const result = runRebuildScript(projectDir, {
          DOBIUS_REBUILD_TEST_LOG: rebuildLogPath
        })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      try {
        ...
        expect(result.stdout).toContain("expected build/Release so Dobius's node-pty patch is active")
...
function mkTempProject() {
  const projectDir = mkdtempSync(join(tmpdir(), 'dobius-rebuild-native-deps-'))
...
function runRebuildScript(projectDir, extraEnv = {}) {
  ...
  for (const key of Object.keys(env)) {
    if (
      key.toLowerCase() === 'dobius_strict_electron_install' ||
