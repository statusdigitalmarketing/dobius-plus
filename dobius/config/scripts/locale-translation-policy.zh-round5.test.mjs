...
describe('locale-translation-policy zh round 5', () => {
  it('fixes brand spacing, hosted review, and Dobius Mobile regressions', () => {
    expect(
      repairTranslatedValue({
        key: 'auto.components.stats.ShareUsageCard.0eb31e79ee',
        enValue: 'Dobius IDE',
        localeValue: 'Dobius集成开发环境',
        locale: 'zh'
      })
    ).toBe('Dobius IDE')
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Dobius first',
        localeValue: 'Dobius第一',
        locale: 'zh'
      })
    ).toBe('Dobius 优先')
    expect(
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Show Dobius Mobile Button',
        localeValue: '显示 Dobius 移动按钮',
        locale: 'zh'
      })
    ).toBe('显示 Dobius Mobile 按钮')
    expect(
      repairTranslatedValue({
        key: 'auto.components.settings.GitPane.e02ea23a32',
        enValue: 'Dobius Attribution',
        localeValue: 'Dobius归属',
        locale: 'zh'
      })
    ).toBe('Dobius 归因')
    expect(
  ...
  it('keeps workflow terms in English', () => {
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue:
          'Git could not safely delete branch "{{value0}}"{{value1}}, so Dobius kept it to avoid losing local commits.',
        localeValue:
          'Git 无法安全删除分支“{{value0}}”{{value1}}，因此 Dobius 保留它以避免丢失本地提交。',
        locale: 'zh'
      })
    ).toBe('Git 无法安全删除分支“{{value0}}”{{value1}}，因此 Dobius 保留它以避免丢失本地 commits。')
  })
