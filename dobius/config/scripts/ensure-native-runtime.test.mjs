    ...
    try {
      ...
      const result = spawnSync(process.execPath, [scriptPath, '--runtime=node'], {
        ...
        env: envWithPrependedPath(binDir, {
          DOBIUS_NATIVE_TEST_LOG: logPath,
          DOBIUS_NATIVE_TEST_MARKER: markerPath
        })
      })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      try {
        ...
        expect(result.status, result.stderr).toBe(0)
        expect(result.stderr).toContain("expected build/Release so Dobius's node-pty patch is active")
        expect(readFileSync(logPath, 'utf8')).toContain('pnpm rebuild node-pty\n')
      } finally {
        rmSync(projectDir, { recursive: true, force: true })
      }
...
function mkTempProject() {
  const projectDir = mkdtempSync(join(tmpdir(), 'dobius-native-runtime-'))
...
exports.loadNativeModule = function loadNativeModule(nativeName) {
  const markerExists = existsSync(process.env.DOBIUS_NATIVE_TEST_MARKER)
  appendFileSync(
    process.env.DOBIUS_NATIVE_TEST_LOG,
...
exports.loadNativeModule = function loadNativeModule(nativeName) {
  const rebuilt = existsSync(process.env.DOBIUS_NATIVE_TEST_MARKER)
  const dir = rebuilt ? '../build/Release/' : '../prebuilds/' + process.platform + '-' + process.arch + '/'
  appendFileSync(process.env.DOBIUS_NATIVE_TEST_LOG, \`node-pty load \${nativeName} dir=\${dir}\\n\`)
  return { dir, module: {} }
}
`
  )
...
appendFileSync(process.env.DOBIUS_NATIVE_TEST_LOG, \`pnpm \${process.argv.slice(2).join(' ')}\\n\`)
writeFileSync(process.env.DOBIUS_NATIVE_TEST_MARKER, 'rebuilt')
`
  )
