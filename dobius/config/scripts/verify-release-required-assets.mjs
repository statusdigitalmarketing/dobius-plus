...
export function getRequiredReleaseAssetNames(tag) {
  ...
  return [
    ...
    `dobius-ide_${version}_amd64.deb`,
    `dobius-ide_${version}_arm64.deb`,
    `dobius-ide-${version}.x86_64.rpm`,
    `dobius-ide-${version}.aarch64.rpm`,
    'dobius-windows-setup.exe',
    'dobius-windows-setup.exe.blockmap',
    `Dobius-${version}-mac.zip`,
    `Dobius-${version}-mac.zip.blockmap`,
    `Dobius-${version}-arm64-mac.zip`,
    `Dobius-${version}-arm64-mac.zip.blockmap`,
    'dobius-macos-x64.dmg',
    'dobius-macos-x64.dmg.blockmap',
    'dobius-macos-arm64.dmg',
    'dobius-macos-arm64.dmg.blockmap'
  ]
}
...
async function main() {
  ...
  const repo = process.env.GITHUB_REPOSITORY || 'statusdigitalmarketing/dobius-plus'
