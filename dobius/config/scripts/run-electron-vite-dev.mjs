...
const useStableElectronName =
  process.env.DOBIUS_DEV_STABLE_NAME === '1' || rawForwardedArgs.includes(STABLE_NAME_FLAG)
const forwardedRaw = rawForwardedArgs.filter((arg) => arg !== STABLE_NAME_FLAG)
if (useStableElectronName) {
  process.env.DOBIUS_DEV_STABLE_NAME = '1'
}
...
function createDockTitle(branch, label) {
  return `Dobius: ${branch || label || 'dev'}`
}

function seedDevInstanceIdentityEnv() {
  const branch =
    process.env.DOBIUS_DEV_BRANCH ||
    readGitValue(['symbolic-ref', '--quiet', '--short', 'HEAD']) ||
    readGitValue(['rev-parse', '--short', 'HEAD'])
  const worktreeName = process.env.DOBIUS_DEV_WORKTREE_NAME || path.basename(repoRoot)
  const label = process.env.DOBIUS_DEV_INSTANCE_LABEL || formatDevInstanceLabel(branch, worktreeName)
  const identitySeed = process.env.DOBIUS_DEV_INSTANCE_KEY || repoRoot
  const dockTitle = process.env.DOBIUS_DEV_DOCK_TITLE || createDockTitle(branch, label)

  process.env.DOBIUS_DEV_REPO_ROOT ||= repoRoot
  process.env.DOBIUS_DEV_INSTANCE_KEY ||= identitySeed
  if (branch) {
    process.env.DOBIUS_DEV_BRANCH ||= branch
  }
  if (worktreeName) {
    process.env.DOBIUS_DEV_WORKTREE_NAME ||= worktreeName
  }
  if (label) {
    ...
    process.env.DOBIUS_DEV_INSTANCE_LABEL ||= label
  }
  process.env.DOBIUS_DEV_DOCK_TITLE ||= dockTitle
}
...
function prepareMacDevElectronApp() {
  ...
  const title = process.env.DOBIUS_DEV_DOCK_TITLE || 'Dobius: dev'
  const identityKey = process.env.DOBIUS_DEV_INSTANCE_KEY || repoRoot
  ...
  const markerPath = path.join(distDir, 'dobius-dev-electron-app.json')
  const bundleId = `com.statusdigitalmarketing.dobius-plus.dev.${sanitizeBundleIdPart(hash)}`
  process.env.DOBIUS_DEV_MACOS_BUNDLE_ID = bundleId
...
function getDevUserDataPath() {
  if (process.env.DOBIUS_DEV_USER_DATA_PATH) {
    return process.env.DOBIUS_DEV_USER_DATA_PATH
  }
  if (process.platform === 'darwin') {
    return path.join(process.env.HOME ?? '', 'Library', 'Application Support', 'dobius-dev')
  }
...
function prepareDevCliWrapper() {
  ...
  if (process.platform === 'win32') {
    writeFileSync(
      path.join(binDir, 'dobius-dev.cmd'),
      `@echo off\r\nset "DOBIUS_USER_DATA_PATH=${userDataPath}"\r\nset "DOBIUS_APP_EXECUTABLE=${electronBin}"\r\nset "DOBIUS_APP_EXECUTABLE_NEEDS_APP_ROOT=1"\r\nnode "${cliPath}" %*\r\n`,
      'utf8'
    )
  } else {
    const wrapperContent = `#!/usr/bin/env bash\nexport DOBIUS_USER_DATA_PATH=${JSON.stringify(userDataPath)}\nexport DOBIUS_APP_EXECUTABLE=${JSON.stringify(electronBin)}\nexport DOBIUS_APP_EXECUTABLE_NEEDS_APP_ROOT=1\nexec node ${JSON.stringify(cliPath)} "$@"\n`
    const wrapperPath = path.join(binDir, 'dobius-dev')
    writeFileSync(wrapperPath, wrapperContent, 'utf8')
    chmodSync(wrapperPath, 0o755)

    mkdirSync(userDataBinDir, { recursive: true })
    for (const commandName of ['dobius-dev', 'dobius']) {
      const userDataWrapperPath = path.join(userDataBinDir, commandName)
      // Why: dev Dobius terminals prepend this directory to PATH; refreshing the
      // `dobius` alias prevents stale global/userData wrappers from hijacking
      // Dobius-owned commands such as `dobius claude-teams`.
  ...
  console.log(`[dobius-dev] Prepared wrapper in ${binDir}`)
}
...
if (process.env.DOBIUS_SKIP_DEV_CLI_PREPARE !== '1') {
  prepareDevCliWrapper()
}

seedDevInstanceIdentityEnv()
if (!useStableElectronName && process.env.DOBIUS_SKIP_DEV_ELECTRON_APP_PREPARE !== '1') {
...
function prepareDevWebClient() {
  if (process.env.DOBIUS_SKIP_DEV_WEB_PREPARE === '1' || isHelpOrVersion) {
    return
  }
  ...
  if (!existsSync(getDevWebClientIndexPath()) && process.env.DOBIUS_DEV_WEB_PREPARE !== '1') {
    console.error(
      '[dobius-dev] Web client bundle missing; skipping pairing web build. Run `pnpm run build:web` or set DOBIUS_DEV_WEB_PREPARE=1 when you need browser pairing.'
    )
    return
  }
  ...
  console.error('[dobius-dev] Building web client for pairing...')
  execFileSync(
    process.execPath,
...
if (!isHelpOrVersion && process.env.DOBIUS_DEV_INSTANCE_LABEL) {
  console.error(`[dobius-dev] Instance: ${process.env.DOBIUS_DEV_INSTANCE_LABEL}`)
}
let forwardedExtras = []
if (!userPassedPort && !isHelpOrVersion) {
  ...
  if (envPortRaw) {
    ...
    if (port === null) {
      console.error(
        `[dobius-dev] Ignoring invalid REMOTE_DEBUGGING_PORT=${JSON.stringify(envPortRaw)}; falling back to probe.`
      )
    }
  }
  if (port === null) {
  ...
  if (port !== null) {
    ...
    console.error(`[dobius-dev] Remote debugging on http://127.0.0.1:${port}`)
  } else {
    console.error(
      '[dobius-dev] No free debug port found in sweep; starting without --remote-debugging-port.'
    )
  }
}
prepareDevWebClient()
