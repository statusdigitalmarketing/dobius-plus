...
$signature = Get-AuthenticodeSignature -FilePath $env:DOBIUS_WINDOWS_INNER_EXECUTABLE
...
export function parseExpectedSigners(value = process.env.DOBIUS_WINDOWS_EXPECTED_SIGNERS) {
...
export function parseExpectedThumbprints(value = process.env.DOBIUS_WINDOWS_EXPECTED_THUMBPRINTS) {
...
export function validateExecutablePath(executablePath) {
  if (typeof executablePath !== 'string' || executablePath.trim() === '') {
    throw new Error('Usage: node config/scripts/verify-windows-inner-signature.mjs <Dobius.exe>')
  }
...
export function getPowerShellSignatureJson(executablePath, spawnSyncImpl = spawnSync) {
  ...
  const result = spawnSyncImpl(
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        DOBIUS_WINDOWS_INNER_EXECUTABLE: executablePath
      }
    }
  )
