...
function loadNodePtyNativeModule() {
  ...
  if (requiresPatchedNodePtySourceBuild() && !isNodePtyReleaseBuildDir(native.dir)) {
    throw new Error(
      `node-pty resolved to ${native.dir}; expected build/Release so Dobius's node-pty patch is active`
    )
  }
}
...
function getPatchedNodePtyRebuildReason() {
  ...
  // Why: a loadable upstream node-pty prebuild is not enough; Dobius's Unix
