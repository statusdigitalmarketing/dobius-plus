#!/usr/bin/env bash
# Capture one-shot macOS launch diagnostics for a published Dobius release.
#
# Usage:
#   DOBIUS_DIAGNOSTIC_TAG=v1.4.42-rc.1 bash config/scripts/macos-launch-diagnostics.sh
#   bash config/scripts/macos-launch-diagnostics.sh --tag v1.4.42-rc.1
set -euo pipefail

REPO="${DOBIUS_DIAGNOSTIC_REPO:-statusdigitalmarketing/dobius-plus}"
TAG="${DOBIUS_DIAGNOSTIC_TAG:-}"
KEEP=0
...
if [[ -z "$TAG" ]]; then
  echo "Set DOBIUS_DIAGNOSTIC_TAG or pass --tag vX.Y.Z-rc.N." >&2
  exit 2
fi

ARCH="$(uname -m)"
case "$ARCH" in
  arm64) ASSET="dobius-macos-arm64.dmg" ;;
  x86_64) ASSET="dobius-macos-x64.dmg" ;;
  *)
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/dobius-launch-diagnostics.XXXXXXXX")"
OUT_DIR="$WORK_DIR/output"
MOUNT_DIR="$WORK_DIR/mount"
APP_DIR="$WORK_DIR/Dobius.app"
DMG_PATH="$WORK_DIR/$ASSET"
mkdir -p "$OUT_DIR" "$MOUNT_DIR"

if [[ -d "$HOME/Desktop" ]]; then
  ZIP_PATH="$HOME/Desktop/dobius-launch-diagnostics-${TAG}-${TIMESTAMP}.zip"
else
  ZIP_PATH="$PWD/dobius-launch-diagnostics-${TAG}-${TIMESTAMP}.zip"
fi

diag_log() {
  printf '[dobius-diagnostics] %s\n' "$*"
}
...
write_environment_report() {
  {
    ...
    echo "$SHELL"
    echo
    echo "## current Dobius processes before diagnostics"
    pgrep -fl 'Dobius|dobius' || true
  } >"$OUT_DIR/environment.txt"
}

ensure_no_existing_dobius() {
  local existing
  existing="$(pgrep -x Dobius || true)"
  if [[ -z "$existing" ]]; then
    return 0
  fi

  {
    echo "An Dobius process is already running. Close Dobius and run this script again."
    echo
    ps -p "$(printf '%s' "$existing" | paste -sd, -)" -o pid=,ppid=,command= || true
  } | tee "$OUT_DIR/existing-dobius-process.txt" >&2
  exit 2
}
...
start_log_stream() {
  local file="$1"
  local predicate='process == "Dobius" OR eventMessage CONTAINS[c] "Dobius" OR eventMessage CONTAINS[c] "com.statusdigitalmarketing.dobius-plus"'
...
latest_dobius_pid_for_app() {
  pgrep -nf "$APP_DIR/Contents/MacOS/Dobius" || true
}
...
wait_for_probe() {
  ...
  for i in $(seq 1 200); do
    local app_pid
    app_pid="$(latest_dobius_pid_for_app)"
...
run_launchservices_probe() {
  ...
  if [[ -n "$extra_env_name" ]]; then
    open -n -W \
      ...
      --env DOBIUS_STARTUP_DIAGNOSTICS=trace \
      --env DOBIUS_STARTUP_DIAGNOSTICS_TRACE_LIMIT=30000 \
      --env DOBIUS_STARTUP_DIAGNOSTICS_FILE="$bootstrap_file" \
...
run_direct_exec_probe() {
  ...
  ELECTRON_ENABLE_LOGGING=1 \
    DOBIUS_STARTUP_DIAGNOSTICS=trace \
    DOBIUS_STARTUP_DIAGNOSTICS_TRACE_LIMIT=30000 \
    DOBIUS_STARTUP_DIAGNOSTICS_FILE="$bootstrap_file" \
    "$APP_DIR/Contents/MacOS/Dobius" >"$stdout_file" 2>"$stderr_file" &
  local runner_pid="$!"
  wait_for_probe "$runner_pid" "$label"
  echo "ended_utc=$(date -u '+%Y-%m-%dT%H:%M:%SZ')" >>"$OUT_DIR/$label.meta"
  stop_log_stream "$stream_pid"
...
run_launchservices_probe "launchservices-bypass-lock" "DOBIUS_BYPASS_SINGLE_INSTANCE_LOCK" "1"
run_direct_exec_probe
write_system_log_snapshot
package_results
