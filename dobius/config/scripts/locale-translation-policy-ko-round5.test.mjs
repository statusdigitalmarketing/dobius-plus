  ...
  it('fixes Korean round 5 review, integration, and search keyword regressions', () => {
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Install the Dobius skill so agents know to use the Dobius CLI.',
        localeValue: '에이전트가 Dobius CLI 사용 방법을 알 수 있도록 Dobius 기술을 설치합니다.',
        locale: 'ko'
      })
    ).toBe('agents가 Dobius CLI를 사용하도록 Dobius 스킬을 설치하세요.')
    expect(
      repairTranslatedValue({
