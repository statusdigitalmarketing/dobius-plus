...
async function installElectronPackageBinary() {
  ...
  const tempDir = mkdtempSync(resolve(tmpdir(), 'dobius-electron-'))
...
function getExtractorCommand(zipPath, extractDir) {
  if (process.env.DOBIUS_ELECTRON_PACKAGE_EXTRACTOR) {
    return {
      file: process.execPath,
      args: [process.env.DOBIUS_ELECTRON_PACKAGE_EXTRACTOR, zipPath, extractDir],
      label: `node ${process.env.DOBIUS_ELECTRON_PACKAGE_EXTRACTOR}`
    }
  }

  if (osPlatform() === 'win32') {
    return {
      file: process.env.DOBIUS_POWERSHELL_BIN || 'powershell',
      args: [
        '-NoProfile',
        '-NonInteractive',
        '-ExecutionPolicy',
  ...
  return {
    file: process.env.DOBIUS_UNZIP_BIN || 'unzip',
