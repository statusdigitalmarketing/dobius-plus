...
function withTempFile(callback) {
  const dir = mkdtempSync(join(tmpdir(), 'dobius-inner-signature-'))
  const filePath = join(dir, 'Dobius.exe')
...
describe('verify-windows-inner-signature', () => {
  const originalExpectedSigners = process.env.DOBIUS_WINDOWS_EXPECTED_SIGNERS
  const originalExpectedThumbprints = process.env.DOBIUS_WINDOWS_EXPECTED_THUMBPRINTS

  beforeEach(() => {
    delete process.env.DOBIUS_WINDOWS_EXPECTED_SIGNERS
    delete process.env.DOBIUS_WINDOWS_EXPECTED_THUMBPRINTS
  })
  ...
  afterEach(() => {
    if (originalExpectedSigners === undefined) {
      ...
      process.env.DOBIUS_WINDOWS_EXPECTED_SIGNERS = originalExpectedSigners
    }

    if (originalExpectedThumbprints === undefined) {
      delete process.env.DOBIUS_WINDOWS_EXPECTED_THUMBPRINTS
    } else {
      process.env.DOBIUS_WINDOWS_EXPECTED_THUMBPRINTS = originalExpectedThumbprints
    }
  })
  ...
  it('rejects missing, nonexistent, and directory executable paths before PowerShell', () => {
    ...
    expect(() => validateExecutablePath(join(tmpdir(), 'missing-Dobius.exe'))).toThrow(
      /does not exist/
    )
  ...
  it('runs PowerShell with an argument array and fails on stderr or nonzero exit', () => {
    ...
    expect(getPowerShellSignatureJson('C:\\Path With Spaces\\Dobius.exe', spawnSyncImpl)).toBe(
      JSON.stringify(validSignature)
    )
    expect(calls[0].command).toBe('pwsh')
    expect(calls[0].args).toContain('-Command')
    expect(calls[0].args.at(-1)).not.toBe('C:\\Path With Spaces\\Dobius.exe')
    expect(calls[0].options).toEqual(
      expect.objectContaining({
        encoding: 'utf8',
        env: expect.objectContaining({
          DOBIUS_WINDOWS_INNER_EXECUTABLE: 'C:\\Path With Spaces\\Dobius.exe'
        })
      })
    )

    expect(() =>
      getPowerShellSignatureJson('Dobius.exe', () => ({ status: 0, stdout: '{}', stderr: 'warning' }))
    ).toThrow(/stderr/)
    expect(() =>
      getPowerShellSignatureJson('Dobius.exe', () => ({ status: 7, stdout: '', stderr: '' }))
    ).toThrow(/exit code 7/)
  })
