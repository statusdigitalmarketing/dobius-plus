...
const forceRebuild =
  process.env.DOBIUS_FORCE_NATIVE_REBUILD === '1' ||
  cliOptions.force ||
  ...
  if (isWindowsNativeLockError(err)) {
    console.error(
      '[rebuild] A Windows process appears to be using a native .node file. ' +
        'Close running Dobius/Electron/dev processes for this worktree, then rerun `pnpm install` ' +
        'or `pnpm run rebuild:electron`.'
    )
    if (isPostinstall() && process.env.DOBIUS_STRICT_NATIVE_REBUILD !== '1') {
      console.error(
...
function continuePostinstallWithoutElectron() {
  if (!isPostinstall() || process.env.DOBIUS_STRICT_ELECTRON_INSTALL === '1') {
    return false
  }
  console.error(
...
function getPatchedNodePtyRebuildReason() {
  ...
  // Why: Dobius patches node-pty's native Unix spawn path; upstream prebuilds can
    ...
    if (requirePatchedNodePtySourceBuild && !isNodePtyReleaseBuildDir(native.dir)) {
      throw new Error(
        'node-pty resolved to ' +
          native.dir +
          '; expected build/Release so Dobius\\'s node-pty patch is active'
      )
    }
    return
  }
