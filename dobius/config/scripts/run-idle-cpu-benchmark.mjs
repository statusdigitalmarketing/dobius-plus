...
function createIdleRepo(worktreeCount) {
  const repoDir = mkdtempSync(path.join(os.tmpdir(), 'dobius-idle-cpu-repo-'))
  ...
  writeFileSync(path.join(repoDir, 'README.md'), '# Dobius idle CPU benchmark\n')
  writeFileSync(
  ...
  for (let i = 2; i <= worktreeCount; i += 1) {
    const worktreeDir = path.join(
      ...
      `dobius-idle-cpu-worktree-${i}-${Date.now()}`
    )
...
async function main() {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'dobius-idle-cpu-userdata-'))
  ...
  const app = await electron.launch({
    ...
    env: {
      ...
      ...(options.headful ? { DOBIUS_E2E_HEADFUL: '1' } : { DOBIUS_E2E_HEADLESS: '1' })
    }
  })
