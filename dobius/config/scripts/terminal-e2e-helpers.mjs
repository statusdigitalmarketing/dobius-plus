#!/usr/bin/env node
/**
 * Terminal E2E helpers for agent-browser + CDP testing against a running Dobius
 ...
 *
 * Usage:
 *   import { DobiusTerminal } from './terminal-e2e-helpers.mjs'
 *
 *   const term = new DobiusTerminal(9444)       // CDP port
 *   await term.connect()
...
export class DobiusTerminal {
  /** @param {number} cdpPort — the --remote-debugging-port used when launching Dobius */
  constructor(cdpPort) {
    this.port = cdpPort
  }
   ...
   */
  probePtyIdWithScreenshot(maxId = 10, screenshotPath = tempScreenshotPath('dobius-pty-probe.png')) {
    for (let i = 1; i <= maxId; i++) {
  ...
  /**
   * Take a screenshot of the Dobius window.
   *
   * @param {string} path — output file path
   * @returns {string} the screenshot path
   */
  screenshot(path = tempScreenshotPath('dobius-terminal.png')) {
    ab(this.port, ['screenshot', path])
    return path
  }

  /**
   * Open a new terminal tab in Dobius.
   * @returns {void}
   */
  newTerminal() {
  ...
  readLang(ptyId) {
    ...
    return this.screenshot(tempScreenshotPath('dobius-lang-check.png'))
  }
}
...
if (process.argv[1]?.endsWith('terminal-e2e-helpers.mjs')) {
  ...
  const term = new DobiusTerminal(port)
  console.log('Connecting to Dobius on CDP port', port, '...')
  term.connect()
