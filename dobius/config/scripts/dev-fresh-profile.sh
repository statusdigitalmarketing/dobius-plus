...
# no saved sessions). Your real `dobius-dev` profile is left untouched.
#
# Usage:
...
#   DOBIUS_FRESH_PROFILE_DIR=/some/path ./config/scripts/dev-fresh-profile.sh   # use a fixed dir
set -euo pipefail
...
PROFILE_DIR="${DOBIUS_FRESH_PROFILE_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/dobius-fresh-profile.XXXXXXXX")}"
mkdir -p "$PROFILE_DIR"

cleanup() {
  if [[ "$KEEP" -eq 0 && -z "${DOBIUS_FRESH_PROFILE_DIR:-}" ]]; then
    # Guard rm -rf against accidental empty/unrelated PROFILE_DIR.
    [[ -n "${PROFILE_DIR:-}" && -d "$PROFILE_DIR" && "$PROFILE_DIR" == */dobius-fresh-profile* ]] || return 0
    rm -rf "$PROFILE_DIR"
...
DOBIUS_DEV_USER_DATA_PATH="$PROFILE_DIR" DOBIUS_DEV_SHOW_FIRST_RUN_EDUCATION=1 pnpm dev
