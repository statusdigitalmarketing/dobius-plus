...
describe('computer-use skill guidance', () => {
  it('keeps web-app targeting on the computer-use surface', () => {
    ...
    expect(skill).toContain('Use this skill for desktop UI through `dobius computer`')
    expect(skill).toContain('operate the desktop browser app/window that contains the page')
    expect(skill).not.toContain('dobius goto')
    expect(skill).not.toContain('dobius snapshot')
    expect(skill).not.toContain('dobius click')
    expect(skill).not.toContain('dobius fill')
