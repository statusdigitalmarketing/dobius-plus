...
const binaryPath = path.join(packagePath, '.build', 'release', 'dobius-computer-use-macos')
const appPath = path.join(packagePath, '.build', 'release', 'Dobius+ Computer Use.app')
const appExecutablePath = path.join(appPath, 'Contents', 'MacOS', 'dobius-computer-use-macos')
...
const bundleId =
  process.env.DOBIUS_COMPUTER_MACOS_BUNDLE_ID ?? 'com.statusdigitalmarketing.dobius-plus.computer-use'
const displayName = 'Dobius+ Computer Use'
...
function buildUniversalBinary() {
  const builtBinaries = universalTriples.map((triple) => {
    ...
    return path.join(packagePath, '.build', triple, 'release', 'dobius-computer-use-macos')
  })
...
function createHelperApp() {
  ...
  // Why: cursor-badged icon keeps the helper distinguishable from the main app
  // in the Dock and permission dialogs (its setup window can land in Dock recents).
  copyFileSync(path.join(repoRoot, 'resources', 'build', 'icon-computer-use.icns'), appIconPath)
...
function codesignArgs(identity, targetPath) {
  ...
  if (process.env.DOBIUS_MAC_RELEASE === '1' && identity !== '-') {
...
function resolveSigningIdentity() {
  const explicitIdentity = process.env.DOBIUS_COMPUTER_MACOS_SIGN_IDENTITY ?? process.env.CSC_NAME
  if (explicitIdentity) {
  ...
  if (process.env.DOBIUS_MAC_RELEASE !== '1' && developmentMatch) {
...
<dict>
  ...
  <string>dobius-computer-use-macos</string>
  ...
  <string>Dobius+ Computer Use needs Accessibility permission to read and interact with app interfaces when you ask Dobius to use apps.</string>
  <key>NSScreenCaptureUsageDescription</key>
  <string>Dobius+ Computer Use needs Screen Recording permission to capture app windows when you ask Dobius to inspect your screen.</string>
</dict>
</plist>
`
}
