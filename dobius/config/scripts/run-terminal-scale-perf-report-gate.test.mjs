...
function tempReportPath() {
  const dir = mkdtempSync(join(tmpdir(), 'dobius-terminal-perf-gate-'))
  tempDirs.push(dir)
...
describe('run-terminal-scale-perf-report-gate', () => {
  ...
  it('runs the scale suite with JSON output, then summarizes and budget-checks the report', () => {
    ...
    const status = runTerminalScalePerfReportGate({
      ...
      env: { ...process.env, DOBIUS_TEST_MARKER: '1' },
      spawnSyncImpl
    })
    ...
    expect(calls[0].options.env.DOBIUS_TEST_MARKER).toBe('1')
  ...
  it('uses the report path from env when no flag is provided', () => {
    ...
    const status = runTerminalScalePerfReportGate({
      env: { ...process.env, DOBIUS_E2E_TERMINAL_PERF_REPORT_PATH: reportPath },
      spawnSyncImpl
    })
