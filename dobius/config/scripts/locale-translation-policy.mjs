...
export function shouldPreserveEnglishValue(enValue, key = '') {
  ...
  if (/^https?:\/\//.test(enValue) || enValue.startsWith('dobius://')) {
    return true
  }
...
function applyCjkLatinTermSpacing(localeValue, locale) {
  ...
  if (locale === 'ko') {
    ...
    // boundary — re-glue "Dobius 에"/"PR 을"/"에서는" but keep "Jira 이슈"/"Dobius 로고"/"agent 에뮬레이터".
...
export function repairTranslatedValue({ key, enValue, localeValue, locale }) {
  ...
  if (enValue.includes('dobius://')) {
    result = result.replace(/虎鲸:\/\//g, 'dobius://')
  }

  if (enValue === 'Dobius' || enValue.startsWith('Dobius ')) {
    result = result
      .replaceAll('虎鲸', 'Dobius')
      .replaceAll('逆戟鲸', 'Dobius')
      .replaceAll('シャチ', 'Dobius')
  }

  if (enValue.includes('dobius://')) {
    result = result.replace(/シャチ:\/\//g, 'dobius://')
  }
...
export function repairCatalog(enCatalog, localeCatalog, locale) {
  ...
  if (localeCatalog.menu) {
    if (locale === 'zh') {
      if (localeCatalog.menu.exploreDobius !== '探索 Dobius') {
        localeCatalog.menu.exploreDobius = '探索 Dobius'
        repaired += 1
      }
      if (localeCatalog.menu.gettingStarted !== 'Dobius 入门') {
        localeCatalog.menu.gettingStarted = 'Dobius 入门'
        repaired += 1
      }
    }
    if (locale === 'ko') {
      if (localeCatalog.menu.exploreDobius !== 'Dobius 둘러보기') {
        localeCatalog.menu.exploreDobius = 'Dobius 둘러보기'
        repaired += 1
      }
      if (localeCatalog.menu.gettingStarted !== 'Dobius 시작하기') {
        localeCatalog.menu.gettingStarted = 'Dobius 시작하기'
        repaired += 1
      }
    }
  }
