...
describe('electron-builder config', () => {
  ...
  it('matches the Linux desktop entry to Electron window class', () => {
    expect(electronBuilderConfig.linux.desktop.entry.StartupWMClass).toBe('dobius')
  })
  ...
  it('uses AppImage and deb as local Linux targets without changing existing artifact names', () => {
    ...
    expect(electronBuilderConfig.appImage.artifactName).toBe('dobius-linux.${ext}')
    expect(electronBuilderConfig.deb.artifactName).toBe('dobius-ide_${version}_${arch}.${ext}')
    expect(electronBuilderConfig.rpm).toMatchObject({
      packageName: 'dobius-ide',
      artifactName: 'dobius-ide-${version}.${arch}.${ext}'
    })
  })

  it('uses a distinct AppImage name for Linux arm64 release uploads', () => {
    const configPath = require.resolve('../electron-builder.config.cjs')
    const original = process.env.DOBIUS_LINUX_ARM64_RELEASE
    try {
      delete require.cache[configPath]
      process.env.DOBIUS_LINUX_ARM64_RELEASE = '1'
      expect(require('../electron-builder.config.cjs').appImage.artifactName).toBe(
        'dobius-linux-arm64.${ext}'
      )
    } finally {
      if (original === undefined) {
        delete process.env.DOBIUS_LINUX_ARM64_RELEASE
      } else {
        process.env.DOBIUS_LINUX_ARM64_RELEASE = original
      }
      delete require.cache[configPath]
  ...
  it('uses Dobius native rebuild hook instead of electron-builder default rebuild', () => {
  ...
  it('verifies packaged main runtime deps from Windows-style asar entries', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'dobius-runtime-deps-'))
    try {
  ...
  it('prunes non-target node-pty prebuilds from packaged runtime resources', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'dobius-node-pty-prune-'))
    try {
  ...
  it('copies the Windows node-pty ConPTY runtime beside the rebuilt addon', async () => {
    for (const arch of ['x64', 'arm64']) {
      const resourcesDir = await mkdtemp(join(tmpdir(), `dobius-node-pty-conpty-${arch}-`))
      try {
  ...
  it('prunes non-target @parcel/watcher platform subpackages from packaged runtime resources', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'dobius-parcel-watcher-prune-'))
    try {
  ...
  it('leaves unrelated @parcel/* runtime deps untouched when pruning the watcher', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'dobius-parcel-watcher-prune-unrelated-'))
    try {
      const parcelDir = join(resourcesDir, 'node_modules', '@parcel')
      await mkdir(join(parcelDir, 'watcher'), { recursive: true })
      await mkdir(join(parcelDir, 'watcher-darwin-arm64'), { recursive: true })
  ...
  it('prunes type declaration artifacts from packaged runtime node_modules', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'dobius-runtime-type-prune-'))
    try {
  ...
  it('prunes duplicate darwin sherpa-onnx runtime dylib aliases', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'dobius-sherpa-prune-'))
    try {
  ...
  it('prunes zod TypeScript sources from packaged runtime resources', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'dobius-zod-prune-'))
    try {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'dobius-electron-builder-config-'))
      try {
        const resourcesDir = join(root, 'linux-unpacked', 'resources')
        const launcherPath = join(resourcesDir, 'bin', 'dobius-ide')
