...
function readAppDirArg(argv) {
  ...
  if (process.platform === 'darwin') {
    return 'dist/mac-arm64/Dobius.app'
  }
...
function getPackagedCliPath(appDir) {
  if (process.platform === 'darwin' || appDir.endsWith('.app')) {
    return join(appDir, 'Contents', 'Resources', 'bin', 'dobius')
  }
  if (process.platform === 'win32') {
    return join(appDir, 'resources', 'bin', 'dobius.cmd')
  }
  return join(appDir, 'resources', 'bin', 'dobius')
}

const appDir = resolve(readAppDirArg(process.argv.slice(2)))
const tempRoot = await mkdtemp(join(tmpdir(), 'dobius-packaged-cli-smoke-'))
