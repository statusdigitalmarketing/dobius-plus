  ...
  it('includes both mac updater ZIP names for the tag version', () => {
    expect(getRequiredReleaseAssetNames('v1.4.27')).toEqual(
      expect.arrayContaining([
        'Dobius-1.4.27-mac.zip',
        'Dobius-1.4.27-mac.zip.blockmap',
        'Dobius-1.4.27-arm64-mac.zip',
        'Dobius-1.4.27-arm64-mac.zip.blockmap'
      ])
    )
  })
  ...
  it('includes x64 and arm64 Linux assets', () => {
    expect(getRequiredReleaseAssetNames('v1.4.27')).toEqual(
      expect.arrayContaining([
        ...
        'dobius-linux.AppImage',
        'dobius-linux-arm64.AppImage',
        'dobius-ide_1.4.27_amd64.deb',
        'dobius-ide_1.4.27_arm64.deb',
        'dobius-ide-1.4.27.x86_64.rpm',
        'dobius-ide-1.4.27.aarch64.rpm'
      ])
    )
  })
})
  ...
  it('extracts relative and absolute manifest asset names', () => {
    expect(
      extractManifestAssetNames(
        [
          'files:',
          '  - url: Dobius-1.4.27-arm64-mac.zip',
          '  - url: https://example.com/downloads/dobius-windows-setup.exe',
          'path: dobius-linux.AppImage'
        ].join('\n')
      )
    ).toEqual(['Dobius-1.4.27-arm64-mac.zip', 'dobius-windows-setup.exe', 'dobius-linux.AppImage'])
  })
})
...
describe('verifyRequiredReleaseAssets', () => {
  it('fails when a manifest-referenced asset has not been uploaded', async () => {
    ...
    const assets = required.filter((name) => name !== 'Dobius-1.4.27-arm64-mac.zip')
      ...
      .mockResolvedValueOnce(
        jsonResponse(
          [
            ...
            '  - url: Dobius-1.4.27-arm64-mac.zip',
            '    sha512: test',
            'path: Dobius-1.4.27-arm64-mac.zip'
          ].join('\n')
        )
      )
    ...
    await expect(
      verifyRequiredReleaseAssets({ repo: 'statusdigitalmarketing/dobius-plus', tag, token: 'token' })
    ).rejects.toThrow('Missing: Dobius-1.4.27-arm64-mac.zip')
  ...
  it('checks assets referenced by the Linux arm64 updater manifest', async () => {
      ...
      .mockResolvedValueOnce(
        jsonResponse(
          [
            'version: 1.4.27',
            'files:',
            '  - url: dobius-linux-arm64.AppImage.blockmap',
            'path: dobius-linux-arm64.AppImage'
          ].join('\n')
        )
      )
      .mockResolvedValue(jsonResponse('version: 1.4.27\n'))
    ...
    await expect(
      verifyRequiredReleaseAssets({ repo: 'statusdigitalmarketing/dobius-plus', tag, token: 'token' })
    ).rejects.toThrow('Missing: dobius-linux-arm64.AppImage.blockmap')
