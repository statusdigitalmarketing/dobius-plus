...
const wrapperPath = path.join(projectDir, 'config', 'scripts', 'dobius-dev.mjs')

describe('dobius-dev package bin', () => {
  it('uses a Node entrypoint for cross-platform package installs', () => {
    expect(packageJson.bin['dobius-dev']).toBe('./config/scripts/dobius-dev.mjs')
    expect(readFileSync(wrapperPath, 'utf8')).toMatch(/^#!\/usr\/bin\/env node\n/)
  })

  it('runs the dev CLI through Node without requiring Bash', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'dobius-dev-bin-'))
    ...
    writeFileSync(
      ...
      [
        ...
        '  userDataPath: process.env.DOBIUS_USER_DATA_PATH,',
        '  appExecutable: process.env.DOBIUS_APP_EXECUTABLE',
        '}));'
      ].join('\n'),
      'utf8'
    )
    ...
    execFileSync(process.execPath, [wrapperPath, '--help'], {
      env: {
        ...process.env,
        DOBIUS_DEV_CLI_ENTRY_PATH: cliEntry,
        DOBIUS_DEV_USER_DATA_PATH: path.join(root, 'user-data'),
        DOBIUS_APP_EXECUTABLE: path.join(root, 'Electron')
      },
      stdio: 'ignore'
    })
