...
Main-process spawn attribution: `DOBIUS_PTY_SPAWN_TIMING=1` → `[pty-spawn-timing]` lines
...
- Existing startup diagnostics: `DOBIUS_STARTUP_DIAGNOSTICS=1` writes `[startup] <event>` lines to stderr
...
the electron-vite build with `DOBIUS_STARTUP_DIAGNOSTICS=1`, milestones parsed from stderr).
...
freshly-written small files → real %APPDATA%\Dobius measured 62s for the same command.)
...
- User report: UI freezes ~5s after sending prompt; OpenCode session itself continues fine
  (visible from external terminal). So the agent process is healthy — the freeze is in Dobius's
...
2. **Sync `runtime.onPtyData` per data event before batching** (MED-HIGH):
   `src/main/ipc/pty.ts:1376-1430` → `dobius-runtime.ts:3256-3420`: normalizeTerminalChunk +
...
- Dobius's plugin POSTed that full text to the agent-hook server on EVERY event →
...
1. **Plugin throttle + cap (source fix)** — `src/main/opencode/hook-service.ts`:
   ...
   prompts bypass the throttle slot). Plugin file is rewritten on every Dobius-launched
...
- This machine's real packaged-Dobius userData: `%APPDATA%\Dobius` = **28,650 files / 2.06 GB**
  (mostly Chromium caches: Cache, Code Cache, GPUCache, blob_storage…).
- Measured the exact command Dobius runs in `openMainWindow()` (src/main/index.ts:517-523):
...
- `DOBIUS_STARTUP_DIAGNOSTICS=1` → `[startup] <event>` lines on stderr (startup-diagnostics.ts).
...
- Hermetic benchmark launch path: `DOBIUS_E2E_USER_DATA_DIR=<dir>` redirects userData
  (works packaged + dev), `DOBIUS_E2E_HEADLESS=1` keeps window hidden. Dev/preview mode
  skips single-instance lock → safe alongside installed Dobius.
