...
name: orchestration
description: >-
  Use Dobius+ orchestration for structured multi-agent coordination: threaded
  messages, blocking ask/reply flows, task dispatch, worker_done/escalation
  waits, task DAGs, decision gates, coordinator loops, or decomposing work
  across agents. Use `dobius-cli` instead for full ownership handoffs, including
  ...
  supervise, monitor, wait for results, or coordinate a DAG. Use `dobius-cli` for
  ordinary terminal control, lightweight terminal prompts, shell commands, Dobius+
  worktree management, reading or waiting on terminals, and automation of the
  browser embedded inside Dobius+. Use Computer Use for browser windows, webviews,
  Dobius+ app UI, or desktop UI outside Dobius+'s embedded browser.
---

# Dobius+ Inter-Agent Orchestration

Orchestration is Dobius+'s structured coordination layer for agent messages, task ownership, dispatch state, and worker completion tracking.

Use this skill when coordination state matters. For lightweight terminal prompts or basic worktree/terminal/built-in-browser control, use `dobius-cli`.
...
- `dobius status --json` should show a running runtime.
- `dobius` must be on PATH.
- The orchestration experimental feature must be enabled in Settings > Experimental.
- `dobius orchestration` commands are RPC calls to the running Dobius+ runtime.
...
- Do not use `dobius orchestration dispatch --inject` for full handoffs. It injects a coordinator preamble that tells the worker to send `worker_done`, heartbeat, `ask`, and post-completion polling messages back to the original terminal.
- Do not run `dobius orchestration task-create`, `dobius orchestration dispatch --inject`, or `dobius orchestration check --wait` for full handoffs. Do not peek at terminal output after prompt delivery to monitor progress.
...
```bash
dobius orchestration task-list --json
dobius terminal list --json
# If inherited context includes a task id:
dobius orchestration dispatch-show --task <task_id> --json
```

## Messaging

```bash
dobius orchestration send --to <handle|@group> --subject <text> [--from <handle>] [--body <text>] [--type <type>] [--priority <level>] [--thread-id <id>] [--payload <json>] [--json]
dobius orchestration check [--terminal <handle>] [--unread] [--types <type,...>] [--inject] [--wait] [--timeout-ms <n>] [--json]
dobius orchestration reply --id <msg_id> --body <text> [--from <handle>] [--json]
dobius orchestration ask --to <handle> --question <text> [--options <csv>] [--timeout-ms <n>] [--from <handle>] [--json]
dobius orchestration inbox [--limit <n>] [--json]
```

Rules:

- Omit `--from` unless impersonating another terminal; Dobius+ auto-resolves it from the current terminal.
- While supervising workers manually, use `check --wait --types worker_done,escalation,decision_gate --timeout-ms <n>` instead of sleep/poll loops. Reply to `decision_gate` messages with `dobius orchestration reply --id <msg_id> --body <answer> --json`, then keep waiting.
...
```bash
dobius orchestration task-create --spec <text> [--deps <json_array>] [--parent <task_id>] [--json]
dobius orchestration task-list [--status <status>] [--ready] [--json]
dobius orchestration task-update --id <task_id> --status <status> [--result <json>] [--json]
dobius orchestration dispatch --task <task_id> --to <handle> [--from <handle>] [--inject] [--json]
dobius orchestration dispatch-show --task <task_id> [--json]
```
...
- If the target is a bare shell, omit `--inject`, dispatch for tracking if needed, then send the prompt manually with `dobius terminal send --terminal <handle> --text <prompt> --enter --json`.
...
```bash
dobius orchestration gate-create --task <task_id> --question <text> [--options <json_array>] [--json]
dobius orchestration gate-resolve --id <gate_id> --resolution <text> [--json]
dobius orchestration gate-list [--task <task_id>] [--status <status>] [--json]
dobius orchestration run --spec <text> [--from <handle>] [--poll-interval-ms <n>] [--max-concurrent <n>] [--worktree <selector>] [--json]
dobius orchestration run-stop [--json]
```
...
Recovery only: `dobius orchestration reset --tasks|--messages|--all --json` clears runtime-global orchestration state. Do not run it during active coordination unless explicitly abandoning that state.
...
`--no-parent` only controls Dobius+ lineage; it does not choose the Git base. For an independent top-level worktree, omit `--base-branch` so Dobius+ uses the repo default base, or explicitly pass the repo default base (`origin/main`, `origin/master`, or the `dobius repo show --repo <selector> --json` value); never base it on the current feature branch unless the user explicitly asks for stacked work or "branch from current". Put current-branch context in the prompt instead.
...
```bash
dobius terminal create --worktree active --title <task-name> --command "codex" --json
dobius terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
dobius orchestration dispatch --task <task_id> --to <handle> --inject --json
```

Reuse an idle agent in the required worktree only if the prompt allows reuse; otherwise create a fresh terminal there. Use a new worktree only when explicitly requested or when independent isolated checkout state is intended. For supervised new-worktree workers, decide the Git base separately from lineage: `--no-parent` makes the worktree top-level in Dobius+, while omitted `--base-branch` uses the repo default base.

```bash
dobius worktree create --name <task-name> --agent codex --json
dobius terminal list --worktree id:<newWorktreeId> --json
dobius terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
dobius orchestration dispatch --task <task_id> --to <handle> --inject --json
```

For new-worktree workers, read the id from `worktree create`, then use `terminal list` to get the agent handle. Omit `--repo` only inside a Dobius+-managed worktree; otherwise pass `--repo <selector>`. `--agent` reveals the new worktree and launches the selected agent in its first terminal, so do not create a separate startup terminal. Do not run `worktree create` when the task must stay in the current worktree.

Use `dobius worktree create --prompt ...` or `dobius terminal send ...` for full handoffs or untracked/lightweight prompts. Those paths do not attach `taskId`/`dispatchId`; the worker should not send lifecycle messages unless the prompt supplies a live orchestration preamble.

Other terminal commands coordinators often need:

```bash
dobius terminal list [--worktree <selector>] [--json]
dobius terminal create [--worktree <selector>] [--title <text>] [--command <cmd>] [--json]
dobius terminal split --terminal <handle> [--direction horizontal|vertical] [--command <cmd>] [--json]
dobius terminal wait --terminal <handle> --for tui-idle --timeout-ms <n> --json
dobius terminal read --terminal <handle> --json
dobius terminal send --terminal <handle> --text <text> --enter --json
```

If an older CLI rejects `worktree create --agent`, create the worktree normally, then run `dobius terminal create --worktree <selector> --command "codex" --json` or `--command "claude"`.
...
- Workers with a valid live preamble must send `worker_done` exactly once, even on failure:
  `dobius orchestration send --to <coordinator_handle> --type worker_done --subject "<short status>" --body "<3-sentence summary: what you did, what you found, what's left>" --payload '{"taskId":"<task_id>","dispatchId":"<dispatch_id>","filesModified":["path/a"],"reportPath":"<optional>"}' --json`
- For long tasks, send heartbeat/status only when the preamble asks for it, including both IDs:
  `dobius orchestration send --to <coordinator_handle> --type heartbeat --subject "alive" --payload '{"taskId":"<task_id>","dispatchId":"<dispatch_id>","phase":"implementing"}' --json`
...
```bash
dobius terminal create --worktree active --title login-css-worker --command "claude" --json
dobius terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
dobius orchestration task-create --spec "Fix the login button CSS" --json
dobius orchestration dispatch --task <task_id> --to <handle> --inject --json
dobius orchestration check --wait --types worker_done,escalation,decision_gate --timeout-ms 900000 --json
```

## Next Action

Coordinator: confirm `dobius status --json`, inspect `task-list`/`dispatch-show` if inheriting state, then choose either a manual loop (`task-create` -> worker -> `dispatch --inject` -> `check --wait`) or `orchestration run`.
