...
REM launcher resolves APP_DIR back to resources/ and `dobius open` cannot find
REM Dobius.exe on packaged Windows installs.
for %%I in ("%RESOURCES_DIR%\..") do set "APP_DIR=%%~fI"
set "ELECTRON=%APP_DIR%\Dobius.exe"

if not exist "%ELECTRON%" (
  echo Unable to locate Dobius.exe next to "%RESOURCES_DIR%" 1>&2
  exit /b 1
)

REM Why: Dobius packages the CLI entrypoint outside app.asar so the public shell
