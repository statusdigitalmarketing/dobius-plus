...
import { DobiusTitlebarButtons } from './components/DobiusTitlebarButtons'
...
import { applyAppSkin } from './lib/app-skin'
...
function App(): React.JSX.Element {
  ...
  const titlebarRightClusterRef = useRef<HTMLDivElement | null>(null)
  const [collapsedSidebarHeaderWidth, setCollapsedSidebarHeaderWidth] = useState(0)
  const [titlebarRightClusterWidth, setTitlebarRightClusterWidth] = useState(0)
  ...
  useEffect(() => {
    ...
    if (featureTipsDecision.tipId === 'dobius-cli') {
      trackDobiusCliFeatureTipShown('app_open')
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        // dobius-data.json, silently erasing the user's saved tabs. The fix is
  ...
  // Apply the app-wide skin. Runs after the theme effect above so the skin owns
  // the dark/light class while active; clearing it (`none`) reverts to the
  // stylesheet theme the effect above already set.
  useEffect(() => {
    applyAppSkin(settings?.appSkin)
  }, [settings?.appSkin, settings?.theme])
  ...
  // Why: Skills and Knowledge keep the persistent left sidebar (and collapse to
  // full-width) like the Agents page, so the main nav surfaces behave the same.
  const showSidebar =
  ...
  useEffect(() => {
    ...
    const dispatchShortcutInput = (input: ShortcutDispatchInput): void => {
      ...
      const notifyTerminalCapture = (actionId: KeybindingActionId): void => {
        if (context !== 'terminal' || (terminalShortcutPolicy ?? 'dobius-first') !== 'dobius-first') {
          return
        }
  ...
  // Why: the floating top-right cluster (TV, Quick Commands, notifications,
  // sidebar toggle) overlays the rightmost pane's tab strip in a separate DOM
  // branch. Measuring its real width — rather than hardcoding a per-button sum
  // that silently under-reserves whenever a button is added — lets the tab strip
  // reserve exactly enough space so long tab titles never slide under it.
  useLayoutEffect(() => {
    const cluster = titlebarRightClusterRef.current
    if (!cluster) {
      setTitlebarRightClusterWidth(0)
      return
    }

    const updateWidth = (): void => {
      setTitlebarRightClusterWidth(cluster.getBoundingClientRect().width)
    }
      ...
      <div className="flex h-full items-center">
        {isMac && !isFullScreen ? (
          ...
          /* Why: on Windows/Linux the native title bar is removed, so we render
             the Dobius+ logo as a non-interactive identity anchor and a ··· button
              ...
              <ContextMenu>
                <ContextMenuTrigger asChild>
                  <div
                    ...
                    aria-label={translate('auto.App.5096cbbc86', 'Dobius+')}
                  >
                    <span className="titlebar-app-name-main">
                      {translate('auto.App.5096cbbc86', 'Dobius+')}
                    </span>
                  </div>
                </ContextMenuTrigger>
                <ContextMenuContent>
      ...
      style={
        {
          ...
          // Why: measured width of the floating top-right button cluster; the
          // rightmost pane's tab strip reserves this so tabs don't slide under it.
          '--titlebar-right-cluster-width': `${titlebarRightClusterWidth}px`
                ...
                page keep the titlebar. */}
                  ...
                  <div className="flex flex-row flex-1 min-h-0 overflow-hidden">
                    {showSidebar ? (
                     ...
                     (traffic lights, sidebar toggle, "Dobius+" title, agent badge)
