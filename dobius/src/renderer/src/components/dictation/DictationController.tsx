...
export function DictationController() {
  ...
  const finishDictationSession = useCallback(
    async (sessionId: string) => {
      ...
      const finishNotice = resolveDictationFinishNotice({
        sessionErrored,
        finalTranscriptReceived: finalTranscriptReceivedRef.current,
        capturedChunkCount: getCapturedChunkCount(),
        dictationMode: settings?.voice?.dictationMode === 'hold' ? 'hold' : 'toggle'
      })
      if (finishNotice === 'no-speech') {
        toast.message(
          translate(
      ...
      } else if (finishNotice === 'hold-mode-hint') {
        toast.message('Voice is in Hold mode: keep the shortcut held down while you speak.', {
          action: {
            label: translate(
    ...
    [setDictationState, setPartialTranscript, stopCapture, getCapturedChunkCount, settings]
  )
