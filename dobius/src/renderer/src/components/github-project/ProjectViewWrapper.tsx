...
const DOBIUS_FEATURE_REQUEST_URL = 'https://github.com/statusdigitalmarketing/dobius-plus/issues/new'
...
export default function ProjectViewWrapper({ selectedRepoIds }: Props): React.JSX.Element {
  ...
  // simplified slug-mode dialog. `repoNotInDobius` drives the fallback modal
  // from the design doc's `repo-not-in-dobius` interaction state.
  ...
  // in Dobius+ (matched repos go through the full GitHubItemDialog above), so
  // there's no `matchedRepo` to track here. The repo-not-in-dobius modal —
  // owned by this parent, not the slug dialog — handles "Start work".
  const [slugDialog, setSlugDialog] = useState<{
    origin: GitHubItemDialogProjectOrigin
  } | null>(null)
  const [repoNotInDobius, setRepoNotInDobius] = useState<{
    owner: string
    repo: string
    url: string | null
  } | null>(null)
  ...
  if (resolvedDialogRepoItem !== dialogRepoItem) {
    ...
    // Dobius+; clear them before the modal tree receives stale repo ids.
  ...
  if (resolvedMissingRepoDialogs.repoNotInDobius !== repoNotInDobius) {
    setRepoNotInDobius(resolvedMissingRepoDialogs.repoNotInDobius)
  }
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col">
      ...
      {/* Slug-only simplified dialog for rows whose repo isn't added to Dobius+.
          Why: no Start-work affordance lives inside the slug dialog — the
          parent's `handleStartWork`/`repoNotInDobius` modal owns that flow, so
      ...
      {/* repo-not-in-dobius prompt: see design doc Interaction States. */}
      <Dialog
        open={resolvedMissingRepoDialogs.repoNotInDobius !== null}
        onOpenChange={(open) => !open && setRepoNotInDobius(null)}
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {translate(
                ...
                'Repository not in Dobius+'
              )}
            </DialogTitle>
            <DialogDescription>
              {resolvedMissingRepoDialogs.repoNotInDobius
                ? translate(
                    'auto.components.github.project.ProjectViewWrapper.1850fceac8',
                    "{{value0}}/{{value1}} isn't added to Dobius+. Add it to start work, or open in GitHub.",
                    {
                      value0: resolvedMissingRepoDialogs.repoNotInDobius.owner,
                      value1: resolvedMissingRepoDialogs.repoNotInDobius.repo
                    }
                  )
                : null}
            </DialogDescription>
          ...
          <DialogFooter className="gap-2 sm:justify-end">
            <Button variant="ghost" onClick={() => setRepoNotInDobius(null)}>
              {translate('auto.components.github.project.ProjectViewWrapper.dffa899f36', 'Cancel')}
            </Button>
            {resolvedMissingRepoDialogs.repoNotInDobius?.url ? (
              <Button
                variant="outline"
                onClick={() => {
                  if (resolvedMissingRepoDialogs.repoNotInDobius?.url) {
                    void window.api.shell.openUrl(resolvedMissingRepoDialogs.repoNotInDobius.url)
                  }
                  setRepoNotInDobius(null)
                }}
              >
                {translate(
  ...
  return (
    <div className="project-view-tab-strip flex min-h-[41px] min-w-0 flex-none items-end gap-1 overflow-x-auto overflow-y-hidden border-b border-border/50 bg-muted/20 px-3 pt-3">
      {views.map((v) => {
            ...
            title={
              supported
                ...
                : translate(
                    ...
                    "{{value0}} — Dobius+ doesn't support {{value1}} project views yet. File a feature request at {{value2}}.",
                    { value0: v.name, value1: layoutLabel, value2: DOBIUS_FEATURE_REQUEST_URL }
                  )
            }
            className={cn(
        ...
        const unsupportedMessage = `Dobius+ doesn't support ${layoutLabel} project views yet.`
        return (
            ...
            <HoverCardTrigger asChild>
              <span
                ...
                aria-label={translate(
                  ...
                  { value0: v.name, value1: unsupportedMessage, value2: DOBIUS_FEATURE_REQUEST_URL }
                )}
              ...
              <div className="space-y-2">
                <p className="text-xs leading-5 text-muted-foreground">
                  ...
                  {translate(
                    ...
                    'Switch to a Table view to work with this project in Dobius+.'
                  )}
                </p>
                <Button
                  type="button"
                  size="xs"
                  variant="outline"
                  onClick={() => void window.api.shell.openUrl(DOBIUS_FEATURE_REQUEST_URL)}
                >
                  {translate(
...
function ErrorState({
  ...
  const copy =
    error.type === 'too_large'
      ? `This view has ${totalCount ?? 'many'} items — too large to render in Dobius+. Narrow the view's filter on GitHub.`
      : error.type === 'unsupported_layout'
        ? 'Dobius+ only renders table views yet. This is a Board or Roadmap view.'
