...
describe('github-project-picker-filter', () => {
  it('excludes pinned and recent projects from browse results', () => {
    const projects = [
      project('statusdigitalmarketing', 1, 'Pinned Project'),
      project('statusdigitalmarketing', 2, 'Recent Project'),
      project('statusdigitalmarketing', 3, 'Browse Project')
    ]

    expect(
      filterGitHubProjectPickerProjects({
        projects,
        pinned: [{ owner: 'statusdigitalmarketing', ownerType: 'organization', number: 1 }],
        recent: [
          {
  ...
  it('matches project title, owner, and number case-insensitively', () => {
    const projects = [
      project('statusdigitalmarketing', 42, 'Roadmap'),
  ...
  it('enforces the query budget by UTF-8 byte length', () => {
    ...
    expect(
      filterGitHubProjectPickerProjects({
        projects: [project('statusdigitalmarketing', 1, 'Roadmap')],
        pinned: [],
        recent: [],
        query
      })
