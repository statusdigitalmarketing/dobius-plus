        ...
        inboxWorktrees={[
          {
            ...
            path: '/worktrees/dobius/payments-refactor'
          }
        ]}
        pending={false}
        error={null}
...
describe('NewExternalWorktreesInboxLine', () => {
  ...
  it('keeps suppress as a hover-revealed header icon instead of expanded text action', async () => {
    ...
    const suppressButton = container.querySelector<HTMLButtonElement>(
      'button[aria-label="Hide external worktrees permanently for dobius"]'
    )
    ...
    const expandButton = container.querySelector<HTMLButtonElement>(
      'button[aria-label="Expand new externally-created worktrees for dobius"]'
    )
    await act(async () => {
      expandButton?.click()
    })
