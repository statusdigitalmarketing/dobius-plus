...
export function LinearAgentSkillSetupPrompt({
  ...
  const command = useMemo(
    () => buildSkillCommandForRuntime(DOBIUS_LINEAR_SKILL_INSTALL_COMMAND, agentRuntime),
    [agentRuntime]
  )
  const canonicalSkillInstalled = hasInstalledAgentSkill(skill.skills, DOBIUS_LINEAR_SKILL_NAME, {
    sourceKinds: GLOBAL_AGENT_SKILL_SOURCE_KINDS
  })
  ...
  const cliAvailable = isDobiusCliAvailableOnPath(cliStatus)
  ...
  const setupDialog = (
    <LinearAgentSkillSetupDialog
      ...
      onInstall={async () => {
        await window.api.skills.installBundled(DOBIUS_LINEAR_SKILL_NAME)
      }}
      onRecheck={async () => {
        if (surface === 'modal') {
