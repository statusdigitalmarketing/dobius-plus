...
const HOST_DISMISS_STORAGE_KEY = 'dobius.linearTicketsSkill.setupDismissed.host'
...
vi.mock('@/lib/agent-skill-cli-prerequisite', () => ({
  ...
  ensureDobiusCliAvailableForAgentSkillTerminal: mocks.ensureCli,
  isDobiusCliAvailableOnPath: (status: CliInstallStatus | null | undefined) =>
...
function cliStatus(overrides: Partial<CliInstallStatus>): CliInstallStatus {
  return {
    platform: 'darwin',
    commandName: 'dobius',
    commandPath: '/usr/local/bin/dobius',
    pathDirectory: '/usr/local/bin',
    pathConfigured: true,
    launcherPath: '/Applications/Dobius.app/Contents/MacOS/Dobius',
    installMethod: 'symlink',
    supported: true,
    state: 'installed',
    currentTarget: '/Applications/Dobius.app/Contents/MacOS/Dobius',
...
describe('LinearAgentSkillSetupPrompt reminder toast', () => {
  ...
  it('shows a warning toast on a later modal-only activation after Not now', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      'Dobius CLI and Linear skill are missing',
      expect.objectContaining({
        id: 'linear-agent-skill-setup-dobius.linearTicketsSkill.setupDismissed.host',
        description:
          'Install the Dobius CLI and the Linear skill to enable your agents to read and edit Linear tasks.',
        action: {
          label: 'Set up',
  ...
  it('does not repeat the Dobius CLI in CLI-only reminder toast copy', async () => {
    mocks.skillState.installed = true
    await snoozeInitialModal({ linked: true, remote: false, surface: 'modal' })
    await renderPrompt({ linked: true, remote: false, surface: 'modal' })

    expect(toast.warning).toHaveBeenCalledWith(
      'Dobius CLI is missing',
      expect.objectContaining({
        description: 'Install the Dobius CLI to enable your agents to read and edit Linear tasks.'
      })
    )
  })
  ...
  it('keeps remote setup nuance in reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        description:
          'Install the Dobius CLI and the Linear skill to enable your agents to read and edit Linear tasks. Remote agent environments may need their own setup.'
      })
    )
  })
  ...
  it('keeps WSL target nuance in reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        description:
          'Install the Dobius CLI and the Linear skill to enable your agents to read and edit Linear tasks. This setup runs in the selected WSL agent runtime.'
      })
    )
  })
  ...
  it('opens the setup dialog from the reminder toast action', async () => {
    ...
    expect(toast.dismiss).toHaveBeenCalledWith(
      'linear-agent-skill-setup-dobius.linearTicketsSkill.setupDismissed.host'
    )
  })
