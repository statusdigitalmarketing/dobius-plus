...
const hiddenWorktrees = [
  ...
  },
  {
    ...
    path: '/srv/repos/dobius/ssh-worktree',
...
describe('ImportedWorktreesVisibilityLine', () => {
  it('renders the compact repo-group line with expand and dismiss actions', () => {
    ...
    expect(markup).toContain('Expand hidden worktrees for dobius')
    expect(markup).toContain(
      'Keep 4 discovered worktrees hidden for dobius; recover from the project menu'
    )
    expect(markup).toContain('aria-expanded="false"')
    expect(markup).not.toContain('Imported 4 existing worktrees')
    expect(markup).not.toContain('Dobius found 4 worktrees')
  ...
  it('scopes pinned fallback copy to the repo name without a dismiss action', () => {
    ...
    expect(markup).toContain('Hiding 4 discovered worktrees in dobius')
    ...
    const groups = groupWorktreesByParentPath([
      {
        ...
        path: 'C:\\Repos\\Dobius\\FeatureX'
      }
    ])

    expect(groups).toMatchObject([{ path: 'C:/Repos/Dobius' }])
    expect(groups[0]?.path).not.toBe('C:\\Repos\\Dobius')
  })
