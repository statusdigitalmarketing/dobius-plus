...
vi.mock('@/lib/agent-skill-cli-prerequisite', () => ({
  ...
  ensureDobiusCliAvailableForAgentSkillTerminal: vi.fn(async () => null),
...
function discoveredSkill(overrides: Partial<DiscoveredSkill>): DiscoveredSkill {
  return {
    ...
    directoryPath: '/Users/test/.agents/skills/dobius-linear',
    skillFilePath: '/Users/test/.agents/skills/dobius-linear/SKILL.md',
    installed: true,
    fileCount: 1,
    updatedAt: null,
    ...overrides
...
describe('LinearAgentSkillSetupPrompt update command', () => {
  ...
  it('uses the canonical update command when the canonical Linear skill is installed', async () => {
    mocks.skillState.skills = [discoveredSkill({ name: 'dobius-linear' })]

    await renderPrompt()

    expect(mocks.panelProps.at(-1)).toEqual(
      expect.objectContaining({ installedCommand: 'Reinstall bundled skill: dobius-linear' })
    )
  })
  ...
  it('uses the legacy update command when only the legacy Linear skill is installed', async () => {
    ...
    expect(mocks.panelProps.at(-1)).toEqual(
      expect.objectContaining({ installedCommand: 'Reinstall bundled skill: linear-tickets' })
    )
  })

  it('prefers the canonical update command when both Linear skill names are installed', async () => {
    mocks.skillState.skills = [discoveredSkill({ name: 'dobius-linear' }), legacyLinearSkillPath()]
