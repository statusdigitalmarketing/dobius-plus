...
describe('useCreateProjectDefaults', () => {
  beforeEach(() => {
    ...
    mocks.getDefaultCreateProjectParent.mockResolvedValue('/Users/alice/dobius/projects')
  })
  ...
  it('auto-fills the local default parent and reports Git availability', async () => {
    ...
    expect(setCreateParent).toHaveBeenCalledWith('/Users/alice/dobius/projects')
    expect(mocks.stateValues[DEFAULT_PARENT_STATE]).toBe('/Users/alice/dobius/projects')
  ...
  it('keeps the local default marker after the auto-filled parent rerenders the hook', async () => {
    ...
    useHarness()
    await flushAsync()
    useHarness({ createParent: '/Users/alice/dobius/projects' })
  ...
  it('resolves the runtime default parent from the host home directory', async () => {
    ...
    expect(setCreateParent).toHaveBeenCalledWith('/home/alice/dobius/projects')
    expect(mocks.stateValues[DEFAULT_PARENT_STATE]).toBe('/home/alice/dobius/projects')
  ...
  it('replaces an untouched local default when switching to a runtime target', async () => {
    ...
    await flushAsync()
    expect(local.setCreateParent).toHaveBeenCalledWith('/Users/alice/dobius/projects')

    const runtime = useHarness({
      activeRuntimeEnvironmentId: 'env-1',
      createParent: '/Users/alice/dobius/projects'
    })
    ...
    expect(mocks.browseRuntimeServerDirectory).toHaveBeenCalledWith('env-1', '~')
    expect(resolvedRuntime.setCreateParent).toHaveBeenCalledWith('/home/alice/dobius/projects')
    expect(mocks.stateValues[DEFAULT_PARENT_STATE]).toBe('/home/alice/dobius/projects')
  ...
  it('does not replace a touched parent when switching to a runtime target', async () => {
    ...
    const local = useHarness({ createParent: '/Users/alice/dobius/projects' })
    local.result.markCreateParentTouched('/Users/alice/dobius/projects/pr5115-target-switch')

    const runtime = useHarness({
      activeRuntimeEnvironmentId: 'env-1',
      createParent: '/Users/alice/dobius/projects/pr5115-target-switch'
    })
    await flushAsync()
