...
describe('shouldDeferRemoteFileBrowserPasteResolve', () => {
  it('keeps small path and filter pastes on the immediate resolver path', () => {
    expect(shouldDeferRemoteFileBrowserPasteResolve('/workspaces/dobius/src')).toBe(false)
...
describe('parsePathInput', () => {
  ...
  it('`Documents/dobius` commits `Documents` and filters by `dobius`', () => {
    expect(parsePathInput('Documents/dobius')).toEqual({
      mode: 'path',
      base: 'cwd',
      committedSegments: ['Documents'],
      trailingFilter: 'dobius'
    })
  })
...
describe('resolveSegmentStep', () => {
  const listing: DirEntry[] = [
    ...
    { name: 'dobius-internal', isDirectory: true },
  ...
  it('unique prefix descends', () => {
    expect(resolveSegmentStep('dobius', '/home/neil', listing)).toEqual({
      type: 'descend',
      name: 'dobius-internal'
    })
  })
