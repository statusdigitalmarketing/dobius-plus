...
describe('CommentMarkdown', () => {
  it('autolinks same-repo GitHub issue references when repo context is provided', () => {
    const markup = renderToStaticMarkup(
      <CommentMarkdown
        variant="document"
        githubRepo={{ owner: 'statusdigitalmarketing', repo: 'dobius' }}
        content="Automated fix-PR from pr-bug-scan for parent **#2316**."
      />
    )

    expect(markup).toContain('href="https://github.com/statusdigitalmarketing/dobius-plus/issues/2316"')
  ...
  it('does not autolink GitHub issue references inside existing links or code', () => {
    ...
    expect(markup).not.toContain('href="https://github.com/statusdigitalmarketing/dobius-plus/issues/2316"')
    expect(markup).not.toContain('href="https://github.com/statusdigitalmarketing/dobius-plus/issues/2317"')
  })
  ...
  it('keeps non-attachment document links as links', () => {
    const url = 'https://github.com/statusdigitalmarketing/dobius-plus/pull/5265'
    const markup = renderToStaticMarkup(<CommentMarkdown variant="document" content={url} />)
  ...
  it('autolinks very large generated GitHub reference comments', () => {
    ...
    const transform = remarkGitHubReferences({ owner: 'statusdigitalmarketing', repo: 'dobius' })()

    expect(() => transform(tree)).not.toThrow()
    expect(tree.children[0]?.children).toHaveLength(referenceCount * 2 - 1)
    expect(tree.children[0]?.children[0]).toMatchObject({
      type: 'link',
      url: 'https://github.com/statusdigitalmarketing/dobius-plus/issues/1'
    })
  })
