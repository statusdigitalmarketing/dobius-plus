...
const FEDORA_DISMISS_STORAGE_KEY = 'dobius.linearTicketsSkill.setupDismissed.wsl.Fedora'
  ...
  AgentSkillSetupPanel: (props: Record<string, unknown> & { children?: ReactNode }) => {
    ...
    return (
      <section data-testid="linear-skill-inline-panel">
        ...
        <button type="button" onClick={() => void (props.onInstall as () => void)()}>
          Mock install
        </button>
        <button
          type="button"
...
describe('LinearAgentSkillSetupPrompt', () => {
  ...
  it('shows a compact setup prompt when a linked Linear worktree is missing CLI or skill setup', async () => {
    ...
    expect(rendered.textContent).toContain('Dobius+ CLI and Linear agent skill are missing')
  ...
  it('uses WSL discovery, status, command, and prerequisite setup together', async () => {
    ...
    expect(document.body.textContent).toContain(
      "wsl.exe -d 'Fedora' -- bash -lc 'Install bundled skill: dobius-linear'"
    )
    expect(mocks.panelProps.at(-1)).toEqual(
      expect.objectContaining({
        installedCommand: "wsl.exe -d 'Fedora' -- bash -lc 'Reinstall bundled skill: dobius-linear'",
    ...
    expect(window.api.skills.installBundled).toHaveBeenCalledWith('dobius-linear')
  ...
  it('opens the terminal setup panel in a dialog only after the user asks to set up', async () => {
    ...
    expect(document.body.textContent).toContain('dobius-linear')
  ...
  it('auto-opens as a modal-only prompt and treats Not now as a casual close', async () => {
    ...
    expect(document.body.textContent).toContain('Dobius+ CLI and Linear agent skill are missing.')
  ...
  it('ignores stale CLI success after the runtime context changes during Re-check', async () => {
    ...
    expect(document.body.textContent).toContain('Dobius+ CLI is missing.')
    expect(document.body.textContent).not.toContain('Linear ticket access is ready')
  })
