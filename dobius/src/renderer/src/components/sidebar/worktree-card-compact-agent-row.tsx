...
type CompactAgentRowProps = {
  ...
  onOpenTerminal?: (tabId: string, paneKey: string) => void
...
export const CompactAgentRow = React.memo(function CompactAgentRow({
  ...
  useEffect(
    () => () => {
      if (pendingSingleClickRef.current !== null) window.clearTimeout(pendingSingleClickRef.current)
    },
    []
  )
  ...
  const handleActivate = useCallback(
    ...
    [agent.paneKey, agent.tab.id, onActivate, onOpenTerminal]
  )
  const handleOpenTerminal = useCallback(
    (e: React.MouseEvent) => {
      ...
      onOpenTerminal(agent.tab.id, agent.paneKey)
    },
    [agent.paneKey, agent.tab.id, onOpenTerminal]
  )
