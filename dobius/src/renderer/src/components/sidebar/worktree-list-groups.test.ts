...
const remoteWorktree: Worktree = {
  ...
  path: '/home/alice/dobius-feature',
...
describe('getPRGroupKey', () => {
  ...
  it('prefers repo-scoped PR status over stale legacy path-scoped status', () => {
    const prCache = {
      '/tmp/dobius::feature/super-critical': {
        data: { state: 'closed' }
      },
      'repo-1::feature/super-critical': {
        data: { state: 'merged' }
...
describe('buildRows with pinned worktrees', () => {
  ...
  it('groups multiple host setups for the same project under one project header', () => {
    ...
    expect(rows).toMatchObject([
      { type: 'header', key: 'project:github:statusdigitalmarketing/dobius-plus', label: 'Dobius', count: 2 },
  ...
  it('orders project identity headers by the manual repo order anchor', () => {
    const analyticsProject: Project = {
      ...project,
      id: 'github:statusdigitalmarketing/analytics',
    ...
    const analyticsRepo: Repo = {
      ...
      path: '/tmp/analytics',
      displayName: 'analytics',
      upstream: { owner: 'statusdigitalmarketing', repo: 'analytics' }
    }
    ...
    expect(headers.map((row) => row.key)).toEqual([
      'project:github:statusdigitalmarketing/dobius-plus',
      'project:github:statusdigitalmarketing/analytics'
    ])
    expect(headers[0]).toMatchObject({
      key: 'project:github:statusdigitalmarketing/dobius-plus',
      repo: { id: repo.id, badgeColor: repo.badgeColor }
    })
  })

  it('splits same-host checkouts of one project into separate per-setup groups', () => {
    const repoB: Repo = { ...repo, id: 'repo-2', path: '/tmp/dobius-2', displayName: 'dobius-2' }
    const worktreeB: Worktree = {
      ...worktree,
      id: 'wt-2',
      repoId: repoB.id,
      path: '/tmp/dobius-2-feature',
    ...
    expect(headers).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          key: 'project:github:statusdigitalmarketing/dobius-plus::setup:repo-1',
          label: 'dobius'
        }),
        expect.objectContaining({
          key: 'project:github:statusdigitalmarketing/dobius-plus::setup:repo-2',
          label: 'dobius-2'
        })
      ])
    )
  ...
  it('splits all project setups when one host has duplicate checkouts', () => {
    ...
    const headers = rows.filter((row) => row.type === 'header')
    expect(headers.map((row) => row.key)).toEqual([
      'project:github:statusdigitalmarketing/dobius-plus::setup:repo-1',
      'project:github:statusdigitalmarketing/dobius-plus::setup:repo-local-b',
      'project:github:statusdigitalmarketing/dobius-plus::setup:repo-remote'
    ])
  })
  ...
  it('keeps a provisioned runtime copy under the project header alongside a same-host checkout', () => {
    const runtimeRepoB: Repo = {
      ...repo,
      id: 'repo-runtime-b',
      path: '/tmp/dobius-runtime-b',
      displayName: 'dobius-runtime-b'
    }
    const runtimeWorktreeB: Worktree = {
      ...worktree,
      id: 'wt-runtime-b',
      repoId: runtimeRepoB.id,
      path: '/tmp/dobius-runtime-b-feature',
  ...
  it('splits duplicate user checkouts while a provisioned copy nests, on one host', () => {
    ...
    expect(
      headers.some((row) => row.key === 'project:github:statusdigitalmarketing/dobius-plus::setup:repo-runtime-b')
    ).toBe(false)
    expect(headers.find((row) => row.key === 'project:github:statusdigitalmarketing/dobius-plus')).toMatchObject({
      count: 1
    })
  })
  ...
  it('groups Windows host and WSL setups on the same runtime host', () => {
    ...
    const windowsRepo: Repo = {
      ...repo,
      id: 'repo-windows',
      path: String.raw`C:\Users\alice\git\dobius`,
      displayName: 'dobius',
    ...
    const wslRepo: Repo = {
      ...repo,
      id: 'repo-wsl',
      path: String.raw`\\wsl.localhost\Ubuntu\home\alice\git\dobius`,
      displayName: 'dobius',
      executionHostId: runtimeHostId
    }
    const windowsWorktree: Worktree = {
      ...
      path: String.raw`C:\Users\alice\git\dobius\feature`
    }
    const wslWorktree: Worktree = {
      ...worktree,
      id: 'wt-wsl',
      repoId: wslRepo.id,
      path: String.raw`\\wsl.localhost\Ubuntu\home\alice\git\dobius\feature`
    }
  ...
  it('uses saved host labels for mixed-host sidebar card badges', () => {
    const runtimeRepo: Repo = {
      ...remoteRepo,
      id: 'repo-runtime',
      path: '/Users/alice/runtime-dobius',
      connectionId: null,
  ...
  it('keeps same-named repos separate without project setup identity', () => {
    const rows = buildRows(
      ...
      [worktree, remoteWorktree],
      new Map([
        [repo.id, { ...repo, displayName: 'dobius' }],
        [remoteRepo.id, { ...remoteRepo, displayName: 'dobius' }]
      ]),
      null,
      new Set()
    )
  ...
  it('returns project group keys for worktree reveal when project setup identity exists', () => {
    ...
    ).toBe('project:github:statusdigitalmarketing/dobius-plus')
  })
  ...
  it('emits an empty ungrouped repo placeholder before imported cards are merged', () => {
    ...
    expect(rows).toMatchObject([
      { type: 'header', key: 'repo:repo-1', label: 'dobius' },
      {
        type: 'imported-worktrees-card',
        key: 'imported-worktrees-card:repo-group:repo-1',
        placement: 'repo-group'
