  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from Dobius+. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from Dobius+. Git worktrees will also be removed from git and disk; folder workspaces will only remove the Dobius+ workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from Dobius+. The project folder on disk will not be deleted.'
