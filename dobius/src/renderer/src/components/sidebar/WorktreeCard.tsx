...
function isWebClient(): boolean {
  return Boolean((window as unknown as { __DOBIUS_WEB_CLIENT__?: boolean }).__DOBIUS_WEB_CLIENT__)
}
...
const WorktreeCard = React.memo(function WorktreeCard({
  ...
  const sshStatus = useAppStore((s) => {
    ...
    // owned by the runtime layer — Dobius+ suppresses their ssh:state-changed broadcasts, so their
  ...
  // Why: runtime ("Dobius+ server") hosts get the same disconnected treatment as
  ...
  useEffect(() => {
    ...
    // Why: PRs created outside Dobius+ (for example `gh pr create`) do not emit a
  ...
  const handleOpenGitHubIssueInDobius = useCallback(
    (e: React.MouseEvent) => {
      e.stopPropagation()
  ...
  const handleOpenReviewInDobius = useCallback(
    (e: React.MouseEvent) => {
      e.stopPropagation()
  ...
  const handleOpenLinearIssueInDobius = useCallback(
    (e: React.MouseEvent) => {
      e.stopPropagation()
      if (!linearIssue) {
        return
    ...
    : compactCards && (showBranchIdentityHover || hasDetails || hasPorts)
      ? (title: React.ReactElement): React.ReactElement => (
          <WorktreeCardDetailsHover
            ...
            onOpenGitHubIssueInDobius={
              metaIssue && 'url' in metaIssue && metaIssue.url
                ? handleOpenGitHubIssueInDobius
                : undefined
            }
            onOpenLinearIssueInDobius={linearIssue?.url ? handleOpenLinearIssueInDobius : undefined}
            onOpenReviewInDobius={
              metaReview?.url && metaReview.provider === 'github'
                ? handleOpenReviewInDobius
                : undefined
            }
  ...
  const detailsAndPorts =
    detailsAndPortsContent && !newCardStyle ? (
      <WorktreeCardDetailsHover
        ...
        onEditComment={affiliateListMode ? undefined : handleEditComment}
        onOpenGitHubIssueInDobius={
          metaIssue && 'url' in metaIssue && metaIssue.url ? handleOpenGitHubIssueInDobius : undefined
        }
        onOpenLinearIssueInDobius={linearIssue?.url ? handleOpenLinearIssueInDobius : undefined}
        onOpenReviewInDobius={
          metaReview?.url && metaReview.provider === 'github' ? handleOpenReviewInDobius : undefined
        }
        onOpenAutomation={affiliateListMode ? undefined : handleOpenAutomation}
        onOpenAutomationRun={affiliateListMode ? undefined : handleOpenAutomationRun}
  ...
  const parentCardBodyWithHoverDetails =
    hasHoverDetails && !titleRenaming ? (
      <WorktreeCardDetailsHover
        ...
        onOpenLinearIssueInDobius={linearIssue?.url ? handleOpenLinearIssueInDobius : undefined}
        onOpenReviewInDobius={
          hoverReview?.url && hoverReview.provider === 'github' ? handleOpenReviewInDobius : undefined
        }
        onOpenAutomation={affiliateListMode ? undefined : handleOpenAutomation}
        onOpenAutomationRun={affiliateListMode ? undefined : handleOpenAutomationRun}
        // Why: branch lookup can show a review without persisted metadata. Only
