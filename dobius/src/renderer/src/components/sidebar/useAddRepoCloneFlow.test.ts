...
describe('useAddRepoCloneFlow', () => {
  beforeEach(() => {
    ...
    mocks.refIndex = 0
    mocks.refValues = []
    mocks.stateValues = ['https://github.com/statusdigitalmarketing/dobius-plus.git', '/srv', false, null, null]
  ...
  it('does not prefill SSH clone destinations from the local workspace directory', async () => {
    mocks.stateValues = ['https://github.com/statusdigitalmarketing/dobius-plus.git', '', false, null, null]
    const { useAddRepoCloneFlow } = await import('./useAddRepoCloneFlow')
    ...
    const result = useAddRepoCloneFlow({
      ...
      activeRuntimeEnvironmentId: null,
      sshTargetId: 'ssh-1',
      workspaceDir: '/private/tmp/dobius-setup-e2e.hOWO1f',
      fetchWorktrees: mocks.fetchWorktrees,
      onGitRepoReady: mocks.onGitRepoReady
    })

    expect(result.cloneDestination).toBe('')
    expect(mocks.stateSetters[1]).not.toHaveBeenCalledWith('/private/tmp/dobius-setup-e2e.hOWO1f')
  })

  it('strips Electron IPC wrappers from clone errors', async () => {
    const cloneError =
      'Clone failed: Destination already exists and is not empty: /srv/dobius. Choose a different parent folder, delete the existing folder, or add the existing repository instead.'
