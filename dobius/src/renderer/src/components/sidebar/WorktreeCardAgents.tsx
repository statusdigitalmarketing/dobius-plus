...
export const SUPPRESS_WORKTREE_LIST_SCROLL_ADJUSTMENT_EVENT =
  'dobius-suppress-worktree-list-scroll-adjustment'
...
const WorktreeCardAgentsBody = React.memo(function WorktreeCardAgentsBody({
  ...
  const handleActivateAgentChat = useCallback(
    (tabId: string, paneKey: string) => {
      ...
      requestAnimationFrame(() => {
        const tab = document.querySelector<HTMLElement>(
          `[data-terminal-tab-id="${CSS.escape(tabId)}"]`
        )
        tab?.querySelector<HTMLTextAreaElement>('[data-native-chat-root="true"] textarea')?.focus()
      })
    },
        ...
        <DashboardAgentRow
          ...
          onActivate={
            agent.rowSource === 'retained'
              ...
              : nativeChatEnabled
                ? handleActivateAgentChat
                : handleActivateAgentTab
          }
          onOpenTerminal={
            agent.rowSource === 'retained' || !nativeChatEnabled
              ? undefined
              : handleActivateAgentTab
          }
          now={now}
