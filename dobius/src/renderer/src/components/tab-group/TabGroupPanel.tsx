        ...
        <div className="flex h-full items-stretch pr-1.5">
          ...
          >
            <div className={focusedActionChromeClassName}>
              {/* Quick Commands button moved to the top-right Dobius titlebar cluster. */}
          ...
          {/* Why: Electron's native drag hit-test ignores z-index — a no-drag
              ...
              sibling in another branch. The floating right-titlebar cluster
              (TV, Quick Commands, notifications, sidebar toggle) and the
              fixed-position window-controls overlay for custom desktop chrome
              both sit in separate DOM trees, so we need an explicit no-drag
              child here to punch holes in the drag surface beneath them.
              --titlebar-right-cluster-width is the measured width of that
              cluster (falls back to the 40px toggle before it resolves);
              window controls add --window-controls-width (138px when active,
              0px otherwise) on top. */}
          {reserveClosedExplorerToggleSpace && !rightSidebarOpen ? (
            <div
              className="shrink-0"
              style={
                {
                  width: 'calc(var(--titlebar-right-cluster-width, 40px) + var(--window-controls-width, 0px))',
                  WebkitAppRegion: 'no-drag'
                } as React.CSSProperties
              }
            />
