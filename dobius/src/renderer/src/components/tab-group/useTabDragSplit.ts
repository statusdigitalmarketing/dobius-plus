...
import { tearOffTerminalTab } from '../tab-bar/terminal-tab-tear-off'
...
function isOutsideAppWindowContent(pointer: { x: number; y: number } | null): boolean {
  if (!pointer) {
    return false
  }
  const margin = 8
  return (
    pointer.x < -margin ||
    pointer.y < -margin ||
    pointer.x > window.innerWidth + margin ||
    pointer.y > window.innerHeight + margin
  )
}
...
export function useTabDragSplit({
  ...
  const onDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      // Why: collisionDetection falls back to closestCenter, so event.over is
      // never null during a tab drag — the pointer leaving the window viewport
      // is the real tear-off signal, checked before any in-window split/reorder.
      if (activeData.tabType === 'terminal' && isOutsideAppWindowContent(getDragPointer(event))) {
        void tearOffTerminalTab(activeData.visibleTabId)
        finishDrag(true)
        return
      }
