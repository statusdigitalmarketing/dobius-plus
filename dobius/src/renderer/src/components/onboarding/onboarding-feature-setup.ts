...
import { showDobiusCliRegistrationPromptToast } from '@/lib/agent-skill-cli-prerequisite'
import {
...
export function createOnboardingFeatureSetupDeps(): OnboardingFeatureSetupDeps {
  ...
  return {
    ...
    showCliRegistrationPrompt: showDobiusCliRegistrationPromptToast,
...
export async function runOnboardingFeatureSetup(
  ...
  try {
    ...
    if (!status.supported) {
      warnings.push({
        featureId: 'cli',
        message: status.detail ?? 'Dobius+ CLI registration is not available on this platform.'
      })
      ...
      if (next.state !== 'installed') {
        warnings.push({
          featureId: 'cli',
          message: next.detail ?? 'Dobius+ CLI registration needs attention.'
        })
