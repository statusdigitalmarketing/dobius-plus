...
import {
  DOBIUS_EDITOR_REQUEST_FILE_CLOSE_EVENT,
  DOBIUS_EDITOR_SAVE_AND_CLOSE_EVENT,
  DOBIUS_EDITOR_REQUEST_CMD_SAVE_EVENT,
...
function Terminal(): React.JSX.Element | null {
  ...
  const terminalShortcutPolicy = useAppStore(
    (s) => s.settings?.terminalShortcutPolicy ?? 'dobius-first'
  )
  ...
  // the same row as the "Dobius+" title. The target element is created by App.tsx.
  ...
  // DOBIUS_EDITOR_REQUEST_FILE_CLOSE_EVENT) must not re-open the dialog over
  ...
  const handleSaveDialogSave = useCallback(async () => {
    ...
    setSaveDialogFileId(null)
    window.dispatchEvent(new CustomEvent(DOBIUS_EDITOR_SAVE_AND_CLOSE_EVENT, { detail: { fileId } }))
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent): void => {
      ...
      const notifyTerminalCapture = (actionId: KeybindingActionId): void => {
        if (context !== 'terminal' || terminalShortcutPolicy !== 'dobius-first') {
          return
        }
      ...
      if (!e.repeat && matchShortcut('editor.save')) {
        ...
        if (!inEditor) {
          ...
          if (state.activeTabType === 'editor' && state.activeFileId) {
            ...
            window.dispatchEvent(new Event(DOBIUS_EDITOR_REQUEST_CMD_SAVE_EVENT))
            return
          }
        }
      }
