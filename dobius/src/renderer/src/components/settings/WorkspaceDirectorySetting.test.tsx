...
describe('WorkspaceDirectorySetting', () => {
  it('keeps typed workspace paths local until blur', () => {
    ...
    expect(updateSettings).toHaveBeenCalledWith({ workspaceDir: 'dobius-workspaces' })
  })
  ...
  it('does not commit Enter while IME composition is active', () => {
    ...
    expect(getInput().value).toBe('dobius-workspaces')
    expect(updateSettings).not.toHaveBeenCalled()
  })
  ...
  it('allows retrying the same draft when the persisted value has not changed', () => {
    ...
    expect(updateSettings).toHaveBeenNthCalledWith(1, { workspaceDir: 'dobius-workspaces' })
    expect(updateSettings).toHaveBeenNthCalledWith(2, { workspaceDir: 'dobius-workspaces' })
  })
})
