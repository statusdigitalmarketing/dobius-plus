...
const DOBIUS_STARGAZERS_URL = 'https://github.com/statusdigitalmarketing/dobius-plus/stargazers'
...
export function GeneralSupportSection({
  ...
  useEffect(() => {
    let cancelled = false
    void window.api.gh.checkDobiusStarred().then((result) => {
      if (cancelled) {
        return
      }
      if (result === null) {
  ...
  const handleStarClick = async (): Promise<void> => {
    if (starState === 'web-fallback') {
      ...
      await window.api.shell.openUrl(DOBIUS_STARGAZERS_URL)
    ...
    const ok = await window.api.gh.starDobius('settings')
    if (!ok) {
      if (mountedRef.current) {
        setStarState('web-fallback')
      }
