...
import { RepositoryInstructionsSection } from './RepositoryInstructionsSection'
...
export function RepositoryPane({
  ...
  const handleCopyTemplate = async () => {
    // Why: the missing-`dobius.yaml` state is a migration aid, so copying the shared-template
  ...
  const instructionsEntries = allEntries.filter((entry) => entry.title === 'Project instructions')
  ...
  const visibleSections = [
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <SearchableSetting
            ...
            description={translate(
              ...
              'Remove this project from Dobius+.'
            )}
    ...
    forceFullPaneForRepoMatch || matchesSettingsSearch(searchQuery, instructionsEntries) ? (
      <RepositoryInstructionsSection
