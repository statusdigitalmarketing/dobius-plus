...
function getInstallDescription(platform: string): string {
  if (platform === 'darwin') {
    return 'Register `dobius` in /usr/local/bin.'
  }
  if (platform === 'linux') {
    return 'Register `dobius` in ~/.local/bin.'
  }
  if (platform === 'win32') {
    return 'Register `dobius` in your user PATH.'
  }
  return 'CLI registration is not yet available on this platform.'
}

function getFallbackCommandName(platform: string): string {
  return platform === 'win32' ? 'dobius.cmd' : 'dobius'
}
    ...
    <section className="space-y-4" data-settings-section="cli">
      <div className="space-y-1">
        <h2 className="text-sm font-semibold">
          {translate('auto.components.settings.CliSection.c5c0f2641d', 'Dobius+ CLI')}
        </h2>
        <p className="text-xs text-muted-foreground">
          {translate(
            'auto.components.settings.CliSection.6930feda9e',
            'Use Dobius+ from your terminal to open the app, manage worktrees, and interact with Dobius+ terminals.'
          )}
        </p>
      </div>
