...
describe('PrivacyPane — isEnvBlocked', () => {
  ...
  it('returns true for DOBIUS_TELEMETRY_DISABLED', () => {
    expect(isEnvBlocked({ effective: 'disabled', reason: 'dobius_disabled' })).toBe(true)
  })
...
describe('PrivacyPane — envVarNameForReason', () => {
  ...
  it('maps dobius_disabled to DOBIUS_TELEMETRY_DISABLED', () => {
    expect(envVarNameForReason('dobius_disabled')).toBe('DOBIUS_TELEMETRY_DISABLED')
  })
...
describe('PrivacyPane — computeBlockedReason', () => {
  ...
  it('names DOBIUS_TELEMETRY_DISABLED as the env reason when set', () => {
    const result = computeBlockedReason({ effective: 'disabled', reason: 'dobius_disabled' })
    expect(result).toEqual({ kind: 'env', reason: 'dobius_disabled' })
  })
