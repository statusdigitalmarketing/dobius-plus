...
function DiagnosticsDisabledStateNote({
  ...
  const message =
    reason === 'do_not_track'
      ...
      : reason === 'dobius_telemetry_disabled'
        ? translate(
            'auto.components.settings.PrivacyDiagnosticsRows.63d03261d1',
            'DOBIUS_TELEMETRY_DISABLED=1 is set — creating and sending diagnostic files is disabled.'
          )
        : reason === 'dobius_diagnostics_disabled'
          ? translate(
              'auto.components.settings.PrivacyDiagnosticsRows.d37e92a06b',
              'DOBIUS_DIAGNOSTICS_DISABLED=1 is set — app diagnostics are off.'
            )
          : reason === 'ci'
            ? translate(
