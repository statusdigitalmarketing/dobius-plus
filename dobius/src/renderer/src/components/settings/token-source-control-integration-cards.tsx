          ...
          <p className="text-xs text-muted-foreground">
            {status === 'unavailable' ? (
              ...
              translate(
                ...
                'Bitbucket credentials are configured but could not authenticate. Check the token and repository permissions, then restart Dobius+ if environment variables changed.'
              )
            )}
          </p>
              ...
              <>
                ...
                <span className="font-mono text-[11px]">
                  {translate(
                    ...
                    'DOBIUS_AZURE_DEVOPS_ACCESS_TOKEN'
                  )}
                </span>
                {translate(
                ...
                <span className="font-mono text-[11px]">
                  {translate(
                    ...
                    'DOBIUS_AZURE_DEVOPS_API_BASE_URL'
                  )}
                </span>{' '}
                {translate(
                  'auto.components.settings.token.source.control.integration.cards.7bd345e3f6',
                  'only when Dobius+ cannot derive the API base URL from the git remote.'
                )}
              </>
            ) : (
              translate(
                'auto.components.settings.token.source.control.integration.cards.40f678df73',
                'Azure DevOps credentials are configured but could not authenticate. Check the token, API base URL, and repository permissions, then restart Dobius+ if environment variables changed.'
              )
            )}
          </p>
          <div className="flex items-center gap-2">
              ...
              <>
                ...
                <span className="font-mono text-[11px]">
                  {translate(
                    ...
                    'DOBIUS_GITEA_API_BASE_URL'
                  )}
                </span>{' '}
                {translate(
                  'auto.components.settings.token.source.control.integration.cards.60708f23da',
                  'only when Dobius+ cannot derive the API URL from the remote.'
                )}
              </>
            ) : (
              translate(
                'auto.components.settings.token.source.control.integration.cards.19fb419c12',
                'Gitea credentials are configured but could not authenticate. Check the token, API base URL, and repository permissions, then restart Dobius+ if environment variables changed.'
              )
            )}
          </p>
          <div className="flex items-center gap-2">
