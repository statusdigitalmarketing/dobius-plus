...
const KEEP_LOCAL_MAIN_UP_TO_DATE_DESCRIPTION =
  'When you create a workspace, Dobius+ refreshes the remote base and safely fast-forwards your matching local branch, such as main or master. This keeps commands like git diff main...HEAD from comparing against stale history. Dobius+ skips the update if that branch has uncommitted changes or local-only commits.'
...
export function GitPane({
  ...
  const visibleSections = [
        ...
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            {translate(
              ...
              'When you create a workspace, Dobius+ refreshes the remote base and safely fast-forwards your matching local branch, such as'
            )}
            ...
            {translate(
              ...
              'from comparing against stale history. Dobius+ skips the update if that branch has uncommitted changes or local-only commits.'
            )}
          </p>
        </div>
        <button
    ...
    matchesSettingsSearch(searchQuery, {
      title: translate('auto.components.settings.GitPane.e02ea23a32', 'Dobius+ Attribution'),
      description: translate(
        'auto.components.settings.GitPane.d2eede4c54',
        'Add Dobius+ attribution to commits, PRs, and issues.'
      ),
      keywords: [
        ...
        translate('auto.components.settings.GitPane.e71ce09c42', 'dobius')
      ]
    }) ? (
      <SearchableSetting
        ...
        keywords={['github', 'gh', 'pr', 'issue', 'co-author', 'coauthored', 'attribution', 'dobius']}
        className="flex items-center justify-between gap-4 py-2"
      >
        <div className="space-y-0.5">
          <Label>
            {translate('auto.components.settings.GitPane.e02ea23a32', 'Dobius+ Attribution')}
          </Label>
          <p className="text-xs text-muted-foreground">
            {translate(
              'auto.components.settings.GitPane.d2eede4c54',
