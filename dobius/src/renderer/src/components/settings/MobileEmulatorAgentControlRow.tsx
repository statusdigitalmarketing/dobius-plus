...
export function MobileEmulatorAgentControlRow(): React.JSX.Element {
  ...
  const cliSkillInstallCommand = buildSkillCommandForRuntime(DOBIUS_CLI_SKILL_INSTALL_COMMAND)
  const cliSkillUpdateCommand = buildSkillCommandForRuntime(DOBIUS_CLI_SKILL_UPDATE_COMMAND)
        ...
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            {translate(
              ...
              'Let coding agents control the active mobile emulator with Dobius+ CLI commands.'
            )}
          </p>
        </div>
        <span
          ...
          <div className="min-w-0 flex-1 space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              {translate(
                ...
                'Registers the Dobius+ CLI command so agents can control the active emulator from their shell.'
              )}
            </p>
        ...
        <div className={cn('py-3', setup.step2Blocked && 'opacity-60')}>
          <AgentSkillSetupPanel
            ...
            description={translate(
              ...
              'Enables agents to use Dobius+ CLI commands, including mobile emulator control.'
            )}
            command={cliSkillInstallCommand}
            installedCommand={cliSkillUpdateCommand}
            terminalTitle="Dobius+ CLI skill setup"
            terminalAriaLabel="Dobius+ CLI skill install terminal"
            terminalWorktreeId="settings-mobile-emulator-dobius-cli-skill-terminal"
            ...
            openingHint={translate(
              ...
              'Checking Dobius+ CLI before opening skill setup.'
            )}
            onInstall={async () => {
              await window.api.skills.installBundled(DOBIUS_CLI_SKILL_NAME)
              await window.api.skills.installBundled(DOBIUS_EMULATOR_SKILL_NAME)
            }}
