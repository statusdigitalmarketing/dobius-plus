...
const INSTALL_COMMAND = 'Install bundled skill: dobius-cli'
const UPDATE_COMMAND = 'Reinstall bundled skill: dobius-cli'
...
function panelProps(
  ...
  return {
    title: 'CLI skill',
    description: 'Enables agents to use Dobius workflows.',
...
describe('AgentSkillSetupPanel', () => {
  ...
  it('keeps update copy until the installed panel checks CLI prerequisites', () => {
    const html = renderPanel({
      ...
      preInstallNotice: 'Install the Dobius CLI before running agent skill setup.'
    })
  ...
  it('runs a local bundled install instead of opening a terminal when provided', async () => {
    const onInstall = vi.fn().mockResolvedValue({ installed: true, path: '/home/test/.claude' })
    const onRecheck = vi.fn().mockResolvedValue(true)
    await renderInteractivePanel({ onInstall, onRecheck })
    ...
    expect(mocks.toastSuccess).toHaveBeenCalledWith('Skill installed.')
  })
