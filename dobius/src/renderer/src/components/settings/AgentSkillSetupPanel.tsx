...
export function AgentSkillSetupPanel({
  ...
  const [localInstallRunning, setLocalInstallRunning] = useState(false)
  ...
  const runLocalInstall = async (): Promise<void> => {
    if (!onInstall || localInstallRunning) {
      return
    }
    setLocalInstallRunning(true)
    try {
      await onInstall()
      ...
      toast.success(
        translate(
          'auto.components.settings.AgentSkillSetupPanel.localInstallComplete',
          'Skill installed.'
        )
      )
    } catch (installError) {
      toast.error(
        installError instanceof Error
          ...
          : translate(
              'auto.components.settings.AgentSkillSetupPanel.localInstallFailed',
              'Failed to install skill.'
            )
      )
    } finally {
      if (mountedRef.current) {
        setLocalInstallRunning(false)
      }
    }
  }
    ...
    <div className="mt-3 flex flex-wrap items-center gap-2">
      {!installed || showInstallWhenInstalled ? (
        <Button
          ...
          disabled={terminalOpen || installDisabled || terminalOpening || localInstallRunning}
        >
          {terminalOpening || localInstallRunning ? (
          ...
          {terminalOpening || localInstallRunning
            ? localInstallRunning
              ? translate(
                  'auto.components.settings.AgentSkillSetupPanel.localInstallRunning',
                  'Installing...'
                )
              : translate(
