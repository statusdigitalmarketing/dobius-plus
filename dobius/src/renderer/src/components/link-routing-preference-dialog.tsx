...
type LinkRoutingPreferenceDialogRequest = {
  ...
  resolve: (openInDobius: boolean) => void
}
...
const PREVIEW_STORAGE_KEY = 'dobius.previewLinkRoutingPreferenceDialog'
...
export function LinkRoutingPreferenceDialogProvider({
  ...
  useEffect(() => {
    ...
    void requestPreference({
      openLinksInAppDefault: previewDefault === 'dobius',
      preview: true,
      url: 'https://github.com/statusdigitalmarketing/dobius-plus/pull/1234'
    })
  }, [requestPreference])

  const settleActiveRequest = useCallback((openInDobius: boolean) => {
    const request = activeRequestRef.current
    if (!request) {
      return
    }
    request.resolve(openInDobius)
        ...
        >
                ...
                <DialogTitle className="text-xl leading-tight">
                  {openLinksInAppDefault
                    ? translate(
                        ...
                        "Keep terminal links in Dobius+'s browser?"
                      )
                    : translate(
                        'auto.components.link.routing.preference.dialog.title',
                        "Open terminal links in Dobius+'s browser?"
                      )}
                </DialogTitle>
                <DialogDescription className="text-sm leading-relaxed">
                  {openLinksInAppDefault
                    ...
                    : translate(
                        ...
                        "Use Dobius+'s browser for terminal links, or keep your system browser."
                      )}
                </DialogDescription>
              </div>
            </DialogHeader>
          ...
          <div className="space-y-3 px-6">
            ...
            <div className="flex gap-2 rounded-lg border border-border bg-muted/20 p-3 text-xs leading-relaxed text-muted-foreground">
              ...
              <div className="space-y-1">
                <p>
                  {translate(
                    'auto.components.link.routing.preference.dialog.dobius.note',
                    'Dobius+ can use imported cookies for logged-in sites.'
                  )}
                </p>
                <p>
                  {translate(
          ...
          <DialogFooter className="border-t border-border bg-muted/20 px-6 py-4 sm:justify-between">
            ...
            <Button autoFocus onClick={() => settleActiveRequest(true)}>
              {openLinksInAppDefault
                ? translate(
                    'auto.components.link.routing.preference.dialog.keep.dobius.button',
                    'Keep Dobius+'
                  )
                : translate(
                    'auto.components.link.routing.preference.dialog.dobius.button',
                    'Open in Dobius+'
                  )}
            </Button>
          </DialogFooter>
