...
describe('DashboardAgentRow', () => {
  it('renders orchestration task preview instead of the raw dispatch preamble prompt', () => {
    const markup = renderRow(
      makeAgent(
        {},
        {
          prompt: 'You are working inside Dobius, a multi-agent IDE.',
          orchestration: {
            taskId: 'task-1',
            dispatchId: 'ctx-1',
    ...
    expect(markup).not.toContain('You are working inside Dobius')
  })
  ...
  it('reserves a real working tool line before tool metadata arrives', () => {
    ...
    const activeToolMarkup = renderRow(
      makeAgent({}, { toolName: 'ListDir', toolInput: '/Users/nwparker/dobius' })
    )
