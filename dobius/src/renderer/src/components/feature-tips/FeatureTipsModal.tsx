  ...
  const handlePrimaryAction = async (): Promise<void> => {
    ...
    switch (currentTip.action) {
      case 'learn-cmd-j-palette': {
        ...
        trackCmdJPaletteFeatureTipAcknowledged(
          getDobiusCliFeatureTipTelemetrySource(modalData.source)
        )
        closeModal()
        break
      }
      ...
      case 'setup-cli': {
        ...
        const canApplySetupResult = (): boolean =>
          ...
          setupRequestIdRef.current === setupRequestId
        const telemetrySource = getDobiusCliFeatureTipTelemetrySource(modalData.source)
        trackDobiusCliFeatureTipSetupClicked(telemetrySource)
        setPrimaryBusy(true)
        try {
          const result = await installCliFromFeatureTip(() => window.api.cli.install())
          if (result.kind === 'installed') {
            trackDobiusCliFeatureTipSetupResult(telemetrySource, 'installed')
          ...
          trackDobiusCliFeatureTipSetupResult(telemetrySource, 'needs_attention')
          if (!canApplySetupResult()) {
            return
          }
          toast.warning(
          ...
          const message = error instanceof Error ? error.message : 'Failed to install Dobius+ CLI.'
          if (
            import.meta.env.DEV &&
            message.includes('Development mode uses a generated launcher for validation only')
          ) {
            trackDobiusCliFeatureTipSetupResult(telemetrySource, 'dev_preview')
            if (!canApplySetupResult()) {
              return
            }
            enableOrchestrationSkillSetup()
          ...
          trackDobiusCliFeatureTipSetupResult(telemetrySource, 'failed')
          if (canApplySetupResult()) {
            toast.error(message)
          }
        } finally {
