...
export const DOBIUS_BROWSER_PAGE_ZOOM_EVENT = 'dobius:browser-page-zoom'
...
export function dispatchBrowserPageZoomEvent(detail: BrowserPageZoomEventDetail): void {
  window.dispatchEvent(
    new CustomEvent<BrowserPageZoomEventDetail>(DOBIUS_BROWSER_PAGE_ZOOM_EVENT, {
      detail
    })
  )
}
...
export function addBrowserPageZoomEventListener(
  ...
  window.addEventListener(DOBIUS_BROWSER_PAGE_ZOOM_EVENT, listener)
  return () => window.removeEventListener(DOBIUS_BROWSER_PAGE_ZOOM_EVENT, listener)
}
