...
import { DOBIUS_BROWSER_BLANK_URL, DOBIUS_BROWSER_PARTITION } from '../../../../shared/constants'
import type {
  BrowserLoadError,
...
import {
  ...
  DOBIUS_BROWSER_FOCUS_REQUEST_EVENT,
...
function toDisplayUrl(url: string): string {
  return url === DOBIUS_BROWSER_BLANK_URL ? 'about:blank' : redactKagiSessionToken(url)
}

function getBrowserDisplayTitle(title: string | null | undefined, url: string): string {
  if (
    url === 'about:blank' ||
    url === DOBIUS_BROWSER_BLANK_URL ||
    title === 'about:blank' ||
    title === DOBIUS_BROWSER_BLANK_URL ||
    !title
  ) {
    return 'New Tab'
  }
...
function RemoteBrowserPagePane({
  ...
  const ensureRemotePage = useCallback(
    async (token: RemoteBrowserOperationToken): Promise<string | null> => {
      ...
      const createRemotePage = async (): Promise<string | null> => {
        ...
        const initialUrl =
          currentUrl === DOBIUS_BROWSER_BLANK_URL ? 'about:blank' : currentUrl || 'about:blank'
  ...
  useEffect(() => {
    ...
    window.addEventListener(DOBIUS_BROWSER_FOCUS_REQUEST_EVENT, handleBrowserFocusRequest)
    return () =>
      window.removeEventListener(DOBIUS_BROWSER_FOCUS_REQUEST_EVENT, handleBrowserFocusRequest)
...
function BrowserPagePane({
  ...
  const sessionProfile = sessionProfileId
    ...
    : null
  const webviewPartition = sessionPartition ?? sessionProfile?.partition ?? DOBIUS_BROWSER_PARTITION
    ...
    (webview: Electron.WebviewTag): void => {
      try {
        onUpdatePageStateRef.current(browserTab.id, {
          ...
          // Only explicit navigation/load events should drive Dobius+'s loading UI.
          canGoBack: webview.canGoBack(),
          canGoForward: webview.canGoForward()
        })
      } catch {
        // Why: Electron only exposes these getters after the guest fully
        // attaches. Ignoring the transient failure avoids crashing Dobius+ while
  ...
  useEffect(() => {
    ...
    const handleDidStopLoading = (): void => {
      ...
      if (activeLoadFailure) {
        ...
        if (normalizedAttemptedUrl === normalizedCurrentUrl) {
          ...
          // navigation falls back to a blank Chromium surface even though Dobius+
      ...
      if (keepAddressBarFocusRef.current && currentUrl === DOBIUS_BROWSER_BLANK_URL) {
        focusAddressBarNow()
      } else {
        keepAddressBarFocusRef.current = false
      }
    ...
    const handleFailLoad = (event: {
      ...
      if (event.errorCode === -3) {
        ...
        // when the replacement navigation succeeds. Ignore that noise so Dobius+
    ...
    if (needsInitialNavigation) {
      ...
      // Dobius+ never misses the initial did-fail-load signal for a new tab.
      // Only non-blank initial tabs should light up Dobius+'s loading indicator.
      const initialUrl =
        normalizeBrowserNavigationUrl(initialBrowserUrlRef.current) ?? DOBIUS_BROWSER_BLANK_URL
      trackNextLoadingEventRef.current = initialUrl !== DOBIUS_BROWSER_BLANK_URL
  ...
  useEffect(() => {
    ...
    if (
      ...
      // Why: browserTab.url changes are Dobius+-driven navigations (address bar,
      ...
      trackNextLoadingEventRef.current = normalizedUrl !== DOBIUS_BROWSER_BLANK_URL
      lastKnownWebviewUrlRef.current = normalizedUrl
      webview.src = normalizedUrl
      if (normalizedUrl !== DOBIUS_BROWSER_BLANK_URL) {
        keepAddressBarFocusRef.current = false
        if (document.activeElement === addressBarInputRef.current) {
          focusWebviewNow()
        }
  ...
  useEffect(() => {
    ...
    // Polling only while the active tab is "loading" gives Dobius+ a last-resort
  ...
  const navigateToUrl = useCallback(
    (url: string): void => {
      const navigateBrowserUrl = (targetUrl: string): void => {
        ...
        trackNextLoadingEventRef.current = targetUrl !== DOBIUS_BROWSER_BLANK_URL
        lastKnownWebviewUrlRef.current =
          normalizeBrowserNavigationUrl(browserModelUrl) ?? browserModelUrl
        webview.src = targetUrl
        if (targetUrl !== DOBIUS_BROWSER_BLANK_URL) {
          focusWebviewNow()
        }
      }
  ...
  const isBlankTab = browserTab.url === 'about:blank' || browserTab.url === DOBIUS_BROWSER_BLANK_URL
                    ...
                    <div className="mt-5 flex items-center gap-2">
                      ...
                      {externalUrl ? (
                        <Button
                          ...
                          onClick={() => {
                            // Why: page failures inside Dobius+ can still be recoverable
