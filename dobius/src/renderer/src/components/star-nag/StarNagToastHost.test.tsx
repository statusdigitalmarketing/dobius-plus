...
type StarNagApi = {
  ...
  starDobius: ReturnType<typeof vi.fn>
}
...
describe('StarNagToastHost', () => {
  ...
  beforeEach(() => {
    ...
    starNag = {
      ...
      starDobius: vi.fn().mockResolvedValue(true)
    }
    shell = {
  ...
  it('renders exact onboarding toast copy and confirms only after direct star succeeds', async () => {
    ...
    expect(toastContainer.textContent).toContain(
      'If you’re enjoying Dobius so far, a GitHub star helps other developers discover it.'
    )
    ...
    expect(starNag.starDobius).toHaveBeenCalledTimes(1)
  ...
  it('opens GitHub fallback without calling direct star success path', async () => {
    ...
    expect(shell.openUrl).toHaveBeenCalledWith('https://github.com/statusdigitalmarketing/dobius-plus')
    expect(starNag.openWeb).toHaveBeenCalledTimes(1)
    expect(starNag.starDobius).not.toHaveBeenCalled()
  ...
  it('switches to the explicit GitHub fallback when direct star fails', async () => {
    starNag.starDobius.mockResolvedValueOnce(false)
    ;({ root, container } = renderHost())
  ...
  it('does not allow unresolved close while the primary action is busy', async () => {
    ...
    starNag.starDobius.mockReturnValueOnce(pendingStar.promise)
    ;({ root, container } = renderHost())
