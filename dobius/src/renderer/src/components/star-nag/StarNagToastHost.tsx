...
const DOBIUS_REPO_URL = 'https://github.com/statusdigitalmarketing/dobius-plus'
...
function StarNagToast({
  ...
  const act = async (): Promise<void> => {
    ...
    if (mode === 'web') {
      try {
        await window.api.shell.openUrl(DOBIUS_REPO_URL)
    ...
    let ok = false
    try {
      ok = await window.api.starNag.starDobius()
    } catch {
      ok = false
    }
    if (!ok) {
