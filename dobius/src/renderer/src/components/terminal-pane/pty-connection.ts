...
export function connectPanePty(
  ...
  const scheduleReattachIdleAgentCursorReset = (): void => {
    ...
    reattachIdleAgentCursorResetTimer = setTimeout(() => {
      ...
      // fire because the agent was already idle before Dobius+ restarted.
  ...
  const dropCommandFinishedStatusIfSameTurn = (
    ...
    const state = useAppStore.getState()
    if (!entry) {
      // Why: an Dobius+-started agent can exit before its first hook status. The
  ...
  const onBell = (): void => {
    ...
    // standalone BEL some time after daemon snapshot reattach, even when Dobius+
  ...
  // Why: inject DOBIUS_PANE_KEY so global Claude/Codex hooks can attribute their
  // callbacks to the correct Dobius+ pane without resolving worktrees from cwd.
  // The key matches the `${tabId}:${leafId}` composite used for cacheTimerByKey
  // and agentStatusByPaneKey. Treat it as opaque outside Dobius+.
  const state = useAppStore.getState()
  ...
  const folderWorkspace =
    parsedWorkspaceKey?.type === 'folder'
      ...
      : null
  const workspaceEnv: Record<string, string> = { DOBIUS_WORKSPACE_ID: deps.worktreeId }
  if (folderWorkspace) {
    workspaceEnv.DOBIUS_PROJECT_GROUP_ID = folderWorkspace.projectGroupId
    workspaceEnv.DOBIUS_WORKSPACE_ROOT = folderWorkspace.folderPath
  }
  const paneIdentityEnv = {
    ...workspaceEnv,
    DOBIUS_PANE_KEY: cacheKey,
    DOBIUS_TAB_ID: deps.tabId,
    DOBIUS_WORKTREE_ID: deps.worktreeId,
  ...
  const transportOptions = {
    ...
    onAgentExited,
    // Why: local IPC terminals are now model-owned in main: DobiusRuntimeService
  ...
  const runDeferredConnect = (): void => {
    ...
    const buildColdRestoreAgentResumeStartup = (): ColdRestoreAgentResumeStartup | null => {
        ...
        if (fallbackLaunchConfig?.agentCommand?.trim()) {
          ...
          return {
            ...
            env: {
              ...
              DOBIUS_AGENT_LAUNCH_TOKEN: freshLaunchToken
            },
            launchConfig: {
      ...
      return {
        ...
        env: {
          ...startupPlan.env,
          DOBIUS_AGENT_LAUNCH_TOKEN: coldRestoreLaunchToken
        },
    ...
    const mergeStartupEnvWithPaneIdentity = (
      ...
      env
        ? {
            ...env,
            ...paneIdentityEnv,
            ...(env.DOBIUS_AGENT_LAUNCH_TOKEN
              ? { DOBIUS_AGENT_LAUNCH_TOKEN: env.DOBIUS_AGENT_LAUNCH_TOKEN }
              : {})
          }
        : undefined
    ...
    const schedulePendingStartupCommandDelivery = (): void => {
      ...
      if (!sshStartupShellReady) {
        if (sshShellReadyFallbackTimer === null) {
          // Why: some SSH shells cannot emit Dobius+'s ready marker. Prefer the
