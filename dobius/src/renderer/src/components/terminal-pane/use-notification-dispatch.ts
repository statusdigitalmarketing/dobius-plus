...
import {
  isDobiusWindowForegroundFocused,
...
export function dispatchTerminalNotification(
  ...
  if (event.source === 'agent-task-complete') {
    ...
    const shouldMarkUnread = event.paneKey
      ...
      : state.activeWorktreeId !== worktreeId || !isDobiusWindowForegroundFocused()
    if (shouldMarkUnread) {
      // Why: activeWorktreeId is only in-app selection. If Dobius+ is backgrounded,
