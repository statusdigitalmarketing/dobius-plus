...
describe('createIpcPtyTransport', () => {
  ...
  it('suppresses the SSH-not-active toast for a runtime-owned (per-workspace-env) target', async () => {
    ...
    const spawnMock = vi
      .fn()
      .mockRejectedValue(new Error('No PTY provider for connection runtime-ssh-dobius-1'))
    ;(globalThis as { window: typeof window }).window = {
      ...originalWindow,
      api: {
        ...originalWindow?.api,
    ...
    const onError = vi.fn()
    await createIpcPtyTransport({ connectionId: 'runtime-ssh-dobius-1' }).connect({
      url: '',
      callbacks: { onError }
    })
