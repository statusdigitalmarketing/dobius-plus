...
export default function TerminalPane({
  ...
  // when Dobius+ launched a *supported* agent here or one was detected live for the
  ...
  const requestOpenLinksInAppPreference = useCallback(
    (url: string): Promise<boolean> | null => {
      ...
      const preferencePromise = (async () => {
        const openInDobius = await requestLinkRoutingPreference({
          openLinksInAppDefault: settingsRef.current?.openLinksInApp === true,
          url
        })
        await updateSettings({
          openLinksInApp: openInDobius,
  ...
  useEffect(() => {
    ...
    if (
      !isHostAuthoritativeLayout({
        isWebClient: !!(globalThis as { __DOBIUS_WEB_CLIENT__?: boolean }).__DOBIUS_WEB_CLIENT__,
  ...
  useTerminalKeyboardShortcuts({
    ...
    macOptionAsAltRef,
    keybindings,
    terminalShortcutPolicy: settings?.terminalShortcutPolicy ?? 'dobius-first'
  })
  ...
  useEffect(() => {
    if (
      !(globalThis as { __DOBIUS_WEB_CLIENT__?: boolean }).__DOBIUS_WEB_CLIENT__ ||
      !isVisible ||
      !isActive
    ) {
      return
  ...
  const terminalContainerStyle: CSSProperties = {
    ...
    overflow: 'hidden',
    ...hiddenStartupStyle,
    ['--dobius-terminal-divider-color' as string]:
      effectiveAppearance?.dividerColor ?? DEFAULT_TERMINAL_DIVIDER_DARK,
    ['--dobius-terminal-divider-color-strong' as string]: normalizeColor(
