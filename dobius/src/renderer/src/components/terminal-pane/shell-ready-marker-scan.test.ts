...
describe('scanForShellReadyMarker', () => {
  it('strips the marker and preserves surrounding output', () => {
    ...
    expect(scanForShellReadyMarker(state, 'before \x1b]777;dobius-shell-ready\x07 after')).toEqual({
      output: 'before  after',
      matched: true
    })
  })
  ...
  it('matches markers split across chunks', () => {
    ...
    expect(scanForShellReadyMarker(state, 'before \x1b]777;dobius')).toEqual({
      output: 'before ',
      matched: false
    })
  ...
  it('flushes marker-like output when the full marker is not BEL-terminated', () => {
    ...
    expect(scanForShellReadyMarker(state, 'before \x1b]777;dobius-shell-readyx')).toEqual({
