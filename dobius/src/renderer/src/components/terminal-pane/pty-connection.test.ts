...
describe('connectPanePty', () => {
  ...
  beforeEach(() => {
    ...
    mockStoreState = {
      ...
      repos: [{ id: 'repo1', connectionId: null, displayName: 'dobius' }],
      projects: [],
  ...
  it('observes live terminal GitHub PR URLs before agent completion', async () => {
    ...
    capturedDataCallback.current?.('Created https://github.com/acme/dobius/pull/42\r\n')

    expect(mockStoreState.observeTerminalGitHubPullRequestLink).toHaveBeenCalledWith(
      'wt-1',
      expect.objectContaining({
        url: 'https://github.com/acme/dobius/pull/42',
        slug: { owner: 'acme', repo: 'dobius' },
        number: 42
      })
    )
  })
  ...
  it('waits for the SSH shell-ready marker before sending hinted startup commands', async () => {
    ...
    try {
      ...
      capturedDataCallback.current?.('\x1b]777;dobius-shell-ready\x07user@remote $ ')
      for (const fn of pendingTimeouts.splice(0)) {
        fn()
      }
  ...
  it('pastes a startup draft when Codex renders its composer in the first observed output', async () => {
    ...
    const deps = createDeps({
      startup: {
        ...
        draftPrompt: 'https://github.com/statusdigitalmarketing/dobius-plus/issues/42'
      }
    })
    ...
    expect(window.api.pty.writeAccepted).toHaveBeenCalledWith(
      'pty-codex',
      '\x1b[200~https://github.com/statusdigitalmarketing/dobius-plus/issues/42\x1b[201~'
    )
  })
  ...
  it('clears pre-hook launch config when an Dobius-started command exits', async () => {
    const { connectPanePty } = await import('./pty-connection')
      ...
      expect(transport.connect).toHaveBeenNthCalledWith(
        ...
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            ...
            DOBIUS_WORKSPACE_ID: 'wt-1',
            DOBIUS_AGENT_LAUNCH_TOKEN: expect.stringMatching(new RegExp(`^${UUID_RE}$`))
          })
        })
      )
  ...
  it('cold-restores a fresh agent from the sleeping record when its session is unresumable', async () => {
    ...
    expect(reattach?.env?.DOBIUS_AGENT_LAUNCH_TOKEN).toMatch(new RegExp(`^${UUID_RE}$`))
  })
  ...
  it('cold-restores a fresh agent with the sleeping record launch config when resume is unavailable', async () => {
      ...
      sleepingAgentSessionsByPaneKey: {
        [paneKey]: {
          ...
          launchConfig: {
            ...
            agentEnv: { CODEX_HOME: '/tmp/dobius-stored-codex' }
          }
        }
      }
    } as StoreState
    ...
    expect(reattach?.env).toMatchObject({
      CODEX_HOME: '/tmp/dobius-stored-codex',
      DOBIUS_AGENT_LAUNCH_TOKEN: expect.stringMatching(new RegExp(`^${UUID_RE}$`))
    })
  })
  ...
  it('uses the known SSH remote platform when fresh-launching an unresumable agent', async () => {
    ...
    const reattach = findReattachConnect(transport)
    expect(reattach?.command).toMatch(/^dobius\.cmd claude-teams/)
    expect(reattach?.command).not.toContain('dobius-ide')
  ...
  it('uses sleeping-record launch config for pane cold restore after settings change', async () => {
    ...
    const launchConfig = {
      ...
      agentEnv: {
        ...
        DOBIUS_PANE_KEY: 'wrong-pane',
        DOBIUS_TAB_ID: 'wrong-tab',
        DOBIUS_WORKTREE_ID: 'wrong-worktree',
        DOBIUS_WORKSPACE_ID: 'wrong-workspace'
      }
    }
    mockStoreState = {
      ...mockStoreState,
  ...
  it('does not retry remote snapshots for ordinary hidden runtime output', async () => {
    ...
    expect(transport.serializeBuffer).not.toHaveBeenCalled()
    expect(pane.terminal.write).not.toHaveBeenCalledWith(
      expect.stringContaining('Dobius skipped hidden terminal output'),
      expect.any(Function)
    )
    expect(pane.terminal.write).not.toHaveBeenCalledWith(
    ...
    try {
      ...
      expect(pane.terminal.write).toHaveBeenCalledWith(
        expect.stringContaining(
          'Dobius skipped hidden terminal output because main recovery was unavailable.'
        ),
        expect.any(Function)
      )
      expect(pane.terminal.write).toHaveBeenCalledWith(
  ...
  it('does not schedule WebGL atlas recovery for ordinary foreground shell rewrites', async () => {
    ...
    try {
      ...
      capturedDataCallback.current?.('\r\x1b[Kdobius % npm test')
  ...
  it('applies the Windows CJK repaint path to SSH panes on Windows clients after terminal input', async () => {
    ...
    try {
      ...
      mockStoreState = {
        ...mockStoreState,
        repos: [{ id: 'repo1', connectionId: 'conn-1', displayName: 'dobius' }]
      }
