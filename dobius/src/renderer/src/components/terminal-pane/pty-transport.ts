...
export function createPtyOutputProcessor({
  ...
  function applyObservedTerminalTitle(title: string, suppressAgentTracker = false): void {
    ...
    // even while it's actively working. Dobius+ drives the cursor spinner/unread
  ...
  function scheduleSideEffectDrain(): void {
    ...
    // Why: xterm.write() buffers parsing onto its own timer. Defer Dobius+'s
  ...
  function processData(
    ...
    // Why: OSC 9999 is an Dobius+ control protocol. Parse it before xterm sees
...
export function createIpcPtyTransport(opts: IpcPtyTransportOptions = {}): PtyTransport {
  ...
  // unread marks, or notifications for unrelated worktrees just because Dobius+
