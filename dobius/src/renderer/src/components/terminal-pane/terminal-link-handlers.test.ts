...
describe('handleOscLink', () => {
  ...
  it('uses the system browser for shift+cmd/ctrl+click even when Dobius browser tabs are enabled', () => {
    setPlatform('Windows')
    storeState.settings = { openLinksInApp: true }
  ...
  it('opens local .html file paths in Dobius browser tabs with the platform modifier', async () => {
    setPlatform('Macintosh')
  ...
  it('also opens local .htm paths in Dobius browser tabs with the platform modifier', async () => {
    setPlatform('Macintosh')
  ...
  it('opens local file paths in Dobius and reveals default column 1 with the platform modifier', async () => {
    setPlatform('Macintosh')
  ...
  it('preserves explicit column for Dobius opens from :line:column links', async () => {
    setPlatform('Macintosh')
  ...
  it('falls back to Dobius when shift+cmd/ctrl-click system default open fails', async () => {
    setPlatform('Macintosh')
  ...
  it('opens local file URL links in Dobius with the platform modifier on desktop', async () => {
    setPlatform('Windows')
  ...
  it('opens #L file URL links in Dobius and preserves anchors', async () => {
    setPlatform('Macintosh')
  ...
  it('opens SSH file links through Dobius without local authorization', async () => {
    setPlatform('Macintosh')
  ...
  it('ignores stale async completion so latest local click wins for Dobius open and reveal', async () => {
    setPlatform('Macintosh')
...
describe('createFilePathLinkProvider range bounds', () => {
  ...
  it('shows the Dobius plus default-app hint for local file link hover', async () => {
    setPlatform('Macintosh')
  ...
  it('shows the Dobius hint for SSH file link hover', async () => {
    setPlatform('Macintosh')
    vi.mocked(getConnectionId).mockReturnValue('ssh-1')
    const { provider, linkTooltip } = createProviderSetup([makeBufferLine('CLAUDE.md')])
    ...
    expect(linkTooltip.textContent).toBe('/repo/CLAUDE.md (⌘+click to open in Dobius)')
  })
  ...
  it('retries a wrapped file click even when xterm already marked the link active', async () => {
    setPlatform('Macintosh')
    const rows = [
      makeBufferLine('/private/tmp/dobius-setup-e2e.hOW01f/workspaces/test-wt-5/mobile/'),
    ...
    const disposable = installFilePathLinkClickFallback(1, terminal, {
      startupCwd: '/private/tmp/dobius-setup-e2e.hOW01f/workspaces/test-wt-5',
      worktreeId: 'wt-1',
      worktreePath: '/private/tmp/dobius-setup-e2e.hOW01f/workspaces/test-wt-5',
      runtimeEnvironmentId: null,
    ...
    expect(openFilePathMock).toHaveBeenCalledWith(
      '/private/tmp/dobius-setup-e2e.hOW01f/workspaces/test-wt-5/mobile/packages/expo-two-way-audio/android/src/main/java/expo/modules/twowayaudio/ExpoTwoWayAudioLifeCycleListener.kt'
    )
    expect(preventDefault).toHaveBeenCalled()
  ...
  it('does not intercept regular URL clicks in the file-path fallback', async () => {
    setPlatform('Macintosh')
    const rows = [
      makeBufferLine('PR opened: https://github.com/statusdigitalmarketing/dobius-plus-marketing-website/pull/82')
    ]
    const { terminal, element } = makeFallbackTerminal(rows)
    const disposable = installFilePathLinkClickFallback(1, terminal, {
      startupCwd: '/tmp',
  ...
  it('opens regular URLs from a direct modifier-click fallback when xterm did not handle them', async () => {
    ...
    expect(openUrlMock).toHaveBeenCalledWith(
      'https://github.com/statusdigitalmarketing/dobius-plus-marketing-website/pull/82'
    )
    expect(preventDefault).toHaveBeenCalled()
    expect(stopPropagation).not.toHaveBeenCalled()
    expect(terminal.clearSelection).toHaveBeenCalled()
  ...
  it('does not steal macOS ctrl-click context-menu gestures in the URL fallback', async () => {
    setPlatform('Macintosh')
    storeState.settings = { openLinksInApp: false }
    const rows = [makeBufferLine('Open https://github.com/statusdigitalmarketing/dobius-plus/pull/2914')]
    const { terminal, element } = makeFallbackTerminal(rows)
    const disposable = installHttpLinkClickFallback(terminal, { worktreeId: 'wt-1' })
    const mouseUp = getRegisteredBubbleMouseUpHandler(element)
    const preventDefault = vi.fn()
