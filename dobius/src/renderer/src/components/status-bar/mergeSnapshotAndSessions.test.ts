...
describe('mergeSnapshotAndSessions', () => {
  ...
  it('passes through snapshot worktrees with numeric metrics and hasLocalSamples', () => {
    const wt: WorktreeMemory = {
      worktreeId: 'dobius::/Users/me/Triton',
      worktreeName: 'Triton',
      repoId: 'dobius',
      repoName: 'DOBIUS',
      cpu: 1.5,
  ...
  it('@@ parse: an SSH-style session id resolves to its worktree group', () => {
    const ds: DaemonSession[] = [
      { id: 'dobius::/remote/Stingray@@abcd1234', cwd: '', title: 'dobius/Stingray' }
    ]
    const ctx = baseCtx({
      repoConnectionIdById: new Map([['dobius', 'ssh-conn-1']])
    })
    ...
    expect(out[0].worktrees[0]).toMatchObject({
      worktreeId: 'dobius::/remote/Stingray',
    ...
    expect(out[0].worktrees[0].sessions[0]).toMatchObject({
      sessionId: 'dobius::/remote/Stingray@@abcd1234',
      hasLocalSamples: false,
      cpu: null,
      memory: null,
      bound: false
  ...
  it('warm-reattach local PTY: chip stays off when repo has no connectionId', () => {
    ...
    const ds: DaemonSession[] = [
      { id: 'dobius::/local/Triton@@deadbeef', cwd: '/local/Triton', title: 'dobius/Triton' }
    ]
    const ctx = baseCtx({
      repoConnectionIdById: new Map([['dobius', null]])
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0]).toMatchObject({
      repoId: 'dobius',
  ...
  it('tab walk wins over @@ parse when they disagree', () => {
    const tabId = 'tab-xyz'
    const ds: DaemonSession[] = [{ id: 'dobius::/wrong/path@@feedface', cwd: '', title: 'dobius' }]
    const ctx = baseCtx({
      tabsByWorktree: {
        'dobius::/correct/path': [makeTab(tabId, 'My Tab')]
      },
      ptyIdsByTabId: { [tabId]: ['dobius::/wrong/path@@feedface'] }
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0].worktrees[0].worktreeId).toBe('dobius::/correct/path')
  ...
  it('treats startup deferred reattach tab ptyId wake hints as bound sessions', () => {
    ...
    const sessionId = 'dobius::/Users/me/Triton@@deferred'
    const ds: DaemonSession[] = [{ id: sessionId, cwd: '/Users/me/Triton', title: 'dobius/Triton' }]
    const restoredTab = { ...makeTab(tabId, 'Restored'), ptyId: sessionId }
    const ctx = baseCtx({
      tabsByWorktree: {
        'dobius::/Users/me/Triton': [restoredTab]
      },
      ptyIdsByTabId: { [tabId]: [] }
    })
  ...
  it('local-bound interaction state: numeric metrics + bound=true + tabId set', () => {
    ...
    const ctx = baseCtx({
      tabsByWorktree: { 'dobius::/Users/me/Triton': [makeTab(tabId)] },
  ...
  it('remote-orphan interaction state: null metrics + bound=false', () => {
    const ds: DaemonSession[] = [{ id: 'dobius::/remote/Wt@@deadbeef', cwd: '', title: 'dobius/Wt' }]
    const out = mergeSnapshotAndSessions(null, ds, baseCtx())
    const session = out[0].worktrees[0].sessions[0]
    expect(session).toMatchObject({
      hasLocalSamples: false,
  ...
  it('uses repoDisplayNameById to humanize new project groups when available', () => {
    const ds: DaemonSession[] = [{ id: 'stably-ai/dobius::/remote/Wt@@1', cwd: '', title: '' }]
    const ctx = baseCtx({
      repoDisplayNameById: new Map([['stably-ai/dobius', 'DOBIUS']])
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0].repoName).toBe('DOBIUS')
  })
