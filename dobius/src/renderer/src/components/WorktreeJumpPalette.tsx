  ...
  useEffect(() => {
    if (visible && !wasVisibleRef.current) {
      ...
      previousBrowserFocusTargetRef.current =
        ...
        document.activeElement.closest('[data-dobius-browser-address-bar="true"]')
          ? 'address-bar'
          : 'webview'
  ...
  const requestBrowserFocus = useCallback(
    (detail: { pageId: string; target: 'webview' | 'address-bar' }) => {
      ...
      window.dispatchEvent(
        new CustomEvent(DOBIUS_BROWSER_FOCUS_REQUEST_EVENT, {
          detail
        })
      )
    },
  ...
  const emptyState = (() => {
    ...
    return {
      ...
      subtitle: translate(
        ...
        'Create a worktree or open a tab in Dobius+ to get started.'
      )
    }
  })()
