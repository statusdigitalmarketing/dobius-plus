...
import { DOBIUS_BROWSER_BLANK_URL } from '../../../../shared/constants'
...
function formatBrowserTabUrlLabel(url: string): string {
  if (url === DOBIUS_BROWSER_BLANK_URL || url === 'about:blank') {
    return 'New Tab'
  }
  try {
...
export function getBrowserTabLabel(tab: BrowserTabState): string {
  if (
    !tab.title ||
    tab.title === tab.url ||
    tab.title === DOBIUS_BROWSER_BLANK_URL ||
...
function isBlankBrowserTab(tab: BrowserTabState): boolean {
  return tab.url === DOBIUS_BROWSER_BLANK_URL || tab.url === 'about:blank'
}
