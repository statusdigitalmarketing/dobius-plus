...
// Why: run-notification delivery extracts the NOTIFY: block for phone-sized
// channels, so stock prompts must teach the agent to write one.
function notifySuffix(): string {
  return ` ${translate(
    'auto.components.automations.automation.templates.notifySuffix',
    'End your final message with a line starting "NOTIFY:" followed by a 1-3 sentence plain-text summary of the outcome, written for a phone notification.'
  )}`
}
