...
import type { DobiusHooks, Repo, Worktree } from '../../../../shared/types'
...
const AUTOMATIONS_CHANGED_EVENT = 'dobius:automations-changed'
...
export default function AutomationsPage(): React.JSX.Element {
  ...
  const [createTarget, setCreateTarget] = useState<AutomationCreateTarget>('dobius')
  ...
  const automationHookCheckPromisesRef = useRef<
    Map<string, Promise<{ hooks: DobiusHooks | null; ok: boolean }>>
  >(new Map())
  const [automationYamlHooksByRepoKey, setAutomationYamlHooksByRepoKey] = useState<
    Record<string, DobiusHooks | null>
  >({})
  ...
  const loadAutomationYamlHooksForRepo = useCallback(
    async (repoId: string): Promise<DobiusHooks | null> => {
      ...
      const promise = checkRuntimeHooks(settingsForRepo, repoId)
        .then((result) => ({
          hooks: result.status === 'error' ? null : ((result.hooks as DobiusHooks | null) ?? null),
  ...
  const openEditDialog = async (automation: Automation): Promise<void> => {
    ...
    const nextDraft: AutomationDraft = {
      ...
      notifyDestinationId: latest.notification?.destinationId ?? '',
      notifyOn: latest.notification?.notifyOn ?? 'always',
      notifyDepth: latest.notification?.depth ?? 'brief'
    }
    setDraft(nextDraft)
    setDraftAtOpen(nextDraft)
    setCreateOpen(true)
  ...
  const saveAutomation = async (): Promise<void> => {
    ...
    try {
      ...
      let setupDecision = resolveAutomationSetupDecisionForSave({
        ...
        projectHostSetups,
        yamlHooks:
          createTarget === 'dobius' && draft.workspaceMode === 'new_per_run'
      ...
      const notification: AutomationNotification | null = draft.notifyDestinationId
        ? {
            destinationId: draft.notifyDestinationId,
            notifyOn: draft.notifyOn,
            depth: draft.notifyDepth
          }
        : null
