...
function getAutomationSetupSource(
  ...
  yamlHooks: DobiusHooks | null | undefined
...
export function getVisibleAutomationSetupDecision(args: {
  ...
  if (args.createTarget !== 'dobius' || args.workspaceMode !== 'new_per_run') {
    return undefined
  }
...
export function resolveAutomationSetupDecisionForSave(args: {
  ...
  if (args.yamlHooks === undefined) {
    // Why: automations cannot pause later for an dobius.yaml trust prompt; when
