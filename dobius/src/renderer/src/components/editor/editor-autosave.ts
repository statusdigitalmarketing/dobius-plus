...
export const DOBIUS_EDITOR_QUIESCE_FILE_SAVES_EVENT = 'dobius:editor-quiesce-file-saves'
export const DOBIUS_EDITOR_EXTERNAL_FILE_CHANGE_EVENT = 'dobius:editor-external-file-change'
export const DOBIUS_EDITOR_SAVE_FILE_EVENT = 'dobius:editor-save-file'
export const DOBIUS_EDITOR_SAVE_AND_CLOSE_EVENT = 'dobius:save-and-close'
export const DOBIUS_EDITOR_FILE_SAVED_EVENT = 'dobius:editor-file-saved'
export const DOBIUS_EDITOR_REQUEST_CMD_SAVE_EVENT = 'dobius:editor-request-cmd-save'
export const DOBIUS_EDITOR_REQUEST_FILE_CLOSE_EVENT = 'dobius:editor-request-file-close'
...
export async function requestEditorSaveQuiesce(target: EditorSaveQuiesceTarget): Promise<void> {
  await new Promise<void>((resolve) => {
    ...
    window.dispatchEvent(
      new CustomEvent<EditorSaveQuiesceDetail>(DOBIUS_EDITOR_QUIESCE_FILE_SAVES_EVENT, {
        detail: {
          ...target,
          claim: () => {
            claimed = true
...
export async function requestEditorFileSave(target: EditorSaveFileTarget): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    let claimed = false
    window.dispatchEvent(
      new CustomEvent<EditorSaveFileDetail>(DOBIUS_EDITOR_SAVE_FILE_EVENT, {
        detail: {
          ...target,
          claim: () => {
            claimed = true
...
export function requestEditorFileClose(fileId: string): void {
  window.dispatchEvent(
    new CustomEvent<EditorRequestFileCloseDetail>(DOBIUS_EDITOR_REQUEST_FILE_CLOSE_EVENT, {
      detail: { fileId }
    })
  )
}
...
export function notifyEditorExternalFileChange(target: EditorPathMutationTarget): void {
  window.dispatchEvent(
    new CustomEvent<EditorPathMutationTarget>(DOBIUS_EDITOR_EXTERNAL_FILE_CHANGE_EVENT, {
      detail: target
    })
  )
}
