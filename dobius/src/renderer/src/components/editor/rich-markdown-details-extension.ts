...
const DobiusDetails = Details.extend({
  ...
  addAttributes() {
    return {
      ...
      variant: {
        default: null,
        parseHTML: (element) =>
          element.getAttribute('data-dobius-toggle') === 'heading-1' ? 'heading-1' : null,
        renderHTML: ({ variant }) =>
          variant === 'heading-1' ? { 'data-dobius-toggle': 'heading-1' } : {}
      }
    }
  },
...
const DobiusDetailsContent = DetailsContent.extend({
...
export function createDobiusDetailsExtensions(): AnyExtension[] {
  return [
    DobiusDetails.configure({
      persist: true,
      HTMLAttributes: {
        class: 'dobius-details'
      }
    }),
    DetailsSummary,
    DobiusDetailsContent
  ]
}
