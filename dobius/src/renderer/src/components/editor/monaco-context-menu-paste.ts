...
// reads the clipboard with navigator.clipboard.readText(). Dobius+ runs the renderer with
...
// (execCommand('copy') from a user gesture) both work. We route the read through Dobius+'s
// trusted clipboard IPC bridge instead, matching how the terminal already reads it.
export const DOBIUS_CONTEXT_MENU_PASTE_PRIORITY = 10001
export const DOBIUS_CONTEXT_MENU_PASTE_NAME = 'dobius-ipc-paste'
...
export type DobiusContextMenuPasteDeps = {
...
export type DobiusContextMenuPasteOutcome =
 ...
 * bridge); otherwise performs the paste through Dobius+'s IPC clipboard read and
 * returns a Promise (truthy) so Monaco's blocked default never runs.
 */
export function runDobiusContextMenuPaste(
  deps: DobiusContextMenuPasteDeps
): false | Promise<DobiusContextMenuPasteOutcome> {
  ...
  return performDobiusContextMenuPaste(editorInstance, deps)
}

async function performDobiusContextMenuPaste(
  editorInstance: PasteCapableEditor,
  deps: DobiusContextMenuPasteDeps
): Promise<DobiusContextMenuPasteOutcome> {
  let text: string
  try {
