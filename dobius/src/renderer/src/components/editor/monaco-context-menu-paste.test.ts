...
import { runDobiusContextMenuPaste, type DobiusContextMenuPasteDeps } from './monaco-context-menu-paste'
...
function makeDeps(
  overrides: Partial<DobiusContextMenuPasteDeps> & { editor?: ReturnType<typeof makeEditor> }
): DobiusContextMenuPasteDeps {
...
describe('runDobiusContextMenuPaste', () => {
  ...
  it('reads via IPC and dispatches a native paste for writable focused editors', async () => {
    ...
    const outcome = runDobiusContextMenuPaste(deps)
  ...
  it('applies in-app copy metadata (empty-selection + multicursor) when enabled', async () => {
    ...
    await runDobiusContextMenuPaste(deps)
    expect(handle.trigger).toHaveBeenCalledWith('keyboard', 'paste', {
      text: 'line text',
      pasteOnNewLine: true,
      multicursorText: ['a', 'b'],
  ...
  it('does nothing on an empty clipboard', async () => {
    ...
    await expect(runDobiusContextMenuPaste(deps)).resolves.toEqual({
      status: 'noop',
      reason: 'empty'
    })
  ...
  it('routes oversized pastes through the chunked inserter instead of a native trigger', async () => {
    ...
    const outcome = await runDobiusContextMenuPaste(deps)
