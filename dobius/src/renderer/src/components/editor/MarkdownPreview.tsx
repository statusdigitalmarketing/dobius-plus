...
const markdownPreviewSanitizeSchema = {
  ...
  protocols: {
    ...
    // user-selected path before opening it in Dobius+. Sanitization must preserve
  ...
  attributes: {
    ...
    details: [
      ...
      'open',
      ['className', 'dobius-details'],
      ['dataDobiusToggle', 'heading-1']
    ],
      ...
      a: ({ href, children, className, ...props }) => {
        ...
        const handleClick = async (event: React.MouseEvent<HTMLAnchorElement>): Promise<void> => {
          ...
          if (target.protocol === 'http:' || target.protocol === 'https:') {
            ...
            // click honors the "open links in Dobius+" setting; openHttpLink keeps
          ...
          if (!targetWorktree) {
            if (sourceRoutingWorktreeId && worktreeRoot) {
              ...
              // so there may be no repo worktree even though Dobius+ can stat/open
...
function MarkdownSingleNoteSendMenu({
  ...
  return (
    <NotesSendMenu
      ...
      triggerClassName="dobius-diff-comment-pill-btn"
...
function MarkdownAnnotationComposer({
  ...
  return (
    <div className="markdown-annotation-composer" onClick={(event) => event.stopPropagation()}>
      <div className="dobius-diff-comment-popover-label">
