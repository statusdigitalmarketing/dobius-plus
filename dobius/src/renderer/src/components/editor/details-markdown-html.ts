...
export function parseDetailsAttributes(rawAttributes: string): Record<string, unknown> {
  return {
    ...
    variant: /\sdata-dobius-toggle=(?:"heading-1"|'heading-1'|heading-1)(?:\s|$)/i.test(rawAttributes)
      ? 'heading-1'
      : null
  }
}
...
export function renderDetailsAttributes(attrs: Record<string, unknown> | undefined): string {
  const attributes = ['class="dobius-details"']

  if (attrs?.variant === 'heading-1') {
    attributes.push('data-dobius-toggle="heading-1"')
  }
...
function hasOnlySupportedDetailsAttributes(rawAttributes: string): boolean {
  return (
    rawAttributes
      ...
      .replace(/\s+class\s*=\s*(?:"dobius-details"|'dobius-details'|dobius-details)(?=\s|$)/giu, '')
      .replace(/\s+data-dobius-toggle\s*=\s*(?:"heading-1"|'heading-1'|heading-1)(?=\s|$)/giu, '')
      .trim() === ''
  )
}
