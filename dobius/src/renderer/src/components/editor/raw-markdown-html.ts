...
const INLINE_PLACEHOLDER_PREFIX = '[[DOBIUS_RAW_HTML_INLINE:'
const BLOCK_PLACEHOLDER_PREFIX = '[[DOBIUS_RAW_HTML_BLOCK:'
const DOC_LINK_PLACEHOLDER_PREFIX = '[[DOBIUS_DOC_LINK:'
...
export function encodeRawMarkdownHtmlForRichEditor(content: string): string {
  ...
  while (index < normalizedContent.length) {
    ...
    // skipped by the guards above. The [[DOBIUS_ prefix check prevents re-encoding
    // sibling placeholders that were already emitted earlier in this pass.
    if (
      normalizedContent[index] === '[' &&
      normalizedContent[index + 1] === '[' &&
      !normalizedContent.startsWith('[[DOBIUS_', index) &&
...
export const RawMarkdownHtmlInline = Node.create({
  ...
  // while still letting Dobius+ round-trip the raw markup verbatim.
