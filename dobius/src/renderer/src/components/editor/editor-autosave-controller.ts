...
import {
  ...
  DOBIUS_EDITOR_EXTERNAL_FILE_CHANGE_EVENT,
  DOBIUS_EDITOR_FILE_SAVED_EVENT,
  DOBIUS_EDITOR_QUIESCE_FILE_SAVES_EVENT,
  DOBIUS_EDITOR_SAVE_AND_CLOSE_EVENT,
  DOBIUS_EDITOR_SAVE_FILE_EVENT,
...
import {
  ...
  DOBIUS_EDITOR_SAVE_DIRTY_FILES_EVENT,
...
export function attachEditorAutosaveController(store: AppStoreApi): () => void {
    ...
    const queuedSave = previousSave
      ...
      .then(async () => {
        ...
        window.dispatchEvent(
          new CustomEvent<EditorFileSavedDetail>(DOBIUS_EDITOR_FILE_SAVED_EVENT, {
  ...
  window.addEventListener(DOBIUS_EDITOR_SAVE_DIRTY_FILES_EVENT, handleSaveDirtyFiles as EventListener)
  window.addEventListener(DOBIUS_EDITOR_PREPARE_HOT_EXIT_EVENT, handlePrepareHotExit as EventListener)
  window.addEventListener(DOBIUS_EDITOR_SAVE_AND_CLOSE_EVENT, handleSaveAndClose as EventListener)
  window.addEventListener(DOBIUS_EDITOR_SAVE_FILE_EVENT, handleSaveFile as EventListener)
  window.addEventListener(DOBIUS_EDITOR_QUIESCE_FILE_SAVES_EVENT, handleQuiesce as EventListener)
  ...
  return () => {
    ...
    window.removeEventListener(DOBIUS_EDITOR_SAVE_FILE_EVENT, handleSaveFile as EventListener)
    window.removeEventListener(DOBIUS_EDITOR_QUIESCE_FILE_SAVES_EVENT, handleQuiesce as EventListener)
    window.removeEventListener(
      DOBIUS_EDITOR_EXTERNAL_FILE_CHANGE_EVENT,
      handleExternalFileChange as EventListener
    )
