...
import { DOBIUS_EDITOR_REQUEST_CMD_SAVE_EVENT } from './editor-autosave'
...
export function useEditorCmdSaveRequest({
  ...
  useEffect(() => {
    ...
    window.addEventListener(DOBIUS_EDITOR_REQUEST_CMD_SAVE_EVENT, handler)
    return () => window.removeEventListener(DOBIUS_EDITOR_REQUEST_CMD_SAVE_EVENT, handler)
