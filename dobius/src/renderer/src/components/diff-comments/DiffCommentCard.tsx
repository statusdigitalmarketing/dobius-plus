...
export function DiffCommentCard({
  ...
  return (
    <div className="dobius-diff-comment-card">
      <div className="dobius-diff-comment-content-col">
        {/* Header Row */}
        <div className="dobius-diff-comment-header">
          <div className="dobius-diff-comment-meta-group">{metaText}</div>

          {/* Action buttons pill (only shown if not editing) */}
          {!editing && (
            <div
              className="dobius-diff-comment-actions-pill"
              onMouseDown={(ev) => ev.stopPropagation()}
            >
              {headerActions}
              {headerActions && (url || onSubmitEdit || onDelete) && (
                <span className="dobius-diff-comment-pill-divider" />
              )}
              {url && (
                <>
                  <button
                    type="button"
                    className="dobius-diff-comment-pill-btn"
                    title={translate(
              ...
              {onSubmitEdit && (
                <>
                  ...
                  {onDelete && <span className="dobius-diff-comment-pill-divider" />}
                </>
              )}
              {onDelete && (
                <button
                  type="button"
                  className="dobius-diff-comment-pill-btn dobius-diff-comment-pill-btn-danger"
                  title={translate(
        ...
        {/* Quote Block */}
        {quote ? (
          <div className="dobius-diff-comment-quote">
            <div className="dobius-diff-comment-quote-text">{quote}</div>
          </div>
        ) : null}
        ...
        {editing ? (
          <div className="flex flex-col gap-2 mt-1">
            <textarea
              ref={textareaRef}
              className="dobius-diff-comment-popover-textarea"
              value={draft}
              onChange={(e) => {
              ...
              rows={3}
            />
            <div className="dobius-diff-comment-popover-footer">
          ...
          <div className="dobius-diff-comment-body">{body}</div>
        )}
      </div>
    </div>
  )
