...
const projects: NewWorkspaceProjectOption[] = [
  ...
  {
    kind: 'project',
    id: 'github:statusdigitalmarketing/noqa',
    projectId: 'github:statusdigitalmarketing/noqa',
    displayName: 'noqa',
    badgeColor: '#222222',
    detail: 'statusdigitalmarketing/noqa'
  },
  {
    kind: 'project-group',
...
describe('ProjectCombobox', () => {
  it('renders a logical project label without host-specific SSH chrome', () => {
    act(() => {
      root.render(
        <ProjectCombobox options={projects} value="github:statusdigitalmarketing/dobius-plus" onValueChange={vi.fn()} />
      )
    })

    const trigger = container.querySelector('[data-project-combobox-root="true"][role="combobox"]')
    expect(trigger?.textContent).toContain('dobius')
  ...
  it('selects projects by logical project id', () => {
    ...
    act(() => {
      root.render(
        <ProjectCombobox
          options={projects}
          value="github:statusdigitalmarketing/dobius-plus"
    ...
    act(() => {
      container
        .querySelector<HTMLButtonElement>('[data-command-value="github:statusdigitalmarketing/noqa"]')
        ?.dispatchEvent(new MouseEvent('click', { bubbles: true }))
    })

    expect(onValueChange).toHaveBeenCalledWith('github:statusdigitalmarketing/noqa')
  })
