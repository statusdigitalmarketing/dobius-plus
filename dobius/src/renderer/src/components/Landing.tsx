...
export default function Landing(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    // Why: users often install/authenticate gh outside Dobius+. Re-check when the
  ...
  useEffect(() => {
    ...
    let cancelled = false
    // Why: some users complete `gh auth login` without ever leaving the Dobius+
      ...
      <div className="w-full max-w-lg px-6">
        <div className="flex flex-col items-center gap-4 py-8">
          ...
          >
            <img
              src={logo}
              alt={translate('auto.components.Landing.520304a067', 'Dobius+ logo')}
              className="size-12"
            />
          </div>
          <h1 className="text-4xl font-bold text-foreground tracking-tight">
            {translate('auto.components.Landing.6ca6ff404e', 'Dobius+')}
          </h1>
