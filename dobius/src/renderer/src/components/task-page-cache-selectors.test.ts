...
describe('task page cache selectors', () => {
  ...
  it('scopes repo source rows by source cache scope for retry ownership', () => {
    const localRepo = {
      id: 'repo-1',
      path: '/same/path',
      sourceCacheScope: 'source:local:github:statusdigitalmarketing/dobius-plus'
    }
    const sshRepo = {
      id: 'repo-1',
      path: '/same/path',
      sourceCacheScope: 'source:ssh:devbox:github:statusdigitalmarketing/dobius-plus'
    }

    expect(buildTaskPageRepoSourceState([localRepo, sshRepo], [])).toMatchObject([
      {
        sourceKey: 'repo-1::source:local:github:statusdigitalmarketing/dobius-plus'
      },
      {
        sourceKey: 'repo-1::source:ssh:devbox:github:statusdigitalmarketing/dobius-plus'
      }
    ])
  })
