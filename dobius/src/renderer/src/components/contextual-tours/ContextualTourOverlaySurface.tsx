...
if (typeof window !== 'undefined') {
  const guardedWindow = window as Window & {
    __dobiusContextualTourGlobalKeyGuardInstalled?: boolean
  }
  if (!guardedWindow.__dobiusContextualTourGlobalKeyGuardInstalled) {
    guardedWindow.__dobiusContextualTourGlobalKeyGuardInstalled = true
    window.addEventListener('keydown', handleContextualTourGlobalKeyDown, true)
  }
}

const PANEL_BASE_CLASSES =
  'dobius-contextual-tour-panel rounded-lg border border-border text-popover-foreground backdrop-blur-[2px]'
  ...
  return (
    ...
    >
      {showTargetRings ? (
        <div
          aria-hidden="true"
          className="dobius-contextual-tour-target-rings fixed z-[75]"
