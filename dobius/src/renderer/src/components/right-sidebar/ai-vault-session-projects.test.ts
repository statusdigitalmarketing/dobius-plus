...
const baseSession: AiVaultSession = {
  ...
  totalTokens: 1200,
  previewMessages: [],
  resumeCommand: "cd '/Users/ada/dobius' && claude --resume 'session-1'"
}
...
describe('buildAiVaultProjectContext', () => {
  it('uses durable worktree project ids before repo fallback', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Legacy Repo', path: '/Users/ada/dobius' })
    const project = makeProject({ id: 'project-1', displayName: 'Canonical Dobius' })
    const worktree = makeWorktree({
      id: 'wt-1',
      repoId: repo.id,
      projectId: project.id,
      path: '/Users/ada/dobius'
    })
    ...
    expect(context.sessionProjectById.get(baseSession.id)).toMatchObject({
      ...
      label: 'Canonical Dobius'
    })
  })

  it('normalizes compatibility project ids to repo keys', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Dobius', path: '/Users/ada/dobius' })
    const worktree = makeWorktree({
      id: 'wt-1',
      repoId: repo.id,
    ...
    const context = buildAiVaultProjectContext({
      ...
      worktrees: [worktree],
      projectHostSetupProjection: makeProjection({
        projects: [makeProject({ id: 'repo:repo-1', displayName: 'Compatibility Dobius' })],
    ...
    expect(context.projectLabelByKey.get('repo:repo-1')).toBe('Dobius')
  })
  ...
  it('inherits setup project ids for legacy worktrees without project metadata', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Dobius Repo', path: '/repo/dobius' })
    const worktree = makeWorktree({
      id: 'wt-legacy',
      repoId: repo.id,
      path: '/repo/dobius'
    })
    const session = makeSession({ id: 'claude:legacy-worktree', cwd: '/repo/dobius/src' })

    const context = buildAiVaultProjectContext({
      repos: [repo],
      worktrees: [worktree],
      projectHostSetupProjection: makeProjection({
        projects: [makeProject({ id: 'github:statusdigitalmarketing/dobius-plus', displayName: 'Canonical Dobius' })],
        setups: [
          makeSetup({
            repoId: repo.id,
    ...
    expect(context.activeProjectKey).toBe('project:github:statusdigitalmarketing/dobius-plus')
    expect(context.sessionProjectById.get(session.id)).toMatchObject({
      kind: 'repo',
  ...
  it('uses active worktree setup project ids when active repo is unavailable', () => {
    ...
    const session = makeSession({ id: 'claude:restored', cwd: '/repo/dobius/src' })
    ...
    expect(context.activeProjectKey).toBe('project:github:statusdigitalmarketing/dobius-plus')
    expect(context.sessionProjectById.get(session.id)?.key).toBe('project:github:statusdigitalmarketing/dobius-plus')
  })

  it('inherits setup host ids for legacy worktrees without host metadata', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Runtime Repo', path: '/runtime/dobius' })
    const worktree = makeWorktree({
      id: 'wt-runtime',
      repoId: repo.id,
      path: '/runtime/dobius'
  ...
  it('uses the session host when matching overlapping local and SSH project paths', () => {
    const localRepo = makeRepo({ id: 'local', displayName: 'Local', path: '/srv/dobius' })
  ...
  it('falls back to folder when a legacy hostless session matches multiple host buckets', () => {
    const localRepo = makeRepo({ id: 'local', displayName: 'Local', path: '/srv/dobius' })
    const runtimeRepo = makeRepo({ id: 'runtime', displayName: 'Runtime', path: '/srv/dobius' })
    const session = makeSession({
  ...
  it('maps null cwd sessions to unknown', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Dobius', path: '/repo' })
