...
import { DOBIUS_WORKTREE_FILE_CHANGE_EVENT } from '@/hooks/worktree-file-change-event'
...
describe('useGitStatusPolling rerender stability', () => {
  ...
  it('keeps the poll runner stable when openFiles changes mid-cooldown', async () => {
    ...
    expect(addSpy).toHaveBeenCalledWith(DOBIUS_WORKTREE_FILE_CHANGE_EVENT, expect.any(Function))

    // Fire a file-watch event to drive fetchStatus during the cooldown.
    window.dispatchEvent(
      new CustomEvent(DOBIUS_WORKTREE_FILE_CHANGE_EVENT, {
        detail: {
          payload: {
