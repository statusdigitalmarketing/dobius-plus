...
import {
  DOBIUS_TERMINAL_COMMAND_FINISHED_EVENT,
...
export function useGitStatusPushSignalRefresh({
  ...
  useEffect(() => {
    ...
    window.addEventListener(DOBIUS_TERMINAL_COMMAND_FINISHED_EVENT, handleCommandFinished)
    return () => {
      window.removeEventListener(DOBIUS_TERMINAL_COMMAND_FINISHED_EVENT, handleCommandFinished)
    }
