...
function RightSidebarInner(): React.JSX.Element {
  ...
  const activityItems = useMemo<ActivityBarItem[]>(
    () => [
      ...
      },
      {
        ...
        title: translate('auto.components.right.sidebar.index.aiVaultSessionHistory', 'Sessions'),
        shortcut: ''
      },
      {
        id: 'tasks',
        icon: ListTodo,
        title: translate('auto.components.right.sidebar.index.dobiusTasks', 'Tasks'),
        shortcut: ''
      },
      {
        id: 'prompts',
        icon: MessageSquareText,
        title: translate('auto.components.right.sidebar.index.dobiusPrompts', 'Prompts'),
        shortcut: ''
      },
      {
        id: 'workspaces',
  ...
  // Why: moved out of the titlebar — the mobile visual is an action (opens the
  // phone-preview split), not a routed tab, so it renders as an extra activity
  // button outside the measured tab strip to keep overflow math intact.
  const mobileEmulatorEnabled = useAppStore((s) => s.settings?.mobileEmulatorEnabled !== false)
  const openMobileVisual = (): void => {
    const store = useAppStore.getState()
    const worktreeId = store.activeWorktreeId
    void window.api.window.openPhoneVisual({
      worktreeId: worktreeId ?? undefined,
      mode: 'app'
    })
  }
  const mobileVisualItem = {
    icon: Smartphone,
    title: translate('auto.components.right.sidebar.index.mobileVisual', 'Mobile visual'),
    shortcut: ''
  }
  const topVisualButtons = (
    <>
      {mobileEmulatorEnabled ? (
        <ActivityBarButton
          item={mobileVisualItem}
          active={false}
          onClick={openMobileVisual}
          layout="top"
        />
      ) : null}
    </>
