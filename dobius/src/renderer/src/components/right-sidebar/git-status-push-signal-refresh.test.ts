...
import { DOBIUS_TERMINAL_COMMAND_FINISHED_EVENT } from '@/hooks/terminal-command-finished-event'
...
async function renderHookOnce(params: HookParams): Promise<{
  ...
  return {
    ...
    emitCommandFinished: (worktreeId: string) => {
      const listener = windowListeners.get(DOBIUS_TERMINAL_COMMAND_FINISHED_EVENT)
