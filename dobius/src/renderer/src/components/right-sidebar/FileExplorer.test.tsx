...
describe('FileExplorerToolbar', () => {
  ...
  it('adds open-in launchers to the overflow menu', () => {
    ...
    expect(openInItems.props.worktreePath).toBe('/tmp/dobius')
...
describe('FileExplorerRow collapse folder action', () => {
  ...
  it('shows OS file copy for single local rows and SSH file rows on desktop', () => {
    const previous = (globalThis as { __DOBIUS_WEB_CLIENT__?: boolean }).__DOBIUS_WEB_CLIENT__
    try {
      ...
      ;(globalThis as { __DOBIUS_WEB_CLIENT__?: boolean }).__DOBIUS_WEB_CLIENT__ = previous
    }
  })
