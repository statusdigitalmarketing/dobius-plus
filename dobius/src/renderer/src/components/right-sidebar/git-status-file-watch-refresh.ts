...
export function useGitStatusFileWatchRefresh({
  ...
  useEffect(() => {
    ...
    window.addEventListener(DOBIUS_WORKTREE_FILE_CHANGE_EVENT, handleFsChanged as EventListener)

    return () => {
      if (refreshTimer) {
        clearTimeout(refreshTimer)
      }
      window.removeEventListener(DOBIUS_WORKTREE_FILE_CHANGE_EVENT, handleFsChanged as EventListener)
    }
