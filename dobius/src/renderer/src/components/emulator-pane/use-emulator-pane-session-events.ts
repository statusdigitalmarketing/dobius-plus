...
export const EMULATOR_LOCAL_SHUTDOWN_EVENT = 'dobius:emulator-shutdown'
...
export function useEmulatorPaneSessionEvents({
  ...
  useEffect(() => {
    ...
    window.addEventListener('dobius:emulator-auto-attach', onAuto)
    return () => window.removeEventListener('dobius:emulator-auto-attach', onAuto)
