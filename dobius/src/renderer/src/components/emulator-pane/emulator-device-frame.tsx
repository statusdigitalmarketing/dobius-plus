...
import { useEmulatorWheelGesture } from './use-emulator-wheel-gesture'
import { PhoneHardwareButtons } from './emulator-phone-hardware-buttons'
import { EmulatorChromelessScreen } from './emulator-chromeless-screen'
...
type EmulatorDeviceFrameProps = {
  ...
  isActive: boolean
  /** Why: the floating phone window already draws its own bezel/hardware —
   *  render only the interactive screen (no phone-in-phone chrome). */
  chromeless?: boolean
...
export function EmulatorDeviceFrame({
  ...
  const { handleWheel } = useEmulatorWheelGesture({
    canInteract,
    streamSize,
    sendTouch,
    sendGesturePoints
  ...
  const screenInteractionProps = {
    ...
    onPaste: handlePaste,
    onWheel: handleWheel,
    role: isLive ? ('application' as const) : undefined,
  ...
  if (chromeless) {
    return (
      <EmulatorChromelessScreen
        isLive={isLive}
        aspectRatioStyle={screenAspectRatioStyle}
        interactionProps={screenInteractionProps}
      >
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          data-dobius-emulator-frame="true"
