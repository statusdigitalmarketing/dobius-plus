...
import { DOBIUS_CLI_SKILL_NAME } from '@/lib/agent-feature-install-commands'
import {
  ensureDobiusCliAvailableForAgentSkillTerminal,
...
function getCliActionLabel(status: CliInstallStatus | null, busy: boolean): string {
  ...
  if (isDobiusCliAvailableOnPath(status)) {
    return translate(
...
export function useMobileEmulatorAgentSetupState(enabled = true): {
  ...
  const cliEnabled = isDobiusCliAvailableOnPath(cliInstallStatus)
  ...
  const recheckSetup = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const cliReady = isDobiusCliAvailableOnPath(cliStatus)
      if (!mountedRef.current) {
        return
      }
      ...
      if (skillInstalled) {
        toast.message(
          translate(
            ...
            'Skill is installed. Enable the Dobius+ CLI to finish setup.'
          )
        )
        return
      }
      if (cliReady) {
        toast.message(
          translate(
            ...
            'Dobius+ CLI is ready. Install the skill to finish setup.'
          )
        )
        return
      }
  ...
  const handleEnableCli = useCallback(async (): Promise<void> => {
    setCliBusy(true)
    try {
      const next = await ensureDobiusCliAvailableForAgentSkillTerminal({
        onStatusChange: setCliInstallStatus
      })
      if (mountedRef.current && isDobiusCliAvailableOnPath(next)) {
        toast.success(
          translate(
            'auto.components.emulator.pane.use.mobile.emulator.agent.setup.state.2b519eed94',
            'Registered the Dobius+ CLI in PATH.'
          )
        )
      }
    } finally {
