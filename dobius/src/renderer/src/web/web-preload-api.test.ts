...
function writeStoredRuntimeEnvironment(storage: Storage): void {
  storage.setItem(
    'dobius.web.runtimeEnvironment.v1',
    JSON.stringify({
      id: 'web-env-1',
      name: 'Test runtime',
      createdAt: 1,
...
describe('web keybindings preload API', () => {
  ...
  it('returns snapshots and persists customized bindings in browser storage', async () => {
    ...
    expect(storage.getItem('dobius.web.keybindings.v1')).toContain('worktree.palette')
...
describe('web settings preload API', () => {
  ...
  it('migrates first-work branch auto-rename on for stored legacy web settings once', async () => {
    ...
    const stored = JSON.parse(globals.storage.getItem('dobius.web.settings.v1') ?? '{}') as {
  ...
  it('migrates inherited terminal bar cursor defaults for stored web settings once', async () => {
    const globals = installBrowserGlobals('Linux')
    globals.storage.setItem('dobius.web.settings.v1', JSON.stringify({ terminalCursorStyle: 'bar' }))
    const { installWebPreloadApi } = await import('./web-preload-api')
    installWebPreloadApi()
  ...
  it('stamps the first-work branch auto-rename guard for web setting updates', async () => {
    ...
    const stored = JSON.parse(storage.getItem('dobius.web.settings.v1') ?? '{}') as {
      autoRenameBranchFromWork?: boolean
      autoRenameBranchFromWorkDefaultedOn?: boolean
    }
...
describe('web UI preload API', () => {
  ...
  it('saves browser clipboard images through bounded upload chunks', async () => {
      ...
      WebRuntimeClient: class {
        call(method: string, params?: unknown): Promise<RuntimeRpcResponse<unknown>> {
          ...
          return Promise.resolve({
            id: `call-${runtimeCalls.length}`,
            ok: true,
            result: 'C:\\Users\\alice\\AppData\\Local\\Temp\\dobius-paste-image.png',
            _meta: { runtimeId: 'runtime-1' }
          })
        }
    ...
    ).resolves.toBe('C:\\Users\\alice\\AppData\\Local\\Temp\\dobius-paste-image.png')
    expect(runtimeCalls).toEqual([
      {
  ...
  it('falls back to one-shot clipboard save for small payloads when the host lacks upload RPCs', async () => {
      ...
      WebRuntimeClient: class {
        call(method: string, params?: unknown): Promise<RuntimeRpcResponse<unknown>> {
          ...
          return Promise.resolve({
            id: `call-${runtimeCalls.length}`,
            ok: true,
            result: '/tmp/dobius-paste-image.png',
            _meta: { runtimeId: 'runtime-1' }
          })
        }
    ...
    ).resolves.toBe('/tmp/dobius-paste-image.png')
    expect(runtimeCalls).toEqual([
      {
        method: 'clipboard.startImageUpload',
        params: {
  ...
  it('keeps explicit local right sidebar visibility over the legacy default', async () => {
    const { api, storage } = await installApi('Linux')
    storage.setItem('dobius.web.ui.v1', JSON.stringify({ rightSidebarOpen: true }))
  ...
  it('keeps newer feature interaction counts when runtime responses resolve out of order', async () => {
    ...
    const stored = JSON.parse(globals.storage.getItem('dobius.web.ui.v1') ?? '{}') as {
  ...
  it('does not keep a local shadow copy of main-owned feature telemetry markers', async () => {
    ...
    const ui = await globals.window.api.ui.get()
    const stored = JSON.parse(globals.storage.getItem('dobius.web.ui.v1') ?? '{}') as Record<
      string,
      unknown
    >
        ...
        call(method: string, params: unknown): Promise<RuntimeRpcResponse<unknown>> {
          ...
          if (method === 'computer.permissionsStatus') {
            return Promise.resolve({
              ...
              result: {
                platform: 'darwin',
                helperAppPath: '/Applications/Dobius+ Computer Use.app',
...
describe('web repos preload API', () => {
  ...
  it.each([
    ['/home/alice', '/home/alice/dobius/projects'],
    ['/', '/dobius/projects'],
    ['C:\\', 'C:\\dobius\\projects']
  ])(
...
describe('web worktree preload API', () => {
  ...
  it('falls back to legacy worktree.list when detectedList is unavailable', async () => {
    ...
    expect(result).toMatchObject({
      ...
      worktrees: [{ id: worktree.id, ownership: 'dobius-managed', visible: true }]
    })
    expect(runtimeCalls).toEqual([
    ...
    await globals.window.api.worktrees.create({
      ...
      startup: {
        ...
        launchConfig: {
          ...
          agentEnv: { DOBIUS_AGENT_MODE: 'direct' }
        },
    ...
    expect(runtimeCalls).toEqual([
      {
        ...
        params: expect.objectContaining({
          ...
          startupEnv: { DOBIUS_AGENT_MODE: 'direct' },
          startupLaunchConfig: {
            agentCommand: 'codex',
            agentArgs: '--model gpt-5',
            agentEnv: { DOBIUS_AGENT_MODE: 'direct' }
  ...
  it('rejects SSH clone requests in paired web clients', async () => {
    ...
    await expect(
      api.repos.cloneRemote({
        connectionId: 'ssh-1',
        url: 'https://github.com/statusdigitalmarketing/dobius-plus.git',
  ...
  it('routes every runtime-backed GitHub method through the expected RPC method', async () => {
    ...
    const routeCases: {
      ...
      },
      {
        ...
        args: { repoPath, owner: 'acme', repo: 'dobius', number: 7, type: 'pr' },
      ...
      },
      {
        ...
        args: { owner: 'acme', repo: 'dobius', number: 7, type: 'issue' },
        expectedMethod: 'github.project.workItemDetailsBySlug',
        expectedParams: { owner: 'acme', repo: 'dobius', number: 7, type: 'issue' }
      },
      {
      ...
      },
      {
        ...
        args: { owner: 'acme', repo: 'dobius', number: 7, updates: { title: 'New' } },
        expectedMethod: 'github.project.updateIssueBySlug',
        expectedParams: { owner: 'acme', repo: 'dobius', number: 7, updates: { title: 'New' } }
      },
      {
      ...
      },
      {
        ...
        args: { owner: 'acme', repo: 'dobius', number: 7, body: 'Fixed' },
        expectedMethod: 'github.project.addIssueCommentBySlug',
        expectedParams: { owner: 'acme', repo: 'dobius', number: 7, body: 'Fixed' }
      },
      {
        key: 'updateIssueCommentBySlug',
        args: { owner: 'acme', repo: 'dobius', commentId: 9, body: 'Edited' },
        expectedMethod: 'github.project.updateIssueCommentBySlug',
        expectedParams: { owner: 'acme', repo: 'dobius', commentId: 9, body: 'Edited' }
      },
      {
        key: 'deleteIssueCommentBySlug',
        args: { owner: 'acme', repo: 'dobius', commentId: 9 },
        expectedMethod: 'github.project.deleteIssueCommentBySlug',
        expectedParams: { owner: 'acme', repo: 'dobius', commentId: 9 }
      },
      {
        key: 'listLabelsBySlug',
        args: { owner: 'acme', repo: 'dobius' },
        expectedMethod: 'github.project.listLabelsBySlug',
        expectedParams: { owner: 'acme', repo: 'dobius' }
      },
      {
        key: 'listAssignableUsersBySlug',
        args: { owner: 'acme', repo: 'dobius', seedLogins: ['alice'] },
        expectedMethod: 'github.project.listAssignableUsersBySlug',
        expectedParams: { owner: 'acme', repo: 'dobius', seedLogins: ['alice'] }
      },
      {
      ...
      },
      {
        ...
        args: { owner: 'acme', repo: 'dobius', number: 7, issueTypeId: 'it-1' },
        expectedMethod: 'github.project.updateIssueTypeBySlug',
        expectedParams: { owner: 'acme', repo: 'dobius', number: 7, issueTypeId: 'it-1' }
      }
    ]
