...
export default function WebConnect({
  ...
  const [name, setName] = useState(existingEnvironment?.name ?? 'Dobius+ Server')
  ...
  const connect = async (): Promise<void> => {
    setError(null)
    if (!parsedOffer) {
      setError('Enter a valid Dobius+ pairing URL or pairing code.')
      return
    }
    if (parsedOffer.scope === 'mobile') {
      setError(
        translate(
          ...
          'This QR code grants limited (mobile) access. To use the full web app, open the browser access link from Settings → Runtime Environments → Share this Dobius+ server → New Link.'
        )
      )
      return
    }
    if (isMixedContentWebSocket(parsedOffer.endpoint)) {
      setError(
        'This HTTPS page cannot connect to a plain ws:// Dobius+ server. Open the web client over HTTP or pair with a wss:// endpoint.'
      )
      return
    }
    setConnecting(true)
      ...
      <div className="flex w-full max-w-[520px] flex-col gap-5 rounded-lg border border-border bg-card p-5 shadow-sm">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <h1 className="text-base font-semibold leading-6">
              {translate('auto.web.WebConnect.e3bcd082ac', 'Connect to Dobius+')}
            </h1>
            <p className="mt-1 text-sm leading-5 text-muted-foreground">
              {translate(
                'auto.web.WebConnect.3affe7de3a',
                'Paste a pairing URL from a Dobius+ server that this browser can reach.'
              )}
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-2">
          ...
          <Input
            ...
            placeholder={translate('auto.web.WebConnect.27393856e4', 'dobius://pair?code=...')}
            autoComplete="off"
            spellCheck={false}
          />
        </div>
