...
export class WebRuntimeClient {
  ...
  async call(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!this.sendEncrypted({ id, deviceToken: this.pairing.deviceToken, method, params })) {
        ...
        reject(new Error('Remote Dobius+ runtime is not connected.'))
      }
    })
  }
  ...
  private async subscribeOnCurrentConnection(
    ...
    if (!this.sendEncrypted({ id, deviceToken: this.pairing.deviceToken, method, params })) {
      ...
      throw new Error('Remote Dobius+ runtime is not connected.')
    }
    return {
      unsubscribe: () => {
        this.subscriptions.delete(id)
  ...
  close(options: { notifySubscriptions?: boolean } = {}): void {
    ...
    this.clearTimers()
    this.rejectAllPending('Remote Dobius+ runtime connection closed.')
    this.rejectAllWaiters(new Error('Remote Dobius+ runtime connection closed.'))
  ...
  private waitForConnected(timeoutMs = REQUEST_TIMEOUT_MS): Promise<void> {
    ...
    if (this.intentionallyClosed) {
      return Promise.reject(new Error('Remote Dobius+ runtime connection closed.'))
    }
    return new Promise((resolve, reject) => {
      const timeout = window.setTimeout(() => {
        ...
        reject(
          new Error(
            withRemoteRuntimeTailscaleHint(
              'Timed out while connecting to the remote Dobius+ runtime.',
              this.pairing.endpoint
            )
          )
        )
  ...
  private handleSocketClosed(closedWs: WebSocket): void {
    ...
    this.rejectAllPending('Remote Dobius+ runtime connection interrupted.')
    this.notifySubscriptionsClosed()
