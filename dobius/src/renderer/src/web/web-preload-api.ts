...
import type { BundledSkillInstallResult, SkillDiscoveryResult } from '../../../shared/skills'
import {
...
const SETTINGS_STORAGE_KEY = 'dobius.web.settings.v1'
const UI_STORAGE_KEY = 'dobius.web.ui.v1'
const SESSION_STORAGE_KEY = 'dobius.web.workspaceSession.v1'
const ONBOARDING_STORAGE_KEY = 'dobius.web.onboarding.v1'
const GITHUB_CACHE_STORAGE_KEY = 'dobius.web.githubCache.v1'
const KEYBINDINGS_STORAGE_KEY = 'dobius.web.keybindings.v1'
...
export function installWebPreloadApi(): void {
  ...
  const webWindow = window as unknown as { __DOBIUS_WEB_CLIENT__?: boolean }
  webWindow.__DOBIUS_WEB_CLIENT__ = true
...
function createWebPreloadApi(): Partial<PreloadApi> {
  return {
    ...
    window: {
      tearOffTerminal: () =>
        Promise.reject(new Error('Terminal tear-off is unavailable in the web client.')),
      openPhoneVisual: () =>
        Promise.reject(new Error('Floating phone is unavailable in the web client.')),
      closePhoneVisual: () =>
        Promise.reject(new Error('Floating phone is unavailable in the web client.')),
      minimizePhoneVisual: () =>
        Promise.reject(new Error('Floating phone is unavailable in the web client.')),
      toggleMaximizePhoneVisual: () =>
        Promise.reject(new Error('Floating phone is unavailable in the web client.')),
      setPhoneVisualBounds: () =>
        Promise.reject(new Error('Floating phone is unavailable in the web client.')),
      onPhoneVisualUpdate: () => noopUnsubscribe,
    },
    starNag: {
      ...
      starDobius: () => Promise.resolve(false),
...
function createRuntimeEnvironmentsApi(): NonNullable<Partial<PreloadApi>['runtimeEnvironments']> {
  return {
    ...
    addFromPairingCode: async ({ name, pairingCode }) => {
      ...
      if (!offer) {
        throw new Error('Invalid Dobius+ pairing code.')
      }
...
function createBrowserApi(): NonNullable<Partial<PreloadApi>['browser']> {
  return {
    ...
    onOpenLinkInDobiusTab: () => noopUnsubscribe,
...
function createGitHubApi(): WebGitHubApi {
  ...
  const githubApi = {
    ...
    checkDobiusStarred: () => Promise.resolve(null),
    starDobius: () => Promise.resolve(false),
    rateLimit: (args) =>
...
function createCliApi(): NonNullable<Partial<PreloadApi>['cli']> {
  const status = {
    platform: getBrowserPlatform(),
    commandName: getBrowserPlatform() === 'win32' ? 'dobius.cmd' : 'dobius',
    commandPath: null,
    pathDirectory: null,
    ...
    detail: 'CLI registration is managed on the Dobius+ server, not in the web browser.'
  } as const
  return {
...
function createAgentHooksApi(): NonNullable<Partial<PreloadApi>['agentHooks']> {
  const status = (
    ...
    Promise.resolve({
      ...
      detail: 'Agent hook status is only available on the Dobius+ server.'
    } as const)
  return {
  ...
  return {
    ...
    openSetup: (args) =>
      callRuntimeResult<ComputerUsePermissionSetupResult>(
        ...
        nextStep: 'Computer-use permissions are managed on the Dobius+ server.'
      })),
    reset: () =>
      Promise.resolve({
        platform: getBrowserPlatform(),
...
function createSkillsApi(): NonNullable<Partial<PreloadApi>['skills']> {
  return {
    ...
    installBundled: (skillName): Promise<BundledSkillInstallResult> =>
      Promise.reject(
        new Error(`Bundled skill installs are unavailable in the web client: ${skillName}`)
      ),
    isBundledInstalled: () => Promise.resolve(false)
  }
}
...
function resolveEnvironment(selector: string): StoredWebRuntimeEnvironment {
  ...
  throw new Error(`Unknown Dobius+ runtime environment: ${selector}`)
}
...
function requireActiveEnvironment(): StoredWebRuntimeEnvironment {
  ...
  if (!activeEnvironment) {
    throw new Error('Pair this web client with an Dobius+ server first.')
  }
...
function getStoredOnboarding(): OnboardingState {
  ...
  // Why: pairing already means the user has an Dobius+ server. Desktop first-run
...
function mapRepoPathArg(args: unknown): unknown {
  ...
  return {
    ...
    // Dobius+'s repo id on task calls, so prefer the explicit selector.
