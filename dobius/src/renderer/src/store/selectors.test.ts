...
describe('store selectors', () => {
  ...
  it('groups hydrated VM project setups under the repo-derived project identity', () => {
    ...
    const projects = [
      {
        ...
        displayName: 'dobius',
        badgeColor: '#737373',
        sourceRepoIds: ['local-dobius'],
        createdAt: 1,
        updatedAt: 1
      },
      {
        id: 'repo:vm-dobius',
        displayName: 'vercel/sandbox/dobius',
        badgeColor: '#737373',
        sourceRepoIds: ['vm-dobius'],
        createdAt: 1,
        updatedAt: 1
      }
    ]
    ...
    expect(projection.projects.map((project) => project.id)).toEqual(['github:statusdigitalmarketing/dobius-plus'])
    expect(projection.setups).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ id: 'local-setup', projectId: 'github:statusdigitalmarketing/dobius-plus' }),
        expect.objectContaining({ id: 'vm-setup', projectId: 'github:statusdigitalmarketing/dobius-plus' })
      ])
    )
  })
