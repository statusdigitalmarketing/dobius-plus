...
function makeLinearIssue(overrides: Partial<LinearIssue> = {}): LinearIssue {
  return {
    ...
    identifier: 'ORC-1',
    title: 'Fix task flow',
    url: 'https://linear.app/dobius/issue/ORC-1/fix-task-flow',
    ...
    labels: [],
    labelIds: [],
    team: { id: 'team-1', name: 'Dobius', key: 'ORC' },
...
function makeJiraIssue(overrides: Partial<JiraIssue> = {}): JiraIssue {
  return {
    ...
    project: { id: '10000', key: 'ORC', name: 'Dobius', siteId: 'site-1' },
...
describe('createUISlice contextual tours', () => {
  ...
  it('retires an auto tour after two starts so a flickering target cannot re-nag', () => {
    const store = createUIStore()
    ...
    // Cancelled before it painted — the old bug never marked it seen.
    store.getState().cancelContextualTour('tasks')
    expect(store.getState().activeContextualTourId).toBeNull()

    // Second auto start hits the cap → marked seen.
    store.getState().requestContextualTour('tasks', 'tasks_open')
    expect(store.getState().activeContextualTourId).toBe('tasks')
    expect(store.getState().contextualToursSeenIds).toContain('tasks')
    expect(store.getState().contextualTourStartCounts.tasks).toBe(2)

    // Cancel again; a third request is now gated by seen — no re-nag.
    store.getState().cancelContextualTour('tasks')
    store.getState().requestContextualTour('tasks', 'tasks_open')
    expect(store.getState().activeContextualTourId).toBeNull()
  })
