...
async function assertProjectHostSetupRuntimeCapability(
  ...
  await assertRuntimeEnvironmentCapability(
    ...
    PROJECT_HOST_SETUP_RUNTIME_CAPABILITY,
    'The selected Dobius+ server does not support project host setup yet. Update Dobius+ on the server and try again.',
    15_000
  )
}
...
async function assertProjectHostSetupMutationRuntimeCapabilities(
  ...
  await assertRuntimeEnvironmentCapability(
    target.environmentId,
    WORKSPACE_RUN_CONTEXT_RUNTIME_CAPABILITY,
    'The selected Dobius+ server does not support explicit workspace run hosts yet. Update Dobius+ on the server and try again.',
    15_000
  )
}
...
async function fetchRuntimeAddProjectPathStatus(args: {
  ...
  await assertRuntimeEnvironmentCapability(
    ...
    translate(
      ...
      'Update Dobius+ server to open non-Git folders on this runtime.'
    ),
    15_000
  )
  try {
...
export const createRepoSlice: StateCreator<AppState, [], [], RepoSlice> = (set, get) => ({
    ...
    for (const projectId of targets.projectIds) {
      ...
      if (stillExists) {
        failedProjectRemovals.push({
          projectId,
          reason: 'Project remained in Dobius+ after removeProject completed.'
        })
      } else {
  ...
  addRepoPath: async (path, kind = 'git', options) => {
    try {
      ...
      if (alreadyAdded) {
        get().clearDobiusHookTrustForRepo(repo.id)
      }
      set((s) => {
  ...
  removeProject: async (projectId) => {
    try {
      ...
      get().clearDobiusHookTrustForRepo(projectId)
