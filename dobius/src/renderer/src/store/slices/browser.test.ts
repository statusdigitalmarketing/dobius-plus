...
describe('createBrowserSlice runtime guard', () => {
        ...
        profiles: [
          {
            id: 'default',
            scope: 'default',
            partition: 'persist:dobius-default',
            label: 'Default',
            source: null
          }
        ]
        ...
        profiles: [
          {
            id: 'remote-default',
            scope: 'default',
            partition: 'persist:dobius-remote',
    ...
    mockApi.browser.sessionListProfiles.mockResolvedValueOnce([
      {
        id: 'local-default',
        scope: 'default',
        partition: 'persist:dobius-local',
        label: 'Local Default',
        source: null
      }
    ])
  ...
  it('stores a runtime-resolved browser partition without a renderer profile mirror', () => {
    ...
    const tab = store.getState().createBrowserTab('wt-1', 'https://example.com', {
      ...
      sessionPartition: 'persist:dobius-browser-session-profile-isolated'
    })

    expect(tab.sessionProfileId).toBe('profile-isolated')
    expect(tab.sessionPartition).toBe('persist:dobius-browser-session-profile-isolated')
    expect(store.getState().browserTabsByWorktree['wt-1']?.[0]?.sessionPartition).toBe(
      'persist:dobius-browser-session-profile-isolated'
    )
  })
  ...
  it('stores a runtime-resolved partition when switching browser tab profiles', () => {
    ...
    const tab = store.getState().createBrowserTab('wt-1', 'https://example.com', {
      ...
      sessionPartition: 'persist:dobius-browser'
    })
