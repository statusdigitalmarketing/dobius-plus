...
export type AgentStatusSlice = {
   ...
   *  carry Dobius+ launch settings, so the renderer supplies them from startup. */
  ...
  return {
    ...
    setAgentStatus: (paneKey, payload, terminalTitle, timing, routing, metadata) => {
      ...
      set((s) => {
        ...
        // an Dobius+-launched one. Once the provider session changes, the old
        ...
        // Why: launch tokens can remain in a shell after an Dobius+-started TUI exits;
      ...
      if (completionRefreshWorktreeId) {
        ...
        // Why: agents can create a PR via `gh pr create`, bypassing Dobius+'s
