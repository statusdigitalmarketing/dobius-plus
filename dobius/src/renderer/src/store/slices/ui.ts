...
// Auto contextual tours retire after this many impressions so a flickering
// target can't re-nag the user indefinitely.
const MAX_AUTO_TOUR_STARTS = 2
...
function filterTrustedDobiusHooksToValidRepos(
  trust: PersistedTrustedDobiusHooks,
  validRepoIds: Set<string>
): PersistedTrustedDobiusHooks {
  const next: PersistedTrustedDobiusHooks = {}
...
export type UISlice = {
  ...
  markDobiusHookScriptConfirmed: (
    repoId: string,
    kind: DobiusHookScriptKind,
    contentHash: string
  ) => void
  markDobiusHookRepoAlwaysTrusted: (repoId: string) => void
  clearDobiusHookTrustForRepo: (repoId: string) => void
...
export const createUISlice: StateCreator<AppState, [], [], UISlice> = (set, get) => ({
  ...
  previousViewBeforeKnowledge: 'terminal',
  ...
  openKnowledgePage: () =>
    set((state) => ({
      ...
      previousViewBeforeKnowledge:
        state.activeView === 'knowledge' ? state.previousViewBeforeKnowledge : state.activeView
    })),
  closeKnowledgePage: () =>
    set((state) => ({
      activeView: state.previousViewBeforeKnowledge
    })),
  openMobilePage: () =>
    set((state) => ({
      activeView: 'mobile',
  ...
  requestContextualTour: (id, source, wasFeaturePreviouslyInteracted, options) =>
    set((s) => {
      ...
      const { [id]: _consumed, ...remainingNavigationSnapshot } =
        s.contextualTourNavigationInteractionSnapshot
      void _consumed
      // Why: cap AUTO starts. A tour whose target flickers gets cancelled before
      // it paints (so never marked seen) and would re-nag forever; after
      // MAX_AUTO_TOUR_STARTS impressions, retire it by marking it seen. Forced
      // replays (help menu) bypass this via options.force.
      const nextStartCounts = { ...s.contextualTourStartCounts }
      let nextSeenIds = s.contextualToursSeenIds
      if (options?.force !== true) {
        const nextCount = (s.contextualTourStartCounts[id] ?? 0) + 1
        nextStartCounts[id] = nextCount
        if (nextCount >= MAX_AUTO_TOUR_STARTS && !nextSeenIds.includes(id)) {
          nextSeenIds = [...nextSeenIds, id]
        }
        if (typeof window !== 'undefined') {
          void window.api.ui
            .set({
              contextualTourStartCounts: nextStartCounts,
              contextualToursSeenIds: nextSeenIds
            })
            .catch(console.error)
        }
      }
  ...
  trustedDobiusHooks: {},
  markDobiusHookScriptConfirmed: (repoId, kind, contentHash) =>
    set((s) => {
      const existing = s.trustedDobiusHooks[repoId]
      ...
      const next = { ...s.trustedDobiusHooks, [repoId]: nextRepo }
      window.api.ui.set({ trustedDobiusHooks: next }).catch(console.error)
      return { trustedDobiusHooks: next }
    }),
  ...
  clearDobiusHookTrustForRepo: (repoId) =>
    set((s) => {
      if (!(repoId in s.trustedDobiusHooks)) {
        return s
      }
      const next = { ...s.trustedDobiusHooks }
      delete next[repoId]
      window.api.ui.set({ trustedDobiusHooks: next }).catch(console.error)
      return { trustedDobiusHooks: next }
    }),
      ...
      const hydrated = {
        ...
        contextualToursAutoEligible:
          typeof ui.contextualToursAutoEligible === 'boolean'
            ...
            : null,
        trustedDobiusHooks: filterTrustedDobiusHooksToValidRepos(
          ui.trustedDobiusHooks ?? {},
          validRepoIds
        ),
