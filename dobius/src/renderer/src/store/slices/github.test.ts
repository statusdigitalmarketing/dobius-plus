...
describe('createGitHubSlice.fetchPRForBranch', () => {
  ...
  it('does not let an older direct PR refresh overwrite a newer hosted-review cache entry', async () => {
    ...
    const newerReview: HostedReviewInfo = {
      ...
      state: 'open',
      url: 'https://github.com/acme/dobius/pull/12',
      status: 'success',
      updatedAt: '2026-03-28T00:00:00Z',
      mergeable: 'MERGEABLE'
    }
  ...
  it('writes exact linked PR data after create-PR handoff races a hosted-review refresh', async () => {
    ...
    const createdReview: HostedReviewInfo = {
      ...
      title: 'Created PR',
      state: 'open',
      url: 'https://github.com/acme/dobius/pull/88',
      status: 'pending',
      updatedAt: '2026-03-28T00:00:00Z',
      mergeable: 'UNKNOWN'
    }
      ...
      hostedReviewCache: {
        [hostedReviewCacheKey]: {
          data: {
            ...
            state: 'open',
            url: 'https://github.com/acme/dobius/pull/44',
            status: 'pending',
            updatedAt: '2026-03-28T00:00:00Z',
            mergeable: 'UNKNOWN'
          },
  ...
  it('does not overwrite a non-GitHub hosted review from GitHub PR refresh events', () => {
    ...
    const gitlabReview: HostedReviewInfo = {
      ...
      title: 'GitLab MR',
      state: 'open',
      url: 'https://gitlab.com/acme/dobius/-/merge_requests/5',
      status: 'pending',
      updatedAt: '2026-03-28T00:00:00Z',
      mergeable: 'UNKNOWN'
    }
...
describe('createGitHubSlice.fetchWorkItems source/error envelope', () => {
  ...
  it('routes work item fetches through an explicit GitHub source context', async () => {
    ...
    const sourceContext = {
      ...
      providerIdentity: { provider: 'github' as const, owner: 'statusdigitalmarketing', repo: 'dobius' }
    }
  ...
  it('keeps explicit GitHub source identities in separate work-item cache buckets', async () => {
    ...
    const firstSourceContext = {
      ...
      projectHostSetupId: 'setup-1',
      repoId: 'repo-1',
      providerIdentity: { provider: 'github' as const, owner: 'acme', repo: 'dobius' }
    }
    ...
    mockApi.gh.listWorkItems
      .mockResolvedValueOnce({
        ...
        sources: { issues: { owner: 'acme', repo: 'dobius' }, prs: { owner: 'acme', repo: 'dobius' } }
      })
      .mockResolvedValueOnce({
...
describe('IssueSourceIndicator suppression', () => {
  it('hides when sources deep-equal, shows when they differ, hides when either is null', async () => {
    ...
    // Case-insensitive equality — the parent design doc calls out that `statusdigitalmarketing/dobius-plus`
    // and `statusdigitalmarketing/dobius-plus` resolve to the same repo and must suppress.
    expect(
      sameGitHubOwnerRepo({ owner: 'statusdigitalmarketing', repo: 'Dobius' }, { owner: 'statusdigitalmarketing', repo: 'dobius' })
    ).toBe(true)
