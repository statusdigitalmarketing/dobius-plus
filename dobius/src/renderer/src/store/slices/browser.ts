...
import { FLOATING_TERMINAL_WORKTREE_ID, DOBIUS_BROWSER_BLANK_URL } from '../../../../shared/constants'
...
function normalizeBrowserTitle(title: string | null | undefined, url: string): string {
  if (
    ...
    // Why: blank pages render through Dobius+'s inert data: URL guest. Persisting
...
function buildBrowserPage(
  ...
  return {
    ...
    loading: normalizedUrl !== 'about:blank' && normalizedUrl !== DOBIUS_BROWSER_BLANK_URL,
    faviconUrl: null,
    canGoBack: false,
    canGoForward: false,
    loadError: null,
...
export const createBrowserSlice: StateCreator<AppState, [], [], BrowserSlice> = (set, get) => ({
    ...
    set((s) => {
      ...
      const shouldFocusAddressBar =
        ...
        (options?.focusAddressBar ??
          (page.url === 'about:blank' || page.url === DOBIUS_BROWSER_BLANK_URL))
  ...
  setBrowserPageUrl: (pageId, url) => {
    ...
    if (nextUrl !== 'about:blank' && nextUrl !== DOBIUS_BROWSER_BLANK_URL) {
  ...
  addBrowserHistoryEntry: (url, title) => {
    ...
    if (safeUrl === DOBIUS_BROWSER_BLANK_URL || safeUrl === 'about:blank' || !safeUrl) {
      return
    }
