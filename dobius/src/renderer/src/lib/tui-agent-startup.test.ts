...
describe('buildAgentDraftLaunchPlan', () => {
  ...
  it('uses DOBIUS_PI_PREFILL env var for pi (no CLI flag exists)', () => {
    // Why: pi has no `--prefill` flag, and bracketed-paste-after-ready races
    // against pi's lengthy startup output. The Dobius overlay installs an
    // `dobius-prefill` extension that reads DOBIUS_PI_PREFILL on session_start
    ...
    expect(
      ...
      agent: 'pi',
      launchCommand: 'pi; unset DOBIUS_PI_PREFILL',
      expectedProcess: 'pi',
      env: { DOBIUS_PI_PREFILL: 'https://github.com/acme/repo/issues/42' },
