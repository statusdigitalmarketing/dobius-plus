...
describe('getSmartGitHubSubmitIntent', () => {
  it('treats GitHub issue and pull URLs as submit-time source intent', () => {
    expect(getSmartGitHubSubmitIntent('https://github.com/statusdigitalmarketing/dobius-plus/pull/2049')).toEqual({
      kind: 'link',
    ...
    expect(getSmartGitHubSubmitIntent('https://github.com/statusdigitalmarketing/dobius-plus/issues/2050')).toEqual({
      kind: 'link',
      owner: 'statusdigitalmarketing',
      repo: 'dobius',
      number: 2050,
  ...
  it('finds an embedded GitHub item URL when prose punctuation touches the URL', () => {
    expect(
      getSmartGitHubSubmitIntent('review (https://github.com/statusdigitalmarketing/dobius-plus/pull/2049), please')
    ).toEqual({
      kind: 'link',
      owner: 'statusdigitalmarketing',
      repo: 'dobius',
    ...
    expect(getSmartGitHubSubmitIntent('fix https://github.com/statusdigitalmarketing/dobius-plus/issues/2050.')).toEqual(
      {
        kind: 'link',
        owner: 'statusdigitalmarketing',
        repo: 'dobius',
...
describe('lookupSmartGitHubSubmitItem', () => {
  ...
  it('reuses an in-flight direct URL lookup for the same repo and intent', async () => {
    const item = {
      ...
      state: 'open' as const,
      url: 'https://github.com/statusdigitalmarketing/dobius-plus/pull/2049',
      labels: [],
  ...
  it('prunes expired distinct lookup entries on later lookups', async () => {
    ...
    const workItem = vi.fn().mockImplementation(({ number }) =>
      Promise.resolve({
        ...
        title: `Issue ${number}`,
        state: 'open' as const,
        url: `https://github.com/statusdigitalmarketing/dobius-plus/issues/${number}`,
        labels: [],
        updatedAt: '2026-05-26T00:00:00.000Z',
        author: 'octocat',
        repoId: 'repo-1'
...
describe('getSmartGitHubSubmitResolution', () => {
  ...
  it('strips duplicated issue prefixes while preserving linked issue metadata', () => {
    const resolution = getSmartGitHubSubmitResolution({
      ...
      url: 'https://github.com/statusdigitalmarketing/dobius-plus/issues/2050'
    })
  ...
  it('uses the URL path to normalize stale PR-typed issue results', () => {
    expect(
      getSmartGitHubSubmitResolution({
        ...
        url: 'https://github.com/statusdigitalmarketing/dobius-plus/issues/6933'
      })
    ).toEqual({
