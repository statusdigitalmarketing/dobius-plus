...
describe('new-workspace-composer-repo', () => {
  ...
  describe('resolveComposerActiveRepoId', () => {
    const localDobius = makeRepo('local-dobius', { upstream: { owner: 'statusdigitalmarketing', repo: 'dobius' } })
    const runtimeDobius = makeRepo('runtime-dobius', {
      connectionId: 'runtime-ssh-dobius-1',
      upstream: { owner: 'statusdigitalmarketing', repo: 'dobius' }
    })
    const otherProject = makeRepo('noqa', { upstream: { owner: 'statusdigitalmarketing', repo: 'noqa' } })
    const repos = [otherProject, localDobius, runtimeDobius]
    ...
    it('maps an active runtime-owned SSH repo to its local same-project sibling', () => {
      expect(resolveComposerActiveRepoId(repos, eligibleRepos, 'runtime-dobius')).toBe('local-dobius')
    })

    it('leaves a normal active repo unchanged', () => {
      expect(resolveComposerActiveRepoId(repos, eligibleRepos, 'local-dobius')).toBe('local-dobius')
    })

    it('keeps the runtime repo id when no same-project sibling is eligible', () => {
      const onlyRuntime = [runtimeDobius]
      expect(
        resolveComposerActiveRepoId(
          onlyRuntime,
