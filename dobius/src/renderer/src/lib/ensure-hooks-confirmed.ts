...
import type { DobiusHooks } from '../../../shared/types'
import { resolveHookCommandSourcePolicy } from '../../../shared/hook-command-source-policy'
import { hashDobiusHookScript, type DobiusHookScriptKind } from './dobius-hook-trust'
...
export type HookScriptKind = DobiusHookScriptKind
...
function getSetupTrustContent(yamlHooks: DobiusHooks | null): string {
...
function getVmRecipeTrustContent(yamlHooks: DobiusHooks | null): string {
...
export async function ensureHooksConfirmed(
  ...
  return enqueueTrustPrompt(async () => {
    if (state.trustedDobiusHooks[repoId]?.all) {
      return 'run'
    }
    ...
    try {
      if (scriptKind === 'issueCommand') {
        // Local overrides are user-owned; only shared dobius.yaml commands need repo trust.
        ...
        const yamlHooks = (result.hooks as DobiusHooks | null) ?? null
        scriptContent =
          scriptKind === 'setup'
    ...
    const contentHash = await hashDobiusHookScript(scriptContent)
    const existingHash = state.trustedDobiusHooks[repoId]?.[scriptKind]?.contentHash
    ...
    // version of this script; the prompt is reappearing because dobius.yaml changed.
    const previouslyApproved = Boolean(existingHash)

    return new Promise<'run' | 'skip'>((resolve) => {
      state.openModal('confirm-dobius-yaml-hooks', {
        repoId,
        repoName,
        scriptKind,
        scriptContent,
