...
describe('ai vault resume target ownership', () => {
  ...
  it('resolves runtime-owned worktree targets through their repo owner', () => {
    expect(
      getAiVaultResumeWorktreeTargetStatus({
        worktreeId: 'repo-1::/repo/dobius',
        worktrees: [{ id: 'repo-1::/repo/dobius', repoId: 'repo-1' }],
  ...
  it('prefers explicit worktree host ownership over repo ownership', () => {
    expect(
      getAiVaultResumeWorktreeTargetStatus({
        worktreeId: 'repo-1::/repo/dobius',
        worktrees: [{ id: 'repo-1::/repo/dobius', repoId: 'repo-1', hostId: 'ssh:ssh-1' }],
        repos: [{ id: 'repo-1', connectionId: null, executionHostId: 'runtime:env-1' }]
      })
    ).toBe('ssh')
  })
  ...
  it('resolves explicit workspace keys through the target worktree owner', () => {
    expect(
      getAiVaultResumeWorkspaceTargetStatus(
        makeState({
          worktreesByRepo: {
            'repo-1': [{ id: 'repo-1::/repo/dobius', repoId: 'repo-1' }]
          },
          repos: [{ id: 'repo-1', connectionId: null, executionHostId: 'runtime:env-1' }]
        }),
        'worktree:repo-1::/repo/dobius'
      )
    ).toBe('runtime')
  })
  ...
  it('resolves explicit workspace keys through the target worktree host', () => {
    expect(
      getAiVaultResumeWorkspaceTargetStatus(
        makeState({
          worktreesByRepo: {
            'repo-1': [{ id: 'repo-1::/repo/dobius', repoId: 'repo-1', hostId: 'runtime:env-1' }]
          },
  ...
  it('resolves exact execution host ids for active workspaces', () => {
    const state = makeState({
      worktreesByRepo: {
        'repo-1': [{ id: 'repo-1::/repo/dobius', repoId: 'repo-1', hostId: 'ssh:ssh-1' }]
      },
      repos: [{ id: 'repo-1', connectionId: null, executionHostId: 'local' }]
    })

    expect(getAiVaultResumeWorkspaceExecutionHostId(state, 'repo-1::/repo/dobius')).toBe('ssh:ssh-1')
    expect(getAiVaultResumeWorkspaceExecutionHostId(state, 'worktree:repo-1::/repo/dobius')).toBe(
      'ssh:ssh-1'
    )
  })
