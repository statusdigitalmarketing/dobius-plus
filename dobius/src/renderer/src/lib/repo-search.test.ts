...
function makeRepo(overrides: Partial<Repo> = {}): Repo {
  return {
    id: 'repo-1',
    path: '/Users/test/src/dobius',
    displayName: 'statusdigitalmarketing/dobius-plus',
    badgeColor: '#22c55e',
    addedAt: 0,
    ...overrides
  }
...
describe('repo-search', () => {
  ...
  it('matches display names case-insensitively', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'statusdigitalmarketing/dobius-plus', path: '/repos/dobius' }),
      makeRepo({ id: '2', displayName: 'statusdigitalmarketing/noqa', path: '/repos/noqa' })
    ]

    expect(searchRepos(repos, 'DOBIUS').map((repo) => repo.id)).toEqual(['1'])
  })

  it('falls back to matching repo paths', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'frontend', path: '/src/team-a/dobius' }),
  ...
  it('keeps display-name matches ahead of path-only matches', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'misc', path: '/src/dobius-tools/misc' }),
      makeRepo({ id: '2', displayName: 'dobius', path: '/src/team-a/project' })
    ]

    expect(searchRepos(repos, 'dobius').map((repo) => repo.id)).toEqual(['2', '1'])
  })
