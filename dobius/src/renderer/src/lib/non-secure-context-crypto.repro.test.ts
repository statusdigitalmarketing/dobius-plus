...
describe('non-secure context (plain HTTP LAN web client)', () => {
  ...
  it('hashDobiusHookScript does not throw when crypto.subtle is missing', async () => {
    const { hashDobiusHookScript } = await import('./dobius-hook-trust')
  ...
  it('produces the same hash as crypto.subtle did in a secure context', async () => {
    ...
    const secureHash = await (async () => {
      Object.defineProperty(globalThis, 'crypto', { configurable: true, value: realCrypto })
      return hashDobiusHookScript('echo hi')
    })()
    Object.defineProperty(globalThis, 'crypto', {
      configurable: true,
      value: { getRandomValues: realCrypto.getRandomValues.bind(realCrypto) }
    })
    expect(await hashDobiusHookScript('echo hi')).toBe(secureHash)
  })
