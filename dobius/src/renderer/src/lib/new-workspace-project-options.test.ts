...
function setup(overrides: Partial<ProjectHostSetup>): ProjectHostSetup {
  return {
    ...
    projectId: overrides.projectId ?? 'github:statusdigitalmarketing/dobius-plus',
    hostId: overrides.hostId ?? 'local',
    repoId: overrides.repoId ?? 'local-repo',
    path: overrides.path ?? '/tmp/dobius',
    displayName: overrides.displayName ?? 'dobius',
...
describe('buildNewWorkspaceProjectOptions', () => {
    ...
    expect(options).toEqual([
      {
        ...
        displayName: 'dobius',
        badgeColor: '#111111',
        detail: 'statusdigitalmarketing/dobius-plus'
      }
    ])
  })
  ...
  it('excludes projects that do not have a ready eligible setup', () => {
    ...
    expect(options.map((option) => option.id)).toEqual(['github:statusdigitalmarketing/dobius-plus'])
  })
  ...
  it('filters project options by display name and detail', () => {
    const options: NewWorkspaceProjectOption[] = [
      ...
      },
      {
        ...
        displayName: 'Docs',
        badgeColor: '#222222',
        detail: 'statusdigitalmarketing/docs'
      }
    ]

    expect(searchNewWorkspaceProjectOptions(options, 'docs')).toEqual([options[1]])
    expect(searchNewWorkspaceProjectOptions(options, 'statusdigitalmarketing/dobius-plus')).toEqual([options[0]])
  })
...
describe('buildNewWorkspaceFolderSourceOptions', () => {
  it('keeps concrete source repos separate even when they are the same logical project', () => {
    const options = buildNewWorkspaceFolderSourceOptions([
      repo('local-repo', { displayName: 'dobius', path: '/tmp/dobius' }),
      repo('ssh-repo', {
        displayName: 'dobius',
        path: '/srv/dobius',
    ...
    expect(options.map((option) => option.detail).sort()).toEqual(['/srv/dobius', '/tmp/dobius'])
