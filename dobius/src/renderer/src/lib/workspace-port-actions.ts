...
export function shouldOpenWorkspacePortInDobiusBrowser(
...
export function resolvePortOpenInDobiusBrowser({
  settings,
  event,
  isMac
}: {
  ...
  return shouldOpenWorkspacePortInDobiusBrowser(settings)
}
...
export async function openWorkspacePortInBrowser(args: {
  ...
  if (args.openInDobiusBrowser === false && args.runtimeTarget.kind === 'local') {
    try {
