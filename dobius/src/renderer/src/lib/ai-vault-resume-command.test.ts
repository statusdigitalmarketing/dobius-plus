...
describe('ai vault resume command runtime', () => {
  ...
  it('gates clean Claude resume on the cleanClaudeResume setting', () => {
    const session = {
    ...
    // Default (unset) → clean resume: spawn in the recorded folder, and the
    // visible command carries no cd into that path.
    const clean = buildAiVaultResumeStartupForWorktree({
      state: makeState({ worktreePath: 'C:\\Users\\alice\\repo' }),
      worktreeId: 'repo-1::worktree-1',
      session
    })
    expect(clean.startupCwd).toBe('C:\\Users\\alice\\repo')
    expect(clean.command).not.toContain('alice')

    // Off → fall back to the cd-prefixed command with no separate startupCwd.
    const offState = makeState({ worktreePath: 'C:\\Users\\alice\\repo' })
    offState.settings = { ...offState.settings, cleanClaudeResume: false } as never
    const off = buildAiVaultResumeStartupForWorktree({
      state: offState,
      worktreeId: 'repo-1::worktree-1',
      session
    })
    expect(off.startupCwd).toBeUndefined()
    expect(off.command).toContain('alice')
  })
