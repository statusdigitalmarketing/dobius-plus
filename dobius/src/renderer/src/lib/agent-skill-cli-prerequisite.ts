...
type EnsureDobiusCliAvailableOptions = {
...
export const AGENT_SKILL_CLI_PREREQUISITE_NOTICE =
  'Before opening setup, Dobius+ may show a system prompt to register the Dobius+ CLI command on PATH.'

export const CLI_PREREQUISITE_REGISTRATION_TOAST = 'Dobius+ needs to register its CLI on PATH.'
export const CLI_PREREQUISITE_REGISTRATION_TOAST_DESCRIPTION =
  'Approve the system prompt so skill setup can use the Dobius+ CLI command.'

export function isDobiusCliAvailableOnPath(status: CliInstallStatus | null | undefined): boolean {
  return status?.state === 'installed' && status.pathConfigured
}

export async function ensureDobiusCliAvailableForAgentSkillTerminal({
  onStatusChange,
  registrationPromptDelayMs = 700
}: EnsureDobiusCliAvailableOptions = {}): Promise<CliInstallStatus | null> {
  try {
    ...
    if (status.state !== 'installed' || !status.pathConfigured) {
      ...
      await showDobiusCliRegistrationPromptToast(registrationPromptDelayMs)
    ...
    toast.error(
      error instanceof Error
        ...
        : translate(
            ...
            'Failed to register the Dobius+ CLI in PATH.'
          )
    )
    return null
  }
...
export async function showDobiusCliRegistrationPromptToast(delayMs = 700): Promise<void> {
...
function showCliPrerequisiteWarning(status: CliInstallStatus): void {
  if (!status.supported) {
    toast.warning(
      translate(
        ...
        'Dobius+ CLI registration is unavailable'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            'Install the Dobius+ CLI before running agent skill setup.'
          )
      }
    )
    return
  ...
  if (status.state !== 'installed') {
    toast.warning(
      translate(
        ...
        'Dobius+ CLI registration needs attention'
      ),
      {
        description:
          status.detail ??
  ...
  if (!status.pathConfigured) {
    ...
    // Dobius+ affordances when that shell can resolve the Dobius+ CLI command.
    toast.warning(
      translate(
        'auto.lib.agent.skill.cli.prerequisite.79371593b0',
        'Dobius+ CLI is not visible on PATH yet'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            'Restart your shell or add the Dobius+ CLI directory to PATH before setup.'
          )
      }
    )
  }
