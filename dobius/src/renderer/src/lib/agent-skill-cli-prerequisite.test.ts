...
describe('isDobiusCliAvailableOnPath', () => {
  it('requires the installed CLI command to be visible on PATH', () => {
    expect(isDobiusCliAvailableOnPath(cliStatus())).toBe(true)
    expect(isDobiusCliAvailableOnPath(cliStatus({ pathConfigured: false }))).toBe(false)
    expect(isDobiusCliAvailableOnPath(cliStatus({ state: 'not_installed' }))).toBe(false)
  })
})

describe('ensureDobiusCliAvailableForAgentSkillTerminal', () => {
  afterEach(() => {
    vi.useRealTimers()
    vi.unstubAllGlobals()
    vi.clearAllMocks()
  ...
  it('lets the registration toast paint before opening the native installer', async () => {
    ...
    const pending = ensureDobiusCliAvailableForAgentSkillTerminal({ registrationPromptDelayMs: 700 })
    await vi.waitFor(() => {
      expect(toast.message).toHaveBeenCalledWith(CLI_PREREQUISITE_REGISTRATION_TOAST, {
        description: CLI_PREREQUISITE_REGISTRATION_TOAST_DESCRIPTION
      })
