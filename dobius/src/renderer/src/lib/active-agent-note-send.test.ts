...
describe('active agent note send', () => {
  ...
  it('offers the active terminal send target for an Dobius-launched agent before hooks report', () => {
  ...
  it('sends notes only after the active terminal is verified as an idle agent', async () => {
    ...
    expect(testState.callRuntimeRpc).toHaveBeenCalledWith(
      ...
      'terminal.send',
      {
        ...
        client: { id: 'dobius-desktop', type: 'desktop' }
      },
      { timeoutMs: 15000 }
    )
  ...
  it('uses selected-target failure wording for explicit note targets', () => {
    ...
    expect(
      ...
      'The notes may already be pasted in the selected terminal, but Dobius could not submit them.'
    )
    expect(activeAgentNotesSendFailureMessage('partial-submit-failed')).toBe(
      'The notes may already be pasted in the active terminal, but Dobius could not submit them.'
    )
  })
