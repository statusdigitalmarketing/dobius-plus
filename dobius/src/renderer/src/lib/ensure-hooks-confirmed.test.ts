...
import type { PersistedTrustedDobiusHooks } from '../../../shared/types'
...
function createTestState(overrides?: Partial<AppState>): {
  ...
  const trust: PersistedTrustedDobiusHooks = {}
  const state = {
    trustedDobiusHooks: trust,
...
describe('ensureHooksConfirmed', () => {
  ...
  it('short-circuits to run when the persisted content hash matches the current script', async () => {
    ...
    const hash = await hashDobiusHookScript(script)
    state.trustedDobiusHooks['repo-1'] = {
  ...
  it('re-prompts when the script content differs from the persisted hash', async () => {
    const { state, pending } = createTestState()
    const staleHash = await hashDobiusHookScript('old script')
    state.trustedDobiusHooks['repo-1'] = {
    ...
    // dobius.yaml changed, not because they've never approved this hook.
  ...
  it('includes default tab commands in the setup trust prompt', async () => {
    ...
    expect(pending[0].data.contentHash).toBe(await hashDobiusHookScript(expectedContent))
  ...
  it('does not prompt for dobius.yaml when the repo uses local commands only', async () => {
    const { state, pending } = createTestState({
      repos: [
        {
          id: 'repo-1',
  ...
  it('does not prompt for dobius.yaml when local commands are the implicit default', async () => {
    const { state, pending } = createTestState({
      repos: [
        {
          id: 'repo-1',
  ...
  it('opens a modal with the computed content hash and resolves with the user decision', async () => {
    ...
    expect(pending[0].data).toMatchObject({
      ...
      contentHash: await hashDobiusHookScript('pnpm install'),
