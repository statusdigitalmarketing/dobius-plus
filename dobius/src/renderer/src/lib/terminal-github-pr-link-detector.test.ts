...
describe('createTerminalGitHubPRLinkDetector', () => {
  it('extracts GitHub pull request URLs from terminal output', () => {
    ...
    expect(observe('Created https://github.com/acme/dobius/pull/42\r\n')).toEqual([
      {
  ...
  it('waits for a boundary when the URL is split across PTY chunks', () => {
    ...
    expect(observe('https://github.com/acme/dobius/pull/4')).toEqual([])
    expect(observe('2\r\n')).toEqual([
      {
        url: 'https://github.com/acme/dobius/pull/42',
        slug: { owner: 'acme', repo: 'dobius' },
  ...
  it('detects a URL split inside the GitHub prefix', () => {
    ...
    expect(observe('ub.com/acme/dobius/pull/42\n')).toEqual([
      {
        url: 'https://github.com/acme/dobius/pull/42',
        slug: { owner: 'acme', repo: 'dobius' },
        number: 42
  ...
  it('trims terminal punctuation around printed URLs', () => {
    ...
    expect(observe('Opened (https://github.com/acme/dobius/pull/42).\n')[0]?.url).toBe(
      'https://github.com/acme/dobius/pull/42'
    )
  })
  ...
  it('does not repeat the same PR URL from overlapping carry text', () => {
    ...
    expect(observe('https://github.com/acme/dobius/pull/42\n')).toHaveLength(1)
  ...
  it('ignores non-PR GitHub-shaped links', () => {
    ...
    expect(observe('https://github.com/acme/dobius/issues/42\n')).toEqual([])
  })
  ...
  it('scans huge terminal chunks containing pull markers without global regex iteration', () => {
    ...
    expect(observe(`${noise}Created https://github.com/acme/dobius/pull/42\r\n`)).toEqual([
      {
        url: 'https://github.com/acme/dobius/pull/42',
        slug: { owner: 'acme', repo: 'dobius' },
        number: 42
  ...
  it('drops overlong incomplete URL carry instead of retaining pasted megabytes', () => {
    ...
    expect(observe(`https://github.com/acme/dobius/pull/${'4'.repeat(10_000)}`)).toEqual([])
    expect(observe('2\r\n')).toEqual([])
  })
})
