...
export function buildAiVaultResumeStartupForWorktree(args: {
  ...
  // Why: the clean-resume behavior (spawn in the recorded folder, queue only the
  // resume command) is a user toggle. Off falls back to the cd-prefixed command.
  const cleanResumeEnabled = args.state.settings?.cleanClaudeResume ?? true
  ...
  if (isResumableTuiAgent(args.session.agent)) {
    ...
    if (startupPlan) {
      const startsInRecordedCwd =
        cleanResumeEnabled && args.session.agent === 'claude' && Boolean(args.session.cwd)
      return {
        command: buildAiVaultResumeShellCommand({
          resumeCommand: startupPlan.launchCommand,
          // Why: Claude can resume by session id from a terminal already spawned
          // in the recorded directory. Keep the visible command clean instead of
          // typing an unnecessary `cd … && claude --resume …` sequence.
          cwd: startsInRecordedCwd ? null : args.session.cwd,
          platform,
          codexHome,
          shell: queuedShell
        }),
        ...(startsInRecordedCwd && args.session.cwd ? { startupCwd: args.session.cwd } : {}),
