...
describe('buildGitHubRepoUrl', () => {
  it('builds a GitHub repository URL from an owner/repo slug', () => {
    expect(buildGitHubRepoUrl({ owner: 'statusdigitalmarketing', repo: 'dobius' })).toBe(
      'https://github.com/statusdigitalmarketing/dobius-plus'
    )
  })

  it('encodes path segments', () => {
    expect(buildGitHubRepoUrl({ owner: 'stably ai', repo: 'dobius/tools' })).toBe(
      'https://github.com/stably%20ai/dobius%2Ftools'
    )
  })
})
...
describe('parseGitHubIssueOrPRNumber', () => {
  it('parses plain issue numbers and GitHub pull request URLs', () => {
    ...
    expect(parseGitHubIssueOrPRNumber('#42')).toBe(42)
    expect(parseGitHubIssueOrPRNumber('https://github.com/statusdigitalmarketing/dobius-plus/pull/123')).toBe(123)
    expect(parseGitHubIssueOrPRNumber('https://github.com/statusdigitalmarketing/dobius-plus/issues/923')).toBe(923)
...
describe('parseGitHubIssueOrPRLink', () => {
  it('parses slug, number, and type for direct item URLs', () => {
    expect(parseGitHubIssueOrPRLink('https://github.com/statusdigitalmarketing/dobius-plus/pull/123')).toEqual({
      slug: { owner: 'statusdigitalmarketing', repo: 'dobius' },
      number: 123,
      type: 'pr'
    })
    ...
    expect(parseGitHubIssueOrPRLink('https://github.com/statusdigitalmarketing/dobius-plus/issues/923')).toEqual({
      slug: { owner: 'statusdigitalmarketing', repo: 'dobius' },
      number: 923,
      type: 'issue'
    })
...
describe('normalizeGitHubLinkQuery', () => {
  it('accepts full GitHub URLs whose slug differs from the selected repo slug', () => {
    expect(normalizeGitHubLinkQuery('https://github.com/statusdigitalmarketing/dobius-plus/issues/923')).toEqual({
      query: 'https://github.com/statusdigitalmarketing/dobius-plus/issues/923',
      directNumber: 923,
      directLink: {
        slug: { owner: 'statusdigitalmarketing', repo: 'dobius' },
        number: 923,
  ...
  it('preserves PR route intent for full GitHub URLs', () => {
    expect(normalizeGitHubLinkQuery('https://github.com/statusdigitalmarketing/dobius-plus/pull/6934')).toEqual({
      query: 'https://github.com/statusdigitalmarketing/dobius-plus/pull/6934',
      directNumber: 6934,
      directLink: {
        slug: { owner: 'statusdigitalmarketing', repo: 'dobius' },
        number: 6934,
