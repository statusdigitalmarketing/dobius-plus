...
export type ComposerCardProps = {
  ...
  ephemeralVmRecipes: NonNullable<DobiusHooks['environmentRecipes']>
...
export function getInitialAutoManagedWorkspaceName({
  ...
  // match the linked item seed Dobius+ generated for a source selection.
...
export function useComposerState(options: UseComposerStateOptions): UseComposerStateResult {
  ...
  const [ephemeralVmRecipes, setEphemeralVmRecipes] = useState<
    NonNullable<DobiusHooks['environmentRecipes']>
  >([])
  ...
  // Why: SSH remotes deploy the CLI shim as plain `dobius`, so the Linux-only
  // `dobius-ide` rename must not be applied to remote launch commands.
  ...
  const [yamlHooks, setYamlHooks] = useState<DobiusHooks | null>(null)
  ...
  const commitHookCheckIfCurrent = useCallback(
    (targetRepoId: string, hooks: DobiusHooks | null): boolean => {
  ...
  const uploadComposerPaths = useCallback(
    async (
      ...
      const destinationDir = joinPath(targetRepoPath, '.dobius/drops')
  ...
  const handleBaseBranchPrSelect = useCallback(
    (
      ...
      // worktree's comment should surface (`dobius worktree current`, sidebar).
