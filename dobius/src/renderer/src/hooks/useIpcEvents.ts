...
export function useIpcEvents(): void {
  useEffect(() => {
    ...
    const handleWorktreesChanged = async (
      ...
      // deletions (CLI `dobius worktree rm`, other window, out-of-band RPC)
    ...
    // Why: `dobius tab switch --focus` lands here after the bridge's state-only
    ...
    unsubs.push(
      window.api.browser.onOpenLinkInDobiusTab(({ browserPageId, url }) => {
        const store = useAppStore.getState()
        ...
        // Why: the guest process can request "open this link in Dobius+", but it
        // does not own Dobius+'s worktree/tab model. Resolve the source page's
        // worktree and create a new outer browser tab so the link opens as a
        // separate tab in the outer Dobius+ tab bar.
    ...
    const unsubscribeEmulatorAutoAttach = window.api.emulator?.onAutoAttach(
      ({ worktreeId, info }) => {
        ...
        window.setTimeout(() => {
          window.dispatchEvent(
            new CustomEvent('dobius:emulator-auto-attach', {
