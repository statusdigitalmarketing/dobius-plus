...
import {
  DOBIUS_WORKTREE_FILE_CHANGE_EVENT,
...
// `DOBIUS_EDITOR_EXTERNAL_FILE_CHANGE_EVENT` per raw event fan-outs into N full
  ...
  const handleFsChanged = (
    ...
    if (typeof window !== 'undefined' && typeof window.dispatchEvent === 'function') {
      window.dispatchEvent(
        new CustomEvent<WorktreeFileChangeEventDetail>(DOBIUS_WORKTREE_FILE_CHANGE_EVENT, {
...
function scheduleSelfWriteAwareExternalReload(
  ...
  // with the saved text so we suppress only the echo of Dobius+'s own write.
