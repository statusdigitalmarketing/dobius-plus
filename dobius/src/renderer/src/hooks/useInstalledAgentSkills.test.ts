...
describe('hasInstalledAgentSkill', () => {
  it('matches installed skills by summarized name', () => {
    expect(hasInstalledAgentSkill([skill({ name: 'dobius-cli' })], 'dobius-cli')).toBe(true)
  })
    ...
    expect(
      hasInstalledAgentSkill(
        [
          skill({
            name: 'Dobius CLI',
            directoryPath: 'C:\\Users\\test\\.agents\\skills\\dobius-cli'
          })
        ],
        'dobius-cli'
      )
  ...
  it('ignores non-installed discovery entries', () => {
    expect(
      hasInstalledAgentSkill([skill({ name: 'dobius-cli', installed: false })], 'dobius-cli')
    ).toBe(false)
  })
      ...
      hasInstalledAgentSkill(
        [
          skill({
            ...
            directoryPath: '/repo/.agents/skills/dobius-cli',
            skillFilePath: '/repo/.agents/skills/dobius-cli/SKILL.md'
          }),
          skill({
            id: 'skill-2',
            name: 'dobius-cli',
            ...
            directoryPath: '/Users/test/.codex/plugins/cache/vendor/dobius-cli',
            skillFilePath: '/Users/test/.codex/plugins/cache/vendor/dobius-cli/SKILL.md'
          })
        ],
        'dobius-cli',
  ...
  it('counts home skills when matching global installs', () => {
    expect(
      hasInstalledAgentSkill([skill({ name: 'dobius-cli' })], 'dobius-cli', {
        sourceKinds: GLOBAL_AGENT_SKILL_SOURCE_KINDS
      })
    ).toBe(true)
  })
    ...
    expect(
      hasInstalledAgentSkillNamed(
        [
          skill({
            name: 'Linear Tickets',
            directoryPath: 'C:\\Users\\test\\.agents\\skills\\dobius-linear'
          })
        ],
        ['dobius-linear', 'linear-tickets']
      )
  ...
  it('keeps aliases opt-in for unrelated single-name checks', () => {
    expect(hasInstalledAgentSkill([skill({ name: 'linear-tickets' })], 'dobius-linear')).toBe(false)
  })
})
...
describe('discoverInstalledAgentSkills', () => {
  ...
  it('starts a fresh scan when a forced refresh arrives during a background scan', async () => {
    ...
    const freshResult = discoveryResult([skill({ name: 'dobius-cli' })])
