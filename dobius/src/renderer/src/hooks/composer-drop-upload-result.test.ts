...
describe('composer drop upload result', () => {
  it('separates imported files and folders while counting skipped or failed paths', () => {
    const results: ComposerDropUploadImportResult[] = [
      { status: 'imported', kind: 'file', destPath: '/repo/.dobius/drops/file.txt' },
      { status: 'imported', kind: 'directory', destPath: '/repo/.dobius/drops/folder' },
      { status: 'skipped' },
      { status: 'failed' }
    ]

    expect(collectComposerDropUploadResult(results)).toEqual({
      filePaths: ['/repo/.dobius/drops/file.txt'],
      folderPaths: ['/repo/.dobius/drops/folder'],
      skippedOrFailed: 2
    })
  })
