...
const {
  ...
  callMock: vi.fn(),
  serveDobiusAppMock: vi.fn(),
  getDefaultUserDataPathMock: vi.fn(() => '/tmp/dobius-user-data'),
...
vi.mock('./runtime-client', () => {
    ...
    constructor(
      ...
      const effectivePairingCode =
        remotePairingCode === undefined
          ? (process.env.DOBIUS_PAIRING_CODE ?? process.env.DOBIUS_REMOTE_PAIRING)
          : remotePairingCode
      const effectiveEnvironment =
        environmentSelector === undefined ? process.env.DOBIUS_ENVIRONMENT : environmentSelector
  ...
  return {
    ...
    serveDobiusApp: serveDobiusAppMock,
...
describe('dobius root help', () => {
  ...
  it('progressively discloses Linear commands', async () => {
    ...
    expect(groupHelp).toContain('dobius linear')
    ...
    expect(issueHelp).toContain('dobius linear issue [<id>]')
    ...
    expect(searchHelp).toContain('dobius linear search <query>')
  ...
  it('hides removed parent-workspace help and scopes create parent selectors', async () => {
    ...
    expect(createHelp).not.toContain('active Dobius workspace')
    ...
    expect(createHelp).toContain(
      '--no-parent only affects Dobius lineage; omit --base-branch to use the repo default base'
    )
...
describe('dobius cli worktree awareness', () => {
  const originalTerminalHandle = process.env.DOBIUS_TERMINAL_HANDLE
  const originalUserDataPath = process.env.DOBIUS_USER_DATA_PATH
  const originalPairingCode = process.env.DOBIUS_PAIRING_CODE
  const originalRemotePairing = process.env.DOBIUS_REMOTE_PAIRING
  const originalEnvironment = process.env.DOBIUS_ENVIRONMENT
  const originalWorkspaceId = process.env.DOBIUS_WORKSPACE_ID
  const originalWorktreeId = process.env.DOBIUS_WORKTREE_ID

  beforeEach(() => {
    callMock.mockReset()
    delete process.env.DOBIUS_TERMINAL_HANDLE
    delete process.env.DOBIUS_USER_DATA_PATH
    delete process.env.DOBIUS_WORKSPACE_ID
    delete process.env.DOBIUS_WORKTREE_ID
    serveDobiusAppMock.mockReset()
  ...
  afterEach(() => {
    ...
    if (originalTerminalHandle === undefined) {
      ...
      process.env.DOBIUS_TERMINAL_HANDLE = originalTerminalHandle
    }
    if (originalUserDataPath === undefined) {
      delete process.env.DOBIUS_USER_DATA_PATH
    } else {
      process.env.DOBIUS_USER_DATA_PATH = originalUserDataPath
    }
    if (originalPairingCode === undefined) {
      delete process.env.DOBIUS_PAIRING_CODE
    } else {
      process.env.DOBIUS_PAIRING_CODE = originalPairingCode
    }
    if (originalRemotePairing === undefined) {
      delete process.env.DOBIUS_REMOTE_PAIRING
    } else {
      process.env.DOBIUS_REMOTE_PAIRING = originalRemotePairing
    }
    if (originalEnvironment === undefined) {
      delete process.env.DOBIUS_ENVIRONMENT
    } else {
      process.env.DOBIUS_ENVIRONMENT = originalEnvironment
    }
    if (originalWorkspaceId === undefined) {
      delete process.env.DOBIUS_WORKSPACE_ID
    } else {
      process.env.DOBIUS_WORKSPACE_ID = originalWorkspaceId
    }
    if (originalWorktreeId === undefined) {
      delete process.env.DOBIUS_WORKTREE_ID
    } else {
      process.env.DOBIUS_WORKTREE_ID = originalWorktreeId
    }
  })
  ...
  it.skipIf(process.platform === 'win32')(
    'prepares and starts Claude Agent Teams in the current Dobius terminal',
    async () => {
      process.env.DOBIUS_PANE_KEY = 'tab-1:11111111-1111-4111-8111-111111111111'
      queueFixtures(
        callMock,
        okFixture('req_agent_teams_prepare', {
          launch: {
            env: {
              ...
              TMUX: '/tmp/dobius-claude-agent-teams/team-1,0,1',
              TMUX_PANE: '%1',
              PATH: '/tmp/dobius-shim:/usr/bin'
            }
          }
        })
      )
      ...
      expect(callMock).toHaveBeenCalledWith('agentTeams.prepareLaunch', {
        ...
        env: expect.objectContaining({
          DOBIUS_PANE_KEY: 'tab-1:11111111-1111-4111-8111-111111111111'
        })
      })
  ...
  it('resolves project and host flags to the matching repo for worktree.create', async () => {
    queueFixtures(
      ...
      okFixture('req_create', {
        worktree: buildWorktree('/srv/dobius/feature', 'feature', 'abc', 'repo-gpu'),
        lineage: null,
        warnings: []
      })
    )
    ...
    await main(
      [
        ...
        'create',
        '--project',
        'github:statusdigitalmarketing/dobius-plus',
        '--host',
        'runtime:gpu',
        '--name',
        'feature',
  ...
  it('passes folder workspace environment lineage through worktree.create', async () => {
    process.env.DOBIUS_WORKSPACE_ID = 'folder:folder-1'
    queueFixtures(
      callMock,
      worktreeListFixture([buildWorktree('/tmp/repo', 'main', 'abc', 'repo-1')]),
      okFixture('req_create', {
  ...
  it('passes caller terminal handle through worktree.create with cwd fallback', async () => {
    process.env.DOBIUS_TERMINAL_HANDLE = 'term_parent'
    queueFixtures(
      callMock,
      worktreeListFixture([buildWorktree('/tmp/repo', 'main', 'abc', 'repo-1')]),
      okFixture('req_create', {
  ...
  it('starts a foreground headless server through `serve`', async () => {
    serveDobiusAppMock.mockResolvedValue(0)
    process.env.DOBIUS_ENVIRONMENT = 'stale-env'
    ...
    expect(serveDobiusAppMock).toHaveBeenCalledWith({
      json: true,
      port: '6768',
  ...
  it('runs vm recipe doctor locally without contacting the app runtime', async () => {
    const repoPath = mkdtempSync(path.join(tmpdir(), 'dobius-vm-doctor-'))
    try {
      mkdirSync(path.join(repoPath, 'scripts', 'dobius-vm'), { recursive: true })
      const startScript = path.join(repoPath, 'scripts', 'dobius-vm', 'start.sh')
      const cleanupScript = path.join(repoPath, 'scripts', 'dobius-vm', 'cleanup.sh')
      ...
      writeFileSync(
        path.join(repoPath, 'dobius.yaml'),
        [
          'environmentRecipes:',
          '  - id: cloud-sandbox',
          '    name: Cloud Sandbox',
          '    create: ./scripts/dobius-vm/start.sh',
          '    destroy: ./scripts/dobius-vm/cleanup.sh'
        ].join('\n')
      )
      const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {})
      ...
      expect(output.checks).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ id: 'dobius_yaml.parse', status: 'pass' }),
  ...
  it('warns when vm recipe doctor finds no cleanup hook', async () => {
    ...
    try {
      mkdirSync(path.join(repoPath, 'scripts', 'dobius-vm'), { recursive: true })
      writeFileSync(path.join(repoPath, 'scripts', 'dobius-vm', 'start.sh'), '#!/bin/sh\n')
      writeFileSync(
        path.join(repoPath, 'dobius.yaml'),
        [
          'environmentRecipes:',
  ...
  it('runs vm recipe doctor provision mode and invokes cleanup', async () => {
    const repoPath = mkdtempSync(path.join(tmpdir(), 'dobius-vm-doctor-provision-'))
    ...
    try {
      mkdirSync(path.join(repoPath, 'scripts', 'dobius-vm'), { recursive: true })
      writeFileSync(
        path.join(repoPath, 'scripts', 'dobius-vm', 'start.js'),
        [
      ...
      writeFileSync(
        path.join(repoPath, 'scripts', 'dobius-vm', 'cleanup.js'),
        [
          "const fs = require('fs')",
      ...
      writeFileSync(
        ...
        [
          ...
          '  - id: cloud-sandbox',
          '    name: Cloud Sandbox',
          `    create: ${JSON.stringify(`${process.execPath} ./scripts/dobius-vm/start.js`)}`,
          `    destroy: ${JSON.stringify(`${process.execPath} ./scripts/dobius-vm/cleanup.js`)}`
        ].join('\n')
      )
      const { EventEmitter } = await import('node:events')
  ...
  it('returns the full create transcript when provision fails so the agent can self-diagnose', async () => {
    const repoPath = mkdtempSync(path.join(tmpdir(), 'dobius-vm-doctor-provision-fail-'))
    try {
      mkdirSync(path.join(repoPath, 'scripts', 'dobius-vm'), { recursive: true })
      writeFileSync(path.join(repoPath, 'scripts', 'dobius-vm', 'start.js'), 'process.exit(0)')
      writeFileSync(
        path.join(repoPath, 'dobius.yaml'),
        [
          'environmentRecipes:',
  ...
  it('rejects recipe JSON output without a project root', async () => {
    ...
    expect(serveDobiusAppMock).not.toHaveBeenCalled()
    expect([...logSpy.mock.calls, ...errSpy.mock.calls].flat().join('\n')).toContain(
  ...
  it('lists saved environments even when DOBIUS_ENVIRONMENT is set', async () => {
    process.env.DOBIUS_ENVIRONMENT = 'stale-env'
    ...
    expect(listEnvironmentsMock).toHaveBeenCalledWith('/tmp/dobius-user-data')
    expect(callMock).not.toHaveBeenCalled()
    expect(logSpy.mock.calls[0]?.[0]).not.toContain('token')
    expect(logSpy.mock.calls[0]?.[0]).not.toContain('publicKeyB64')
  })

  it('adds saved environments even when DOBIUS_ENVIRONMENT is set', async () => {
    process.env.DOBIUS_ENVIRONMENT = 'stale-env'
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {})

    await main(
      ['environment', 'add', '--name', 'desk', '--pairing-code', 'dobius://pair#abc', '--json'],
      '/tmp/repo'
    )

    expect(addEnvironmentFromPairingCodeMock).toHaveBeenCalledWith('/tmp/dobius-user-data', {
      name: 'desk',
      pairingCode: 'dobius://pair#abc'
    })
    expect(callMock).not.toHaveBeenCalled()
    expect(logSpy.mock.calls[0]?.[0]).not.toContain('token')
    expect(logSpy.mock.calls[0]?.[0]).not.toContain('publicKeyB64')
  ...
  it('lists projects through the project-first runtime API', async () => {
    queueFixtures(
      ...
      okFixture('req_project_list', {
        projects: [
          {
            id: 'github:statusdigitalmarketing/dobius-plus',
            displayName: 'Dobius',
            badgeColor: '#7c3aed',
            providerIdentity: {
              provider: 'github',
  ...
  it('filters project host setups locally after fetching setup compatibility state', async () => {
    ...
    await main(
      ['project', 'setups', '--project', 'github:statusdigitalmarketing/dobius-plus', '--host', 'runtime:gpu'],
      '/tmp/repo'
    )
  ...
  it('sends remote repo.add absolute paths unchanged', async () => {
    ...
    await main(
      ['repo', 'add', '--path', '/srv/dobius/web', '--pairing-code', 'remote-runtime', '--json'],
      '/tmp/repo'
    )
  ...
  it('formats group orchestration sends in text mode', async () => {
    process.env.DOBIUS_TERMINAL_HANDLE = 'term_sender'
  ...
  it('rejects unknown task-update status with an enum-aware error', async () => {
    process.env.DOBIUS_TERMINAL_HANDLE = 'term_coord'
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {})
    const errSpy = vi.spyOn(console, 'error').mockImplementation(() => {})
    const priorExitCode = process.exitCode
  ...
  it('passes the caller terminal handle through orchestration task-create', async () => {
    process.env.DOBIUS_TERMINAL_HANDLE = 'term_creator'
    callMock.mockResolvedValueOnce({
      id: 'req_task_create',
      ok: true,
      result: {
  ...
  it('passes dev mode to injected orchestration dispatches', async () => {
    process.env.DOBIUS_TERMINAL_HANDLE = 'term_sender'
    process.env.DOBIUS_USER_DATA_PATH = '/tmp/dobius-dev'
    callMock.mockResolvedValueOnce({
      id: 'req_dispatch',
      ok: true,
      result: {
  ...
  it('sends explicit remote terminal create worktree selectors unchanged', async () => {
    queueFixtures(
      ...
      okFixture('req_terminal_create', {
        terminal: {
          handle: 'term_1',
          worktreeId: 'repo-1::/srv/dobius/feature',
    ...
    expect(callMock).toHaveBeenCalledWith('terminal.create', {
      worktree: 'id:repo-1::/srv/dobius/feature',
      command: undefined,
      title: undefined,
      focus: false
    })
  ...
  it('exits nonzero when terminal wait returns an unsatisfied blocked result', async () => {
    process.env.DOBIUS_TERMINAL_HANDLE = 'term_worker'
    callMock.mockResolvedValueOnce({
  ...
  it('passes automation source context JSON through create', async () => {
    const sourceContext = {
      ...
      projectHostSetupId: 'setup-gpu',
      repoId: 'repo-gpu',
      providerIdentity: { provider: 'github', owner: 'statusdigitalmarketing', repo: 'dobius' },
  ...
  it('updates project host setup metadata through the project-first runtime API', async () => {
    ...
    expect(callMock).toHaveBeenCalledWith('projectHostSetup.update', {
      ...
      updates: {
        displayName: 'GPU VM',
        path: path.resolve('/tmp/repo', '/srv/dobius'),
