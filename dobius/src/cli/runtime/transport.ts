  ...
  return await new Promise((resolve, reject) => {
    ...
    if (!transport) {
      reject(
        new RuntimeClientError(
          'runtime_unavailable',
          'No compatible transport found in Dobius runtime metadata.'
        )
      )
      return
    }
    ...
    socket.once('error', () => {
      finish({
        ...
        error: new RuntimeClientError(
          'runtime_unavailable',
          'Could not connect to the running Dobius app. Restart Dobius and try again.'
        )
      })
    })
    ...
    socket.once('close', () => {
      finish({
        ...
        error: new RuntimeClientError(
          'runtime_unavailable',
          'The Dobius runtime closed the connection before responding. Restart Dobius and try again.'
        )
      })
    })
    socket.on('data', (chunk) => {
      ...
      while (newlineIndex !== -1 && !settled) {
        ...
        try {
          ...
          finish({
            ...
            error: new RuntimeClientError(
              ...
              'The Dobius runtime returned an invalid response frame.'
            )
          })
          return
        }
        ...
        // setTimeout().refresh() is stable since Node 10 (Dobius ships on
        ...
        // the decode boundary so version skew between the CLI and the Dobius
        ...
        if (response.id !== requestId) {
          finish({
            ...
            error: new RuntimeClientError(
              'invalid_runtime_response',
              'The Dobius runtime returned a mismatched response id.'
            )
          })
          return
        }
        if (response._meta?.runtimeId && response._meta.runtimeId !== metadata.runtimeId) {
          finish({
            ...
            error: new RuntimeClientError(
              'runtime_unavailable',
              'The Dobius runtime changed while the request was in flight. Retry the command.'
            )
          })
          return
        }
