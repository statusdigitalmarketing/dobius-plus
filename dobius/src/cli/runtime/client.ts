...
import { launchDobiusApp } from './launch'
...
export class RuntimeClient {
  ...
  constructor(
    ...
    remotePairingCode = process.env.DOBIUS_PAIRING_CODE ?? process.env.DOBIUS_REMOTE_PAIRING ?? null,
    ...
    if (this.remotePairing) {
      ...
      return {
        ...
        result: {
          ...
          // that this client machine has a local Dobius desktop process.
          app: {
            running: false,
            pid: null
          },
  ...
  async openDobius(timeoutMs = 15_000): Promise<RuntimeRpcSuccess<CliStatusResult>> {
    ...
    throw new RuntimeClientError(
      ...
      'Timed out waiting for Dobius to start. Run the Dobius app manually and try again.'
    )
  }
}
...
function resolveRemotePairing(
  ...
  if (!pairing) {
    throw new RuntimeClientError(
      'invalid_argument',
      'Invalid remote pairing code. Expected a dobius://pair?... URL or bare pairing payload.'
    )
  }
  return pairing
}
