...
export function launchDobiusApp(): void {
  const overrideCommand = process.env.DOBIUS_OPEN_COMMAND
  if (typeof overrideCommand === 'string' && overrideCommand.trim().length > 0) {
    spawnDetached(overrideCommand, [], { shell: true })
    return
  }

  const overrideExecutable = process.env.DOBIUS_APP_EXECUTABLE
  ...
  throw new RuntimeClientError(
    'runtime_open_failed',
    'Could not determine how to launch Dobius. Start Dobius manually and try again.'
  )
}
...
function spawnDetached(command: string, args: string[], options: SpawnOptions): void {
  ...
  // returns; openDobius already reports the user-facing timeout if startup fails.
  child.once('error', () => {})
  child.unref()
}

export function serveDobiusApp(
  args: {
    json?: boolean
    port?: string | null
  ...
  const executable = resolveForegroundDobiusExecutable()
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once('exit', (code, signal) => {
      ...
      reject(new RuntimeClientError('runtime_serve_failed', `Dobius serve exited via ${signal}`))
    })
  })
}
    ...
    const onExit = (code: number | null, signal: NodeJS.Signals | null): void => {
      ...
      finish(
        new RuntimeClientError(
          ...
          typeof code === 'number'
            ? `Dobius serve exited before printing recipe JSON with code ${code}.`
            : `Dobius serve exited before printing recipe JSON via ${signal}.`
        )
      )
    }
...
function getExecutableAppArgs(): string[] {
  return process.env.DOBIUS_APP_EXECUTABLE_NEEDS_APP_ROOT === '1' ? [resolveAppRoot()] : []
}
...
function resolveAppRoot(): string {
  ...
  // process.cwd(). Pin it to the app root so `dobius serve` behaves the same
  // regardless of the shell directory it was launched from.
  return resolve(__dirname, '../../..')
}

function resolveForegroundDobiusExecutable(): string {
  const overrideExecutable = process.env.DOBIUS_APP_EXECUTABLE
  if (typeof overrideExecutable === 'string' && overrideExecutable.trim().length > 0) {
  ...
  throw new RuntimeClientError(
    'runtime_serve_failed',
    'Could not determine how to start Dobius server. Set DOBIUS_APP_EXECUTABLE to the Dobius executable.'
  )
}
