...
const ROOT_HELP_TEXT = `dobius

Usage: dobius <command> [options]

Startup:
  open                      Launch Dobius+ and wait for the runtime to be reachable
  serve                     Start a headless Dobius+ runtime server
  status                    Show app/runtime/graph readiness

Diagnostics:
  diagnostics memory        Collect a memory snapshot for Dobius+ and managed terminals

Environments:
  environment add           Save a remote Dobius+ runtime from a pairing code
  environment list          List saved remote Dobius+ runtimes
  environment show          Show one saved remote Dobius+ runtime
  environment rm            Remove a saved remote Dobius+ runtime

Environment Recipes:
  vm recipe doctor          Validate a per-workspace environment recipe

Automations:
  automations list          List scheduled Dobius+ automations
  automations show          Show one Dobius+ automation
  automations create        Create a scheduled Dobius+ automation
  automations edit          Edit an Dobius+ automation
  automations remove        Remove an Dobius+ automation and its run history
  automations run           Run an Dobius+ automation now
  automations runs          List automation run history

Manager (filed docs + mail index; agent context):

Agents (Agents tab — persistent Claude SDK agents; for scheduled terminal runs use Automations):
  agents list               List Agents-tab agents
  agents show               Show one agent
  agents create             Create an agent in the Agents tab
  agents edit               Edit an agent
  agents remove             Remove an agent
  agents run                Run an agent now with a prompt
  agents runs               List agent run history

Projects:
  project list              List durable projects known to Dobius+
...
Repos:
  repo list                 List repos registered in Dobius+
  repo add                  Add a project to Dobius+ by filesystem path
...
Worktrees:
  worktree list             List Dobius+-managed worktrees
  worktree show             Show one worktree
  worktree current          Show the Dobius+-managed worktree for the current directory
  worktree create           Create a new Dobius+-managed worktree
  worktree set              Update Dobius+ metadata for a worktree
  worktree rm               Remove a worktree from Dobius+ and git
  worktree ps               Show a compact orchestration summary across worktrees

Files:
  file open                 Open a workspace file in the Dobius+ editor
  file diff                 Open a workspace file diff in the Dobius+ editor
  file open-changed         Open all git-changed files for a workspace

Terminals:
  terminal list             List live Dobius+-managed terminals
...
Mobile Emulator (iOS Simulator):
  emulator list             List available/running emulators (Dobius+-managed + raw serve-sim)
...
Common Commands:
  dobius open [--json]
  dobius serve [--port <port>] [--pairing-address <host>] [--mobile-pairing] [--no-pairing] [--project-root <path>] [--recipe-json] [--json]
  dobius status [--json]
  dobius diagnostics memory [--json]
  dobius environment add --name <name> --pairing-code <code> [--json]
  dobius environment list [--json]
  dobius environment show --environment <selector> [--json]
  dobius environment rm --environment <selector> [--json]
  dobius worktree list [--repo <selector>] [--limit <n>] [--json]
  ...
  dobius worktree rm --worktree <selector> [--force] [--run-hooks] [--json]
  dobius worktree ps [--limit <n>] [--json]
  dobius file open <path> [--worktree <selector>] [--json]
  dobius file diff <path> [--staged] [--worktree <selector>] [--json]
  dobius file open-changed [--mode edit|diff|both] [--worktree <selector>] [--json]
  dobius terminal list [--worktree <selector>] [--limit <n>] [--json]
  dobius terminal show [--terminal <handle>] [--json]
  dobius terminal read [--terminal <handle>] [--cursor <n>] [--limit <n>] [--json]
  ...
  dobius terminal stop --worktree <selector> [--json]
  dobius terminal create [--worktree <selector>] [--title <name>] [--command <text>] [--focus] [--json]
  dobius terminal split [--terminal <handle>] [--direction horizontal|vertical] [--json]
  dobius terminal switch [--terminal <handle>] [--json]
  dobius terminal close [--terminal <handle>] [--json]
  dobius project list [--json]
  dobius project setups [--project <id>] [--host <host-id>] [--json]
  ...
  dobius project setup-delete --setup <setup-id> [--json]
  dobius repo list [--json]
  dobius repo add --path <path> [--json]
  dobius repo show --repo <selector> [--json]
  dobius repo set-base-ref --repo <selector> --ref <ref> [--json]
  dobius repo search-refs --repo <selector> --query <text> [--limit <n>] [--json]
...
Selectors:
  ...
  --terminal <handle>       Runtime-issued terminal handle returned by \`dobius terminal list --json\`
...
Output Options:
  ...
  --pairing-code <code>      Connect to a remote Dobius+ runtime using a dobius://pair?... code
  --environment <selector>   Connect using a saved environment id or name
  --help                    Show this help message

Behavior:
  Most commands require a running Dobius+ runtime. If Dobius+ is not open yet, run \`dobius open\` first.
  Remote runtime access can also be supplied with DOBIUS_PAIRING_CODE or DOBIUS_ENVIRONMENT.
...
Agent Sessions And Worktrees:
  ...
  To start a fresh agent in the current worktree, use:
    dobius terminal create --worktree active --command "codex"

Browser Workflow:
  1. Create or navigate:  dobius tab create --url https://example.com
                          dobius goto --url https://example.com
  2. Inspect the page:    dobius snapshot
     (Returns an accessibility tree with element refs like e1, e2, e3)
     For concurrent workflows, prefer: dobius tab list --json
     then reuse tabs[].browserPageId with --page <id> on later commands.
  3. Interact:            dobius click --element e2
                          dobius fill --element e5 --value "search query"
                          dobius keypress --key Enter
  4. Re-inspect:          dobius snapshot
...
Examples:
  ...
  $ dobius diagnostics memory --json
  $ dobius repo list
  $ dobius worktree create --name agent-task --agent codex --prompt "hi"
  $ dobius worktree create --repo name:dobius --name cli-test-1 --issue 273
  $ dobius worktree create --repo name:dobius --name linear-task --linear-issue https://linear.app/stably/issue/STA-335/test-issue
  $ dobius worktree create --name linear-task --linear-issue STA-335
  $ dobius worktree show --worktree branch:Jinwoo-H/cli
  $ dobius worktree current
  $ dobius worktree set --worktree active --comment "waiting on review"
  $ dobius worktree set --worktree active --linear-issue null
  $ dobius worktree ps --limit 10
  $ dobius file open-changed --mode diff
  $ dobius file open src/App.tsx
  $ dobius terminal create --worktree active --command "codex"
  $ dobius terminal list --worktree path:/Users/me/dobius/workspaces/dobius/cli-test-1 --json
  $ dobius terminal send --terminal term_123 --text "hi" --enter
  $ dobius terminal wait --terminal term_123 --for exit --timeout-ms 60000 --json
  $ dobius tab current --json
  $ dobius tab show --page page_123 --json
  $ dobius tab create --url https://example.com --profile work
  $ dobius tab profile clone --page page_123 --profile work --json
  $ dobius snapshot
  $ dobius click --element e3
  $ dobius fill --element e5 --value "hello"
  $ dobius goto --url https://example.com/login
  $ dobius keypress --key Enter
  $ dobius eval --expression "document.title"
...
export function formatCommandHelp(spec: CommandSpec): string {
  const lines = [`dobius ${spec.path.join(' ')}`, '', `Usage: ${spec.usage}`, '', spec.summary]
...
export function formatGroupHelp(specs: CommandSpec[], group: string): string {
  ...
  const lines = [
    `dobius ${group}`,
    '',
    `Usage: dobius ${group} <command> [options]`,
    '',
    'Commands:'
  ]
  ...
  lines.push('', `Run \`dobius ${group} <command> --help\` for command-specific usage.`)
  return lines.join('\n')
}
...
export function formatFlagHelp(flag: string): string {
  const helpByFlag: Record<string, string> = {
    ...
    comment: '--comment <text>       Comment stored in Dobius+ metadata',
    ...
    activate: '--activate             Reveal the new worktree in the Dobius+ app',
    ...
    'display-name': '--display-name <name>  Override the Dobius+ display name',
    description: '--description <text>   Short description of what the agent is for',
    'system-prompt': '--system-prompt <text> System prompt defining the agent persona',
    model: '--model <id>           Claude model id for the agent, e.g. claude-fable-5',
    tools: '--tools <a,b>          Comma-separated allowed tools, e.g. Read,Grep,Glob,Bash',
    skills: '--skills <a,b>         Comma-separated skill names available to the agent',
    cwd: '--cwd <path>           Default working directory for agent runs',
    ...
    focus: '--focus                Reveal the created terminal session in Dobius+',
    ...
    name: '--name <name>          Name for the new worktree, automation, or agent',
    ...
    page: '--page <id>            Stable browser page id from `dobius tab list --json`',
  ...
  if (flag === 'current') {
    return '--current              Use the current Dobius+ worktree linked Linear issue'
  }
  if (flag === 'comments') {
