...
vi.mock('../runtime-client', () => {
  class RuntimeClient {
    ...
    constructor(
      ...
      remotePairingCode = process.env.DOBIUS_PAIRING_CODE ?? null,
      environmentSelector = process.env.DOBIUS_ENVIRONMENT ?? null
    ) {
...
describe('dobius linear CLI handlers', () => {
  ...
  beforeEach(() => {
    ...
    // Why: these tests can run inside an Dobius-managed terminal, which exports
  ...
  it('passes verified current-context hints without resolving cwd for remote runtimes', async () => {
    process.env.DOBIUS_TERMINAL_HANDLE = 'term_123'
    process.env.DOBIUS_WORKTREE_ID = 'repo::/srv/app'
    process.env.DOBIUS_PAIRING_CODE = 'dobius://pair?payload=bad'
    queueFixtures(callMock, okFixture('req_linear', issueResult()))
