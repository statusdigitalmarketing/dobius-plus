...
import { RuntimeClientError, serveDobiusApp } from '../runtime-client'
...
export const CORE_HANDLERS: Record<string, CommandHandler> = {
  'claude-teams': async ({ client, rawArgs }) => {
    ...
    if (!paneKey) {
      throw new RuntimeClientError(
        'invalid_environment',
        'dobius claude-teams must be run inside an Dobius terminal.'
      )
    }
  ...
  open: async ({ client, json }) => {
    const result = await client.openDobius()
  ...
  serve: async ({ flags, json }) => {
    ...
    const exitCode = await serveDobiusApp({
      json,
      port,
      pairingAddress:
