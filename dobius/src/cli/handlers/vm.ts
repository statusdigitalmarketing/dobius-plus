...
function doctorRecipe(repoPath: string, recipeId: string): DoctorResult {
  const yamlPath = join(repoPath, 'dobius.yaml')
  if (!existsSync(yamlPath)) {
    return {
      ...
      checks: [
        {
          id: 'dobius_yaml.exists',
          status: 'fail',
          message: `No dobius.yaml found at ${yamlPath}`,
          remediation: 'Add environmentRecipes to the repo dobius.yaml.'
        }
      ]
    }
  }

  const hooks = parseDobiusYaml(readTextFile(yamlPath))
  const parseCheck: EphemeralVmRecipeDoctorCheck = {
    id: 'dobius_yaml.parse',
    status: hooks ? 'pass' : 'fail',
    message: hooks
      ? 'dobius.yaml parsed successfully.'
      : 'dobius.yaml has no supported Dobius config.',
    ...(hooks ? {} : { remediation: 'Add an environmentRecipes entry to dobius.yaml.' })
  }
...
function loadRecipe(repoPath: string, recipeId: string): DobiusVmRecipe | null {
  const hooks = parseDobiusYaml(readTextFile(join(repoPath, 'dobius.yaml')))
