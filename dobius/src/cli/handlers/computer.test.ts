...
describe('dobius computer observation CLI handlers', () => {
  beforeEach(() => {
    vi.restoreAllMocks()
    callMock.mockReset()
  ...
  it('prints targeted permission setup command help', async () => {
    ...
    expect(output).toContain(
      'dobius computer permissions [--id <accessibility|screenshots>] [--json]'
    )
  ...
  it('prints provider capabilities without resolving a worktree', async () => {
    queueFixtures(
      ...
      okFixture('req_capabilities', {
        platform: 'darwin',
        provider: 'dobius-computer-use-macos',
    ...
    expect(vi.mocked(console.log).mock.calls[0][0]).toContain('dobius-computer-use-macos')
  })
  ...
  it('opens computer permission setup without resolving a worktree', async () => {
    ...
    const output = vi.mocked(console.log).mock.calls[0][0]
    expect(output).toContain('Opened Dobius+ Computer Use permission setup')
    expect(output).toContain('/Applications/Dobius+ Computer Use.app')
  })
