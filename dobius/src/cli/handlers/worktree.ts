...
function getEnvParentWorkspace(): string | undefined {
  const workspaceId = process.env.DOBIUS_WORKSPACE_ID
  if (typeof workspaceId === 'string' && isWorkspaceKey(workspaceId)) {
    return workspaceId
  }
  const worktreeId = process.env.DOBIUS_WORKTREE_ID
...
async function getCreateRepoSelector(
  ...
  throw new RuntimeClientError(
    'invalid_argument',
    'Missing repo selector. Pass --repo or run from inside an Dobius-managed worktree.'
  )
}
...
export const WORKTREE_HANDLERS: Record<string, CommandHandler> = {
  ...
  'worktree create': async ({ flags, client, cwd, json }) => {
    ...
    const callerTerminalHandle =
      typeof process.env.DOBIUS_TERMINAL_HANDLE === 'string' &&
      process.env.DOBIUS_TERMINAL_HANDLE.length > 0
        ? process.env.DOBIUS_TERMINAL_HANDLE
        : undefined
    ...
    if (
      ...
      try {
        // Why: agent shells can lose DOBIUS_TERMINAL_HANDLE while still running
        // inside an Dobius worktree. Cwd keeps CLI-created children nestable and
