...
export const VM_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    usage:
      'dobius vm recipe doctor <recipe-id> [--repo-path <path>] [--provision|--connect] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'recipe-id', 'repo-path', 'provision', 'connect'],
    positionalArgs: ['recipe-id'],
    notes: [
      'Reads environmentRecipes from dobius.yaml in the repo path, validates the selected recipe, and reports agent-friendly checks.',
    ...
    examples: [
      'dobius vm recipe doctor cloud-sandbox',
      'dobius vm recipe doctor cloud-sandbox --repo-path /path/to/repo --json',
      'dobius vm recipe doctor cloud-sandbox --provision --json'
    ]
  }
]
