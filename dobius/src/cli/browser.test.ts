...
describe('dobius cli browser page targeting', () => {
  beforeEach(() => {
    callMock.mockReset()
  })
  ...
  it('passes explicit page ids to snapshot without resolving the current worktree', async () => {
    ...
    await main(['snapshot', '--page', 'page-1', '--json'], '/tmp/not-an-dobius-worktree')
  ...
  it('shows tab profile labels in text mode when requested', async () => {
    ...
    await main(['tab', 'list', '--show-profile', '--worktree', 'all'], '/tmp/not-an-dobius-worktree')
  ...
  it('shows a single tab by page id', async () => {
    ...
    await main(['tab', 'show', '--page', 'page-1', '--json'], '/tmp/not-an-dobius-worktree')
  ...
  it('resolves the current tab in a worktree', async () => {
    ...
    await main(['tab', 'current', '--worktree', 'all', '--json'], '/tmp/not-an-dobius-worktree')
...
describe('dobius cli browser tab profiles', () => {
  beforeEach(() => {
    callMock.mockReset()
  })
...
describe('dobius cli browser tab profiles', () => {
  ...
  it('lists browser tab profiles', async () => {
    queueFixtures(
      ...
      okFixture('req_profiles', {
        profiles: [
          { id: 'default', scope: 'default', label: 'Default', partition: 'persist:dobius-browser' },
          {
            id: 'work',
            scope: 'isolated',
            label: 'Work',
            partition: 'persist:dobius-browser-session-work'
          }
        ]
      })
    )
    ...
    await main(['tab', 'profile', 'list', '--json'], '/tmp/not-an-dobius-worktree')
  ...
  it('reports an empty browser tab profile list with a friendly message', async () => {
    ...
    await main(['tab', 'profile', 'list'], '/tmp/not-an-dobius-worktree')
    ...
    queueFixtures(
      ...
      okFixture('req_profile_create', {
        profile: {
          ...
          scope: 'imported',
          label: 'From Chrome',
          partition: 'persist:dobius-browser-session-imp'
        }
      })
    )
    vi.spyOn(console, 'log').mockImplementation(() => {})
  ...
  it('surfaces a runtime error if the registry refuses to create a profile', async () => {
    ...
    await main(['tab', 'profile', 'create', '--label', 'Bogus'], '/tmp/not-an-dobius-worktree')
  ...
  it('reports a not-deleted profile in text mode without throwing', async () => {
    ...
    await main(['tab', 'profile', 'delete', '--profile', 'default'], '/tmp/not-an-dobius-worktree')
...
describe('dobius cli browser cookies', () => {
  beforeEach(() => {
    callMock.mockReset()
...
describe('dobius cli browser waits and viewport flags', () => {
  beforeEach(() => {
    callMock.mockReset()
    process.exitCode = undefined
  })
  ...
  it('does not tell users Dobius is down for a generic runtime timeout', async () => {
    callMock.mockRejectedValueOnce(
      new RuntimeClientError(
        'runtime_timeout',
        'Timed out waiting for the Dobius runtime to respond.'
      )
    )
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {})

    await main(['wait', '--selector', '#ready', '--worktree', 'all'], '/tmp/not-an-dobius-worktree')

    expect(errorSpy).toHaveBeenCalledWith('Timed out waiting for the Dobius runtime to respond.')
  })
