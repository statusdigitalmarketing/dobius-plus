...
afterEach(() => {
  vi.restoreAllMocks()
  delete process.env.DOBIUS_COMPUTER_SCREENSHOT_TMPDIR
...
describe('formatCliError', () => {
  it('prints structured computer-use startup recovery steps', () => {
    ...
    expect(output).toContain('Next step: Run `dobius computer list-apps --json`')
    expect(output).toContain('desktop browser app/window')
    expect(output).toContain('--app <web app>')
    expect(output).not.toContain('dobius goto')
  })
...
describe('formatAutomationShow', () => {
  ...
  it('shows explicit run context before the legacy repo id', () => {
    ...
    expect(output).toContain('runProjectId: github:statusdigitalmarketing/dobius-plus')
    ...
    expect(output).toContain('runPath: /srv/dobius')
...
describe('formatComputerAction', () => {
  it('includes routed worktree and explicit window target in the suggested follow-up command', () => {
    ...
    expect(output).toContain(
      `Use \`dobius computer get-app-state --app ${quoteCliCommandArgument('Text Editor')} --worktree id:repo::/tmp/repo --window-id 99\``
    )
  ...
  it('preserves explicit window-index targeting in the suggested follow-up command', () => {
    ...
    expect(output).toContain(
      'Use `dobius computer get-app-state --app com.apple.finder --session manual --window-index 1`'
    )
  })
  ...
  it('surfaces action screenshot failures in pretty output', () => {
    ...
    expect(output).toContain(
      'Use `dobius computer get-app-state --app com.apple.finder --window-id 42`'
    )
  ...
  it('drops stale requested window selectors after a window-changed fallback', () => {
    ...
    expect(output).toContain(
      'Use `dobius computer get-app-state --app com.apple.finder --session manual --window-id 42`'
    )
  ...
  it('uses snapshot window index for follow-up commands when the provider has no stable window id', () => {
    ...
    expect(output).toContain(
      `Use \`dobius computer get-app-state --app ${quoteCliCommandArgument('Linux Browser')} --window-index 2\``
    )
...
describe('printResult computer screenshots', () => {
  ...
  it('removes expired screenshot temp files when cleanup is due', () => {
    testScreenshotDir = mkdtempSync(join(tmpdir(), 'dobius-format-test-'))
    process.env.DOBIUS_COMPUTER_SCREENSHOT_TMPDIR = testScreenshotDir
  ...
  it('keeps inline screenshot data when temp export fails', () => {
    testScreenshotDir = join(tmpdir(), `dobius-format-blocked-${Date.now()}`)
