...
export async function resolveCurrentWorktreeSelector(
  ...
  if (!enclosingWorktree) {
    throw new RuntimeClientError(
      'selector_not_found',
      `No Dobius-managed worktree contains the current directory: ${currentPath}`
    )
  }
...
// like `dobius terminal send --text "hello" --enter` Just Work.
...
export type EmulatorCliTarget = {
  worktree?: string
  device?: string
  emulator?: string // Dobius id from list
}
