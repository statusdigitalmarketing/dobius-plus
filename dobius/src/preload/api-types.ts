...
export type DiagnosticsStatusPayload = {
  ...
  readonly disabledReason?:
    | 'do_not_track'
    | 'dobius_telemetry_disabled'
    | 'dobius_diagnostics_disabled'
    | 'ci'
}
...
export type AppApi = {
  ...
  /** Restarts Dobius through the normal quit pipeline so daemon-backed terminal
  ...
  /** Emits a startup benchmark marker when DOBIUS_STARTUP_DIAGNOSTICS is enabled. */
  ...
  /** Resolves Dobius's app-owned directory for auto-created Floating Workspace
   *  markdown notes. */
...
export type PreloadApi = {
  ...
  window: {
    ...
    openPhoneVisual: (args: FloatingPhoneWindowArgs) => Promise<FloatingPhoneWindowResult>
    closePhoneVisual: () => Promise<void>
    minimizePhoneVisual: () => Promise<void>
    toggleMaximizePhoneVisual: () => Promise<void>
    setPhoneVisualBounds: (bounds: FloatingPhoneBounds) => Promise<void>
    onPhoneVisualUpdate: (callback: (update: FloatingPhoneUpdate) => void) => () => void
  }
  platform: {
    get: () => {
  ...
  imessageBridge: {
    getConfig: () => Promise<ImessageBridgeConfig>
    updateConfig: (
      updates: Partial<Omit<ImessageBridgeConfig, 'lastSeenRowid'>>
    ) => Promise<ImessageBridgeConfig>
    status: () => Promise<ImessageBridgeStatus>
    openFullDiskAccess: () => Promise<void>
    testSend: () => Promise<ImessageBridgeSendResult>
  }
  asana: {
    getConfig: () => Promise<AsanaConfig>
    updateConfig: (updates: Partial<AsanaConfig>) => Promise<AsanaConfig>
    setToken: (pat: string) => Promise<void>
    hasToken: () => Promise<boolean>
    clearToken: () => Promise<void>
    listTasks: () => Promise<AsanaTasksSnapshot>
    refresh: () => Promise<AsanaTasksSnapshot>
  ...
  agents: {
    ...
    delete: (id: string) => Promise<CustomAgent[]>
    ...
    readCrewFiles: () => Promise<AgentCrewFiles>
    ...
    listBriefing: () => Promise<BriefingItem[]>
    dismissBriefing: () => Promise<void>
    listDrafts: () => Promise<AgentDraftComment[]>
  ...
  prompts: {
    ...
    delete: (id: string) => Promise<DobiusPrompt[]>
    inject: (text: string) => Promise<{ ok: boolean }>
  }
  crashReports: {
  ...
  gh: {
    ...
    checkDobiusStarred: () => Promise<boolean | null>
  ...
  ephemeralVm: {
    listRecipes: (args: { repoId: string }) => Promise<{
      ...
      recipes: DobiusHooks['environmentRecipes']
      diagnostics: NonNullable<DobiusHooks['environmentRecipeDiagnostics']>
      message?: string
    }>
    listRecipeCatalog: () => Promise<
      {
        ...
        repoName: string
        repoPath: string
        recipes: NonNullable<DobiusHooks['environmentRecipes']>
        diagnostics: NonNullable<DobiusHooks['environmentRecipeDiagnostics']>
      }[]
    >
