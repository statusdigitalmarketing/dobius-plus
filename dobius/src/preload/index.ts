...
import type { AsanaConfig, AsanaTasksSnapshot } from '../shared/asana'
import type {
  AgentRun,
  AgentRunEvent,
  AgentCrewFileName,
...
import type { FloatingPhoneUpdate } from '../shared/floating-phone'
import type {
  BaseRefSearchResult,
...
import {
  DOBIUS_EDITOR_PREPARE_HOT_EXIT_EVENT,
...
import {
  DOBIUS_INTERNAL_FILE_DRAG_TYPE,
...
function requestEditorHotExitBackup(): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    ...
    window.dispatchEvent(
      new CustomEvent<EditorPrepareHotExitDetail>(DOBIUS_EDITOR_PREPARE_HOT_EXIT_EVENT, {
        detail: {
          claim: () => {
            claimed = true
          },
...
document.addEventListener(
  ...
  (e) => {
    ...
    if (e.dataTransfer?.types.includes(DOBIUS_INTERNAL_FILE_DRAG_TYPE)) {
      return
    }
...
const startupDiagnosticsEnabled = process.env.DOBIUS_STARTUP_DIAGNOSTICS === '1'
...
const api = {
  app: {
    ...
    restart: async (): Promise<void> => {
      await prepareRendererForAppRestart({
        startedEventName: DOBIUS_APP_RESTART_STARTED_EVENT,
        abortedEventName: DOBIUS_APP_RESTART_ABORTED_EVENT
      })
      try {
        return await ipcRenderer.invoke('app:restart')
      } catch (error) {
        window.dispatchEvent(new Event(DOBIUS_APP_RESTART_ABORTED_EVENT))
        throw error
      }
    },
  ...
  window: {
    tearOffTerminal: (args): Promise<{ ok: true; windowId: number } | { ok: false }> =>
      ipcRenderer.invoke('window:tearOffTerminal', args),
    openPhoneVisual: (args) => ipcRenderer.invoke('window:openPhoneVisual', args),
    closePhoneVisual: () => ipcRenderer.invoke('phone-visual:close'),
    minimizePhoneVisual: () => ipcRenderer.invoke('phone-visual:minimize'),
    toggleMaximizePhoneVisual: () => ipcRenderer.invoke('phone-visual:toggleMaximize'),
    setPhoneVisualBounds: (bounds) => ipcRenderer.invoke('phone-visual:setBounds', bounds),
    onPhoneVisualUpdate: (callback): (() => void) => {
      const listener = (_event: Electron.IpcRendererEvent, update: FloatingPhoneUpdate): void =>
        callback(update)
      ipcRenderer.on('phone-visual:update', listener)
      return () => ipcRenderer.removeListener('phone-visual:update', listener)
    },
  } satisfies PreloadApi['window'],
  ...
  projectFiles: {
    list: (repoId: string): Promise<ProjectFilesListResult> =>
      ipcRenderer.invoke('project-files:list', repoId),
    read: (repoId: string, name: ProjectFileName): Promise<ProjectFileReadResult> =>
      ipcRenderer.invoke('project-files:read', repoId, name),
    write: (
      repoId: string,
      name: ProjectFileName,
      content: string
    ): Promise<ProjectFileWriteResult> =>
      ipcRenderer.invoke('project-files:write', repoId, name, content),
    delete: (repoId: string, name: ProjectFileName): Promise<ProjectFileDeleteResult> =>
      ipcRenderer.invoke('project-files:delete', repoId, name)
  } satisfies PreloadApi['projectFiles'],
  ...
  imessageBridge: {
    getConfig: (): Promise<ImessageBridgeConfig> => ipcRenderer.invoke('imessageBridge:getConfig'),
    updateConfig: (
      updates: Partial<Omit<ImessageBridgeConfig, 'lastSeenRowid'>>
    ): Promise<ImessageBridgeConfig> => ipcRenderer.invoke('imessageBridge:updateConfig', updates),
    status: (): Promise<ImessageBridgeStatus> => ipcRenderer.invoke('imessageBridge:status'),
    openFullDiskAccess: (): Promise<void> =>
      ipcRenderer.invoke('imessageBridge:openFullDiskAccess'),
    testSend: (): Promise<ImessageBridgeSendResult> => ipcRenderer.invoke('imessageBridge:testSend')
  } satisfies PreloadApi['imessageBridge'],

  asana: {
    getConfig: (): Promise<AsanaConfig> => ipcRenderer.invoke('asana:getConfig'),
    updateConfig: (updates: Partial<AsanaConfig>): Promise<AsanaConfig> =>
      ipcRenderer.invoke('asana:updateConfig', updates),
    setToken: (pat: string): Promise<void> => ipcRenderer.invoke('asana:setToken', pat),
    hasToken: (): Promise<boolean> => ipcRenderer.invoke('asana:hasToken'),
    clearToken: (): Promise<void> => ipcRenderer.invoke('asana:clearToken'),
    listTasks: (): Promise<AsanaTasksSnapshot> => ipcRenderer.invoke('asana:listTasks'),
    refresh: (): Promise<AsanaTasksSnapshot> => ipcRenderer.invoke('asana:refresh'),
    markLocalDone: (gid: string): Promise<AsanaTasksSnapshot> =>
      ipcRenderer.invoke('asana:markLocalDone', gid),
    clearLocalDone: (gid: string): Promise<AsanaTasksSnapshot> =>
      ipcRenderer.invoke('asana:clearLocalDone', gid),
    completeTask: (gid: string): Promise<AsanaTasksSnapshot> =>
      ipcRenderer.invoke('asana:completeTask', gid),
    onTasksUpdated: (callback: (snapshot: AsanaTasksSnapshot) => void): (() => void) => {
      const listener = (_event: unknown, snapshot: AsanaTasksSnapshot): void => callback(snapshot)
      ipcRenderer.on('asana:tasksUpdated', listener)
      return () => ipcRenderer.removeListener('asana:tasksUpdated', listener)
    }
  ...
  agents: {
    ...
    update: (id: string, updates: CustomAgentUpdate): Promise<CustomAgent[]> =>
      ipcRenderer.invoke('agents:update', id, updates),
    delete: (id: string): Promise<CustomAgent[]> => ipcRenderer.invoke('agents:delete', id),
    readFiles: (agentId: string): Promise<AgentReadableFiles> =>
      ipcRenderer.invoke('agents:readFiles', agentId),
    writeFile: (
      agentId: string,
      name: AgentIdentityFileName | 'brief' | 'memory',
      content: string
    ): Promise<void> => ipcRenderer.invoke('agents:writeFile', agentId, name, content),
    readCrewFiles: (): Promise<AgentCrewFiles> => ipcRenderer.invoke('agents:readCrewFiles'),
    writeCrewFile: (name: AgentCrewFileName, content: string): Promise<void> =>
      ipcRenderer.invoke('agents:writeCrewFile', name, content),
    resetSession: (id: string): Promise<CustomAgent[]> =>
      ipcRenderer.invoke('agents:resetSession', id),
    run: (args: { agentId: string; prompt: string }): Promise<string> =>
      ipcRenderer.invoke('agents:run', args),
    stop: (runId: string): Promise<void> => ipcRenderer.invoke('agents:stop', runId),
    listRuns: (): Promise<AgentRun[]> => ipcRenderer.invoke('agents:listRuns'),
    listDecisions: (): Promise<PendingAgentDecision[]> =>
      ipcRenderer.invoke('agents:listDecisions'),
    resolveDecision: (resolution: AgentDecisionResolution): Promise<{ ok: true; note?: string }> =>
      ipcRenderer.invoke('agents:resolveDecision', resolution),
    listNotifications: (): Promise<AgentNotificationsSnapshot> =>
      ipcRenderer.invoke('agents:listNotifications'),
    markNotificationsRead: (): Promise<AgentNotificationsSnapshot> =>
      ipcRenderer.invoke('agents:markNotificationsRead'),
    listBriefing: (): Promise<BriefingItem[]> => ipcRenderer.invoke('agents:listBriefing'),
    dismissBriefing: (): Promise<void> => ipcRenderer.invoke('agents:dismissBriefing'),
    listDrafts: (): Promise<AgentDraftComment[]> => ipcRenderer.invoke('agents:listDrafts'),
    approveDraft: (id: string): Promise<AgentDraftComment> =>
      ipcRenderer.invoke('agents:approveDraft', id),
    discardDraft: (id: string): Promise<AgentDraftComment> =>
      ipcRenderer.invoke('agents:discardDraft', id),
    getPaused: (): Promise<boolean> => ipcRenderer.invoke('agents:getPaused'),
    setPaused: (paused: boolean): Promise<boolean> =>
      ipcRenderer.invoke('agents:setPaused', paused),
    ...
    onRunsChanged: (callback: () => void): (() => void) => {
      ...
      ipcRenderer.on('agents:runsChanged', listener)
      return () => ipcRenderer.removeListener('agents:runsChanged', listener)
    },
    onDecisionsChanged: (callback: () => void): (() => void) => {
      const listener = (): void => callback()
      ipcRenderer.on('agents:decisionsChanged', listener)
      return () => ipcRenderer.removeListener('agents:decisionsChanged', listener)
    },
    onNotificationsChanged: (callback: () => void): (() => void) => {
      const listener = (): void => callback()
      ipcRenderer.on('agents:notificationsChanged', listener)
      return () => ipcRenderer.removeListener('agents:notificationsChanged', listener)
    },
    onBriefingChanged: (callback: () => void): (() => void) => {
      const listener = (): void => callback()
      ipcRenderer.on('agents:briefingChanged', listener)
      return () => ipcRenderer.removeListener('agents:briefingChanged', listener)
    },
    onDraftsChanged: (callback: () => void): (() => void) => {
      const listener = (): void => callback()
      ipcRenderer.on('agents:draftsChanged', listener)
      return () => ipcRenderer.removeListener('agents:draftsChanged', listener)
    }
  } satisfies PreloadApi['agents'],

  prompts: {
    list: (): Promise<DobiusPrompt[]> => ipcRenderer.invoke('prompts:list'),
    save: (input: { id?: string; title: string; text: string }): Promise<DobiusPrompt[]> =>
      ipcRenderer.invoke('prompts:save', input),
    delete: (id: string): Promise<DobiusPrompt[]> => ipcRenderer.invoke('prompts:delete', id),
    inject: (text: string): Promise<{ ok: boolean }> => ipcRenderer.invoke('prompts:inject', text)
  } satisfies PreloadApi['prompts'],
  ...
  gh: {
    ...
    checkDobiusStarred: (): Promise<boolean | null> => ipcRenderer.invoke('gh:checkDobiusStarred'),
    starDobius: (source: AppStarSource): Promise<boolean> =>
      ipcRenderer.invoke('gh:starDobius', source),
  ...
  starNag: {
    ...
    starDobius: (): Promise<boolean> => ipcRenderer.invoke('star-nag:starDobius'),
  ...
  skills: {
    ...
    installBundled: (skillName: string): Promise<BundledSkillInstallResult> =>
      ipcRenderer.invoke('skills:installBundled', skillName),
    isBundledInstalled: (skillName: string): Promise<boolean> =>
      ipcRenderer.invoke('skills:isInstalled', skillName)
  },

  knowledge: {
    tree: (): Promise<KnowledgeTree> => ipcRenderer.invoke('knowledge:tree'),
    read: (filePath: string): Promise<KnowledgeReadResult> =>
      ipcRenderer.invoke('knowledge:read', filePath),
    onChanged: (callback: () => void): (() => void) => {
      void ipcRenderer.invoke('knowledge:watch')
      const listener = (): void => callback()
  ...
  browser: {
    ...
    onOpenLinkInDobiusTab: (
      ...
      ipcRenderer.on('browser:open-link-in-dobius-tab', listener)
      return () => ipcRenderer.removeListener('browser:open-link-in-dobius-tab', listener)
    },
  ...
  updater: {
    ...
    quitAndInstall: async (): Promise<void> => {
      await prepareRendererForAppRestart({
        startedEventName: DOBIUS_UPDATER_QUIT_AND_INSTALL_STARTED_EVENT,
        abortedEventName: DOBIUS_UPDATER_QUIT_AND_INSTALL_ABORTED_EVENT
      })
      try {
        return await ipcRenderer.invoke('updater:quitAndInstall')
      } catch (error) {
        window.dispatchEvent(new Event(DOBIUS_UPDATER_QUIT_AND_INSTALL_ABORTED_EVENT))
        throw error
      }
    },
    onStatus: (callback) => {
  ...
  destinations: {
    list: (): Promise<Destination[]> => ipcRenderer.invoke('destinations:list'),
    save: (input: DestinationSaveInput): Promise<Destination> =>
      ipcRenderer.invoke('destinations:save', input),
    delete: (id: string): Promise<boolean> => ipcRenderer.invoke('destinations:delete', id),
    test: (id: string): Promise<DestinationTestResult> =>
      ipcRenderer.invoke('destinations:test', id)
  },
