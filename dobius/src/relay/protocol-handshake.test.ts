...
describe('handshake framing', () => {
  it('round-trips an dobius-relay-handshake envelope through the existing framing', () => {
    const sent = encodeHandshakeFrame({
      type: 'dobius-relay-handshake',
    ...
    expect(msg).toEqual({ type: 'dobius-relay-handshake', version: '0.1.0+deadbeef' })
  })

  it('round-trips an dobius-relay-handshake-ok reply', () => {
    const sent = encodeHandshakeFrame({
      type: 'dobius-relay-handshake-ok',
      version: '0.1.0+deadbeef'
    })
    const frames: DecodedFrame[] = []
    const decoder = new FrameDecoder((f) => frames.push(f))
    decoder.feed(sent)
    const msg = parseHandshakeMessage(frames[0].payload)
    expect(msg).toEqual({ type: 'dobius-relay-handshake-ok', version: '0.1.0+deadbeef' })
  })

  it('round-trips an dobius-relay-handshake-mismatch reply', () => {
    const sent = encodeHandshakeFrame({
  ...
  it('rejects payloads with unknown type', () => {
    const bogus = Buffer.from(JSON.stringify({ type: 'dobius-something-else', version: 'x' }))
