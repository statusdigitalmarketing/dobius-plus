...
// Dobius Relay — lightweight daemon deployed to remote hosts.
...
const EMPTY_DETACHED_STARTUP_GRACE_MS = parseNonNegativeIntEnv(
  'DOBIUS_RELAY_EMPTY_STARTUP_GRACE_MS',
  60_000
)
...
async function runDobiusCliMode(sockPath: string, argv: string[]): Promise<void> {
  const myVersion = readLaunchVersion()
  const stdin = shouldReadRemoteCliStdin(argv) ? await readDobiusCliStdin() : undefined
  const sock = createConnection({ path: sockPath })
  let nextSeq = 1
  ...
  const connectTimeout = setTimeout(() => {
    process.stderr.write(`[dobius-cli] Relay connection timed out after ${CONNECT_TIMEOUT_MS}ms\n`)
    sock.destroy()
    process.exit(1)
  }, CONNECT_TIMEOUT_MS)
  ...
  sock.on('error', (err) => {
    clearTimeout(connectTimeout)
    process.stderr.write(`[dobius-cli] Relay socket error: ${err.message}\n`)
    process.exit(1)
  })
}

async function readDobiusCliStdin(): Promise<string | undefined> {
...
async function main(): Promise<void> {
  ...
  if (cliMode) {
    const marker = process.argv.indexOf('--dobius-cli')
    await runDobiusCliMode(sockPath, marker >= 0 ? process.argv.slice(marker + 1) : [])
    return
  }
  ...
  dispatcher.onRequest('dobius.cli', async (params, context) => {
    return await dispatcher.requestAnyClient('dobius.cli', params, {
  ...
  // host. Each parsed payload is forwarded to Dobius via an `agent.hook`
  ...
  const hookServer = new RelayAgentHookServer({
    ...
    forward: (envelope) => {
      ...
      // replay the last status for each live pane after Dobius re-wires its
  ...
  // already sees populated DOBIUS_AGENT_HOOK_* env. The bind is a local-loopback
  ...
  // Why: every relay-spawned PTY needs the live DOBIUS_AGENT_HOOK_* coords. The
  ...
  ptyHandler.addEnvAugmenter((ctx) => {
    ...
    if (pluginOverlay.hasOpenCodeSource()) {
      ...
      if (dir) {
        ...
        env.DOBIUS_OPENCODE_CONFIG_DIR = dir
        if (sourceDir) {
          env.DOBIUS_OPENCODE_SOURCE_CONFIG_DIR = sourceDir
        }
      }
    }
    if (pluginOverlay.hasPiSource()) {
      // Why: source-dir defaulting is keyed on which Pi-compatible agent is
      // being launched (Pi vs OMP). Install Dobius's guarded extension into that
      ...
      if (kind === 'pi') {
        ...
        if (dir) {
          env.DOBIUS_PI_SOURCE_AGENT_DIR = dir
        }
      }
      if (shouldPrepareOmpShadow) {
        ...
        const sourceDir =
          kind === 'omp'
            ...
            : ctx.env.DOBIUS_OMP_SOURCE_AGENT_DIR
        const dir = pluginOverlay.materializePi(overlayId, sourceDir, 'omp')
        if (dir) {
          env.DOBIUS_OMP_STATUS_EXTENSION = getRelayPiStatusExtensionPath(dir)
          env.DOBIUS_OMP_SOURCE_AGENT_DIR = dir
        }
      }
    }
    return env
  ...
  // Why: request-driven replay. Dobius issues this *after* it re-wires the
  ...
  // Why: Dobius ships the OpenCode plugin / Pi extension source bodies over
  ...
  // would force a relay redeploy on every Dobius update). Cache them so each
  ...
  // Why: bound the per-source size so a buggy/hostile Dobius can't OOM the
