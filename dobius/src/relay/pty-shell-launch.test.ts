...
describe('getRelayShellLaunchConfig', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      const zshRoot = join(homeDir, '.dobius-relay', 'shell-ready', 'zsh')
      ...
      expect(readFileSync(join(zshRoot, '.zprofile'), 'utf8')).toContain(
        '_dobius_home="${DOBIUS_USER_ZDOTDIR:-${DOBIUS_ORIG_ZDOTDIR:-$HOME}}"'
      )
      ...
      expect(zshrc).toContain('_dobius_home="${DOBIUS_USER_ZDOTDIR:-${DOBIUS_ORIG_ZDOTDIR:-$HOME}}"')
      expect(zlogin).toContain('_dobius_home="${DOBIUS_USER_ZDOTDIR:-${DOBIUS_ORIG_ZDOTDIR:-$HOME}}"')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.args).toEqual(['-l'])
      expect(config.env.ZDOTDIR).toBe(zshRoot)
      expect(config.env.DOBIUS_SHELL_READY_MARKER).toBe('1')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env.DOBIUS_SHELL_READY_MARKER).toBe('1')
      expect(bashRc).toContain('__dobius_append_prompt_command "__dobius_prompt_mark"')
      expect(bashRc).toContain('printf "\\033]777;dobius-shell-ready\\007"')
    }
  )
  ...
  // Dobius inherits ends in a ";"+whitespace separator. Prepend/append must not
