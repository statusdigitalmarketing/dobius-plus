// Why: relay-side equivalent of Dobius's local agent integration installers.
// OpenCode still needs a config overlay, while Pi/OMP now get Dobius-managed
...
// relay is versioned independently from Dobius and the plugin source changes
...
// directly: those modules import `electron` and ride on Dobius's userData
// path. The relay's electron-free constraint forces a thin parallel
// implementation rooted at $HOME/.dobius-relay/ for OpenCode and at the remote
...
const RELAY_HOOKS_DIR = '.dobius-relay'
const OPENCODE_OVERLAY_SUBDIR = 'opencode-overlays'
const PI_OVERLAY_SUBDIR_BY_KIND: Record<PiAgentKind, string> = {
  pi: 'pi-overlays',
  omp: 'omp-overlays'
}
const OPENCODE_PLUGIN_FILE = 'dobius-opencode-status.js'
const PI_EXTENSION_FILE = 'dobius-agent-status.ts'
const PI_AGENT_SUBDIR = 'agent'
const DOBIUS_MANAGED_EXTENSION_MARKER = '@dobius-managed-pi-extension'

function withDobiusManagedPiExtensionMarker(source: string): string {
  return source.includes(DOBIUS_MANAGED_EXTENSION_MARKER)
    ? source
    : `// ${DOBIUS_MANAGED_EXTENSION_MARKER}\n${source}`
}
...
function safeDirName(input: string): string {
  ...
  // filesystem-unsafe characters in some Dobius builds. Hash to a fixed-width
...
export type PluginSources = {
  /** Source body of `dobius-opencode-status.js` to drop into <overlay>/plugins/. */
  opencodePluginSource?: string
  /** Source body of Pi's `dobius-agent-status.ts` to drop into <overlay>/extensions/. */
  piExtensionSource?: string
  /** Source body of OMP's `dobius-agent-status.ts` to drop into <overlay>/extensions/. */
...
export class PluginOverlayManager {
  ...
  /** Replace the cached source bodies. Called from relay.ts when Dobius sends
   *  `agent_hook.installPlugins`. The first install enables the augmenter
   *  output; subsequent installs (e.g. Dobius version upgrade in flight) refresh
  ...
  setSources(sources: PluginSources): void {
    ...
    if (typeof sources.piExtensionSource === 'string') {
      this.piExtensionSources.pi = withDobiusManagedPiExtensionMarker(sources.piExtensionSource)
    }
    if (typeof sources.ompExtensionSource === 'string') {
      this.piExtensionSources.omp = withDobiusManagedPiExtensionMarker(sources.ompExtensionSource)
    }
  }
  ...
  materializeOpenCode(id: string, existingConfigDir?: string): string | null {
    ...
    try {
      ...
      if (existingConfigDir) {
        ...
        // remote root into the overlay before adding Dobius's plugin so status
  ...
  private canOverwritePiExtension(path: string): boolean {
    try {
      return readFileSync(path, 'utf8').includes(DOBIUS_MANAGED_EXTENSION_MARKER)
    } catch {
      return true
    }
  }
