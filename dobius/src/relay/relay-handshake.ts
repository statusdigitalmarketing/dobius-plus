// Wire-level handshake helpers for the Dobius relay.
//
...
export function setupDaemonHandshake(sock: Socket, cb: DaemonHandshakeCallbacks): void {
  ...
  ;(sock as Socket & { __dobiusOnHandshake?: typeof onHandshakeData }).__dobiusOnHandshake =
    onHandshakeData
}

export function detachHandshakeListener(sock: Socket): void {
  const tagged = sock as Socket & { __dobiusOnHandshake?: (chunk: Buffer) => void }
  if (tagged.__dobiusOnHandshake) {
    sock.removeListener('data', tagged.__dobiusOnHandshake)
    delete tagged.__dobiusOnHandshake
  }
}
...
function handleDaemonHandshakeFrame(
  ...
  if (msg.type !== 'dobius-relay-handshake') {
    process.stderr.write(
  ...
  if (msg.version !== launchVersion) {
    ...
    try {
      sock.write(
        encodeHandshakeFrame({
          type: 'dobius-relay-handshake-mismatch',
  ...
  sock.write(encodeHandshakeFrame({ type: 'dobius-relay-handshake-ok', version: launchVersion }))
  return true
}
...
export function runConnectHandshake(
  ...
  const decoder: FrameDecoder = new FrameDecoder(
    (frame: DecodedFrame) => {
      ...
      if (msg.type === 'dobius-relay-handshake-ok') {
      ...
      if (msg.type === 'dobius-relay-handshake-mismatch') {
  ...
  sock.write(encodeHandshakeFrame({ type: 'dobius-relay-handshake', version: myVersion }))
}
