...
export class RelayDispatcher {
  ...
  // Why: synced remote workspaces can have more than one Dobius client attached
  ...
  requestAnyClient(
    ...
    // though the owning Dobius is attached through a Unix-socket client. Prefer a
    // real attached client so remote `dobius` shims do not forward to dead stdout.
    const target = candidates.find((client) => client !== this.primaryClient) ?? candidates[0]
    if (!target) {
      return Promise.reject(new Error('No owning Dobius client is connected to the relay'))
    }
