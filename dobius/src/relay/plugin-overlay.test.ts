...
describe('PluginOverlayManager', () => {
  ...
  it('materializes OpenCode plugin into <overlay>/plugins/<file>', () => {
    ...
    const expected = join(dir!, 'plugins', 'dobius-opencode-status.js')
    expect(existsSync(expected)).toBe(true)
    expect(readFileSync(expected, 'utf8')).toBe('export const X = 1')
  })

  it('mirrors a preexisting remote OpenCode config dir before adding Dobius plugin', () => {
    ...
    writeFileSync(join(userConfigDir, 'plugins', 'dobius-opencode-status.js'), 'user same-name')

    manager.setSources({ opencodePluginSource: 'dobius plugin' })
    ...
    expect(readFileSync(join(dir!, 'plugins', 'dobius-opencode-status.js'), 'utf8')).toBe(
      'dobius plugin'
    )
  ...
  it('installs Pi extension into the real agent extensions dir', () => {
    ...
    expect(dir).not.toBeNull()
    const file = join(dir!, 'extensions', 'dobius-agent-status.ts')
    expect(existsSync(file)).toBe(true)
    expect(readFileSync(file, 'utf8')).toContain('@dobius-managed-pi-extension')
  })

  it("does not overwrite a user's same-named remote Pi extension file", () => {
    const piAgentDir = join(homeDir, '.pi', 'agent')
    const extensionFile = join(piAgentDir, 'extensions', 'dobius-agent-status.ts')
  ...
  it('uses the kind-specific Pi-compatible extension source when available', () => {
    ...
    expect(readFileSync(join(piDir!, 'extensions', 'dobius-agent-status.ts'), 'utf8')).toContain(
      '// pi extension'
    )
    expect(readFileSync(join(ompDir!, 'extensions', 'dobius-agent-status.ts'), 'utf8')).toContain(
      '// omp extension'
    )
  })

  it('installs Dobius status extension into the remote default Pi agent dir', () => {
    const piAgentDir = join(homeDir, '.pi', 'agent')
  ...
  it('mirrors a preexisting remote Pi agent dir instead of the default', () => {
    ...
    expect(readFileSync(join(dir!, 'extensions', 'dobius-agent-status.ts'), 'utf8')).toContain(
      '// pi extension'
    )
  })
  ...
  it('leaves lazy OMP agent.db in the real remote home on the relay', () => {
    ...
    expect(existsSync(join(homeDir, '.dobius-relay', 'omp-overlays'))).toBe(false)
  ...
  describe('per-agent default source dir (no cross-agent fallback)', () => {
    ...
    it('launching pi with both ~/.pi/agent and ~/.omp/agent present installs into ~/.pi/agent', () => {
      ...
      expect(extensions).toContain('dobius-agent-status.ts')
    ...
    it('launching omp when only ~/.pi/agent exists does NOT mirror Pi state', () => {
      ...
      const extensions = readdirSync(join(dir!, 'extensions')).sort()
      expect(extensions).toEqual(['dobius-agent-status.ts'])
    })
  })
  ...
  it('produces stable overlay dirs for a given id (idempotent re-materialization)', () => {
    ...
    expect(readFileSync(join(dirA, 'plugins', 'dobius-opencode-status.js'), 'utf8')).toBe('second')
  })
  ...
  it('hashes unsafe pane ids into portable overlay directory names', () => {
    ...
    expect(existsSync(join(dir!, 'plugins', 'dobius-opencode-status.js'))).toBe(true)
  })
})
...
describe('resolvePiSourceAgentDir', () => {
  it('uses only the selected kind source shadow when resolving inherited overlays', () => {
    const env = {
      ...
      DOBIUS_PI_SOURCE_AGENT_DIR: '/user/.pi/agent'
    }
    try {
  ...
  it('keeps explicit PI_CODING_AGENT_DIR values when they are not Dobius overlays', () => {
    const env = {
      HOME: mkdtempSync(join(tmpdir(), 'plugin-overlay-env-')),
