...
type ManagedPty = {
  ...
  /** Renderer-supplied paneKey from spawn env (DOBIUS_PANE_KEY). Captured so
   *  external observers (the relay-hook-server cache) can evict per-pane
   *  state when this PTY exits. Symmetric with Dobius's local pty.ts. */
  paneKey?: string
  tabId?: string
  worktreeId?: string
...
export class PtyHandler {
  ...
  // server's DOBIUS_AGENT_HOOK_* coords). Run on every spawn so every PTY
   ...
   *  relay-hook server to inject DOBIUS_AGENT_HOOK_PORT/TOKEN/ENV/VERSION/
   ...
   *  see the fresh DOBIUS_AGENT_HOOK_* coords just like freshly-spawned ones,
  ...
  private async spawn(
    ...
    // Why: server-side augmenter values (DOBIUS_AGENT_HOOK_* and plugin overlay
    ...
    const paneKey = typeof env?.DOBIUS_PANE_KEY === 'string' ? env.DOBIUS_PANE_KEY : undefined
    ...
    const terminalHandle =
      typeof env?.DOBIUS_TERMINAL_HANDLE === 'string' ? env.DOBIUS_TERMINAL_HANDLE : undefined
    ...
    const term = pty.spawn(shell, shellLaunch.args, {
      ...
      rows,
      cwd,
      // Why: relay shells inherit process.env; never let an ambient Dobius marker
      // enable shell-ready behavior unless this spawn explicitly requested it.
      env: { ...spawnEnv, DOBIUS_SHELL_READY_MARKER: '0', ...shellLaunch.env }
    })
    ...
    // separate ptyId→paneKey map. DOBIUS_PANE_KEY is shaped `${tabId}:${paneId}`
    // and is bounded by the renderer; the relay treats it as opaque.
    const tabId = typeof env?.DOBIUS_TAB_ID === 'string' ? env.DOBIUS_TAB_ID : undefined
    const worktreeId =
      typeof env?.DOBIUS_WORKTREE_ID === 'string' ? env.DOBIUS_WORKTREE_ID : undefined
      ...
      ...(shouldProviderDeliverCommand
        ? {
            startupCommand: {
              command,
              delivered: false,
              waitForShellReady: shellLaunch.env.DOBIUS_SHELL_READY_MARKER === '1',
              scanState:
                shellLaunch.env.DOBIUS_SHELL_READY_MARKER === '1'
                  ? createShellReadyScanState()
                  : null,
              timer: null
            }
  ...
  private async revive(params: Record<string, unknown>): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (entry.paneKey) {
        revivedEnv.DOBIUS_PANE_KEY = entry.paneKey
      }
      if (entry.tabId) {
        revivedEnv.DOBIUS_TAB_ID = entry.tabId
      }
      if (entry.worktreeId) {
        revivedEnv.DOBIUS_WORKTREE_ID = entry.worktreeId
      }
      if (entry.terminalHandle) {
        revivedEnv.DOBIUS_TERMINAL_HANDLE = entry.terminalHandle
      }
