...
describe('PtyHandler', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const spawnOptions = mockPtySpawn.mock.calls[0]?.[2] as
        ...
        | undefined
      expect(spawnOptions?.env?.DOBIUS_SHELL_READY_MARKER).toBe('1')
      expect(handler.retainedStartupCommandCount).toBe(0)
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      dataCallback?.('\x1b]777;dobius-shell-ready\x07user@remote $ ')
      vi.advanceTimersByTime(49)
      expect(term.write).not.toHaveBeenCalled()
      vi.advanceTimersByTime(1)
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      expect(dispatcher.notify).toHaveBeenCalledWith('pty.data', {
        id: 'pty-1',
        data: '\x1b]777;dobius-shell-ready'
      })
      ...
      expect(result).toEqual({ replay: '\x1b]777;dobius-shell-ready' })
    }
  )
  ...
  it('notifies pty.exit when graceful shutdown falls back to SIGKILL', async () => {
    ...
    await dispatcher.callRequest('pty.spawn', { env: { DOBIUS_PANE_KEY: 'tab-fallback:0' } })
    await dispatcher.callRequest('pty.shutdown', { id: 'pty-1', immediate: false })
    vi.advanceTimersByTime(5000)
  ...
  it('applies env augmenters after process.env and renderer-supplied env (augmenter wins on key conflict)', async () => {
    handler.addEnvAugmenter(() => ({
      ...
      DOBIUS_AGENT_HOOK_TOKEN: 'abc-uuid',
      ...
      DOBIUS_PANE_KEY: 'augmenter-wins'
    }))

    await dispatcher.callRequest('pty.spawn', {
      cols: 80,
      rows: 24,
      env: { DOBIUS_PANE_KEY: 'tab-1:0', DOBIUS_TAB_ID: 'tab-1' }
    })

    expect(mockPtySpawn).toHaveBeenCalled()
    const callArgs = mockPtySpawn.mock.calls[0][2] as { env: Record<string, string> }
    expect(callArgs.env.DOBIUS_AGENT_HOOK_PORT).toBe('12345')
    expect(callArgs.env.DOBIUS_AGENT_HOOK_TOKEN).toBe('abc-uuid')
    // Augmenter override beats the renderer-supplied value:
    expect(callArgs.env.DOBIUS_PANE_KEY).toBe('augmenter-wins')
    // Renderer-supplied keys not in augmenter map flow through:
    expect(callArgs.env.DOBIUS_TAB_ID).toBe('tab-1')
  })
  ...
  it('passes the PTY id and renderer paneKey to env augmenters', async () => {
    ...
    await dispatcher.callRequest('pty.spawn', {
      env: { DOBIUS_PANE_KEY: 'tab-context:0' }
    })
    await dispatcher.callRequest('pty.spawn', {})
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const oldShell = process.env.SHELL
      const oldHome = process.env.HOME
      const oldDobiusPi = process.env.DOBIUS_PI_CODING_AGENT_DIR
      ...
      try {
        ...
        handler.addEnvAugmenter(() => ({
          ...
          DOBIUS_OPENCODE_CONFIG_DIR: '/remote/overlay/opencode',
          DOBIUS_OMP_STATUS_EXTENSION: '/remote/.omp/agent/extensions/dobius-agent-status.ts'
        }))
        ...
        if (oldDobiusPi === undefined) {
          delete process.env.DOBIUS_PI_CODING_AGENT_DIR
        } else {
          process.env.DOBIUS_PI_CODING_AGENT_DIR = oldDobiusPi
        }
      }

      const shellArgs = mockPtySpawn.mock.calls[0][1]
      const spawnOptions = mockPtySpawn.mock.calls[0][2] as { env: Record<string, string> }
      const rcfile = join(homeDir, '.dobius-relay', 'shell-ready', 'bash', 'rcfile')

      expect(shellArgs).toEqual(['--rcfile', rcfile])
      expect(spawnOptions.env.DOBIUS_OPENCODE_CONFIG_DIR).toBe('/remote/overlay/opencode')
      expect(spawnOptions.env.DOBIUS_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(readFileSync(rcfile, 'utf8')).toContain(
        'export OPENCODE_CONFIG_DIR="${DOBIUS_OPENCODE_CONFIG_DIR}"'
      )
      expect(readFileSync(rcfile, 'utf8')).not.toContain('DOBIUS_PI_CODING_AGENT_DIR')
  ...
  it('revive restores pane identity env alongside hook-server coordinates', async () => {
    await dispatcher.callRequest('pty.spawn', {
      ...
      cwd: '/tmp',
      env: {
        DOBIUS_PANE_KEY: 'tab-5:1',
        DOBIUS_TAB_ID: 'tab-5',
        DOBIUS_WORKTREE_ID: 'wt-5'
      }
    })
    ...
    const callArgs = mockPtySpawn.mock.calls[0][2] as { env: Record<string, string> }
    expect(callArgs.env.DOBIUS_PANE_KEY).toBe('tab-5:1')
    expect(callArgs.env.DOBIUS_TAB_ID).toBe('tab-5')
    expect(callArgs.env.DOBIUS_WORKTREE_ID).toBe('wt-5')
    expect(callArgs.env.DOBIUS_AGENT_HOOK_PORT).toBe('12345')
    expect(callArgs.env.DOBIUS_AGENT_HOOK_TOKEN).toBe('abc-uuid')
  })
  ...
  it('immediate shutdown invokes the exit listener once even if onExit arrives later', async () => {
    ...
    await dispatcher.callRequest('pty.spawn', {
      env: { DOBIUS_PANE_KEY: 'tab-shutdown:0' }
    })
    await dispatcher.callRequest('pty.shutdown', { id: 'pty-1', immediate: true })
    onExitCb!({ exitCode: 0 })
  ...
  it('dispose kills all PTYs with SIGKILL and invokes exit listeners', async () => {
    ...
    await dispatcher.callRequest('pty.spawn', { env: { DOBIUS_PANE_KEY: 'tab-dispose:0' } })
    await dispatcher.callRequest('pty.spawn', { env: { DOBIUS_PANE_KEY: 'tab-dispose:1' } })
