 ...
 *
 * Wires Dobius's main-side SshChannelMultiplexer to the relay-side
...
describe('Integration: relay hook server → mux → AgentHookServer.ingestRemote', () => {
  ...
  let dobiusServer: AgentHookServer
  ...
  beforeEach(async () => {
    ...
    dobiusServer = new AgentHookServer()
    // Why: Dobius-side never starts an HTTP server in this test — `ingestRemote`
  ...
  it('forwards a Claude UserPromptSubmit POST through to ingestRemote', async () => {
    ...
    dobiusServer.setListener((event) => {
      events.push({
  ...
  it('clears remote Claude permission when the approved tool starts with matching identity', async () => {
    ...
    const start = Date.now()
    while (dobiusServer.getStatusSnapshot()[0]?.state !== 'working' && Date.now() - start < 1500) {
      await new Promise((r) => setImmediate(r))
    }
    expect(dobiusServer.getStatusSnapshot()).toEqual([
