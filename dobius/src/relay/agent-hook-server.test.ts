...
describe('RelayAgentHookServer', () => {
  ...
  it('keeps named-pipe endpoint files on a real filesystem path', () => {
    const endpointDir = endpointDirForRelaySocket('\\\\.\\pipe\\dobius-relay-abc123')

    expect(endpointDir).toBe(join(homedir(), '.dobius-relay', 'agent-hooks', 'dobius-relay-abc123'))
  ...
  it('forwards a parsed Claude UserPromptSubmit POST as a normalized envelope', async () => {
    ...
    try {
      ...
      // Why: the relay forwards body env/version so Dobius's warn-once
  ...
  it('rejects requests with the wrong bearer token (403)', async () => {
    ...
    try {
      ...
      const res = await fetch(`http://127.0.0.1:${port}/hook/claude`, {
        ...
        headers: {
          'Content-Type': 'application/json',
          'X-Dobius-Agent-Hook-Token': 'wrong'
        },
        body: '{}'
      })
  ...
  // the wire, even though Dobius main re-validates at the SSH trust boundary.
  ...
  it('exposes DOBIUS_AGENT_HOOK_* env vars after start', async () => {
    const forward = vi.fn()
    const server = new RelayAgentHookServer({ endpointDir: dir, forward })
    await server.start()
    try {
      const env = server.buildPtyEnv()
      expect(env.DOBIUS_AGENT_HOOK_PORT).toMatch(/^\d+$/)
      expect(env.DOBIUS_AGENT_HOOK_TOKEN).toBeTruthy()
      expect(env.DOBIUS_AGENT_HOOK_ENV).toBe('remote')
      expect(env.DOBIUS_AGENT_HOOK_VERSION).toBe('1')
      expect(env.DOBIUS_AGENT_HOOK_ENDPOINT).toBeTruthy()
    } finally {
      server.stop()
    }
  })
  ...
  it('can defer endpoint file publication until relay socket ownership is proven', async () => {
    ...
    try {
      ...
      expect(server.buildPtyEnv().DOBIUS_AGENT_HOOK_ENDPOINT).toBeTruthy()
    } finally {
      server.stop()
    }
  })
