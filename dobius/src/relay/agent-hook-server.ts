...
// a loopback HTTP server (same shape as Dobius's main-process server: bind
...
const RELAY_HOOKS_DIR_NAME = '.dobius-relay'
...
export type RelayHookServerOptions = {
  /** Where to put endpoint.env / endpoint.cmd. Defaults to `$HOME/.dobius-relay/agent-hooks`. */
  endpointDir?: string
...
export class RelayAgentHookServer {
  ...
  // the original `source`/`env`/`version` so Dobius's warn-once diagnostics fire
   ...
   *  forwards each entry as a fresh notification. Called after Dobius has
  ...
  buildPtyEnv(): Record<string, string> {
    ...
    if (this.endpointFileWritten) {
      env.DOBIUS_AGENT_HOOK_ENDPOINT = this.endpointFilePath
    }
    return env
  }
