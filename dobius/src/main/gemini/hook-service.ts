...
// (approvals flow through inline UI), so Dobius cannot surface a waiting state
// for Gemini — that is an upstream limitation, not an Dobius bug.
//
...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  ...
  return [
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${DOBIUS_AGENT_HOOK_PORT}/hook/gemini" \\',
...
export class GeminiHookService {
  ...
  install(): AgentHookInstallStatus {
    ...
    // Why: when Dobius stops subscribing to an event, install() must sweep the
  ...
  // Why: install Dobius's managed Gemini hooks on the remote box. Mirrors
  ...
  async installRemote(sftp: SFTPWrapper, remoteHome: string): Promise<AgentHookInstallStatus> {
    ...
    const remoteScriptPath = `${remoteHome.replace(/\/$/, '')}/.dobius/agent-hooks/gemini-hook.sh`
    try {
