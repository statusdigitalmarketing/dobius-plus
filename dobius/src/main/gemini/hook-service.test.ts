...
describe('GeminiHookService', () => {
  ...
  beforeAll(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'dobius-gemini-home-'))
    userDataDir = mkdtempSync(join(tmpdir(), 'dobius-gemini-userdata-'))
  ...
  it('removes stale PreToolUse hooks when reinstalling managed Gemini hooks', () => {
    ...
    const staleManagedHookPath =
      process.platform === 'win32'
        ? `C:\\Users\\ramzi\\.dobius\\agent-hooks\\${managedHookFileName}`
        : `/Users/ramzi/.dobius/agent-hooks/${managedHookFileName}`
    const staleManagedCommand =
      process.platform === 'win32'
        ? staleManagedHookPath
        : `if [ -x '${staleManagedHookPath}' ]; then /bin/sh '${staleManagedHookPath}'; fi`
    const managedHookPath = join(homeDir, '.dobius', 'agent-hooks', managedHookFileName)
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'dobius gemini home with spaces')
