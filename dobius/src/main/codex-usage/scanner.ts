...
import { getDobiusManagedCodexHomePath, getSystemCodexHomePath } from '../codex/codex-home-paths'
...
export function getCodexSessionsDirectory(): string {
  // Why: Dobius-launched Codex processes receive an Dobius-owned CODEX_HOME, so
  // callers that need the primary runtime path should not consult ambient
  // shell CODEX_HOME.
  return join(getDobiusManagedCodexHomePath(), 'sessions')
}

export function getCodexSessionDirectories(): string[] {
  // Why: upgraded users still have ordinary Codex history under ~/.codex, while
  // new Dobius-launched sessions are written under Dobius's managed runtime home.
...
function hasLegacyCopiedSessionBridgeMarkers(): boolean {
  return existsSync(join(getDobiusManagedCodexHomePath(), '.dobius-session-copies'))
}
...
export async function attributeCodexUsageEvent(
  ...
  if (event.cwd) {
    ...
    if (worktree) {
      ...
      // Why: all-local mode should still collapse repeated off-Dobius sessions by
