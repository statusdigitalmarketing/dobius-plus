...
const { getPathMock } = vi.hoisted(() => ({
  getPathMock: vi.fn(() => '/tmp/dobius-test-userdata')
}))
...
describe('CodexUsageStore', () => {
  ...
  beforeEach(() => {
    tempUserData = mkdtempSync(join(tmpdir(), 'dobius-codex-usage-store-'))
  ...
  it('persists a successful refresh with one compact disk write', async () => {
    ...
    const persistedJson = readFileSync(join(tempUserData, 'dobius-codex-usage.json'), 'utf-8')
  ...
  it('reports no data for Dobius scope when only non-Dobius Codex usage exists', async () => {
    const store = createStoreWithState({
      sessions: [
        {
          sessionId: 'session-1',
    ...
    const summary = await store.getSummary('dobius', '30d')
  ...
  it('prices current Codex models with current model rates', async () => {
    ...
    const summary = await store.getSummary('dobius', '30d')
    const breakdown = await store.getBreakdown('dobius', '30d', 'model')
  ...
  it('uses only Dobius-scoped models when projecting mixed-scope sessions', async () => {
    const store = createStoreWithState({
      sessions: [
        {
          sessionId: 'session-1',
    ...
    const breakdown = await store.getBreakdown('dobius', '30d', 'model')
    const recentSessions = await store.getRecentSessions('dobius', '30d', 10)
