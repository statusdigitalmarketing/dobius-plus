...
const SEARCH_ISSUES_QUERY = `
  query DobiusLinearIssueSearch($term: String!, $first: Int) {
...
const VIEWER_ASSIGNED_ISSUES_QUERY = `
  query DobiusLinearViewerAssignedIssues(
    $first: Int,
    $after: String,
    $filter: IssueFilter,
    $orderBy: PaginationOrderBy
...
const VIEWER_CREATED_ISSUES_QUERY = `
  query DobiusLinearViewerCreatedIssues(
    $first: Int,
    $after: String,
    $filter: IssueFilter,
    $orderBy: PaginationOrderBy
...
const ISSUE_BY_UUID_QUERY = `
  query DobiusLinearIssueByUuid($id: String!) {
    issue(id: $id) {
...
const COMMENT_BY_UUID_QUERY = `
  query DobiusLinearCommentByUuid($id: String!) {
    comment(id: $id) {
      id
      url
      body
...
const ATTACHMENT_BY_UUID_QUERY = `
  query DobiusLinearAttachmentByUuid($id: String!) {
    attachment(id: $id) {
      id
      title
      url
...
async function readIssueConnectionPages(
  ...
  while (items.length < limit) {
    // Why: Linear caps connection pages at 50, so larger Dobius reads must walk
