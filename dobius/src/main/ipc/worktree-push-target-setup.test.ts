...
const FORK_SSH = 'git@github.com:contributor/dobius.git'
const FORK_HTTPS = 'https://github.com/contributor/dobius.git'
...
describe('prepareWorktreePushTargetWithExec', () => {
  it('adds a new fork remote and fetches its head when none matches', async () => {
    const exec = makeRepoExec({ origin: 'git@github.com:statusdigitalmarketing/dobius-plus.git' })
    ...
    expect(callsMatching(exec, ['remote', 'add'])).toEqual([
      ['remote', 'add', 'pr-contributor-dobius', FORK_SSH]
    ])
    expect(callsMatching(exec, ['fetch'])).toEqual([
      [
        'fetch',
        'pr-contributor-dobius',
        '+refs/heads/contributor/fix:refs/remotes/pr-contributor-dobius/contributor/fix'
      ]
    ])
  ...
  it('reuses an existing remote pointing at the same fork (SSH vs HTTPS) without adding', async () => {
    const exec = makeRepoExec({
      origin: 'git@github.com:statusdigitalmarketing/dobius-plus.git',
      'pr-contributor-dobius': FORK_HTTPS
    })
  ...
  it('disambiguates with a numeric suffix when the preferred remote name is taken by a different URL', async () => {
    const exec = makeRepoExec({ 'pr-contributor-dobius': 'git@github.com:someone-else/dobius.git' })

    const result = await prepareWorktreePushTargetWithExec(exec, REPO, forkTarget(), () => false)

    expect(callsMatching(exec, ['remote', 'add'])).toEqual([
      ['remote', 'add', 'pr-contributor-dobius-2', FORK_SSH]
    ])
    expect(result.remoteName).toBe('pr-contributor-dobius-2')
    expect(result.remoteCreated).toBe(true)
  })
...
describe('configureCreatedWorktreePushTargetWithExec', () => {
  it('points the new branch upstream at the fork remote', async () => {
    ...
    expect(exec).toHaveBeenCalledWith(
      ['branch', '--set-upstream-to', 'pr-contributor-dobius/contributor/fix', 'local-branch'],
      '/wt/path'
    )
