...
describe('registerNotificationHandlers', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = mkdtempSync(join(tmpdir(), 'dobius-notification-test-'))
  ...
  it('opens the current macOS app notification settings entry', () => {
    ...
    const originalBundleId = process.env.DOBIUS_DEV_MACOS_BUNDLE_ID
    Object.defineProperty(process, 'platform', { value: 'darwin', configurable: true })
    process.env.DOBIUS_DEV_MACOS_BUNDLE_ID = 'com.statusdigitalmarketing.dobius-plus.dev.fb5a47066f08'
    try {
      registerNotificationHandlers({
        getSettings: () => ({
          notifications: {
      ...
      expect(shellOpenExternalMock).toHaveBeenCalledWith(
        'x-apple.systempreferences:com.apple.Notifications-Settings.extension?id=com.statusdigitalmarketing.dobius-plus.dev.fb5a47066f08'
      )
    } finally {
      Object.defineProperty(process, 'platform', { value: originalPlatform, configurable: true })
      if (originalBundleId === undefined) {
        delete process.env.DOBIUS_DEV_MACOS_BUNDLE_ID
      } else {
        process.env.DOBIUS_DEV_MACOS_BUNDLE_ID = originalBundleId
      }
    }
  })
  ...
  it('suppresses active-worktree notifications while Dobius is focused', () => {
    getAllWindowsMock.mockReturnValue([
      {
        isDestroyed: () => false,
        isFocused: () => true
  ...
  it('delivers a notification when the event is allowed', () => {
    ...
    const handler = getDispatchHandler()
    expect(
      handler({}, { source: 'agent-task-complete', repoLabel: 'dobius', worktreeLabel: 'feat/notis' })
  ...
  it('uses the macOS default notification sound when no custom sound is configured', () => {
    ...
    try {
      ...
      expect(notificationCtorMock).toHaveBeenCalledWith({
        title: 'Dobius notifications are on',
        body: 'This is a test notification from Dobius.',
        sound: 'default'
      })
    } finally {
      Object.defineProperty(process, 'platform', { value: originalPlatform, configurable: true })
  ...
  it('includes the repo name when multiple repos are active', () => {
    ...
    expect(notificationCtorMock).toHaveBeenCalledWith(
      expectedNativeNotificationOptions({
        title: 'dobius / feat/notis - Codex finished',
        body: 'Updated the notification body.'
      })
    )
  })
...
describe('triggerStartupNotificationRegistration', () => {
  ...
  it('shows welcome notification when not yet requested', () => {
    ...
    expect(notificationCtorMock).toHaveBeenCalledWith({
      title: 'Dobius is ready to notify you',
      body: 'Allow notifications so Dobius can alert you when agents finish or terminals need attention.'
    })
    expect(notificationShowMock).toHaveBeenCalledTimes(1)
  })
