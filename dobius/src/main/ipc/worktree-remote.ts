...
import type { RemoteFetchResult, RemoteTrackingBase } from '../runtime/dobius-runtime'
...
async function spawnLocalStartupAndSetupTerminals(args: {
  runtime: DobiusRuntimeService | undefined
...
async function getOrStartSshWorktreeCreateFetch(
  ...
  const promise = enqueueSshWorktreeCreateFetch(queueKey, async () => {
    ...
    await fetch()
    // Why: SSH creation has no DobiusRuntimeService instance to share, but
...
async function prepareWorktreePushTargetSsh(
  ...
  if (target.remoteUrl) {
    ...
    if (existingRemote) {
      ...
      // Why: if a later PR worktree reuses an Dobius-created fork remote, it
...
async function readRemoteEffectiveHooks(
  ...
  return getEffectiveHooksFromConfig(repo, await readRemoteDobiusYaml(fsProvider, hooksRootPath))
}

async function readRemoteDobiusYaml(
  fsProvider: IFilesystemProvider,
  hooksRootPath: string
): Promise<ReturnType<typeof parseDobiusYaml>> {
  try {
    const result = await fsProvider.readFile(joinWorktreeRelativePath(hooksRootPath, 'dobius.yaml'))
    return result.isBinary ? null : parseDobiusYaml(result.content)
  } catch {
    return null
  }
}
...
async function createRemoteSetupRunnerScript(
  ...
  const runnerRelativePath = useWindowsFormat ? 'dobius/setup-runner.cmd' : 'dobius/setup-runner.sh'
...
export async function createRemoteWorktree(
  ...
  if (fsProvider) {
    await timing.time('prepare_setup', async () => {
      const yamlHooks = await readRemoteDobiusYaml(fsProvider, created.path)
...
export async function createLocalWorktree(
  ...
  // Why: the worktree's own `dobius.yaml` (at the tip of the base branch) is
