...
import type { GlobalSettings, DobiusWorkspaceLayout, Repo } from '../../shared/types'
 ...
 * Windows workspace layout inside ~/dobius/workspaces on the WSL filesystem
 * (e.g. \\wsl.localhost\Ubuntu\home\user\dobius\workspaces\repo\feature).
 */
...
export function computeWorkspaceRoot(repoPath: string, settings: { workspaceDir: string }): string {
  ...
  if (wsl && shouldMirrorWorkspaceDirInsideWsl(repoPath, settings.workspaceDir)) {
    ...
    if (wslHome) {
      ...
      return win32.join(wslHome, 'dobius', 'workspaces')
    }
  }
...
export function areWorktreePathsEqual(
  ...
  if (platform === 'win32' || looksLikeWindowsPath(leftPath) || looksLikeWindowsPath(rightPath)) {
    ...
    // creation. Dobius must treat those as the same worktree or a successful
