...
vi.mock('../hooks', () => ({
  ...
  parseDobiusYaml: parseDobiusYamlMock,
  runHook: runHookMock,
...
describe('registerWorktreeHandlers', () => {
  ...
  beforeEach(() => {
    ...
    parseDobiusYamlMock.mockReturnValue(null)
    ...
    getSetupRunnerEnvVarsMock.mockImplementation(
      (repoArg: { path: string }, worktreePath: string) => ({
        DOBIUS_ROOT_PATH: repoArg.path,
        DOBIUS_WORKTREE_PATH: worktreePath,
        DOBIUS_WORKSPACE_NAME: worktreePath.split('/').at(-1) ?? '',
        CONDUCTOR_ROOT_PATH: repoArg.path,
        GHOSTX_ROOT_PATH: repoArg.path
      })
    )
    createSetupRunnerScriptMock.mockReturnValue({
      runnerScriptPath: '/workspace/repo/.git/dobius/setup-runner.sh',
      envVars: {
        DOBIUS_ROOT_PATH: '/workspace/repo',
        DOBIUS_WORKTREE_PATH: '/workspace/improve-dashboard'
      }
    })
    createIssueCommandRunnerScriptMock.mockReturnValue({
      runnerScriptPath: '/workspace/repo/.git/dobius/issue-command-runner.sh',
      envVars: {
        DOBIUS_ROOT_PATH: '/workspace/repo',
        DOBIUS_WORKTREE_PATH: '/workspace/improve-dashboard'
      }
  ...
  it('strips Dobius provenance fields from renderer metadata updates', () => {
  ...
  it('uses a repo-specific worktree base path when creating local worktrees', async () => {
    ...
    expect(store.setWorktreeMeta).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        dobiusCreationWorkspaceLayout: { path: '../worktrees', nestWorkspaces: false }
      })
    )
  })
  ...
  it('spawns a startup terminal and setup terminal after local worktree registration', async () => {
    ...
    expect(runtimeStub.createTerminal).toHaveBeenNthCalledWith(
      ...
      {
        title: 'Setup',
        command: expect.stringContaining('bash /workspace/repo/.git/dobius/setup-runner.sh'),
        env: {
          DOBIUS_ROOT_PATH: '/workspace/repo',
          DOBIUS_WORKTREE_PATH: '/workspace/improve-dashboard'
        },
    ...
    expect(setupCommand).toBe('bash /workspace/repo/.git/dobius/setup-runner.sh')
  ...
  it('configures a PR push target during local create', async () => {
    ...
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(
      ['remote', 'add', 'pr-prateek-dobius', 'git@github.com:prateek/dobius.git'],
      { cwd: '/workspace/repo' }
    )
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(
      [
        'fetch',
        'pr-prateek-dobius',
        '+refs/heads/prateek/fix-sidebar-agents-toggle:refs/remotes/pr-prateek-dobius/prateek/fix-sidebar-agents-toggle'
      ],
      { cwd: '/workspace/repo' }
    )
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(
      [
        'branch',
        '--set-upstream-to',
        'pr-prateek-dobius/prateek/fix-sidebar-agents-toggle',
        'improve-dashboard'
      ],
  ...
  it('keeps the Dobius-created marker when a new worktree reuses an Dobius-created fork remote', async () => {
    listWorktreesMock.mockResolvedValue([
      {
        path: '/workspace/improve-dashboard',
        head: 'abc123',
    ...
    const existingPushTarget = {
      ...
      remoteUrl: 'https://github.com/contributor/dobius.git',
      remoteCreated: true
    }
    store.getAllWorktreeMeta.mockReturnValue({
    ...
    gitExecFileAsyncMock.mockImplementation(async (args: string[]) => {
      if (args[0] === 'remote' && args.length === 1) {
        return { stdout: 'pr-contributor-dobius\n', stderr: '' }
      }
      if (args[0] === 'remote' && args[1] === 'get-url') {
        return { stdout: 'https://github.com/contributor/dobius.git\n', stderr: '' }
      }
      return { stdout: '', stderr: '' }
    })
  ...
  it('routes fork push target setup through the selected WSL project runtime', async () => {
    ...
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(
      ['remote', 'add', 'pr-contributor-dobius', 'git@github.com:contributor/dobius.git'],
      { cwd: '/workspace/repo', wslDistro: 'Ubuntu' }
    )
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(
      [
        ...
        '+refs/heads/contributor/wsl-fork:refs/remotes/pr-contributor-dobius/contributor/wsl-fork'
      ],
      { cwd: '/workspace/repo', wslDistro: 'Ubuntu' }
    )
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(
      ['branch', '--set-upstream-to', 'pr-contributor-dobius/contributor/wsl-fork', 'wsl-fork'],
  ...
  it('reads remote dobius.yaml and returns a setup launch payload during SSH create', async () => {
    const repo = {
      id: 'repo-ssh',
      path: '/remote/repo',
      displayName: 'ssh',
    ...
    const provider = {
      exec: vi.fn().mockImplementation(async (args: string[]) => {
        ...
        if (args[0] === 'rev-parse' && args[1] === '--git-path') {
          return {
            stdout: '/remote/repo/.git/worktrees/improve-dashboard/dobius/setup-runner.sh\n',
            stderr: ''
          }
        }
        if (args[0] === 'rev-parse') {
    ...
    getActiveMultiplexerMock.mockReturnValue(mux)
    store.setWorktreeMeta.mockImplementation((_worktreeId, meta) => meta)
    parseDobiusYamlMock.mockReturnValue({ scripts: { setup: 'pnpm install' } })
    getEffectiveHooksFromConfigMock.mockReturnValue({ scripts: { setup: 'pnpm install' } })
    shouldRunSetupForCreateMock.mockReturnValue(true)
    ...
    expect(fsProvider.readFile).toHaveBeenCalledWith('/remote/improve-dashboard/dobius.yaml')
    expect(provider.exec).toHaveBeenCalledWith(
      ['rev-parse', '--git-path', 'dobius/setup-runner.sh'],
      '/remote/improve-dashboard'
    )
    expect(fsProvider.createDir).toHaveBeenCalledWith(
      '/remote/repo/.git/worktrees/improve-dashboard/dobius'
    )
    expect(fsProvider.writeFile).toHaveBeenCalledWith(
      '/remote/repo/.git/worktrees/improve-dashboard/dobius/setup-runner.sh',
      '#!/usr/bin/env bash\nset -e\npnpm install\n'
    )
    expect(result).toEqual(
      expect.objectContaining({
        setup: {
          runnerScriptPath: '/remote/repo/.git/worktrees/improve-dashboard/dobius/setup-runner.sh',
          envVars: expect.objectContaining({
            DOBIUS_ROOT_PATH: '/remote/repo',
            DOBIUS_WORKTREE_PATH: '/remote/improve-dashboard'
          })
        }
      })
    )
  ...
  it('repairs legacy project ids when SSH worktree listing falls back to persisted metadata', async () => {
    ...
    store.getRepo.mockReturnValue(repo)
    store.getAllWorktreeMeta.mockReturnValue({
      'repo-ssh::/remote/dobius': makeWorktreeMeta({
        instanceId: 'existing-instance',
        projectId: 'repo:repo-ssh',
        hostId: 'ssh:ssh-target-1',
        projectHostSetupId: 'repo-ssh',
    ...
    expect(store.setWorktreeMeta).toHaveBeenCalledWith('repo-ssh::/remote/dobius', {
      projectId: 'github:statusdigitalmarketing/dobius-plus'
    })
    expect(listed).toEqual([
      expect.objectContaining({
        id: 'repo-ssh::/remote/dobius',
        projectId: 'github:statusdigitalmarketing/dobius-plus',
        hostId: 'ssh:ssh-target-1',
        projectHostSetupId: 'repo-ssh',
        lastActivityAt: 42
  ...
  it('launches setup even when primary and worktree dobius.yaml scripts diverge', async () => {
  ...
  it('recovers forced Windows long-path worktree removal through local deletion and prune', async () => {
    setPlatform('win32')
    const parentDir = await mkdtemp(join(tmpdir(), 'dobius-ipc-long-path-'))
  ...
  it('runs the archive hook before removing an SSH worktree', async () => {
    ...
    expect(provider.execNonInteractive).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        DOBIUS_ROOT_PATH: '/remote/repo',
        DOBIUS_WORKTREE_PATH: '/remote/feature-wt'
      })
    )
  ...
  it('uses cmd.exe for archive hooks on Windows-like SSH worktree paths', async () => {
    ...
    expect(provider.execNonInteractive).toHaveBeenCalledWith(
      ...
      undefined,
      expect.objectContaining({
        DOBIUS_ROOT_PATH: 'C:\\remote\\repo',
        DOBIUS_WORKTREE_PATH: 'C:\\remote\\feature-wt'
      })
    )
  })
  ...
  it('removes an unused Dobius-created fork remote after deleting its worktree', async () => {
    mockKnownFeatureWorktree()
    removeWorktreeMock.mockResolvedValue(undefined)
    const pushTarget = {
      remoteName: 'pr-contributor-dobius',
    ...
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(['remote', 'remove', 'pr-contributor-dobius'], {
      cwd: '/workspace/repo'
    })
  })

  it('keeps an Dobius-created fork remote while another worktree still uses it', async () => {
    mockKnownFeatureWorktree()
    removeWorktreeMock.mockResolvedValue(undefined)
    const pushTarget = {
      remoteName: 'pr-contributor-dobius',
    ...
    expect(gitExecFileAsyncMock).not.toHaveBeenCalledWith(
      ['remote', 'remove', 'pr-contributor-dobius'],
      expect.any(Object)
    )
  })
  ...
  it('force-removes a legacy Dobius-created orphaned worktree directory after Git tracking is gone', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'dobius-ipc-orphan-'))
    const repoPath = join(parentDir, 'repo')
  ...
  it('prompts for force before removing an Dobius-created orphaned worktree directory', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'dobius-ipc-orphan-'))
    const repoPath = join(parentDir, 'repo')
    const orphanPath = join(parentDir, 'orphan')
    const adminWorktreePath = join(repoPath, '.git', 'worktrees', 'orphan')
    ...
    mockKnownFeatureWorktree(join(parentDir, 'real-feature'), repoPath)
    store.getWorktreeMeta.mockReturnValue(
      makeWorktreeMeta({ dobiusCreatedAt: Date.now(), dobiusCreationSource: 'runtime' })
    )
  ...
  it('prompts then force-removes an Dobius-created unregistered leftover directory with no git marker', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'dobius-ipc-leftover-'))
    const repoPath = join(parentDir, 'repo')
  ...
  it('rejects an Dobius-created unregistered local directory with a git directory', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'dobius-ipc-standalone-'))
    const repoPath = join(parentDir, 'repo')
  ...
  it('does not inspect or delete a local path when SSH orphan cleanup has no filesystem provider', async () => {
    const localPath = await mkdtemp(join(tmpdir(), 'dobius-ipc-ssh-missing-fs-'))
    const repo = {
    ...
    store.getRepo.mockReturnValue(repo)
    store.getWorktreeMeta.mockReturnValue(
      makeWorktreeMeta({ dobiusCreatedAt: Date.now(), dobiusCreationSource: 'ssh' })
    )
    getSshGitProviderMock.mockReturnValue(provider)
  ...
  it('creates remote .gitignore only when it is missing while writing SSH issue commands', async () => {
    ...
    expect(fsProvider.writeFile).toHaveBeenNthCalledWith(1, '/remote/repo/.gitignore', '.dobius\n')
    expect(fsProvider.writeFile).toHaveBeenNthCalledWith(
      2,
      '/remote/repo/.dobius/issue-command',
      'dobius issue command\n'
    )
  })
