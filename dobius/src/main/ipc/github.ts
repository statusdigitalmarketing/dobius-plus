...
export function registerGitHubHandlers(store: Store, stats: StatsCollector): void {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      // This captures PRs opened from any workflow (Dobius UI, gh CLI, github.com).
  ...
  // Star operations target the Dobius repo itself — no repoPath validation needed
  ipcMain.handle('gh:viewer', () => getAuthenticatedViewer())
  ipcMain.handle('gh:checkDobiusStarred', () => checkDobiusStarred())
  ipcMain.handle('gh:starDobius', async (_event, source: unknown) => {
    const sourceParse = appStarSourceSchema.safeParse(source)
    const starred = await starDobius()
