...
describe('diagnostics IPC handlers', () => {
  beforeEach(() => {
    ...
    delete (globalThis as { DOBIUS_BUILD_IDENTITY?: unknown }).DOBIUS_BUILD_IDENTITY
    delete (globalThis as { DOBIUS_DIAGNOSTICS_TOKEN_URL?: unknown }).DOBIUS_DIAGNOSTICS_TOKEN_URL
    process.env.DOBIUS_DIAGNOSTICS_TOKEN_URL = 'https://diagnostics.example.com/diagnostics/token'
  ...
  it('pins official builds to the compile-time diagnostics endpoint', async () => {
    ...
    const globalOverrides = globalThis as {
      DOBIUS_BUILD_IDENTITY?: 'stable'
      DOBIUS_DIAGNOSTICS_TOKEN_URL?: string
    }
    globalOverrides.DOBIUS_BUILD_IDENTITY = 'stable'
    globalOverrides.DOBIUS_DIAGNOSTICS_TOKEN_URL = 'https://official.example.com/diagnostics/token'
    process.env.DOBIUS_DIAGNOSTICS_TOKEN_URL = 'https://attacker.example.com/diagnostics/token'
    collectDiagnosticBundleMock.mockReturnValue(bundle)
    readFileSyncMock.mockReturnValue(bundle.payload)
    const collect = handlers.get('diagnostics:collectBundle')!
    const openPreview = handlers.get('diagnostics:openBundlePreview')!
