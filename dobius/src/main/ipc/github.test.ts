...
describe('registerGitHubHandlers', () => {
  ...
  it('emits app_starred_dobius once after a successful star with cohort context', async () => {
    starDobiusMock.mockResolvedValue(true)
    getCohortAtEmitMock.mockReturnValue({ nth_repo_added: 3 })

    registerGitHubHandlers(store as never, stats as never)

    await expect(handlers['gh:starDobius'](null, 'settings')).resolves.toBe(true)

    expect(starDobiusMock).toHaveBeenCalledTimes(1)
  ...
  it('accepts every app star source for success telemetry', async () => {
    ...
    for (const source of [
      ...
      await expect(handlers['gh:starDobius'](null, source)).resolves.toBe(true)
    }
  ...
  it('does not emit app_starred_dobius when the star action returns false', async () => {
    starDobiusMock.mockResolvedValue(false)

    registerGitHubHandlers(store as never, stats as never)

    await expect(handlers['gh:starDobius'](null, 'landing')).resolves.toBe(false)
  ...
  it('does not emit app_starred_dobius when the star action throws', async () => {
    starDobiusMock.mockRejectedValue(new Error('gh failed'))

    registerGitHubHandlers(store as never, stats as never)

    await expect(handlers['gh:starDobius'](null, 'star_nag')).rejects.toThrow('gh failed')
  ...
  it('preserves star result but skips telemetry for an invalid IPC source', async () => {
    ...
    await expect(handlers['gh:starDobius'](null, 'github_website')).resolves.toBe(true)
