...
const MACOS_PACKAGED_BUNDLE_ID = 'com.statusdigitalmarketing.dobius-plus'
...
function getMacNotificationSettingsUrl(): string {
  const bundleId = process.env.DOBIUS_DEV_MACOS_BUNDLE_ID ?? MACOS_PACKAGED_BUNDLE_ID
...
export function registerNotificationHandlers(store: Store, runtime?: DobiusRuntimeService): void {
  ...
  ipcMain.handle(
    ...
    (
      ...
      if (getEffectiveNotificationSoundId(settings) !== 'system') {
        ...
        // Why: macOS treats an unset notification sound as silent. When Dobius is
      ...
      // Why: clicking a notification should bring Dobius to the foreground and
...
export function triggerStartupNotificationRegistration(store: Store): void {
  ...
  // Notification Settings so they can verify/enable notifications for Dobius.
