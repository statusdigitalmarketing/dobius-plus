...
describe('createWorktreeSymlinks', () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'dobius-symlinks-'))
  ...
  it('does not overwrite a file target that appears before APFS clone-copy is published', async () => {
    ...
    expect(existsSync(join(worktree, '.dobius-apfs-clone-file-race'))).toBe(false)
    expect(warn).not.toHaveBeenCalled()
    expect(error).not.toHaveBeenCalled()
  })
...
describe('removeWorktreeSymlinks', () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'dobius-unlink-'))
    primary = join(root, 'primary')
    worktree = join(root, 'worktree')
    mkdirSync(primary, { recursive: true })
    mkdirSync(worktree, { recursive: true })
