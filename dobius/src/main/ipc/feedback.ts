...
const FEEDBACK_API_URL = 'https://www.ondobius.dev/v1/feedback'
const FEEDBACK_API_FALLBACK_URL = 'https://api.ondobius.dev/v1/feedback'
...
// which Dobius build and which OS the feedback came from. The main process is
...
function feedbackRequestBodyInit(body: FeedbackSubmitBody): Pick<RequestInit, 'body' | 'headers'> {
  ...
  formData.append(
    ...
    `dobius-diagnostics-${body.diagnosticBundle.bundleSubmissionId}.ndjson`
  )
...
export async function submitFeedback(
  ...
  try {
    ...
    // Why: keep api.ondobius.dev as a compatibility fallback, but prefer the
