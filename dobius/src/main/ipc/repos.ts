...
function getDefaultCreateProjectParent(): string {
  return join(homedir(), 'dobius', 'projects')
}
...
export function registerRepoHandlers(mainWindow: BrowserWindow, store: Store): void {
  ...
  // Creates a new repo or folder from scratch (dobius#763). An empty initial
  // commit is required for git repos so HEAD has a branch ref — Dobius's
  ...
  ipcMain.handle(
    ...
    async (
      ...
      let targetExists = false
      try {
        // Why: the name-first default points at ~/dobius/projects, which may not
  ...
  ipcMain.handle(
    ...
    async (_event, args: { url: string; destination: string }): Promise<Repo> => {
      ...
      // the repo folder name from the URL (e.g. "dobius" from .../dobius.git).
      ...
      return runWithClonePathLock(clonePathKey, async () => {
        ...
        // exist yet (e.g. ~/dobius). recursive: true is a no-op when present.
