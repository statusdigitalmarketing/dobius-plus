...
const {
  ...
  resolveDiagnosticDobiusChannelMock,
  spanEndMock,
  startSpanMock,
  submitFeedbackMock
} = vi.hoisted(() => {
  ...
  return {
    ...
    resolveDiagnosticDobiusChannelMock: vi.fn(),
    spanEndMock,
    startSpanMock: vi.fn(() => ({
      traceId: 'trace-id',
      spanId: 'span-id',
...
vi.mock('../observability/diagnostic-upload-endpoint', () => ({
  resolveDiagnosticDobiusChannel: resolveDiagnosticDobiusChannelMock
}))
...
describe('registerCrashReportingHandlers', () => {
  beforeEach(() => {
    ...
    resolveDiagnosticDobiusChannelMock.mockReset()
    resolveDiagnosticDobiusChannelMock.mockReturnValue('stable')
  ...
  it('submits an uncaptured Help menu crash report with an attached diagnostic bundle', async () => {
    ...
    expect(collectDiagnosticBundleMock).toHaveBeenCalledWith(
      expect.objectContaining({ lookbackMinutes: 3 * 24 * 60, dobiusChannel: 'stable' })
    )
    expect(submitFeedbackMock).toHaveBeenCalledWith({
