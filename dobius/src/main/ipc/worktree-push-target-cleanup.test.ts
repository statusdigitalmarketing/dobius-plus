...
const FORK_URL = 'git@github.com:contributor/dobius.git'
const FORK_REMOTE = 'pr-contributor-dobius'
...
describe('cleanupUnusedWorktreePushTargetRemoteWithExec', () => {
  it('removes an Dobius-created fork remote that nothing else uses', async () => {
    const exec = makeExec()
  ...
  it('keeps a remote Dobius did not create (remoteCreated falsy)', async () => {
    const exec = makeExec()
    await cleanupUnusedWorktreePushTargetRemoteWithExec(
      REPO_PATH,
      'repo-1::/wt/a',
  ...
  it('keeps the remote when its URL no longer matches the fork (repurposed by the user)', async () => {
    const exec = makeExec({ getUrl: 'git@github.com:someone-else/dobius.git' })
    await cleanupUnusedWorktreePushTargetRemoteWithExec(
      REPO_PATH,
      'repo-1::/wt/a',
      forkTarget(),
...
describe('sameGitHubRemoteUrl', () => {
  it('matches SSH and HTTPS forms of the same GitHub fork', () => {
    expect(
      sameGitHubRemoteUrl(
        'git@github.com:contributor/dobius.git',
        'https://github.com/contributor/dobius.git'
      )
    ).toBe(true)
  })
  ...
  it('does not match different forks', () => {
    expect(
      sameGitHubRemoteUrl(
        'git@github.com:contributor/dobius.git',
        'git@github.com:someone-else/dobius.git'
      )
    ).toBe(false)
  })
  ...
  it('falls back to exact equality for non-GitHub hosts', () => {
    expect(
      sameGitHubRemoteUrl(
        'git@gitlab.com:contributor/dobius.git',
        'git@gitlab.com:contributor/dobius.git'
      )
    ).toBe(true)
    expect(
      sameGitHubRemoteUrl(
        'git@gitlab.com:contributor/dobius.git',
        'https://gitlab.com/contributor/dobius.git'
      )
    ).toBe(false)
  })
})
