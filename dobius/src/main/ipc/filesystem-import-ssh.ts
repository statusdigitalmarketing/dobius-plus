...
export async function importExternalPathsSsh(
  ...
  try {
    if (options?.ensureDir) {
      // Why: terminal-drop staging needs `${worktree}/.dobius/drops` to exist
      ...
      // .dobius/.gitignore marker only when absent prevents clobbering user-
      // authored patterns. .dobius/ is reserved as Dobius-owned remote state;
...
async function ensureDropStagingDir(sftp: SFTPWrapper, destDir: string): Promise<void> {
  // destDir is a posix remote path, expected to be `${worktreePath}/.dobius/drops`.
  ...
  if (!(await sftpPathExists(sftp, gitignorePath))) {
    // Why: negate the marker so .dobius/.gitignore itself is trackable if we
