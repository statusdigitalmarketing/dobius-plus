...
import { installBundledSkill, isBundledSkillInstalled } from '../skills/local-skill-installer'
import type {
...
export function registerSkillsHandlers(store: Store): void {
  ipcMain.handle(
    ...
    async (_event, skillName: string): Promise<BundledSkillInstallResult> =>
      installBundledSkill(skillName)
  )

  ipcMain.handle(
    'skills:isInstalled',
    async (_event, skillName: string): Promise<boolean> => isBundledSkillInstalled(skillName)
  )
