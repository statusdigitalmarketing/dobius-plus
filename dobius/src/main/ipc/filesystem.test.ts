...
describe('registerFilesystemHandlers', () => {
  ...
  it('allows readDir when a registered worktree resolves to a macOS canonical alias', async () => {
    const aliasWorktreePath = path.resolve('/var/folders/dobius/worktrees/feature')
    const canonicalWorktreePath = path.resolve('/private/var/folders/dobius/worktrees/feature')
  ...
  it('prepares the Dobius-managed Codex home for the default system selection', async () => {
    const context = {
      branch: 'feature/ai',
      stagedSummary: 'M\tREADME.md',
      stagedPatch: '+hello'
    ...
    registerFilesystemHandlers(store as never, {
      prepareForCodexLaunch: () => '/dobius-managed/codex-home'
    })
    ...
    expect(generateCommitMessageFromContextMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        kind: 'local',
        cwd: WORKTREE_FEATURE_PATH,
        env: expect.objectContaining({ CODEX_HOME: '/dobius-managed/codex-home' })
      })
    )
  })
