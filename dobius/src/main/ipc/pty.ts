...
import { addDobiusWslInteropEnv } from '../pty/wsl-dobius-env'
...
import {
  isRegisteredRendererWebContents,
  rendererWindowContents
} from '../window/renderer-window-registry'
...
function stripRemotePaneEnvWhenHooksDisabled(
  ...
  if (
    !env ||
    (!('DOBIUS_PANE_KEY' in env) &&
      !('DOBIUS_TAB_ID' in env) &&
      !('DOBIUS_WORKTREE_ID' in env) &&
      !('DOBIUS_AGENT_LAUNCH_TOKEN' in env))
  ) {
    return env
  }
  const stripped = { ...env }
  delete stripped.DOBIUS_PANE_KEY
  delete stripped.DOBIUS_TAB_ID
  delete stripped.DOBIUS_WORKTREE_ID
  delete stripped.DOBIUS_AGENT_LAUNCH_TOKEN
  return stripped
}
...
function promoteAgentTeamsShimPath(
  ...
  if (!env?.DOBIUS_AGENT_TEAMS_TEAM_ID) {
    return
  }
  ...
  // Why: host env injection can prepend Dobius's attribution/dev shims. Claude
...
const CODEX_HOME_ENV_KEYS = ['CODEX_HOME', 'DOBIUS_CODEX_HOME'] as const
...
function resolvePiAgentSourceDir(
  ...
  const sourceKey = kind === 'omp' ? 'DOBIUS_OMP_SOURCE_AGENT_DIR' : 'DOBIUS_PI_SOURCE_AGENT_DIR'
  const overlayKey = kind === 'omp' ? 'DOBIUS_OMP_CODING_AGENT_DIR' : 'DOBIUS_PI_CODING_AGENT_DIR'
  const otherOverlayKey = kind === 'omp' ? 'DOBIUS_PI_CODING_AGENT_DIR' : 'DOBIUS_OMP_CODING_AGENT_DIR'
  ...
  // Why: if PI_CODING_AGENT_DIR is just a restored Dobius overlay from either
...
function clearPiAgentShadowEnv(baseEnv: Record<string, string>, kind: PiAgentKind): void {
  if (kind === 'omp') {
    delete baseEnv.DOBIUS_OMP_CODING_AGENT_DIR
    delete baseEnv.DOBIUS_OMP_SOURCE_AGENT_DIR
    delete baseEnv.DOBIUS_OMP_STATUS_EXTENSION
    return
  }
  delete baseEnv.DOBIUS_PI_CODING_AGENT_DIR
  delete baseEnv.DOBIUS_PI_SOURCE_AGENT_DIR
}
...
function exposePiManagedExtensionEnv(
  ...
  if (kind === 'omp') {
    delete baseEnv.DOBIUS_OMP_CODING_AGENT_DIR
    if (managedEnv.DOBIUS_OMP_SOURCE_AGENT_DIR) {
      baseEnv.DOBIUS_OMP_SOURCE_AGENT_DIR = managedEnv.DOBIUS_OMP_SOURCE_AGENT_DIR
    } else {
      delete baseEnv.DOBIUS_OMP_SOURCE_AGENT_DIR
    }
    if (managedEnv.DOBIUS_OMP_STATUS_EXTENSION) {
      baseEnv.DOBIUS_OMP_STATUS_EXTENSION = managedEnv.DOBIUS_OMP_STATUS_EXTENSION
    } else {
      delete baseEnv.DOBIUS_OMP_STATUS_EXTENSION
    }
    return
  ...
  delete baseEnv.DOBIUS_PI_CODING_AGENT_DIR
  if (managedEnv.DOBIUS_PI_SOURCE_AGENT_DIR) {
    baseEnv.DOBIUS_PI_SOURCE_AGENT_DIR = managedEnv.DOBIUS_PI_SOURCE_AGENT_DIR
  } else {
    delete baseEnv.DOBIUS_PI_SOURCE_AGENT_DIR
  }
}
...
// Why: when agent status is disabled, a nested Dobius terminal can still pass
// through prior OpenCode or legacy Pi/OMP overlay env. Restore the user's
// original source dir when Dobius recorded one, otherwise strip only values
// known to be ours.
...
function resolveMimocodeSourceHome(baseEnv: Record<string, string>): string | undefined {
  const sourceHome = baseEnv.DOBIUS_MIMOCODE_SOURCE_HOME ?? process.env.DOBIUS_MIMOCODE_SOURCE_HOME
  if (sourceHome) {
    return sourceHome
  }
  const configHome = baseEnv.MIMOCODE_HOME ?? process.env.MIMOCODE_HOME
  const dobiusHome = baseEnv.DOBIUS_MIMOCODE_HOME ?? process.env.DOBIUS_MIMOCODE_HOME
  if (configHome && dobiusHome && configHome === dobiusHome) {
    return undefined
  }
  return configHome
}
...
function resolveOpenCodeSourceConfigDir(baseEnv: Record<string, string>): string | undefined {
  const sourceDir =
    baseEnv.DOBIUS_OPENCODE_SOURCE_CONFIG_DIR ?? process.env.DOBIUS_OPENCODE_SOURCE_CONFIG_DIR
  if (sourceDir) {
    return sourceDir
  }

  const configDir = baseEnv.OPENCODE_CONFIG_DIR ?? process.env.OPENCODE_CONFIG_DIR
  const dobiusConfigDir = baseEnv.DOBIUS_OPENCODE_CONFIG_DIR ?? process.env.DOBIUS_OPENCODE_CONFIG_DIR
  // Why: nested Dobius terminals inherit OPENCODE_CONFIG_DIR from the parent
  // PTY. If there is no recorded source dir, that value is Dobius-owned, not a
  // user config. Treating it as user config makes child Dobiuss mirror Dobius's
  // hook dir and can create large OpenCode runtime trees per terminal.
  if (configDir && dobiusConfigDir && configDir === dobiusConfigDir) {
    return undefined
  }
 ...
 *
 * This is the single source of truth for the env shape an Dobius PTY needs
...
export function buildPtyHostEnv(
  ...
  if (opts.agentStatusHooksEnabled) {
    ...
    // value cannot coexist with an Dobius-only injection. Hand the user's value
    // (when present) to the hook service and let it materialize a source-scoped
    // mirror overlay that lets the user's plugins and Dobius's status plugin
    ...
    if (baseEnv.OPENCODE_CONFIG_DIR) {
      ...
      baseEnv.DOBIUS_OPENCODE_CONFIG_DIR = baseEnv.OPENCODE_CONFIG_DIR
      if (preexistingOpenCodeConfigDir) {
        // Why: terminals launched from another Dobius terminal inherit the overlay
        // as OPENCODE_CONFIG_DIR; keep the original source so overlays do not
        // mirror overlays and drop the user's real config.
        baseEnv.DOBIUS_OPENCODE_SOURCE_CONFIG_DIR = preexistingOpenCodeConfigDir
      } else {
        delete baseEnv.DOBIUS_OPENCODE_SOURCE_CONFIG_DIR
      }
    }
    if (isMimoLaunchCommand(launchCommandHint)) {
      ...
      if (baseEnv.MIMOCODE_HOME) {
        baseEnv.DOBIUS_MIMOCODE_HOME = baseEnv.MIMOCODE_HOME
        if (preexistingMimocodeHome) {
          baseEnv.DOBIUS_MIMOCODE_SOURCE_HOME = preexistingMimocodeHome
        } else {
          delete baseEnv.DOBIUS_MIMOCODE_SOURCE_HOME
        }
      }
    }
  } else {
    restoreOrStripOverlayEnv(baseEnv, {
      ...
      overlay: 'DOBIUS_OPENCODE_CONFIG_DIR',
      source: 'DOBIUS_OPENCODE_SOURCE_CONFIG_DIR'
    })
    restoreOrStripOverlayEnv(baseEnv, {
      primary: 'MIMOCODE_HOME',
      overlay: 'DOBIUS_MIMOCODE_HOME',
      source: 'DOBIUS_MIMOCODE_SOURCE_HOME'
    })
  }

  // Why: Claude/Codex native hooks run inside the shell process, so Dobius
  ...
  // to the correct Dobius pane.
  // Why: nested Dobius terminals can inherit another process's hook endpoint or
  ...
  // that home as the user's normal source of truth and install only Dobius-owned,
  ...
  if (opts.agentStatusHooksEnabled) {
    ...
    restoreOrStripOverlayEnv(baseEnv, {
      ...
      overlay: 'DOBIUS_PI_CODING_AGENT_DIR',
      source: 'DOBIUS_PI_SOURCE_AGENT_DIR'
    })
    restoreOrStripOverlayEnv(baseEnv, {
      primary: 'PI_CODING_AGENT_DIR',
      overlay: 'DOBIUS_OMP_CODING_AGENT_DIR',
      source: 'DOBIUS_OMP_SOURCE_AGENT_DIR'
    })
    delete baseEnv.DOBIUS_OMP_STATUS_EXTENSION
  }

  // Why: Codex account switching now materializes auth into an Dobius-scoped
  // runtime home, and Codex launched inside Dobius terminals must use that same
  // prepared home as quota fetches and other entry points. Keep the override
  // PTY-scoped so dev/prod Dobiuss do not share hooks through ~/.codex.
  if (opts.skipCodexHomeEnv) {
    delete baseEnv.CODEX_HOME
    delete baseEnv.DOBIUS_CODEX_HOME
    ...
    baseEnv.DOBIUS_CODEX_HOME = opts.selectedCodexHomePath
  }

  // Why: in dev mode the `dobius` CLI defaults to the production userData
  // path, which routes status updates to the packaged Dobius instead of this
  // dev instance. Injecting DOBIUS_USER_DATA_PATH ensures CLI calls from
  // agents running inside dev terminals reach the correct runtime. We also
  // prepend the dev CLI launcher directory to PATH so `dobius` resolves to
  // the dev build (which supports DOBIUS_USER_DATA_PATH) instead of the
  // production binary at /usr/local/bin/dobius.
  if (!opts.isPackaged) {
    baseEnv.DOBIUS_USER_DATA_PATH ??= opts.userDataPath
  ...
  // Dobius's own PTYs. Injecting lightweight PATH shims at spawn-time keeps
  // the behavior local to Dobius instead of rewriting user git config or
  // touching external shells.
  if (!opts.githubAttributionEnabled) {
    delete baseEnv.DOBIUS_ENABLE_GIT_ATTRIBUTION
    delete baseEnv.DOBIUS_GIT_COMMIT_TRAILER
    delete baseEnv.DOBIUS_GH_PR_FOOTER
    delete baseEnv.DOBIUS_GH_ISSUE_FOOTER
    delete baseEnv.DOBIUS_ATTRIBUTION_SHIM_DIR
  }
...
export function hasLivePty(ptyId: string): boolean {
  return ptyOwnership.has(ptyId) || ptySizes.has(ptyId)
}

let killLivePty: ((ptyId: string) => Promise<boolean>) | null = null

export async function killPty(ptyId: string): Promise<boolean> {
  return (await killLivePty?.(ptyId)) ?? false
}
...
export function registerPtyHandlers(
  ...
  if (localProvider instanceof LocalPtyProvider) {
    localProvider.configure({
      ...
      buildSpawnEnv: (id, baseEnv, ctx) => {
        ...
        const requestedHandle = baseEnv.DOBIUS_TERMINAL_HANDLE
        ...
        if (requestedHandle && requestedHandle !== preAllocatedHandle) {
          delete env.DOBIUS_TERMINAL_HANDLE
        }
        if (preAllocatedHandle) {
          env.DOBIUS_TERMINAL_HANDLE = preAllocatedHandle
        }
        if (ctx?.isWsl === true) {
  ...
  function sendPtyDataToRenderer(id: string, payload: PtyDataPayload): void {
    ...
    recordPtyRendererDeliveryPressure()
    // Why: broadcast to all app windows (1-3 in practice); each renderer
    // already filters pty:data by id. If window count ever grows large, switch
    // to a per-ptyId -> target routing map.
    for (const contents of rendererWindowContents()) {
      contents.send('pty:data', payload)
    }
  }
  ...
  function sendPtyExitToRenderer(payload: { id: string; code: number }): void {
    const contents = rendererWindowContents()
  ...
  runtime?.setPtyController({
    spawn: async (args) => {
      ...
      let env: Record<string, string> | undefined = claudeAuth
        ...
        : sshScopedEnv
      const requestedAgentTeamsPath = env?.DOBIUS_AGENT_TEAMS_TEAM_ID ? env.PATH : undefined
      if (args.preAllocatedHandle) {
        env = { ...env, DOBIUS_TERMINAL_HANDLE: args.preAllocatedHandle }
      }
      ...
      try {
        ...
        const paneKey = rememberPaneKeyForPty(result.id, env?.DOBIUS_PANE_KEY)
  ...
  ipcMain.handle(
    ...
    async (
      ...
      args: {
        ...
        // threads what Dobius was *asked* to launch through this field; main
      ...
      // could emit statuses this Dobius build is not prepared to route.
      ...
      const baseEnvWithAuth = claudeAuth
        ...
        : sshSourceEnv
      const spawnPaneKey = baseEnvWithAuth?.DOBIUS_PANE_KEY
      ...
      const requestedAgentTeamsPath = baseEnv?.DOBIUS_AGENT_TEAMS_TEAM_ID ? baseEnv.PATH : undefined
      const agentTeamsEnvToDelete = shouldRefreshAgentTeamsEnv
        ? ['TERM_PROGRAM', 'DOBIUS_ATTRIBUTION_SHIM_DIR']
        : undefined
      if (baseEnv && stablePaneKey) {
        baseEnv.DOBIUS_PANE_KEY = stablePaneKey
        if (typeof args.tabId === 'string') {
          baseEnv.DOBIUS_TAB_ID = args.tabId
        } else if (!args.connectionId) {
          delete baseEnv.DOBIUS_TAB_ID
        }
        if (typeof args.worktreeId === 'string') {
          baseEnv.DOBIUS_WORKTREE_ID = args.worktreeId
        } else if (!args.connectionId) {
          delete baseEnv.DOBIUS_WORKTREE_ID
        }
      } else if (baseEnv) {
        // Why: DOBIUS_PANE_KEY crosses into shells and hook registries. Only the
        // key proven to match this spawn's tab+leaf may leave the IPC boundary.
        delete baseEnv.DOBIUS_PANE_KEY
        delete baseEnv.DOBIUS_TAB_ID
        delete baseEnv.DOBIUS_WORKTREE_ID
        delete baseEnv.DOBIUS_AGENT_LAUNCH_TOKEN
      }
      const validatedPaneKey = stablePaneKey
      // Why: SSH can strip DOBIUS_PANE_KEY when remote hooks are disabled; the
      ...
      try {
        ...
        const relayResultId = getRelayPtyId(args.connectionId, result.id)
        if (store && args.connectionId) {
          // Why: remote PTYs live in the SSH relay grace window after Dobius
        ...
        // Why: renderer sets DOBIUS_PANE_KEY in `args.env` for every pane-owned
  ...
  // Why: PTY input is accepted from any first-party app window (main window +
  // torn-off terminal windows), all tracked in the renderer registry. Webview
  // guests / offscreen windows are never registered, so keystroke injection
  // from untrusted frames is still rejected.
  const isPtyWriteEventFromAppWindow = (event: IpcMainEvent | IpcMainInvokeEvent): boolean =>
    typeof event.sender.isDestroyed === 'function' &&
    !event.sender.isDestroyed() &&
    isRegisteredRendererWebContents(event.sender)
  ...
  ipcMain.on('pty:write', (event, args: unknown) => {
    if (!isPtyWriteEventFromAppWindow(event) || !isPtyWritePayload(args)) {
      return
    }
    writePtyInput(args)
  })
  ...
  const shutdownPtyById = async (
    ...
    const shutdownProvider = provider ?? getProviderForPty(ptyId)
    let providerExitObserved = false
    try {
      providerExitObserved = await shutdownProviderAndDetectExit(shutdownProvider, ptyId, {
        immediate: true,
        keepHistory: opts?.keepHistory ?? false
      })
    } catch (err) {
  ...
  killLivePty = shutdownPtyById

  ipcMain.handle('pty:kill', async (_event, args: { id: string; keepHistory?: boolean }) => {
    await shutdownPtyById(args.id, { keepHistory: args.keepHistory })
  })
...
export function registerHeadlessPtyRuntime(
  ...
  // Why: headless `dobius serve` has no renderer window, but the runtime still
