...
// Why: worktrees discovered on disk (not created via Dobius's UI) have no
...
async function getArchiveHooksForRemoval(repo: Repo): Promise<DobiusHooks | null> {
  if (!repo.connectionId) {
  ...
  try {
    ...
    const yamlHooks = result.isBinary ? null : parseDobiusYaml(result.content)
...
function buildDetectedGitWorktrees(
  ...
  const knownDobiusLayouts = buildKnownDobiusWorkspaceLayouts(settings, repo)
...
function buildDisconnectedDetectedWorktrees(
  ...
  return worktrees.map((worktree) => {
    ...
    return {
      ...detected,
      visible: true,
      ownership: detected.ownership === 'dobius-managed' ? 'dobius-managed' : 'unknown-legacy'
    }
  })
}
...
export function registerWorktreeHandlers(
      ...
      const removal = (async (): Promise<RemoveWorktreeResult> => {
        ...
        if (!registeredWorktree) {
          ...
          if (await isAlreadyRemovedWorktreePath(repo, worktreePath, localWorktreeGitOptions)) {
            ...
            // an Dobius-known row, so no force confirmation is needed.
            if (repo.connectionId) {
              await cleanupUnusedWorktreePushTargetRemoteSsh(
                provider!,
                repo.path,
    ...
    (_event, args: { worktreeId: string; updates: Partial<WorktreeMeta> }) => {
      const updates =
        args.updates.displayName !== undefined
          ...
          : args.updates
      const meta = store.setWorktreeMeta(args.worktreeId, stripDobiusProvenanceMetaUpdates(updates))
  ...
  ipcMain.handle('hooks:check', async (_event, args: { repoId: string }) => {
    ...
    if (repo.connectionId) {
      ...
      try {
        ...
        return {
          ...
          hooks: result.isBinary ? null : parseDobiusYaml(result.content),
          mayNeedUpdate: false
        }
      } catch (error) {
        return {
    ...
    // Why: when a newer Dobius version adds a top-level key to `dobius.yaml`, older
    ...
    const mayNeedUpdate = has && !hooks && hasUnrecognizedDobiusYamlKeys(repo.path)
    return {
      status: 'ok',
      hasHooks: has,
      hooks,
    ...
    if (repo.connectionId) {
      ...
      try {
        ...
        sharedContent = result.isBinary
          ? null
          : parseDobiusYaml(result.content)?.issueCommand?.trim() || null
      } catch (error) {
        if (!isENOENT(error)) {
          status = 'error'
        }
    ...
    async (_event, args: { repoId: string; content: string }) => {
      ...
      if (repo.connectionId) {
        ...
        try {
          ...
          if (!result.isBinary && !/^\.dobius\/?$/m.test(result.content)) {
