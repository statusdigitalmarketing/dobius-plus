...
import { registerImessageBridgeHandlers } from './imessage-bridge'
...
import { registerDestinationHandlers } from './destinations'
...
import { setBuildDispatcher, setRepoLister } from '../agents/agent-dispatch-registry'
...
export function registerCoreHandlers(
  ...
  setRepoLister(() =>
    runtime.listRepos().map((repo) => ({ id: repo.id, name: repo.displayName || repo.id }))
  )
  setBuildDispatcher(async ({ repoId, name, baseBranch, brief, buildAgent }) => {
    try {
      const result = await runtime.createManagedWorktree({
        repoSelector: repoId,
        name,
        baseBranch,
        startupAgent: buildAgent,
        startupPrompt: brief,
        activate: false,
        createdWithAgent: buildAgent
      })
      return { worktreeId: result.worktree.id, ok: true }
    } catch (error) {
      return {
        worktreeId: '',
        ok: false,
  ...
  registerKnowledgeHandlers(store)
  if (automations) {
    registerAutomationHandlers(store, automations)
  }
  registerDestinationHandlers()
  if (keybindings) {
