...
describe('GitLab IPC handlers', () => {
    ...
    await expect(
      handler?.(null, {
        ...
        sourceContext: {
          kind: 'task-source',
          provider: 'gitlab',
          projectId: 'gitlab:statusdigitalmarketing/dobius-plus',
  ...
  it('rejects source context for a different host', async () => {
    registerGitLabHandlers(
      storeWithRepos([repo({ id: 'repo-local', path: '/local/dobius' })]) as Store
    )
  ...
  it('resolves pasted URL lookups by repoId and source host context', async () => {
    ...
    await expect(
      handler?.(null, {
        ...
        host: 'gitlab.com',
        path: 'statusdigitalmarketing/dobius-plus',
        iid: 42,
        type: 'issue'
      })
    ...
    expect(getWorkItemByProjectRefMock).toHaveBeenCalledWith(
      '/ssh/dobius',
      { host: 'gitlab.com', path: 'statusdigitalmarketing/dobius-plus' },
      42,
      'issue',
      'builder'
    )
  ...
  it('routes local WSL project GitLab issue, MR, work-item, and todo IPC through project git options', async () => {
    ...
    getProjectSlugMock.mockResolvedValue({ host: 'gitlab.com', path: 'statusdigitalmarketing/dobius-plus' })
    ...
    await ipcHandlers.get('gitlab:projectSlug')?.(null, { repoPath: '/local/dobius' })
    await ipcHandlers.get('gitlab:mrForBranch')?.(null, {
      repoPath: '/local/dobius',
      branch: 'feature/wsl'
    })
    await ipcHandlers.get('gitlab:mr')?.(null, { repoPath: '/local/dobius', iid: 8 })
    await ipcHandlers.get('gitlab:listMRs')?.(null, {
      repoPath: '/local/dobius',
      state: 'opened',
      page: 1,
    ...
    await ipcHandlers.get('gitlab:issue')?.(null, { repoPath: '/local/dobius', number: 7 })
    ...
    await ipcHandlers.get('gitlab:listLabels')?.(null, { repoPath: '/local/dobius' })
    await ipcHandlers.get('gitlab:listAssignableUsers')?.(null, { repoPath: '/local/dobius' })
    await ipcHandlers.get('gitlab:todos')?.(null, { repoPath: '/local/dobius' })

    const hostedReviewOptions = { localGitExecOptions: localGitOptions }
    expect(getProjectSlugMock).toHaveBeenCalledWith('/local/dobius', null, hostedReviewOptions)
    ...
    expect(getMergeRequestMock).toHaveBeenCalledWith('/local/dobius', 8, null, hostedReviewOptions)
    expect(listMergeRequestsMock).toHaveBeenCalledWith(
      '/local/dobius',
      'opened',
      1,
    ...
    expect(getIssueMock).toHaveBeenCalledWith('/local/dobius', 7, null, localGitOptions)
    ...
    expect(listLabelsMock).toHaveBeenCalledWith('/local/dobius', undefined, null, localGitOptions)
    ...
    expect(listTodosMock).toHaveBeenCalledWith('/local/dobius', null, localGitOptions)
  })
  ...
  it('routes local WSL project GitLab MR details, review, job, and pasted URL IPC through project git options', async () => {
    ...
    await ipcHandlers.get('gitlab:closeMR')?.(null, { repoPath: '/local/dobius', iid: 8 })
    await ipcHandlers.get('gitlab:reopenMR')?.(null, { repoPath: '/local/dobius', iid: 8 })
    ...
    await ipcHandlers.get('gitlab:jobTrace')?.(null, { repoPath: '/local/dobius', jobId: 99 })
    await ipcHandlers.get('gitlab:retryJob')?.(null, { repoPath: '/local/dobius', jobId: 99 })
