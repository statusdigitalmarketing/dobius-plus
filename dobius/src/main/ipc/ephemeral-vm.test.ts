...
describe('registerEphemeralVmHandlers', () => {
  ...
  beforeEach(() => {
    ...
    connectRuntimeOwnedSshTargetMock.mockResolvedValue({
      targetId: 'runtime-ssh-dobius-instance-1',
      target: {
        id: 'runtime-ssh-dobius-instance-1',
        label: 'Sandbox',
  ...
  it('lists recipes from local repo dobius.yaml', async () => {
    const repoPath = makeDir('dobius-ephemeral-vm-ipc-repo-')
    writeFileSync(
      join(repoPath, 'dobius.yaml'),
      [
        'environmentRecipes:',
        '  - id: cloud-sandbox',
        '    name: Cloud Sandbox',
  ...
  it('provisions a recipe and persists the ephemeral runtime', async () => {
    const userDataPath = makeDir('dobius-ephemeral-vm-ipc-user-data-')
    const repoPath = makeDir('dobius-ephemeral-vm-ipc-repo-')
  ...
  it('provisions an ssh recipe without creating a runtime environment', async () => {
    ...
    expect(result).toMatchObject({
      ok: true,
      connectionType: 'ssh',
      sshTargetId: 'runtime-ssh-dobius-instance-1',
      runtime: {
        repoId: 'repo-1',
        status: 'running',
        connectionMode: 'ssh',
  ...
  it('removes the runtime-owned SSH target on cleanup even when destroy fails', async () => {
    ...
    expect(removeRuntimeOwnedSshTargetMock).toHaveBeenCalledWith('runtime-ssh-dobius-instance-1')
