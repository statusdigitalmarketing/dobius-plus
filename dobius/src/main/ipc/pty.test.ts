...
const expectedAttributionShimDir = join(
  '/tmp/dobius-user-data',
  'dobius-terminal-attribution',
...
const TEST_CODEX_HOME =
  process.platform === 'win32'
    ? 'C:\\Users\\test\\AppData\\Roaming\\dobius\\codex-runtime-home\\home'
    : '/tmp/dobius-codex-home'
...
describe('registerPtyHandlers', () => {
  ...
  const savedDobiusOpenCodeConfigDir = process.env.DOBIUS_OPENCODE_CONFIG_DIR
  const savedDobiusOpenCodeSourceConfigDir = process.env.DOBIUS_OPENCODE_SOURCE_CONFIG_DIR
  const savedPiAgentDir = process.env.PI_CODING_AGENT_DIR
  const savedDobiusPiAgentDir = process.env.DOBIUS_PI_CODING_AGENT_DIR
  const savedDobiusPiSourceAgentDir = process.env.DOBIUS_PI_SOURCE_AGENT_DIR
  const savedDobiusCodexHome = process.env.DOBIUS_CODEX_HOME
  const savedDobiusOmpAgentDir = process.env.DOBIUS_OMP_CODING_AGENT_DIR
  const savedDobiusOmpSourceAgentDir = process.env.DOBIUS_OMP_SOURCE_AGENT_DIR
  const savedDobiusOmpStatusExtension = process.env.DOBIUS_OMP_STATUS_EXTENSION
  const savedDobiusClaudeAgentStatusSettings = process.env.DOBIUS_CLAUDE_AGENT_STATUS_SETTINGS
  const savedProcessPlatform = Object.getOwnPropertyDescriptor(process, 'platform')
  const savedDisableMacosLoginShell = process.env.DOBIUS_DISABLE_MACOS_LOGIN_SHELL
  ...
  beforeEach(() => {
    ...
    // below re-enables it.
    process.env.DOBIUS_DISABLE_MACOS_LOGIN_SHELL = '1'
    delete process.env.OPENCODE_CONFIG_DIR
    delete process.env.DOBIUS_OPENCODE_SOURCE_CONFIG_DIR
    delete process.env.DOBIUS_OPENCODE_CONFIG_DIR
    delete process.env.DOBIUS_AGENT_HOOK_ENDPOINT
    delete process.env.DOBIUS_CLAUDE_AGENT_STATUS_SETTINGS
    delete process.env.PI_CODING_AGENT_DIR
    delete process.env.DOBIUS_PI_SOURCE_AGENT_DIR
    delete process.env.DOBIUS_PI_CODING_AGENT_DIR
    delete process.env.DOBIUS_CODEX_HOME
    delete process.env.DOBIUS_OMP_SOURCE_AGENT_DIR
    delete process.env.DOBIUS_OMP_CODING_AGENT_DIR
    delete process.env.DOBIUS_OMP_STATUS_EXTENSION
    handlers.clear()
    ...
    getPathMock.mockReturnValue('/tmp/dobius-user-data')
    existsSyncMock.mockReturnValue(true)
    statSyncMock.mockReturnValue({ isDirectory: () => true, mode: 0o755 })
    readFileSyncMock.mockReturnValue('')
    openCodeBuildPtyEnvMock.mockImplementation((_ptyId: string, existingConfigDir?: string) => ({
      DOBIUS_OPENCODE_HOOK_PORT: '4567',
      DOBIUS_OPENCODE_HOOK_TOKEN: 'opencode-token',
      DOBIUS_OPENCODE_PTY_ID: 'test-pty',
      OPENCODE_CONFIG_DIR: existingConfigDir
        ? '/tmp/dobius-opencode-overlay'
        : '/tmp/dobius-opencode-config'
    }))
    mimoCodeBuildPtyEnvMock.mockImplementation((_ptyId: string, existingHome?: string) => ({
      MIMOCODE_HOME: existingHome ? '/tmp/dobius-mimocode-overlay' : '/tmp/dobius-mimocode-shared'
    }))
    buildAgentHookEnvMock.mockReturnValue({
      DOBIUS_AGENT_HOOK_PORT: '5678',
      DOBIUS_AGENT_HOOK_TOKEN: 'agent-token'
    })
    piBuildPtyEnvMock.mockImplementation(
      (_ptyId: string, existingAgentDir?: string, kind?: string) =>
        kind === 'omp'
          ? {
              DOBIUS_OMP_SOURCE_AGENT_DIR: existingAgentDir ?? '/tmp/default-omp-agent',
              DOBIUS_OMP_STATUS_EXTENSION: `${existingAgentDir ?? '/tmp/default-omp-agent'}/extensions/dobius-agent-status.ts`
            }
          : {
              DOBIUS_PI_SOURCE_AGENT_DIR: existingAgentDir ?? '/tmp/default-pi-agent'
            }
    )
  ...
  afterEach(() => {
    ...
    if (savedDisableMacosLoginShell !== undefined) {
      process.env.DOBIUS_DISABLE_MACOS_LOGIN_SHELL = savedDisableMacosLoginShell
    } else {
      delete process.env.DOBIUS_DISABLE_MACOS_LOGIN_SHELL
    }
    ...
    if (savedDobiusOpenCodeConfigDir !== undefined) {
      process.env.DOBIUS_OPENCODE_CONFIG_DIR = savedDobiusOpenCodeConfigDir
    } else {
      delete process.env.DOBIUS_OPENCODE_CONFIG_DIR
    }
    if (savedDobiusOpenCodeSourceConfigDir !== undefined) {
      process.env.DOBIUS_OPENCODE_SOURCE_CONFIG_DIR = savedDobiusOpenCodeSourceConfigDir
    } else {
      delete process.env.DOBIUS_OPENCODE_SOURCE_CONFIG_DIR
    }
    ...
    if (savedDobiusPiAgentDir !== undefined) {
      process.env.DOBIUS_PI_CODING_AGENT_DIR = savedDobiusPiAgentDir
    } else {
      delete process.env.DOBIUS_PI_CODING_AGENT_DIR
    }
    if (savedDobiusPiSourceAgentDir === undefined) {
      delete process.env.DOBIUS_PI_SOURCE_AGENT_DIR
    } else {
      process.env.DOBIUS_PI_SOURCE_AGENT_DIR = savedDobiusPiSourceAgentDir
    }
    if (savedDobiusCodexHome === undefined) {
      delete process.env.DOBIUS_CODEX_HOME
    } else {
      process.env.DOBIUS_CODEX_HOME = savedDobiusCodexHome
    }
    if (savedDobiusOmpAgentDir !== undefined) {
      process.env.DOBIUS_OMP_CODING_AGENT_DIR = savedDobiusOmpAgentDir
    } else {
      delete process.env.DOBIUS_OMP_CODING_AGENT_DIR
    }
    if (savedDobiusOmpSourceAgentDir !== undefined) {
      process.env.DOBIUS_OMP_SOURCE_AGENT_DIR = savedDobiusOmpSourceAgentDir
    } else {
      delete process.env.DOBIUS_OMP_SOURCE_AGENT_DIR
    }
    if (savedDobiusOmpStatusExtension !== undefined) {
      process.env.DOBIUS_OMP_STATUS_EXTENSION = savedDobiusOmpStatusExtension
    } else {
      delete process.env.DOBIUS_OMP_STATUS_EXTENSION
    }
    if (savedDobiusClaudeAgentStatusSettings === undefined) {
      delete process.env.DOBIUS_CLAUDE_AGENT_STATUS_SETTINGS
    } else {
      process.env.DOBIUS_CLAUDE_AGENT_STATUS_SETTINGS = savedDobiusClaudeAgentStatusSettings
    }
  })
  ...
  describe('spawn environment', () => {
    ...
    it('always sets TERM and COLORTERM regardless of env', async () => {
      ...
      expect(env.TERM_PROGRAM).toBe('Dobius')
    })
    ...
    it('advertises OSC 8 hyperlink support via FORCE_HYPERLINK', async () => {
      ...
      // TERM_PROGRAM=Dobius, so tools like Claude Code emit plain text instead
    ...
    it('surfaces DOBIUS_APP_VERSION as TERM_PROGRAM_VERSION for TUI feature gating', async () => {
      const env = await spawnAndGetEnv(undefined, { DOBIUS_APP_VERSION: '1.2.3-test' })
      expect(env.TERM_PROGRAM_VERSION).toBe('1.2.3-test')
    })

    it('falls back to a placeholder version when DOBIUS_APP_VERSION is unset', async () => {
      const env = await spawnAndGetEnv(undefined, { DOBIUS_APP_VERSION: undefined })
      expect(env.TERM_PROGRAM_VERSION).toBe('0.0.0-dev')
    })

    it('injects the selected Codex home into Dobius terminal PTYs', async () => {
      const env = await spawnAndGetEnv(undefined, undefined, () => TEST_CODEX_HOME)
      expect(env.CODEX_HOME).toBe(TEST_CODEX_HOME)
      expect(env.DOBIUS_CODEX_HOME).toBe(TEST_CODEX_HOME)
    })

    it('injects the OpenCode hook env into Dobius terminal PTYs', async () => {
      ...
      expect(env.DOBIUS_OPENCODE_HOOK_PORT).toBe('4567')
      expect(env.DOBIUS_OPENCODE_HOOK_TOKEN).toBe('opencode-token')
      expect(env.DOBIUS_OPENCODE_PTY_ID).toBe('test-pty')
      expect(env.OPENCODE_CONFIG_DIR).toEqual(expect.any(String))
      expect(env.DOBIUS_OPENCODE_CONFIG_DIR).toBe(env.OPENCODE_CONFIG_DIR)
    })

    it('mirrors the original OpenCode source dir when launched from an Dobius overlay shell', async () => {
      const env = await spawnAndGetEnv({
        OPENCODE_CONFIG_DIR: '/tmp/parent-dobius-opencode-overlay',
        DOBIUS_OPENCODE_SOURCE_CONFIG_DIR: '/tmp/user-opencode-config'
      })
      expect(openCodeBuildPtyEnvMock).toHaveBeenCalledWith(
        expect.any(String),
        '/tmp/user-opencode-config'
      )
      expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/dobius-opencode-overlay')
      expect(env.DOBIUS_OPENCODE_CONFIG_DIR).toBe('/tmp/dobius-opencode-overlay')
      expect(env.DOBIUS_OPENCODE_SOURCE_CONFIG_DIR).toBe('/tmp/user-opencode-config')
    })

    it('does not treat inherited Dobius OpenCode config as user config without a source dir', async () => {
      const env = await spawnAndGetEnv({
        OPENCODE_CONFIG_DIR: '/tmp/parent-dobius-opencode-overlay',
        DOBIUS_OPENCODE_CONFIG_DIR: '/tmp/parent-dobius-opencode-overlay'
      })

      expect(openCodeBuildPtyEnvMock).toHaveBeenCalledWith(expect.any(String), undefined)
      expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/dobius-opencode-config')
      expect(env.DOBIUS_OPENCODE_CONFIG_DIR).toBe('/tmp/dobius-opencode-config')
      expect(env.DOBIUS_OPENCODE_SOURCE_CONFIG_DIR).toBeUndefined()
    })

    it('restores user OpenCode config when agent status hooks are disabled in a nested Dobius shell', async () => {
      const env = await spawnAndGetEnv(
        {
          OPENCODE_CONFIG_DIR: '/tmp/parent-dobius-opencode-overlay',
          DOBIUS_OPENCODE_CONFIG_DIR: '/tmp/parent-dobius-opencode-overlay',
      ...
      expect(env.DOBIUS_OPENCODE_CONFIG_DIR).toBeUndefined()
      expect(env.DOBIUS_OPENCODE_SOURCE_CONFIG_DIR).toBeUndefined()
    })
    ...
    it('injects MiMo overlay env only when launch command is mimo', async () => {
      ...
      expect(env.MIMOCODE_HOME).toBe('/tmp/dobius-mimocode-shared')
      expect(env.DOBIUS_MIMOCODE_HOME).toBe('/tmp/dobius-mimocode-shared')
      expect(env.DOBIUS_MIMOCODE_SOURCE_HOME).toBeUndefined()
    })
    ...
    it('restores user MiMo home when agent status hooks are disabled in a nested Dobius shell', async () => {
      const env = await spawnAndGetEnv(
        {
          MIMOCODE_HOME: '/tmp/parent-dobius-mimocode-overlay',
          DOBIUS_MIMOCODE_HOME: '/tmp/parent-dobius-mimocode-overlay',
          DOBIUS_MIMOCODE_SOURCE_HOME: '/tmp/user-mimocode-home'
        },
        undefined,
        undefined,
        () => ({ agentStatusHooksEnabled: false }),
      ...
      expect(env.DOBIUS_MIMOCODE_HOME).toBeUndefined()
      expect(env.DOBIUS_MIMOCODE_SOURCE_HOME).toBeUndefined()
    })

    posixOnlyIt(
      'reproduces issue #1534: GUI-launched Dobius mirrors zshrc-only OpenCode config',
      async () => {
        ...
        const env = await spawnAndGetEnv(undefined, {
          ...
          DOBIUS_OPENCODE_SOURCE_CONFIG_DIR: undefined
        })
        ...
        expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/dobius-opencode-overlay')
        expect(env.DOBIUS_OPENCODE_CONFIG_DIR).toBe('/tmp/dobius-opencode-overlay')
        expect(env.DOBIUS_OPENCODE_SOURCE_CONFIG_DIR).toBe('/home/pim/company/opencode-config')
        expect(env.OPENCODE_CONFIG_DIR).not.toBe(env.DOBIUS_OPENCODE_SOURCE_CONFIG_DIR)
      }
    )

    it('installs Pi managed extensions without redirecting Dobius terminal PTY homes', async () => {
      ...
      expect(env.DOBIUS_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(env.DOBIUS_PI_SOURCE_AGENT_DIR).toBe('/tmp/user-pi-agent')
      expect(env.DOBIUS_OMP_CODING_AGENT_DIR).toBeUndefined()
      expect(env.DOBIUS_OMP_STATUS_EXTENSION).toBe(
        '/tmp/default-omp-agent/extensions/dobius-agent-status.ts'
      )
    })

    it('threads command: "omp" through to piBuildPtyEnv and emits OMP status metadata', async () => {
      // Why: OMP launches must emit OMP-named Dobius shadow vars (DOBIUS_OMP_*),
      ...
      // packages/utils/src/dirs.ts), but every other Dobius-owned env name
      ...
      expect(env.DOBIUS_OMP_CODING_AGENT_DIR).toBeUndefined()
      expect(env.DOBIUS_OMP_STATUS_EXTENSION).toBe(
        '/tmp/user-omp-agent/extensions/dobius-agent-status.ts'
      )
      expect(env.DOBIUS_OMP_SOURCE_AGENT_DIR).toBe('/tmp/user-omp-agent')
      // CRITICAL: a Pi-named shadow MUST NOT leak into an OMP PTY env.
      expect(env.DOBIUS_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(env.DOBIUS_PI_SOURCE_AGENT_DIR).toBeUndefined()
    })
    ...
    it('mirrors the original Pi source dir when launched from an Dobius overlay shell', async () => {
      const env = await spawnAndGetEnv({
        PI_CODING_AGENT_DIR: '/tmp/parent-dobius-pi-overlay',
        DOBIUS_PI_SOURCE_AGENT_DIR: '/tmp/user-pi-agent'
      })
      expect(piBuildPtyEnvMock).toHaveBeenCalledWith(expect.any(String), '/tmp/user-pi-agent', 'pi')
      expect(env.PI_CODING_AGENT_DIR).toBe('/tmp/parent-dobius-pi-overlay')
      expect(env.DOBIUS_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(env.DOBIUS_PI_SOURCE_AGENT_DIR).toBe('/tmp/user-pi-agent')
    })
    ...
    it('does not use an inherited Pi overlay source for an OMP launch', async () => {
      const env = await spawnAndGetEnv(
        {
          PI_CODING_AGENT_DIR: '/tmp/parent-dobius-pi-overlay',
          DOBIUS_PI_CODING_AGENT_DIR: '/tmp/parent-dobius-pi-overlay',
          DOBIUS_PI_SOURCE_AGENT_DIR: '/tmp/user-pi-agent'
        },
        undefined,
        undefined,
      ...
      expect(piBuildPtyEnvMock).toHaveBeenCalledWith(expect.any(String), undefined, 'omp')
      expect(env.DOBIUS_OMP_CODING_AGENT_DIR).toBeUndefined()
      expect(env.DOBIUS_OMP_SOURCE_AGENT_DIR).toBe('/tmp/default-omp-agent')
      expect(env.DOBIUS_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(env.DOBIUS_PI_SOURCE_AGENT_DIR).toBeUndefined()
    })
    ...
    it('does not use an inherited OMP overlay source for an explicit Pi launch', async () => {
      const env = await spawnAndGetEnv(
        {
          PI_CODING_AGENT_DIR: '/tmp/parent-dobius-omp-overlay',
          DOBIUS_OMP_CODING_AGENT_DIR: '/tmp/parent-dobius-omp-overlay',
          DOBIUS_OMP_SOURCE_AGENT_DIR: '/tmp/user-omp-agent'
        },
        undefined,
        undefined,
        undefined,
      ...
      expect(env.DOBIUS_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(env.DOBIUS_PI_SOURCE_AGENT_DIR).toBe('/tmp/default-pi-agent')
      expect(env.DOBIUS_OMP_CODING_AGENT_DIR).toBeUndefined()
      expect(env.DOBIUS_OMP_SOURCE_AGENT_DIR).toBeUndefined()
      expect(env.DOBIUS_OMP_STATUS_EXTENSION).toBeUndefined()
    })

    it('restores user Pi config when agent status hooks are disabled in a nested Dobius shell', async () => {
      const env = await spawnAndGetEnv(
        {
          PI_CODING_AGENT_DIR: '/tmp/parent-dobius-pi-overlay',
          DOBIUS_PI_CODING_AGENT_DIR: '/tmp/parent-dobius-pi-overlay',
    ...
    posixOnlyIt(
      ...
      async () => {
        ...
        expect(env.DOBIUS_PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.DOBIUS_PI_SOURCE_AGENT_DIR).toBe('/home/tester/.config/pi-agent')
      }
    )

    it('injects the agent hook receiver env into Dobius terminal PTYs', async () => {
      const env = await spawnAndGetEnv()
      ...
      expect(env.DOBIUS_AGENT_HOOK_PORT).toBe('5678')
      expect(env.DOBIUS_AGENT_HOOK_TOKEN).toBe('agent-token')
    })

    it('strips stale inherited hook receiver env before injecting this runtime', async () => {
      const env = await spawnAndGetEnv({
        DOBIUS_AGENT_HOOK_PORT: '1111',
        DOBIUS_AGENT_HOOK_TOKEN: 'stale-token',
        DOBIUS_AGENT_HOOK_ENV: 'production',
        DOBIUS_AGENT_HOOK_VERSION: 'stale-version',
        DOBIUS_AGENT_HOOK_ENDPOINT: '/tmp/stale-endpoint.env',
        DOBIUS_CLAUDE_AGENT_STATUS_SETTINGS: '/tmp/dobius/agent-hooks/claude-agent-status-settings.json'
      })

      expect(env.DOBIUS_AGENT_HOOK_PORT).toBe('5678')
      expect(env.DOBIUS_AGENT_HOOK_TOKEN).toBe('agent-token')
      expect(env.DOBIUS_AGENT_HOOK_ENV).toBeUndefined()
      expect(env.DOBIUS_AGENT_HOOK_VERSION).toBeUndefined()
      expect(env.DOBIUS_AGENT_HOOK_ENDPOINT).toBeUndefined()
      expect(env.DOBIUS_CLAUDE_AGENT_STATUS_SETTINGS).toBeUndefined()
    })
    ...
    it('does not leak inherited hook receiver env if the hook server is unavailable', async () => {
      ...
      expect(env.DOBIUS_AGENT_HOOK_PORT).toBeUndefined()
      expect(env.DOBIUS_AGENT_HOOK_TOKEN).toBeUndefined()
      expect(env.DOBIUS_AGENT_HOOK_ENV).toBeUndefined()
      expect(env.DOBIUS_AGENT_HOOK_VERSION).toBeUndefined()
      expect(env.DOBIUS_AGENT_HOOK_ENDPOINT).toBeUndefined()
      expect(env.DOBIUS_CLAUDE_AGENT_STATUS_SETTINGS).toBeUndefined()
    ...
    it('prepends local git/gh attribution shims when attribution is enabled', async () => {
      ...
      expect(env.DOBIUS_ENABLE_GIT_ATTRIBUTION).toBe('1')
      expect(env.DOBIUS_GIT_COMMIT_TRAILER).toBe('Co-authored-by: Dobius <noreply@statusdigitalmarketing.com>')
      expect(env.DOBIUS_GH_PR_FOOTER).toBe('Made with [Dobius](https://github.com/statusdigitalmarketing/dobius-plus) 🐋')
      expect(env.DOBIUS_GH_ISSUE_FOOTER).toBe('Made with [Dobius](https://github.com/statusdigitalmarketing/dobius-plus) 🐋')
    ...
    it('skips git/gh attribution shims when attribution is disabled', async () => {
      ...
      expect(env.DOBIUS_ENABLE_GIT_ATTRIBUTION).toBeUndefined()
      expect(env.DOBIUS_GIT_COMMIT_TRAILER).toBeUndefined()
      expect(env.DOBIUS_GH_PR_FOOTER).toBeUndefined()
      expect(env.DOBIUS_GH_ISSUE_FOOTER).toBeUndefined()
    ...
    it('overrides ambient CODEX_HOME with the Dobius-managed home for system default', async () => {
      const env = await spawnAndGetEnv(
        undefined,
    ...
    describe('daemon-active provider (parity with LocalPtyProvider)', () => {
      ...
      it('mirrors a user-provided OPENCODE_CONFIG_DIR into a source-scoped overlay on the daemon path', async () => {
        ...
        expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/dobius-opencode-overlay')
        expect(env.DOBIUS_OPENCODE_CONFIG_DIR).toBe('/tmp/dobius-opencode-overlay')
        expect(env.DOBIUS_OPENCODE_SOURCE_CONFIG_DIR).toBe('/user/custom/opencode')
      })

      it('uses source OpenCode config env instead of remirroring a parent overlay', async () => {
        const env = await daemonSpawnAndGetEnv({
          OPENCODE_CONFIG_DIR: '/tmp/parent-dobius-opencode-overlay',
          DOBIUS_OPENCODE_SOURCE_CONFIG_DIR: '/user/custom/opencode'
        })
        expect(openCodeBuildPtyEnvMock).toHaveBeenCalledWith(
          expect.any(String),
          '/user/custom/opencode'
      ...
      it('installs Pi managed extensions without redirecting homes on the daemon path', async () => {
        ...
        expect(env.DOBIUS_PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.DOBIUS_PI_SOURCE_AGENT_DIR).toBe('/user/.pi/agent')
        expect(env.DOBIUS_OMP_CODING_AGENT_DIR).toBeUndefined()
        expect(env.DOBIUS_OMP_STATUS_EXTENSION).toBe(expectedOmpStatusExtension)
      })
      ...
      it('threads command: "omp" through to piBuildPtyEnv on the daemon path with OMP status metadata', async () => {
        ...
        expect(env.DOBIUS_OMP_CODING_AGENT_DIR).toBeUndefined()
        expect(env.DOBIUS_OMP_STATUS_EXTENSION).toBe(
          '/user/.omp/agent/extensions/dobius-agent-status.ts'
        )
        expect(env.DOBIUS_OMP_SOURCE_AGENT_DIR).toBe('/user/.omp/agent')
        expect(env.DOBIUS_PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.DOBIUS_PI_SOURCE_AGENT_DIR).toBeUndefined()
      })
      ...
      it('skips host Codex home when a daemon-backed Windows spawn targets a WSL cwd', async () => {
        ...
        try {
          const spawnOptions = await daemonSpawnAndGetOptions(
            ...
            undefined,
            {
              CODEX_HOME: 'C:\\Users\\test\\AppData\\Roaming\\Dobius\\codex-runtime-home\\home',
              DOBIUS_CODEX_HOME: 'C:\\Users\\test\\AppData\\Roaming\\Dobius\\codex-runtime-home\\home'
            },
            {
          ...
          expect(env.DOBIUS_CODEX_HOME).toBeUndefined()
          expect(spawnOptions.envToDelete).toEqual(
            expect.arrayContaining(['CODEX_HOME', 'DOBIUS_CODEX_HOME'])
          )
        } finally {
          Object.defineProperty(process, 'platform', {
            configurable: true,
      ...
      it('skips host Codex home when a daemon-backed Windows spawn uses a WSL shell override', async () => {
        ...
        try {
          ...
          expect(spawnOptions.env.DOBIUS_CODEX_HOME).toBeUndefined()
          expect(spawnOptions.envToDelete).toEqual(
            expect.arrayContaining(['CODEX_HOME', 'DOBIUS_CODEX_HOME'])
          )
        } finally {
      ...
      it('deletes stale Claude scoped settings env from daemon-hosted PTYs', async () => {
        const spawnOptions = await daemonSpawnAndGetOptions({}, undefined, undefined, {
          DOBIUS_CLAUDE_AGENT_STATUS_SETTINGS:
            '/tmp/dobius/agent-hooks/claude-agent-status-settings.json'
        })
        expect(spawnOptions.env.DOBIUS_CLAUDE_AGENT_STATUS_SETTINGS).toBeUndefined()
        expect(spawnOptions.envToDelete).toEqual(
          expect.arrayContaining(['DOBIUS_CLAUDE_AGENT_STATUS_SETTINGS'])
        )
        expect(spawnOptions.env.DOBIUS_AGENT_HOOK_PORT).toBe('5678')
        expect(spawnOptions.env.DOBIUS_AGENT_HOOK_TOKEN).toBe('agent-token')
      })
      ...
      it('deletes stale Claude scoped settings env from runtime-created daemon PTYs', async () => {
        ...
        process.env.DOBIUS_CLAUDE_AGENT_STATUS_SETTINGS =
          '/tmp/dobius/agent-hooks/claude-agent-status-settings.json'
        handlers.clear()
      ...
      it('keeps the Agent Teams tmux shim ahead of host PATH shims for runtime-created daemon PTYs', async () => {
        ...
        await controller.spawn({
          ...
          command: 'claude',
          env: {
            PATH: `/tmp/dobius-agent-teams-bin${delimiter}/usr/bin`,
            DOBIUS_AGENT_TEAMS_TEAM_ID: 'team-test',
            TERM_PROGRAM: 'Dobius',
            DOBIUS_ATTRIBUTION_SHIM_DIR: '/tmp/stale-attribution'
          },
          envToDelete: ['TERM_PROGRAM', 'DOBIUS_ATTRIBUTION_SHIM_DIR']
        })

        const spawnOptions = daemonSpawn.mock.calls.at(-1)?.[0] as DaemonSpawnCall
        expect(spawnOptions.env.PATH.split(delimiter)[0]).toBe('/tmp/dobius-agent-teams-bin')
        expect(spawnOptions.env.PATH).toContain(expectedAttributionShimDir)
        expect(spawnOptions.env.TERM_PROGRAM).toBeUndefined()
        expect(spawnOptions.env.DOBIUS_ATTRIBUTION_SHIM_DIR).toBeUndefined()
        expect(spawnOptions.envToDelete).toEqual(
          expect.arrayContaining(['TERM_PROGRAM', 'DOBIUS_ATTRIBUTION_SHIM_DIR'])
        )
      })
      ...
      it('injects dev-mode DOBIUS_USER_DATA_PATH + dev CLI PATH on the daemon path', async () => {
        ...
        try {
          ...
          expect(env.DOBIUS_USER_DATA_PATH).toBe('/tmp/dobius-user-data')
          expect(env.PATH).toContain(join('/tmp/dobius-user-data', 'cli', 'bin'))
        } finally {
          mockedApp.isPackaged = prev
        }
      })
      ...
      it('preserves the inherited PATH when dev-mode daemon env omits PATH', async () => {
        ...
        try {
          ...
          expect(env.PATH).toContain(
            `${join('/tmp/dobius-user-data', 'cli', 'bin')}${delimiter}/system/bin`
          )
        } finally {
          mockedApp.isPackaged = prev
        }
      ...
      it('falls back to process.env.PI_CODING_AGENT_DIR when baseEnv lacks it on the daemon path', async () => {
        ...
        // second arg so Dobius installs managed extensions in the user's root.
        const env = await daemonSpawnAndGetEnv({}, undefined, undefined, {
        ...
        expect(env.PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.DOBIUS_PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.DOBIUS_PI_SOURCE_AGENT_DIR).toBe('/ambient/pi/agent')
      })
      ...
      it('does not mutate the caller-provided args.env on the daemon path', async () => {
        ...
        // Dobius host env (hook tokens, overlay paths) back into the renderer's
        ...
        expect(spawnEnv.DOBIUS_AGENT_HOOK_PORT).toBe('5678')
      ...
      it('does NOT inject host-local env on SSH spawns (connectionId set)', async () => {
        ...
        await handlers.get('pty:spawn')!(null, {
          cols: 80,
          rows: 24,
          env: { FOO: 'bar', DOBIUS_PANE_KEY: makePaneKey('tab-1', leafId) },
          connectionId: 'ssh-1',
          worktreeId: 'wt-1',
          tabId: 'tab-1',
          leafId
        ...
        // server is on the Dobius host's 127.0.0.1, dev CLI / attribution /
        ...
        await handlers.get('pty:spawn')!(null, {
          cols: 80,
          rows: 24,
          env: { DOBIUS_PANE_KEY: 'tab-1:pane:1' },
          connectionId: 'ssh-1',
          worktreeId: 'wt-1',
          tabId: 'tab-1',
          leafId: 'pane:1'
        ...
        expect(sshSpawn.mock.calls.at(-1)?.[0].env.DOBIUS_PANE_KEY).toBeUndefined()
      ...
      it('strips DOBIUS_PANE_KEY/TAB_ID/WORKTREE_ID from SSH spawn env when remote agent hooks are disabled', async () => {
        const sshSpawn = vi.fn(async (_opts: { env: Record<string, string> }) => ({
          id: 'ssh-pty'
        }))
        registerSshPtyProvider('ssh-1', {
        ...
        handlers.clear()
        registerPtyHandlers(mainWindow as never)
        const prevFlag = process.env.DOBIUS_FEATURE_REMOTE_AGENT_HOOKS
        process.env.DOBIUS_FEATURE_REMOTE_AGENT_HOOKS = '0'
        try {
          await handlers.get('pty:spawn')!(null, {
            cols: 80,
            rows: 24,
            env: {
              FOO: 'bar',
              DOBIUS_PANE_KEY: 'tab-1:0',
              DOBIUS_TAB_ID: 'tab-1',
              DOBIUS_WORKTREE_ID: 'wt-1'
            },
            connectionId: 'ssh-1'
          })
          const env = sshSpawn.mock.calls.at(-1)![0].env
          expect(env.FOO).toBe('bar')
          expect(env.DOBIUS_PANE_KEY).toBeUndefined()
          expect(env.DOBIUS_TAB_ID).toBeUndefined()
          expect(env.DOBIUS_WORKTREE_ID).toBeUndefined()
          expect(env.DOBIUS_AGENT_HOOK_TOKEN).toBeUndefined()
          ...
          if (prevFlag === undefined) {
            delete process.env.DOBIUS_FEATURE_REMOTE_AGENT_HOOKS
          } else {
            process.env.DOBIUS_FEATURE_REMOTE_AGENT_HOOKS = prevFlag
          }
        }
      })

      it('forwards DOBIUS_PANE_KEY/TAB_ID/WORKTREE_ID over SSH by default', async () => {
        const sshSpawn = vi.fn(async (_opts: { env: Record<string, string> }) => ({
          id: 'ssh-pty'
        }))
        registerSshPtyProvider('ssh-1', {
        ...
        try {
          ...
          await handlers.get('pty:spawn')!(null, {
            ...
            env: {
              FOO: 'bar',
              DOBIUS_PANE_KEY: paneKey,
              DOBIUS_TAB_ID: 'tab-2',
              DOBIUS_WORKTREE_ID: 'wt-2'
            },
            connectionId: 'ssh-1',
            tabId: 'tab-2',
            leafId
          ...
          const env = sshSpawn.mock.calls.at(-1)![0].env
          expect(env.DOBIUS_PANE_KEY).toBe(paneKey)
          expect(env.DOBIUS_TAB_ID).toBe('tab-2')
          expect(env.DOBIUS_WORKTREE_ID).toBe('wt-2')
  ...
  it('injects DOBIUS_TERMINAL_HANDLE for non-local PTY providers', async () => {
    ...
    expect(spawn).toHaveBeenCalledWith(
      expect.objectContaining({
        env: expect.objectContaining({
          EXISTING: '1',
          DOBIUS_TERMINAL_HANDLE: 'term_remote'
        })
      })
    )
  ...
  it('refreshes captured native Agent Teams env for renderer PTY spawns', async () => {
    ...
    const runtime = {
      ...
      prepareClaudeAgentTeamsLeaderForHandle: vi.fn(async () => ({
        env: {
          ...
          TMUX: '/tmp/dobius-claude-agent-teams/team-fresh,0,1',
          TMUX_PANE: '%1',
          DOBIUS_AGENT_TEAMS_TEAM_ID: 'team-fresh',
          DOBIUS_AGENT_TEAMS_TOKEN: 'fresh-token'
        }
      })),
      registerPreAllocatedHandleForPty: vi.fn(),
      registerPty: vi.fn(),
    ...
    const result = (await handlers.get('pty:spawn')!(mainWindowIpcEvent, {
      ...
      worktreeId: 'wt-1',
      env: {
        DOBIUS_PANE_KEY: `tab-1:${leafId}`,
        DOBIUS_TAB_ID: 'tab-1',
        DOBIUS_WORKTREE_ID: 'wt-1',
        CLAUDE_PROFILE: 'captured',
        PATH: `/tmp/stale-agent-teams${delimiter}/usr/bin`,
        TMUX: '/tmp/dobius-claude-agent-teams/team-stale,0,1',
        DOBIUS_AGENT_TEAMS_TEAM_ID: 'team-stale',
        DOBIUS_AGENT_TEAMS_TOKEN: 'stale-token',
        TERM_PROGRAM: 'Dobius',
        DOBIUS_ATTRIBUTION_SHIM_DIR: '/tmp/stale-attribution'
      },
      launchConfig: {
    ...
    expect(spawnOptions.env).toMatchObject({
      CLAUDE_PROFILE: 'captured',
      CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS: '1',
      DOBIUS_TERMINAL_HANDLE: 'term_agent_teams',
      DOBIUS_AGENT_TEAMS_TEAM_ID: 'team-fresh',
      DOBIUS_AGENT_TEAMS_TOKEN: 'fresh-token',
      TMUX: '/tmp/dobius-claude-agent-teams/team-fresh,0,1',
      TMUX_PANE: '%1'
  ...
  it('reuses the runtime background handle in local PTY spawn env', async () => {
    ...
    expect(env.DOBIUS_TERMINAL_HANDLE).toBe('term_expected')
  ...
  it('persists runtime-owned headless session bindings when explicitly requested', async () => {
    ...
    await spawnController.spawn({
      ...
      tabId: 'tab-headless',
      leafId,
      env: { DOBIUS_PANE_KEY: makePaneKey('tab-headless', leafId) },
  ...
  it('reuses runtime materialization when renderer focuses the same pane during spawn', async () => {
    ...
    const runtimeSpawn = spawnController.spawn({
      ...
      tabId: 'tab-race',
      leafId,
      env: { DOBIUS_PANE_KEY: paneKey },
      persistHostSessionBinding: true
    })
    await Promise.resolve()

    // Why: SSH can strip DOBIUS_PANE_KEY before spawn; tab/leaf metadata must
    ...
    const rendererSpawn = handlers.get('pty:spawn')!(null, {
      ...
      leafId,
      env: {
        DOBIUS_TAB_ID: 'tab-race',
        DOBIUS_WORKTREE_ID: 'repo-1::/tmp'
      }
    }) as Promise<{ id: string }>
    await Promise.resolve()
  ...
  it('strips runtime-owned SSH pane env when remote agent hooks are disabled', async () => {
    ...
    const savedRemoteHooks = process.env.DOBIUS_FEATURE_REMOTE_AGENT_HOOKS
    process.env.DOBIUS_FEATURE_REMOTE_AGENT_HOOKS = '0'
    ...
    try {
      ...
      await spawnController.spawn({
        ...
        env: {
          FOO: 'bar',
          DOBIUS_PANE_KEY: makePaneKey('tab-remote', leafId),
          DOBIUS_TAB_ID: 'tab-remote',
          DOBIUS_WORKTREE_ID: 'wt-remote'
        },
      ...
      if (savedRemoteHooks === undefined) {
        ...
        process.env.DOBIUS_FEATURE_REMOTE_AGENT_HOOKS = savedRemoteHooks
      }
  ...
  it('does not leave SSH leases when runtime-owned binding persistence fails after reattach', async () => {
    ...
    ).rejects.toThrow(/DOBIUS_TERMINAL_SESSION_STATE_SAVE_FAILED/)
  ...
  it('ignores renderer-provided DOBIUS_TERMINAL_HANDLE for local PTY spawns', async () => {
    const runtime = {
      setPtyController: vi.fn(),
      preAllocateHandleForPty: vi.fn(() => 'term_trusted'),
      onPtySpawned: vi.fn(),
    ...
    await handlers.get('pty:spawn')!(null, {
      cols: 80,
      rows: 24,
      env: { DOBIUS_TERMINAL_HANDLE: 'term_untrusted' }
    })

    const spawnCall = spawnMock.mock.calls.at(-1)!
    const env = spawnCall[2].env as Record<string, string>
    expect(env.DOBIUS_TERMINAL_HANDLE).toBe('term_trusted')
    expect(runtime.preAllocateHandleForPty).toHaveBeenCalledWith(expect.any(String))
  })

  it('forwards the trusted Dobius terminal handle into managed WSL terminals', async () => {
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'win32'
    ...
    expect(env.DOBIUS_TERMINAL_HANDLE).toBe('term_wsl')
  ...
  it('spawns a plain POSIX login shell and queues startup commands for the live session', async () => {
    ...
    try {
      ...
      expect(options.env.ZDOTDIR).toBe('/tmp/dobius-user-data/shell-ready/zsh')
      expect(options.env.DOBIUS_ORIG_ZDOTDIR).toBe(process.env.HOME)
    } finally {
      Object.defineProperty(process, 'platform', {
        configurable: true,
        value: originalPlatform
  ...
  it('uses the POSIX shell wrapper so OpenCode config survives shell startup files', async () => {
    ...
    try {
      ...
      expect(shell).toBe('/bin/zsh')
      expect(args).toEqual(['-l'])
      expect(options.env.OPENCODE_CONFIG_DIR).toBe('/tmp/dobius-opencode-config')
      expect(options.env.DOBIUS_OPENCODE_CONFIG_DIR).toBe('/tmp/dobius-opencode-config')
      expect(options.env.ZDOTDIR).toBe('/tmp/dobius-user-data/shell-ready/zsh')
      expect(options.env.DOBIUS_SHELL_READY_MARKER).toBe('0')
    } finally {
      Object.defineProperty(process, 'platform', {
        configurable: true,
        value: originalPlatform
  ...
  it('uses the POSIX shell wrapper so Pi config survives shell startup files', async () => {
    ...
    try {
      ...
      expect(options.env.DOBIUS_OPENCODE_CONFIG_DIR).toBeUndefined()
      expect(options.env.PI_CODING_AGENT_DIR).toBe('/tmp/user-pi-agent')
      expect(options.env.DOBIUS_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(options.env.DOBIUS_PI_SOURCE_AGENT_DIR).toBe('/tmp/user-pi-agent')
      expect(options.env.ZDOTDIR).toBe('/tmp/dobius-user-data/shell-ready/zsh')
      expect(options.env.DOBIUS_SHELL_READY_MARKER).toBe('0')
    } finally {
      Object.defineProperty(process, 'platform', {
  ...
  posixOnlyIt('waits for shell-ready before writing delivery-hinted Codex startup', async () => {
    ...
    try {
      ...
      const [, , options] = spawnMock.mock.calls[0]!
      expect(options.env.DOBIUS_SHELL_READY_MARKER).toBe('1')
      expect(mockProc.proc.write).not.toHaveBeenCalled()
      ...
      mockProc.emitData('\x1b]777;dobius-shell-ready\x07')
      await Promise.resolve()
    ...
    try {
      ...
      expect(spawnMock).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          cwd: '/tmp',
          env: expect.objectContaining({
            DOBIUS_OPENCODE_CONFIG_DIR: '/tmp/dobius-opencode-config',
            DOBIUS_SHELL_READY_MARKER: '0',
            ZDOTDIR: '/tmp/dobius-user-data/shell-ready/zsh'
          })
        })
      )
      expect(warnSpy).toHaveBeenCalledWith(
  ...
  it('upgrades legacy numeric pane keys when the spawn metadata proves the stable leaf', async () => {
    ...
    await handlers.get('pty:spawn')!(null, {
      ...
      tabId: 'tab-1',
      leafId,
      env: { DOBIUS_PANE_KEY: stablePaneKey }
    })
    ...
    await handlers.get('pty:spawn')!(null, {
      ...
      tabId: 'tab-1',
      leafId,
      env: { DOBIUS_PANE_KEY: makePaneKey('tab-2', leafId) }
    })
