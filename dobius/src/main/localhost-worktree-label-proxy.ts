...
const DOBIUS_LOCALHOST_SUFFIX = '.dobius.localhost'
...
export class LocalhostWorktreeLabelProxy {
  ...
  private buildLabeledUrl(label: string, target: URL): string {
    ...
    url.hostname = `${label}${DOBIUS_LOCALHOST_SUFFIX}`
  ...
  private async handleRequest(request: IncomingMessage, response: ServerResponse): Promise<void> {
    ...
    if (!route) {
      ...
      response.end('Unknown Dobius+ localhost label.')
      return
    }
  ...
  private routeForRequest(request: IncomingMessage): RegisteredRoute | null {
    const host =
      String(request.headers.host ?? '')
        .split(':')[0]
        ?.toLowerCase() ?? ''
    if (!host.endsWith(DOBIUS_LOCALHOST_SUFFIX)) {
      return null
    }
    const label = host.slice(0, -DOBIUS_LOCALHOST_SUFFIX.length)
