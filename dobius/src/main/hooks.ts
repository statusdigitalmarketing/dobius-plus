...
import { parseDobiusYaml } from '../shared/dobius-yaml'
import { resolveRepoConfigYamlPath } from './repo-config-yaml'
...
export { parseDobiusYaml }

/**
 * Load hooks from dobius.yaml in the given repo root.
 */
export function loadHooks(repoPath: string): DobiusHooks | null {
  const yamlPath = resolveRepoConfigYamlPath(repoPath)
  if (!yamlPath) {
    return null
  }
  ...
  try {
    ...
    return parseDobiusYaml(content)
  } catch {
    return null
  }
}
...
/**
 * Check whether a dobius.yaml (or legacy dobius.yaml) exists for a repo.
 */
export function hasHooksFile(repoPath: string): boolean {
  return resolveRepoConfigYamlPath(repoPath) !== null
}

// Why: when a newer Dobius release adds a top-level key to `dobius.yaml` (like
// `issueCommand` was added here), older versions that don't recognise it will
// return `null` from `parseDobiusYaml` and show a confusing "could not be parsed"
// error.  Detecting well-formed but unrecognised keys lets the UI suggest an
// update instead of implying the file is broken.
const RECOGNIZED_DOBIUS_YAML_KEYS = new Set([
  'scripts',
  'issueCommand',
  'defaultTabs',
  'environmentRecipes'
...
/**
 * Return true when `dobius.yaml` contains at least one top-level key that this
 * version of Dobius does not handle.
 */
export function hasUnrecognizedDobiusYamlKeys(repoPath: string): boolean {
  try {
    const yamlPath = resolveRepoConfigYamlPath(repoPath)
    if (!yamlPath) {
      return false
    ...
    for (const line of iterateLfScriptLines(content)) {
      ...
      if (m != null && !RECOGNIZED_DOBIUS_YAML_KEYS.has(m[1])) {
        return true
      }
    }
    return false
...
// ─── Issue command files ────────────────────────────────────────────────
// Why: `dobius.yaml` is the tracked, project-wide defaults surface, while
// `.dobius/issue-command` remains the per-user override. Keeping the local file in
// `.dobius/` lets users customize agent automation without editing committed config.

const DOBIUS_DIR = '.dobius'
const ISSUE_COMMAND_FILENAME = 'issue-command'

export function getIssueCommandFilePath(repoPath: string): string {
  return join(repoPath, DOBIUS_DIR, ISSUE_COMMAND_FILENAME)
}
...
/**
 * Write the per-user issue command override to `{repoRoot}/.dobius/issue-command`.
 * Creates `.dobius/` and ensures it is in `.gitignore` on first write.
 * If content is empty, deletes only the override so the shared `dobius.yaml`
...
export function writeIssueCommand(repoPath: string, content: string): void {
  ...
  try {
    ...
    const dobiusDir = join(repoPath, DOBIUS_DIR)
    if (!existsSync(dobiusDir)) {
      mkdirSync(dobiusDir, { recursive: true })
    }
    ensureDobiusDirIgnored(repoPath)
...
/**
 * Ensure `.dobius` is listed in the repo's `.gitignore` so the per-user
 * directory is never accidentally committed.
 */
function ensureDobiusDirIgnored(repoPath: string): void {
  const gitignorePath = join(repoPath, '.gitignore')
  try {
    if (existsSync(gitignorePath)) {
      const content = readFileSync(gitignorePath, 'utf-8')
      if (/^\.dobius\/?$/m.test(content)) {
        return
      }
      const separator = content.endsWith('\n') ? '' : '\n'
      writeFileSync(gitignorePath, `${content}${separator}.dobius\n`, 'utf-8')
    } else {
      writeFileSync(gitignorePath, '.dobius\n', 'utf-8')
    }
  } catch {
    console.warn('[hooks] Could not update .gitignore to exclude .dobius')
  }
}
...
export function getEffectiveHooksFromConfig(
  ...
  // Why: committed `dobius.yaml` and local Settings commands can intentionally
...
export function getEffectiveHooks(repo: Repo, worktreePath?: string): DobiusHooks | null {
...
export function getDefaultTabCommandTrustContent(hooks: DobiusHooks | null): string {
...
export function getDefaultTabsLaunch(
  ...
  // Why: default tab commands come from committed `dobius.yaml`; a repo set to
...
function getSetupEnvVars(repo: Repo, worktreePath: string): Record<string, string> {
  return {
    DOBIUS_ROOT_PATH: repo.path,
    DOBIUS_WORKTREE_PATH: worktreePath,
    DOBIUS_WORKSPACE_NAME: getRuntimePathBasename(worktreePath),
...
function createWorktreeRunnerScript(
  ...
  const gitRelPath = useWindowsFormat ? `dobius/${runnerBaseName}.cmd` : `dobius/${runnerBaseName}.sh`
  ...
  // Why: when the worktree is on WSL, env vars like DOBIUS_ROOT_PATH and
  // DOBIUS_WORKTREE_PATH contain Windows UNC paths. The setup script runs
...
export function runHook(
  ...
  if (wslInfo) {
    ...
    // Why: translate DOBIUS_ROOT_PATH / DOBIUS_WORKTREE_PATH to Linux paths so
    // hook scripts that reference $DOBIUS_WORKTREE_PATH get usable paths
