...
import type { DobiusVmRecipe } from '../shared/types'
...
function makeRepo(): string {
  const root = mkdtempSync(join(tmpdir(), 'dobius-vm-recipe-runner-'))
  tmpRoots.push(root)
  return root
}
...
describe('runEphemeralVmRecipeStart', () => {
  it('runs a recipe from the repo root and parses its JSON result', async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        'console.error(`instance:${process.env.DOBIUS_VM_INSTANCE_ID}`)',
        'console.log(JSON.stringify({',
        '  schemaVersion: 1,',
        `  pairingCode: ${JSON.stringify(makePairingCode())},`,
        "  projectRoot: '/workspace/repo',",
        '  userData: { providerResourceId: process.env.DOBIUS_VM_INSTANCE_ID }',
        '}))'
      ].join('\n')
    )
    ...
    if (result.ok) {
      ...
      expect(result.context.instanceId).toMatch(/^dobius-/)
  ...
  it('returns a parse failure when stdout is not the recipe result contract', async () => {
    ...
    const result = await runEphemeralVmRecipeStart({
      ...
      context: { instanceId: 'dobius-test-instance' }
    })
    ...
    expect(result).toMatchObject({
      ...
      signal: null,
      context: {
        instanceId: 'dobius-test-instance',
...
describe('runEphemeralVmRecipeCleanup', () => {
  it('builds a copyable cleanup payload and command', () => {
    const repoPath = makeRepo()
    const recipe: DobiusVmRecipe = {
      id: 'cloud-sandbox',
      name: 'Cloud Sandbox',
      create: 'unused',
      destroy: './scripts/dobius-vm/destroy.sh'
    }
    ...
    ).toContain('| ./scripts/dobius-vm/destroy.sh')
  })
  ...
  it('passes cleanup context and recipe result on stdin', async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        '    envMode: process.env.DOBIUS_VM_MODE,',
        '    envWorkspace: process.env.DOBIUS_WORKSPACE_NAME',
        '  }))',
        '})'
      ].join('\n')
    )
  ...
  it('passes suspend context and recipe result on stdin', async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "process.stdin.on('end', () => {",
        '  const payload = JSON.parse(input)',
        '  console.log(JSON.stringify({ mode: payload.mode, envMode: process.env.DOBIUS_VM_MODE }))',
        '})'
      ].join('\n')
    )
