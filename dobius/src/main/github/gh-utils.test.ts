...
describe('github owner/repo resolution', () => {
  ...
  it('parses GitHub HTTPS and SSH remotes', () => {
    ...
    expect(parseGitHubOwnerRepo('git@github.com:statusdigitalmarketing/dobius-plus.git')).toEqual({
    ...
    expect(parseGitHubOwnerRepo('ssh://git@github.com/statusdigitalmarketing/dobius-plus.git')).toEqual({
      owner: 'statusdigitalmarketing',
      repo: 'dobius'
    })
    expect(parseGitHubOwnerRepo('ssh://git@ssh.github.com:443/statusdigitalmarketing/dobius-plus.git')).toEqual({
      owner: 'statusdigitalmarketing',
      repo: 'dobius'
    })
    expect(parseGitHubOwnerRepo('git@example.com:statusdigitalmarketing/dobius-plus.git')).toBeNull()
  })

  it('parses GitHub Enterprise host identity', () => {
    expect(parseGitHubRemoteIdentity('https://ghe.acme.internal/acme/dobius.git')).toEqual({
      host: 'ghe.acme.internal',
      owner: 'acme',
      repo: 'dobius'
    })
    expect(parseGitHubRemoteIdentity('git@ghe.acme.internal:acme/dobius.git')).toEqual({
      host: 'ghe.acme.internal',
      owner: 'acme',
      repo: 'dobius'
    })
    expect(parseGitHubOwnerRepo('https://ghe.acme.internal/acme/dobius.git')).toBeNull()
  })

  it('keeps getOwnerRepo origin-based', async () => {
    gitExecFileAsyncMock.mockResolvedValueOnce({
      stdout: 'git@github.com:fork/dobius.git\n'
    })

    await expect(getOwnerRepo('/repo')).resolves.toEqual({ owner: 'fork', repo: 'dobius' })
  ...
  it('prefers upstream for issue owner/repo resolution', async () => {
    gitExecFileAsyncMock.mockResolvedValueOnce({
      stdout: 'git@github.com:statusdigitalmarketing/dobius-plus.git\n'
    })

    await expect(getIssueOwnerRepo('/repo')).resolves.toEqual({ owner: 'statusdigitalmarketing', repo: 'dobius' })
  ...
  it('falls back to origin when upstream is missing or non-GitHub', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@example.com:statusdigitalmarketing/dobius-plus.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:fork/dobius.git\n' })

    await expect(getIssueOwnerRepo('/repo')).resolves.toEqual({ owner: 'fork', repo: 'dobius' })
  ...
  it('does not mix origin and upstream cache entries for the same repo path', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@github.com:fork/dobius.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:statusdigitalmarketing/dobius-plus.git\n' })
  ...
  it('resolves SSH repo remotes through the registered SSH git provider', async () => {
    const sshProvider = {
      exec: vi.fn().mockResolvedValue({ stdout: 'git@github.com:statusdigitalmarketing/dobius-plus.git\n', stderr: '' })
    }
    getSshGitProviderMock.mockReturnValue(sshProvider)

    await expect(getOwnerRepo('/home/user/dobius', 'openclaw-2')).resolves.toEqual({
      owner: 'statusdigitalmarketing',
      repo: 'dobius'
    })
  ...
  it('keeps local and SSH owner/repo cache entries separate for the same path', async () => {
    const sshProvider = {
      exec: vi.fn().mockResolvedValue({ stdout: 'git@github.com:remote/dobius.git\n', stderr: '' })
    }
    gitExecFileAsyncMock.mockResolvedValueOnce({ stdout: 'git@github.com:local/dobius.git\n' })
    getSshGitProviderMock.mockReturnValue(sshProvider)

    await expect(getOwnerRepo('/repo')).resolves.toEqual({ owner: 'local', repo: 'dobius' })
    await expect(getOwnerRepo('/repo', 'ssh-1')).resolves.toEqual({ owner: 'remote', repo: 'dobius' })
  })

  it('keeps local host and local WSL owner/repo cache entries separate for the same path', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@github.com:host/dobius.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:wsl/dobius.git\n' })

    await expect(getOwnerRepo('/repo')).resolves.toEqual({ owner: 'host', repo: 'dobius' })
  ...
  it('prunes expired distinct owner/repo cache entries on later lookups', async () => {
    ...
    try {
      ...
      await expect(getOwnerRepo('/repo-a')).resolves.toEqual({ owner: 'statusdigitalmarketing', repo: 'dobius' })
  ...
  it('resolves PR candidates as upstream then origin and de-dupes matching slugs', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@github.com:Acme/Dobius.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:acme/dobius.git\n' })

    await expect(resolvePRRepositoryCandidates('/repo')).resolves.toEqual({
      candidates: [{ owner: 'Acme', repo: 'Dobius' }],
      headRepo: { owner: 'acme', repo: 'dobius' }
    })
  })

  it('ignores non-GitHub upstream while keeping origin as the head repo', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@example.com:Acme/Dobius.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:fork/dobius.git\n' })

    await expect(resolvePRRepositoryCandidates('/repo')).resolves.toEqual({
      candidates: [{ owner: 'fork', repo: 'dobius' }],
  ...
  it('expires cached remote owner/repo entries after the TTL', async () => {
    ...
    try {
      gitExecFileAsyncMock
        .mockResolvedValueOnce({ stdout: 'git@github.com:old/dobius.git\n' })
        .mockResolvedValueOnce({ stdout: 'git@github.com:new/dobius.git\n' })
  ...
  it('keeps local missing-remote probes cached beyond the short positive TTL', async () => {
    const repoPath = await mkdtemp(join(tmpdir(), 'dobius-gh-utils-'))
...
describe('resolveIssueSource', () => {
  ...
  it("'auto' + upstream exists → upstream, fellBack=false", async () => {
    ...
    await expect(resolveIssueSource('/repo', 'auto')).resolves.toEqual({
      source: { owner: 'statusdigitalmarketing', repo: 'dobius' },
      fellBack: false
    })
  })
  ...
  it("'auto' + no upstream → origin, fellBack=false", async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@example.com:statusdigitalmarketing/dobius-plus.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:solo/dobius.git\n' })

    await expect(resolveIssueSource('/repo', 'auto')).resolves.toEqual({
      source: { owner: 'solo', repo: 'dobius' },
      fellBack: false
    })
  })
  ...
  it("'origin' + no upstream → origin, fellBack=false", async () => {
    gitExecFileAsyncMock.mockResolvedValueOnce({
      stdout: 'git@github.com:solo/dobius.git\n'
    })
...
describe('gh error classification', () => {
  ...
  it('classifies "has disabled issues" stderr as issues_disabled', () => {
    const stderr =
      "Command failed: gh issue list --limit 36 --json number,title,state --repo brennanb2025/dobius --state open\nthe 'brennanb2025/dobius' repository has disabled issues"
