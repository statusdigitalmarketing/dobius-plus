...
describe('checkDobiusStarred', () => {
  ...
  it('returns true only for an included successful GitHub response', async () => {
    ...
    await expect(checkDobiusStarred()).resolves.toBe(true)

    expect(execFileAsyncMock).toHaveBeenCalledWith(
      'gh',
      ['api', '--include', 'user/starred/statusdigitalmarketing/dobius-plus'],
      { encoding: 'utf-8' }
    )
  })
  ...
  it('returns false for GitHub 404 not starred responses', async () => {
    ...
    await expect(checkDobiusStarred()).resolves.toBe(false)
  })
  ...
  it('returns null when gh exits successfully without response headers', async () => {
    ...
    await expect(checkDobiusStarred()).resolves.toBe(null)
  })
})
...
describe('getPRForBranch', () => {
  ...
  it('resolves fork PRs from the upstream PR repo with the origin head owner', async () => {
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
      candidates: [
        { owner: 'statusdigitalmarketing', repo: 'dobius' },
        { owner: 'fork', repo: 'dobius' }
      ],
      headRepo: { owner: 'fork', repo: 'dobius' }
    })
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify([
        {
          ...
          title: 'Fork PR',
          state: 'open',
          html_url: 'https://github.com/statusdigitalmarketing/dobius-plus/pull/1738',
          updated_at: '2026-03-28T00:00:00Z',
          draft: false,
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['api', 'repos/statusdigitalmarketing/dobius-plus/pulls?head=fork%3Afeature%2Ftest&state=all&per_page=1'],
      { cwd: '/repo-root' }
    )
    expect(pr).toMatchObject({
      number: 1738,
      prRepo: { owner: 'statusdigitalmarketing', repo: 'dobius' },
      headRepo: { owner: 'fork', repo: 'dobius' }
    })
  })
  ...
  it('looks up a linked PR number across PR repo candidates', async () => {
    ...
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          state: 'OPEN',
          url: 'https://github.com/fork/dobius/pull/99',
          statusCheckRollup: [],
    ...
    expect(pr?.prRepo).toEqual({ owner: 'fork', repo: 'dobius' })
  })
    ...
    ghExecFileAsyncMock
        ...
        stdout: JSON.stringify([
          {
            ...
            html_url: 'https://github.com/statusdigitalmarketing/dobius-plus/pull/5511',
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          title: 'Merged branch PR',
          state: 'MERGED',
          url: 'https://github.com/statusdigitalmarketing/dobius-plus/pull/5511',
          statusCheckRollup: [],
  ...
  it('reports upstream error when fallback branch discovery fails transiently then retry misses', async () => {
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
      candidates: [{ owner: 'statusdigitalmarketing', repo: 'dobius' }],
      headRepo: null
    })
    ghExecFileAsyncMock
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
      ['api', 'repos/statusdigitalmarketing/dobius-plus/pulls?head=statusdigitalmarketing%3Afeature%2Ftest&state=all&per_page=1'],
      { cwd: '/repo-root' }
    )
  })
    ...
    ghExecFileAsyncMock
        ...
        stdout: JSON.stringify([
          {
            ...
            state: 'open',
            html_url: 'https://github.com/statusdigitalmarketing/dobius-plus/pull/42',
            updated_at: '2026-03-28T00:00:00Z',
            draft: false,
            mergeable: true,
            base: { ref: 'main', sha: 'base-oid' },
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          state: 'OPEN',
          url: 'https://github.com/statusdigitalmarketing/dobius-plus/pull/42',
          statusCheckRollup: [],
          updatedAt: '2026-03-28T00:00:00Z',
          isDraft: false,
          mergeable: 'MERGEABLE',
  ...
  it('does not carry a merged upstream branch head repo into a fallback PR number', async () => {
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
      candidates: [{ owner: 'statusdigitalmarketing', repo: 'dobius' }],
      headRepo: { owner: 'origin-owner', repo: 'dobius' }
    })
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'fork-owner', repo: 'dobius' })
    gitExecFileAsyncMock.mockResolvedValueOnce({
  ...
  it('uses the tracked upstream remote owner for fork branch lookup', async () => {
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
      candidates: [
        { owner: 'statusdigitalmarketing', repo: 'dobius' },
        { owner: 'origin-owner', repo: 'dobius' }
      ],
      headRepo: { owner: 'origin-owner', repo: 'dobius' }
    })
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'fork-owner', repo: 'dobius' })
    ghExecFileAsyncMock
        ...
        stdout: JSON.stringify([
          {
            ...
            state: 'open',
            html_url: 'https://github.com/statusdigitalmarketing/dobius-plus/pull/78',
            updated_at: '2026-03-28T00:00:00Z',
            draft: false,
            mergeable: true,
            base: { ref: 'main', sha: 'base-oid' },
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          state: 'OPEN',
          url: 'https://github.com/statusdigitalmarketing/dobius-plus/pull/78',
          statusCheckRollup: [],
          updatedAt: '2026-03-28T00:00:00Z',
          isDraft: false,
          mergeable: 'MERGEABLE',
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        'api',
        'repos/statusdigitalmarketing/dobius-plus/pulls?head=fork-owner%3Acontributor%2Foriginal&state=all&per_page=1'
      ],
      { cwd: '/repo-root' }
    )
    expect(pr).toMatchObject({
      ...
      title: 'Hydrated fork upstream branch PR',
      prRepo: { owner: 'statusdigitalmarketing', repo: 'dobius' },
      headRepo: { owner: 'fork-owner', repo: 'dobius' }
    })
  })
  ...
  it('uses the tracked upstream remote owner when the fork branch name matches locally', async () => {
    ...
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'brennanb2025', repo: 'dobius' })
    ghExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: JSON.stringify([]) })
      .mockResolvedValueOnce({ stdout: JSON.stringify([]) })
      .mockResolvedValueOnce({
        stdout: JSON.stringify([
          {
            ...
            state: 'open',
            html_url: 'https://github.com/statusdigitalmarketing/dobius-plus/pull/6433',
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          title: 'Recover Windows worktree deletes from long paths',
          state: 'OPEN',
          url: 'https://github.com/statusdigitalmarketing/dobius-plus/pull/6433',
          statusCheckRollup: [],
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        'api',
        'repos/statusdigitalmarketing/dobius-plus/pulls?head=brennanb2025%3Abrennanb2025%2Fworktree-remove-fix&state=all&per_page=1'
      ],
      { cwd: '/repo-root' }
    )
    expect(pr).toMatchObject({
      number: 6433,
      prRepo: { owner: 'statusdigitalmarketing', repo: 'dobius' },
      headRepo: { owner: 'brennanb2025', repo: 'dobius' }
    })
  })
  ...
  it('uses the same-name tracked upstream fork owner through the SSH git provider', async () => {
        ...
        stdout: JSON.stringify([
          {
            ...
            state: 'open',
            html_url: 'https://github.com/statusdigitalmarketing/dobius-plus/pull/79',
            updated_at: '2026-03-28T00:00:00Z',
            draft: false,
            mergeable: true,
            base: { ref: 'main', sha: 'base-oid' },
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      3,
      ['api', 'repos/statusdigitalmarketing/dobius-plus/pulls?head=fork-owner%3Acontributor%2Ffix&state=all&per_page=1'],
      {}
    )
    expect(pr).toMatchObject({
      number: 79,
  ...
  it('resolves fork PR push target using the origin URL protocol', async () => {
    getOwnerRepoMock.mockResolvedValueOnce({ owner: 'statusdigitalmarketing', repo: 'dobius' })
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'statusdigitalmarketing', repo: 'dobius' })
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'prateek/fix-sidebar-agents-toggle',
          repo: {
            full_name: 'prateek/dobius',
            name: 'dobius',
            clone_url: 'https://github.com/prateek/dobius.git',
            ssh_url: 'git@github.com:prateek/dobius.git',
            owner: { login: 'prateek' }
          }
        }
      })
    })
    getRemoteUrlForRepoMock.mockResolvedValueOnce('git@github.com:statusdigitalmarketing/dobius-plus.git')

    const target = await getPullRequestPushTarget('/repo-root', 1738)

    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(['api', 'repos/statusdigitalmarketing/dobius-plus/pulls/1738'], {
      cwd: '/repo-root'
    })
    expect(target).toEqual({
      pushTarget: {
        remoteName: 'pr-prateek-dobius',
        branchName: 'prateek/fix-sidebar-agents-toggle',
        remoteUrl: 'git@github.com:prateek/dobius.git'
      }
    })
  })
    ...
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'fix-sidebar',
          repo: {
            full_name: 'statusdigitalmarketing/dobius-plus',
            name: 'dobius',
            clone_url: 'https://github.com/statusdigitalmarketing/dobius-plus.git',
            ssh_url: 'git@github.com:statusdigitalmarketing/dobius-plus.git',
            owner: { login: 'statusdigitalmarketing' }
          }
        }
      })
    })
  ...
  it('resolves a distinct upstream remote as the repo upstream', async () => {
    getOwnerRepoMock.mockResolvedValueOnce({ owner: 'tmchow', repo: 'dobius' })
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'statusdigitalmarketing', repo: 'dobius' })
  ...
  it('does not treat a same-repository upstream remote as a fork', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['repo', 'view', 'statusdigitalmarketing/dobius-plus', '--json', 'isFork,parent'],
  ...
  it('falls back to the GitHub parent when no upstream remote is configured', async () => {
    ...
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        isFork: true,
        parent: { name: 'dobius', owner: { login: 'statusdigitalmarketing' } }
      })
    })
  ...
  it('probes additional PR repo candidates when the first lookup is not found', async () => {
    ...
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'fork', repo: 'dobius' })
    ghExecFileAsyncMock
      .mockRejectedValueOnce(new Error('HTTP 404: Not Found'))
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          head: {
            ref: 'feature/test',
            repo: {
              full_name: 'fork/dobius',
              name: 'dobius',
              clone_url: 'https://github.com/fork/dobius.git',
              ssh_url: 'git@github.com:fork/dobius.git',
              owner: { login: 'fork' }
            }
          }
        })
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(1, ['api', 'repos/fork/dobius/pulls/1849'], {
      cwd: '/repo-root'
    })
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
      ['api', 'repos/statusdigitalmarketing/dobius-plus/pulls/1849'],
      { cwd: '/repo-root' }
    )
  })
...
describe('updatePRState', () => {
  ...
  it('reopens pull requests through the gh PR command', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['pr', 'reopen', '3977', '--repo', 'statusdigitalmarketing/dobius-plus'],
      { cwd: '/repo-root' }
    )
  ...
  it('closes pull requests through the gh PR command', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['pr', 'close', '3977', '--repo', 'statusdigitalmarketing/dobius-plus'],
      { cwd: '/repo-root' }
    )
  })
...
describe('GitHub GraphQL rate-limit guard', () => {
        ...
        stdout: JSON.stringify([
          {
            ...
            body: 'top-level',
            created_at: '2026-04-01T00:00:00Z',
            html_url: 'https://github.com/statusdigitalmarketing/dobius-plus/pull/7#issuecomment-10'
          }
        ])
      })
      .mockResolvedValueOnce({ stdout: '[]' })

    await getPRComments('/repo-root', 7, { prRepo: { owner: 'statusdigitalmarketing', repo: 'dobius' } }, undefined)

    expect(getOwnerRepoMock).not.toHaveBeenCalled()
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      1,
      ['api', '--cache', '60s', 'repos/statusdigitalmarketing/dobius-plus/issues/7/comments?per_page=100'],
      { cwd: '/repo-root' }
    )
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
      ['api', '--cache', '60s', 'repos/statusdigitalmarketing/dobius-plus/pulls/7/reviews?per_page=100'],
      { cwd: '/repo-root' }
    )
  })
  ...
  it('uses explicit PR repo for merge and title mutations', async () => {
    ghExecFileAsyncMock
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          title: 'PR',
          state: 'OPEN',
          url: 'https://github.com/statusdigitalmarketing/dobius-plus/pull/7',
          statusCheckRollup: [],
    ...
    await expect(
      mergePR('/repo-root', 7, 'squash', undefined, { owner: 'statusdigitalmarketing', repo: 'dobius' })
    ).resolves.toEqual({ ok: true })
    await expect(
      updatePRTitle('/repo-root', 7, 'New title', undefined, { owner: 'statusdigitalmarketing', repo: 'dobius' })
    ).resolves.toBe(true)
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
      ['pr', 'merge', '7', '--squash', '--repo', 'statusdigitalmarketing/dobius-plus'],
      expect.objectContaining({
        cwd: '/repo-root',
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      3,
      ['pr', 'edit', '7', '--title', 'New title', '--repo', 'statusdigitalmarketing/dobius-plus'],
      { cwd: '/repo-root' }
    )
  })
  ...
  it('sets and disables PR auto-merge with explicit PR repos and SSH context', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      1,
      ['pr', 'view', '7', '--json', 'id,headRefOid,baseRefName', '--repo', 'statusdigitalmarketing/dobius-plus'],
      {}
    )
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      3,
      ['pr', 'merge', '7', '--disable-auto', '--repo', 'statusdigitalmarketing/dobius-plus'],
      expect.objectContaining({
        env: expect.objectContaining({ GH_PROMPT_DISABLED: '1' })
      })
    )
  ...
  it('uses the queue-aware gh merge path when the base branch has a merge queue', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      3,
      ['pr', 'merge', '7', '--auto', '--squash', '--repo', 'statusdigitalmarketing/dobius-plus'],
      expect.objectContaining({
        cwd: '/repo-root',
        env: expect.objectContaining({ GH_PROMPT_DISABLED: '1' })
      })
  ...
  it('detects merge queues once per base branch and blocks direct merges', async () => {
    ...
    expect(ghExecFileAsyncMock.mock.calls[1]?.[0]).toEqual(
      expect.arrayContaining(['-f', 'owner=statusdigitalmarketing', '-f', 'repo=dobius', '-f', 'branch=true'])
    )
  ...
  it('caches unknown merge queue probes after GraphQL failures', async () => {
    getOwnerRepoMock.mockResolvedValue({ owner: 'statusdigitalmarketing', repo: 'dobius' })
    const prView = {
      number: 7,
      title: 'PR',
      state: 'OPEN',
    ...
    ghExecFileAsyncMock.mockImplementation(async (args) => {
      ...
      return {
        stdout: JSON.stringify({
          ...
          title: 'PR',
          state: 'OPEN',
          url: `https://github.com/statusdigitalmarketing/dobius-plus/pull/${prViewCount}`,
          statusCheckRollup: [],
          updatedAt: '2026-04-01T00:00:00Z',
          isDraft: false,
          mergeable: 'MERGEABLE',
  ...
  it('does not run merge conflict preflight for SSH-backed repos', async () => {
    ...
    await expect(
      mergePR('/remote/repo-root', 7, 'squash', 'ssh-1', { owner: 'statusdigitalmarketing', repo: 'dobius' })
    ).resolves.toEqual({ ok: true })
