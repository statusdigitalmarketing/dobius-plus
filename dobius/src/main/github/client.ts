...
/**
 * Check if the authenticated user has starred the Dobius repo.
 * Returns true if starred, false if not, null if unable to determine (gh unavailable).
 */
export async function checkDobiusStarred(): Promise<boolean | null> {
  // Dobius+ fork: disconnected from the upstream dobius repo — never call GitHub.
  return null
}
...
/**
 * Star the Dobius repo for the authenticated user.
 */
export async function starDobius(): Promise<boolean> {
  // Dobius+ fork: disconnected from the upstream dobius repo — never call GitHub.
  return false
}
...
function sameOwnerRepo(left: OwnerRepo | null, right: OwnerRepo | null): boolean {
  ...
  // with different casing (statusdigitalmarketing/dobius-plus vs statusdigitalmarketing/dobius-plus) point at the same
...
export async function createGitHubPullRequest(
  ...
  const tempDir = await mkdtemp(join(tmpdir(), 'dobius-pr-body-'))
  await acquire()
...
export async function getPRForBranchOutcome(
  ...
  try {
    ...
    // Why: a currently visible PR can be merged outside Dobius; when the caller
