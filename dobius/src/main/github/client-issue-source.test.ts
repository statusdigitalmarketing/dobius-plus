...
describe('GitHub issue source split', () => {
  ...
  it('uses upstream for issues and origin for PRs in mixed recent results', async () => {
    getIssueOwnerRepoMock.mockResolvedValueOnce({ owner: 'statusdigitalmarketing', repo: 'dobius' })
    getOwnerRepoMock.mockResolvedValueOnce({ owner: 'fork', repo: 'dobius' })
    ghExecFileAsyncMock
        ...
        stdout: JSON.stringify([
          {
            ...
            state: 'open',
            html_url: 'https://github.com/statusdigitalmarketing/dobius-plus/issues/923',
            labels: [],
        ...
        stdout: JSON.stringify([
          {
            ...
            title: 'Fork PR',
            state: 'open',
            html_url: 'https://github.com/fork/dobius/pull/42',
            labels: [],
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        ...
        '--cache',
        '120s',
        'repos/statusdigitalmarketing/dobius-plus/issues?per_page=10&state=open&sort=updated&direction=desc'
      ],
      { cwd: '/repo-root' }
    )
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        ...
        '--cache',
        '120s',
        'repos/fork/dobius/pulls?per_page=10&state=open&sort=updated&direction=desc'
      ],
      { cwd: '/repo-root' }
    )
  })
  ...
  it('omits gh api cache args for no-cache recent work-item requests', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      1,
      ['api', 'repos/statusdigitalmarketing/dobius-plus/issues?per_page=10&state=open&sort=updated&direction=desc'],
      { cwd: '/repo-root' }
    )
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
      ['api', 'repos/fork/dobius/pulls?per_page=10&state=open&sort=updated&direction=desc'],
      { cwd: '/repo-root' }
    )
  })
  ...
  it('lists SSH repo work items with explicit owner/repo and no local cwd', async () => {
    ...
    await listWorkItems('/home/jinwoo/dobius', 10, undefined, undefined, 'auto', 'openclaw-2')
    ...
    expect(getOwnerRepoMock).toHaveBeenCalledWith('/home/jinwoo/dobius', 'openclaw-2', {})
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      1,
      [
        'api',
  ...
  it('uses upstream for issue-only queries and origin for PR-only queries', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      expect.arrayContaining(['--repo', 'statusdigitalmarketing/dobius-plus']),
      { cwd: '/repo-root' }
    )
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      expect.arrayContaining(['--repo', 'fork/dobius']),
      { cwd: '/repo-root' }
    )
  })
  ...
  it("uses upstream for recent PRs when preference='upstream'", async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        ...
        '--cache',
        '120s',
        'repos/statusdigitalmarketing/dobius-plus/pulls?per_page=10&state=open&sort=updated&direction=desc'
      ],
      { cwd: '/repo-root' }
    )
  })
  ...
  it("uses upstream for PR counts when preference='upstream'", async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      [
        ...
        '--cache',
        '120s',
        `search/issues?q=${encodeURIComponent('repo:statusdigitalmarketing/dobius-plus is:pull-request is:open')}&per_page=1`,
        '--jq',
        '.total_count'
      ],
      { cwd: '/repo-root' }
  ...
  it("falls back to origin for PRs when preference='upstream' and upstream is missing", async () => {
    ...
    expect(result.sources).toEqual({
      issues: { owner: 'fork', repo: 'dobius' },
      prs: { owner: 'fork', repo: 'dobius' },
      originCandidate: { owner: 'fork', repo: 'dobius' },
  ...
  it('counts default work items across upstream issues and origin PRs', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        ...
        '--cache',
        '120s',
        `search/issues?q=${encodeURIComponent('repo:statusdigitalmarketing/dobius-plus is:issue is:open')}&per_page=1`,
        '--jq',
        '.total_count'
      ],
      { cwd: '/repo-root' }
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        ...
        '--cache',
        '120s',
        `search/issues?q=${encodeURIComponent('repo:fork/dobius is:pull-request is:open')}&per_page=1`,
        '--jq',
        '.total_count'
      ],
      { cwd: '/repo-root' }
  ...
  it('raw number lookup tries upstream issue before origin PR', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      1,
      ['api', 'repos/statusdigitalmarketing/dobius-plus/issues/42'],
      { cwd: '/repo-root' }
    )
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
  ...
  it('surfaces a 403 from upstream issues through the listWorkItems envelope', async () => {
    ...
    expect(result.sources).toMatchObject({
      issues: { owner: 'statusdigitalmarketing', repo: 'dobius' },
      prs: { owner: 'fork', repo: 'dobius' }
    })
  ...
  describe('per-repo issue-source preference', () => {
    ...
    it("preference='auto' + no upstream → queries origin", async () => {
      ...
      getOwnerRepoMock.mockResolvedValueOnce({ owner: 'solo', repo: 'dobius' })
      ghExecFileAsyncMock.mockResolvedValueOnce({ stdout: '[]' }).mockResolvedValueOnce({
        stdout: '[]'
      })
      ...
      expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
        ...
        [
          ...
          '--cache',
          '120s',
          'repos/solo/dobius/issues?per_page=10&state=open&sort=updated&direction=desc'
        ],
        { cwd: '/repo-root' }
      )
    })
    ...
    it("preference='origin' + upstream exists → queries origin (not upstream)", async () => {
      ...
      expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
        1,
        expect.arrayContaining([
          'repos/fork/dobius/issues?per_page=10&state=open&sort=updated&direction=desc'
        ]),
        { cwd: '/repo-root' }
      )
    })
    ...
    it('surfaces upstreamCandidate in sources regardless of effective preference', async () => {
      ...
      expect(result.sources).toEqual({
        ...
        prs: { owner: 'fork', repo: 'dobius' },
        originCandidate: { owner: 'fork', repo: 'dobius' },
        upstreamCandidate: { owner: 'statusdigitalmarketing', repo: 'dobius' }
      })
    })
