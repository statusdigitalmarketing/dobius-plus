...
describe('issue source operations', () => {
  ...
  it('gets a single issue from the issue owner/repo', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['api', '--cache', '300s', 'repos/statusdigitalmarketing/dobius-plus/issues/923'],
      { cwd: '/repo-root' }
    )
  })
  ...
  it('routes local WSL issue operations through repo resolution and gh execution options', async () => {
    ...
    getIssueOwnerRepoMock.mockResolvedValue({ owner: 'statusdigitalmarketing', repo: 'dobius' })
    ...
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          number: 924,
          html_url: 'https://github.com/statusdigitalmarketing/dobius-plus/issues/924'
        })
      })
      .mockResolvedValueOnce({ stdout: '' })
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          html_url: 'https://github.com/statusdigitalmarketing/dobius-plus/issues/923#issuecomment-1'
        })
      })
  ...
  it('lists issues from the issue owner/repo', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      [
        ...
        '--cache',
        '120s',
        'repos/statusdigitalmarketing/dobius-plus/issues?per_page=5&state=open&sort=updated&direction=desc'
      ],
      { cwd: '/repo-root' }
    )
  })
  ...
  it('creates issues in the issue owner/repo', async () => {
    ...
    await expect(createIssue('/repo-root', 'New issue', 'Body')).resolves.toEqual({
      ok: true,
      number: 924,
      url: 'https://github.com/statusdigitalmarketing/dobius-plus/issues/924'
    })
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      [
        'api',
        '-X',
        'POST',
        'repos/statusdigitalmarketing/dobius-plus/issues',
        '--raw-field',
        'title=New issue',
        '--raw-field',
        'body=Body'
  ...
  it('creates issues with labels and assignees', async () => {
    ...
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        number: 925,
        html_url: 'https://github.com/statusdigitalmarketing/dobius-plus/issues/925'
      })
    })
    ...
    await expect(
      ...
      ok: true,
      number: 925,
      url: 'https://github.com/statusdigitalmarketing/dobius-plus/issues/925'
    })
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      [
        'api',
  ...
  it('updates issue body through the REST issue endpoint', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['api', '-X', 'PATCH', 'repos/statusdigitalmarketing/dobius-plus/issues/924', '--raw-field', 'body=Updated body'],
      { cwd: '/repo-root' }
    )
  })
  ...
  it('closes issues with completed, not planned, and duplicate reasons', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      1,
      ['issue', 'close', '924', '--repo', 'statusdigitalmarketing/dobius-plus', '--reason', 'completed'],
      { cwd: '/repo-root' }
    )
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
      ['issue', 'close', '925', '--repo', 'statusdigitalmarketing/dobius-plus', '--reason', 'not planned'],
      { cwd: '/repo-root' }
    )
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      3,
      ['issue', 'close', '926', '--repo', 'statusdigitalmarketing/dobius-plus', '--duplicate-of', '99'],
      { cwd: '/repo-root' }
    )
  })
  ...
  it('reopens issues through gh issue reopen', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['issue', 'reopen', '924', '--repo', 'statusdigitalmarketing/dobius-plus'],
      { cwd: '/repo-root' }
    )
  })
})
