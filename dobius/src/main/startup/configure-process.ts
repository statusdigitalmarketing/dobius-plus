...
const HTTP1_COMPATIBILITY_ENV_VAR = 'DOBIUS_DISABLE_HTTP2'
...
function readPersistedHttp1CompatibilityMode(userDataPath: string): boolean {
  const dataFile = join(userDataPath, 'dobius-data.json')
...
function requestDevParentShutdown(): void {
  ...
  const forceExitTimer = setTimeout(() => {
    ...
    // handlers or window-close guards stall and would otherwise leave Dobius
  ...
  if (process.platform !== 'win32') {
    ...
    if (home) {
      extraPaths.push(
        ...
        // user can run them from Terminal. See statusdigitalmarketing/dobius-plus#829.
...
export function configureDevUserDataPath(isDev: boolean): void {
  ...
  if (isDev) {
    const overrideUserDataPath = process.env.DOBIUS_DEV_USER_DATA_PATH
    if (overrideUserDataPath) {
      // Why: automated Electron repros need an isolated profile so persisted
      // tabs/worktrees from the developer's normal `dobius-dev` session do not
  ...
  // Why: development runs share the same machine as packaged Dobius, and both
  ...
  // public `dobius` CLI look broken even though the packaged app is still open.
  app.setPath('userData', join(app.getPath('appData'), 'Dobius+-dev'))
}

export function configureDobiusUserDataPathEnv(): void {
  // Why: app relaunches can inherit an DOBIUS_USER_DATA_PATH from an older CLI or
  // updater process. Main must canonicalize it before CLI-shared modules build
  // runtime-home paths, or migrations can bridge two Dobius app-data directories.
  process.env.DOBIUS_USER_DATA_PATH = app.getPath('userData')
}

export function shouldInstallManagedHooks(isDev: boolean): boolean {
  void isDev
  // Why: managed hook installation now targets Dobius-owned, environment-scoped
...
export function installDevParentWatchdog(isDev: boolean): void {
  ...
  const timer = setInterval(() => {
    ...
    if (parentPidChanged || parentMissing) {
      ...
      // the dev runner while leaving Dobius open. Watching the original parent PID
