...
const BASE_APP_NAME = 'Dobius+'
const BASE_APP_USER_MODEL_ID = 'com.statusdigitalmarketing.dobius-plus'
...
export function getDevInstanceIdentity(
  ...
  const repoRoot = cleanEnvValue(env.DOBIUS_DEV_REPO_ROOT)
  const branch = cleanEnvValue(env.DOBIUS_DEV_BRANCH)
  const worktreeName =
    cleanEnvValue(env.DOBIUS_DEV_WORKTREE_NAME) ??
    cleanEnvValue(path.basename(repoRoot ?? process.cwd()))
  const devLabel = cleanEnvValue(env.DOBIUS_DEV_INSTANCE_LABEL) ?? formatLabel(branch, worktreeName)
  const dockTitle =
    cleanEnvValue(env.DOBIUS_DEV_DOCK_TITLE) ?? `${BASE_APP_NAME}: ${branch ?? devLabel ?? 'dev'}`
