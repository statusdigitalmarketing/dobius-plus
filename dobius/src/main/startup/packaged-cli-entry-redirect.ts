...
const REDIRECT_ATTEMPT_ENV = 'DOBIUS_PACKAGED_CLI_ENTRY_REDIRECTED'

/**
 * Why: on Windows the bundled `dobius.cmd` runs `Dobius.exe <unpacked CLI entry>`
 * with ELECTRON_RUN_AS_NODE=1. When that env var is dropped (e.g. a wrapper or
 * shell that resets it), Dobius boots as a GUI, loses the single-instance lock to
 ...
 * CLI's status.
 *
 * Security: the spawned program is always `execPath` (Dobius.exe) and the script
...
export function maybeRedirectPackagedCliEntryLaunch(options: RedirectOptions = {}): RedirectResult {
  ...
  if (env[REDIRECT_ATTEMPT_ENV] === '1') {
    process.stderr.write('Unable to start the Dobius CLI through Electron node mode.\n')
    return { redirected: true, status: 1 }
  }
  if (!exists(cliEntryPath)) {
    process.stderr.write(`Unable to locate the Dobius CLI entrypoint at ${cliEntryPath}\n`)
    return { redirected: true, status: 1 }
  }
...
function buildElectronRunAsNodeEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const childEnv = { ...env }
  // Why: the CLI re-reads these from the DOBIUS_-prefixed copies; clearing the
  // originals keeps Electron's own node bootstrap from inheriting them.
  childEnv.DOBIUS_NODE_OPTIONS = env.NODE_OPTIONS ?? ''
  childEnv.DOBIUS_NODE_REPL_EXTERNAL_MODULE = env.NODE_REPL_EXTERNAL_MODULE ?? ''
