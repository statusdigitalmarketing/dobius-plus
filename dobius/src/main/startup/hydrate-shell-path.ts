...
// `which` probe even though they work fine from Terminal (see statusdigitalmarketing/dobius-plus#829).
//
...
const DELIMITER = '__DOBIUS_SHELL_PATH__'
...
function spawnShellAndReadPath(shell: string): Promise<HydrationResult> {
  return new Promise((resolve) => {
    ...
    const child = spawn(shell, ['-ilc', command], {
      ...
      // and we don't want that in Dobius's console.
      env: process.env,
