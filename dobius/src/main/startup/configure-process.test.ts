...
describe('patchPackagedProcessPath', () => {
  ...
  it('prepends agent-CLI install dirs (~/.opencode/bin, ~/.vite-plus/bin) for packaged darwin runs', async () => {
    ...
    // Without them on PATH, GUI-launched Dobius reports both as "Not installed"
...
describe('configureDevUserDataPath', () => {
  it('uses an explicit dev userData override when provided', async () => {
    ...
    const originalOverride = process.env.DOBIUS_DEV_USER_DATA_PATH
    process.env.DOBIUS_DEV_USER_DATA_PATH = '/tmp/dobius-dev-repro'

    try {
      configureDevUserDataPath(true)
    } finally {
      if (originalOverride === undefined) {
        delete process.env.DOBIUS_DEV_USER_DATA_PATH
      } else {
        process.env.DOBIUS_DEV_USER_DATA_PATH = originalOverride
      }
    }

    expect(app.setPath).toHaveBeenCalledWith('userData', '/tmp/dobius-dev-repro')
  })

  it('moves dev runs onto a Dobius+-dev userData path', async () => {
    const { app } = await import('electron')
    const { configureDevUserDataPath } = await import('./configure-process')
    ...
    // Why: production code uses path.join(app.getPath('appData'), 'Dobius+-dev')
    // which produces platform-specific separators.
    expect(app.setPath).toHaveBeenCalledWith('userData', join('/tmp/app-data', 'Dobius+-dev'))
  })
...
describe('configureDobiusUserDataPathEnv', () => {
  it('overwrites stale inherited DOBIUS_USER_DATA_PATH with Electron userData', async () => {
    const { app } = await import('electron')
    const { configureDobiusUserDataPathEnv } = await import('./configure-process')
    const originalUserDataPath = process.env.DOBIUS_USER_DATA_PATH
    process.env.DOBIUS_USER_DATA_PATH = '/tmp/stale-dobius-user-data'
    app.setPath('userData', '/tmp/current-dobius-user-data')
    let configuredUserDataPath: string | undefined

    try {
      configureDobiusUserDataPathEnv()
      configuredUserDataPath = process.env.DOBIUS_USER_DATA_PATH
    } finally {
    ...
    expect(configuredUserDataPath).toBe('/tmp/current-dobius-user-data')
  })
})
...
describe('configureElectronNetworkCompatibility', () => {
  ...
  const originalEnvValue = process.env.DOBIUS_DISABLE_HTTP2

  function createUserDataDir(settings: Record<string, unknown>): string {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dobius-http1-compat-'))
    tempDirs.push(userDataPath)
    writeFileSync(join(userDataPath, 'dobius-data.json'), JSON.stringify({ settings }), 'utf-8')
    return userDataPath
  }
  ...
  afterEach(() => {
    ...
    if (originalEnvValue === undefined) {
      delete process.env.DOBIUS_DISABLE_HTTP2
    } else {
      process.env.DOBIUS_DISABLE_HTTP2 = originalEnvValue
    }
  })
...
describe('enableMainProcessGpuFeatures', () => {
  const originalPlatform = Object.getOwnPropertyDescriptor(process, 'platform')
  const originalE2EUserDataDir = process.env.DOBIUS_E2E_USER_DATA_DIR
  ...
  afterEach(() => {
    ...
    if (originalE2EUserDataDir === undefined) {
      delete process.env.DOBIUS_E2E_USER_DATA_DIR
    } else {
      process.env.DOBIUS_E2E_USER_DATA_DIR = originalE2EUserDataDir
    }
  })
  ...
  it('disables the GPU process for Linux E2E runs', async () => {
    ...
    setPlatform('linux')
    process.env.DOBIUS_E2E_USER_DATA_DIR = '/tmp/dobius-e2e'
    vi.mocked(app.disableHardwareAcceleration).mockClear()
    vi.mocked(app.commandLine.appendSwitch).mockClear()
