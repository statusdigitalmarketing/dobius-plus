...
const resourcesPath = 'C:\\Users\\me\\AppData\\Local\\Programs\\Dobius\\resources'
const execPath = 'C:\\Users\\me\\AppData\\Local\\Programs\\Dobius\\Dobius.exe'
...
describe('packaged CLI entry redirect', () => {
  ...
  it('spawns the in-package CLI in Electron node mode before the single-instance lock can win', () => {
    ...
    expect(spawn).toHaveBeenCalledWith(execPath, [cliEntryPath, 'status', '--json'], {
      env: expect.objectContaining({
        ...
        DOBIUS_PACKAGED_CLI_ENTRY_REDIRECTED: '1',
        DOBIUS_NODE_OPTIONS: '--inspect',
        DOBIUS_NODE_REPL_EXTERNAL_MODULE: 'external-loader'
      }),
      stdio: 'inherit'
    })
  ...
  it('does not redirect development launches', () => {
    ...
    const result = maybeRedirectPackagedCliEntryLaunch({
      argv: ['C:\\dev\\Dobius.exe', cliEntryPath, 'status'],
      platform: 'win32',
      isPackaged: false,
      resourcesPath,
      execPath: 'C:\\dev\\Dobius.exe',
      exists: () => true,
      spawn: spawn as never
    })
  ...
  it('reports a clear failure instead of locating a missing entrypoint', () => {
    ...
    try {
      ...
      expect(stderrWrite).toHaveBeenCalledWith(
        `Unable to locate the Dobius CLI entrypoint at ${cliEntryPath}\n`
      )
      expect(spawn).not.toHaveBeenCalled()
    } finally {
  ...
  it('fails clearly instead of recursively redirecting when node mode already failed once', () => {
    ...
    try {
      ...
      expect(result).toEqual({ redirected: true, status: 1 })
      expect(stderrWrite).toHaveBeenCalledWith(
        'Unable to start the Dobius CLI through Electron node mode.\n'
      )
      expect(spawn).not.toHaveBeenCalled()
    } finally {
      stderrWrite.mockRestore()
