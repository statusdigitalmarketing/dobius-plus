...
describe('run-electron-vite-dev', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), 'dobius-dev-wrapper-'))
      ...
      const wrapper = spawn(process.execPath, [wrapperPath], {
        ...
        env: devWrapperTestEnv({
          DOBIUS_ELECTRON_VITE_CLI: fakeCliPath,
          DOBIUS_SKIP_DEV_CLI_PREPARE: '1',
          DOBIUS_SKIP_DEV_ELECTRON_APP_PREPARE: '1',
          DOBIUS_SKIP_DEV_WEB_PREPARE: '1',
          DOBIUS_DEV_WRAPPER_TEST_PID_FILE: pidFile
        }),
        stdio: 'ignore'
      })
  ...
  it('forwards dev instance identity to electron-vite', async () => {
    ...
    const wrapper = spawn(process.execPath, [wrapperPath, '--remote-debugging-port=9444'], {
      ...
      env: devWrapperTestEnv({
        ...
        DOBIUS_SKIP_DEV_WEB_PREPARE: '1',
        DOBIUS_DEV_WRAPPER_TEST_PID_FILE: pidFile,
        DOBIUS_DEV_WRAPPER_TEST_ENV_FILE: envFile,
    ...
    expect(envSnapshot.dockTitle).toBe('Dobius: feature/billing-shell')
  ...
  it.skipIf(process.platform === 'win32')(
    'prepares userData dobius and dobius-dev wrappers for dev terminals',
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), 'dobius-dev-wrapper-'))
      ...
      const wrapper = spawn(process.execPath, [wrapperPath], {
        cwd: resolve('.'),
        env: devWrapperTestEnv({
          DOBIUS_DEV_USER_DATA_PATH: userDataPath,
          DOBIUS_ELECTRON_VITE_CLI: fakeCliPath,
          DOBIUS_SKIP_DEV_ELECTRON_APP_PREPARE: '1',
          DOBIUS_SKIP_DEV_WEB_PREPARE: '1',
          DOBIUS_DEV_WRAPPER_TEST_PID_FILE: pidFile,
      ...
      const trackedPids = trackPidFile(pidFile)
      const devWrapper = readFileSync(join(userDataPath, 'cli', 'bin', 'dobius-dev'), 'utf8')
      const publicAliasWrapper = readFileSync(join(userDataPath, 'cli', 'bin', 'dobius'), 'utf8')
      expect(publicAliasWrapper).toBe(devWrapper)
      expect(publicAliasWrapper).toContain('DOBIUS_USER_DATA_PATH')
    ...
    const wrapper = spawn(
      ...
      [wrapperPath, '--stable-name', '--remote-debugging-port=9445'],
      {
        ...
        env: devWrapperTestEnv({
          ...
          DOBIUS_DEV_WRAPPER_TEST_PID_FILE: pidFile,
          DOBIUS_DEV_WRAPPER_TEST_ENV_FILE: envFile,
          DOBIUS_DEV_BRANCH: 'feature/stable-name',
          DOBIUS_DEV_WORKTREE_NAME: 'stable-ui'
        }),
        stdio: 'ignore'
      }
    )
  ...
  it.skipIf(process.platform !== 'darwin')(
    ...
    async () => {
      ...
      const baseEnv = devWrapperTestEnv({
        ...
        DOBIUS_SKIP_DEV_CLI_PREPARE: '1',
        DOBIUS_SKIP_DEV_WEB_PREPARE: '1',
        DOBIUS_DEV_BRANCH: 'feature/rebuild-electron-app',
        DOBIUS_DEV_WORKTREE_NAME: 'electron-app-rebuild'
      })
    ...
    async () => {
      ...
      const wrapper = spawn(process.execPath, [wrapperPath, '--remote-debugging-port=9448'], {
        ...
        env: devWrapperTestEnv({
          ...
          DOBIUS_DEV_WRAPPER_TEST_PID_FILE: pidFile,
          DOBIUS_DEV_WRAPPER_TEST_ENV_FILE: envFile,
          DOBIUS_DEV_BRANCH: 'feature/framework-symlinks',
          DOBIUS_DEV_WORKTREE_NAME: 'symlink-ui'
        }),
        stdio: 'ignore'
      })
