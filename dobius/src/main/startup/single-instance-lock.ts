...
export const SINGLE_INSTANCE_LOCK_FAILURE_MESSAGE =
  '[single-instance] Another Dobius instance is already running for this userData profile; exiting this launch after requesting the existing window. If no Dobius process is running, this may be an Electron/macOS single-instance lock failure.'
export const SINGLE_INSTANCE_LOCK_BYPASS_ENV = 'DOBIUS_BYPASS_SINGLE_INSTANCE_LOCK'
export const SINGLE_INSTANCE_LOCK_BYPASS_MESSAGE =
  '[single-instance] DOBIUS_BYPASS_SINGLE_INSTANCE_LOCK=1 is set; bypassing the packaged macOS single-instance lock for diagnostics. Do not use this with another Dobius instance running for the same profile.'

/**
 * Why: Dobius writes two canonical discovery files into `<userData>/`:
 * `dobius-runtime.json` (RPC endpoint + authToken for the bundled CLI) and
 ...
 * instance quits, metadata points at a dead pid and `dobius status` reports
 ...
 * way dev (`dobius-dev` userData) and packaged (`dobius` userData) runs lock in
