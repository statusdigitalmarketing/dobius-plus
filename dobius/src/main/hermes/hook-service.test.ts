...
describe('HermesHookService', () => {
  ...
  beforeEach(() => {
    ...
    homeDir = mkdtempSync(join(tmpdir(), 'dobius-hermes-hooks-'))
  ...
  it('preserves other enabled plugins and removes Dobius from disabled list', () => {
    writeFileSync(
  ...
  it('registered plugin hooks post normalized JSON to Dobius', async () => {
    ...
    const received = new Promise<Record<string, unknown>>((resolve, reject) => {
      const server = createServer((req, res) => {
        ...
        req.on('end', () => {
          try {
            ...
            expect(req.headers['x-dobius-agent-hook-token']).toBe('token-1')
            res.writeHead(204)
            res.end()
      ...
      server.listen(0, '127.0.0.1', () => {
        ...
        const script = [
          ...
          `spec = importlib.util.spec_from_file_location("dobius_status", ${JSON.stringify(initPath)})`,
        ...
        execFile(
          ...
          ['-c', script],
          {
            env: {
              ...
              DOBIUS_AGENT_HOOK_TOKEN: 'token-1',
              DOBIUS_AGENT_HOOK_ENDPOINT: '',
              DOBIUS_PANE_KEY: PANE_KEY,
              DOBIUS_TAB_ID: 'tab-1',
