...
const HERMES_PLUGIN_NAME = 'dobius-status'
const HERMES_PLUGIN_MARKER = 'Managed by Dobius. Do not edit; changes may be overwritten.'
...
function enablePlugin(config: HermesConfig): HermesConfig {
  ...
  if (disabled === null) {
    ...
    // so Dobius's install status matches what the real Hermes loader will do.
    plugins.disabled = []
...
function getPluginFilesState(pluginDir = getPluginDir()): {
  ...
  try {
    ...
    return {
      present: true,
      managed,
      detail: managed ? null : 'Hermes dobius-status plugin exists but is not Dobius-managed'
    }
  } catch (error) {
    return {
      present: true,
...
function buildStatus(configPath: string, config: HermesConfig): AgentHookInstallStatus {
  ...
  const details = [
    pluginFiles.detail,
    enablement.detail,
    !enablement.enabled ? 'dobius-status is not enabled in Hermes config.yaml' : null,
    enablement.disabled ? 'dobius-status is disabled in Hermes config.yaml' : null
...
function getPluginManifest(): string {
  return [
    ...
    'version: 1.0.0',
    'description: "Reports Hermes Agent lifecycle events to Dobius."',
    'author: "Dobius"',
    'kind: standalone',
    'provides_hooks:',
...
def _endpoint_env() -> dict[str, str]:
    env = dict(os.environ)
    endpoint = env.get("DOBIUS_AGENT_HOOK_ENDPOINT", "")
...
def _post_to_dobius(payload: dict[str, Any]) -> None:
    env = _endpoint_env()
    port = env.get("DOBIUS_AGENT_HOOK_PORT", "")
    token = env.get("DOBIUS_AGENT_HOOK_TOKEN", "")
    pane_key = env.get("DOBIUS_PANE_KEY", "")
    if not port or not token or not pane_key:
        return
    body = {
        "paneKey": pane_key,
        "launchToken": env.get("DOBIUS_AGENT_LAUNCH_TOKEN", ""),
        "tabId": env.get("DOBIUS_TAB_ID", ""),
        "worktreeId": env.get("DOBIUS_WORKTREE_ID", ""),
        "env": env.get("DOBIUS_AGENT_HOOK_ENV", ""),
        "version": env.get("DOBIUS_AGENT_HOOK_VERSION", ""),
        "payload": payload,
    }
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "X-Dobius-Agent-Hook-Token": token,
        },
    )
    try:
...
def _make_hook(event_name: str) -> Callable[..., None]:
    def _hook(**kwargs: Any) -> None:
        _post_to_dobius(_payload_for_event(event_name, kwargs))
