...
function isGuestCancellationPayload(rawPayload: unknown): boolean {
  ...
  if (payload.__dobiusCancelled === true) {
    return true
  }
...
export class BrowserGrabSessionController {
  ...
  awaitGrabSelection(
    ...
    return new Promise<BrowserGrabResult>((resolve) => {
      ...
      // __dobiusGrabResolve() which is wired by the 'awaitClick' script to
      ...
      const awaitGuestClick = async (): Promise<void> => {
        try {
          ...
          // Why: the guest wraps right-click results in { __dobiusContextMenu, payload }
          // so the renderer can show the full action dropdown instead of auto-copying.
          const isContextMenu =
            '__dobiusContextMenu' in (rawPayload as Record<string, unknown>) &&
            (rawPayload as Record<string, unknown>).__dobiusContextMenu === true
