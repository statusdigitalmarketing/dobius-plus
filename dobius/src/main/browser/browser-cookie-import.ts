...
function getDiagLogPath(): string {
  if (!_diagLog) {
    try {
      ...
      _diagLog = join(tmpdir(), 'dobius-cookie-import-diag.log')
    }
  }
  return _diagLog
}
...
async function importCookiesFromFirefox(
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'dobius-cookie-import-'))
  ...
  try {
    ...
    if (isPermError) {
      return {
        ok: false,
        reason:
          'macOS denied access to Safari cookies. Grant Full Disk Access to Dobius in System Settings → Privacy & Security → Full Disk Access.'
      }
    }
...
export async function importCookiesFromBrowser(
  ...
  try {
    ...
    // cookies prevents stale cookies from a previous Dobius browsing session from
