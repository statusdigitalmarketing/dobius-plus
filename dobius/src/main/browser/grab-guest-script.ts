...
// itself on teardown.
//
// Why a string builder rather than a bundled file: Dobius's browser guests have
...
// It stores state on window.__dobiusGrab so finalize/teardown can access it.
// ---------------------------------------------------------------------------
const ARM_SCRIPT = `(function() {
  'use strict';

  // Why: always tear down any pre-existing state before arming. A malicious
  // guest page could predefine window.__dobiusGrab with a fake extractPayload
  // function. By tearing down unconditionally we ensure our freshly installed
  // extraction logic is the only code that runs.
  if (window.__dobiusGrab) {
    try {
      if (typeof window.__dobiusGrab.cleanup === 'function') {
        window.__dobiusGrab.cleanup();
      }
    } catch(e) {}
    delete window.__dobiusGrab;
  }
...
const AWAIT_CLICK_SCRIPT = `new Promise(function(resolve, reject) {
  ...
  function onContext(e) {
    ...
    if (!payload) return;
    grab.freezeHighlight();
    resolve({ __dobiusContextMenu: true, payload: payload });
  }
  ...
  grab.cancelAwait = function() {
    ...
    resolve({ __dobiusCancelled: true });
  };
})`
