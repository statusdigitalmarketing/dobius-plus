...
export class AgentBrowserBridge {
  ...
  async onTabClosed(webContentsId: number): Promise<void> {
    ...
    if (browserPageId) {
      const sessionName = `dobius-tab-${browserPageId}`
  ...
  async wait(
    ...
    return this.enqueueTargetedCommand(worktreeId, browserPageId, async (sessionName) => {
      ...
      // or a documented per-command `--timeout`. Dobius normalizes "visible" back
  ...
  async setViewport(
    ...
    return this.enqueueTargetedCommand(worktreeId, browserPageId, async (_sessionName, target) => {
      ...
      // it has no `mobile` flag. Dobius's CLI exposes `--mobile`, so apply the
  ...
  async exec(command: string, worktreeId?: string, browserPageId?: string): Promise<unknown> {
    return this.enqueueTargetedCommand(worktreeId, browserPageId, async (sessionName) => {
      ...
      // caller cannot override Dobius's selected browser page or CDP proxy.
  ...
  private async enqueueTargetedCommand<T>(
    ...
    const sessionName = `dobius-tab-${target.browserPageId}`
  ...
  private async ensureSession(
    ...
    const createSession = async (): Promise<void> => {
      ...
      // across Dobius restarts. A stale session ignores --cdp (already initialized) and
