...
const HIDE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  var g = window.__dobiusGrab;
  if (g && g.host) g.host.style.display = 'none';
  document.querySelectorAll('[data-dobius-browser-annotation-overlay]').forEach(function(el) {
    el.setAttribute('data-dobius-previous-display', el.style.display || '');
...
const RESTORE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  ...
  document.querySelectorAll('[data-dobius-browser-annotation-overlay]').forEach(function(el) {
    el.style.display = el.getAttribute('data-dobius-previous-display') || '';
    el.removeAttribute('data-dobius-previous-display');
  });
})()`
