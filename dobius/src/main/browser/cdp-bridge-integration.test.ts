...
describe('Browser automation pipeline (integration)', () => {
  let server: DobiusRuntimeRpcServer
  let endpoint: string
  ...
  beforeEach(async () => {
    ...
    server = new DobiusRuntimeRpcServer({ runtime, userDataPath })
    await server.start()
  ...
  it('returns browser_no_tab when no tabs are registered', async () => {
    ...
    const runtime2 = new DobiusRuntimeService()
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    runtime2.setAgentBrowserBridge(emptyBridge as any)

    const server2 = new DobiusRuntimeRpcServer({ runtime: runtime2, userDataPath: userDataPath2 })
    await server2.start()
