...
describe('BrowserSessionRegistry', () => {
  ...
  it('has a default profile on construction', () => {
    ...
    expect(defaultProfile.partition).toBe(DOBIUS_BROWSER_PARTITION)
  })

  it('allows the default partition', () => {
    expect(browserSessionRegistry.isAllowedPartition(DOBIUS_BROWSER_PARTITION)).toBe(true)
  })
  ...
  it('creates an isolated profile with a unique partition', () => {
    ...
    expect(profile!.partition).toMatch(/^persist:dobius-browser-session-/)
    expect(profile!.partition).not.toBe(DOBIUS_BROWSER_PARTITION)
  ...
  it('resolves default partition for null/undefined profileId', () => {
    expect(browserSessionRegistry.resolvePartition(null)).toBe(DOBIUS_BROWSER_PARTITION)
    expect(browserSessionRegistry.resolvePartition(undefined)).toBe(DOBIUS_BROWSER_PARTITION)
  })

  it('resolves default partition for unknown profileId', () => {
    expect(browserSessionRegistry.resolvePartition('nonexistent')).toBe(DOBIUS_BROWSER_PARTITION)
  })
  ...
  it('strictly resolves known profile partitions without downgrading unknown profiles', () => {
    ...
    expect(browserSessionRegistry.resolveKnownPartition(null)).toBe(DOBIUS_BROWSER_PARTITION)
    expect(browserSessionRegistry.resolveKnownPartition(undefined)).toBe(DOBIUS_BROWSER_PARTITION)
    expect(browserSessionRegistry.resolveKnownPartition('default')).toBe(DOBIUS_BROWSER_PARTITION)
  ...
  it('hydrates profiles from persisted data', () => {
    const fakeProfile = {
      ...
      partition: 'persist:dobius-browser-session-00000000-0000-0000-0000-000000000001',
      label: 'Hydrated',
  ...
  it('routes media permission requests through macOS TCC for isolated partitions', async () => {
    ...
    // after the user grants Camera/Microphone to Dobius.
