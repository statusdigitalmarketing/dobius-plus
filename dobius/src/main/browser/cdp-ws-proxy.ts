...
export class CdpWsProxy {
  ...
  private handleHttpRequest(req: IncomingMessage, res: ServerResponse): void {
    ...
    if (url === '/json/version' || url === '/json/version/') {
      ...
      // "Dobius/CdpWsProxy" leaks that this is an embedded automation surface, which
  ...
  private handleClientMessage(client: WebSocket, raw: string): void {
    ...
    if (msg.method === 'Target.attachToTarget') {
      this.clientSessionId = 'dobius-proxy-session'
      this.sendResult(clientId, { sessionId: this.clientSessionId }, client)
      return
    }
    if (msg.method === 'Browser.getVersion') {
      // Why: returning "Dobius/Electron" identifies this as an embedded automation
    ...
    // Use the same direct webContents reload path as Dobius's own browser.reload.
  ...
  private async reloadWithLifecycle(
    ...
    if (unsupportedParam) {
      this.sendError(
        clientId,
        `Page.reload parameter "${unsupportedParam}" is not supported for Dobius tab reloads`,
        client
      )
      return
    }
