...
// Why: headless dobius serve has no renderer window to host a <webview>, so each
...
export class OffscreenBrowserBackend implements BrowserBackend {
  ...
  async createTab(params: BrowserBackendCreateTab): Promise<{ browserPageId: string }> {
    ...
    const partition = profile?.partition ?? DOBIUS_BROWSER_PARTITION
    ...
    const win = new BrowserWindow({
      ...
      webPreferences: {
        ...
        ...DOBIUS_BROWSER_GUEST_WEB_PREFERENCES,
        partition,
        sandbox: true,
