...
function releaseAutomationVisibilityToken(renderer: Electron.WebContents, token: string): void {
  ...
  renderer
    .executeJavaScript(
      `(function() {
        var bridge = window.__dobiusBrowserAutomationVisibility;
...
export class BrowserManager {
  ...
  // Why: screenshot sessions target guest page ids, but Dobius's visible browser
        ...
        .executeJavaScript(
          `(function() {
            ...
            if (
              ...
              // Why: Dobius remembers the last browser workspace/page even when
  ...
  async acquireAutomationVisibility(guestWebContentsId: number): Promise<() => void> {
    ...
    // sites, but must not steal the user's visible Dobius tab/worktree.
  ...
  attachGuestPolicies(guest: Electron.WebContents): void {
    ...
    // Dobius window is not the focused foreground app. With throttling enabled,
    ...
    guest.setWindowOpenHandler(({ url }) => {
      ...
      // flows, but Dobius still does not host child windows itself. For normal
      // web URLs, route the request into Dobius's own browser-tab model first so
      // the user stays in the IDE. Only fall back to the system browser when
      // Dobius cannot safely host the destination or when the guest is not yet
      // associated with a trusted browser tab/renderer.
      if (browserTabId && browserUrl && this.openLinkInDobiusTab(browserTabId, browserUrl)) {
        this.forwardOrQueuePopupEvent(guest.id, {
          origin: safeOrigin(browserUrl),
          action: 'opened-in-dobius'
        })
      } else if (externalUrl) {
  ...
  // Why: headless dobius serve has no renderer window to mount a <webview>, so its
  ...
  unregisterAll(): void {
    ...
    for (const downloadId of this.downloadsById.keys()) {
      this.cancelDownloadInternal(downloadId, 'Dobius is shutting down.')
    }
  ...
  handleGuestWillDownload(args: { guestWebContentsId: number; item: Electron.DownloadItem }): void {
    ...
    try {
      ...
      try {
        ...
        // state; cancellation is best-effort after Dobius has made the UI terminal.
      }
      return
    }
  ...
  // Why: guest browser surfaces are intentionally isolated from Dobius's preload
  ...
  private cancelDownloadInternal(downloadId: string, reason: string): void {
    ...
    try {
      ...
      // source of truth for whether Dobius still considers the request active.
    }
  ...
  private openLinkInDobiusTab(browserTabId: string, rawUrl: string): boolean {
    const renderer = this.resolveRendererForBrowserTab(browserTabId)
    if (!renderer) {
      return false
    }
    ...
    // URL back to that renderer so it can open a sibling Dobius browser tab in
