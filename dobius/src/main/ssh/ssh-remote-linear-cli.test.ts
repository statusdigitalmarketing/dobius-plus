...
describe('runRemoteDobiusCli Linear commands', () => {
  ...
  it('dispatches Linear discovery and list reads through the remote runtime', async () => {
    ...
    const teamList = await runRemoteDobiusCli(runtime, {
      argv: ['linear', 'team', 'list', '--workspace', 'all', '--json'],
      cwd: '/home/alice/remote-repo',
      env: { DOBIUS_TERMINAL_HANDLE: 'term_ssh' }
    })
    const labels = await runRemoteDobiusCli(runtime, {
      argv: ['linear', 'team', 'labels', '--team', 'ENG', '--workspace', 'workspace-1', '--json'],
      cwd: '/home/alice/remote-repo',
      env: { DOBIUS_TERMINAL_HANDLE: 'term_ssh' }
    })
    const list = await runRemoteDobiusCli(runtime, {
      argv: [
        'linear',
        'list',
        '--filter',
    ...
    const projects = await runRemoteDobiusCli(runtime, {
      argv: [
        'linear',
        'project',
        'list',
  ...
  it('prints SSH Linear non-json next steps from structured errors', async () => {
    ...
    linearIssueAddComment.mockRejectedValueOnce(
      Object.assign(new Error('Linear may have applied the write.'), {
        ...
        data: { nextSteps: ['Retry once with the pinned command: `dobius linear comment add`.'] }
      })
    )
  ...
  it('shows SSH Linear command help without dispatching to the runtime', async () => {
    ...
    expect(result.exitCode).toBe(0)
    expect(result.stdout).toContain('dobius linear issue')
    expect(result.stdout).toContain('Usage: dobius linear issue')
  ...
  it('shows SSH Linear group help without dispatching to the runtime', async () => {
    ...
    expect(result.exitCode).toBe(0)
    expect(result.stdout).toContain('dobius linear')
    expect(result.stdout).toContain('Usage: dobius linear <command> [options]')
  ...
  it('shows SSH Linear help through the local help command form', async () => {
    ...
    const group = await runRemoteDobiusCli(runtime, {
      argv: ['help', 'linear'],
      cwd: '/home/alice/remote-repo',
      env: { DOBIUS_TERMINAL_HANDLE: 'term_ssh' }
    })
    const issue = await runRemoteDobiusCli(runtime, {
    ...
    expect(group.stdout).toContain('Usage: dobius linear <command> [options]')
    expect(issue.exitCode).toBe(0)
    expect(issue.stdout).toContain('Usage: dobius linear issue')
    expect(linearIssueContext).not.toHaveBeenCalled()
  })
