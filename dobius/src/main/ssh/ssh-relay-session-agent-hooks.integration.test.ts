...
import {
  ...
  DOBIUS_FEATURE_REMOTE_AGENT_HOOKS_ENV,
...
function createFakeRelay(): FakeRelay {
  ...
  dispatcher.onRequest('session.resolveHome', async (params) => ({
    resolvedPath: params.path === '~' ? '/home/dobius' : params.path
  }))
  ...
  dispatcher.onRequest(AGENT_HOOK_REQUEST_REPLAY_METHOD, async () => {
    // Why: relay replay must arrive after Dobius wires its listener and before
...
describe('SshRelaySession agent hooks over a fake relay transport', () => {
  ...
  beforeEach(() => {
    ...
    previousRemoteHooksFlag = process.env[DOBIUS_FEATURE_REMOTE_AGENT_HOOKS_ENV]
    process.env[DOBIUS_FEATURE_REMOTE_AGENT_HOOKS_ENV] = '1'
  ...
  afterEach(() => {
    ...
    if (previousRemoteHooksFlag === undefined) {
      delete process.env[DOBIUS_FEATURE_REMOTE_AGENT_HOOKS_ENV]
    } else {
      process.env[DOBIUS_FEATURE_REMOTE_AGENT_HOOKS_ENV] = previousRemoteHooksFlag
    }
  })
  ...
  it('establishes through a fake relay, spawns a remote PTY, and forwards agent status', async () => {
    ...
    const spawn = await provider!.spawn({
      cols: 120,
      rows: 40,
      cwd: '/home/dobius/project',
      env: {
        DOBIUS_PANE_KEY: `tab-ssh:${SSH_LEAF_ID}`,
        DOBIUS_TAB_ID: 'tab-ssh',
        DOBIUS_WORKTREE_ID: 'wt-ssh'
      }
    })
  ...
  it('drops malformed remote hook notifications at Dobius main before caching', async () => {
    relay = createFakeRelay()
    vi.mocked(deployAndLaunchRelay).mockResolvedValue({
      transport: relay.transport,
      platform: 'linux-x64'
