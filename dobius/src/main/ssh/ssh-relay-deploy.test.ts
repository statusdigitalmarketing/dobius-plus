...
vi.mock('./relay-protocol', () => ({
  RELAY_VERSION: '0.1.0',
  RELAY_REMOTE_DIR: '.dobius-remote',
  ...
  RELAY_SENTINEL: 'DOBIUS-RELAY v0.1.0 READY\n',
...
vi.mock('./ssh-relay-versioned-install', () => ({
  ...
  computeRemoteRelayDir: (home: string, v: string) => `${home}/.dobius-remote/relay-${v}`,
...
describe('deployAndLaunchRelay', () => {
  ...
  it('calls exec to detect remote platform', async () => {
    ...
    mockExecCommand.mockResolvedValueOnce('DOBIUS-NATIVE-DEPS-OK') // native deps probe
  ...
  it('resolves the remote node path once per deploy', async () => {
    ...
    mockExecCommand.mockResolvedValueOnce('Linux x86_64')
    mockExecCommand.mockResolvedValueOnce('/home/user')
    mockExecCommand.mockResolvedValueOnce('DOBIUS-NATIVE-DEPS-OK')
  ...
  it('uses a content-hashed versioned remote install directory', async () => {
    ...
    const sawVersionedDir = allCmds.some((cmd) =>
      cmd.includes('/.dobius-remote/relay-0.1.0+abcdef012345')
    )
  ...
  it('uses distinct target-specific relay socket paths', async () => {
    ...
    mockExecCommand
      ...
      .mockResolvedValueOnce('DOBIUS-NATIVE-DEPS-OK') // native deps probe A
      ...
      .mockResolvedValueOnce('DOBIUS-NATIVE-DEPS-OK') // native deps probe B
  ...
  it('launches Windows remotes via a named pipe endpoint', async () => {
    ...
    mockExecCommand
      ...
      .mockResolvedValueOnce('DOBIUS-NATIVE-DEPS-OK') // native deps probe
    ...
    expect(result.sockPath).toMatch(/^\\\\\.\\pipe\\dobius-relay-[0-9a-f]{20}$/)
    ...
    expect(launchScript).toContain(
      '"C:/Users/me user/.dobius-remote/relay-0.1.0+abcdef012345/relay.js"'
    )
    expect(launchScript).toContain(
      '"C:/Users/me user/.dobius-remote/relay-0.1.0+abcdef012345/agent-hooks/dobius-relay-'
    )
  ...
  it('relaunches Windows remotes on a fallback pipe when reconnecting the occupied pipe fails', async () => {
    ...
    expect(primaryPipe).toMatch(/^\\\\\.\\pipe\\dobius-relay-[0-9a-f]{20}$/)
    expect(fallbackPipe).toMatch(/^\\\\\.\\pipe\\dobius-relay-[0-9a-f]{20}$/)
  ...
  it('prefers a persisted Windows fallback pipe on later reconnects', async () => {
    const conn = makeMockConnection()
    const mockExecCommand = vi.mocked(execCommand)
    const persistedPipe = '\\\\.\\pipe\\dobius-relay-1234567890abcdef1234'
    vi.mocked(resolveRemoteNodePath).mockResolvedValue('C:/Program Files/nodejs/node.exe')
    mockExecCommand
      .mockRejectedValueOnce(new Error('uname not found')) // uname -sm
      .mockResolvedValueOnce('Windows X64') // PowerShell platform probe
  ...
  it('scopes persisted Windows active pipe markers by relay target', async () => {
    ...
    mockExecCommand
      ...
      .mockResolvedValueOnce('DOBIUS-NATIVE-DEPS-OK')
