...
import { runRemoteDobiusCli } from './ssh-remote-dobius-cli'
...
export class SshRelaySession {
  ...
  constructor(
    ...
    private runtime?: DobiusRuntimeService,
    ...
    try {
      ...
      this.remoteCliBridgeEnv =
        remoteHome && remoteRelayDir && nodePath && sockPath && hostPlatform
          ? {
              remoteHome,
              binDir: joinRemotePath(hostPlatform, remoteHome, '.dobius-relay', 'bin'),
  ...
  detach(): void {
    ...
    // owns the grace timer, so Dobius must unregister local providers without
  ...
  private async registerProviders(
    ...
    try {
      await this.installRemoteDobiusCliShim()
    } catch (error) {
      // Why: the remote `dobius` CLI shim is a convenience bridge. On session-
      ...
      console.warn(
        `[ssh-relay-session] remote dobius CLI shim install failed for ${this.targetId}: ${
    ...
    this.wireUpRemoteDobiusCli(mux)
  ...
  // Why: the relay can inject DOBIUS_AGENT_HOOK_* env into SSH PTYs, but
  // hook-script agents (Claude/Codex/Gemini/etc.) still need their config
  // files on the remote host to call Dobius's managed script. Install those
  ...
  private async installRemoteDobiusCliShim(): Promise<void> {
  ...
  private wireUpRemoteDobiusCli(mux: SshChannelMultiplexer): void {
    mux.onRequest('dobius.cli', async (params) => {
      if (!this.runtime) {
        throw new Error('Dobius runtime is unavailable')
      }
      ...
      return await runRemoteDobiusCli(this.runtime, {
        argv,
        cwd,
        env,
  ...
  // would force a relay redeploy on every Dobius update. See
  ...
  // Why: route the relay's `agent.hook` JSON-RPC notification into Dobius's
  // shared `agentHookServer` via `ingestRemote`. The wire envelope carries
  // `connectionId: null` (the relay does not know Dobius's local handle); we
  ...
  //
  // The Dobius-side mux's `notificationHandlers` is a flat array — each
  ...
  private wireUpAgentHookEvents(mux: SshChannelMultiplexer): void {
    ...
    this.muxNotificationCleanup = mux.onNotification((method, params) => {
      ...
      // Why: forward env/version verbatim so Dobius's warn-once cross-build /
...
function buildRemoteCliShim(env: RemoteCliBridgeEnv): {
  ...
  if (isWindowsRemoteHost(env.hostPlatform)) {
    const shimPath = joinRemotePath(env.hostPlatform, env.binDir, 'dobius.cmd')
    return {
      path: shimPath,
      contents: [
        '@echo off',
        'setlocal',
        `if not defined DOBIUS_RELAY_NODE_PATH set "DOBIUS_RELAY_NODE_PATH=${env.nodePath}"`,
        `if not defined DOBIUS_RELAY_DIR set "DOBIUS_RELAY_DIR=${env.relayDir}"`,
        `if not defined DOBIUS_RELAY_SOCKET_PATH set "DOBIUS_RELAY_SOCKET_PATH=${env.sockPath}"`,
        '"%DOBIUS_RELAY_NODE_PATH%" "%DOBIUS_RELAY_DIR%/relay.js" --sock-path "%DOBIUS_RELAY_SOCKET_PATH%" --dobius-cli %*',
        'exit /b %ERRORLEVEL%',
        ''
      ].join('\r\n')
    }
  ...
  const shimPath = joinRemotePath(env.hostPlatform, env.binDir, 'dobius')
  return {
    path: shimPath,
    contents: [
      '#!/usr/bin/env sh',
      'set -eu',
      `DOBIUS_RELAY_NODE_PATH=\${DOBIUS_RELAY_NODE_PATH:-${quoteSh(env.nodePath)}}`,
      `DOBIUS_RELAY_DIR=\${DOBIUS_RELAY_DIR:-${quoteSh(env.relayDir)}}`,
      `DOBIUS_RELAY_SOCKET_PATH=\${DOBIUS_RELAY_SOCKET_PATH:-${quoteSh(env.sockPath)}}`,
      'if [ ! -S "$DOBIUS_RELAY_SOCKET_PATH" ]; then',
      '  echo "Dobius SSH CLI bridge cannot find the relay socket: $DOBIUS_RELAY_SOCKET_PATH" >&2',
      '  exit 1',
      'fi',
      'exec "$DOBIUS_RELAY_NODE_PATH" "$DOBIUS_RELAY_DIR/relay.js" --sock-path "$DOBIUS_RELAY_SOCKET_PATH" --dobius-cli "$@"',
      ''
    ].join('\n')
  }
}
