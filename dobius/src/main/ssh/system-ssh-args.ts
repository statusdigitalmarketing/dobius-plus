...
export type SystemSshBuildArgsOptions = {
  ...
  suppressDobiusControlMaster?: boolean
}
...
export function buildSshArgs(target: SshTarget, options?: SystemSshBuildArgsOptions): string[] {
  ...
  const controlPath = getDobiusControlSocketPath(target, options)
...
export function getDobiusControlSocketPath(
  target: SshTarget,
  options?: SystemSshBuildArgsOptions
): string | null {
  if (shouldDisableDobiusControlMaster(target, options)) {
    return null
  }
...
export function getSystemSshBuildArgsFromOperationOptions(
  ...
  if (options?.suppressDobiusControlMaster === true) {
    buildArgsOptions.suppressDobiusControlMaster = true
  }
  return Object.keys(buildArgsOptions).length === 0 ? undefined : buildArgsOptions
}

function shouldDisableDobiusControlMaster(
  target: SshTarget,
  options?: SystemSshBuildArgsOptions
): boolean {
  // Why: unresolved ssh_config aliases could otherwise share one Dobius socket
  ...
  return (
    ...
    options?.suppressDobiusControlMaster === true ||
...
function hasUserConfiguredControlMaster(
  ...
  // created, but they do not create the setup-burst master Dobius needs.
  return (
