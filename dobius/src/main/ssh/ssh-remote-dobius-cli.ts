...
export type RemoteDobiusCliRequest = {
  argv: string[]
  cwd: string
...
export type RemoteDobiusCliResult = {
  stdout: string
  stderr: string
  exitCode: number
}
...
export async function runRemoteDobiusCli(
  runtime: DobiusRuntimeService,
  request: RemoteDobiusCliRequest
): Promise<RemoteDobiusCliResult> {
...
async function dispatchRemoteCli(
  ...
  switch (command) {
    ...
    default:
      throw new Error(`Unsupported SSH Dobius CLI command: ${command}`)
  }
}
...
function resolveHandle(
  ...
  return optionalString(flags, flagName) ?? env.DOBIUS_TERMINAL_HANDLE ?? 'unknown'
}
