...
function expectNoDobiusControlMasterArgs(args: string[]): void {
...
describe('spawnSystemSsh', () => {
  ...
  it('passes explicit options for manual targets with implicit configHost', () => {
    const args = buildSshArgs(
      createTarget({
        ...
        host: '127.0.0.1',
        port: 2222,
        identityFile: '/tmp/dobius-docker-key',
        identitiesOnly: true
      })
    )

    expect(args).toEqual(expect.arrayContaining(['-p', '2222', '-i', '/tmp/dobius-docker-key']))
    expect(args).toContain('IdentitiesOnly=yes')
    expect(args).toContain('deploy@127.0.0.1')
  })

  it('does not inject Dobius ControlMaster flags when ssh config already owns muxing', () => {
    ...
    expectNoDobiusControlMasterArgs(args)
    expect(args).not.toContain('-S')
    expect(args).toContain('deploy@workbox')
  })

  it('injects Dobius ControlMaster flags when ssh config only sets ControlPersist', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }), {
      resolvedConfig: createResolvedConfig({
        controlMaster: 'no',
        controlPersist: '10m'
  ...
  it('injects Dobius ControlMaster flags when ssh config only sets ControlPath', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }), {
      resolvedConfig: createResolvedConfig({
        controlMaster: 'no',
        controlPath: '/Users/me/.ssh/cm/%r@%h:%p'
  ...
  it('injects Dobius ControlMaster flags when ssh config omits ControlPath', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }), {
      resolvedConfig: createResolvedConfig({
        controlMaster: 'auto'
      })
  ...
  it('does not inject Dobius ControlMaster flags for unresolved ssh-config targets', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }))
  ...
  it('does not inject Dobius ControlMaster flags for unresolved legacy config aliases', () => {
  ...
  it('can inject Dobius ControlMaster flags for ssh-config targets with resolved config', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }), {
      resolvedConfig: createResolvedConfig()
    })
  ...
  it('adds keepalive options to Dobius-owned ControlMaster connections', () => {
  ...
  it('uploads directories to Windows system SSH targets in one PowerShell batch', async () => {
    const localDir = mkdtempSync(join(tmpdir(), 'dobius-system-ssh-upload-'))
    ...
    expect(payload).toEqual(
      expect.arrayContaining([
        { kind: 'directory', path: 'C:/Users/me/.dobius-remote/relay' },
        {
          kind: 'file',
          path: 'C:/Users/me/.dobius-remote/relay/relay.js',
