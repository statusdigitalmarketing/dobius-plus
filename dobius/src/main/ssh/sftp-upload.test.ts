...
describe('sftp-upload', () => {
  it('can create the first binary upload chunk without clobbering an existing temp file', async () => {
    ...
    await uploadBuffer(sftp, Buffer.from('png'), '/remote/.logo.dobius-upload', {
      exclusive: true
    })

    expect(sftp.createWriteStream).toHaveBeenCalledWith('/remote/.logo.dobius-upload', {
      flags: 'wx'
    })
  ...
  it('uses no-clobber writes for nested files during exclusive directory upload', async () => {
    const localDir = await mkdtemp(join(tmpdir(), 'dobius-sftp-upload-'))
