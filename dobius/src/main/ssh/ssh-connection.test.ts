...
const {
  getDobiusControlSocketPathMock,
  removeControlSocketPathMock,
  spawnSystemSshCommandMock,
  spawnSystemSshMock
} = vi.hoisted(() => ({
  getDobiusControlSocketPathMock: vi.fn(),
...
vi.mock('./ssh-system-fallback', () => ({
  getDobiusControlSocketPath: getDobiusControlSocketPathMock,
...
function createSystemCommandChannel(): EventEmitter & {
  ...
  channel.close = vi.fn()
  queueMicrotask(() => {
    channel.emit('data', Buffer.from('DOBIUS-SYSTEM-SSH-OK'))
...
function createSystemSshProcess() {
  ...
  queueMicrotask(() => {
    proc.stdout.emit('data', Buffer.from('DOBIUS-SYSTEM-SSH-READY'))
  })
  return proc
}
...
describe('SshConnection', () => {
  beforeEach(() => {
    ...
    clientInstances = []
    getDobiusControlSocketPathMock.mockReset()
    getDobiusControlSocketPathMock.mockReturnValue(null)
  ...
  it('falls back to direct private key auth when agent auth fails', async () => {
    vi.stubEnv('SSH_AUTH_SOCK', '/tmp/agent.sock')
    const tempDir = mkdtempSync(join(tmpdir(), 'dobius-ssh-key-'))
  ...
  it('allows concurrent exec commands for system SSH with an Dobius ControlMaster socket', async () => {
    getDobiusControlSocketPathMock.mockReturnValue('/tmp/dobius-ssh-501/live-socket')
    vi.mocked(resolveWithSshG).mockResolvedValueOnce(createResolvedConfig())
    const conn = new SshConnection(createTarget({ configHost: 'fdpass-host' }), createCallbacks())
  ...
  it('retries a failed system SSH probe without ControlMaster and disables mux for the session', async () => {
    getDobiusControlSocketPathMock.mockImplementation(
      (_target: SshTarget, options?: { disableControlMaster?: boolean }) =>
        options?.disableControlMaster ? null : '/tmp/dobius-ssh-501/stale-socket'
    )
    spawnSystemSshCommandMock
    ...
    expect(removeControlSocketPathMock).toHaveBeenCalledWith('/tmp/dobius-ssh-501/stale-socket')
  ...
  it('passes the detected host platform to system SSH file operations', async () => {
    ...
    await conn.connect()
    await conn.uploadDirectory('/tmp/local-relay', 'C:/Users/me/.dobius-remote/relay', {
      hostPlatform
    })
    await conn.writeFile('C:/Users/me/.dobius-remote/relay/.version', '0.1.0', {
      hostPlatform
    })
    ...
    expect(uploadDirectoryViaSystemSsh).toHaveBeenCalledWith(
      expect.objectContaining({ configHost: 'fdpass-host' }),
      '/tmp/local-relay',
      'C:/Users/me/.dobius-remote/relay',
      expect.objectContaining({
        hostPlatform,
        resolvedConfig: expect.objectContaining({ proxyUseFdpass: true })
      })
    ...
    expect(writeFileViaSystemSsh).toHaveBeenCalledWith(
      expect.objectContaining({ configHost: 'fdpass-host' }),
      'C:/Users/me/.dobius-remote/relay/.version',
      '0.1.0',
      expect.objectContaining({
        hostPlatform,
        resolvedConfig: expect.objectContaining({ proxyUseFdpass: true })
  ...
  it('does not spawn direct system SSH retry after cancellation between mux failure and retry', async () => {
    getDobiusControlSocketPathMock.mockReturnValue('/tmp/dobius-ssh-501/stale-socket')
...
describe('shouldUseSystemSshTransport', () => {
  ...
  it('allows an environment override for e2e coverage', () => {
    vi.stubEnv('DOBIUS_SSH_FORCE_SYSTEM_TRANSPORT', '1')
