...
describe('ssh remote command builders', () => {
  it('keeps POSIX deploy commands POSIX-native', () => {
    ...
    expect(makeRemoteDirectoryCommand(posix, '/home/me/.dobius-remote')).toContain('mkdir -p')
  ...
  it('uses encoded PowerShell for Windows deploy commands', () => {
    ...
    expect(makeRemoteDirectoryCommand(windows, 'C:/Users/me/.dobius-remote')).toContain(
      '-EncodedCommand'
    )
  ...
  it('uses -Path for Windows New-Item commands', () => {
    const mkdirScript = decodePowerShellCommand(
      makeRemoteDirectoryCommand(windows, 'C:/Users/me/.dobius-remote')
    )
    const lockScript = decodePowerShellCommand(
      tryCreateInstallLockCommand(windows, 'C:/Users/me/.dobius-remote/relay/.install-lock')
    )
  ...
  it('uses named pipe try-connect liveness for Windows GC', () => {
    const command = relayLivenessProbeCommand(windows, 'C:/Users/me/.dobius-remote/relay-0.1.0', {
      nodePath: 'C:/Program Files/nodejs/node.exe',
      pipePaths: ['\\\\.\\pipe\\dobius-relay-1234567890abcdef1234']
    })
    ...
    expect(listRelayBaseDirsCommand(windows, 'C:/Users/me/.dobius-remote')).toContain(
      '-EncodedCommand'
    )
  })
  ...
  it('escapes double quotes before passing JavaScript to native Windows commands', () => {
    const script = decodePowerShellCommand(
      relayLivenessProbeCommand(windows, 'C:/Users/me/.dobius-remote/relay-0.1.0', {
        nodePath: 'C:/Program Files/nodejs/node.exe',
        pipePaths: ['\\\\.\\pipe\\dobius-relay-1234567890abcdef1234']
      })
    )
  ...
  it('prepends the Windows node bin directory to PATH with native separators', () => {
    const script = decodePowerShellCommand(
      commandWithNodePath(
        ...
        'C:/Users/me/.dobius-remote/relay-0.1.0',
        "'READY'"
      )
    )
  ...
  it('makes Windows remote directory changes fail before running scoped commands', () => {
    const scopedCommand = decodePowerShellCommand(
      commandInRemoteDirectory(windows, 'C:/Users/me/.dobius-remote/relay-0.1.0', "'READY'")
    )
    ...
    expect(scopedCommand).toContain(
      "Set-Location -ErrorAction Stop -LiteralPath 'C:/Users/me/.dobius-remote/relay-0.1.0'"
    )
