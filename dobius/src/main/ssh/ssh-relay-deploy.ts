 ...
 * 5. Wait for DOBIUS-RELAY sentinel on stdout
...
async function deployAndLaunchRelayInner(
  ...
  if (!hostPlatform) {
    throw new Error(
      'Unsupported remote platform. Dobius relay supports: linux-x64, linux-arm64, darwin-x64, darwin-arm64, win32-x64, win32-arm64.'
    )
  }
  ...
  if (!localRelayDir) {
    throw new Error(
      `Relay package for ${platform} not found locally. ` +
        `This may be a packaging issue — try reinstalling Dobius.`
    )
  }
  ...
  try {
    const command = isWindowsRemoteHost(hostPlatform)
      ? commandWithNodePath(
          ...
          nodePath,
          remoteDir,
          `try { & ${powerShellLiteral(nodePath)} -e ${powerShellNativeArg('require.resolve("node-pty"); require.resolve("@parcel/watcher"); console.log("DOBIUS-NATIVE-DEPS-OK")')} } catch { 'MISSING' }`
        )
      : commandWithNodePath(
          hostPlatform,
          nodePath,
          remoteDir,
          `(${escapedNode} -e 'require.resolve("node-pty"); require.resolve("@parcel/watcher"); console.log("DOBIUS-NATIVE-DEPS-OK")' 2>/dev/null || echo MISSING)`
        )
    const probe = await execHostCommand(conn, hostPlatform, command)
    return probe.includes('DOBIUS-NATIVE-DEPS-OK')
  } catch {
    return false
  }
}
...
async function installNativeDeps(
  ...
  const PROBE_OK = 'DOBIUS-NPTY-PROBE-OK'
...
export function getLocalRelayCandidates(platform: RelayPlatform): string[] {
  ...
  if (process.env.DOBIUS_RELAY_PATH) {
    candidates.push(join(process.env.DOBIUS_RELAY_PATH, platform))
  }
...
async function launchRelay(
  ...
  const escapedNode = shellEscape(nodePath)
  // Why: remoteRelayDir is shared by every Dobius target for the same remote
