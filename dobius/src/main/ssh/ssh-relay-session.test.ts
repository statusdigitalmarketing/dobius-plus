...
describe('SshRelaySession', () => {
  ...
  it('installs remote managed hooks and relay-owned plugin assets before registering the SSH PTY provider', async () => {
    process.env.DOBIUS_FEATURE_REMOTE_AGENT_HOOKS = '1'
    muxRequestMock.mockImplementation(async (method: string) => {
      if (method === 'session.resolveHome') {
        return { resolvedPath: '/home/dobius' }
      }
      return { ok: true }
    })
    ...
    expect(installRemoteManagedAgentHooksMock).toHaveBeenCalledWith(sftp, '/home/dobius')
  ...
  it('does not run POSIX managed hook installers on Windows remotes', async () => {
    ...
    vi.mocked(deployAndLaunchRelay).mockResolvedValueOnce({
      ...
      remoteRelayDir: 'C:/Users/me/.dobius-remote/relay-v1',
  ...
  it('installs a native Windows Dobius CLI bridge without POSIX shell commands', async () => {
    const { mockStore, mockPortForward, getMainWindow } = createMockDeps()
    const mockConn = {
      writeFile: vi.fn().mockResolvedValue(undefined)
    } as unknown as SshConnection
    ...
    expect(mockConn.writeFile).toHaveBeenCalledWith(
      'C:/Users/me/.dobius-relay/bin/dobius.cmd',
      expect.stringContaining('@echo off'),
      { hostPlatform: getRemoteHostPlatform('win32-x64') }
    )
    const shim = vi.mocked(mockConn.writeFile).mock.calls[0]?.[1] as string
    expect(shim).toContain('C:/Users/me/.dobius-remote/relay-v1')
    expect(shim).toContain('\\\\.\\pipe\\dobius-relay-123')
    expect(shim).not.toContain('if not exist "%DOBIUS_RELAY_SOCKET_PATH%"')
    expect(shim).not.toContain('Dobius SSH CLI bridge cannot find the relay socket')
