...
describe('runRemoteDobiusCli', () => {
  ...
  it('uses the remote DOBIUS_TERMINAL_HANDLE as orchestration sender identity', async () => {
  ...
  it('uses the remote DOBIUS_TERMINAL_HANDLE as orchestration check identity', async () => {
    const { runtime, db } = createRuntime()
    db.insertMessage({
      from: 'term_windows',
      to: 'term_ssh',
