...
// DOBIUS_LIVE_SSH_HOST is set; never runs in normal CI or unit-test loops.
vi.mock('electron', () => ({
...
const LIVE_HOST = process.env.DOBIUS_LIVE_SSH_HOST
const LIVE_USER = process.env.DOBIUS_LIVE_SSH_USER ?? process.env.USERNAME ?? process.env.USER ?? ''
const LIVE_IDENTITY = resolveSshConfigHomePath(
  process.env.DOBIUS_LIVE_SSH_IDENTITY ?? '~/.ssh/id_ed25519'
)
const rawLivePort = process.env.DOBIUS_LIVE_SSH_PORT
...
describe.skipIf(!LIVE_HOST)('live ssh:connect pipeline', () => {
  ...
  it('connects, deploys the relay, and spawns a real PTY', { timeout: 360_000 }, async () => {
    if (!Number.isInteger(LIVE_PORT) || LIVE_PORT < 1 || LIVE_PORT > 65535) {
      throw new Error(`Invalid DOBIUS_LIVE_SSH_PORT: ${rawLivePort}`)
    }
