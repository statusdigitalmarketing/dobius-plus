...
export class SshConnection {
  ...
  canRunConcurrentExecCommands(): boolean {
    ...
    return (
      getDobiusControlSocketPath(this.target, {
  ...
  private async attemptConnect(): Promise<void> {
    ...
    try {
      ...
      if (isSystemSshFallbackError(err)) {
        ...
        this.proxyProcess = null
        try {
          // Why: on macOS, per-app network policy can block Dobius's direct
  ...
  private async doSystemSshProbe(connectGeneration: number): Promise<void> {
    ...
    const channel = this.spawnTrackedSystemSshCommand('echo DOBIUS-SYSTEM-SSH-OK', {
      wrapCommand: false
    })
    try {
      await new Promise<void>((resolve, reject) => {
        ...
        const onClose = (code: number | null): void => {
          settle(() => {
            ...
            if (code !== 0 || !stdout.includes('DOBIUS-SYSTEM-SSH-OK')) {
              reject(
                new Error(
  ...
  private async doSystemSshProbeWithControlMasterRetry(
    ...
    this.systemSshControlMasterDisabledForSession = false
    const controlPath = getDobiusControlSocketPath(this.target, {
