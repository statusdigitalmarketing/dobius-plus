...
import { checkDobiusStarred } from '../github/client'
...
export class StarNagService {
  ...
  registerIpcHandlers(): void {
    ...
    ipcMain.handle('star-nag:starDobius', () => this.starDobiusFromNag())
  ...
  private async maybeShow(
    ...
    try {
      // Why: checkDobiusStarred lets us skip users who already starred outside
      ...
      // button.
      const starred = await checkDobiusStarred()
  ...
  private async starDobiusFromNag(): Promise<boolean> {
    const session = this.promptSession
    if (!session) {
      return false
    }
    ...
    const attempt = this.runStarDobiusAttempt(session)
  ...
  private async runStarDobiusAttempt(session: StarNagPromptSession): Promise<boolean> {
