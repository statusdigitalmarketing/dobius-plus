...
const {
  ...
  checkDobiusStarredMock: vi.fn(),
  starDobiusMock: vi.fn(),
  trackMock: vi.fn(),
...
vi.mock('../github/client', () => ({
  checkDobiusStarred: checkDobiusStarredMock,
  starDobius: starDobiusMock
}))
...
describe('StarNagService', () => {
  ...
  beforeEach(() => {
    ...
    checkDobiusStarredMock.mockReset()
    checkDobiusStarredMock.mockResolvedValue(false)
    starDobiusMock.mockReset()
    starDobiusMock.mockResolvedValue(true)
    trackMock.mockReset()
  ...
  it('shows the browser fallback when checkDobiusStarred cannot determine star state', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    checkDobiusStarredMock.mockResolvedValue(null)
    const { service, emitAgentStarted } = createHarness()
  ...
  it('does not log a threshold exposure when checkDobiusStarred returns true', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    checkDobiusStarredMock.mockResolvedValue(true)
    const { service, emitAgentStarted } = createHarness()
  ...
  it('hides a superseded visible card when onboarding completion detects an existing star', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    checkDobiusStarredMock.mockResolvedValueOnce(true)
    const { service, ui } = createHarness()
  ...
  it('queues onboarding completed while a threshold star check is in flight', async () => {
    ...
    checkDobiusStarredMock.mockReturnValueOnce(deferredStarCheck.promise).mockResolvedValueOnce(null)
    const { service, emitAgentStarted, ui } = createHarness()
  ...
  it('keeps threshold source when force_show is requested during a successful threshold evaluation', async () => {
    ...
    browserWindowMock.getAllWindows.mockReturnValue([window])
    const deferredStarCheck = createDeferred<boolean | null>()
    checkDobiusStarredMock.mockReturnValue(deferredStarCheck.promise)
    const { service, emitAgentStarted } = createHarness()
  ...
  it('does not replay a stale queued force_show after threshold delivery wins', async () => {
    ...
    checkDobiusStarredMock.mockReturnValueOnce(firstStarCheck.promise).mockResolvedValue(null)
    const { service, emitAgentStarted } = createHarness()
  ...
  it('emits direct-star attempted and succeeded outcomes plus app_starred_dobius', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    const { service, ui } = createHarness()
    ...
    service.registerIpcHandlers()
    getIpcHandler('star-nag:forceShow')()
    const ok = await getIpcHandler('star-nag:starDobius')()
    ...
    expect(trackMock).toHaveBeenCalledWith('app_starred_dobius', {
      source: 'star_nag',
      nth_repo_added: 3
    })
  })
  ...
  it('uses the source moment for confirmed direct-star success telemetry', async () => {
    ...
    service.registerIpcHandlers()
    await getIpcHandler('star-nag:onboardingCompleted')()
    await getIpcHandler('star-nag:starDobius')()
  ...
  it('uses fresh cohort context for canonical app_starred_dobius success telemetry', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    getCohortAtEmitMock
  ...
  it('records success and completion when direct star resolves after dismissal cleared the visible session', async () => {
    ...
    starDobiusMock.mockReturnValue(deferredStar.promise)
    const { service, ui } = createHarness()

    service.registerIpcHandlers()
    getIpcHandler('star-nag:forceShow')()
    const starPromise = getIpcHandler('star-nag:starDobius')()
    getIpcHandler('star-nag:dismiss')()
  ...
  it('clears the in-flight direct-star guard after thrown attempts so the user can retry', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    starDobiusMock.mockRejectedValueOnce(new Error('gh failed')).mockResolvedValueOnce(true)
    const { service, ui } = createHarness()

    service.registerIpcHandlers()
    getIpcHandler('star-nag:forceShow')()
    const starFromNag = getIpcHandler('star-nag:starDobius')

    await expect(starFromNag()).rejects.toThrow('gh failed')
    await expect(starFromNag()).resolves.toBe(true)

    expect(starDobiusMock).toHaveBeenCalledTimes(2)
    expect(ui.starNagCompleted).toBe(true)
  })
