 ...
 * coalesced and capped before POSTing to Dobius's agent-hook server. The
 * un-throttled plugin re-posted the full accumulated reply per streamed
 * append — O(n²) bytes per turn — which saturated Dobius's main + renderer
...
const ENV_KEYS = ['DOBIUS_PANE_KEY', 'DOBIUS_AGENT_HOOK_PORT', 'DOBIUS_AGENT_HOOK_TOKEN'] as const
...
describe('OpenCode plugin MessagePart throttling', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'dobius-opencode-plugin-test-'))
    posts = []
    savedEnv = {}
    for (const key of ENV_KEYS) {
      savedEnv[key] = process.env[key]
    }
    process.env.DOBIUS_PANE_KEY = 'tab-1:leaf-1'
    process.env.DOBIUS_AGENT_HOOK_PORT = '45678'
    process.env.DOBIUS_AGENT_HOOK_TOKEN = 'test-token'
  ...
  async function loadPluginEventHandler(): Promise<PluginEventHandler> {
    const pluginPath = join(tempDir, 'dobius-opencode-status.mjs')
    writeFileSync(pluginPath, _internals.getOpenCodePluginSource())
    const module = (await import(pathToFileURL(pluginPath).href)) as {
      DobiusOpenCodeStatusPlugin: (ctx: unknown) => Promise<{ event: PluginEventHandler }>
    }
    const client = {
      session: {
    ...
    const hooks = await module.DobiusOpenCodeStatusPlugin({ client })
    return hooks.event
  }
