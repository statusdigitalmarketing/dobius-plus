...
const DOBIUS_OPENCODE_PLUGIN_FILE = 'dobius-opencode-status.js'
...
const OPENCODE_OVERLAY_MANIFEST_FILE = '.dobius-opencode-overlay-manifest.json'
...
export function getOpenCodeFamilyPluginSource(hookPathname: string): string {
  ...
  // same DOBIUS_PANE_KEY / DOBIUS_TAB_ID / DOBIUS_WORKTREE_ID / DOBIUS_AGENT_HOOK_*
  // env vars that Dobius injects into every PTY, so OpenCode panes flow into
  ...
  return [
    ...
    "// OpenCode's Node process (not Dobius's) and has no access to server.ts's",
    ...
    '// which reads the endpoint file. The file only changes on Dobius restart',
    ...
    '  const path = process.env.DOBIUS_AGENT_HOOK_ENDPOINT;',
    '  if (!path) return null;',
    '  try {',
    '    const fs = require("fs");',
    '    try {',
    ...
    '      // writeEndpointFile on the Dobius side) allocates a fresh inode on',
    ...
    '        // digits in the key for forward-compat with future DOBIUS_AGENT_HOOK_*',
    ...
    '      console.warn("[dobius-hook] failed to parse endpoint file:", err.message);',
    '    }',
    '    return null;',
    '  }',
    '}',
    ...
    '  // frozen when OpenCode was fork()ed — stale after an Dobius restart. The',
    '  // file is rewritten on every Dobius start(), so sourcing it per post lets',
    '  // a long-running OpenCode session reach the current server. Falls back',
    '  // to process.env when the file is absent (first-run / pre-endpoint-file / Dobius',
    ...
    '    env: fileEnv.DOBIUS_AGENT_HOOK_ENV || process.env.DOBIUS_AGENT_HOOK_ENV || "",',
    '    version: fileEnv.DOBIUS_AGENT_HOOK_VERSION || process.env.DOBIUS_AGENT_HOOK_VERSION || "",',
    '  };',
    '}',
    '',
    ...
    '// per turn through Dobius (loopback HTTP -> main JSON parse -> status compare',
    ...
    '// flip the root Dobius pane to done or overwrite the parent turn preview.',
    ...
    '  // rewritten by a newer Dobius since the last call. Pane/tab/worktree IDs',
    '  // stay on process.env because they are per-PTY (stable for the life of',
    '  // the OpenCode process), not per-Dobius-instance.',
    '  const coords = resolveHookCoords();',
    '  const paneKey = process.env.DOBIUS_PANE_KEY;',
    '  if (!coords.port || !coords.token || !paneKey) return;',
    `  const url = \`http://127.0.0.1:\${coords.port}${hookPathname}\`;`,
    '  const body = JSON.stringify({',
    '    paneKey,',
    '    launchToken: process.env.DOBIUS_AGENT_LAUNCH_TOKEN || "",',
    '    tabId: process.env.DOBIUS_TAB_ID || "",',
    '    worktreeId: process.env.DOBIUS_WORKTREE_ID || "",',
    '    env: coords.env,',
    ...
    '        "X-Dobius-Agent-Hook-Token": coords.token,',
    '      },',
    '      body,',
    '    });',
    '  } catch {',
    '    // Why: OpenCode session events must never fail the agent run just',
    '    // because Dobius is unavailable or the local loopback request failed.',
    '  }',
    '}',
    '',
    ...
    'export const DobiusOpenCodeStatusPlugin = async (_ctx) => {',
...
export class OpenCodeHookService {
  ...
  buildPtyEnv(ptyId: string, existingConfigDir?: string | undefined): Record<string, string> {
    if (!isUsableId(ptyId)) {
      ...
      // user's own config — only the Dobius status plugin is forfeited.
    ...
    // Dobius-owned dir is the exact config-replacement failure mode documented in
  ...
  private clearManifestEntries(overlayDir: string, manifest: OpenCodeOverlayManifest): void {
    ...
    for (const entryName of manifest.pluginEntries) {
      if (entryName === DOBIUS_OPENCODE_PLUGIN_FILE) {
        continue
      }
  ...
  // so Dobius can drop a sibling file alongside the user's plugins; everything
  ...
  private mirrorUserConfig(sourceDir: string, overlayDir: string): void {
    ...
    // Dobius previously mirrored, so deleted/replaced user config cannot stay
      ...
      if (entry.name === 'plugins') {
        ...
        if ((!isSymlink && entry.isDirectory()) || isLinkPointingToDir) {
          ...
          for (const pluginEntry of readdirSync(resolvedSource, { withFileTypes: true })) {
            // Why: skip a user file with the same filename as Dobius's plugin —
            ...
            // semantics: Dobius's status plugin runs and the user's same-named
            // plugin is shadowed for this PTY only — their source file on disk
            // is untouched.
            if (pluginEntry.name === DOBIUS_OPENCODE_PLUGIN_FILE) {
              continue
            }
            mirrorEntry(
  ...
  // Why: write Dobius's status plugin into the overlay's plugins/ dir. The
  ...
  private writePluginIntoOverlay(overlayDir: string): void {
    ...
    const pluginPath = join(pluginsDir, DOBIUS_OPENCODE_PLUGIN_FILE)
    try {
  ...
  private writeSharedPluginConfig(): string | null {
    ...
    try {
      mkdirSync(pluginsDir, { recursive: true })
      writeFileSync(join(pluginsDir, DOBIUS_OPENCODE_PLUGIN_FILE), getOpenCodePluginSource())
    } catch {
