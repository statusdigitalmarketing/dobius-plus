...
describe('OpenCode hook plugin source', () => {
  ...
  it('still accepts an optional opaque plugin context instead of destructuring', () => {
    ...
    expect(source).toContain('export const DobiusOpenCodeStatusPlugin = async (_ctx) => {')
  ...
  it('resolves hook coords from the endpoint file before falling back to process.env', () => {
    // Why: a long-running OpenCode session was fork()ed with the prior Dobius's
    // PORT/TOKEN frozen into process.env. The plugin must prefer the on-disk
    // endpoint file (rewritten on every Dobius start()) over env, otherwise it
    // keeps posting to a dead port after an Dobius restart.
    const source = _internals.getOpenCodePluginSource()

    expect(source).toContain('function readEndpointFile()')
    expect(source).toContain('process.env.DOBIUS_AGENT_HOOK_ENDPOINT')
    ...
    expect(source).toContain(
      'port: fileEnv.DOBIUS_AGENT_HOOK_PORT || process.env.DOBIUS_AGENT_HOOK_PORT'
    )
    expect(source).toContain(
      'token: fileEnv.DOBIUS_AGENT_HOOK_TOKEN || process.env.DOBIUS_AGENT_HOOK_TOKEN'
    )
    ...
    expect(source).toContain('"X-Dobius-Agent-Hook-Token": coords.token')
  })
...
describe('OpenCodeHookService buildPtyEnv / clearPty round-trip', () => {
  ...
  beforeAll(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'dobius-opencode-hooks-'))
  ...
  it('writes a shared OPENCODE_CONFIG_DIR and installs the plugin file', () => {
    ...
    const pluginPath = join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'dobius-opencode-status.js')
    expect(existsSync(pluginPath)).toBe(true)
    // Sanity-check the file has plugin source, not a stray write.
    const pluginSource = readFileSync(pluginPath, 'utf8')
    expect(pluginSource).toContain('DobiusOpenCodeStatusPlugin')
  ...
  it('buildPtyEnv preserves a user-set OPENCODE_CONFIG_DIR when the id is unusable', () => {
    ...
    const service = new OpenCodeHookService()
    const userDir = mkdtempSync(join(tmpdir(), 'dobius-opencode-userdir-'))
    try {
  ...
  it('works end-to-end for a plain UUID id (non-daemon path)', () => {
    ...
    expect(env.OPENCODE_CONFIG_DIR).toBe(join(userDataDir, 'opencode-hooks', 'shared'))
    expect(existsSync(join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'dobius-opencode-status.js'))).toBe(
      true
    )
...
describe('OpenCodeHookService overlay mode (user OPENCODE_CONFIG_DIR set)', () => {
  ...
  // Dobius must mirror it into a source-scoped overlay rather than `delete` its
  // own injection or overwrite the user's value. The user's auth/models/keymap
  // and Dobius's status plugin both load via a single OPENCODE_CONFIG_DIR.
  const ptyId = 'overlay-pty-1'
  let userDataDir: string
  let userConfigDir: string

  beforeAll(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'dobius-opencode-overlay-userdata-'))
    getPathMock.mockImplementation((name: string) => {
      if (name === 'userData') {
        return userDataDir
      }
  ...
  beforeEach(() => {
    userConfigDir = mkdtempSync(join(tmpdir(), 'dobius-opencode-overlay-userconfig-'))
  ...
  it('builds an overlay under userData and exposes user config + Dobius plugin together', () => {
    const service = new OpenCodeHookService()
    ...
    // Dobius's status plugin is a sibling, not a replacement.
    const dobiusPluginPath = join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'dobius-opencode-status.js')
    expect(existsSync(dobiusPluginPath)).toBe(true)
    expect(readFileSync(dobiusPluginPath, 'utf8')).toContain('DobiusOpenCodeStatusPlugin')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      // Why: only the plugins/ subtree needs entry-by-entry mirroring so Dobius
      ...
      // plugins/ must be a real directory in the overlay so Dobius can write
  ...
  it("does not overwrite a user plugin file with the same filename as Dobius's plugin", () => {
    // Why: the failure mode this guards against — a user-owned plugin file
    // happens to be named dobius-opencode-status.js. Without the per-entry
    // skip in mirrorUserConfig, the file would be linked into the overlay
    // and Dobius's writeFileSync would write through the symlink, destroying
    // the user's content on their real filesystem.
    const userDobiusSentinel = 'USER OWNED DOBIUS-NAMED PLUGIN — DO NOT CLOBBER'
    writeFileSync(join(userConfigDir, 'plugins', 'dobius-opencode-status.js'), userDobiusSentinel)

    const service = new OpenCodeHookService()
    const env = service.buildPtyEnv(ptyId, userConfigDir)

    // User's source file must be untouched.
    expect(readFileSync(join(userConfigDir, 'plugins', 'dobius-opencode-status.js'), 'utf8')).toBe(
      userDobiusSentinel
    )

    // Overlay copy is Dobius's real plugin source, not the user's file.
    const overlayPlugin = readFileSync(
      join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'dobius-opencode-status.js'),
      'utf8'
    )
    expect(overlayPlugin).toContain('DobiusOpenCodeStatusPlugin')
    expect(overlayPlugin).not.toBe(userDobiusSentinel)
    expectUserConfigIntact()
  })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      // Why: if plugins/ is a symlink (common dotfiles pattern), writing Dobius's
      ...
      const realPluginsDir = mkdtempSync(join(tmpdir(), 'dobius-real-plugins-'))
      try {
        ...
        // The user's real filesystem must NOT receive Dobius's status plugin.
        expect(existsSync(join(realPluginsDir, 'dobius-opencode-status.js'))).toBe(false)
        ...
        // Dobius's status plugin lands in the overlay only.
        expect(
          existsSync(join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'dobius-opencode-status.js'))
        ).toBe(true)
  ...
  it("preserves the user's OPENCODE_CONFIG_DIR when the path does not exist", () => {
    // Why: typoed user path — overriding it with an Dobius-owned dir would let
    // Dobius's status plugin "succeed" while silently hiding the user's typo.
    // The design rejects that: leave the user's value alone and let OpenCode
    // surface the typo on its own.
    const service = new OpenCodeHookService()
    const missingPath = join(tmpdir(), `dobius-opencode-nope-${Date.now()}`)
  ...
  it("preserves the user's OPENCODE_CONFIG_DIR when the mirror step fails", async () => {
    ...
    // the user's plugins/auth/models keep loading; only Dobius's status plugin
    // is forfeited.
  ...
  it('rebuilding the overlay for the same ptyId does not corrupt the user dir', () => {
    ...
    expect(
      readFileSync(join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'dobius-opencode-status.js'), 'utf8')
    ).toContain('DobiusOpenCodeStatusPlugin')
    expectUserConfigIntact()
  })
