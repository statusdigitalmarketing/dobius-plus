...
function dataFile(): string {
  return join(testState.dir, 'dobius-data.json')
}
...
describe('Store', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'dobius-test-'))
    trackMock.mockReset()
  ...
  it('snapshots automation contexts onto runs', async () => {
    const store = await createStore()
    store.addRepo(makeRepo({ upstream: { owner: 'statusdigitalmarketing', repo: 'dobius' } }))
    const automation = store.createAutomation({
      name: 'Nightly',
      prompt: 'Run checks',
      agentId: 'claude',
  ...
  it('merges rollback commit-message AI writes into existing source-control AI on load', async () => {
    ...
    store.flush()
    const persisted = JSON.parse(readFileSync(join(testState.dir, 'dobius-data.json'), 'utf-8'))
  ...
  it('normalizes malformed terminal shortcut policy on load', async () => {
    ...
    const store = await createStore()
    expect(store.getSettings().terminalShortcutPolicy).toBe('dobius-first')
  })
  ...
  it('creates independent project host setup records for provisioning flows', async () => {
    ...
    store.addRepo({
      ...
      upstream: { owner: 'statusdigitalmarketing', repo: 'cloud-project' }
    })

    const result = store.createProjectHostSetup({
      projectId: 'github:statusdigitalmarketing/cloud-project',
      hostId: 'runtime:gpu-vm',
    ...
    expect(result?.project).toMatchObject({
      id: 'github:statusdigitalmarketing/cloud-project',
      displayName: 'Cloud Project'
    })
  ...
  it('updateRepo normalizes and persists repo upstream metadata', async () => {
    ...
    expect(updated!.upstream).toEqual({ owner: 'statusdigitalmarketing', repo: 'dobius' })
  ...
  it('normalizes disabled TUI agents on load and update', async () => {
    writeFileSync(
      join(testState.dir, 'dobius-data.json'),
      JSON.stringify({
        settings: {
  ...
  it('snapshots the cache at flush and seeds the next Store from the sidecar', async () => {
    ...
    store.flush()
    expect(existsSync(join(testState.dir, 'dobius-github-cache.json'))).toBe(true)
  ...
  it('missing terminalMacOptionAsAlt in persisted file defaults to "auto" and flags migrated', async () => {
    ...
    // pre-Option-as-Alt Dobius: land on 'auto' and mark migrated so we don't
    // re-examine.
    writeDataFile({
      schemaVersion: 1,
      repos: [],
  ...
  // contract they installed Dobius under.
  ...
  it('still classifies as existing-user cohort when the data file is corrupt', async () => {
    ...
    // Otherwise a user whose `dobius-data.json` got corrupted would be
