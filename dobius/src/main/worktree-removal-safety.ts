...
const DOBIUS_CREATION_SOURCES = new Set<NonNullable<WorktreeMeta['dobiusCreationSource']>>([
  'desktop',
  'runtime',
  'cli',
  'ssh'
])
const DOBIUS_OWNED_PROVENANCE_META_KEYS = [
  'dobiusCreatedAt',
  'dobiusCreationSource',
  'dobiusCreationWorkspaceLayout',
  'automationProvenance'
] as const
type UnregisteredDobiusCleanupMeta = Pick<
  WorktreeMeta,
  | 'dobiusCreatedAt'
  | 'dobiusCreationSource'
  | 'createdAt'
...
export function canCleanupUnregisteredDobiusWorktreeDirectory(args: {
  meta: UnregisteredDobiusCleanupMeta | null | undefined
}): boolean {
  if (hasCurrentDobiusCreationProvenance(args.meta)) {
    return true
  }

  if (hasLegacyDobiusCreationEvidence(args.meta)) {
    return true
  }

  // Why: path shape alone is not authority; users can create plain Git
  // worktrees inside Dobius's workspace directory too.
  return false
}

export async function canCleanupUnregisteredDobiusLeftoverDirectory(args: {
  meta: UnregisteredDobiusCleanupMeta | null | undefined
  worktreePath: string
  ...
  // ownership for recursive deletion; require persisted Dobius-created evidence.
  if (!hasCurrentDobiusCreationProvenance(args.meta) && !hasLegacyDobiusCreationEvidence(args.meta)) {
    return false
  }
...
function hasCurrentDobiusCreationProvenance(
  meta: Pick<WorktreeMeta, 'dobiusCreatedAt' | 'dobiusCreationSource'> | null | undefined
): boolean {
  return (
    typeof meta?.dobiusCreatedAt === 'number' &&
    !!meta.dobiusCreationSource &&
    DOBIUS_CREATION_SOURCES.has(meta.dobiusCreationSource)
  )
}

function hasLegacyDobiusCreationEvidence(
  meta: UnregisteredDobiusCleanupMeta | null | undefined
): boolean {
  return Boolean(
    meta?.createdAt ||
...
export function stripDobiusProvenanceMetaUpdates(
  updates: Partial<WorktreeMeta> | null | undefined
): Partial<WorktreeMeta> {
  const sanitized = { ...updates }
  for (const key of DOBIUS_OWNED_PROVENANCE_META_KEYS) {
