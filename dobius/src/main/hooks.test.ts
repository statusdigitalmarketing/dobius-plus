...
import { getDefaultTabsLaunch, parseDobiusYaml } from './hooks'
...
const TEST_REPO_DOBIUS_YAML_PATH = join(TEST_REPO_PATH, 'dobius.yaml')
const TEST_WORKTREE_DOBIUS_YAML_PATH = join(TEST_WORKTREE_PATH, 'dobius.yaml')
const TEST_ISSUE_COMMAND_PATH = join(TEST_REPO_PATH, '.dobius', 'issue-command')
const TEST_GITIGNORE_PATH = join(TEST_REPO_PATH, '.gitignore')

describe('parseDobiusYaml', () => {
  it('parses YAML with setup script only', () => {
    const yaml = `scripts:\n  setup: |\n    echo "setting up"\n    npm install\n`
    const result = parseDobiusYaml(yaml)
  ...
  it('returns null when there is no scripts block', () => {
    const yaml = `other:\n  key: value\n`
    expect(parseDobiusYaml(yaml)).toBeNull()
  })
  ...
  it('returns null for empty string', () => {
    expect(parseDobiusYaml('')).toBeNull()
  })
  ...
  it('parses default terminal tabs from dobius.yaml', () => {
    const yaml = [
      'defaultTabs:',
      '  - title: Claude',
      '    color: "#f97316"',
    ...
    expect(parseDobiusYaml(yaml)).toEqual({
      scripts: {},
      defaultTabs: [
  ...
  it('parses environmentRecipes from dobius.yaml', () => {
    const yaml = [
      'environmentRecipes:',
      '  - id: cloud-sandbox',
      '    name: Cloud Sandbox',
      '    description: Starts a per-workspace VM.',
      '    create: ./scripts/dobius-vm/start-cloud-sandbox.sh',
      '    suspend: ./scripts/dobius-vm/suspend-cloud-sandbox.sh',
      '    resume: ./scripts/dobius-vm/resume-cloud-sandbox.sh',
      '    destroy: ./scripts/dobius-vm/destroy-cloud-sandbox.sh'
    ].join('\n')
  ...
  it('parses legacy environmentRecipes command and cleanup aliases', () => {
    const yaml = [
      ...
      '  - id: manual-sandbox',
      '    name: Manual Sandbox',
      '    command: ./scripts/dobius-vm/start-manual-sandbox.sh',
      '    cleanup: none'
    ].join('\n')
      ...
      environmentRecipes: [
        {
          id: 'manual-sandbox',
          name: 'Manual Sandbox',
          create: './scripts/dobius-vm/start-manual-sandbox.sh',
  ...
  it('drops invalid and duplicate environmentRecipes', () => {
    const yaml = [
      ...
      '    create: ./scripts/dobius-vm/start-duplicate.sh',
      '  - id: missing-create',
      '    name: Missing Create',
      '  - name: Missing Id',
      '    create: ./scripts/dobius-vm/start-missing-id.sh',
      '  - id: "Cloud Sandbox"',
      '    name: Unsafe Id',
      '    create: ./scripts/dobius-vm/start-unsafe-id.sh',
      '  - 42'
    ].join('\n')
...
describe('hasUnrecognizedDobiusYamlKeys', () => {
  it('returns true when the file contains only keys this version does not handle', async () => {
    ...
    const { hasUnrecognizedDobiusYamlKeys } = await import('./hooks')
    expect(hasUnrecognizedDobiusYamlKeys('/test/repo')).toBe(true)
  })
  ...
  it('returns false when the file contains only recognised keys', async () => {
    ...
    const { hasUnrecognizedDobiusYamlKeys } = await import('./hooks')
    expect(hasUnrecognizedDobiusYamlKeys('/test/repo')).toBe(false)
  })
...
describe('readIssueCommand', () => {
  it('prefers the local override over the shared dobius.yaml command', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockImplementation(
      (path) => path === TEST_ISSUE_COMMAND_PATH || path === TEST_REPO_DOBIUS_YAML_PATH
    )
    vi.mocked(fs.readFileSync).mockImplementation((path) => {
      if (path === TEST_ISSUE_COMMAND_PATH) {
        return 'local command\n'
      }
      if (path === TEST_REPO_DOBIUS_YAML_PATH) {
        return 'issueCommand: |\n  shared command\n'
      }
      return ''
    })
  ...
  it('falls back to the shared dobius.yaml command when no local override exists', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockImplementation((path) => path === TEST_REPO_DOBIUS_YAML_PATH)
    vi.mocked(fs.readFileSync).mockImplementation((path) => {
      if (path === TEST_REPO_DOBIUS_YAML_PATH) {
        return 'issueCommand: |\n  shared command\n'
      }
...
describe('writeIssueCommand', () => {
  it('writes only the local override file and keeps .dobius ignored locally', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockImplementation(
      (path) => path === TEST_GITIGNORE_PATH || path === join(TEST_REPO_PATH, '.dobius')
    )
    vi.mocked(fs.readFileSync).mockImplementation((path) => {
...
describe('getEffectiveHooks', () => {
  ...
  it('uses hooks from dobius.yaml when present', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockReturnValue(true)
  ...
  it("loads setup hooks from the target worktree's dobius.yaml when a worktree path is provided", async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockImplementation(
      (path) => path === TEST_REPO_DOBIUS_YAML_PATH || path === TEST_WORKTREE_DOBIUS_YAML_PATH
    )
    vi.mocked(fs.readFileSync).mockImplementation((path) => {
      if (path === TEST_REPO_DOBIUS_YAML_PATH) {
        return 'scripts:\n  setup: |\n    echo old-version\n'
      }
      if (path === TEST_WORKTREE_DOBIUS_YAML_PATH) {
        return 'scripts:\n  setup: |\n    echo new-version\n'
      }
      return ''
    })
  ...
  it('uses local settings by default even when dobius.yaml defines only one command', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockReturnValue(true)
    vi.mocked(fs.readFileSync).mockReturnValue('scripts:\n  archive: |\n    echo "yaml archive"\n')
  ...
  it('treats legacy shared-first policy as dobius.yaml only', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockReturnValue(true)
    vi.mocked(fs.readFileSync).mockReturnValue('scripts:\n  archive: |\n    echo "yaml archive"\n')
...
describe('runHook', () => {
      ...
      expect(options).toEqual(
        expect.objectContaining({
          env: expect.objectContaining({
            DOBIUS_ROOT_PATH: '/mnt/c/Users/jinwo/git/dobius',
            DOBIUS_WORKTREE_PATH: '/home/jin/feature',
            CONDUCTOR_ROOT_PATH: '/mnt/c/Users/jinwo/git/dobius',
            GHOSTX_ROOT_PATH: '/mnt/c/Users/jinwo/git/dobius'
          })
        })
      )
      return {} as never
  ...
  it('runs Windows-path hooks through WSL when the project runtime targets WSL', async () => {
    ...
    execFileMock.mockImplementation((_file, _args, options, callback) => {
      ...
      expect(options).toEqual(
        expect.objectContaining({
          env: expect.objectContaining({
            DOBIUS_ROOT_PATH: '/mnt/c/Users/jinwo/git/dobius',
            DOBIUS_WORKTREE_PATH: '/mnt/c/Users/jinwo/git/dobius-feature',
            CONDUCTOR_ROOT_PATH: '/mnt/c/Users/jinwo/git/dobius',
            GHOSTX_ROOT_PATH: '/mnt/c/Users/jinwo/git/dobius'
          })
        })
    ...
    try {
      ...
      const result = await runHook(
        'setup',
        'C:\\Users\\jinwo\\git\\dobius-feature',
        {
          ...makeRepo(),
          path: 'C:\\Users\\jinwo\\git\\dobius'
        },
      ...
      expect(execFileMock).toHaveBeenCalledWith(
        ...
        [
          ...
          'bash',
          '-c',
          "cd '/mnt/c/Users/jinwo/git/dobius-feature' && echo hello"
        ],
        expect.any(Object),
        expect.any(Function)
      )
  ...
  it('writes Windows-path setup runners through WSL git when the project runtime targets WSL', async () => {
    ...
    gitExecFileSyncMock.mockReturnValue('/mnt/c/Users/jinwo/git/dobius/.git/dobius/setup-runner.sh\n')
    ...
    try {
      ...
      expect(gitExecFileSyncMock).toHaveBeenCalledWith(
        ...
        {
          cwd: 'C:\\Users\\jinwo\\git\\dobius-feature',
          wslDistro: 'Ubuntu'
        }
      )
...
describe('createSetupRunnerScript', () => {
  ...
  it('omits waitForAgentStartup unless the repo explicitly waits for setup', async () => {
    gitExecFileSyncMock.mockReset()
    gitExecFileSyncMock.mockReturnValue('/test/repo/.git/dobius/setup-runner.sh\n')
    const { createSetupRunnerScript } = await import('./hooks')
