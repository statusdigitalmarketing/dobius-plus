...
describe('skill discovery', () => {
  it('discovers home and repo SKILL.md packages with provider metadata', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dobius-skills-'))
  ...
  it('discovers skill packages through symlinked skill directories', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dobius-skills-'))
    const home = join(root, 'home')
    const realSkill = join(root, 'central-skills', 'dobius-cli')
    const linkedSkill = join(home, '.agents', 'skills', 'dobius-cli')
    await mkdir(realSkill, { recursive: true })
    await mkdir(join(home, '.agents', 'skills'), { recursive: true })
    await writeFile(join(realSkill, 'SKILL.md'), '# Dobius CLI\n\nUse the Dobius CLI.')
    ...
    const skill = result.skills.find((entry) => entry.name === 'Dobius CLI')
  ...
  it('keeps home classification when cwd points at the same directory as home', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dobius-skills-'))
    const home = join(root, 'home')
    const skillDir = join(home, '.agents', 'skills', 'dobius-cli')
    await mkdir(skillDir, { recursive: true })
    await writeFile(
      join(skillDir, 'SKILL.md'),
      ['---', 'name: dobius-cli', 'description: Use the Dobius CLI.', '---', ''].join('\n')
    )
    ...
    expect(result.skills.filter((entry) => entry.name === 'dobius-cli')).toMatchObject([
      {
        sourceKind: 'home',
