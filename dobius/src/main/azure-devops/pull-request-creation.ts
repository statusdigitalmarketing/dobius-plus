...
import { readProviderEnv } from '../source-control/provider-env'
...
function getAuthConfig(): AzureDevOpsCreateAuthConfig {
  return {
    apiBaseUrl: readProviderEnv('AZURE_DEVOPS_API_BASE_URL'),
    pat: readProviderEnv('AZURE_DEVOPS_TOKEN') ?? readProviderEnv('AZURE_DEVOPS_PAT'),
    accessToken: readProviderEnv('AZURE_DEVOPS_ACCESS_TOKEN'),
    username: readProviderEnv('AZURE_DEVOPS_USERNAME')
  }
}
...
function classifyCreateError(error: unknown): CreateHostedReviewResult {
  ...
  if (
    ...
    return {
      ...
      code: 'auth_required',
      error:
        'Create PR failed: Azure DevOps is not authenticated. Next step: set DOBIUS_AZURE_DEVOPS_TOKEN in this environment.'
    }
  }
