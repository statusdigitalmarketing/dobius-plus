 ...
 */
const DISABLE_ENV_VAR = 'DOBIUS_DISABLE_MACOS_LOGIN_SHELL'
 ...
 * get their own TCC identity instead of collapsing into Dobius's bundle id — signed
 * CLIs like `op` otherwise re-prompt every launch because tccd attributes the grant
 * to Dobius and never persists it (#6996). This mirrors how Terminal.app spawns shells.
 *
 * Why the env(1) interposition: login(1) overwrites SHELL from the account DB even
 * under -p, so `/usr/bin/env SHELL=<shell>` re-asserts the shell Dobius actually runs
