...
describe('scanForShellReady', () => {
  it('flushes marker-like output when the full marker is not BEL-terminated', () => {
    ...
    expect(scanForShellReady(state, 'before \x1b]777;dobius-shell-readyx')).toEqual({
      output: 'before \x1b]777;dobius-shell-readyx',
      matched: false,
  ...
  it('reports post-marker bytes only when bytes follow the BEL terminator in the matching call', () => {
    ...
    expect(scanForShellReady(state, 'before \x1b]777;dobius-shell-ready\x07')).toEqual({
      output: 'before ',
      matched: true,
      postMarkerBytesObserved: false
    })
    ...
    state = createShellReadyScanState()
    expect(scanForShellReady(state, 'before \x1b]777;dobius-shell-ready')).toEqual({
      output: 'before ',
      matched: false,
      postMarkerBytesObserved: false
    })
    ...
    state = createShellReadyScanState()
    expect(scanForShellReady(state, '\x1b]777;dobius-shell-ready')).toEqual({
      output: '',
      matched: false,
      postMarkerBytesObserved: false
    })
...
function expectZdotdirSourceContext(content: string, fileName: '.zprofile' | '.zshrc' | '.zlogin') {
  expect(content).toContain('export ZDOTDIR="$_dobius_home"')
  expect(content).toContain(`source "$_dobius_home/${fileName}"`)
  expect(content).toContain('export ZDOTDIR="$_dobius_wrapper_zdotdir"')
}

function expectFinalZdotdirRestoreContext(content: string) {
  expect(content).toContain("after Dobius's last wrapper file has loaded")
  expect(content).toContain('export ZDOTDIR="$_dobius_home"')
}

describePosix('local PTY shell-ready launch config', () => {
  let userDataPath: string
  let previousDobiusOrigZdotdir: string | undefined

  beforeEach(() => {
    previousDobiusOrigZdotdir = process.env.DOBIUS_ORIG_ZDOTDIR
    delete process.env.DOBIUS_ORIG_ZDOTDIR
    userDataPath = mkdtempSync(join(tmpdir(), 'local-pty-shell-ready-test-'))
    getUserDataPathMock.mockReturnValue(userDataPath)
  })

  afterEach(() => {
    if (previousDobiusOrigZdotdir === undefined) {
      delete process.env.DOBIUS_ORIG_ZDOTDIR
    } else {
      process.env.DOBIUS_ORIG_ZDOTDIR = previousDobiusOrigZdotdir
    }
    rmSync(userDataPath, { recursive: true, force: true })
    vi.restoreAllMocks()
  })

  it('falls back to HOME for DOBIUS_ORIG_ZDOTDIR when inherited ZDOTDIR points at a wrapper dir', async () => {
    // Why: mirrors the daemon path — guards the same zsh recursion loop for
    // PTYs spawned by the renderer/local provider when Dobius is launched from
    // inside an Dobius terminal (e.g. `pn dev`).
    const previousZdotdir = process.env.ZDOTDIR
    const previousHome = process.env.HOME
    process.env.ZDOTDIR = '/some/other/dobius/shell-ready/zsh'
    process.env.HOME = '/Users/alice'
    try {
      const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
      expect(config.env.DOBIUS_ORIG_ZDOTDIR).toBe('/Users/alice')
      expect(config.env.DOBIUS_ZSHENV_SOURCE_DIR).toBe('/Users/alice')
    } finally {
  ...
  it('uses inherited DOBIUS_ORIG_ZDOTDIR when ZDOTDIR is an Dobius wrapper dir', async () => {
    const previousZdotdir = process.env.ZDOTDIR
    const previousOrigZdotdir = process.env.DOBIUS_ORIG_ZDOTDIR
    const previousHome = process.env.HOME
    process.env.ZDOTDIR = '/some/other/dobius/shell-ready/zsh'
    process.env.DOBIUS_ORIG_ZDOTDIR = '/Users/alice/.config/zsh'
    process.env.HOME = '/Users/alice'
    try {
      const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
      expect(config.env.DOBIUS_ORIG_ZDOTDIR).toBe('/Users/alice/.config/zsh')
      expect(config.env.DOBIUS_ZSHENV_SOURCE_DIR).toBe('/Users/alice')
    } finally {
      if (previousZdotdir === undefined) {
        delete process.env.ZDOTDIR
      ...
      if (previousOrigZdotdir === undefined) {
        ...
        process.env.DOBIUS_ORIG_ZDOTDIR = previousOrigZdotdir
      }
      if (previousHome === undefined) {
        delete process.env.HOME
      } else {
  ...
  it('falls back to HOME when inherited DOBIUS_ORIG_ZDOTDIR points at a wrapper dir', async () => {
    const previousZdotdir = process.env.ZDOTDIR
    const previousOrigZdotdir = process.env.DOBIUS_ORIG_ZDOTDIR
    const previousHome = process.env.HOME
    delete process.env.ZDOTDIR
    process.env.DOBIUS_ORIG_ZDOTDIR = '/some/other/dobius/shell-ready/zsh'
    process.env.HOME = '/Users/alice'
    try {
      const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
  ...
  it('writes zsh wrappers that guard against DOBIUS_ORIG_ZDOTDIR self-loops', async () => {
    const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
    ...
    expect(zshenv).toContain('_dobius_user_zdotdir="${_dobius_spawn_orig_zdotdir:-$HOME}"')
    expect(zshenv).toContain('*/shell-ready/zsh) _dobius_user_zdotdir="$HOME" ;;')
    expect(zshenv).toContain('""|*/shell-ready/zsh) export DOBIUS_ORIG_ZDOTDIR="$HOME" ;;')
  ...
  it('owns zle-line-init for the shell-ready marker instead of an azhw hook', async () => {
    ...
    const zlogin = readFileSync(join(userDataPath, 'shell-ready', 'zsh', '.zlogin'), 'utf8')
    expect(zlogin).toContain('zle -N zle-line-init __dobius_prompt_mark')
    expect(zlogin).toContain('__dobius_prev_line_init_fn="${widgets[zle-line-init]#user:}"')
    expect(zlogin).toContain('printf "\\033]777;dobius-shell-ready\\007"')
    ...
    expect(zlogin).toContain('== "user:__dobius_prompt_mark"')
  })
  ...
  it('writes wrappers without restoring Pi/OMP homes after user startup files', async () => {
    ...
    const codexRestoreLine =
      '[[ -n "${DOBIUS_CODEX_HOME:-}" ]] && export CODEX_HOME="${DOBIUS_CODEX_HOME}"'
    const agentTeamsPathRestoreLine = '[[ -n "${DOBIUS_AGENT_TEAMS_SHIM_DIR:-}" ]] || return 0'
    const ompWrapperLine = 'command omp --extension "${DOBIUS_OMP_STATUS_EXTENSION}" "$@"'
    ...
    expect(zshrc).not.toContain('DOBIUS_PI_CODING_AGENT_DIR')
    expect(zlogin).not.toContain('DOBIUS_PI_CODING_AGENT_DIR')
    expect(bashRc).not.toContain('DOBIUS_PI_CODING_AGENT_DIR')
    ...
    expect(zshrc).not.toContain('DOBIUS_OMP_CODING_AGENT_DIR')
    expect(zlogin).not.toContain('DOBIUS_OMP_CODING_AGENT_DIR')
    expect(bashRc).not.toContain('DOBIUS_OMP_CODING_AGENT_DIR')
  ...
  it('emits OSC 133 C/D markers in the bash wrapper so agent exit cleanup fires', async () => {
    ...
    expect(bashRc.indexOf("trap '__dobius_osc133_preexec' DEBUG")).toBeGreaterThan(
      bashRc.indexOf('if [[ "${DOBIUS_SHELL_READY_MARKER:-0}" == "1" ]]; then')
    )
  ...
  it('preserves a real inherited ZDOTDIR as DOBIUS_ORIG_ZDOTDIR', async () => {
    const previousZdotdir = process.env.ZDOTDIR
    ...
    try {
      ...
      const config = getShellReadyLaunchConfig('/bin/zsh')
      expect(config.env.DOBIUS_ORIG_ZDOTDIR).toBe('/Users/alice/.config/zsh')
      expect(config.env.DOBIUS_ZSHENV_SOURCE_DIR).toBe('/Users/alice/.config/zsh')
    } finally {
      if (previousZdotdir === undefined) {
        delete process.env.ZDOTDIR
      } else {
  ...
  it('preserves ZDOTDIR that contains /shell-ready/zsh as a substring but does not end with it', async () => {
    ...
    try {
      const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
      expect(config.env.DOBIUS_ORIG_ZDOTDIR).toBe('/Users/alice/shell-ready/zsh-custom')
    } finally {
      if (previousZdotdir === undefined) {
        delete process.env.ZDOTDIR
      } else {
  ...
  it('sources user .zshenv at wrapper top level before repinning ZDOTDIR', async () => {
    ...
    expect(zshenv).toContain('_dobius_zshenv_source_dir="${DOBIUS_ZSHENV_SOURCE_DIR:-$HOME}"')
    expect(zshenv).toContain('source "${_dobius_zshenv_path}"')
    expect(zshenv).toContain('_dobius_discovered_zdotdir="${ZDOTDIR:-}"')
  ...
  it('preserves spawn-env DOBIUS_ORIG_ZDOTDIR as fallback when discovery yields nothing', async () => {
    // Why: if user .zshenv returns early or doesn't set ZDOTDIR, the wrapper
    // should fall back to the spawn-env DOBIUS_ORIG_ZDOTDIR (if present), then HOME.
    const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
    ...
    expect(zshenv).toContain('_dobius_spawn_orig_zdotdir="${DOBIUS_ORIG_ZDOTDIR:-}"')

    // Fallback chain: discovered → normalized spawn-env path → HOME
    expect(zshenv).toContain('${_dobius_discovered_zdotdir:-${_dobius_user_zdotdir:-$HOME}}')
  })
})
...
describePosix('live zsh subprocess tests', () => {
  ...
  describeIfZsh('ZDOTDIR discovery with real zsh', () => {
    ...
    beforeEach(async () => {
      testHome = mkdtempSync(join(tmpdir(), 'dobius-zsh-test-home-'))
      userDataPath = mkdtempSync(join(tmpdir(), 'dobius-zsh-test-userdata-'))
      getUserDataPathMock.mockReturnValue(userDataPath)
    })
...
`
      ...
      // Build clean env: use wrapper ZDOTDIR but let wrapper discover DOBIUS_ORIG_ZDOTDIR at runtime
      ...
      delete cleanEnv.DOBIUS_ORIG_ZDOTDIR
      ...
      delete cleanEnv.DOBIUS_ATTRIBUTION_SHIM_DIR
      cleanEnv.ZDOTDIR = config.env.ZDOTDIR // Point to Dobius wrapper dir
      ...
      const result = spawnSync(
        ...
        [
          '-i',
          '-c',
          'echo "DOBIUS_ORIG_ZDOTDIR=${DOBIUS_ORIG_ZDOTDIR}" && echo "PATH_HAS_CUSTOM=${PATH%%:*}"'
        ],
        {
      ...
      expect(output).toContain(`DOBIUS_ORIG_ZDOTDIR=${xdgZshDir}`)
    ...
    it('preserves top-level .zshenv path and function side effects', async () => {
      ...
      // Dropping those side effects regresses non-Dobius zsh startup semantics.
      const xdgZshDir = join(testHome, '.config', 'zsh')
      mkdirSync(xdgZshDir, { recursive: true })
      writeFileSync(
        join(testHome, '.zshenv'),
...
dobius_zshenv_func() { echo "from-zshenv-function"; }
export ZDOTDIR="$HOME/.config/zsh"
`
      )
      ...
      const result = spawnSync(
        ...
        [
          '-c',
          'echo "PATH_HEAD=${PATH%%:*}" && echo "MY_VAR=${MY_VAR:-unset}" && dobius_zshenv_func'
        ],
        {
          env: cleanEnv as NodeJS.ProcessEnv,
          encoding: 'utf8'
...
`
      ...
      const result = spawnSync(
        'zsh',
        ['-c', 'echo "survived" && echo "DOBIUS_ORIG_ZDOTDIR=${DOBIUS_ORIG_ZDOTDIR}"'],
        {
          env: cleanEnv as NodeJS.ProcessEnv,
          encoding: 'utf8'
        }
      ...
      expect(result.stdout).toContain(`DOBIUS_ORIG_ZDOTDIR=${testHome}`)
    })
...
`
      ...
      const result = spawnSync('zsh', ['-c', 'echo "DOBIUS_ORIG_ZDOTDIR=${DOBIUS_ORIG_ZDOTDIR}"'], {
        env: cleanEnv as NodeJS.ProcessEnv,
        encoding: 'utf8'
      })
  ...
  describeIfZsh('high-priority edge cases', () => {
    ...
    beforeEach(async () => {
      testHome = mkdtempSync(join(tmpdir(), 'dobius-zsh-edge-'))
      userDataPath = mkdtempSync(join(tmpdir(), 'dobius-zsh-userdata-'))
      getUserDataPathMock.mockReturnValue(userDataPath)
    })
    ...
    it('discovers ZDOTDIR when .zshenv sources another file that sets it', async () => {
      ...
      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`DOBIUS_ORIG_ZDOTDIR=${xdgZshDir}`)
    })
    ...
    it('preserves ZDOTDIR with spaces in path', async () => {
      ...
      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`DOBIUS_ORIG_ZDOTDIR=${spacePath}`)
    })
    ...
    it('captures last ZDOTDIR value when set multiple times', async () => {
      ...
      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`DOBIUS_ORIG_ZDOTDIR=${lastPath}`)
    })
    ...
    it('handles conditional ZDOTDIR based on environment', async () => {
      ...
      let result = spawnSync('zsh', ['-c', 'echo "DOBIUS_ORIG_ZDOTDIR=${DOBIUS_ORIG_ZDOTDIR}"'], {
        env: cleanEnv as NodeJS.ProcessEnv,
        encoding: 'utf8'
      })

      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`DOBIUS_ORIG_ZDOTDIR=${localPath}`)
      ...
      result = spawnSync('zsh', ['-c', 'echo "DOBIUS_ORIG_ZDOTDIR=${DOBIUS_ORIG_ZDOTDIR}"'], {
        env: cleanEnv as NodeJS.ProcessEnv,
        encoding: 'utf8'
      })

      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`DOBIUS_ORIG_ZDOTDIR=${remotePath}`)
    })
    ...
    it('does not source /.zshenv when HOME is empty', async () => {
      ...
      expect(zshenv).toContain('if [[ -n "${_dobius_zshenv_source_dir:-}"')
    })
    ...
    it('handles ZDOTDIR with single quote in path', async () => {
      ...
      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`DOBIUS_ORIG_ZDOTDIR=${quotePath}`)
    })
    ...
    it('does not evaluate command substitution in ZDOTDIR', async () => {
      ...
      expect(result.stdout).toContain(`DOBIUS_ORIG_ZDOTDIR=${safePath}`)
    })
    ...
    it('handles ZDOTDIR with multiple trailing slashes', async () => {
      ...
      expect(result.stdout).toContain(`DOBIUS_ORIG_ZDOTDIR=${cleanPath}`)
    })
  })
  ...
  describeIfZsh('terminal emulator edge cases', () => {
    ...
    beforeEach(async () => {
      testHome = mkdtempSync(join(tmpdir(), 'dobius-term-'))
      userDataPath = mkdtempSync(join(tmpdir(), 'dobius-term-userdata-'))
      getUserDataPathMock.mockReturnValue(userDataPath)
    })
    ...
    it('handles sudo -E where HOME and ZDOTDIR mismatch', async () => {
      ...
      try {
        ...
        expect(config.env.DOBIUS_ORIG_ZDOTDIR).toBe(userZdotdir)
      } finally {
        if (previousZdotdir === undefined) {
          delete process.env.ZDOTDIR
        } else {
    ...
    it('re-discovers ZDOTDIR despite stale DOBIUS_ORIG_ZDOTDIR from previous session', async () => {
      ...
      const previousDobiusZdotdir = process.env.DOBIUS_ORIG_ZDOTDIR
      process.env.DOBIUS_ORIG_ZDOTDIR = '/opt/dobius-old/shell-ready/zsh' // stale wrapper path
      ...
      try {
        ...
        const cleanEnv: Record<string, string | undefined> = {
          ...process.env,
          HOME: testHome,
          DOBIUS_ORIG_ZDOTDIR: '/opt/dobius-old/shell-ready/zsh'
        }
        delete cleanEnv.ZDOTDIR
        cleanEnv.ZDOTDIR = config.env.ZDOTDIR
        ...
        expect(result.stdout).toContain(`DOBIUS_ORIG_ZDOTDIR=${currentZdotdir}`)
      } finally {
        if (previousDobiusZdotdir === undefined) {
          delete process.env.DOBIUS_ORIG_ZDOTDIR
        } else {
          process.env.DOBIUS_ORIG_ZDOTDIR = previousDobiusZdotdir
        }
      }
    })

    it('prioritizes fresh discovery over inherited DOBIUS_ORIG_ZDOTDIR', async () => {
      ...
      process.env.DOBIUS_ORIG_ZDOTDIR = oldZdotdir
      ...
      try {
        ...
        const cleanEnv: Record<string, string | undefined> = {
          ...process.env,
          HOME: testHome,
          DOBIUS_ORIG_ZDOTDIR: oldZdotdir
        }
        delete cleanEnv.ZDOTDIR
        cleanEnv.ZDOTDIR = config.env.ZDOTDIR
        ...
        expect(result.stdout).toContain(`DOBIUS_ORIG_ZDOTDIR=${freshZdotdir}`)
      } finally {
        if (previousDobiusZdotdir === undefined) {
          delete process.env.DOBIUS_ORIG_ZDOTDIR
        } else {
    ...
    it('sources launch-time ZDOTDIR .zshenv when it is explicitly inherited', async () => {
      ...
      try {
        const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
        const config = getShellReadyLaunchConfig('/bin/zsh')
        expect(config.env.DOBIUS_ZSHENV_SOURCE_DIR).toBe(inheritedZdotdir)
        ...
        const result = spawnSync(
          ...
          [
            '-c',
            'echo "DOBIUS_ORIG_ZDOTDIR=${DOBIUS_ORIG_ZDOTDIR}" && echo "SOURCE_MARKER=${SOURCE_MARKER:-unset}"'
          ],
          {
            env: cleanEnv as NodeJS.ProcessEnv,
            encoding: 'utf8'
        ...
        expect(result.status).toBe(0)
        expect(result.stdout).toContain(`DOBIUS_ORIG_ZDOTDIR=${inheritedZdotdir}`)
  ...
  describeIfZsh('automation and edge cases', () => {
    ...
    beforeEach(async () => {
      testHome = mkdtempSync(join(tmpdir(), 'dobius-auto-'))
      userDataPath = mkdtempSync(join(tmpdir(), 'dobius-auto-userdata-'))
      getUserDataPathMock.mockReturnValue(userDataPath)
    })
    ...
    it('handles empty HOME gracefully', async () => {
      ...
      expect(result.status).toBe(0)
      // Empty HOME falls back to empty DOBIUS_ORIG_ZDOTDIR
      expect(result.stdout).toContain('DOBIUS_ORIG_ZDOTDIR=\n')
    })
    ...
    it('handles unset HOME gracefully', async () => {
      ...
      expect(result.stdout).toMatch(/DOBIUS_ORIG_ZDOTDIR=.+/)
    })
