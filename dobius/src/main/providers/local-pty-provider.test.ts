...
vi.mock('electron', () => ({
  app: {
    getPath: vi.fn(() => '/tmp/dobius-user-data')
  }
}))
  ...
  describe('spawn', () => {
    ...
    it('uses fallback shell readiness when startup-command shell spawn falls back', async () => {
      ...
      try {
        ...
        dataCallback('\x1b]777;dobius-shell-ready\x07user@host % ')
        await Promise.resolve()
    ...
    it('releases held marker-prefix bytes when local shell readiness times out', async () => {
      ...
      try {
        ...
        dataCallback('\x1b]777;dobius-shell-ready')
    ...
    it('honors explicit terminal env overrides after deleting requested defaults', async () => {
      provider.configure({
        buildSpawnEnv: (_id, env) => {
          env.TERM_PROGRAM = 'Dobius'
          env.DOBIUS_ATTRIBUTION_SHIM_DIR = '/tmp/dobius-attribution'
          env.PATH = `/tmp/dobius-attribution:${env.PATH ?? ''}`
          return env
        }
      })
      ...
      expect(spawnCall[2].env.PATH.split(':')[0]).toBe('/tmp/dobius-agent-teams-bin')
      expect(spawnCall[2].env.TERM_PROGRAM).toBeUndefined()
      expect(spawnCall[2].env.DOBIUS_ATTRIBUTION_SHIM_DIR).toBeUndefined()
    })
    ...
    it('does not inherit AppImage runtime env into Linux PTY shells', async () => {
      ...
      process.env.LD_LIBRARY_PATH = ['/tmp/.mount_dobius123/usr/lib', '/opt/audio/lib'].join(
        delimiter
      )
    ...
    it('uses shell wrapper when MiMo home must survive shell startup', async () => {
      provider.configure({
        buildSpawnEnv: (_id, env) => {
          env.MIMOCODE_HOME = '/tmp/dobius-mimocode-overlay'
          env.DOBIUS_MIMOCODE_HOME = '/tmp/dobius-mimocode-overlay'
          return env
        }
      })
      ...
      expect(spawnCall[2].env.DOBIUS_SHELL_READY_MARKER).toBe('0')
    })
    ...
    it('does not pass a Windows Codex home into WSL terminals', async () => {
      ...
      provider.configure({
        buildSpawnEnv: (_id, env) => {
          ...
          env.DOBIUS_CODEX_HOME = 'C:\\Users\\jin\\.codex'
          return env
        }
      })
      ...
      expect(spawnCall[2].env.DOBIUS_CODEX_HOME).toBeUndefined()
    })
    ...
    it('translates a WSL managed Codex home before launching a WSL terminal', async () => {
      ...
      const spawnCall = spawnMock.mock.calls.at(-1)!
      expect(spawnCall[0]).toBe('wsl.exe')
      expect(spawnCall[2].env.CODEX_HOME).toBe('/home/jin/.local/share/dobius/codex-accounts/a/home')
      expect(spawnCall[2].env.DOBIUS_CODEX_HOME).toBe(
        '/home/jin/.local/share/dobius/codex-accounts/a/home'
      )
      expect(spawnCall[2].env.WSLENV).toContain('CODEX_HOME')
      expect(spawnCall[2].env.WSLENV).toContain('DOBIUS_CODEX_HOME')
    })
    ...
    it('uses the preferred WSL distro for Windows cwd WSL terminals', async () => {
      ...
      expect(spawnCall[1][5]).toContain('exec "\\$_dobius_wsl_shell" -l')
    ...
    it('marks Dobius terminal handle for WSL import when buildSpawnEnv opts in', async () => {
      Object.defineProperty(process, 'platform', { configurable: true, value: 'win32' })
      ...
      provider.configure({
        buildSpawnEnv: (_id, env, ctx) => {
          env.DOBIUS_TERMINAL_HANDLE = 'term_wsl'
          if (ctx?.isWsl) {
            env.WSLENV = 'DOBIUS_TERMINAL_HANDLE/u'
          }
          return env
        }
      })
      ...
      expect(spawnCall[2].env.WSLENV).toBe(
        'DOBIUS_TERMINAL_HANDLE/u:POWERLEVEL9K_DISABLE_CONFIGURATION_WIZARD'
      )
    })
    ...
    it('does not inherit parent Dobius pane identity when caller omits pane env', async () => {
      const saved = {
      ...
      const spawnCall = spawnMock.mock.calls.at(-1)!
      expect(spawnCall[2].env.DOBIUS_PANE_KEY).toBeUndefined()
      expect(spawnCall[2].env.DOBIUS_TAB_ID).toBeUndefined()
      expect(spawnCall[2].env.DOBIUS_WORKTREE_ID).toBeUndefined()
    })

    it('preserves explicit child Dobius pane identity over parent env', async () => {
      const saved = {
        DOBIUS_PANE_KEY: process.env.DOBIUS_PANE_KEY,
        DOBIUS_TAB_ID: process.env.DOBIUS_TAB_ID,
        DOBIUS_WORKTREE_ID: process.env.DOBIUS_WORKTREE_ID
      ...
      const spawnCall = spawnMock.mock.calls.at(-1)!
      expect(spawnCall[2].env.DOBIUS_PANE_KEY).toBe('child-tab:child-leaf')
      expect(spawnCall[2].env.DOBIUS_TAB_ID).toBe('child-tab')
      expect(spawnCall[2].env.DOBIUS_WORKTREE_ID).toBe('child-worktree')
    })
