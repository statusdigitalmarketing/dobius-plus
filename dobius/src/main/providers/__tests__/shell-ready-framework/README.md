...
EOF
    ...
    zsh -c 'env | grep -E "^(DOBIUS_|ZDOTDIR|HOME)=" | sort'
  `)
...
1. **Creates temp directories** for `$HOME` and Dobius's `userDataPath`

2. **Splits the script** on the `# Run:` marker:
   - Lines before the marker → setup commands
   - Lines after the marker → run command to test

3. **Gets Dobius's wrapper config** by calling `getShellReadyLaunchConfig()`
...
**Note**: The test framework applies Dobius's wrapper configuration (sets `ZDOTDIR` to wrapper directory, etc.). When running manually, you'll see different output unless you also configure the wrapper environment.
