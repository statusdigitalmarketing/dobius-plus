...
export class LocalPtyProvider implements IPtyProvider {
  ...
  async spawn(args: PtySpawnOptions): Promise<PtySpawnResult> {
    ...
    const spawnEnv: Record<string, string> = {
      ...
      // autodetection, bat/delta paging hints). Sourced from DOBIUS_APP_VERSION
    ...
    // Why: Dobius can be launched from an Dobius terminal while developing. Pane
      ...
      if (pathWin32.basename(shellPath).toLowerCase() === 'wsl.exe') {
        if (codexHomeWslInfo) {
          if (launchWslDistro && launchWslDistro !== codexHomeWslInfo.distro) {
            ...
            delete finalEnv.DOBIUS_CODEX_HOME
          } else {
            finalEnv.CODEX_HOME = codexHomeWslInfo.linuxPath
            finalEnv.DOBIUS_CODEX_HOME = codexHomeWslInfo.linuxPath
            // Why: wsl.exe only imports non-default env vars named in WSLENV.
            addWslEnvKeys(finalEnv, ['CODEX_HOME', 'DOBIUS_CODEX_HOME'])
            if (!launchWslDistro) {
              const resolved = resolveWindowsShellLaunchArgs(shellPath, cwd, defaultCwd, {
    ...
    if (!wslInfo && process.platform !== 'win32') {
      ...
      const needsNoMarkerWrapper =
        finalEnv.DOBIUS_ATTRIBUTION_SHIM_DIR ||
        finalEnv.DOBIUS_OPENCODE_CONFIG_DIR ||
        finalEnv.DOBIUS_MIMOCODE_HOME ||
        finalEnv.DOBIUS_OMP_STATUS_EXTENSION ||
        finalEnv.DOBIUS_CODEX_HOME ||
        finalEnv.DOBIUS_AGENT_TEAMS_SHIM_DIR
    ...
    if (finalEnv.DOBIUS_TERMINAL_HANDLE) {
      ptyTerminalHandle.set(id, finalEnv.DOBIUS_TERMINAL_HANDLE)
    }
