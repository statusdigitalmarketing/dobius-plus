...
describe('SshGitProvider', () => {
  ...
  it('reports an actionable reconnect message when the relay lacks submodule status', async () => {
    ...
    await expect(provider.getSubmoduleStatus('/home/user/repo', 'vendor/lib')).rejects.toThrow(
      'SSH submodule diff support is unavailable on this relay. Reconnect the SSH target to update Dobius on the host, then try again.'
    )
  })
  ...
  it('reports an actionable reconnect message when the relay does not support cloning', async () => {
    ...
    await expect(
      ...
      'SSH clone support is unavailable on this relay. Reconnect the SSH target to update Dobius on the host, then try again.'
    )
  })
  ...
  it('execNonInteractive forwards environment variables to the relay', async () => {
    ...
    await provider.execNonInteractive(
      '/bin/bash',
      ['-lc', 'echo "$DOBIUS_WORKTREE_PATH"'],
      '/home/user/repo',
      120_000,
      undefined,
      {
        DOBIUS_ROOT_PATH: '/home/user/repo',
        DOBIUS_WORKTREE_PATH: '/home/user/repo-feature'
      }
    )

    expect(mux.request).toHaveBeenCalledWith('agent.execNonInteractive', {
      binary: '/bin/bash',
      args: ['-lc', 'echo "$DOBIUS_WORKTREE_PATH"'],
      cwd: '/home/user/repo',
      stdin: null,
      timeoutMs: 120_000,
      env: {
  ...
  it('syncForkDefaultBranch sends git.forkSync request', async () => {
    ...
    const expectedUpstream = { owner: 'statusdigitalmarketing', repo: 'dobius' }
  ...
  it('forceDeletePreservedBranch maps old relays to the reconnect message', async () => {
    ...
    await expect(
      ...
      'This SSH host is running an older Dobius relay that cannot delete preserved branches. Reconnect to deploy the latest relay, then try again.'
    )
  })
