...
describe('resolveWindowsShellLaunchArgs', () => {
  ...
  it('returns PowerShell args that install OSC 133 bootstrap after normal profile loading', () => {
    ...
    const opencodeRestoreIndex = command.indexOf(
      '$env:OPENCODE_CONFIG_DIR = $env:DOBIUS_OPENCODE_CONFIG_DIR'
    )
    const ompWrapperIndex = command.indexOf('function Global:omp')
    const ompExtensionIndex = command.indexOf('--extension $env:DOBIUS_OMP_STATUS_EXTENSION')
    const codexRestoreIndex = command.indexOf('$env:CODEX_HOME = $env:DOBIUS_CODEX_HOME')
    const promptIndex = command.indexOf('function Global:prompt')

    expect(command).not.toContain('$PROFILE')
    expect(command).not.toContain('DOBIUS_PI_CODING_AGENT_DIR')
    expect(command).not.toContain('DOBIUS_OMP_CODING_AGENT_DIR')
    expect(command).not.toContain('$env:PI_CODING_AGENT_DIR = $env:DOBIUS_OMP_SOURCE_AGENT_DIR')
  ...
  it('preserves complex PowerShell startup command text through EncodedCommand', () => {
    const startupCommand =
      '& "C:\\Program Files\\Dobius CLI\\dobius.exe" "--label" "quoted value"; $env:DOBIUS_VALUE = "nested"'
    const result = resolveWindowsShellLaunchArgs(
      'powershell.exe',
      'C:\\Users\\alice',
      'C:\\Users\\alice',
  ...
  it('escapes single quotes when translating a WSL cwd', () => {
    ...
    expect(result.shellArgs[3]).toContain('exec "\\$_dobius_wsl_shell" -l')
  })
