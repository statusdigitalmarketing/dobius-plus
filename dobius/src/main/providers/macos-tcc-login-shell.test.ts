...
describe('wrapShellSpawnForMacosTccAttribution', () => {
  ...
  beforeEach(() => {
    ...
    origDisable = process.env.DOBIUS_DISABLE_MACOS_LOGIN_SHELL
  ...
  afterEach(() => {
    ...
    if (origDisable === undefined) {
      ...
      process.env.DOBIUS_DISABLE_MACOS_LOGIN_SHELL = origDisable
    }
    vi.clearAllMocks()
  })
  ...
  it('keeps bash rcfile args intact after the shell path', () => {
    setPlatform('darwin')
    expect(
      wrapShellSpawnForMacosTccAttribution('/bin/bash', ['--rcfile', '/dobius/bash/rcfile'])
    ).toEqual({
      file: '/usr/bin/login',
      args: [
        '-flpq',
