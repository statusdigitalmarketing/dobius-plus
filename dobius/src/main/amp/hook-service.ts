...
const AMP_PLUGIN_FILE = 'dobius-agent-status.ts'
const AMP_PLUGIN_MARKER = 'Managed by Dobius. Do not edit; changes may be overwritten.'
  ...
  switch (state.kind) {
    ...
    case 'unmanaged':
      return {
        ...
        configPath: pluginPath,
        managedHooksPresent: false,
        detail: 'Amp Dobius status plugin exists but is not Dobius-managed'
      }
    case 'error':
      return {
        agent: 'amp',
...
function getAmpPluginSource(): string {
  return [
    ...
    '  const endpointPath = process.env.DOBIUS_AGENT_HOOK_ENDPOINT',
    ...
    "      if (match[1] === 'DOBIUS_AGENT_HOOK_PORT') out.port = value",
    "      if (match[1] === 'DOBIUS_AGENT_HOOK_TOKEN') out.token = value",
    "      if (match[1] === 'DOBIUS_AGENT_HOOK_ENV') out.env = value",
    "      if (match[1] === 'DOBIUS_AGENT_HOOK_VERSION') out.version = value",
    '    }',
    ...
    "      console.warn('[dobius-hook] failed to parse Amp endpoint file:', (error as Error).message)",
    '    }',
    '    return null',
    '  }',
    '}',
    ...
    '  // Why: Amp sessions can outlive an Dobius restart; the endpoint file is',
    '  // rewritten on each start, so read it per event before falling back to env.',
    '  const fileEnv = readEndpointFile() ?? {}',
    '  return {',
    '    port: fileEnv.port || process.env.DOBIUS_AGENT_HOOK_PORT,',
    '    token: fileEnv.token || process.env.DOBIUS_AGENT_HOOK_TOKEN,',
    '    env: fileEnv.env || process.env.DOBIUS_AGENT_HOOK_ENV || "",',
    '    version: fileEnv.version || process.env.DOBIUS_AGENT_HOOK_VERSION || ""',
    '  }',
    '}',
    '',
    ...
    '    })',
    '  } catch {',
    '    // Why: Dobius status reporting must never affect the Amp run.',
    '  } finally {',
    ...
    '  // payload closures when Dobius is down and each POST waits for timeout.',
