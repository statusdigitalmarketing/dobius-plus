...
function configuredApiBaseUrl(repo: GiteaRepoRef): string {
  const configured = readProviderEnv('GITEA_API_BASE_URL')
...
export function isGiteaReviewCreationAuthenticated(): boolean {
  return readProviderEnv('GITEA_TOKEN') !== null
}

function authHeaders(): Record<string, string> {
  const token = readProviderEnv('GITEA_TOKEN')
...
function classifyCreateError(error: unknown): CreateHostedReviewResult {
  ...
  if (
    ...
    return {
      ...
      code: 'auth_required',
      error:
        'Create PR failed: Gitea is not authenticated. Next step: set DOBIUS_GITEA_TOKEN in this environment.'
    }
  }
