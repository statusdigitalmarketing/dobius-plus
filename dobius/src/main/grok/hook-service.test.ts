...
describe('GrokHookService', () => {
  ...
  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'dobius-grok-home-'))
  ...
  it('installs a dedicated global Grok hook config and managed script', () => {
    ...
    expect(status.configPath).toBe(join(homeDir, '.grok', 'hooks', 'dobius-status.json'))
    expect(status.managedHooksPresent).toBe(true)

    const config = JSON.parse(
      readFileSync(join(homeDir, '.grok', 'hooks', 'dobius-status.json'), 'utf8')
    ) as {
    ...
    const script = readFileSync(
      join(homeDir, '.dobius', 'agent-hooks', GROK_SCRIPT_FILE_NAME),
      'utf8'
    )
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'dobius grok home with spaces')
      ...
      try {
        ...
        const config = JSON.parse(
          readFileSync(join(spaceHome, '.grok', 'hooks', 'dobius-status.json'), 'utf8')
  ...
  it('preserves user-authored hook entries in the Dobius Grok config file', () => {
    const configPath = join(homeDir, '.grok', 'hooks', 'dobius-status.json')
