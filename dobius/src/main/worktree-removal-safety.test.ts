...
describe('canCleanupUnregisteredDobiusLeftoverDirectory', () => {
  const repo = { path: '/repos/main' }
  const ownedMeta = { dobiusCreatedAt: 1, dobiusCreationSource: 'runtime' as const }
  const baseArgs = {
    meta: ownedMeta,
    worktreePath: '/workspaces/dobius-owned',
    runtimeWorktreePath: '/workspaces/dobius-owned',
    repo,
  ...
  it('rejects unregistered existing targets that are files or symlinks', async () => {
    ...
    await expect(
      canCleanupUnregisteredDobiusLeftoverDirectory({
        ...baseArgs,
        statPath: makeStatPath(['/workspaces/dobius-owned']),
        isGitRepository
      })
    ).resolves.toBe(false)
    await expect(
      canCleanupUnregisteredDobiusLeftoverDirectory({
        ...baseArgs,
        statPath: async (path) => {
          if (path === '/workspaces/dobius-owned') {
            return { type: 'symlink' }
          }
          throw missingPath(path)
        },
  ...
  it('rejects unregistered leftover directories with a .git marker', async () => {
    ...
    await expect(
      canCleanupUnregisteredDobiusLeftoverDirectory({
        ...baseArgs,
        statPath: makeStatPath(['/workspaces/dobius-owned/.git'], ['/workspaces/dobius-owned']),
        isGitRepository
      })
    ).resolves.toBe(false)
  ...
  it('rejects no-marker cleanup when only the Dobius path shape matches', async () => {
    const isGitRepository = vi.fn().mockResolvedValue(false)
  ...
  it('rejects unregistered leftover directories that still answer git status', async () => {
    ...
    expect(isGitRepository).toHaveBeenCalledWith('/workspaces/dobius-owned')
  })
    ...
    await expect(
      canCleanupUnregisteredDobiusLeftoverDirectory({
        ...
        registeredWorktrees: [
          ...
          makeGitWorktree('/workspaces/dobius-owned/child')
        ],
        statPath: makeStatPath([], ['/workspaces/dobius-owned']),
        isGitRepository: vi.fn().mockResolvedValue(false)
      })
    ).rejects.toThrow(
      'Refusing to delete worktree because it contains another registered worktree: /workspaces/dobius-owned/child'
    )
  })
  ...
  it('uses runtime paths for filesystem proof and original paths for nested worktree checks', async () => {
    const statPath = vi.fn(async (path: string) => {
      if (path === '/mnt/c/workspaces/dobius-owned') {
        return { type: 'directory' }
      }
      throw missingPath(path)
    })
    ...
    await expect(
      canCleanupUnregisteredDobiusLeftoverDirectory({
        ...baseArgs,
        worktreePath: 'C:\\workspaces\\dobius-owned',
        runtimeWorktreePath: '/mnt/c/workspaces/dobius-owned',
        repo: { path: 'C:\\repos\\main' },
        runtimeRepoPath: '/mnt/c/repos/main',
        registeredWorktrees: [
          makeGitWorktree('C:\\repos\\main', true),
          makeGitWorktree('C:\\workspaces\\dobius-owned-sibling')
        ],
        statPath,
        isGitRepository
      })
    ...
    expect(statPath).toHaveBeenCalledWith('/mnt/c/workspaces/dobius-owned')
    expect(statPath).toHaveBeenCalledWith('/mnt/c/workspaces/dobius-owned/.git')
    expect(statPath).not.toHaveBeenCalledWith('C:\\workspaces\\dobius-owned')
    expect(isGitRepository).toHaveBeenCalledWith('/mnt/c/workspaces/dobius-owned')
  })
  ...
  it('rejects translated-runtime cleanup when original path contains a registered child', async () => {
    await expect(
      canCleanupUnregisteredDobiusLeftoverDirectory({
        ...
        registeredWorktrees: [
          makeGitWorktree('C:\\repos\\main', true),
          makeGitWorktree('C:\\workspaces\\dobius-owned\\child')
        ],
        statPath: makeStatPath([], ['/mnt/c/workspaces/dobius-owned']),
        isGitRepository: vi.fn().mockResolvedValue(false)
      })
    ).rejects.toThrow(
      'Refusing to delete worktree because it contains another registered worktree: C:\\workspaces\\dobius-owned\\child'
    )
  })
})
