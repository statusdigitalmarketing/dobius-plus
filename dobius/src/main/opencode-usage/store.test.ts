...
describe('OpenCodeUsageStore', () => {
  ...
  it('reports no data for Dobius scope when only non-Dobius OpenCode usage exists', async () => {
  ...
  it('uses recorded OpenCode costs and token totals without model pricing inference', async () => {
    ...
    const summary = await store.getSummary('dobius', '30d')
    const daily = await store.getDaily('dobius', '30d')
  ...
  it('returns recent sessions with OpenCode event and token fields', async () => {
    ...
    const sessions = await store.getRecentSessions('dobius', '30d', 5)
