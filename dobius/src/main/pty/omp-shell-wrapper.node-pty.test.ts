...
function makeTempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), 'dobius-omp-node-pty-'))
  tempDirs.push(dir)
  return dir
}
...
function writeFakeOmp(binDir: string): void {
  ...
  writeFileSync(
    ompPath,
    `#!/bin/sh
agent_dir="\${PI_CODING_AGENT_DIR:-\${DOBIUS_FAKE_OMP_DEFAULT_DIR:-}}"
...
describePosix('OMP shell wrapper node-pty reproduction', () => {
  ...
  itWithBash('reproduces why restored shells miss OMP status without the wrapper', async () => {
    ...
    const statusExtension = join(extensionDir, 'dobius-agent-status.ts')
    ...
    const makeEnv = (captureFile: string, afterPiFile: string): Record<string, string> => ({
      ...
      DOBIUS_OMP_STATUS_EXTENSION: statusExtension,
      DOBIUS_FAKE_OMP_DEFAULT_DIR: ompDir,
      DOBIUS_CAPTURE_FILE: captureFile,
      DOBIUS_AFTER_PI_FILE: afterPiFile,
    ...
    await runInteractiveBashPty({
      ...
      input: `omp ask
printf '%s' "$PI_CODING_AGENT_DIR" > "$DOBIUS_AFTER_PI_FILE"
exit 0
`
    })
  ...
  itWithBash('runs OMP config subcommands without redirecting the home', async () => {
    ...
    await runInteractiveBashPty({
      ...
      env: {
        ...
        DOBIUS_PI_CODING_AGENT_DIR: '',
        DOBIUS_OMP_CODING_AGENT_DIR: '',
        DOBIUS_OMP_SOURCE_AGENT_DIR: sourceDir,
        DOBIUS_OMP_STATUS_EXTENSION: statusExtension,
        DOBIUS_FAKE_OMP_DEFAULT_DIR: sourceDir,
        DOBIUS_CAPTURE_FILE: captureFile,
        TERM: process.env.TERM || 'xterm-256color'
      },
      input: `omp config
    ...
    async () => {
      ...
      await runInteractiveBashPty({
        ...
        env: {
          ...
          DOBIUS_OMP_CODING_AGENT_DIR: '',
          DOBIUS_OMP_STATUS_EXTENSION: statusExtension,
          DOBIUS_FAKE_OMP_DEFAULT_DIR: defaultOmpDir,
          DOBIUS_CAPTURE_FILE: captureFile,
          TERM: process.env.TERM || 'xterm-256color'
        },
        input: `omp config
