...
describe.runIf(process.platform === 'win32')('patched node-pty loader override', () => {
  it('loads the conpty binding from DOBIUS_NODE_PTY_NATIVE_DIR', () => {
    ...
    const previousNativeDir = process.env.DOBIUS_NODE_PTY_NATIVE_DIR
    process.env.DOBIUS_NODE_PTY_NATIVE_DIR = destDir!
    try {
      ...
      if (previousNativeDir === undefined) {
        delete process.env.DOBIUS_NODE_PTY_NATIVE_DIR
      } else {
        process.env.DOBIUS_NODE_PTY_NATIVE_DIR = previousNativeDir
      }
    }
  })
})
