...
// Why: Dobius launches bash as a login shell (see local-pty-shell-ready.ts
...
// .bashrc. Scanning .bashrc would mirror values the live Dobius bash never sees.
...
function shellStartupFilePaths(home: string, shell: string | undefined): readonly string[] {
  if (!shell) {
    // Why: Dobius's POSIX default shell is /bin/zsh when $SHELL is unset.
  ...
  // use Dobius's zsh/bash shell-ready startup files, so scanning those files
 ...
 *
 * Why: GUI-launched Dobius does not inherit interactive shell exports, but the
