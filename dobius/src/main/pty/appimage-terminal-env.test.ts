...
describe('removeAppImageRuntimeEnv', () => {
  ...
  it('removes AppImage runtime identity and mount paths from Linux terminal env', () => {
    ...
    const env = {
      APPIMAGE: '/data/apps/dobius.appimage',
      APPDIR: '/tmp/.mount_dobius123',
      ARGV0: '/data/apps/dobius.appimage',
      OWD: '/home/user',
      APPIMAGE_LIBRARY_PATH: '/tmp/.mount_dobius123/usr/lib',
      PATH: ['/tmp/.mount_dobius123', '/tmp/.mount_dobius123/usr/sbin', '/usr/bin'].join(delimiter),
      LD_LIBRARY_PATH: ['/tmp/.mount_dobius123/usr/lib', '/opt/audio/lib'].join(delimiter),
      HOME: '/home/user'
    }
  ...
  it('leaves AppImage-looking env untouched outside Linux', () => {
    ...
    const env = {
      ...
      APPDIR: '/tmp/.mount_dobius123',
      ARGV0: '/data/apps/dobius.appimage',
      PATH: ['/tmp/.mount_dobius123/usr/bin', '/usr/bin'].join(delimiter)
    }
