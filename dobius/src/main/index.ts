/* eslint-disable max-lines -- Why: this is Dobius's main-process entry point;
...
import { DobiusRuntimeService } from './runtime/dobius-runtime'
import { DobiusRuntimeRpcServer } from './runtime/runtime-rpc'
import { awaitRuntimeFileWatcherUnsubscribes } from './runtime/dobius-runtime-files'
...
import {
  ...
  configureDobiusUserDataPathEnv,
...
import { migrateLegacyDobiusConfig } from './migrate-legacy-dobius-config'
...
import { startImessageBridge, stopImessageBridge } from './imessage-bridge/bridge-service'
import { startDobiusCli, stopDobiusCli } from './dobius-cli/dispatch-server'
import { syncVoiceConductorFromSettings } from './voice-conductor/service'
...
import { installLinuxBareDobiusDispatcher } from './cli/linux-bare-dobius-dispatcher'
...
let runtime: DobiusRuntimeService | null = null
let rateLimits: RateLimitService | null = null
let runtimeRpc: DobiusRuntimeRpcServer | null = null
...
// track them so Dobius can move this build onto software rendering.
...
// Why: on Windows a CLI-shaped launch (Dobius.exe <unpacked CLI entry>) that lost
...
function maybeAutoRenameBranchOnFirstWorkFromHook(event: {
  ...
  void maybeAutoRenameBranchOnFirstWork(
    ...
    },
    {
      ...
      canRenameDobiusCreatedBranch: (worktreeId) => {
        ...
        // Only worktrees Dobius stamped at creation are safe to auto-rename.
        return !!meta?.dobiusCreationSource && meta.preserveBranchOnDelete !== true
      },
...
installUncaughtPipeErrorGuard()
// Why: propagate the Dobius app version into `process.env` so PTY-env
...
process.env.DOBIUS_APP_VERSION = app.getVersion()
patchPackagedProcessPath()
...
if (startupDiagnosticsEnabled) {
  logStartupDiagnostic('before-single-instance-lock', {
    ...
    e2eUserData: Boolean(process.env.DOBIUS_E2E_USER_DATA_DIR)
  })
...
// dev (`dobius-dev`) and packaged (`dobius`) runs lock in separate namespaces
...
// `pnpm dev` exit silently. In dev we accept that `dobius-runtime.json` may race
// (the bundled `dobius-dev` CLI routes to whichever instance wrote last). Agent
// hook endpoint files are namespaced per dev instance when the hook server
// starts below. Packaged Dobius keeps the lock to protect against the corruption
...
if (hasSingleInstanceLock) {
  ...
  // `dobius serve` may be launched through a CLI shim or background shell whose
  ...
  // dobius-dev in dev mode) but before app.setName('Dobius') inside whenReady
  ...
  // and the daemon it forks both pick up DOBIUS_NODE_PTY_NATIVE_DIR.
...
function startDesktopFirstWindowStartupServices(): Promise<void> {
  ...
  const startupServices = startFirstWindowStartupServices({
    // Why: the persistent-terminal daemon is desktop-only. Headless `dobius serve`
    ...
    // Why: PTY spawn env reads DOBIUS_AGENT_HOOK_* from the live server state, so
    ...
    startAgentHookServer: async () => {
      ...
      await agentHookServer.start({
        ...
        // env still reaches the current Dobius process after an app restart.
        // Dev uses a namespace because all worktrees share `dobius-dev`.
    ...
    onAgentHookServerError: (error) => {
      ...
      // enrichment only. Dobius must still boot if the loopback receiver fails.
...
// Why: tray "Open Dobius" / left-click restores the window the close handler may
...
async function presentRendererRecoveryPrompt(recentRecoveryCount: number): Promise<void> {
  ...
  const options = {
    ...
    defaultId: 0,
    cancelId: 1,
    title: 'Dobius+ keeps failing to load',
    message: 'The app window crashed repeatedly and stopped reloading automatically.',
    detail: `Dobius tried to recover ${recentRecoveryCount} times in a row without success. This is often a graphics-driver or installation problem. Reload to try again, or quit and relaunch Dobius.`
  }
...
async function printServeReady(options: ServeOptions): Promise<void> {
  ...
  console.log(`Dobius server ready: ${endpoint ?? 'websocket unavailable'}`)
  if (pairing.available) {
...
function installServeSignalHandlers(): void {
  const quit = (): void => {
    // Why: foreground `dobius serve` is controlled by the parent CLI/terminal,
...
app.whenReady().then(async () => {
  ...
  store = new Store()
  await migrateLegacyDobiusConfig(store)
  ...
  // Honors DO_NOT_TRACK / DOBIUS_TELEMETRY_DISABLED / DOBIUS_DIAGNOSTICS_DISABLED
  ...
  const runtimeService = new DobiusRuntimeService(store, stats, {
  ...
  runtimeService.setCommitMessageAgentEnvironmentResolvers({
    // Why: local Codex hooks and auth now live in Dobius's managed runtime home
    // even for the system-default path, so every Dobius-launched Codex process
    // must resolve CODEX_HOME through the runtime-home service.
    prepareForCodexLaunch: prepareCodexRuntimeHomeForLaunch,
    prepareForClaudeLaunch: (target) => claudeRuntimeAuth!.prepareForClaudeLaunch(target)
  })
  starNag = new StarNagService(store, stats)
  // Dobius+ rebrand: do not run the "star Dobius on GitHub" nag (promotes upstream).
  starNag.registerIpcHandlers()
  // Why: the bridge types incoming iMessage commands into the active terminal,
  // so it needs the runtime's terminal resolution + send path. macOS-only
  // (chat.db + osascript); the service no-ops on other platforms.
  startImessageBridge(runtimeService, () => claudeRuntimeAuth!.prepareForClaudeLaunch())
  // Localhost dispatch server + dobius-* CLIs (dobius-send, dobius-task-done).
  // Reuses the runtime's active-terminal resolution + send path.
  startDobiusCli(runtimeService)
  // Voice Conductor: background Opus session that routes voice/dispatched work.
  // Starts/stops in step with the voice.conductorEnabled setting. The iMessage
  // handle (for dobius-ask confirmations) comes from env until it has a setting.
  ...
  // Prompt snippets → inject into the active terminal (needs the runtime).
  ...
  // Why: relaunch Dobius+ after an OS restart so the session daemon comes back
  // and terminals cold-restore with their latest context. See launch-at-login.ts.
  applyLaunchAtLogin()
  if (shouldInstallManagedHooks(is.dev)) {
    // Why: the persisted off switch must run before any auto-install path so
    // users who removed Dobius-managed hooks do not see them silently reappear on launch.
    if (isAgentStatusHooksEnabled(store.getSettings())) {
  ...
  const isE2E = Boolean(process.env.DOBIUS_E2E_USER_DATA_DIR)
  // Why: a developer running `pnpm dev` while the packaged Dobius is also open
  ...
  runtimeRpc = new DobiusRuntimeRpcServer({
    runtime,
  ...
  if (serveOptions) {
    ...
    // Why: the dobius CLI command is normally installed by the renderer onboarding /
    // Settings "Install CLI" flow via the cli:install IPC. Headless serve has no
    // renderer, so the command is never created and an in-terminal `dobius …` fails
    ...
    // `dobius` in /usr/local/bin or ~/.local/bin) and Linux (`dobius-ide`; bare `dobius`
    ...
    if (process.platform === 'darwin' || process.platform === 'linux') {
      try {
        ...
        console.log(
          `[serve] dobius CLI install: ${cliStatus.state}${cliStatus.commandPath ? ` (${cliStatus.commandPath})` : ''}`
        )
      } catch (error) {
        console.warn(
          '[serve] dobius CLI install skipped:',
          error instanceof Error ? error.message : String(error)
        )
      }
    }
    // Why: headless serve needs the Dobius dispatcher on ~/.local/bin so
    // `dobius claude-teams` resolves from the managed terminal PATH.
    if (process.platform === 'linux' && app.isPackaged && process.resourcesPath) {
      try {
        const dispatcher = await installLinuxBareDobiusDispatcher({
          resourcesPath: process.resourcesPath
        })
        console.log(
          `[serve] bare dobius dispatcher ${dispatcher.state}: ${dispatcher.dispatcherPath}` +
            `${dispatcher.target ? ` -> ${dispatcher.target}` : ''}`
        )
      } catch (error) {
        console.warn(
          '[serve] bare dobius dispatcher install skipped:',
          error instanceof Error ? error.message : String(error)
        )
      }
    }
...
app.on('will-quit', (e) => {
  ...
  stats?.flush()
  // Why: agent-browser daemon processes would otherwise linger after Dobius quits,
...
app.on('window-all-closed', () => {
  // Why: headless `dobius serve` has no desktop window, and offscreen browser
