...
import {
  ...
  readConfigFromDobiusOverlapDetail,
...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  if (target === 'local' && process.platform === 'win32') {
    return [
      '@echo off',
      'setlocal',
      // Why: the endpoint file holds the *live* port/token for this Dobius
      // install. A PTY that survived an Dobius restart has stale PORT/TOKEN
      ...
      // is missing (first run / pre-endpoint-file / running outside Dobius).
  ...
  return [
    '#!/bin/sh',
    // Why: the endpoint file holds the *live* port/token for this Dobius
    // install. PTYs that survive an Dobius restart have stale PORT/TOKEN
    ...
    // missing (first-run / pre-endpoint-file scripts / running outside Dobius).
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${DOBIUS_AGENT_HOOK_PORT}/hook/devin" \\',
...
export class DevinHookService {
  getStatus(): AgentHookInstallStatus {
    ...
    return {
      ...
      configPath,
      managedHooksPresent,
      detail: mergeHookInstallDetail(detail, readConfigFromDobiusOverlapDetail(config))
    }
  }
  ...
  // Why: install Dobius's Devin hook settings on the remote box rather than the
  ...
  async installRemote(sftp: SFTPWrapper, remoteHome: string): Promise<AgentHookInstallStatus> {
    ...
    const remoteScriptPath = `${remoteHome.replace(/\/$/, '')}/.dobius/agent-hooks/${remoteScriptFileName}`
