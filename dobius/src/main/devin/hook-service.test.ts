...
describe('DevinHookService', () => {
  ...
  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'dobius-devin-home-'))
...
`
  ...
  it('uses a cmd.exe wrapper for managed hook command on Windows', () => {
    ...
    try {
      const scriptPath = 'C:\\Users\\Ada Lovelace\\.dobius\\agent-hooks\\devin-hook.cmd'
      expect(getDevinManagedCommand(scriptPath)).toBe(
        'cmd /d /s /c ""C:\\Users\\Ada Lovelace\\.dobius\\agent-hooks\\devin-hook.cmd""'
      )
    } finally {
