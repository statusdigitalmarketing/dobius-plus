...
type SystemTrayOptions = {
  ...
  onOpen: () => void
  /** Quit Dobius for real (caller must set the quitting latch before quitting). */
  onQuit: () => void
}
...
export function createSystemTray(opts: SystemTrayOptions): Tray | null {
  ...
  tray = new Tray(image)
  tray.setToolTip('Dobius')
  const menu = Menu.buildFromTemplate([
    { label: translateMain('tray.openDobius', 'Open Dobius+'), click: () => opts.onOpen() },
    { type: 'separator' },
