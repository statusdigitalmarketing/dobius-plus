...
describe('createSystemTray', () => {
  it('creates a tray with an Dobius tooltip and Open/Quit menu on win32', async () => {
    setPlatform('win32')
    ...
    expect(trayInstances[0].setToolTip).toHaveBeenCalledWith('Dobius')
    const items = builtMenuItems()
    expect(items.map((i) => i.label)).toEqual(['Open Dobius', undefined, 'Quit'])
    expect(items[1].type).toBe('separator')
  })

  it('wires Open Dobius, the tray click, and Quit to their callbacks', async () => {
    setPlatform('win32')
    const { createSystemTray } = await loadModule()
    const onOpen = vi.fn()
    const onQuit = vi.fn()
    ...
    const items = builtMenuItems()
    items.find((i) => i.label === 'Open Dobius')?.click?.()
