...
export function initClaudeUsagePath(): void {
  _claudeUsageFile = join(app.getPath('userData'), 'dobius-claude-usage.json')
}
...
export class ClaudeUsageStore {
  ...
  private load(): ClaudeUsagePersistedState {
    try {
      ...
      // preventing Dobius from booting, but we leave the file on disk for debugging.
  ...
  private buildSummary(scope: ClaudeUsageScope, range: ClaudeUsageRange): ClaudeUsageSummary {
    ...
    return {
      ...
      // data here makes the Dobius-only view render empty charts instead of the
      // intended "no usage for this scope" message when only off-Dobius logs exist.
  ...
  async getAutomationRunUsage(input: AutomationUsageLookupInput): Promise<AutomationRunUsage> {
    ...
    return {
      ...
      // Why: Dobius terminal tab ids and Claude usage session ids are different
  ...
  private getFilteredDaily(scope: ClaudeUsageScope, range: ClaudeUsageRange) {
    ...
    return this.state.dailyAggregates.filter((entry) => {
      ...
      if (scope === 'dobius' && entry.worktreeId === null) {
        return false
      }
      return true
    })
