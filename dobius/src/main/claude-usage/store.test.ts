...
vi.mock('electron', () => ({
  app: {
    getPath: vi.fn(() => '/tmp/dobius-test-userdata')
  }
}))
...
describe('ClaudeUsageStore', () => {
  ...
  it('reports no data for Dobius scope when only non-Dobius usage exists', async () => {
  ...
  it('filters sessions by local calendar day instead of raw UTC date prefixes', async () => {
    ...
    const recentSessions = await store.getRecentSessions('dobius', '7d', 10)
