...
function buildAndApplyMenu(options: RegisterAppMenuOptions): void {
  ...
  const featureTourItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.exploreDobius', 'Explore Dobius+'),
    click: (_menuItem, window) => onOpenFeatureTour(window)
  }

  const setupGuideItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.gettingStarted', 'Getting Started with Dobius+'),
  ...
  // redundant "Dobius" entry with roles that don't apply, so we omit it there
    ...
    submenu: [
      ...
      { role: 'copy' },
      {
        ...
        click: () => {
          ...
          // control, so raw Electron paste cannot know which Dobius surface owns it.
  ...
  const appearanceSubmenu: Electron.MenuItemConstructorOptions = {
    ...
    submenu: [
      ...
      {
        label: translateMain('menu.showMobileButton', 'Show Dobius+ Mobile Button'),
        type: 'checkbox',
