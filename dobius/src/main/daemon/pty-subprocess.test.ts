...
const DOBIUS_SHELL_WRAPPER_ENV = [
  'DOBIUS_ATTRIBUTION_SHIM_DIR',
  'DOBIUS_OPENCODE_CONFIG_DIR',
  'DOBIUS_MIMOCODE_HOME',
  'DOBIUS_PI_CODING_AGENT_DIR',
  'DOBIUS_OMP_CODING_AGENT_DIR',
  'DOBIUS_CODEX_HOME'
] as const
...
describe('createPtySubprocess', () => {
  const savedWrapperEnv: Partial<Record<(typeof DOBIUS_SHELL_WRAPPER_ENV)[number], string>> = {}
  ...
  beforeEach(() => {
    ...
    for (const key of DOBIUS_SHELL_WRAPPER_ENV) {
  ...
  itOnMacHost('repairs a deleted macOS daemon cwd before spawning node-pty', () => {
    ...
    const deletedDaemonCwd = mkdtempSync(join(tmpdir(), 'dobius-deleted-daemon-cwd-'))
  ...
  it('keeps menu startup agent foreground during a slow Windows shell enrichment window', async () => {
    ...
    try {
      const handle = createPtySubprocess({
        sessionId: 'repo::C:\\repo\\dobius@@deadbeef',
        cols: 80,
        rows: 24,
        cwd: 'C:\\repo\\dobius',
        command: 'codex'
      ...
      expect(resolveAgentForegroundProcessMock).toHaveBeenCalledWith(
        ...
        'powershell.exe',
        expect.objectContaining({
          contextPaths: expect.arrayContaining(['C:\\repo\\dobius'])
        })
      )
  ...
  it('does not inherit parent Dobius pane identity when caller omits pane env', () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const saved = {
      DOBIUS_PANE_KEY: process.env.DOBIUS_PANE_KEY,
      DOBIUS_TAB_ID: process.env.DOBIUS_TAB_ID,
      DOBIUS_WORKTREE_ID: process.env.DOBIUS_WORKTREE_ID
    }
    process.env.DOBIUS_PANE_KEY = 'parent-tab:parent-leaf'
    process.env.DOBIUS_TAB_ID = 'parent-tab'
    process.env.DOBIUS_WORKTREE_ID = 'parent-worktree'
  ...
  it('preserves explicit child Dobius pane identity over parent env', () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const saved = {
      DOBIUS_PANE_KEY: process.env.DOBIUS_PANE_KEY,
    ...
    try {
      createPtySubprocess({
        ...
        rows: 24,
        env: {
          DOBIUS_PANE_KEY: 'child-tab:child-leaf',
          DOBIUS_TAB_ID: 'child-tab',
          DOBIUS_WORKTREE_ID: 'child-worktree'
        }
      })
    } finally {
      for (const [key, value] of Object.entries(saved)) {
    ...
    const env = spawnMock.mock.calls.at(-1)?.[2].env
    expect(env.DOBIUS_PANE_KEY).toBe('child-tab:child-leaf')
    expect(env.DOBIUS_TAB_ID).toBe('child-tab')
    expect(env.DOBIUS_WORKTREE_ID).toBe('child-worktree')
  })
  ...
  it('does not inherit AppImage runtime env into daemon PTY shells', () => {
    ...
    Object.defineProperty(process, 'platform', { value: 'linux' })
    process.env.APPIMAGE = '/data/apps/dobius.appimage'
    process.env.APPDIR = '/tmp/.mount_dobius123'
    process.env.ARGV0 = '/data/apps/dobius.appimage'
    process.env.OWD = '/home/user/project'
    process.env.APPIMAGE_LIBRARY_PATH = '/tmp/.mount_dobius123/usr/lib'
    process.env.PATH = ['/tmp/.mount_dobius123', '/tmp/.mount_dobius123/usr/sbin', '/usr/bin'].join(
      delimiter
    )
    process.env.LD_LIBRARY_PATH = ['/tmp/.mount_dobius123/usr/lib', '/opt/audio/lib'].join(delimiter)
  ...
  it('does not inherit parent agent hook endpoint for development hook env', () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const previousEndpoint = process.env.DOBIUS_AGENT_HOOK_ENDPOINT
    process.env.DOBIUS_AGENT_HOOK_ENDPOINT = '/tmp/stale-endpoint.env'
    ...
    try {
      createPtySubprocess({
        ...
        rows: 24,
        env: {
          DOBIUS_AGENT_HOOK_ENV: 'development',
          DOBIUS_AGENT_HOOK_PORT: '1234',
      ...
      if (previousEndpoint === undefined) {
        ...
        process.env.DOBIUS_AGENT_HOOK_ENDPOINT = previousEndpoint
      }
    }

    const env = spawnMock.mock.calls.at(-1)?.[2].env
    expect(env.DOBIUS_AGENT_HOOK_ENDPOINT).toBeUndefined()
    expect(env.DOBIUS_AGENT_HOOK_ENV).toBe('development')
    expect(env.DOBIUS_AGENT_HOOK_PORT).toBe('1234')
    expect(env.DOBIUS_AGENT_HOOK_TOKEN).toBe('token')
  })
  ...
  it('preserves explicit development agent hook endpoint files', () => {
    ...
    try {
      createPtySubprocess({
        ...
        env: {
          ...
          DOBIUS_AGENT_HOOK_TOKEN: 'token',
          DOBIUS_AGENT_HOOK_VERSION: '1',
          DOBIUS_AGENT_HOOK_ENDPOINT: '/tmp/fresh-endpoint.env'
        }
      })
    } finally {
      if (previousEndpoint === undefined) {
    ...
    const env = spawnMock.mock.calls.at(-1)?.[2].env
    expect(env.DOBIUS_AGENT_HOOK_ENDPOINT).toBe('/tmp/fresh-endpoint.env')
    expect(env.DOBIUS_AGENT_HOOK_ENV).toBe('development')
    expect(env.DOBIUS_AGENT_HOOK_PORT).toBe('1234')
    expect(env.DOBIUS_AGENT_HOOK_TOKEN).toBe('token')
  })
    ...
    try {
      expect(() =>
        createPtySubprocess({
          ...
          cols: 80,
          rows: 24,
          cwd: '/definitely-missing-dobius-cwd'
        })
      ).toThrow(/definitely-missing-dobius-cwd/)
    } finally {
      if (platform) {
        Object.defineProperty(process, 'platform', platform)
      }
  ...
  it('uses shell wrapper when attribution shims must survive shell startup', () => {
    ...
    try {
      createPtySubprocess({
        ...
        env: {
          SHELL: '/bin/zsh',
          DOBIUS_ATTRIBUTION_SHIM_DIR: '/tmp/dobius-terminal-attribution/posix'
        }
      })
    } finally {
      if (platform) {
    ...
    expect(lastCall[2].env.DOBIUS_SHELL_READY_MARKER).toBe('0')
  })
  ...
  it('uses shell wrapper when OpenCode config must survive shell startup', () => {
    ...
    try {
      createPtySubprocess({
        ...
        env: {
          SHELL: '/bin/zsh',
          OPENCODE_CONFIG_DIR: '/tmp/dobius-opencode-overlay',
          DOBIUS_OPENCODE_CONFIG_DIR: '/tmp/dobius-opencode-overlay'
        }
      })
    } finally {
      if (platform) {
  ...
  it('uses shell wrapper when MiMo home must survive shell startup', () => {
    ...
    try {
      createPtySubprocess({
        ...
        env: {
          SHELL: '/bin/zsh',
          MIMOCODE_HOME: '/tmp/dobius-mimocode-overlay',
          DOBIUS_MIMOCODE_HOME: '/tmp/dobius-mimocode-overlay'
        }
      })
    } finally {
      if (platform) {
  ...
  it('uses shell wrapper when typed OMP commands need the status extension', () => {
    ...
    try {
      createPtySubprocess({
        ...
        env: {
          SHELL: '/bin/zsh',
          DOBIUS_OMP_STATUS_EXTENSION: '/tmp/.omp/agent/extensions/dobius-agent-status.ts'
        }
      })
    } finally {
      if (platform) {
  ...
  it('uses shell wrapper when Codex home must survive shell startup', () => {
    ...
    try {
      createPtySubprocess({
        ...
        env: {
          SHELL: '/bin/zsh',
          CODEX_HOME: '/tmp/dobius-codex-home',
          DOBIUS_CODEX_HOME: '/tmp/dobius-codex-home'
        }
      })
    } finally {
      if (platform) {
  ...
  it('uses shell wrapper when Agent Teams shim path must survive shell startup', () => {
    ...
    try {
      createPtySubprocess({
        ...
        env: {
          SHELL: '/bin/zsh',
          PATH: '/tmp/dobius-agent-teams-bin:/usr/bin',
          DOBIUS_AGENT_TEAMS_TEAM_ID: 'team-test',
          DOBIUS_AGENT_TEAMS_SHIM_DIR: '/tmp/dobius-agent-teams-bin'
        }
      })
    } finally {
      if (platform) {
  ...
  it('uses shell-ready wrapper for delivery-hinted Codex startup commands', () => {
    ...
    expect(lastCall[1]).toEqual(['-l'])
    expect(lastCall[2].env.ZDOTDIR).toMatch(ZSH_SHELL_READY_DIR)
    expect(lastCall[2].env.DOBIUS_SHELL_READY_MARKER).toBe('1')
  })
  ...
  it('honors explicit terminal env overrides after deleting requested defaults', () => {
    ...
    expect(lastCall[2].env.PATH.split(':')[0]).toBe('/tmp/dobius-agent-teams-bin')
    expect(lastCall[2].env.TERM_PROGRAM).toBeUndefined()
    expect(lastCall[2].env.DOBIUS_ATTRIBUTION_SHIM_DIR).toBeUndefined()
  })
  ...
  it('combines HOMEDRIVE and HOMEPATH for Windows default cwd', () => {
    ...
    process.env.HOMEPATH = '\\Users\\dobius'
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      expect.any(String),
      expect.any(Array),
      expect.objectContaining({ cwd: 'D:\\Users\\dobius' })
    )
  })
    ...
    try {
      expect(() =>
        createPtySubprocess({
          ...
          cols: 80,
          rows: 24,
          cwd: 'C:\\definitely-missing-dobius-cwd',
          shellOverride: 'powershell.exe'
        })
      ).toThrow(/Working directory "C:\\definitely-missing-dobius-cwd" does not exist/)
    } finally {
      if (platform) {
        Object.defineProperty(process, 'platform', platform)
      }
    ...
    try {
      expect(() =>
        createPtySubprocess({
          ...
          cols: 80,
          rows: 24,
          cwd: 'C:\\definitely-missing-dobius-wsl-cwd',
          shellOverride: 'wsl.exe'
        })
      ).toThrow(/Working directory "C:\\definitely-missing-dobius-wsl-cwd" does not exist/)
    } finally {
      if (platform) {
        Object.defineProperty(process, 'platform', platform)
      }
  ...
  it('does not pass a Windows Codex home into daemon WSL terminals', () => {
    ...
    try {
      createPtySubprocess({
        ...
        rows: 24,
        cwd: '\\\\wsl.localhost\\Ubuntu\\home\\jin\\repo',
        env: { CODEX_HOME: 'C:\\Users\\jin\\.codex', DOBIUS_CODEX_HOME: 'C:\\Users\\jin\\.codex' }
      })
    } finally {
      if (platform) {
        Object.defineProperty(process, 'platform', platform)
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.not.objectContaining({
          ...
          DOBIUS_CODEX_HOME: expect.anything()
        })
      })
    )
  })
  ...
  it('does not pass a WSL managed Codex home into daemon Windows terminals', () => {
    ...
    try {
      createPtySubprocess({
        ...
        env: {
          CODEX_HOME:
            '\\\\wsl.localhost\\Ubuntu\\home\\jin\\.local\\share\\dobius\\codex-accounts\\a\\home',
          DOBIUS_CODEX_HOME:
            '\\\\wsl.localhost\\Ubuntu\\home\\jin\\.local\\share\\dobius\\codex-accounts\\a\\home'
        }
      })
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.objectContaining({
          CODEX_HOME: '/home/jin/.local/share/dobius/codex-accounts/a/home',
          DOBIUS_CODEX_HOME: '/home/jin/.local/share/dobius/codex-accounts/a/home',
  ...
  it('marks Dobius terminal handles for WSL env import in daemon WSL terminals', () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')
    ...
    expect(spawnCall[2].env.DOBIUS_TERMINAL_HANDLE).toBe('term_wsl')
    ...
    expect(spawnCall[2].env.WSLENV?.split(':')).toEqual(
      expect.arrayContaining(['FOO/u', 'DOBIUS_TERMINAL_HANDLE/u', POWERLEVEL10K_WIZARD_DISABLE_ENV])
    )
  })
