...
export type DaemonPtyAdapterOptions = {
  ...
  historyPath?: string
  /** Current daemons write history in-process so output keeps logging while
   *  the Electron app is closed. Legacy daemons still need adapter-side writes. */
...
export class DaemonPtyAdapter implements IPtyProvider {
  ...
  constructor(opts: DaemonPtyAdapterOptions) {
    ...
    this.historyWrites = opts.historyWrites ?? true
  ...
  async shutdown(id: string, opts: { immediate?: boolean; keepHistory?: boolean }): Promise<void> {
    ...
    if (opts.keepHistory && this.historyWrites) {
  ...
  // later crashes while Dobius is closed, checkpoint.json has recovery data.
  async disconnectOnly(): Promise<void> {
    ...
    if (this.historyWrites && this.checkpointInFlight) {
