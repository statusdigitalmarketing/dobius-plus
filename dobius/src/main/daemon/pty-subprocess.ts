...
function promoteAgentTeamsShimPath(
  ...
  if (!env.DOBIUS_AGENT_TEAMS_TEAM_ID || !requestedPath) {
    return
  }
...
function removeInheritedDevAgentHookEndpoint(
  ...
  if (
    explicitEnv?.DOBIUS_AGENT_HOOK_ENV === 'development' &&
    !explicitEnv.DOBIUS_AGENT_HOOK_ENDPOINT
  ) {
    ...
    delete env.DOBIUS_AGENT_HOOK_ENDPOINT
  }
}
...
function formatMissingDaemonPathError(kind: 'helper' | 'cwd', path: string): DaemonProtocolError {
  ...
  return new DaemonProtocolError(
    `Daemon's ${kind === 'helper' ? 'node-pty install' : 'working directory'} is gone ` +
      `(worktree deleted?). Restart Dobius. node-pty: ${step} failed: ENOENT ` +
...
function repairDaemonCwd(): string | null {
  const candidates = [process.env.DOBIUS_USER_DATA_PATH]
  try {
...
function runSinglePtySpawnHealthProbe(): Promise<void> {
  const cwd = isExistingDirectory(process.env.DOBIUS_USER_DATA_PATH)
    ? process.env.DOBIUS_USER_DATA_PATH
    : getDefaultCwd()
...
export function createPtySubprocess(opts: PtySubprocessOptions): SubprocessHandle {
  ...
  const env: Record<string, string> = {
    ...
    // by main (daemon-init.ts:93) with the parent's env, so DOBIUS_APP_VERSION
    // — set in src/main/index.ts from app.getVersion() — is inherited here.
    TERM_PROGRAM_VERSION: process.env.DOBIUS_APP_VERSION ?? '0.0.0-dev',
    ...
    // for TERM_PROGRAM=Dobius, so callers drop OSC 8 output entirely and emit
    // bare text instead. xterm.js in Dobius parses OSC 8 and the pane's
    ...
    if (pathWin32.basename(shellPath).toLowerCase() === 'wsl.exe') {
      if (codexHomeWslInfo) {
        ...
        if (launchWslDistro && launchWslDistro !== codexHomeWslInfo.distro) {
          ...
          delete env.DOBIUS_CODEX_HOME
        } else {
          env.CODEX_HOME = codexHomeWslInfo.linuxPath
          env.DOBIUS_CODEX_HOME = codexHomeWslInfo.linuxPath
          // Why: wsl.exe only imports non-default env vars named in WSLENV.
          addWslEnvKeys(env, ['CODEX_HOME', 'DOBIUS_CODEX_HOME'])
          if (!launchWslDistro) {
            const resolved = resolveWindowsShellLaunchArgs(
              shellPath,
              requestedCwd,
        ...
        // Why: Dobius's selected Codex runtime home is host-local. WSL Codex
      ...
      // them. DOBIUS_CODEX_HOME must go too because shell-ready scripts restore
