...
export class TerminalHost {
  ...
  private onSessionExit: TerminalHostOptions['onSessionExit']
  ...
  async createOrAttach(opts: CreateOrAttachOptions): Promise<CreateOrAttachResult> {
    ...
    const session = new Session({
      ...
      cols: size.cols,
      rows: size.rows,
      terminalHandle: opts.env?.DOBIUS_TERMINAL_HANDLE,
      subprocess,
      ...
      onExit: (code) => {
        ...
        this.reapSession(opts.sessionId)
      },
    ...
    if (opts.command && !subprocess.startupCommandDeliveredInShellArgs) {
      ...
      // the user would need to press Enter after Dobius launches the agent or
  ...
  async getCwd(sessionId: string): Promise<string | null> {
    ...
    // Why: the emulator's cwd is null until the shell emits OSC 7. Dobius's
