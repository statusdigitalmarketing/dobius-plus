...
describe('DaemonServer', () => {
  ...
  describe('shutdown', () => {
    ...
    async function startServerWithHistory(): Promise<string> {
      const historyPath = join(dir, 'terminal-history')
      server = new DaemonServer({
        socketPath,
        tokenPath,
        historyPath,
    ...
    function readSessionMeta(historyPath: string, sessionId: string): { endedAt: string | null } {
      const metaPath = join(historyPath, getHistorySessionDirName(sessionId), 'meta.json')
      return JSON.parse(readFileSync(metaPath, 'utf-8')) as { endedAt: string | null }
    }

    it('signal shutdown leaves session history restorable (endedAt null)', async () => {
      const historyPath = await startServerWithHistory()
      const c = await connectClient()
      await c.request('createOrAttach', { sessionId: 'sig-session', cols: 80, rows: 24 })

      await server.shutdown({ markSessionsEnded: false })

      // Why: endedAt null is the "unclean" marker the next launch uses to
      // cold-restore scrollback and auto-resume agent sessions. A SIGTERM
      // from an updater/OS shutdown must not erase that restore path.
      expect(readSessionMeta(historyPath, 'sig-session').endedAt).toBeNull()
    })

    it('default shutdown marks sessions cleanly ended (no false cold-restore)', async () => {
      const historyPath = await startServerWithHistory()
      const c = await connectClient()
      await c.request('createOrAttach', { sessionId: 'clean-session', cols: 80, rows: 24 })

      await server.shutdown()

      expect(readSessionMeta(historyPath, 'clean-session').endedAt).not.toBeNull()
    })
  })
})
