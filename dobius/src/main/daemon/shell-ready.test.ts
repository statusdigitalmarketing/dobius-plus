...
const SHELL_READY_MARKER_OUTPUT = '\x1b]777;dobius-shell-ready\x07'
...
async function runInteractiveZshLogin(args: {
  ...
  const proc = pty.spawn('zsh', ['-o', 'noglobalrcs', '-l'], {
    ...
    env: {
      ...
      DOBIUS_ORIG_ZDOTDIR: args.tempHome,
      DOBIUS_ZSHENV_SOURCE_DIR: args.tempHome,
...
describePosix('daemon shell-ready launch config', () => {
  ...
  beforeEach(() => {
    previousUserDataPath = process.env.DOBIUS_USER_DATA_PATH
    ...
    process.env.DOBIUS_USER_DATA_PATH = userDataPath
  })
  ...
  afterEach(() => {
    if (previousUserDataPath === undefined) {
      ...
      process.env.DOBIUS_USER_DATA_PATH = previousUserDataPath
    }
  ...
  it('falls back to HOME for DOBIUS_ORIG_ZDOTDIR when inherited ZDOTDIR points at a wrapper dir', async () => {
    ...
    // was forked from a shell which was itself an Dobius PTY. Such a shell has
    // ZDOTDIR=<some>/shell-ready/zsh; propagating that unchanged would make
    // the wrapper `source "$DOBIUS_ORIG_ZDOTDIR/.zshenv"` source itself.
  ...
  itWithZsh(
    ...
    async () => {
      const { getShellReadyLaunchConfig } = await importFreshShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
      const tempHome = mkdtempSync(join(tmpdir(), 'dobius-zsh-vi-mode-'))
      writeFileSync(
        join(tempHome, '.zshrc'),
        [
  ...
  itWithZsh(
    ...
    async () => {
      const { getShellReadyLaunchConfig } = await importFreshShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
      const tempHome = mkdtempSync(join(tmpdir(), 'dobius-zsh-azhw-'))
      const userHookOutput = 'DOBIUS-TEST-USER-HOOK'
      writeFileSync(
        join(tempHome, '.zshrc'),
        [
          `__dobius_test_line_init_hook() { printf "${userHookOutput}" }`,
          'autoload -Uz add-zle-hook-widget',
          'zle -N __dobius_test_line_init_hook',
          'add-zle-hook-widget line-init __dobius_test_line_init_hook',
          ''
        ].join('\n')
      )
      try {
  ...
  // (nested Dobius, manual re-source) must stay idempotent — it must keep
  ...
  itWithZsh(
    ...
    async () => {
      const zdotdir = mkdtempSync(join(tmpdir(), 'dobius-zsh-resource-'))
      const userHookOutput = 'DOBIUS-TEST-PRIOR-WIDGET'
      const block = getZshShellReadyMarkerRegistrationBlock('\\033]777;dobius-shell-ready\\007')
      writeFileSync(
        join(zdotdir, '.zshrc'),
        [
          // A user widget that mimics oh-my-zsh vi-mode owning zle-line-init.
          `__dobius_test_prior_widget() { printf "${userHookOutput}" }`,
          'zle -N zle-line-init __dobius_test_prior_widget',
          block,
  ...
  it('preserves a real inherited ZDOTDIR as DOBIUS_ORIG_ZDOTDIR', async () => {
    ...
    // ZDOTDIR before launching Dobius. We only want to reject the self-loop
