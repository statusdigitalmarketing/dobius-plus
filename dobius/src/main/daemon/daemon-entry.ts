...
async function main(): Promise<void> {
  ...
  const shutdown = async (): Promise<void> => {
    if (daemon) {
      // Why: a signal means an EXTERNAL stop (OS shutdown, updater, killall),
      // not a user asking to end their sessions. Leave history "unclean" so
      // the next app launch cold-restores scrollback and auto-resumes agents.
      await daemon.shutdown({ markSessionsEnded: false })
      daemon = null
    }
    process.exit(0)
  }
...
function getHistoryPath(): string | undefined {
  const userDataPath = process.env.DOBIUS_USER_DATA_PATH
  if (!userDataPath) {
    return undefined
  }
  const historyPath = join(userDataPath, 'terminal-history')
