  ...
  return async (socketPath, tokenPath) => {
    ...
    if (health === 'healthy') {
      ...
      if (resolverHealth === 'unhealthy') {
        ...
        // /Applications/Dobius.app path is replaced during update.
...
export async function initDaemonPtyProvider(signal?: AbortSignal): Promise<void> {
  ...
  // from a test.
  const e2eInitDelayMs = Number(process.env.DOBIUS_E2E_DAEMON_INIT_DELAY_MS)
...
// later daemon crash while Dobius is closed is still recoverable on next launch.
