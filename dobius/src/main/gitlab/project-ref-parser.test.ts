...
describe('gitlab project ref parsing', () => {
  it('parses HTTPS and SSH GitLab.com remotes', () => {
    ...
    expect(parseGitLabProjectRef('git@gitlab.com:statusdigitalmarketing/dobius-plus.git')).toEqual({
      host: 'gitlab.com',
  ...
  it('returns null for non-GitLab hosts when host not in knownHosts', () => {
    expect(parseGitLabProjectRef('git@github.com:statusdigitalmarketing/dobius-plus.git')).toBeNull()
...
describe('gitlab remote project ref candidates', () => {
  it('extracts self-hosted candidates before the host is trusted', () => {
    expect(parseRemoteProjectRefCandidate('git@gitlab.internal:team/dobius.git')).toEqual({
