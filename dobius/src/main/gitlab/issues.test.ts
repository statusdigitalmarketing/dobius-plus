...
describe('gitlab issue operations', () => {
  ...
  it('gets a single issue from the project ref', async () => {
    getIssueProjectRefMock.mockResolvedValueOnce({ host: 'gitlab.com', path: 'statusdigitalmarketing/dobius-plus' })
    glabExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        iid: 923,
        title: 'Use upstream issues',
        state: 'opened',
        web_url: 'https://gitlab.com/statusdigitalmarketing/dobius-plus/-/issues/923',
        labels: []
      })
    })
    ...
    expect(glabExecFileAsyncMock).toHaveBeenCalledWith(
      ['api', 'projects/statusdigitalmarketing%2Fdobius/issues/923'],
      { cwd: '/repo-root' }
    )
  })
  ...
  it('routes local WSL issue operations through project resolution and glab execution options', async () => {
    ...
    getIssueProjectRefMock.mockResolvedValue({ host: 'gitlab.com', path: 'statusdigitalmarketing/dobius-plus' })
    resolveIssueSourceMock.mockResolvedValue({
      source: { host: 'gitlab.com', path: 'statusdigitalmarketing/dobius-plus' },
      fellBack: false
    })
    glabExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          iid: 924,
          web_url: 'https://gitlab.com/statusdigitalmarketing/dobius-plus/-/issues/924'
        })
      })
      .mockResolvedValueOnce({ stdout: '{}' })
      .mockResolvedValueOnce({
  ...
  it('lists issues with state=opened ordering', async () => {
    ...
    expect(glabExecFileAsyncMock).toHaveBeenCalledWith(
      [
        'api',
        'projects/statusdigitalmarketing%2Fdobius/issues?per_page=5&order_by=updated_at&sort=desc&state=opened'
      ],
      { cwd: '/repo-root' }
    )
  })
  ...
  it('creates an issue and returns its iid + web_url', async () => {
    ...
    await expect(createIssue('/repo-root', 'New issue', 'Body')).resolves.toEqual({
      ok: true,
      number: 924,
      url: 'https://gitlab.com/statusdigitalmarketing/dobius-plus/-/issues/924'
    })
    expect(glabExecFileAsyncMock).toHaveBeenCalledWith(
      [
        'api',
        '-X',
        'POST',
        'projects/statusdigitalmarketing%2Fdobius/issues',
        '-f',
        'title=New issue',
        '-f',
        'description=Body'
  ...
  it('updateIssue closes via `glab issue close` when state=closed', async () => {
    ...
    expect(glabExecFileAsyncMock).toHaveBeenCalledWith(
      ['issue', 'close', '5', '-R', 'statusdigitalmarketing/dobius-plus'],
      { cwd: '/repo-root' }
    )
  })
  ...
  it('updateIssue applies body edits via the issue API', async () => {
    ...
    expect(glabExecFileAsyncMock).toHaveBeenCalledWith(
      ['api', '-X', 'PUT', 'projects/statusdigitalmarketing%2Fdobius/issues/5', '-f', 'description=Updated body'],
      { cwd: '/repo-root' }
    )
  })
  ...
  it('routes issue metadata reads through the selected SSH GitLab host', async () => {
    getIssueProjectRefMock
      .mockResolvedValueOnce({ host: 'git.internal', path: 'statusdigitalmarketing/dobius-plus' })
      .mockResolvedValueOnce({ host: 'git.internal', path: 'statusdigitalmarketing/dobius-plus' })
    glabExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'bug\nfeature\n' })
      .mockResolvedValueOnce({
    ...
    expect(glabExecFileAsyncMock.mock.calls[0][0]).toEqual([
      ...
      'git.internal',
      '--paginate',
      'projects/statusdigitalmarketing%2Fdobius/labels',
      '--jq',
      '.[].name'
    ])
    expect(glabExecFileAsyncMock.mock.calls[1][0]).toEqual([
      ...
      'git.internal',
      '--paginate',
      'projects/statusdigitalmarketing%2Fdobius/members/all?per_page=100',
      '--jq',
  ...
  it('addIssueComment posts to /notes and maps the response', async () => {
    ...
    expect(glabExecFileAsyncMock).toHaveBeenCalledWith(
      ['api', '-X', 'POST', 'projects/statusdigitalmarketing%2Fdobius/issues/5/notes', '-f', 'body=Hello'],
      { cwd: '/repo-root' }
    )
  })
  ...
  it('addIssueComment passes hostname for SSH-backed self-hosted repos', async () => {
    ...
    expect(glabExecFileAsyncMock).toHaveBeenCalledWith(
      [
        ...
        '-X',
        'POST',
        'projects/statusdigitalmarketing%2Fdobius/issues/5/notes',
        '-f',
        'body=Hello'
      ],
      {}
