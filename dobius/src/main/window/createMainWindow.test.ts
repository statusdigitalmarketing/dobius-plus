...
describe('createMainWindow', () => {
  ...
  it('enables renderer sandboxing and opens external links safely', () => {
    ...
    const allowBlankPrefs = { partition: 'persist:dobius-browser' }
    ...
    windowHandlers['will-attach-webview'](
      ...
      { partition: 'persist:dobius-browser' } as never,
  ...
  it('notifies before Dobius-first captures a risky terminal-focused shortcut', () => {
    const windowHandlers: Record<string, (...args: any[]) => void> = {}
    const webContents = {
      on: vi.fn((event, handler) => {
        windowHandlers[event] = handler
    ...
    createMainWindow({
      getUI: () => ({}),
      getSettings: () => ({ terminalShortcutPolicy: 'dobius-first' })
    } as never)
  ...
  it('notifies before Dobius-first captures a terminal-focused double-tap shortcut', () => {
    const windowHandlers: Record<string, (...args: any[]) => void> = {}
    const webContents = {
      on: vi.fn((event, handler) => {
        windowHandlers[event] = handler
  ...
  it('keeps the headless E2E window hidden when the Windows fallback fires', () => {
    vi.useFakeTimers()
    const previousHeadless = process.env.DOBIUS_E2E_HEADLESS
    process.env.DOBIUS_E2E_HEADLESS = '1'
    const { browserWindowInstance } = createStartupRevealWindowFixture()
    ...
    try {
      ...
      if (previousHeadless === undefined) {
        delete process.env.DOBIUS_E2E_HEADLESS
      } else {
        process.env.DOBIUS_E2E_HEADLESS = previousHeadless
      }
    }
  })
