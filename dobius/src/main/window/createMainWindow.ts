...
// Sam's desktop review found the native controls visually high against the
// custom chrome, so their optical center sits 2px below the CSS midpoint.
...
type CreateMainWindowOptions = {
  ...
  /** Returns true when Dobius should reload after an unexpected renderer loss.
...
export function createAppRendererWebPreferences(): Electron.BrowserWindowConstructorOptions['webPreferences'] {
  return {
...
export function createMainWindow(
  ...
  const mainWindow = new BrowserWindow({
    ...
    title: opts?.title ?? 'Dobius+',
    show: false,
    // Why: macOS swallows the app-activating click by default, so clicking
    // back into Dobius (e.g. the floating workspace) needed a second click.
  ...
  if (process.platform === 'darwin') {
    ...
    // visibility transitions hardens Dobius against black-surface failures during
  ...
  const dispatchResolvedWindowShortcutAction = (
    ...
    const capturedTerminalActionId =
      ...
      focusedShortcutContext.terminalShortcutPolicy === 'dobius-first' &&
  ...
  mainWindow.webContents.on('before-input-event', (event, input) => {
    ...
    if (isMacAppPasteInput(input)) {
      ...
      // editable controls, so route Cmd+V through Dobius's paste ownership first.
      event.preventDefault()
      ...
      try {
        new Notification({
          ...
          body: translateMain(
            ...
            'Dobius is still running in the system tray'
          )
        }).show()
      } catch {
  ...
  mainWindow.on('closed', () => {
    ...
    // here or the quit path can crash before Squirrel.Mac relaunches Dobius.
