...
const ANTIGRAVITY_HOOK_BUNDLE_NAME = 'dobius-status'
...
const ANTIGRAVITY_EVENTS = [
  ...
  // Dobius's hook is observational, so installing there can block user tools.
  {
...
function getManagedCommand(scriptPath: string, event: AntigravityEvent): string {
  ...
  return wrapPosixHookCommand(scriptPath, { DOBIUS_ANTIGRAVITY_EVENT: event.eventName })
}
...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  if (target === 'local' && process.platform === 'win32') {
    return [
      '@echo off',
      'setlocal',
      'if /I "%DOBIUS_ANTIGRAVITY_EVENT%"=="Stop" (',
      '  echo {"decision":""}',
      ') else (',
      '  echo {}',
      ')',
  ...
  return [
    '#!/bin/sh',
    'case "$DOBIUS_ANTIGRAVITY_EVENT" in',
    '  Stop)',
    '    printf \'{"decision":""}\\n\'',
    '    ;;',
    '  *)',
    ...
    // the event name so Dobius shows a status row instead of silently dropping it.
    "  payload='{}'",
    'fi',
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${DOBIUS_AGENT_HOOK_PORT}/hook/antigravity" \\',
    ...
    '  --data-urlencode "hook_event_name=${DOBIUS_ANTIGRAVITY_EVENT}" \\',
...
function getWindowsWrapperScript(eventName: string): string {
  return [
    '@echo off',
    'setlocal',
    `set "DOBIUS_ANTIGRAVITY_EVENT=${eventName}"`,
    'set "DOBIUS_ANTIGRAVITY_CORE=%~dp0antigravity-hook.cmd"',
    'if exist "%DOBIUS_ANTIGRAVITY_CORE%" (',
    '  call "%DOBIUS_ANTIGRAVITY_CORE%"',
    '  exit /b 0',
    ')',
    'if /I "%DOBIUS_ANTIGRAVITY_EVENT%"=="Stop" (',
    '  echo {"decision":""}',
...
function buildWindowsAntigravityHookPostCommand(): string {
  ...
  return `"%SystemRoot%\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -NoProfile -ExecutionPolicy Bypass -Command "$utf8=[System.Text.UTF8Encoding]::new($false); [Console]::InputEncoding=$utf8; [Console]::OutputEncoding=$utf8; $inputData=[Console]::In.ReadToEnd(); try { $payload=if ([string]::IsNullOrWhiteSpace($inputData)) { @{} } else { $inputData | ConvertFrom-Json }; $body=@{ paneKey=$env:DOBIUS_PANE_KEY; launchToken=$env:DOBIUS_AGENT_LAUNCH_TOKEN; tabId=$env:DOBIUS_TAB_ID; worktreeId=$env:DOBIUS_WORKTREE_ID; env=$env:DOBIUS_AGENT_HOOK_ENV; version=$env:DOBIUS_AGENT_HOOK_VERSION; hook_event_name=$env:DOBIUS_ANTIGRAVITY_EVENT; payload=$payload } | ConvertTo-Json -Depth 100 -Compress; $bodyBytes=$utf8.GetBytes($body); Invoke-WebRequest -UseBasicParsing -Method Post -Uri ('http://127.0.0.1:' + $env:DOBIUS_AGENT_HOOK_PORT + '/hook/antigravity') -ContentType 'application/json; charset=utf-8' -Headers @{ 'X-Dobius-Agent-Hook-Token'=$env:DOBIUS_AGENT_HOOK_TOKEN } -Body $bodyBytes -TimeoutSec 2 | Out-Null } catch {}"`
}
...
export class AntigravityHookService {
  ...
  async installRemote(sftp: SFTPWrapper, remoteHome: string): Promise<AgentHookInstallStatus> {
    ...
    const remoteScriptPath = `${home}/.dobius/agent-hooks/antigravity-hook.sh`
    try {
      ...
      buildInstalledConfig(
        config,
        (event) =>
          wrapPosixHookCommand(remoteScriptPath, { DOBIUS_ANTIGRAVITY_EVENT: event.eventName }),
        createAntigravityManagedCommandMatcher()
      )
