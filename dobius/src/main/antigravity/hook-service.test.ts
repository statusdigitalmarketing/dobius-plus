...
describe('AntigravityHookService', () => {
  ...
  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'dobius-antigravity-home-'))
  ...
  it('installs Antigravity global hooks.json bundle and managed script', () => {
    ...
    expect(Object.keys(config['dobius-status']).sort()).toEqual(
      ['PostInvocation', 'PostToolUse', 'PreInvocation', 'Stop'].sort()
    )
    expect(config['dobius-status'].PreToolUse).toBeUndefined()
    expect(config['dobius-status'].PostToolUse[0].matcher).toBe('*')
    expect(config['dobius-status'].PreInvocation[0].command).toContain(
      ANTIGRAVITY_PRE_INVOCATION_COMMAND
    )
    if (process.platform === 'win32') {
      expect(config['dobius-status'].PreInvocation[0].command).not.toContain('DOBIUS_ANTIGRAVITY_EVENT')
    } else {
      expect(config['dobius-status'].PreInvocation[0].command).toContain(
        "DOBIUS_ANTIGRAVITY_EVENT='PreInvocation'"
      )
      expect(config['dobius-status'].Stop[0].command).toContain("DOBIUS_ANTIGRAVITY_EVENT='Stop'")
    }

    const script = readFileSync(
      join(homeDir, '.dobius', 'agent-hooks', ANTIGRAVITY_SCRIPT_FILE_NAME),
      'utf8'
    )
    ...
    if (process.platform === 'win32') {
      ...
      expect(script).toContain('hook_event_name=$env:DOBIUS_ANTIGRAVITY_EVENT')
      ...
      expect(script).toContain('hook_event_name=${DOBIUS_ANTIGRAVITY_EVENT}')
  ...
  it('installs Windows event wrappers without nested cmd quoting and removes stale PreToolUse hooks', () => {
    withPlatform('win32', () => {
              ...
              PreToolUse: [
                {
                  ...
                  hooks: [
                    {
                      type: 'command',
                      command: `cmd /d /s /c "set "DOBIUS_ANTIGRAVITY_EVENT=PreToolUse" && call "${staleScriptPath}""`
                    }
                  ]
                }
              ]
      ...
      for (const [eventName, wrapperFileName] of Object.entries(expectedWrappers)) {
        const definition = config['dobius-status'][eventName][0]
        const command =
        ...
        expect(command).not.toContain('DOBIUS_ANTIGRAVITY_EVENT')
        expect(command).not.toContain('"')

        const wrapper = readFileSync(join(homeDir, '.dobius', 'agent-hooks', wrapperFileName), 'utf8')
        expect(wrapper).toContain(`set "DOBIUS_ANTIGRAVITY_EVENT=${eventName}"`)
        expect(wrapper).toContain('call "%DOBIUS_ANTIGRAVITY_CORE%"')
      }

      const script = readFileSync(
        join(homeDir, '.dobius', 'agent-hooks', 'antigravity-hook.cmd'),
        'utf8'
      )
      expect(script).toContain('/hook/antigravity')
      expect(script).toContain('%SystemRoot%\\System32\\WindowsPowerShell\\v1.0\\powershell.exe')
  ...
  it('preserves user-authored hook bundles and entries in Dobius bundle', () => {
    const configPath = join(homeDir, '.gemini', 'config', 'hooks.json')
    mkdirSync(dirname(configPath), { recursive: true })
    writeFileSync(
      configPath,
      `${JSON.stringify(
        {
          ...
          'dobius-status': {
            PreInvocation: [{ type: 'command', command: '/usr/local/bin/dobius-extra' }]
          }
        },
        null,
        2
    ...
    const config = JSON.parse(readFileSync(configPath, 'utf8')) as {
      ...
      'dobius-status': { PreInvocation: { command: string }[] }
    }
    expect(config['user-hook'].PreInvocation[0].command).toBe('/usr/local/bin/user-hook')
    const commands = config['dobius-status'].PreInvocation.map((entry) => entry.command)
    expect(commands).toContain('/usr/local/bin/dobius-extra')
  ...
  it('removes stale managed Antigravity hook entries from retired events', () => {
    ...
    const config = JSON.parse(readFileSync(configPath, 'utf8')) as {
      'dobius-status': Record<string, { command?: string; hooks?: { command: string }[] }[]>
    }
    expect(config['dobius-status'].OldEvent).toBeUndefined()
    expect(config['dobius-status'].PreToolUse).toBeUndefined()
    const commands = config['dobius-status'].PostToolUse.flatMap((definition) =>
      (definition.hooks ?? []).map((hook) => hook.command)
    )
    expect(commands).toHaveLength(1)
    expect(commands[0]).toContain(
      join(homeDir, '.dobius', 'agent-hooks', ANTIGRAVITY_POST_TOOL_USE_COMMAND)
    )
  })
})
