...
describe('MimoCodeHookService buildPtyEnv', () => {
  ...
  beforeEach(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'dobius-mimocode-userdata-'))
    ...
    mimocodeHome = mkdtempSync(join(tmpdir(), 'dobius-mimocode-home-'))
    ...
    writeFileSync(join(configDir, 'plugins', 'dobius-mimocode-status.js'), 'USER PLUGIN')
  })
  ...
  it('mirrors user config into shared overlay and installs Dobius status plugin', () => {
    ...
    const dobiusPlugin = join(overlayHome, 'config', 'plugins', 'dobius-mimocode-status.js')
    expect(existsSync(dobiusPlugin)).toBe(true)
    expect(readFileSync(dobiusPlugin, 'utf8')).toContain('/hook/mimo-code')

    expect(
      readFileSync(join(mimocodeHome, 'config', 'plugins', 'dobius-mimocode-status.js'), 'utf8')
    ).toBe('USER PLUGIN')
  })
  ...
  it('reuses the overlay home on a second buildPtyEnv call', () => {
    ...
    expect(
      readFileSync(join(overlayHome, 'config', 'plugins', 'dobius-mimocode-status.js'), 'utf8')
