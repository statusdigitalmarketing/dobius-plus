...
describe('mapGhosttyToDobius — split-divider-color', () => {
  it('maps valid hex to both dark and light divider colors', () => {
    const result = mapGhosttyToDobius({ 'split-divider-color': '#ff5500' })
  ...
  it('rejects invalid split-divider-color', () => {
    const result = mapGhosttyToDobius({ 'split-divider-color': 'blue' })
    expect(result.diff).toEqual({})
    expect(result.unsupportedKeys).toEqual(['split-divider-color'])
  })
})

describe('mapGhosttyToDobius — unfocused-split-opacity', () => {
  it('maps valid float to terminalInactivePaneOpacity', () => {
    const result = mapGhosttyToDobius({ 'unfocused-split-opacity': '0.5' })
    expect(result.diff).toEqual({ terminalInactivePaneOpacity: 0.5 })
    expect(result.unsupportedKeys).toEqual([])
  })

  it('rejects out-of-range unfocused-split-opacity', () => {
    const result = mapGhosttyToDobius({ 'unfocused-split-opacity': '1.2' })
    expect(result.diff).toEqual({})
    expect(result.unsupportedKeys).toEqual(['unfocused-split-opacity'])
  })

  it('rejects negative unfocused-split-opacity', () => {
    const result = mapGhosttyToDobius({ 'unfocused-split-opacity': '-0.1' })
    expect(result.diff).toEqual({})
    expect(result.unsupportedKeys).toEqual(['unfocused-split-opacity'])
  })
})

describe('mapGhosttyToDobius — scrollback-limit', () => {
  ...
  it('marks scrollback-limit as unsupported', () => {
    const result = mapGhosttyToDobius({ 'scrollback-limit': '50000' })
    expect(result.diff).toEqual({})
...
describe('mapGhosttyToDobius — adjust-cell-height', () => {
  ...
  it('rounds higher half-boundary decimal percentages to the nearest hundredth', () => {
    const result = mapGhosttyToDobius({ 'adjust-cell-height': '1.5%' })
...
describe('mapGhosttyToDobius — cursor-text', () => {
  ...
  it('rejects invalid cursor-text', () => {
    const result = mapGhosttyToDobius({ 'cursor-text': 'white' })
    expect(result.diff).toEqual({})
    expect(result.unsupportedKeys).toEqual(['cursor-text'])
  })
})

describe('mapGhosttyToDobius — bold-color', () => {
  it('maps valid hex to terminalColorOverrides.bold', () => {
    const result = mapGhosttyToDobius({ 'bold-color': '#ff0000' })
    expect(result.diff).toEqual({
