...
async function makeTempRepoDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'dobius-repo-icon-'))
  tempDirs.push(dir)
  return dir
}
...
describe('detectRepoIcon', () => {
  ...
  it('falls back to the GitHub owner avatar for GitHub repos', async () => {
    ...
    await gitExecFileAsync(['remote', 'add', 'origin', 'git@github.com:statusdigitalmarketing/dobius-plus.git'], {
      cwd: repoPath
    })
  ...
  it('skips code-host package homepages so GitHub remotes stay repo-specific', async () => {
    ...
    await writeFile(
      join(repoPath, 'package.json'),
      JSON.stringify({ homepage: 'https://github.com/statusdigitalmarketing/dobius-plus' })
    )
    await gitExecFileAsync(['init'], { cwd: repoPath })
    await gitExecFileAsync(['remote', 'add', 'origin', 'https://github.com/statusdigitalmarketing/dobius-plus.git'], {
      cwd: repoPath
    })
  ...
  it('uses the resolved fork upstream for both metadata and the GitHub avatar', async () => {
    const repoPath = await makeTempRepoDir()
    await gitExecFileAsync(['init'], { cwd: repoPath })
    await gitExecFileAsync(['remote', 'add', 'origin', 'git@github.com:tmchow/dobius.git'], {
      cwd: repoPath
    })
    await gitExecFileAsync(['remote', 'add', 'upstream', 'git@github.com:statusdigitalmarketing/dobius-plus.git'], {
      cwd: repoPath
    })
    ...
    await expect(detectRepoIconAndUpstream({ repoPath, kind: 'git' })).resolves.toEqual({
      gitRemoteIdentity: {
        ...
        remoteName: 'upstream',
        remoteUrl: 'git@github.com:statusdigitalmarketing/dobius-plus.git'
      },
      repoIcon: {
        type: 'image',
        src: 'https://github.com/statusdigitalmarketing.png?size=64',
