...
describe('CommandCodeHookService', () => {
  ...
  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'dobius-command-code-home-'))
  ...
  it('installs the managed command for Command Code status events', () => {
    ...
    if (process.platform !== 'win32') {
      expect(config.hooks.PreToolUse[0].hooks[0].command).toContain(join(homeDir, '.dobius'))
    }
    if (process.platform !== 'win32') {
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'dobius command-code home with spaces')
  ...
  it('installs a hook script that can recover the endpoint when Command Code strips token env', () => {
    ...
    const script = readFileSync(join(homeDir, '.dobius', 'agent-hooks', scriptFileName), 'utf8')

    if (process.platform === 'win32') {
      expect(script).toContain('sourceEndpointByPort')
      expect(script).toContain('dobius-dev\\agent-hooks')
      expect(script).toContain('set DOBIUS_AGENT_HOOK_PORT=')
    } else {
      ...
      expect(script).toContain('__dobius_read_ancestor_var')
      expect(script).toContain('__dobius_fill_from_endpoint_file')
      expect(script).toContain('[ "$__dobius_endpoint_port" != "$DOBIUS_AGENT_HOOK_PORT" ]')
      expect(script).toContain('DOBIUS_PANE_KEY')
      expect(script).toContain('DOBIUS_AGENT_LAUNCH_TOKEN')
      expect(script).toContain('dobius-dev/agent-hooks')
  ...
  itPosix('does not let stale endpoint files clobber recovered connection fields', async () => {
    ...
    const server = createServer((req, res) => {
      ...
      req.on('end', () => {
        requests.push({ body, token: req.headers['x-dobius-agent-hook-token'] })
        res.statusCode = 204
        res.end()
      })
    })
    ...
    try {
      ...
      const scriptPath = join(homeDir, '.dobius', 'agent-hooks', 'command-code-hook.sh')
      const child = spawn('/bin/sh', [scriptPath], {
        env: {
          ...process.env,
          HOME: homeDir,
          DOBIUS_AGENT_HOOK_ENDPOINT: staleEndpointPath,
          DOBIUS_AGENT_HOOK_PORT: String(address.port),
          DOBIUS_AGENT_HOOK_TOKEN: 'current-token',
          DOBIUS_PANE_KEY: 'tab:leaf',
          DOBIUS_TAB_ID: 'tab',
          DOBIUS_WORKTREE_ID: 'worktree',
          DOBIUS_AGENT_HOOK_ENV: 'development',
          DOBIUS_AGENT_HOOK_VERSION: '1'
        },
