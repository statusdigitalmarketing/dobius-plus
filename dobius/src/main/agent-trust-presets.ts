 ...
 * first launch.
 *
 * Why: Dobius's "drop URL into agent input as a draft" flow injects the URL
...
export function markCodexProjectTrusted(workspacePath: string): void {
  ...
  // Why: Dobius-launched Codex runs with an Dobius-owned CODEX_HOME, so the trust
  // preset must also update the runtime config Codex will actually read.
  upsertProjectTrustLevel(join(getDobiusManagedCodexHomePath(), 'config.toml'), absPath, 'trusted')
}
...
function canonicalize(p: string): string {
  ...
  // (dobius caches realpath()'d worktree paths) matches the agent's lookup.
  try {
    if (existsSync(p)) {
