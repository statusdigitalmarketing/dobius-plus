...
describe('resolveSessionFilePath', () => {
  it('globs Claude project subdirs for <sessionId>.jsonl', async () => {
    const root = await makeRoot('dobius-native-chat-resolve-claude-')
  ...
  it('matches Codex rollout files by session id suffix', async () => {
    const root = await makeRoot('dobius-native-chat-resolve-codex-')
  ...
  it('resolves a rollout from the dobius-managed Codex home (DOBIUS_USER_DATA_PATH)', async () => {
    // Dobius launches Codex with its own managed CODEX_HOME, so rollout files land
    // under <userData>/codex-runtime-home/home/sessions, NOT ~/.codex/sessions.
    const root = await makeRoot('dobius-native-chat-resolve-managed-')
    ...
    const previous = process.env.DOBIUS_USER_DATA_PATH
    process.env.DOBIUS_USER_DATA_PATH = root
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.DOBIUS_USER_DATA_PATH = previous
      }
    }
  })

  it('falls back to CODEX_HOME when the managed home has no match', async () => {
    const root = await makeRoot('dobius-native-chat-resolve-codex-home-')
  ...
  it('prefers the hook transcriptPath when it exists (Claude id != file name)', async () => {
    ...
    const root = await makeRoot('dobius-native-chat-resolve-path-')
    const claudeProjectsDir = join(root, 'claude-projects')
    const projectDir = join(claudeProjectsDir, '-Users-ada-repo')
    await mkdir(projectDir, { recursive: true })
  ...
  it('falls back to the id glob when the hook transcriptPath does not exist', async () => {
    const root = await makeRoot('dobius-native-chat-resolve-path-stale-')
    const claudeProjectsDir = join(root, 'claude-projects')
    const projectDir = join(claudeProjectsDir, '-Users-ada-repo')
    await mkdir(projectDir, { recursive: true })
  ...
  it('ignores a non-jsonl transcriptPath and falls back to the glob', async () => {
    const root = await makeRoot('dobius-native-chat-resolve-path-ext-')
    const claudeProjectsDir = join(root, 'claude-projects')
    const projectDir = join(claudeProjectsDir, '-Users-ada-repo')
    await mkdir(projectDir, { recursive: true })
