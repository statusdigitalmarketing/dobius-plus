...
import { getDobiusManagedCodexHomePath } from '../codex/codex-home-paths'
...
// Why: Dobius launches Codex with DOBIUS_CODEX_HOME pointing at its own managed
// runtime home, so Dobius-started Codex rollout files land under
...
// fall back to CODEX_HOME/~/.codex so a non-Dobius Codex transcript still resolves.
// Duplicates are filtered so a managed-home symlink to ~/.codex isn't scanned twice.
function codexSessionsDirs(): string[] {
  const candidates = [
    join(getDobiusManagedCodexHomePath(), 'sessions'),
...
export type ResolveSessionFileOptions = {
   ...
   *  scans). Defaults to the dobius-managed home then CODEX_HOME/~/.codex. */
