...
export class ClaudeAccountService {
  ...
  private async doAddAccount(
    ...
    try {
      ...
      if (!captured.identity.email) {
        throw new Error('Claude login completed, but Dobius could not resolve the account email.')
      }
  ...
  private createTemporaryClaudeConfigDir(location: ManagedClaudeAuthLocation): {
    ...
    if (location.managedAuthRuntime !== 'wsl') {
      return {
        windowsPath: mkdtempSync(join(tmpdir(), 'dobius-claude-login-')),
        linuxPath: null,
        wslDistro: null
      }
    }
    ...
    const linuxPath = execFileSync(
      ...
      [
        ...
        'bash',
        '-lc',
        'mktemp -d "${TMPDIR:-/tmp}/dobius-claude-login.XXXXXX"'
      ],
  ...
  private createManagedAuthDir(
    ...
    writeFileSync(join(managedAuthPath, '.dobius-managed-claude-auth'), `${accountId}\n`, 'utf-8')
    return {
  ...
  private tryCreateWslManagedAuthDir(
    ...
    const wslLinuxAuthPath = `${home.replace(/\/$/, '')}/.local/share/dobius/claude-accounts/${accountId}/auth`
    const markerPath = `${wslLinuxAuthPath}/.dobius-managed-claude-auth`
    execFileSync(
      'wsl.exe',
      [
        '-d',
  ...
  private assertManagedAuthPath(candidatePath: string, expectedAccountId?: string): string {
    ...
    if (wslInfo) {
      if (
        !wslInfo.linuxPath.includes('/.local/share/dobius/claude-accounts/') ||
        !wslInfo.linuxPath.endsWith('/auth')
      ) {
        throw new Error('Managed WSL Claude auth storage is outside Dobius account storage.')
      }
            ...
            [
              ...
              buildEncodedWslBashCommand(
                [
                  ...
                  'managed_root="${HOME%/}/.local/share/dobius/claude-accounts"',
                  'candidate_real=$(readlink -f -- "$candidate")',
                  'managed_root_real=$(readlink -f -- "$managed_root")',
                  'test -f "$candidate_real/.dobius-managed-claude-auth"',
                  expectedAccountId
                    ? `test "$(cat "$candidate_real/.dobius-managed-claude-auth")" = ${shellQuote(expectedAccountId)}`
                    : 'test -n "$(cat "$candidate_real/.dobius-managed-claude-auth")"',
      ...
      if (
        ...
        !existsSync(join(candidatePath, '.dobius-managed-claude-auth'))
      ) {
        throw new Error('Managed Claude auth storage is not owned by Dobius.')
      }
