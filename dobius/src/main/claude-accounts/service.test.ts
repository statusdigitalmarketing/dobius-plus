...
vi.mock('electron', () => ({
  app: {
    getPath: () => '/tmp/dobius-claude-service-test'
  }
}))
...
describe('ClaudeAccountService credential capture', () => {
  ...
  it('falls back to captured credentials file on macOS', async () => {
    tempDir = mkdtempSync(join(tmpdir(), 'dobius-claude-capture-'))
  ...
  it('restores previous managed auth when reauth materialization fails', async () => {
    setPlatform('linux')
    tempDir = '/tmp/dobius-claude-service-test'
    rmSync(tempDir, { recursive: true, force: true })
    ...
    let settings = {
      claudeManagedAccounts: [
        ...
        },
        {
          ...
          wslDistro: 'Ubuntu',
          wslLinuxAuthPath: '/home/jin/.local/share/dobius/claude-accounts/wsl-account/auth',
          authMethod: 'subscription-oauth',
          organizationUuid: null,
          organizationName: null,
          createdAt: 1,
  ...
  it('removes a WSL account without clearing the Windows active account', async () => {
    ...
    mkdirSync(hostAuthPath, { recursive: true })
    mkdirSync(wslAuthPath, { recursive: true })
    writeFileSync(join(wslAuthPath, '.dobius-managed-claude-auth'), 'wsl-account\n', 'utf-8')
    let settings = {
      claudeManagedAccounts: [
        {
          id: 'host-account',
