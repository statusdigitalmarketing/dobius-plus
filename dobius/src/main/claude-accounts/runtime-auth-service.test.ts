...
describe('ClaudeRuntimeAuthService', () => {
  beforeEach(() => {
    ...
    testState.userDataDir = mkdtempSync(join(tmpdir(), 'dobius-claude-runtime-'))
    testState.fakeHomeDir = mkdtempSync(join(tmpdir(), 'dobius-claude-home-'))
  ...
  it('adopts canonical legacy managed auth paths without existing markers', async () => {
    ...
    const markerPath = join(managedAuthPath, '.dobius-managed-claude-auth')
    expect(readFileSync(runtimeCredentialsPath, 'utf-8')).toBe(managedCredentials)
  ...
  it('rejects symlinked managed credential children', async () => {
    ...
    mkdirSync(managedAuthPath, { recursive: true })
    writeFileSync(join(managedAuthPath, '.dobius-managed-claude-auth'), 'account-1\n', 'utf-8')
    writeFileSync(
      join(managedAuthPath, 'oauth-account.json'),
      '{"accountUuid":"account-1"}\n',
      'utf-8'
  ...
  it('restores system auth when switching from an owned account to missing credentials', async () => {
    ...
    writeFileSync(join(managedAuthPath2, '.dobius-managed-claude-auth'), 'account-2\n', 'utf-8')
    writeFileSync(
  ...
  it('routes refreshed Claude credentials to the matching managed account', async () => {
    ...
    // Dobius selected account-2. Persist that refresh to account-1, then restore
  ...
  it('uses account WSL runtime for untargeted Claude preparation instead of stale terminal WSL settings', async () => {
    ...
    const settings = createSettings({
      ...
      claudeManagedAccounts: [
        createClaudeAccount('ubuntu-account', ubuntuAuthPath, {
          ...
          wslDistro: 'Ubuntu',
          wslLinuxAuthPath: '/home/alice/.local/share/dobius/claude-accounts/ubuntu/auth'
        })
      ],
      activeClaudeManagedAccountId: null,
    ...
    expect(preparation).toMatchObject({
      runtime: 'wsl',
      wslDistro: 'Ubuntu',
      wslLinuxConfigDir: '/home/alice/.local/share/dobius/claude-accounts/ubuntu/auth',
    ...
    const settings = createSettings({
      claudeManagedAccounts: [
        createClaudeAccount('account-1', managedAuthPath, {
          managedAuthRuntime: 'wsl',
          wslDistro: 'Ubuntu',
          wslLinuxAuthPath: '/home/alice/.local/share/dobius/claude-accounts/account-1/auth'
        })
      ],
      activeClaudeManagedAccountId: null,
    ...
    const settings = createSettings({
      claudeManagedAccounts: [
        ...
        createClaudeAccount('debian-account', debianAuthPath, {
          managedAuthRuntime: 'wsl',
          wslDistro: 'Debian',
          wslLinuxAuthPath: '/home/alice/.local/share/dobius/claude-accounts/debian/auth'
        })
      ],
      activeClaudeManagedAccountId: null,
      activeClaudeManagedAccountIdsByRuntime: {
