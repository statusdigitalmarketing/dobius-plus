...
export class ClaudeRuntimeAuthService {
  ...
  // Why: tracks the credentials Dobius last wrote to the shared credentials file.
  // On managed→system-default transition, if the file differs from this value,
  // an external login (e.g. `claude auth login`) overwrote it — so Dobius adopts
  ...
  private async doSyncForCurrentSelection(target?: ClaudeAccountSelectionTarget): Promise<void> {
    ...
    if (activeAccount.managedAuthRuntime === 'wsl') {
      if (!this.getOwnedManagedAuthPath(activeAccount)) {
        console.warn(
          '[claude-runtime-auth] Active WSL managed account is not owned by Dobius, restoring system default'
        )
        const nextSelection = setSelectedClaudeAccountIdForTarget(
          normalizeClaudeRuntimeSelection(settings),
          null,
    ...
    if (!this.getOwnedManagedAuthPath(activeAccount)) {
      console.warn(
        '[claude-runtime-auth] Active managed account is not owned by Dobius, restoring system default'
      )
      if (this.lastSyncedAccountId !== null) {
        if (
          previousAccount &&
    ...
    // .credentials.json. If we detect the runtime file differs from what Dobius
            ...
            [
              ...
              buildEncodedWslBashCommand(
                [
                  ...
                  `test "$(cat "$candidate_real/.dobius-managed-claude-auth")" = ${shellQuote(account.id)}`,
  ...
  private getOwnedRuntimeOauthBaseline(
    ...
    // that Dobius wrote .claude.json. Use it only after another surface proves
  ...
  private writeRuntimeCredentials(contents: string): void {
    ...
    // runtime credentials since Dobius last materialized them.
    if (
