...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  ...
  return [
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${DOBIUS_AGENT_HOOK_PORT}/hook/cursor" \\',
...
export class CursorHookService {
  ...
  // Why: install Dobius's managed Cursor hooks on the remote box. Mirrors
  ...
  async installRemote(sftp: SFTPWrapper, remoteHome: string): Promise<AgentHookInstallStatus> {
    ...
    const remoteScriptPath = `${remoteHome.replace(/\/$/, '')}/.dobius/agent-hooks/cursor-hook.sh`
    try {
