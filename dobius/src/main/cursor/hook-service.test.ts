...
describe('CursorHookService', () => {
  ...
  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'dobius-cursor-home-'))
  ...
  it('installs Cursor Agent hooks with the documented top-level command schema', () => {
    ...
    for (const eventName of CURSOR_EVENTS) {
      ...
      if (process.platform !== 'win32') {
        expect(definition?.command).toContain(join(homeDir, '.dobius'))
      }
      expect(definition?.hooks).toBeUndefined()
    }

    const script = readFileSync(
      join(homeDir, '.dobius', 'agent-hooks', CURSOR_SCRIPT_FILE_NAME),
      'utf8'
    )
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'dobius cursor home with spaces')
      ...
      `${JSON.stringify(
        {
          ...
          hooks: {
            beforeSubmitPrompt: [
              ...
              { command: '/old/path/.dobius/agent-hooks/cursor-hook.sh' }
            ],
            retiredEvent: [
              { command: '/old/path/.dobius/agent-hooks/cursor-hook.sh' },
