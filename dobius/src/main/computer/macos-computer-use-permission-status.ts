...
async function getComputerUsePermissionStatusAsync(): Promise<ComputerUsePermissionStatusResult> {
  ...
  if (!helperAppPath) {
    return createUnavailablePermissionStatus('Dobius+ Computer Use.app was not found', null)
  }

  const executablePath = resolveMacOSComputerUseExecutablePath()
  if (!executablePath) {
    return createUnavailablePermissionStatus(
      `${helperAppPath}/Contents/MacOS/dobius-computer-use-macos was not found`,
      helperAppPath
    )
  }
...
async function readPermissionStatusFromHelperApp(
  ...
  const tempDir = await mkdtemp(join(tmpdir(), 'dobius-computer-use-permissions-'))
