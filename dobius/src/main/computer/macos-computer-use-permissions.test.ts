...
const permissionStatusTempDir = '/tmp/dobius-computer-use-permissions-test'
...
describe('openComputerUsePermissions', () => {
  ...
  beforeEach(() => {
    ...
    resolveHelperExecutablePathMock.mockReturnValue(
      '/Applications/Dobius+ Computer Use.app/Contents/MacOS/dobius-computer-use-macos'
    )
  ...
  it('does not launch the setup helper when all permissions are granted', async () => {
    resolveHelperAppPathMock.mockReturnValue('/Applications/Dobius+ Computer Use.app')
    ...
    expect(spawn).not.toHaveBeenCalledWith(
      '/usr/bin/open',
      ['-n', '/Applications/Dobius+ Computer Use.app', '--args', '--permissions'],
  ...
  it('launches the helper app in permissions mode', async () => {
    ...
    await expect(openComputerUsePermissions()).resolves.toEqual({
      ...
      nextStep: 'Grant Screen Recording to Dobius+ Computer Use, then retry get-app-state.'
    })
    expect(spawnSync).toHaveBeenCalledWith(
      '/usr/bin/pkill',
      ['-f', 'dobius-computer-use-macos[[:space:]]+--permission([[:space:]]|$)'],
      { stdio: 'ignore' }
    )
    expect(spawnSync).toHaveBeenCalledWith(
      '/usr/bin/pkill',
      ['-f', 'dobius-computer-use-macos[[:space:]]+--permissions([[:space:]]|$)'],
      { stdio: 'ignore' }
    )
  ...
  it('launches a targeted permission helper flow', async () => {
    ...
    await expect(openComputerUsePermissions('accessibility')).resolves.toEqual({
      ...
      nextStep: 'Grant Accessibility to Dobius+ Computer Use, then retry get-app-state.'
    })
    expect(spawn).toHaveBeenCalledWith(
      '/usr/bin/open',
      ['-n', '/Applications/Dobius+ Computer Use.app', '--args', '--permission', 'accessibility'],
      { detached: true, stdio: 'ignore' }
    )
  })
  ...
  it('throws when the helper app is missing on macOS', async () => {
    ...
    await expect(openComputerUsePermissions()).rejects.toThrow(
      'Dobius+ Computer Use.app was not found'
    )
  })
  ...
  it('throws when the helper executable is missing during setup', async () => {
    ...
    await expect(openComputerUsePermissions('accessibility')).rejects.toThrow(
      '/Applications/Dobius+ Computer Use.app/Contents/MacOS/dobius-computer-use-macos was not found'
    )
  })
  ...
  it('resets stale macOS TCC grants for the helper bundle id', async () => {
    ...
    vi.mocked(execFileSync).mockReturnValueOnce('com.example.dobius.computer-use\n')
    ...
    await expect(resetComputerUsePermissions()).resolves.toEqual({
      ...
      bundleId: 'com.example.dobius.computer-use',
      permissions: [
        { id: 'accessibility', status: 'not-granted' },
        { id: 'screenshots', status: 'not-granted' }
      ]
    ...
    expect(spawnSync).toHaveBeenCalledWith(
      '/usr/bin/tccutil',
      ['reset', 'Accessibility', 'com.example.dobius.computer-use'],
      { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] }
    )
    expect(spawnSync).toHaveBeenCalledWith(
      '/usr/bin/tccutil',
      ['reset', 'ScreenCapture', 'com.example.dobius.computer-use'],
      { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] }
    )
  })
})
