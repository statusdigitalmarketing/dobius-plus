...
const DEFAULT_COMPUTER_USE_BUNDLE_ID = 'com.statusdigitalmarketing.dobius-plus.computer-use'
...
async function openComputerUsePermissionsAsync(
  ...
  if (!helperAppPath) {
    throw new RuntimeClientError('accessibility_error', 'Dobius+ Computer Use.app was not found')
  }
...
function closeExistingPermissionHelpers(): void {
  ...
  const setupHelperPatterns = [
    'dobius-computer-use-macos[[:space:]]+--permission([[:space:]]|$)',
    'dobius-computer-use-macos[[:space:]]+--permissions([[:space:]]|$)'
  ]
...
function nextPermissionStep(
  ...
  return `Grant ${missing.id === 'accessibility' ? 'Accessibility' : 'Screen Recording'} to Dobius+ Computer Use, then retry get-app-state.`
}
