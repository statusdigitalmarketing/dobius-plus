...
const LEGACY_LINUX_COMMAND_NAME = 'dobius'
...
export class CliInstaller {
  ...
  private get commandName(): string {
    ...
    return this.platform === 'linux' ? LINUX_COMMAND_NAME : 'dobius'
  }
  ...
  constructor(options: CliInstallerOptions = {}) {
    ...
    this.commandPathOverride =
      options.commandPathOverride ?? process.env.DOBIUS_CLI_INSTALL_PATH ?? null
    ...
    this.macCommandPath = existsSync(dirname(candidateMacPath))
      ? candidateMacPath
      : join(this.homePath, '.local', 'bin', 'dobius')
    ...
    if (!launcherPath) {
      const detail =
        this.isLinuxAppImage() && this.appImagePath
          ...
          : this.isPackaged
            ? 'The bundled CLI launcher is missing from this Dobius+ build.'
  ...
  async install(): Promise<CliInstallStatus> {
    ...
    if (status.state === 'conflict') {
      throw new Error(`Refusing to replace non-Dobius command at ${status.commandPath}.`)
    }
    ...
    if (status.installMethod === 'symlink') {
      ...
      // Why: packaged Windows already ships resources/bin/dobius.cmd. Registration
  ...
  async remove(): Promise<CliInstallStatus> {
    ...
    if (status.state === 'conflict') {
      throw new Error(`Refusing to remove non-Dobius command at ${status.commandPath}.`)
    }
    if (status.state === 'stale') {
      throw new Error(`Refusing to remove a command not owned by Dobius at ${status.commandPath}.`)
    }
    ...
    for (const commandPath of this.getPathCommandCandidates(defaultCommandPath)) {
      ...
      if (status.state !== 'not_installed') {
        if (reachedDefaultCommandPath && !isDefaultCommandPath && status.state === 'conflict') {
          // Why: a non-Dobius command after an empty/default install slot can be
        ...
        // shell will actually run, while preserving conflicts that shadow Dobius.
        return commandPath
      }
    }
    return null
  ...
  private resolveCommandPath(): string | null {
    ...
    if (!this.isPackaged) {
      ...
      if (this.platform === 'win32') {
        return join(this.localAppDataPath, 'Programs', 'Dobius Dev', 'bin', `${DEV_COMMAND_NAME}.cmd`)
      }
    }
    ...
    if (this.platform === 'win32') {
      return join(this.localAppDataPath, 'Programs', 'Dobius', 'resources', 'bin', 'dobius.cmd')
    }
  ...
  private async removeLegacyLinuxCommandIfManaged(launcherPath: string | null): Promise<void> {
    ...
    try {
      ...
      // Why: after the Linux command rename, the old Dobius-owned `dobius` symlink
      // would keep shadowing GNOME Dobius even though the new command is installed.
  ...
  private isManagedLegacyLinuxTarget(resolvedTarget: string, launcherPath: string): boolean {
    ...
    return /(?:^|[/\\])resources[/\\]bin[/\\]dobius$/.test(resolvedTarget)
  }
    ...
    try {
      ...
      if (!stats.isFile()) {
        return this.buildStatus({
          ...
          state: 'conflict',
          currentTarget: null,
          detail: `${commandPath} exists but is not an Dobius launcher script.`
        })
      }
      ...
      if (isMissingError(error)) {
        return this.buildStatus({
          ...
          state: 'not_installed',
          currentTarget: null,
          detail: `Register ${commandPath} to use Dobius from the terminal.`
        })
      }
      throw error
    }
    ...
    try {
      ...
      if (!stats.isSymbolicLink()) {
        if (stats.isFile()) {
          ...
          if (managedTarget) {
            return this.buildStatus({
              ...
              detail: `${commandPath} contains an older Dobius launcher.`
            })
          }
        }
        ...
        return this.buildStatus({
          ...
          state: 'conflict',
          currentTarget: null,
          detail: `${commandPath} exists but is not an Dobius symlink.`
        })
      }
      ...
      return this.buildStatus({
        ...
        detail: isInstalled
          ...
          : isManagedStaleTarget
            ? `${commandPath} points to an older Dobius launcher.`
            : `${commandPath} points to a non-Dobius launcher.`
      })
    } catch (error) {
      if (isMissingError(error)) {
        return this.buildStatus({
  ...
  private isManagedSymlinkTarget(resolvedTarget: string, launcherPath: string): boolean {
    ...
    if (this.platform === 'darwin') {
      // Why: prior packaged installs can leave a symlink to an older Dobius.app
  ...
  private isSiblingDevLauncherTarget(
    ...
    // profile; packaged Dobius must be able to reclaim that public command.
    return (
    ...
    try {
      ...
      if (isMissingError(error)) {
        return this.buildStatus({
          ...
          state: 'not_installed',
          currentTarget: null,
          detail: `Register ${commandPath} to use Dobius from Command Prompt or PowerShell.`
        })
      }
      throw error
    }
  ...
  private withPathInfo(
    ...
    if (
      ...
      return {
        ...
        state: 'not_installed',
        currentTarget: null,
        detail: `Register ${status.commandPath} to use Dobius from Command Prompt or PowerShell.`
      }
    }
...
async function ensureDevLauncher(args: {
  ...
  // Why: packaged Dobius ships real platform launchers under resources/bin, but
  ...
  if (args.commandName === DEV_COMMAND_NAME && args.platform !== 'win32') {
    ...
    // commands are documented as `dobius ...`. Keep that local alias fresh
    // without claiming the global production command.
    await writeFile(join(dirname(launcherPath), 'dobius'), content, {
      encoding: 'utf8',
      mode: 0o755
    })
  }
...
export DOBIUS_USER_DATA_PATH=${quoteShell(userDataPath)}
if [ -z "\${DOBIUS_APP_EXECUTABLE:-}" ]; then
  export DOBIUS_APP_EXECUTABLE="$ELECTRON"
...
set "DOBIUS_USER_DATA_PATH=${escapeWindowsBatchValue(userDataPath)}"
if not defined DOBIUS_APP_EXECUTABLE (
  set "DOBIUS_APP_EXECUTABLE=%ELECTRON%"
  set "DOBIUS_APP_EXECUTABLE_NEEDS_APP_ROOT=1"
)
set "DOBIUS_NODE_OPTIONS=%NODE_OPTIONS%"
set "DOBIUS_NODE_REPL_EXTERNAL_MODULE=%NODE_REPL_EXTERNAL_MODULE%"
set NODE_OPTIONS=
...
function buildWindowsForwarder(launcherPath: string): string {
  return `@echo off
setlocal
set "DOBIUS_LAUNCHER=${escapeWindowsBatchValue(launcherPath)}"
"%DOBIUS_LAUNCHER%" %*
`
}
...
function extractManagedUnixLauncherTarget(content: string): string | null {
  if (
    ...
    !content.includes('DOBIUS_NODE_OPTIONS') ||
  ...
  // /usr/local/bin/dobius. Treat only Dobius's compiled CLI entrypoints as managed;
...
async function writeWindowsUserPath(value: string): Promise<void> {
  await runWindowsPathCommand([
    '-NoProfile',
    '-Command',
    // Why: PATH registration must stay user-scoped on Windows so the Dobius
...
export function getBundledLauncherPath(
  ...
  if (platform === 'darwin') {
    return join(resourcesPath, 'bin', 'dobius')
  }
  if (platform === 'linux') {
    return join(resourcesPath, 'bin', LINUX_COMMAND_NAME)
  }
  if (platform === 'win32') {
    return join(resourcesPath, 'bin', 'dobius.cmd')
  }
  return null
}
