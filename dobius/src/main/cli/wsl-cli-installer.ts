...
const WSL_COMMAND_NAME = 'dobius'
const LEGACY_WSL_COMMAND_NAME = 'dobius'
  ...
  async getStatus(): Promise<CliInstallStatus> {
    ...
    if (content === null) {
      return this.buildStatus({
        ...
        detail: `Register ${ready.commandPath} to use Dobius from WSL.`
      })
    }
    ...
    if (content === 'not_file') {
      return this.buildStatus({
        ...
        currentTarget: null,
        pathConfigured: ready.pathConfigured,
        detail: `${ready.commandPath} exists but is not an Dobius launcher script.`
      })
    }
      ...
      return this.buildStatus({
        ...
        detail:
          bridgeContent === null || bridgeManaged
            ...
            : `${ready.bridgePath} exists but is not managed by Dobius.`
      })
    }
    ...
    return this.buildStatus({
      ...
      pathConfigured: ready.pathConfigured,
      detail: managed
        ? `${ready.commandPath} points to a different Dobius launcher.`
        : `${ready.commandPath} exists but is not managed by Dobius.`
    })
  }
  ...
  async install(): Promise<CliInstallStatus> {
    ...
    await this.run(
      ...
      [
        ...
        // Why: Dobius owns the new WSL command; remove only the old
        // Dobius-managed wrapper so unmanaged `dobius` commands survive.
  ...
  > {
    ...
    if (!hostStatus.launcherPath) {
      return {
        status: this.unsupported(
          ...
          hostStatus.detail ?? 'The Windows Dobius+ CLI launcher is missing.'
        )
      }
    }
    ...
    if (!interopReady) {
      return {
        status: this.unsupported(
          'launcher_missing',
          'WSL Windows interop is unavailable; Dobius cannot launch the Windows CLI from WSL.'
        )
      }
    }
  ...
  private async readCommandFile(
    ...
    const output = await this.run(
      ...
      [
        ...
        '  printf __DOBIUS_NOT_FILE__',
        `elif [ ! -e ${quoteShell(commandPath)} ]; then`,
        '  printf __DOBIUS_MISSING__',
    ...
    if (output === '__DOBIUS_MISSING__') {
      return null
    }
    if (output === '__DOBIUS_NOT_FILE__') {
      return 'not_file'
    }
    return output
  }
