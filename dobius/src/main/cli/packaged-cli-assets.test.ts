...
const linuxLauncherAsset = new URL('../../../resources/linux/bin/dobius', import.meta.url)
...
describe('packaged CLI assets', () => {
  ...
  itRunsUnixShell(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'dobius-linux-cli-'))
      try {
        const appDir = join(root, 'Dobius')
        const resourcesDir = join(appDir, 'resources')
        const launcherDir = join(resourcesDir, 'bin')
        const cliDir = join(resourcesDir, 'app.asar.unpacked', 'out', 'cli')
        const launcherPath = join(launcherDir, 'dobius')
        const electronPath = join(appDir, 'dobius')
...
`,
        ...
        const commandPath = join(commandDir, 'dobius')
        await mkdir(commandDir, { recursive: true })
        await mkdir(join(homeDir, 'dobius'), { recursive: true })
  ...
  itRunsUnixShell('runs the AppImage CLI wrapper through APPDIR at runtime', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dobius-appimage-cli-'))
    try {
      const appDir = join(root, 'Dobius.AppDir')
      const cliDir = join(appDir, 'resources', 'app.asar.unpacked', 'out', 'cli')
      const cliPath = join(cliDir, 'index.js')
      const appImagePath = join(root, "Dobius's AppImage.AppImage")
      const commandPath = join(root, 'dobius')
      await mkdir(cliDir, { recursive: true })
      await writeFile(
        cliPath,
        `exports.main = (argv) => {
  console.log(JSON.stringify({
    ...
    dobiusNodeOptions: process.env.DOBIUS_NODE_OPTIONS ?? null,
    nodeReplExternalModule: process.env.NODE_REPL_EXTERNAL_MODULE ?? null,
    dobiusNodeReplExternalModule: process.env.DOBIUS_NODE_REPL_EXTERNAL_MODULE ?? null
  }))
}
`,
        'utf8'
...
`,
      ...
      const payload = JSON.parse(result.stdout) as {
        ...
        dobiusNodeOptions: string | null
        nodeReplExternalModule: string | null
        dobiusNodeReplExternalModule: string | null
      }
      ...
      expect(payload.dobiusNodeOptions).toBe('--trace-warnings')
      expect(payload.nodeReplExternalModule).toBeNull()
      expect(payload.dobiusNodeReplExternalModule).toBe('external-loader')
    } finally {
      await rm(root, { recursive: true, force: true })
    }
  })
