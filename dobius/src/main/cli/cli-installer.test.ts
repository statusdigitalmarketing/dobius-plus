...
async function createPackagedMacLauncher(root: string): Promise<string> {
  ...
  await writeFile(join(resourcesPath, 'bin', 'dobius'), '#!/usr/bin/env bash\necho dobius\n', {
    encoding: 'utf8',
    mode: 0o755
  })
...
describe('CliInstaller', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const installPath = join(fixture.root, 'bin', 'dobius')
      const installer = new CliInstaller({
        platform: 'darwin',
        isPackaged: false,
        userDataPath: fixture.userDataPath,
        execPath: '/Applications/Dobius.app/Contents/MacOS/Dobius',
      ...
      expect(initial.launcherPath).toContain(join('userData', 'cli', 'bin', 'dobius'))
      ...
      expect(launcherContent).toContain(`export DOBIUS_USER_DATA_PATH='${fixture.userDataPath}'`)
      expect(launcherContent).toContain('export DOBIUS_APP_EXECUTABLE="$ELECTRON"')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const fixture = await makeFixture()
      const installPath = join(fixture.root, '.local', 'bin', 'dobius-ide')
      const installer = new CliInstaller({
        platform: 'linux',
        isPackaged: false,
        userDataPath: fixture.userDataPath,
      ...
      const installed = await installer.install()
      expect(installed.state).toBe('installed')
      expect(installed.commandName).toBe('dobius-ide')
  ...
  // packaged `dobius` / `dobius-ide` commands developers rely on day to day.
  it.skipIf(process.platform === 'win32')(
    'uses a separate dobius-dev command for default development installs',
    async () => {
      const fixture = await makeFixture()
      ...
      const installed = await installer.install()
      expect(installed.state).toBe('installed')
      expect(installed.commandName).toBe('dobius-dev')
      expect(installed.commandPath).toBe(join(commandDir, 'dobius-dev'))
      expect(installed.launcherPath).toBe(join(fixture.userDataPath, 'cli', 'bin', 'dobius-dev'))
      await expect(readlink(installed.commandPath as string)).resolves.toBe(installed.launcherPath)
      await expect(
        readFile(join(fixture.userDataPath, 'cli', 'bin', 'dobius'), 'utf8')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const installPath = join(commandDir, 'dobius-ide')
      const appImagePath = join(fixture.root, 'Dobius.AppImage')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const commandDir = join(fixture.root, '.local', 'bin')
      const installPath = join(commandDir, 'dobius-ide')
      const oldAppImagePath = join(fixture.root, 'Old-Dobius.AppImage')
      const newAppImagePath = join(fixture.root, 'Dobius.AppImage')
  ...
  // Why: Linux renamed the public command to avoid shadowing GNOME Dobius, so
  // upgrading must clean up only the old symlink owned by prior Dobius installs.
  it.skipIf(process.platform === 'win32')(
    'removes the old managed linux dobius symlink when installing dobius-ide',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const commandDir = join(homePath, '.local', 'bin')
      const resourcesPath = join(fixture.root, 'resources')
      const launcherPath = join(resourcesPath, 'bin', 'dobius-ide')
      const oldLauncherPath = join(resourcesPath, 'bin', 'dobius')
      const legacyCommandPath = join(commandDir, 'dobius')
      await mkdir(commandDir, { recursive: true })
      await mkdir(join(resourcesPath, 'bin'), { recursive: true })
      ...
      const installed = await installer.install()
      expect(installed.commandPath).toBe(join(commandDir, 'dobius-ide'))
      await expect(lstat(legacyCommandPath)).rejects.toMatchObject({ code: 'ENOENT' })
    }
  )

  it.skipIf(process.platform === 'win32')(
    'removes a legacy linux dobius symlink when installing an AppImage wrapper',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const commandDir = join(homePath, '.local', 'bin')
      ...
      await symlink(join('/tmp', '.mount_Dobius1234', 'resources', 'bin', 'dobius'), legacyCommandPath)
  ...
  it('creates a windows wrapper and updates the user PATH', async () => {
    const fixture = await makeFixture()
    const installPath = join(fixture.root, 'Programs', 'Dobius', 'bin', 'dobius.cmd')
    let userPath = 'C:\\Windows\\System32'
    const installer = new CliInstaller({
      platform: 'win32',
      isPackaged: false,
      userDataPath: fixture.userDataPath,
      execPath: 'C:\\Users\\me\\AppData\\Local\\Dobius\\Dobius.exe',
      appPath: fixture.appPath,
      commandPathOverride: installPath,
    ...
    expect(installed.state).toBe('installed')
    expect(installed.pathConfigured).toBe(true)
    expect(userPath).toContain(join(fixture.root, 'Programs', 'Dobius', 'bin'))

    const wrapperContent = await readFile(installPath, 'utf8')
    expect(wrapperContent).toContain('DOBIUS_LAUNCHER=')
    expect(wrapperContent).toContain('dobius.cmd')
    const launcherContent = await readFile(installed.launcherPath as string, 'utf8')
    expect(launcherContent).toContain(`set "DOBIUS_USER_DATA_PATH=${fixture.userDataPath}"`)
    expect(launcherContent).toContain('set "DOBIUS_APP_EXECUTABLE=%ELECTRON%"')

    const removed = await installer.remove()
    expect(removed.state).toBe('not_installed')
    expect(userPath).not.toContain(join(fixture.root, 'Programs', 'Dobius', 'bin'))
  })
  ...
  // Why: this test creates a Unix symlink to /tmp/not-dobius, which only applies on macOS/Linux.
  it.skipIf(process.platform === 'win32')(
    'refuses to replace an unknown symlink at the command path',
    async () => {
      const fixture = await makeFixture()
      const installPath = join(fixture.root, 'bin', 'dobius')
      const existingTarget = '/tmp/not-dobius'
      ...
      await expect(installer.install()).rejects.toThrow('Refusing to replace non-Dobius command')
      await expect(readlink(installPath)).resolves.toBe(existingTarget)
    }
  )

  // Why: packaged app moves can leave a symlink to an older Dobius-owned launcher;
  // those are safe to refresh, unlike arbitrary user symlinks.
  it.skipIf(process.platform === 'win32')(
    'replaces stale packaged Dobius launcher symlinks',
    async () => {
      const fixture = await makeFixture()
      const commandDir = join(fixture.root, 'bin')
      const installPath = join(commandDir, 'dobius')
      const resourcesPath = join(fixture.root, 'Current.app', 'Contents', 'Resources')
      const launcherPath = join(resourcesPath, 'bin', 'dobius')
      const oldLauncherPath = join(fixture.root, 'Old.app', 'Contents', 'Resources', 'bin', 'dobius')
      await mkdir(commandDir, { recursive: true })
      await mkdir(join(resourcesPath, 'bin'), { recursive: true })
      await writeFile(launcherPath, '#!/usr/bin/env bash\n', 'utf8')
  ...
  // Why: old dev/package experiments wrote a generated Dobius launcher file
  // directly into /usr/local/bin/dobius. That broke profiling because Settings
    ...
    async () => {
      ...
      await writeFile(
        ...
        [
          ...
          `CLI='${oldCliPath}'`,
          'export DOBIUS_NODE_OPTIONS="${NODE_OPTIONS-}"',
          'export DOBIUS_NODE_REPL_EXTERNAL_MODULE="${NODE_REPL_EXTERNAL_MODULE-}"',
          'unset NODE_OPTIONS',
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      await writeFile(
        installPath,
        '#!/usr/bin/env bash\nELECTRON_RUN_AS_NODE=1 /tmp/not-dobius "$@"\n',
        'utf8'
      )
      ...
      await expect(installer.install()).rejects.toThrow('Refusing to replace non-Dobius command')
      await expect(readFile(installPath, 'utf8')).resolves.toContain('/tmp/not-dobius')
    }
  )

  // Why: a dev build can temporarily own the public command on developer
  // machines; packaged Dobius should treat that as stale, not a hard conflict.
  it.skipIf(process.platform === 'win32')(
    'replaces stale sibling dev launcher symlinks from packaged installs',
    async () => {
      const fixture = await makeFixture()
      for (const devLauncherName of ['dobius', 'dobius-dev']) {
        const caseRoot = join(fixture.root, devLauncherName)
        const commandDir = join(caseRoot, 'bin')
        const installPath = join(commandDir, 'dobius')
        const userDataPath = join(caseRoot, 'dobius')
  ...
  it.skipIf(process.platform === 'win32')(
    'falls back to ~/.local/bin/dobius on macOS when /usr/local/bin does not exist',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const absentUsrLocalBin = join(fixture.root, 'usr', 'local', 'bin', 'dobius')
      const installer = new CliInstaller({
        platform: 'darwin',
        isPackaged: true,
        resourcesPath,
      ...
      expect(status.commandPath).toBe(join(homePath, '.local', 'bin', 'dobius'))
      expect(status.state).toBe('not_installed')
      expect(status.supported).toBe(true)

      const installed = await installer.install()
      expect(installed.state).toBe('installed')
      expect(installed.commandPath).toBe(join(homePath, '.local', 'bin', 'dobius'))
      expect(installed.pathConfigured).toBe(true)
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    'uses /usr/local/bin/dobius on macOS when /usr/local/bin exists',
    async () => {
      const fixture = await makeFixture()
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const installPath = join(usrLocalBin, 'dobius')
      const installer = new CliInstaller({
        platform: 'darwin',
        isPackaged: true,
        resourcesPath,
  ...
  // Why: users can have a managed Dobius command in ~/.local/bin even when
  // /usr/local/bin exists; Settings must follow the shell-visible command.
  it.skipIf(process.platform === 'win32')(
    'uses an existing managed macOS dobius command from the shell PATH before /usr/local/bin',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const defaultInstallPath = join(usrLocalBin, 'dobius')
      const userInstallPath = join(userLocalBin, 'dobius')
      const launcherPath = join(resourcesPath, 'bin', 'dobius')
      await mkdir(usrLocalBin, { recursive: true })
  ...
  it.skipIf(process.platform === 'win32')(
    'skips a broken managed macOS dobius symlink before /usr/local/bin',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // ~/.local/bin/dobius must not steal status from /usr/local/bin/dobius.
  it.skipIf(process.platform === 'win32')(
    'keeps the default macOS command when a managed dobius appears later on PATH',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // is still the shell-visible Dobius command until the default path is installed.
  it.skipIf(process.platform === 'win32')(
    'uses a later managed macOS dobius command when the default command is missing',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // Why: bash/zsh skip non-executable PATH entries even at Dobius's configured
  // install slot, then keep looking for a runnable command later in PATH.
  it.skipIf(process.platform === 'win32')(
    'uses a later managed macOS dobius command when the default command is not executable',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const installed = await installer.install()
      expect(installed.commandPath).toBe(userInstallPath)
      await expect(readFile(defaultInstallPath, 'utf8')).resolves.toContain('other-dobius')
    }
  )

  // Why: a non-Dobius command after an empty default install slot can be shadowed
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      expect(installed.state).toBe('installed')
      await expect(readlink(defaultInstallPath)).resolves.toBe(launcherPath)
      await expect(readFile(userInstallPath, 'utf8')).resolves.toContain('other-dobius')
    }
  )

  // Why: an off-PATH ~/.local/bin/dobius must not hijack CLI registration and
  // leave the shell-visible /usr/local/bin command missing.
  it.skipIf(process.platform === 'win32')(
    'ignores managed macOS dobius commands that are not visible on the shell PATH',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  it.skipIf(process.platform === 'win32')(
    'reports a conflict for an unmanaged macOS dobius that shadows the install path',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  it.skipIf(process.platform === 'win32')(
    'skips a non-executable unmanaged macOS dobius before the install path',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // Why: when macCommandPath falls back to ~/.local/bin/dobius on arm64, commandName
  // must still be 'dobius' (not 'dobius-ide' which is Linux-only).
  it.skipIf(process.platform === 'win32')(
    'reports commandName as dobius (not dobius-ide) when falling back to ~/.local/bin on macOS',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const status = await installer.getStatus()
      expect(status.commandName).toBe('dobius')
    }
  )
  ...
  it.skipIf(process.platform === 'win32' || process.getuid?.() === 0)(
    ...
    async () => {
      ...
      const installPath = join(protectedDir, 'bin', 'dobius')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      expect(s1.commandPath).toBe(join(homePath, '.local', 'bin', 'dobius'))
    }
  )

  it('resolves packaged Windows command path to resources/bin/dobius.cmd', async () => {
    const fixture = await makeFixture()
    const localAppDataPath = fixture.root
    const resourcesPath = join(fixture.root, 'resources')
    await mkdir(join(resourcesPath, 'bin'), { recursive: true })
    await writeFile(join(resourcesPath, 'bin', 'dobius.cmd'), '@echo off\n', 'utf8')
    ...
    const installer = new CliInstaller({
      ...
      localAppDataPath,
      userDataPath: fixture.userDataPath,
      execPath: join(localAppDataPath, 'Programs', 'Dobius', 'Dobius.exe'),
      appPath: fixture.appPath,
    ...
    expect(status.commandPath).toBe(
      join(localAppDataPath, 'Programs', 'Dobius', 'resources', 'bin', 'dobius.cmd')
    )
  })
  ...
  it('does not overwrite the packaged Windows launcher while registering PATH', async () => {
    const fixture = await makeFixture()
    const localAppDataPath = fixture.root
    const resourcesPath = join(localAppDataPath, 'Programs', 'Dobius', 'resources')
    const bundledLauncher = join(resourcesPath, 'bin', 'dobius.cmd')
  ...
  it.skipIf(process.platform === 'win32')(
    'resolves to ~/.local/bin/dobius on arm64 even when isPackaged is true',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const absentUsrLocalBin = join(fixture.root, 'usr', 'local', 'bin', 'dobius')
      const resourcesPath = join(fixture.root, 'resources')
      const bundledLauncher = join(resourcesPath, 'bin', 'dobius')
      await mkdir(join(resourcesPath, 'bin'), { recursive: true })
