...
// names, which match the event vocabulary already normalized by Dobius's hook
...
const COPILOT_EVENTS = [
  ...
  // Dobius can normalize it without depending on a PascalCase payload.
  'subagentStart',
  'SubagentStop',
  'PreCompact',
  'Stop',
...
function getConfigPath(): string {
  return join(getCopilotHome(), 'hooks', 'dobius.json')
}
...
function getManagedCommand(scriptPath: string, eventName: string): string {
  return process.platform === 'win32'
    ? `$env:DOBIUS_COPILOT_HOOK_EVENT = '${eventName}'; powershell.exe -NoProfile -ExecutionPolicy Bypass -File ${quotePowerShellPath(scriptPath)}`
    : wrapPosixHookCommand(scriptPath, { DOBIUS_COPILOT_HOOK_EVENT: eventName })
}
...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  if (target === 'local' && process.platform === 'win32') {
    return [
      ...
      // lines so surviving PTYs can refresh to the current Dobius server.
      'if ($env:DOBIUS_AGENT_HOOK_ENDPOINT -and (Test-Path -LiteralPath $env:DOBIUS_AGENT_HOOK_ENDPOINT)) {',
      '  try {',
      '    Get-Content -LiteralPath $env:DOBIUS_AGENT_HOOK_ENDPOINT | ForEach-Object {',
      ...
      '  } catch {}',
      '}',
      'if (-not $env:DOBIUS_AGENT_HOOK_PORT -or -not $env:DOBIUS_AGENT_HOOK_TOKEN -or -not $env:DOBIUS_PANE_KEY) { exit 0 }',
      '$inputData = [Console]::In.ReadToEnd()',
      'if ([string]::IsNullOrWhiteSpace($inputData)) { exit 0 }',
      'try {',
      '  $payload = $inputData | ConvertFrom-Json',
      '  $body = @{',
      '    paneKey = $env:DOBIUS_PANE_KEY',
      '    launchToken = $env:DOBIUS_AGENT_LAUNCH_TOKEN',
      '    tabId = $env:DOBIUS_TAB_ID',
      '    worktreeId = $env:DOBIUS_WORKTREE_ID',
      '    hookEventName = $env:DOBIUS_COPILOT_HOOK_EVENT',
      '    env = $env:DOBIUS_AGENT_HOOK_ENV',
      '    version = $env:DOBIUS_AGENT_HOOK_VERSION',
      '    payload = $payload',
      '  } | ConvertTo-Json -Depth 100',
      "  Invoke-WebRequest -UseBasicParsing -Method Post -Uri ('http://127.0.0.1:' + $env:DOBIUS_AGENT_HOOK_PORT + '/hook/copilot') -Headers @{ 'Content-Type'='application/json'; 'X-Dobius-Agent-Hook-Token'=$env:DOBIUS_AGENT_HOOK_TOKEN } -Body $body -TimeoutSec 2 | Out-Null",
      '} catch {}',
      'exit 0',
      ''
    ].join('\r\n')
  ...
  return [
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${DOBIUS_AGENT_HOOK_PORT}/hook/copilot" \\',
    ...
    '  --data-urlencode "hookEventName=${DOBIUS_COPILOT_HOOK_EVENT}" \\',
...
export class CopilotHookService {
  getStatus(): AgentHookInstallStatus {
    ...
    if (!config) {
      return {
        ...
        detail: 'Could not parse Copilot hooks/dobius.json'
      }
    }
  ...
  async installRemote(sftp: SFTPWrapper, remoteHome: string): Promise<AgentHookInstallStatus> {
    ...
    const remoteConfigPath = `${home}/.copilot/hooks/dobius.json`
    const remoteScriptPath = `${home}/.dobius/agent-hooks/copilot-hook.sh`
    ...
    try {
      ...
      if (!config) {
        return {
          ...
          managedHooksPresent: false,
          detail: 'Could not parse remote Copilot hooks/dobius.json'
        }
      }
      ...
      for (const eventName of COPILOT_EVENTS) {
        ...
        nextHooks[eventName] = [
          ...
          getRemoteManagedHookDefinition(
            wrapPosixHookCommand(remoteScriptPath, { DOBIUS_COPILOT_HOOK_EVENT: eventName })
          )
        ]
      }
      ...
      delete config.disableAllHooks
      config.hooks = nextHooks
      // Why: SSH remotes use POSIX scripts regardless of Dobius's local OS. Write
      // the script before hooks/dobius.json so a partial install cannot point
