...
// so Dobius manages only its own marker-delimited block: install rewrites the
...
// Why: mirror the Claude-compatible events Dobius normalizes for status. Kimi uses
...
const BLOCK_START = '# >>> dobius-managed-kimi-hooks (managed by Dobius; do not edit) >>>'
const BLOCK_END = '# <<< dobius-managed-kimi-hooks <<<'
...
// Dobius generates; escape both defensively anyway.
...
// Dobius-managed script (by filename, so a moved userData path is still swept).
