...
// homedir()/.dobius, and getKimiHome() honors KIMI_CODE_HOME. Point both at a
// temp dir so the local install/remove cycle never touches the real ~/.dobius or
...
beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), 'dobius-kimi-hook-'))
...
const scriptPath = (): string => join(home, '.dobius', 'agent-hooks', 'kimi-hook.sh')
...
describe('KimiHookService', () => {
  ...
  it('keeps user config when installing, then restores it on remove', () => {
    ...
    expect((reinstalled.match(/dobius-managed-kimi-hooks \(/g) ?? []).length).toBe(1)
