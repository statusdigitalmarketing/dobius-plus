...
const COMMAND =
  "if [ -x '/home/u/.dobius/agent-hooks/kimi-hook.sh' ]; then /bin/sh '/home/u/.dobius/agent-hooks/kimi-hook.sh'; fi"
...
describe('kimi managed hooks TOML block', () => {
  ...
  it('is idempotent — reinstalling does not duplicate the block', () => {
    ...
    const markerCount = (twice.match(/dobius-managed-kimi-hooks \(/g) ?? []).length
