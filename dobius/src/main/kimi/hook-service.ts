...
function getManagedScript(): string {
  return [
    '#!/bin/sh',
    // Why: refresh PORT/TOKEN/ENV/VERSION from the current Dobius install so a PTY
    // that survived an Dobius restart still reaches the live listener. See
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${DOBIUS_AGENT_HOOK_PORT}/hook/kimi" \\',
...
export class KimiHookService {
  ...
  // Why: install Dobius's managed Kimi hooks on a remote box over SFTP, mirroring
