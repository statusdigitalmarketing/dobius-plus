...
//   DOBIUS_TELEMETRY_DISABLED=1 → identical to DO_NOT_TRACK for this lane.
//   DOBIUS_DIAGNOSTICS_DISABLED=1 → ALSO disable local file writes. The escape
...
export function resolveObservabilityConsent(): ObservabilityConsent {
  ...
  const dobiusDisabled = envOn('DOBIUS_TELEMETRY_DISABLED')
  const diagnosticsDisabled = envOn('DOBIUS_DIAGNOSTICS_DISABLED')
  const ci = inCI()
  ...
  if (diagnosticsDisabled) {
    return {
      localFileEnabled: false,
      bundleEnabled: false,
      disabledReason: 'dobius_diagnostics_disabled'
    }
  }
  if (dnt || dobiusDisabled) {
    // Local file remains active — DNT is a *network* signal, and the local
    // file never leaves the machine.
    return {
      localFileEnabled: true,
      bundleEnabled: false,
      disabledReason: dnt ? 'do_not_track' : 'dobius_telemetry_disabled'
    }
  }
 ...
 *  `~/Library/Application Support/Dobius/logs/main.trace.ndjson`; we resolve
...
export function getTraceFilePath(): string {
  ...
  try {
    ...
    const home = homedir()
    if (platform() === 'darwin') {
      userData = join(home, 'Library', 'Application Support', 'Dobius')
    } else if (platform() === 'win32') {
      userData = join(process.env.APPDATA ?? home, 'Dobius')
    } else {
      userData = join(home, '.config', 'Dobius')
    }
  }
...
export function initObservability(): ObservabilityConsent {
  ...
  if (!c.localFileEnabled) {
    // Disabled at the CI / DOBIUS_DIAGNOSTICS_DISABLED level — leave the
 ...
 *  `platform` / `arch` / `osRelease` / `dobiusChannel` inputs come from main
...
export function collectDiagnosticBundle(
  meta: Pick<
    ...
    'appVersion' | 'platform' | 'arch' | 'osRelease' | 'dobiusChannel' | 'lookbackMinutes'
  >
): CollectedBundle {
