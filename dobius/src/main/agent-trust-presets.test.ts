...
beforeEach(() => {
  testState.fakeHomeDir = mkdtempSync(join(tmpdir(), 'dobius-trust-presets-'))
  testState.userDataDir = mkdtempSync(join(tmpdir(), 'dobius-trust-presets-user-data-'))
...
describe('markCursorWorkspaceTrusted', () => {
  it('writes ~/.cursor/projects/<slug>/.workspace-trusted with the cwd payload', () => {
    const workspace = mkdtempSync(join(tmpdir(), 'dobius-cursor-ws-'))
    try {
...
describe('markCopilotFolderTrusted', () => {
  it('appends the workspace to trustedFolders in ~/.copilot/config.json', () => {
    const workspace = mkdtempSync(join(tmpdir(), 'dobius-copilot-ws-'))
    try {
...
describe('markCodexProjectTrusted', () => {
  it('writes ~/.codex/config.toml with the project marked trusted', () => {
    const workspace = mkdtempSync(join(tmpdir(), 'dobius-codex-ws-'))
    try {
