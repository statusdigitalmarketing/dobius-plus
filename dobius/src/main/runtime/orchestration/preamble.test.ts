...
describe('buildDispatchPreamble', () => {
  ...
  it('uses dobius CLI by default when devMode is not set', () => {
    const result = buildDispatchPreamble(baseParams())
    expect(result).toContain('dobius orchestration send')
    expect(result).toContain('dobius orchestration check')
    expect(result).toContain('dobius orchestration ask')
  })

  it('uses dobius-dev CLI when devMode is true', () => {
    const result = buildDispatchPreamble(baseParams({ devMode: true }))
    expect(result).toContain('dobius-dev orchestration send')
    expect(result).toContain('dobius-dev orchestration check')
    expect(result).toContain('dobius-dev orchestration ask')
    const fragments = result.split('dobius-dev')
    for (const fragment of fragments) {
      expect(fragment).not.toMatch(/dobius orchestration/)
    }
  })

  it('uses dobius CLI when devMode is false', () => {
