...
// Why: the dispatch preamble teaches agents about Dobius CLI commands for
...
export function buildDispatchPreamble(params: PreambleParams): string {
  // Why: in dev mode, agents must use dobius-dev to connect to the dev runtime's
  // socket. Without this, agents inside the dev Electron app would call the
  // production CLI and talk to the wrong Dobius instance (Section 6.4).
