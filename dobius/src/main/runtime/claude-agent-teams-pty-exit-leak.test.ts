 ...
 * called solely from `DobiusRuntimeService.closeTerminal` (the explicit user-close IPC).
...
function internals(runtime: DobiusRuntimeService): RuntimeInternals {
  return runtime as unknown as RuntimeInternals
}

function registerTeam(runtime: DobiusRuntimeService, ptyId: string, leaderHandle: string): void {
  const { claudeAgentTeams, handleByPtyId } = internals(runtime)
  claudeAgentTeams.createLaunchEnv({
    leaderHandle,
    baseEnv: {},
    shimDir: '/tmp/dobius-shim',
    shimBin: 'dobius'
  })
