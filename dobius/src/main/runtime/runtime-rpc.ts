...
type DobiusRuntimeRpcServerOptions = {
...
export class DobiusRuntimeRpcServer {
  private readonly runtime: DobiusRuntimeService
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      // the `dobius` CLI. Close all transports immediately instead of leaving
  ...
  async stop(): Promise<void> {
    ...
    // Dobius processes during restarts, updates, or development, and stale
...
export function createRuntimeTransportMetadata(
  ...
  if (platform === 'win32') {
    return {
      ...
      endpoint: `\\\\.\\pipe\\dobius-${pid}-${endpointSuffix}`
    }
  }
  return {
    kind: 'unix',
