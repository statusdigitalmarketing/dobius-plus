...
describe('runtime metadata', () => {
  ...
  it('writes and reads runtime metadata atomically', () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dobius-runtime-metadata-'))
  ...
  it('clears the runtime metadata file', () => {
    ...
    expect(getRuntimeMetadataPath(userDataPath)).toContain('dobius-runtime.json')
  })
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    () => {
      const userDataPath = mkdtempSync(join(tmpdir(), 'dobius-runtime-secure-files-'))
      tempDirs.push(userDataPath)
      ...
      for (const path of [
        join(userDataPath, 'dobius-devices.json'),
        join(userDataPath, 'dobius-e2ee-keypair.json'),
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    () => {
      const userDataPath = mkdtempSync(join(tmpdir(), 'dobius-runtime-existing-secure-files-'))
      tempDirs.push(userDataPath)
      ...
      const devicesPath = join(userDataPath, 'dobius-devices.json')
      const keypairPath = join(userDataPath, 'dobius-e2ee-keypair.json')
  ...
  it('replaces oversized E2EE keypair files instead of reading them as metadata', () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dobius-runtime-large-keypair-'))
    tempDirs.push(userDataPath)
    const keypairPath = join(userDataPath, 'dobius-e2ee-keypair.json')
