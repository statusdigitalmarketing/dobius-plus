...
import { readRepoConfigYamlViaProvider } from '../repo-config-yaml'
import {
...
import {
  buildKnownDobiusWorkspaceLayouts,
...
import { RuntimeBrowserCommands } from './dobius-runtime-browser'
...
import { RuntimeEmulatorCommands, setEmulatorBridge } from './dobius-runtime-emulator'
import { serveSimStateWatcher } from '../emulator/serve-sim-state-watcher'
import type { EmulatorBridge } from '../emulator/emulator-bridge'
import { RuntimeFileCommands } from './dobius-runtime-files'
import { RuntimeGitCommands } from './dobius-runtime-git'
...
import {
  ...
  canCleanupUnregisteredDobiusLeftoverDirectory,
...
function createTerminalRevealWarning(handle: string, error?: unknown): string {
  const reason =
    error instanceof Error && error.message.trim().length > 0
      ...
      : ''
  return [
    `Terminal ${handle} is running, but Dobius could not make it discoverable.${reason}`,
    `Run \`dobius terminal focus --terminal ${handle}\` to reveal and focus it.`
  ].join(' ')
}
...
export class DobiusRuntimeService {
  ...
  getStatus(): RuntimeStatus {
    ...
    return {
      ...
      // Why: headless dobius serve cannot create/stream BrowserViews, so clients
  ...
  // RPC, but agents need their own handle at spawn time (via DOBIUS_TERMINAL_HANDLE
  ...
  private adoptControllerTerminalHandle(ptyId: string, handle: string | undefined): void {
    ...
    // original DOBIUS_TERMINAL_HANDLE, but the runtime's in-memory map is gone.
  ...
  // Why: daemon-backed PTYs that the runtime adopted after an Dobius relaunch
  ...
  async addRepo(path: string, kind: 'git' | 'folder' = 'git'): Promise<Repo> {
    ...
    if (!isAbsolute(path)) {
      ...
      // server-side repo paths to be explicit so `dobius serve` cwd is irrelevant.
      throw new Error('Project path must be an absolute path')
    }
  ...
  async getRepoHooks(repoSelector: string) {
    ...
    if (repo.connectionId) {
      ...
      const result = await readRepoConfigYamlViaProvider(
        (p) => fsProvider.readFile(p),
        (name) => joinWorktreeRelativePath(repo.path, name)
      )
      if (!result) {
        return {
          hasHooksFile: false,
      ...
      const hooks = result.isBinary ? null : parseDobiusYaml(result.content)
      return {
        hasHooksFile: Boolean(hooks),
        hooks,
        setupRunPolicy: getEffectiveSetupRunPolicy(repo),
        source: hooks ? 'dobius.yaml' : null,
    ...
    return {
      ...
      hooks,
      setupRunPolicy,
      source: hasFile ? 'dobius.yaml' : hooks ? 'legacy' : null,
      setupTrust: this.getSharedSetupHookTrustPayload(
        repo,
  ...
  async checkRepoHooks(repoSelector: string) {
    ...
    if (repo.connectionId) {
      ...
      try {
        const result = await fsProvider.readFile(joinWorktreeRelativePath(repo.path, 'dobius.yaml'))
        if (result.isBinary) {
          return { hasHooks: false, hooks: null, mayNeedUpdate: false }
        }
        return { hasHooks: true, hooks: parseDobiusYaml(result.content), mayNeedUpdate: false }
      } catch {
        return { hasHooks: false, hooks: null, mayNeedUpdate: false }
      }
    }
    ...
    return {
      hasHooks: has,
      hooks,
      mayNeedUpdate: has && !hooks && hasUnrecognizedDobiusYamlKeys(repo.path)
    }
  }
  ...
  async readRepoIssueCommand(repoSelector: string) {
    ...
    if (repo.connectionId) {
      const issueCommandPath = joinWorktreeRelativePath(repo.path, '.dobius/issue-command')
      const fsProvider = getSshFilesystemProvider(repo.connectionId)
      if (!fsProvider) {
        return {
          localContent: null,
  ...
  private async readRemoteSharedIssueCommand(
    ...
    try {
      const result = await fsProvider.readFile(joinWorktreeRelativePath(repoPath, 'dobius.yaml'))
      if (result.isBinary) {
        return null
      }
      return parseDobiusYaml(result.content)?.issueCommand?.trim() || null
    } catch {
      return null
    }
  }
  ...
  async writeRepoIssueCommand(repoSelector: string, content: string): Promise<{ ok: true }> {
    ...
    if (repo.connectionId) {
      ...
      await fsProvider.createDir(joinWorktreeRelativePath(repo.path, '.dobius'))
      await this.ensureRemoteDobiusDirIgnored(fsProvider, repo.path)
  ...
  private async ensureRemoteDobiusDirIgnored(
    fsProvider: IFilesystemProvider,
    repoPath: string,
    ...
    try {
      ...
      if (!isENOENT(error)) {
        ...
        console.warn('[runtime] Could not inspect remote .gitignore for .dobius', error)
        return
      }
      try {
        await fsProvider.writeFile(gitignorePath, '.dobius\n')
      } catch (writeError) {
        if (options.required) {
          throw writeError
        }
        console.warn('[runtime] Could not update remote .gitignore to exclude .dobius', writeError)
      }
      return
    }
    if (result.isBinary) {
      if (options.required) {
        throw new Error('Remote .gitignore is binary; cannot verify .dobius is ignored')
      }
      return
    }
    if (/^\.dobius\/?$/m.test(result.content)) {
      return
    }
    const separator = result.content.endsWith('\n') ? '' : '\n'
    try {
      await fsProvider.writeFile(gitignorePath, `${result.content}${separator}.dobius\n`)
    } catch (writeError) {
      if (options.required) {
        throw writeError
      }
  ...
  async listDetectedManagedWorktrees(repoSelector: string): Promise<DetectedWorktreeListResult> {
    ...
    const detected = scan.worktrees.map((gitWorktree) => {
      ...
      return {
        ...detectedWorktree,
        visible: true,
        ownership: detectedWorktree.ownership === 'dobius-managed' ? 'dobius-managed' : 'unknown-legacy'
  ...
  private toRuntimeDetectedWorktree(repo: Repo, worktree: Worktree): DetectedWorktree {
    ...
    return toDetectedWorktree({
      ...
      settings,
      knownDobiusLayouts: buildKnownDobiusWorkspaceLayouts(settings, repo),
  ...
  private recordCreatedWorktreeLineage(
    ...
    if (
      ...
      warnings.push({
        ...
        message:
          'Worktree created, but Dobius could not record lineage because instance identity was unavailable.',
        details: {
  ...
  async createManagedWorktree(args: {
    ...
    // Why: CLI-managed WSL worktrees live under ~/dobius/workspaces inside the
    ...
    const displayNameMeta = requestedDisplayName
      ...
      : shouldSetDisplayName(effectiveRequestedName, branchName, effectiveSanitizedName)
        ...
        : {}
    const meta = this.store.setWorktreeMeta(worktreeId, {
      // Why: worktree IDs are path-derived. If a path is deleted outside Dobius
      ...
      dobiusCreatedAt: now,
      dobiusCreationSource: 'runtime',
      dobiusCreationWorkspaceLayout: getWorktreeCreationLayout(repo, settings),
      ...displayNameMeta,
    ...
    // defers to the repo's dobius.yaml setupRunPolicy. runHooks === true maps
    ...
    if (shouldRunSetup && hooks?.scripts.setup) {
      ...
      warning = `dobius.yaml setup hook skipped for ${worktreePath}; pass --setup run to run it.`
    ...
    if (sequencedStartup && this.ptyController?.spawn) {
      try {
        ...
        // session later, matching `dobius terminal create` background semantics.
        const startupTrustAgent = effectiveDraftPaste?.agent ?? effectiveCreatedWithAgent
        if (startupTrustAgent) {
  ...
  async getOrStartRemoteFetch(
    ...
    const promise = this.enqueueRemoteFetch(key, () =>
      ...
      // create on this repo until Dobius restarted (the F2 bug §3.3 pins).
  ...
  async updateManagedWorktreeMeta(
    ...
    this.store.setWorktreeMeta(worktree.id, stripDobiusProvenanceMetaUpdates(persistedMetaUpdates))
  ...
  private async resolveWorktreeRemovalTarget(
    ...
    try {
      ...
      // Only exact IDs with persisted Dobius metadata are accepted here so
    ...
    const removal = (async (): Promise<RemoveWorktreeResult & { warning?: string }> => {
      ...
      if (!registeredWorktree) {
        ...
        if (!repo.connectionId) {
          ...
          if (
            await canCleanupUnregisteredDobiusLeftoverDirectory({
              meta: removedMeta,
        ...
        if (await isRuntimeWorktreePathMissing(repo, removalTarget.path, localWorktreeGitOptions)) {
          if (!force && !removedMeta) {
            ...
            // path before deleting Dobius-only state for an unregistered path.
      ...
      if (hooks?.scripts.archive && runHooks) {
        ...
        // Runtime RPC calls have no renderer trust prompt, so hooks require explicit CLI opt-in.
        warning = `dobius.yaml archive hook skipped for ${canonicalWorktreePath}; pass --run-hooks to run it.`
        console.warn(`[hooks] ${warning}`)
      }
      ...
      try {
        ...
        // Dobius has already validated the target and explicit force delete.
  ...
  async createTerminal(
    ...
    const shouldCreateInBackground =
      ...
      ((!requiresRendererFocus && opts.rendererBacked !== true) ||
        // Why: `dobius serve` exposes the local runtime without a renderer
    ...
    if (shouldCreateInBackground) {
      ...
      const baseEnv = {
        ...launchOpts.env,
        ...(launchToken ? { DOBIUS_AGENT_LAUNCH_TOKEN: launchToken } : {})
      }
  ...
  private async resolveMobileSessionTerminalCommand(
    ...
    const platform = this.getAgentLaunchPlatformForWorkspace(workspace)
    // Why: an SSH workspace runs the CLI through the relay shim (plain `dobius`),
    // so the Linux-only `dobius-ide` rename must not be applied.
  ...
  async prepareClaudeAgentTeamsLeader(args: {
    ...
    if (!handle) {
      throw new Error('claude_agent_teams_requires_dobius_terminal')
    }
  ...
  markRendererReloading(windowId: number): void {
    ...
    // Why: handleByPtyId maps ptyId → pre-allocated CLI handle (DOBIUS_TERMINAL_HANDLE).
  ...
  markGraphUnavailable(windowId: number): void {
    ...
    // Why: once the authoritative renderer graph disappears, Dobius must fail
  ...
  private buildTerminalWorkspaceEnv(
    ...
    return {
      ...env,
      DOBIUS_WORKSPACE_ID: scope.id,
      DOBIUS_PROJECT_GROUP_ID: scope.folderWorkspace.projectGroupId,
      DOBIUS_WORKSPACE_ROOT: scope.folderWorkspace.folderPath
    }
  }
    ...
    if (input.envParentWorkspace) {
      try {
        ...
        warnings.push({
          code: 'LINEAGE_PARENT_CONTEXT_MISSING',
          message:
            'Worktree created, but Dobius+ could not validate the environment parent context.',
    ...
    if (input.callerTerminalHandle) {
      try {
        ...
        warnings.push({
          code: 'LINEAGE_PARENT_CONTEXT_MISSING',
          message:
            'Worktree created, but Dobius could not validate the caller terminal as a parent context.',
        ...
        warnings: [
          {
            ...
            message:
              'Worktree created, but Dobius+ could not prove which parent context caused it.',
            details: {
  ...
  private async computeResolvedWorktrees(generation: number): Promise<ResolvedWorktree[]> {
    ...
    // allowed to change outside Dobius. A short TTL avoids shelling out on every
    ...
    for (const tab of snapshot.tabs) {
      if (tab.type === 'browser') {
        ...
        tabs.push({
          ...
          // active Dobius tab. Preserve the renderer's app-level session focus.
  ...
  private rememberDetachedPreAllocatedLeaves(): void {
    for (const leaf of this.leaves.values()) {
      if (leaf.ptyId && this.handleByPtyId.has(leaf.ptyId)) {
        // Why: DOBIUS_TERMINAL_HANDLE is an agent identity, so CLI control should
  ...
  async linearResolveCurrentIssue(
    ...
    if (!worktree && context?.remote !== true && context?.cwd) {
      ...
      if (!worktree) {
        throw new LinearAgentAccessError(
          'linear_issue_required',
          'Run --current from inside an Dobius-managed worktree or pass an issue id.'
        )
      }
    }
      ...
      await this.runLinearAgentWrite(
        ...
        (cause) =>
          linearError(
            ...
            'Linear may have applied the state change, but Dobius could not confirm it.',
            {
              nextSteps: [
                `Run \`dobius linear issue ${target.issue.identifier} --workspace ${target.workspaceId} --json\` and check the current state before retrying.`
              ],
              ...(cause ? { cause } : {})
            }
          )
    ...
    if (!alreadySet) {
      await this.runLinearAgentWrite(
        ...
        (cause) =>
          linearError(
            'linear_write_unconfirmed',
            'Linear may have applied the task update, but Dobius could not confirm it.',
            {
              nextSteps: [
                `Run \`dobius linear issue ${target.issue.identifier} --workspace ${target.workspaceId} --json\` and check the updated field before retrying.`
              ],
              ...(cause ? { cause } : {})
            }
          )
    ...
    if (compatibleNameMatches.length > 1) {
      throw linearError(
        ...
        `Multiple Linear projects exactly matched "${trimmed}".`,
        {
          ...
          nextSteps: ['Run `dobius linear project list --query <name> --json` and retry by id.']
        }
      )
    }
    ...
    const resolved = inputs.map((input) => {
      ...
      throw linearError(
          ...
          : `Multiple labels exactly matched "${input}".`,
        {
          ...
          nextSteps: ['Run `dobius linear team labels --team <key-or-id> --json` and retry by id.']
        }
      )
    })
    ...
    try {
      ...
      if (!root) {
        throw linearError(
          ...
          {
            nextSteps: ['Run `dobius linear issue <id> --comments --json` to list valid comment ids.']
          }
        )
      }
      return root.id
  ...
  private mapLinearReadFailure(error: unknown): LinearAgentAccessError {
    ...
    if (isLinearAuthError(error)) {
      return linearError('linear_auth_expired', 'Linear authentication expired.', {
        nextSteps: ['Reconnect Linear from Dobius settings.']
      })
    }
  ...
  private async resolveLinearCreateTeam(
    ...
    if (!teamInput) {
      throw linearError('linear_team_required', 'Pass --team or create under a parent issue.', {
        nextSteps: ['Run `dobius linear create --team <key> ...` or use --parent-current.']
      })
    }
    ...
    if (teams.length === 0 && (getLinearStatus().workspaces?.length ?? 0) === 0) {
      throw linearError('linear_not_connected', 'Linear is not connected.', {
        nextSteps: ['Connect Linear from Dobius settings, then retry the issue create.']
      })
    }
  ...
  private linearCreateStyleUnconfirmed(
    ...
    const pinned =
      verb === 'create'
        ...
        : [
            `dobius linear ${verb === 'attach' ? 'attach' : 'comment add'}`,
    ...
    return linearError(
      'linear_write_unconfirmed',
      'Linear may have applied the write, but Dobius could not confirm it.',
      {
        writeId,
        workspaceId,
...
function branchSelectorMatches(branch: string, selector: string): boolean {
  ...
  // or `foo` depending on which plumbing path produced the record. Dobius's
