...
import type { RuntimeBrowserCommandHost } from './dobius-runtime-browser'
    ...
    profiles: new Map([
      ...
      [
        'profile-isolated',
        {
          id: 'profile-isolated',
          scope: 'isolated',
          partition: 'persist:dobius-browser-session-profile-isolated',
          label: 'Isolated',
          source: null
        }
      ]
...
describe('RuntimeBrowserCommands browser screencast', () => {
  ...
  it('waits for explicit worktree browser registration after requesting a hidden mount', async () => {
    const { RuntimeBrowserCommands } = await import('./dobius-runtime-browser')
    const send = vi.fn()
    const bridge = {
