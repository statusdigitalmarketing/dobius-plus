...
import { DobiusRuntimeService } from './dobius-runtime'
import { OrchestrationDb } from './orchestration/db'
import * as runtimeMetadataModule from './runtime-metadata'
import { readRuntimeMetadata } from './runtime-metadata'
import { createRuntimeTransportMetadata, DobiusRuntimeRpcServer } from './runtime-rpc'
...
describe('DobiusRuntimeRpcServer', () => {
  ...
  it('writes runtime metadata with transport details when started', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dobius-runtime-rpc-'))
    const runtime = new DobiusRuntimeService()
    const server = new DobiusRuntimeRpcServer({ runtime, userDataPath })
  ...
  it('creates a pairing offer for the active WebSocket transport', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dobius-runtime-rpc-'))
    const runtime = new DobiusRuntimeService()
    const server = new DobiusRuntimeRpcServer({
      runtime,
      userDataPath,
  ...
  it('preserves proxy path prefixes in web client URLs', async () => {
    ...
    try {
      const offer = server.createPairingOffer({
        address: 'wss://runtime.example.com/dobius',
        name: 'Proxy test'
      })
      expect(offer.available).toBe(true)
      if (offer.available) {
        expect(offer.webClientUrl).toContain('https://runtime.example.com/dobius/web-index.html')
      }
    } finally {
      await server.stop()
    }
  ...
  it('formats pairing-address overrides for IPv6 and host-port tunnel endpoints', async () => {
    ...
    try {
      ...
      if (fullUrl.available) {
        expect(fullUrl.endpoint).toBe('wss://runtime.example.com/dobius')
      }
    } finally {
      await server.stop()
    }
  ...
  it('shares one socket close listener across concurrent WebSocket dispatches', async () => {
    ...
    const server = new DobiusRuntimeRpcServer({ runtime, userDataPath, enableWebSocket: false })
    server['deviceRegistry'] = new DeviceRegistry(userDataPath)
    const entry = server['deviceRegistry']!.addDevice('runtime-test', 'runtime')
    const ws = new FakeWebSocket()
    server['wsConnectionIds'].set(ws as unknown as WebSocket, 'conn-test')
  ...
  it('limits mobile-scoped WebSocket tokens to the mobile RPC surface', async () => {
    ...
    await server['handleWebSocketMessage'](
      JSON.stringify({
        ...
        deviceToken: mobile.token,
        params: { owner: 'statusdigitalmarketing', repo: 'dobius' }
      }),
      (response) => replies.push(JSON.parse(response) as Record<string, unknown>),
      () => {}
    )
    ...
    await server['handleWebSocketMessage'](
      JSON.stringify({
        ...
        deviceToken: mobile.token,
        params: { owner: 'statusdigitalmarketing', repo: 'dobius', seedLogins: ['alex'] }
      }),
      (response) => replies.push(JSON.parse(response) as Record<string, unknown>),
      () => {}
    )
  ...
  it('serves terminal.list and terminal.show for live runtime terminals', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dobius-runtime-rpc-'))
    const runtime = new DobiusRuntimeService(makeStore() as never)
    const writes: string[] = []
  ...
  it('serves worktree.ps from the runtime summary builder', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dobius-runtime-rpc-'))
    const runtime = new DobiusRuntimeService(makeStore({ isUnread: true }) as never)
    const server = new DobiusRuntimeRpcServer({ runtime, userDataPath })
  ...
  describe('orchestration DB migration (§3.2)', () => {
    it('is idempotent when delivered_at already exists', () => {
      ...
      const tmpPath = join(mkdtempSync(join(tmpdir(), 'dobius-orch-mig-')), 'orch.sqlite')
