...
import { getDobiusCliCommandNameForPlatform } from '../../shared/dobius-cli-command-name'
...
export function resolveClaudeAgentTeamsShimBin(
  ...
  if (env.DOBIUS_AGENT_TEAMS_SHIM_BIN) {
    return env.DOBIUS_AGENT_TEAMS_SHIM_BIN
  }
  ...
  return (
    findExecutableOnPath(
      process.platform === 'win32' ? 'dobius-dev.cmd' : 'dobius-dev',
      env.PATH
    ) ??
    findExecutableOnPath(getDobiusCliCommandNameForPlatform(process.platform), env.PATH) ??
    getDobiusCliCommandNameForPlatform(process.platform)
  )
}

function defaultShimRoot(): string {
  return join(homedir(), '.dobius', 'claude-agent-teams-bin')
}
...
function bundledLauncherPath(): string | null {
  ...
  if (process.platform === 'darwin') {
    return join(process.resourcesPath, 'bin', 'dobius')
  }
  if (process.platform === 'linux') {
    return join(process.resourcesPath, 'bin', 'dobius')
  }
  if (process.platform === 'win32') {
    return join(process.resourcesPath, 'bin', 'dobius.cmd')
  }
  return null
}
...
function unixShimScript(): string {
  return [
    '#!/usr/bin/env sh',
    'set -eu',
    `exec "\${DOBIUS_AGENT_TEAMS_SHIM_BIN:-${getDobiusCliCommandNameForPlatform(process.platform)}}" agent-teams-tmux "$@"`,
    ''
  ].join('\n')
}
...
function windowsShimScript(): string {
  return [
    '@echo off',
    'setlocal',
    'if "%DOBIUS_AGENT_TEAMS_SHIM_BIN%"=="" (',
    `  set "DOBIUS_AGENT_TEAMS_SHIM_BIN=${getDobiusCliCommandNameForPlatform(process.platform)}"`,
    ')',
    '"%DOBIUS_AGENT_TEAMS_SHIM_BIN%" agent-teams-tmux %*',
    ''
  ].join('\r\n')
}
