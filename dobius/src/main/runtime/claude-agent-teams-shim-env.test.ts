...
describe('claude agent teams shim env', () => {
  it('writes a private tmux shim that calls the Dobius shim command', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dobius-agent-teams-shim-'))
    roots.push(root)
  ...
  it('builds native shim env only for direct Claude commands', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dobius-agent-teams-cli-'))
    roots.push(root)
    const cliName = process.platform === 'win32' ? 'dobius-dev.cmd' : 'dobius-dev'
