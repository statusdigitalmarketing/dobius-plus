...
// real in-process runtime via DOBIUS_USER_DATA_PATH, and asserts:
...
import { DobiusRuntimeRpcServer } from './runtime-rpc'
...
async function runBuiltCli(
  ...
  const child = spawn(process.execPath, [CLI_PATH, ...args], {
    env: {
      ...process.env,
      DOBIUS_USER_DATA_PATH: userDataPath,
      DOBIUS_TERMINAL_HANDLE: 'term_cli',
      ...extraEnv
    },
...
describeIfBuilt('dobius orchestration check --wait subprocess (§3.4)', () => {
  it('emits newline-flushed JSON heartbeats to stderr while waiting', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dobius-cli-sub-'))
    ...
    try {
      // Why: use the DOBIUS_HEARTBEAT_INTERVAL_MS escape hatch to shrink the
      ...
      const child = spawn(
        ...
        ],
        {
          env: {
            ...process.env,
            DOBIUS_USER_DATA_PATH: userDataPath,
            DOBIUS_TERMINAL_HANDLE: 'term_nobody',
            DOBIUS_HEARTBEAT_INTERVAL_MS: String(heartbeatMs)
          },
...
describeIfBuilt('dobius orchestration reset subprocess', () => {
  it('validates reset scopes against an isolated runtime through the built CLI', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dobius-cli-reset-'))
    const runtime = new DobiusRuntimeService()
    const db = new OrchestrationDb(':memory:')
    runtime.setOrchestrationDb(db)
    const server = new DobiusRuntimeRpcServer({ runtime, userDataPath })
