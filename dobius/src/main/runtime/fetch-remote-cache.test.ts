...
// `DobiusRuntimeService.fetchRemoteWithCache` — in particular that a rejected
...
// without disturbing the large dobius-runtime.test.ts mock surface.
...
// Why: dobius-runtime.ts imports heavy modules (hooks, ipc/*, etc.) at top
...
describe('DobiusRuntimeService.fetchRemoteWithCache', () => {
  beforeEach(() => {
  ...
  it('evicts the in-flight Map entry on rejection so the next caller re-fetches', async () => {
    ...
    const runtime = new DobiusRuntimeService(null)
