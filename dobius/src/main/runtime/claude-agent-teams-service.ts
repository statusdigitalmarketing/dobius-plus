...
export class ClaudeAgentTeamsService {
  ...
  createLaunchEnv(args: {
    ...
    const tmuxValue = `/tmp/dobius-claude-agent-teams/${teamId},0,1`
    const env: Record<string, string> = {
      ...
      DOBIUS_AGENT_TEAMS_TEAM_ID: teamId,
      DOBIUS_AGENT_TEAMS_TOKEN: token,
      DOBIUS_AGENT_TEAMS_LEADER_PANE: leaderPane,
      DOBIUS_AGENT_TEAMS_SHIM_DIR: args.shimDir,
      DOBIUS_AGENT_TEAMS_SHIM_BIN: args.shimBin
    }
    if (args.baseEnv.DOBIUS_PAIRING_CODE) {
      env.DOBIUS_PAIRING_CODE = args.baseEnv.DOBIUS_PAIRING_CODE
    }
    if (args.baseEnv.DOBIUS_ENVIRONMENT) {
      env.DOBIUS_ENVIRONMENT = args.baseEnv.DOBIUS_ENVIRONMENT
    }
