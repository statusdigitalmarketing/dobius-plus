...
vi.mock('../hooks', () => ({
  ...
  getSetupRunnerEnvVars: (_repo: never, worktreePath: string) => ({
    DOBIUS_ROOT_PATH: '/remote/repo',
    DOBIUS_WORKTREE_PATH: worktreePath
  }),
  ...
  parseDobiusYaml: vi.fn().mockReturnValue(null)
}))
...
function resetRuntimeTestMocks(): void {
  ...
  vi.mocked(parseDobiusYaml).mockReset()
  ...
  vi.mocked(parseDobiusYaml).mockReturnValue(null)
...
function antigravityReadyScreen(model = 'Gemini 3.5 Flash (High)'): string {
  return [
    ...
    model,
    '~/dobius/workspaces/dobius/agy-dispatch-issue',
    '>'
  ].join('\n')
}
...
function expectStablePaneKeyEnv(env: Record<string, string>): string {
  expect(env.DOBIUS_TAB_ID).toMatch(UUID_RE)
  const leafId = env.DOBIUS_PANE_KEY?.slice(`${env.DOBIUS_TAB_ID}:`.length)
  expect(leafId).toMatch(UUID_RE)
  expect(env.DOBIUS_PANE_KEY).toBe(`${env.DOBIUS_TAB_ID}:${leafId}`)
  return env.DOBIUS_PANE_KEY
}

function createRuntime(): DobiusRuntimeService {
  return new DobiusRuntimeService(store)
}
...
describe('DobiusRuntimeService', () => {
  it('projects runtime-backed settings to paired clients', () => {
    const runtime = new DobiusRuntimeService({
      ...store,
      getSettings: () => ({
        ...store.getSettings(),
  ...
  it('lists live terminals and issues stable handles for synced leaves', async () => {
    const runtime = new DobiusRuntimeService(store)
    ...
    vi.mocked(listWorktrees).mockResolvedValueOnce([
      {
        ...
        head: 'abc',
        branch: 'refs/heads/wolfiesch/dobius-skill-smoke',
        isBare: false,
        isMainWorktree: false
      }
    ])
  ...
  it('routes SSH-backed forward-slash UNC file and git paths without collapsing the root', async () => {
    ...
    const runtime = new DobiusRuntimeService(remoteStore as never)
  ...
  it.each([
    ...
    const folderPath = await mkdtemp(join(tmpdir(), 'dobius-runtime-folder-files-'))
  ...
  it('lists persisted SSH worktrees while the git provider is unavailable', async () => {
    ...
    const runtime = new DobiusRuntimeService(runtimeStore as never)
  ...
  it('creates additional workspace metadata for folder-mode repos through runtime create', async () => {
    ...
    const runtime = new DobiusRuntimeService(runtimeStore as never, undefined, {
  ...
  it('launches SSH setup terminals for runtime task-created worktrees', async () => {
    ...
    const provider = {
      exec: vi.fn(async (args: string[]) => {
        ...
        if (args[0] === 'rev-parse' && args[1] === '--git-path') {
          return {
            stdout: '/remote/repo/.git/worktrees/mobile-setup/dobius/setup-runner.sh\n',
            stderr: ''
          }
        }
        if (args[0] === 'rev-parse') {
    ...
    try {
      ...
      expect(spawn).toHaveBeenNthCalledWith(
        ...
        expect.objectContaining({
          ...
          command: expect.stringContaining(
            'bash /remote/repo/.git/worktrees/mobile-setup/dobius/setup-runner.sh'
          ),
          worktreeId: result.worktree.id
        })
      )
      ...
      const markerPath = `/remote/repo/.git/worktrees/mobile-setup/dobius/setup-runner.sh.${nonceMatch![1]}.done`
  ...
  it('honors split setup placement for SSH worktrees without startup agents', async () => {
    ...
    const provider = {
      exec: vi.fn(async (args: string[]) => {
        ...
        if (args[0] === 'rev-parse' && args[1] === '--git-path') {
          return {
            stdout: '/remote/repo/.git/worktrees/mobile-setup-split/dobius/setup-runner.sh\n',
            stderr: ''
          }
        }
        if (args[0] === 'rev-parse') {
    ...
    try {
      ...
      expect(setupEnv.DOBIUS_TAB_ID).toBe(initialEnv.DOBIUS_TAB_ID)
      const initialLeafId = initialEnv.DOBIUS_PANE_KEY!.slice(
        `${initialEnv.DOBIUS_TAB_ID!}:`.length
      )
      expect(revealTerminalSession).toHaveBeenLastCalledWith(
        result.worktree.id,
        expect.objectContaining({
          ptyId: 'pty-remote-setup-split',
          tabId: initialEnv.DOBIUS_TAB_ID,
          activate: false,
  ...
  it('reads SSH repo hooks through the SSH filesystem provider', async () => {
    ...
    vi.mocked(parseDobiusYaml).mockReturnValue({ scripts: { setup: 'pnpm install' } })
    registerSshFilesystemProvider('ssh-1', fsProvider as never)
    const runtime = new DobiusRuntimeService(remoteStore as never)
    ...
    expect(fsProvider.readFile).toHaveBeenCalledWith('C:\\remote\\repo\\dobius.yaml')
    expect(hasHooksFile).not.toHaveBeenCalled()
    expect(getEffectiveHooks).not.toHaveBeenCalled()
  })

  it('hashes only the shared dobius.yaml setup script for local run-both hooks', async () => {
  ...
  it('uses remote path joins for SSH hook checks and issue-command files', async () => {
    ...
    const fsProvider = {
      readFile: vi.fn(async (filePath: string) => ({
        content: filePath.endsWith('dobius.yaml')
          ? 'scripts:\n  setup: pnpm install\n'
    ...
    try {
      ...
      await expect(runtime.readRepoIssueCommand('id:repo-1')).resolves.toMatchObject({
        ...
        localFilePath: 'C:\\remote\\repo\\.dobius\\issue-command'
      })
    ...
    expect(fsProvider.readFile).toHaveBeenCalledWith('C:\\remote\\repo\\dobius.yaml')
    expect(fsProvider.readFile).toHaveBeenCalledWith('C:\\remote\\repo\\.dobius\\issue-command')
    expect(fsProvider.createDir).toHaveBeenCalledWith('C:\\remote\\repo\\.dobius')
    expect(fsProvider.writeFile).toHaveBeenCalledWith(
      'C:\\remote\\repo\\.dobius\\issue-command',
      'Ship it\n'
    )
    expect(fsProvider.writeFile).toHaveBeenCalledWith(
  ...
  it('resolves SSH issue commands from shared dobius.yaml and deletes empty overrides', async () => {
    const remoteStore = {
      ...store,
      getRepos: () => [
        {
    ...
    vi.mocked(parseDobiusYaml).mockReturnValue({
      scripts: {},
      issueCommand: 'claude -p "Fix #{{issue}}"'
    })
    const fsProvider = {
      readFile: vi.fn(async (filePath: string) => {
        if (filePath.endsWith('.dobius/issue-command')) {
          throw Object.assign(new Error('missing'), { code: 'ENOENT' })
        }
        if (filePath.endsWith('dobius.yaml')) {
    ...
    try {
      await expect(runtime.readRepoIssueCommand('id:repo-1')).resolves.toMatchObject({
        ...
        localFilePath: '/remote/repo/.dobius/issue-command',
        source: 'shared'
      })
    ...
    expect(fsProvider.readFile).toHaveBeenCalledWith('/remote/repo/dobius.yaml')
    expect(fsProvider.deletePath).toHaveBeenCalledWith('/remote/repo/.dobius/issue-command', false)
    expect(fsProvider.writeFile).not.toHaveBeenCalledWith(
      '/remote/repo/.dobius/issue-command',
      expect.anything()
    )
  })
  ...
  it('routes runtime GitHub repo identity helpers through the selected WSL project runtime', async () => {
    ...
    const runtime = new DobiusRuntimeService(runtimeStore as never)
    getRepoSlugMock.mockResolvedValueOnce({ owner: 'acme', repo: 'dobius' })
    getRepoUpstreamMock.mockResolvedValueOnce({ owner: 'statusdigitalmarketing', repo: 'dobius' })
  ...
  it('routes runtime GitHub issue and work-item actions through the selected WSL project runtime', async () => {
    ...
    createGitHubIssueMock.mockResolvedValueOnce({
      ok: true,
      number: 12,
      url: 'https://github.com/acme/dobius/issues/12'
    })
  ...
  it('routes runtime GitHub PR details and actions through the selected WSL project runtime', async () => {
    ...
    const runtime = new DobiusRuntimeService(runtimeStore as never)
    const localGitOptions = { wslDistro: 'Ubuntu' }
    const prRepo = { owner: 'acme', repo: 'dobius' }
  ...
  it('rejects hosted review worktree selectors outside the selected repo', async () => {
    ...
    const runtime = new DobiusRuntimeService(multiRepoStore as never)
  ...
  it('routes local WSL project hosted review flows through runtime git options', async () => {
    ...
    const runtime = new DobiusRuntimeService(wslStore as never)
    getHostedReviewForBranchMock.mockResolvedValueOnce({
      provider: 'github',
      number: 76,
      title: 'Feature WSL',
      state: 'open',
      url: 'https://github.com/acme/dobius/pull/76',
      status: 'success',
    ...
    createHostedReviewMock.mockResolvedValueOnce({
      ok: true,
      number: 77,
      url: 'https://github.com/acme/dobius/pull/77'
    })
  ...
  it('deduplicates runtime repo paths with Windows/UNC comparison semantics', async () => {
    ...
    const runtime = new DobiusRuntimeService(uncStore as never)
  ...
  it('browses runtime server directories before projects are added', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'dobius-runtime-browse-'))
    try {
  ...
  it('defaults runtime addRepo badgeColor to DEFAULT_REPO_BADGE_COLOR', async () => {
    ...
    const runtime = new DobiusRuntimeService(colorStore as never)
  ...
  it('sets up an existing folder on a fresh runtime after importing the repo project', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'dobius-runtime-project-setup-'))
    ...
    try {
      ...
      expect(result.project.id).toBe('github:statusdigitalmarketing/dobius-plus')
  ...
  it('defaults runtime createRepo badgeColor to DEFAULT_REPO_BADGE_COLOR', async () => {
    ...
    const runtime = new DobiusRuntimeService(colorStore as never)
    const parentDir = await mkdtemp('/tmp/dobius-runtime-create-')
    try {
  ...
  it('creates a missing runtime parent before creating the project directory', async () => {
    ...
    const runtime = new DobiusRuntimeService(createStore as never)
    const tempRoot = await mkdtemp('/tmp/dobius-runtime-create-parent-')
    const parentDir = join(tempRoot, 'dobius', 'projects')
    try {
  ...
  it('prepares the runtime worktree root when creating a repo', async () => {
    ...
    const runtime = new DobiusRuntimeService(runtimeStore as never)
    const parentDir = await mkdtemp('/tmp/dobius-runtime-create-root-prep-')
    try {
  ...
  it('rejects runtime cloneRepo dot-segment URLs before spawning git', async () => {
    const spawnSpy = vi.spyOn(gitRunner, 'wslAwareSpawn')
    const runtime = createRuntime()
    const tempRoot = await mkdtemp(join(tmpdir(), 'dobius-runtime-clone-'))
  ...
  it('removes an owned runtime clone target when git exits unsuccessfully', async () => {
    ...
    const runtime = createRuntime()
    const destination = await mkdtemp(join(tmpdir(), 'dobius-runtime-clone-'))
  ...
  it('emits explicit OSC 9999 agent status from runtime PTY data', () => {
    ...
    const runtime = new DobiusRuntimeService(store, undefined, {
    ...
    expect(statuses).toEqual([
      {
        ...
        source: 'pty-record',
        paneKey,
        tabId: spawnedEnv.DOBIUS_TAB_ID,
        worktreeId: TEST_WORKTREE_ID,
        connectionId: null,
        payload: {
          state: 'done',
  ...
  it('creates visible terminal sessions without asking the renderer to focus a tab', async () => {
    ...
    // pre-allocate the tabId and stamp DOBIUS_PANE_KEY/TAB_ID/WORKTREE_ID into
    ...
    const spawnedLeafId = spawnedEnv.DOBIUS_PANE_KEY.slice(`${spawnedEnv.DOBIUS_TAB_ID}:`.length)
    expect(spawnedEnv.DOBIUS_WORKTREE_ID).toBe(TEST_WORKTREE_ID)
    expect(spawnedEnv.DOBIUS_AGENT_LAUNCH_TOKEN).toMatch(UUID_RE)
    expect(revealTerminalSession).toHaveBeenCalledWith(TEST_WORKTREE_ID, {
      ...
      launchToken: spawnedEnv.DOBIUS_AGENT_LAUNCH_TOKEN,
      activate: false,
      tabId: spawnedEnv.DOBIUS_TAB_ID,
  ...
  it('applies Settings agent defaults to bare agent command terminal creates', async () => {
    ...
    const runtime = new DobiusRuntimeService(runtimeStore)
    runtime.setPtyController({
      spawn,
      write: () => true,
      kill: () => true,
    ...
    expect(spawnCall?.env).toMatchObject({
      ...
      DOBIUS_WORKTREE_ID: TEST_WORKTREE_ID
    })
    expect(spawnCall?.env?.DOBIUS_AGENT_LAUNCH_TOKEN).toMatch(UUID_RE)
  ...
  it('keeps non-bare agent command terminal creates unchanged', async () => {
    ...
    expect(spawnCall?.env?.DOBIUS_AGENT_LAUNCH_TOKEN).toBeUndefined()
  })
  ...
  it('injects runtime hook receiver env into terminal sessions', async () => {
    ...
    expect(spawnCall?.env).toEqual(
      expect.objectContaining({
        ...
        DOBIUS_AGENT_HOOK_ENV: 'remote',
        DOBIUS_AGENT_HOOK_VERSION: '1',
        DOBIUS_PANE_KEY: expect.any(String),
        DOBIUS_TAB_ID: expect.any(String),
        DOBIUS_WORKTREE_ID: TEST_WORKTREE_ID
      })
    )
    expect(spawnCall?.env?.DOBIUS_AGENT_HOOK_ENDPOINT).toBeUndefined()
  })
  ...
  it.each([
    ...
    const folderPath = await mkdtemp(join(tmpdir(), 'dobius-runtime-folder-workspace-'))
    ...
    expectStablePaneKeyEnv(spawnedEnv)
    expect(spawnedEnv.DOBIUS_WORKSPACE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
    expect(spawnedEnv.DOBIUS_PROJECT_GROUP_ID).toBe(TEST_FOLDER_PROJECT_GROUP_ID)
    expect(spawnedEnv.DOBIUS_WORKSPACE_ROOT).toBe(folderPath)
    expect(spawnedEnv.DOBIUS_WORKTREE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
  })
  ...
  it.each([
    ...
    expect(spawnCall?.env?.DOBIUS_WORKTREE_ID).toBe(FLOATING_TERMINAL_WORKTREE_ID)
    expect(spawnCall?.env?.DOBIUS_WORKSPACE_ID).toBeUndefined()
    expect(spawnCall?.env?.DOBIUS_PROJECT_GROUP_ID).toBeUndefined()
    expect(spawnCall?.env?.DOBIUS_WORKSPACE_ROOT).toBeUndefined()
  })

  it('rejects folder workspace terminal creation when the backing path is missing', async () => {
    const missingPath = join(tmpdir(), `dobius-missing-folder-workspace-${randomUUID()}`)
    const spawn = vi.fn().mockResolvedValue({ id: 'pty-folder' })
  ...
  it('rejects folder workspace folderPath updates when the new path is missing', async () => {
    const missingPath = join(tmpdir(), `dobius-missing-folder-update-${randomUUID()}`)
  ...
  it('restores captured native Claude Agent Teams mode with fresh service env', async () => {
    ...
    await runtime.createTerminal(`path:${TEST_WORKTREE_PATH}`, {
      ...
      env: {
        CLAUDE_PROFILE: 'captured',
        DOBIUS_AGENT_TEAMS_TEAM_ID: 'stale-team',
        DOBIUS_AGENT_TEAMS_TOKEN: 'stale-token',
        TMUX: '/tmp/dobius-claude-agent-teams/stale-team,0,1'
      },
      launchAgent: 'claude',
      launchConfig: {
        agentCommand: 'claude',
    ...
    expect(spawnCall?.env?.DOBIUS_AGENT_TEAMS_TEAM_ID).toMatch(/^team-/)
    expect(spawnCall?.env?.DOBIUS_AGENT_TEAMS_TEAM_ID).not.toBe('stale-team')
    expect(spawnCall?.env?.DOBIUS_AGENT_TEAMS_TOKEN).not.toBe('stale-token')
    expect(spawnCall?.env?.TMUX).not.toBe('/tmp/dobius-claude-agent-teams/stale-team,0,1')
    ...
    expect(revealedLaunchConfig?.agentEnv.DOBIUS_AGENT_TEAMS_TEAM_ID).not.toBe('stale-team')
    expect(revealedLaunchConfig?.agentEnv.DOBIUS_AGENT_TEAMS_TOKEN).not.toBe('stale-token')
  })
  ...
  it('adopts renderer pane identity for remote runtime terminal creates', async () => {
    ...
    const spawnedEnv =
      (spawn.mock.calls[0]?.[0] as { env?: Record<string, string> } | undefined)?.env ?? {}
    expect(spawnedEnv.DOBIUS_TAB_ID).toBe(tabId)
    expect(spawnedEnv.DOBIUS_PANE_KEY).toBe(`${tabId}:${leafId}`)
  })
  ...
  it('does not adopt web mirror ids as host terminal ids', async () => {
    ...
    const spawnedEnv =
      (spawn.mock.calls[0]?.[0] as { env?: Record<string, string> } | undefined)?.env ?? {}
    expect(spawnedEnv.DOBIUS_TAB_ID).not.toBe(tabId)
    expect(spawnedEnv.DOBIUS_TAB_ID).not.toMatch(/^web-terminal-/)
    expect(spawnedEnv.DOBIUS_PANE_KEY).toMatch(`${spawnedEnv.DOBIUS_TAB_ID}:`)
  })
  ...
  it('splits visible pty-backed terminal sessions through the parent renderer tab', async () => {
    ...
    const sourceEnv =
      (spawn.mock.calls[0]?.[0] as { env?: Record<string, string> } | undefined)?.env ?? {}
    const sourceLeafId = sourceEnv.DOBIUS_PANE_KEY.slice(`${sourceEnv.DOBIUS_TAB_ID}:`.length)

    await expect(runtime.splitTerminal(handle, { direction: 'vertical' })).resolves.toMatchObject({
      handle: expect.stringMatching(/^term_/),
      tabId: sourceEnv.DOBIUS_TAB_ID,
      paneRuntimeId: -1
    })

    const splitEnv =
      (spawn.mock.calls[1]?.[0] as { env?: Record<string, string> } | undefined)?.env ?? {}
    const splitLeafId = splitEnv.DOBIUS_PANE_KEY.slice(`${sourceEnv.DOBIUS_TAB_ID}:`.length)
    expect(splitTerminal).not.toHaveBeenCalled()
    expect(splitEnv.DOBIUS_TAB_ID).toBe(sourceEnv.DOBIUS_TAB_ID)
    expect(splitEnv.DOBIUS_WORKTREE_ID).toBe(TEST_WORKTREE_ID)
    ...
    const siblingSurfaces = publishedTabs.filter(
      (tab): tab is Extract<typeof tab, { type: 'terminal' }> =>
        tab.type === 'terminal' && tab.parentTabId === sourceEnv.DOBIUS_TAB_ID
    )
  ...
  it('splits folder workspace pty-backed terminal sessions with folder cwd and env', async () => {
    const folderPath = await mkdtemp(join(tmpdir(), 'dobius-runtime-folder-split-'))
    const spawn = vi
      .fn()
    ...
    expect(splitEnv.DOBIUS_TAB_ID).toBe(sourceEnv.DOBIUS_TAB_ID)
    expect(splitEnv.DOBIUS_WORKSPACE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
    expect(splitEnv.DOBIUS_PROJECT_GROUP_ID).toBe(TEST_FOLDER_PROJECT_GROUP_ID)
    expect(splitEnv.DOBIUS_WORKSPACE_ROOT).toBe(folderPath)
    expect(splitEnv.DOBIUS_WORKTREE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
  ...
  it('returns an actionable discoverability warning when default adoption fails after spawn', async () => {
    ...
    try {
      ...
      expect(created.warning).toContain(`dobius terminal focus --terminal ${created.handle}`)
      const spawnCall = spawn.mock.calls[0]?.[0] as { env?: Record<string, string> } | undefined
      const spawnedEnv = spawnCall?.env ?? {}
      expectStablePaneKeyEnv(spawnedEnv)
      const spawnedLeafId = spawnedEnv.DOBIUS_PANE_KEY.slice(`${spawnedEnv.DOBIUS_TAB_ID}:`.length)
  ...
  it('resolves tui-idle from a Codex ready prompt preview', async () => {
    ...
    runtime.onPtyData(
      ...
      [
        ...
        ' directory:   ~/dobius/workspaces/dobius/cli-debug\n'
      ].join(''),
      Date.now()
    )
    ...
    runtime.onPtyData(
      ...
      [
        ...
        [
          ...
          'Run /review on my current changes gpt-5.5 high ~/dobius/workspaces/dobius/cli-debug',
          'Run /review on my current changes gpt-5.5 high ~/dobius/workspaces/dobius/cli-debug',
          'Run /review on my current changes gpt-5.5 high ~/dobius/workspaces/dobius/cli-debug',
          'Run /review on my current changes gpt-5.5 high ~/dobius/workspaces/dobius/cli-debug\n'
        ].join('')
  ...
  // Why: PR #2553 fixed Dobius CLI terminal reads so older retained output stays
  ...
  it('strips retained terminal path line and hash locators before matching', () => {
    const colonPath = '/tmp/dobius report/result.json'
  ...
  it('keeps tmpdir artifact paths in bounded PTY path candidates', () => {
    const artifactPath = join(tmpdir(), 'dobius-runtime-retained-result.json')
    const candidates = appendRecentPtyPathCandidates(undefined, `wrote ${artifactPath}\n`)
  ...
  it('matches terminal artifact paths only when they appear in recent terminal output', () => {
    expect(
      recentTerminalOutputIncludesPath(
        'wrote /tmp/dobius report/result.json:12:3',
        '/tmp/dobius report/result.json',
        '/tmp/dobius report/result.json'
      )
    ).toBe(true)
    expect(
      recentTerminalOutputIncludesPath(
        '\x1b]8;;file:///tmp/dobius%20report/result.json\x1b\\result\x1b]8;;\x1b\\',
        '/tmp/dobius report/result.json',
        '/tmp/dobius report/result.json'
      )
    ).toBe(true)
    ...
    expect(
      recentTerminalOutputIncludesPath(
        'wrote /tmp/dobius report/other.json',
        '/tmp/dobius report/result.json',
        '/tmp/dobius report/result.json'
      )
    ).toBe(false)
  ...
  it('adopts preallocated DOBIUS_TERMINAL_HANDLE as a valid runtime handle', async () => {
    const runtime = new DobiusRuntimeService(store)
  ...
  it('recovers exported DOBIUS_TERMINAL_HANDLE from discovered live PTY sessions', async () => {
    const runtime = new DobiusRuntimeService(store)
    const writes: string[] = []
    runtime.setPtyController({
      write: (_ptyId, data) => {
  ...
  it('creates mobile session terminals for folder workspaces in a headless runtime server', async () => {
    const folderPath = await mkdtemp(join(tmpdir(), 'dobius-mobile-folder-workspace-'))
  ...
  it('keeps the activated headless tab active across PTY republishes (serve focus-jump regression)', async () => {
    // Why: in `dobius serve`, focusTerminal has no renderer to persist the remote
  ...
  it('rejects SSH lineage updates when Dobius worktree identity is missing', async () => {
    const remoteRepo = {
      id: 'remote-repo',
      path: '/home/user/repo',
      displayName: 'remote',
  ...
  it('rejects local lineage updates when Dobius worktree identity is missing', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'dobius-runtime-lineage-'))
    ...
    try {
      ...
      await expect(lstat(join(childPath, '.dobius'))).rejects.toThrow()
      await expect(lstat(join(parentPath, '.dobius'))).rejects.toThrow()
      expect(setWorktreeLineage).not.toHaveBeenCalled()
    } finally {
      await rm(tempRoot, { recursive: true, force: true })
    }
  ...
  it('strips Dobius provenance fields from runtime metadata updates', async () => {
    const metaById: Record<string, WorktreeMeta> = {
    ...
    await runtime.updateManagedWorktreeMeta(`id:${TEST_WORKTREE_ID}`, {
      ...
      dobiusCreatedAt: 123,
      dobiusCreationSource: 'runtime',
      dobiusCreationWorkspaceLayout: { path: '/tmp', nestWorkspaces: false }
    })
  ...
  it('keeps cwd-inferred lineage best-effort when the cwd parent cannot be resolved', async () => {
    ...
    expect(result.warnings).toEqual([
      expect.objectContaining({
        ...
        message:
          'Worktree created, but Dobius could not validate the current directory as a parent context.'
      })
    ])
  })
  ...
  it('returns a setup launch payload for CLI-created worktrees when hooks are explicitly enabled', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      runnerScriptPath: '/tmp/repo/.git/dobius/setup-runner.sh',
      envVars: {
        DOBIUS_ROOT_PATH: '/tmp/repo',
        DOBIUS_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-test'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValueOnce([
      {
  ...
  it('passes setup payloads through when explicitly activating CLI-created worktrees', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        DOBIUS_ROOT_PATH: '/tmp/repo',
        DOBIUS_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-activate'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValueOnce([
      {
  ...
  it('follows normal setup policy for CLI-created worktrees without activating them', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        DOBIUS_ROOT_PATH: '/tmp/repo',
        DOBIUS_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-skip'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        cwd: '/tmp/workspaces/runtime-hook-skip',
        command: 'bash /tmp/repo/.git/dobius/setup-runner.sh',
        // Why: createTerminal stamps DOBIUS_PANE_KEY/TAB_ID/WORKTREE_ID into the
        // PTY env on top of the caller-supplied env so hook-based agent status
        // can attribute hook events to a stable pane.
        env: expect.objectContaining({
          DOBIUS_ROOT_PATH: '/tmp/repo',
          DOBIUS_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-skip',
          DOBIUS_TAB_ID: expect.stringMatching(UUID_RE),
          DOBIUS_PANE_KEY: expect.any(String),
          DOBIUS_WORKTREE_ID: result.worktree.id
        }),
        worktreeId: result.worktree.id
      })
    )
    ...
    const setupLeafId = setupSpawnEnv.DOBIUS_PANE_KEY.slice(
      `${setupSpawnEnv.DOBIUS_TAB_ID}:`.length
    )
    expect(revealTerminalSession).toHaveBeenLastCalledWith(result.worktree.id, {
      ptyId: 'pty-setup',
      title: 'Setup',
      activate: false,
      tabId: setupSpawnEnv.DOBIUS_TAB_ID,
      leafId: setupLeafId
    })
  })
  ...
  it('sequences setup before startup for opted-in local headless worktree creates', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        DOBIUS_ROOT_PATH: '/tmp/repo',
        DOBIUS_WORKTREE_PATH: '/tmp/workspaces/runtime-headless-startup-setup'
      },
  ...
  it('starts setup and startup side by side by default for local headless worktree creates', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        DOBIUS_ROOT_PATH: '/tmp/repo',
        DOBIUS_WORKTREE_PATH: '/tmp/workspaces/runtime-headless-parallel'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({ command: 'bash /tmp/repo/.git/dobius/setup-runner.sh' })
    )
  })
  ...
  it('creates the first terminal for CLI-created worktrees without activating them', async () => {
    ...
    const initialLeafId = initialSpawnEnv.DOBIUS_PANE_KEY.slice(
      `${initialSpawnEnv.DOBIUS_TAB_ID}:`.length
    )
    expect(revealTerminalSession).toHaveBeenCalledWith(result.worktree.id, {
      ptyId: 'pty-created-worktree',
      title: null,
      activate: false,
      tabId: initialSpawnEnv.DOBIUS_TAB_ID,
  ...
  it('honors split setup placement for CLI-created worktrees without startup agents', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        DOBIUS_ROOT_PATH: '/tmp/repo',
        DOBIUS_WORKTREE_PATH: '/tmp/workspaces/runtime-cli-setup-split'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(setupEnv.DOBIUS_TAB_ID).toBe(mainEnv.DOBIUS_TAB_ID)
    const mainLeafId = mainEnv.DOBIUS_PANE_KEY!.slice(`${mainEnv.DOBIUS_TAB_ID!}:`.length)
    expect(revealTerminalSession).toHaveBeenLastCalledWith(
      result.worktree.id,
      expect.objectContaining({
        ptyId: 'pty-cli-setup-setup',
        tabId: mainEnv.DOBIUS_TAB_ID,
        activate: false,
  ...
  it('uses desktop task agent selection and bracketed-pastes startup drafts for local worktrees', async () => {
    ...
    const draftUrl = 'https://github.com/statusdigitalmarketing/dobius-plus/issues/123'
    const result = await runtime.createManagedWorktree({
      repoSelector: 'id:repo-1',
  ...
  it('records the resolved fallback agent when the requested startup draft agent is disabled', async () => {
    ...
    const result = await runtime.createManagedWorktree({
      ...
      startupDraft: 'https://github.com/statusdigitalmarketing/dobius-plus/issues/456',
      createdWithAgent: 'codex',
      activate: true
    })
  ...
  it('honors split setup placement for opted-in local startup-draft worktrees', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        DOBIUS_ROOT_PATH: '/tmp/repo',
        DOBIUS_WORKTREE_PATH: '/tmp/workspaces/runtime-startup-setup-split'
      },
      waitForAgentStartup: true
    })
    vi.mocked(listWorktrees).mockResolvedValue([
    ...
    const result = await runtime.createManagedWorktree({
      ...
      startupDraft: 'https://github.com/statusdigitalmarketing/dobius-plus/issues/123',
      setupDecision: 'run',
      activate: true
    })
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        cwd: '/tmp/workspaces/runtime-startup-setup-split',
        command: expect.stringContaining('bash /tmp/repo/.git/dobius/setup-runner.sh'),
        env: expect.objectContaining({
          DOBIUS_ROOT_PATH: '/tmp/repo',
          DOBIUS_WORKTREE_PATH: '/tmp/workspaces/runtime-startup-setup-split',
          DOBIUS_WORKTREE_ID: result.worktree.id
    ...
    const nonceMatch = startupCommand.match(/if \[ "\$seen" = ([0-9a-f-]+) \]/)
    expect(nonceMatch?.[1]).toBeTruthy()
    const markerPath = `/tmp/repo/.git/dobius/setup-runner.sh.${nonceMatch![1]}.done`
    ...
    const setupEnv = (spawn.mock.calls[1]![0] as { env?: Record<string, string> }).env ?? {}
    expect(result.setup).toBeUndefined()
    expect(mainEnv.DOBIUS_TAB_ID).toBeDefined()
    expect(mainEnv.DOBIUS_PANE_KEY).toBeDefined()
    expect(setupEnv.DOBIUS_TAB_ID).toBe(mainEnv.DOBIUS_TAB_ID)
    const mainLeafId = mainEnv.DOBIUS_PANE_KEY!.slice(`${mainEnv.DOBIUS_TAB_ID!}:`.length)
    expect(revealTerminalSession).toHaveBeenLastCalledWith(
      result.worktree.id,
  ...
  it('passes the wrapped setup command to activation when startup spawned but setup did not', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        DOBIUS_ROOT_PATH: '/tmp/repo',
        DOBIUS_WORKTREE_PATH: '/tmp/workspaces/runtime-startup-setup-retry'
      },
      waitForAgentStartup: true
    })
    vi.mocked(listWorktrees).mockResolvedValue([
  ...
  it('lets explicit startup draft agents override the desktop default', async () => {
    ...
    const draftUrl = 'https://github.com/statusdigitalmarketing/dobius-plus/issues/789'
    const result = await runtime.createManagedWorktree({
      repoSelector: 'id:repo-1',
  ...
  it('detects agents on the SSH host before launching remote startup drafts', async () => {
    ...
    const draftUrl = 'https://github.com/statusdigitalmarketing/dobius-plus/pull/456'
    const result = await runtime.createManagedWorktree({
      repoSelector: TEST_REPO_ID,
  ...
  it('pre-marks remote Codex workspaces trusted before pasting startup drafts', async () => {
    ...
    try {
      const result = await runtime.createManagedWorktree({
        ...
        startupDraft: 'https://github.com/statusdigitalmarketing/dobius-plus/issues/789'
      })
  ...
  it('normalizes runtime GitLab issue list arguments like the desktop IPC path', async () => {
    const runtime = new DobiusRuntimeService(store as never)
  ...
  it('routes runtime worktree creation through the selected WSL project runtime', async () => {
    ...
    const gitSpy = vi.spyOn(gitRunner, 'gitExecFileAsync').mockImplementation(async (args) => {
      ...
      if (args[0] === 'remote' && args[1] === 'get-url') {
        return { stdout: 'git@github.com:statusdigitalmarketing/dobius-plus.git\n', stderr: '' }
      }
      return { stdout: '', stderr: '' }
    })
    ...
    try {
      const result = await runtime.createManagedWorktree({
        ...
        name: 'runtime-wsl',
        pushTarget: {
          remoteName: 'pr-contributor-dobius',
          branchName: 'contributor/runtime-wsl',
          remoteUrl: 'git@github.com:contributor/dobius.git'
        }
      })
      ...
      expect(gitSpy).toHaveBeenCalledWith(
        [
          'fetch',
          'pr-contributor-dobius',
          '+refs/heads/contributor/runtime-wsl:refs/remotes/pr-contributor-dobius/contributor/runtime-wsl'
        ],
        { cwd: TEST_REPO_PATH, wslDistro: 'Ubuntu' }
      )
      expect(gitSpy).toHaveBeenCalledWith(
        [
          'branch',
          '--set-upstream-to',
          'pr-contributor-dobius/contributor/runtime-wsl',
          'runtime-wsl'
        ],
  ...
  it('skips archive hooks for CLI worktree removal by default', async () => {
    ...
    expect(result.warning).toBe(
      `dobius.yaml archive hook skipped for ${TEST_WORKTREE_PATH}; pass --run-hooks to run it.`
    )
  })
  ...
  it('recovers forced Windows runtime long-path removal and keeps skipped-hook warnings', async () => {
    ...
    try {
      ...
      expect(result).toEqual({
        ...
        warning: `dobius.yaml archive hook skipped for ${TEST_WORKTREE_PATH}; pass --run-hooks to run it.`
      })
  ...
  it('treats forced runtime deletion of an already-missing unregistered worktree as cleanup', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'dobius-runtime-remove-'))
  ...
  it('force-removes a legacy Dobius-created runtime orphaned worktree directory after Git tracking is gone', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'dobius-runtime-orphan-'))
    ...
    const runtime = new DobiusRuntimeService(runtimeStoreWithRepoPath as never)
    const notifier = { worktreesChanged: vi.fn() }
    runtime.setNotifier(notifier as never)
  ...
  it('prompts then force-removes an Dobius-created runtime unregistered leftover directory with no git marker', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'dobius-runtime-leftover-'))
    const repoPath = join(parentDir, 'repo')
  ...
  it('rejects an Dobius-created runtime unregistered local directory with a git directory', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'dobius-runtime-standalone-'))
    const repoPath = join(parentDir, 'repo')
  ...
  it('does not inspect or delete a local path when SSH runtime orphan cleanup has no filesystem provider', async () => {
    const localPath = await mkdtemp(join(tmpdir(), 'dobius-runtime-ssh-missing-fs-'))
    const repo = {
  ...
  it('still rejects forced runtime unregistered delete paths that exist on disk', async () => {
    const existingWorktreePath = await mkdtemp(join(tmpdir(), 'dobius-runtime-remove-existing-'))
  ...
  async function reconcileWithToken(runtime: DobiusRuntimeService, token: string): Promise<void> {
    await runtime.reconcileWorktreeBaseStatus({
      repoId: TEST_REPO_ID,
      repoPath: TEST_REPO_PATH,
      worktreeId: TEST_WORKTREE_ID,
  ...
  describe('removeManagedWorktree PTY teardown (design §4.3)', () => {
    ...
    it('thunk resolves the installed provider lazily, not at construction time', async () => {
      // Simulates the daemon adapter being installed AFTER DobiusRuntimeService
