...
// app.setName('Dobius') changing how app.getPath('userData') resolves (e.g. from
// lowercase 'dobius' to uppercase 'Dobius' on a case-sensitive filesystem) — the
...
describe('mobile pairing userData path stability', () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'dobius-pairing-path-'))
  ...
  it('keeps returning the path captured before app.setName changes resolution', async () => {
    ...
    // app.setName('Dobius') happens later in startup, changing late resolution.
  ...
  it('writes DeviceRegistry + E2EE keypair under the canonical path, not the late one', async () => {
    ...
    appState.userData = lateDir // app.setName('Dobius') has run by the time the runtime starts

    const { DeviceRegistry } = await import('./device-registry')
    const { loadOrCreateE2EEKeypair } = await import('./e2ee-keypair')

    // Mirrors DobiusRuntimeRpcServer.start(): both read from the same userDataPath.
    ...
    // Pairing credentials land beside dobius-data.json so they survive restarts/updates.
