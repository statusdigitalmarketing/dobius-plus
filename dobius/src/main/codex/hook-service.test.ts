...
beforeEach(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'dobius-codex-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'dobius-codex-user-data-'))
  previousUserDataPath = process.env.DOBIUS_USER_DATA_PATH
  process.env.DOBIUS_USER_DATA_PATH = userDataDir
...
describe('CodexHookService', () => {
  it('installs PermissionRequest with trust so Codex approval prompts reach Dobius', () => {
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'dobius home with spaces')
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const metacharHome = join(tmpdir(), 'dobius %DOBIUS_TEST% ^ home')
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      ...
      const cmdSafe = /^[A-Za-z0-9_.:\\~-]+$/.test(join(tmpHome, '.dobius', 'agent-hooks'))
      if (cmdSafe) {
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    async () => {
      ...
      const scriptPath = join(homedir(), '.dobius', 'agent-hooks', 'codex-hook.cmd')
      ...
      try {
        ...
        // Why: this suite may run inside an Dobius-launched terminal whose env
        // already carries DOBIUS_AGENT_HOOK_ENDPOINT/PORT/TOKEN. The managed
        // script sources that endpoint file, so leave it out or the hook posts
        // to the live Dobius instead of this test's listener.
        const cleanEnv = { ...process.env }
        for (const key of Object.keys(cleanEnv)) {
          if (key.startsWith('DOBIUS_') || key.startsWith('DOBIUS_')) {
            delete cleanEnv[key]
          }
        }
        const child = spawn('cmd.exe', ['/d', '/c', scriptPath], {
          env: {
            ...cleanEnv,
            DOBIUS_AGENT_HOOK_PORT: String(port),
            DOBIUS_AGENT_HOOK_TOKEN: 'tok123',
            DOBIUS_PANE_KEY: '42:leaf-abc',
            DOBIUS_TAB_ID: '42',
            DOBIUS_WORKTREE_ID: 'C:\\work trees\\my repo & co',
        ...
        expect(received.headers['x-dobius-agent-hook-token']).toBe('tok123')
  ...
  it('keeps hooks isolated by Dobius userData instead of mutating system ~/.codex', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    ...
    const devUserDataDir = mkdtempSync(join(tmpdir(), 'dobius-dev-codex-user-data-'))
    const prodUserDataDir = mkdtempSync(join(tmpdir(), 'dobius-prod-codex-user-data-'))
    try {
      getPathMock.mockImplementation((name: string) => {
        if (name === 'userData') {
      ...
      process.env.DOBIUS_USER_DATA_PATH = devUserDataDir
      expect(new CodexHookService().install().state).toBe('installed')
      ...
      process.env.DOBIUS_USER_DATA_PATH = prodUserDataDir
      expect(new CodexHookService().install().state).toBe('installed')
  ...
  it('refreshes runtime user hooks without installing Dobius-managed hooks', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(
  ...
  it('removes legacy Dobius-managed hooks from system ~/.codex during install', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
  ...
  it('removes very large legacy Dobius-managed hook lists from system ~/.codex', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
    const legacyScriptPath = join(
      tmpHome,
  ...
  it('removes the legacy Dobius Codex profile file when it only contains managed hooks', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const profilePath = join(systemCodexHome, 'dobius-agent-status.config.toml')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(
      profilePath,
      [
        '# BEGIN DOBIUS AGENT STATUS HOOKS',
        '[[hooks.PermissionRequest]]',
        '[[hooks.PermissionRequest.hooks]]',
        'type = "command"',
        'command = "codex-hook"',
        '# END DOBIUS AGENT STATUS HOOKS',
        ''
      ].join('\n'),
      'utf-8'
    )
  ...
  it('removes only the legacy Dobius block from a user-edited Codex profile file', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const profilePath = join(systemCodexHome, 'dobius-agent-status.config.toml')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(
    ...
    expect(profileConfig).not.toContain('DOBIUS AGENT STATUS HOOKS')
  ...
  it('cleans duplicate Codex hook representations while keeping status hooks in runtime CODEX_HOME', () => {
    ...
    const legacyProfilePath = join(systemCodexHome, 'dobius-agent-status.config.toml')
    const legacyScriptPath = join(
      tmpHome,
      '.dobius',
      'agent-hooks',
  ...
  it('removes managed trust entries when userData resolves through a symlink', () => {
    ...
    process.env.DOBIUS_USER_DATA_PATH = linkedUserDataDir
