...
export function syncSystemCodexSessionsIntoManagedHome(): void {
  ...
  const managedSessionsRoot = join(getDobiusManagedCodexHomePath(), 'sessions')
...
function replaceSymlinkSessionBridgeWithHardlink(
  ...
  try {
    ...
    replacementPath = `${targetPath}.dobius-link-${process.pid}-${Date.now()}`
...
function getLegacySessionCopyMarkerPath(relativePath: string): string {
  return join(getDobiusManagedCodexHomePath(), '.dobius-session-copies', `${relativePath}.json`)
}
