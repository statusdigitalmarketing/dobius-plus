...
const REAL_APPROVED_COMMAND = '/bin/sh "/tmp/dobius-case-b-mCmCe6/agent-hooks/codex-hook.sh"'
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'dobius-codex-trust-test-'))
...
describe('computeTrustKey', () => {
  ...
  it('uses native Windows backslashes in the trust key Codex looks up', () => {
    ...
    const winPath = 'C:\\Users\\Rod\\AppData\\Roaming\\dobius\\hooks.json'
    ...
    expect(key.startsWith('C:\\Users\\Rod\\AppData\\Roaming\\dobius\\hooks.json:')).toBe(true)
  })
...
describe('upsertHookTrustEntries', () => {
  ...
  it('collapses duplicate blocks for the same hook key while preserving unrelated hook state', () => {
    const sourcePath = 'C:\\Users\\me\\AppData\\Roaming\\dobius\\codex-runtime-home\\home\\hooks.json'
    const key = `${sourcePath}:session_start:0:0`
    const unrelatedSourcePath =
      'C:\\Users\\me\\AppData\\Roaming\\dobius\\codex-runtime-home\\home\\hooks.json'
  ...
  it('preserves `enabled = false` when the user hand-edited it before reinstall', () => {
    ...
    // mute Dobius's hook short of full uninstall.
    const key = '/x/hooks.json:pre_tool_use:0:0'
    const original = `[hooks.state."${key}"]\nenabled = false\ntrusted_hash = "sha256:OLD"\n`
    writeFileSync(configPath, original, 'utf-8')
  ...
  it('finds and replaces a legacy forward-slash block when Dobius upserts with native backslash key', () => {
    // Why: Codex 0.140 can expose Windows trust keys with either separator
    // shape depending on startup cwd, so Dobius replaces stale blocks with both.
    const backslashPath = 'C:\\Users\\Rod\\AppData\\Roaming\\dobius\\hooks.json'
  ...
  it('produces exactly one Windows separator pair after two consecutive upserts', () => {
    ...
    const entry: CodexTrustEntry = {
      sourcePath: 'C:\\Users\\Rod\\AppData\\Roaming\\dobius\\hooks.json',
      eventLabel: 'session_start',
      groupIndex: 0,
      handlerIndex: 0,
      command: 'echo session'
  ...
  it('falls back to TOML basic-string headers when a Windows path contains an apostrophe', () => {
    ...
    const entry: CodexTrustEntry = {
      sourcePath: "C:\\Users\\O'Connor\\AppData\\Roaming\\dobius\\hooks.json",
      eventLabel: 'session_start',
      groupIndex: 0,
      handlerIndex: 0,
      command: 'echo session'
    ...
    expect((written.match(/\[hooks\.state\."/g) ?? []).length).toBe(2)
    expect(written).toContain(
      `[hooks.state."C:\\\\Users\\\\O'Connor\\\\AppData\\\\Roaming\\\\dobius\\\\hooks.json:session_start:0:0"]`
    )
    expect(written).toContain(
      `[hooks.state."C:/Users/O'Connor/AppData/Roaming/dobius/hooks.json:session_start:0:0"]`
    )
  ...
  it.skipIf(process.platform !== 'win32')(
    'finds a Codex-written block with lowercased username when Dobius key has mixed-case username',
    () => {
      // Why: realpathSync.native casing can differ between what Codex wrote
      // (C:\Users\rod\...) and what Dobius resolves (C:\Users\Rod\...).
      // normalizeHookTrustKeyForLookup case-folds on Windows so the existing block is
      // replaced rather than a duplicate appended.
      const lowercasePath = 'C:\\Users\\rod\\AppData\\Roaming\\dobius\\hooks.json'
      const mixedCasePath = 'C:\\Users\\Rod\\AppData\\Roaming\\dobius\\hooks.json'
...
describe('upsertProjectTrustLevel', () => {
  ...
  it('updates an existing legacy Windows forward-slash project block', () => {
    // Why: older Dobius builds normalized Windows project paths to forward
...
describe('readHookTrustEntries', () => {
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      ...
      const rawKey = 'C:\\Users\\rod\\AppData\\Roaming\\dobius\\hooks.json:session_start:0:0'
      const lookupKey = 'C:/Users/Rod/AppData/Roaming/dobius/hooks.json:session_start:0:0'
      const original = [
        `[hooks.state.'${rawKey}']`,
        'enabled = true',
