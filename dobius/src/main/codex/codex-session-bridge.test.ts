...
function writeLegacyCopyMarker(relativePath: string, sourcePath: string, targetPath: string): void {
  ...
  const markerPath = join(getRuntimeCodexHomePath(), '.dobius-session-copies', `${relativePath}.json`)
...
beforeEach(() => {
  ...
  fakeHomeDir = mkdtempSync(join(tmpdir(), 'dobius-codex-session-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'dobius-codex-session-user-data-'))
