...
beforeEach(() => {
  ...
  fsMockState.failSymlink = false
  fakeHomeDir = mkdtempSync(join(tmpdir(), 'dobius-codex-resource-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'dobius-codex-resource-user-data-'))
...
describe('syncSystemCodexResourcesIntoManagedHome', () => {
  it('uses DOBIUS_USER_DATA_PATH when Electron cannot be required', async () => {
    vi.resetModules()
    vi.doMock('electron', () => {
      throw new Error('electron unavailable in packaged CLI')
    })
    const previousUserDataPath = process.env.DOBIUS_USER_DATA_PATH
    process.env.DOBIUS_USER_DATA_PATH = userDataDir
    try {
      const { getDobiusManagedCodexHomePath: getCliSafeManagedPath } =
