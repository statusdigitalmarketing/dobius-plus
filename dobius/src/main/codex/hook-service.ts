...
import { getDobiusManagedCodexHomePath, getSystemCodexHomePath } from './codex-home-paths'
...
// without a decision so Codex still shows its normal approval UI, while Dobius
...
function getConfigPath(): string {
  return join(getDobiusManagedCodexHomePath(), 'hooks.json')
}
...
function writeCodexHooksJson(configPath: string, hooks: Record<string, HookDefinition[]>): void {
  ...
  // bookkeeping such as `_managed` must not survive Dobius's rewrite.
  writeHooksJson(configPath, { hooks })
}

function getCodexConfigTomlPath(): string {
  return join(getDobiusManagedCodexHomePath(), 'config.toml')
}
...
const LEGACY_DOBIUS_PROFILE_NAME = 'dobius-agent-status'
const LEGACY_DOBIUS_PROFILE_BLOCK_START = '# BEGIN DOBIUS AGENT STATUS HOOKS'
const LEGACY_DOBIUS_PROFILE_BLOCK_END = '# END DOBIUS AGENT STATUS HOOKS'
...
function getLegacyCodexProfileTomlPath(): string {
  return join(getSystemCodexHomePath(), `${LEGACY_DOBIUS_PROFILE_NAME}.config.toml`)
}
...
function removeCodexPluginEnvironmentCommands(definitions: HookDefinition[]): HookDefinition[] {
  // Why: Dobius mirrors system hooks into a plain runtime hooks.json. Plugin
...
function getRuntimeHooksWithSystemUserHooks(
  ...
  for (const [eventName, systemDefinitions] of Object.entries(systemConfig.hooks)) {
    ...
    // Why: runtime hooks are derived from the user's system hooks plus Dobius's
    // managed hooks. Reusing old runtime user-hook copies would keep deleted or
    // edited ~/.codex/hooks.json entries alive for new Dobius-launched sessions.
...
function getTrustedSystemUserHookSignatures(
  ...
  try {
    ...
    // trust mirroring; Dobius's managed runtime hooks can still be installed.
...
function buildHookTrustHeaderKeyPattern(key: string): string {
  ...
  const alternatives = [...new Set(keyVariants)].flatMap((variant) => {
    ...
    if (!variant.includes("'")) {
      ...
      // while Dobius repairs mirrored runtime trust across both Windows variants.
...
function cleanupLegacySystemManagedHooks(): void {
  ...
  // Why: Codex hooks moved to Dobius's managed CODEX_HOME; old entries in
  // ~/.codex would keep external Codex sessions reporting into Dobius.
  if (removedManagedHook) {
    // Why: this is the user's system hooks file, not Dobius's runtime copy.
    // Remove only stale Dobius hook entries and preserve other managers' metadata.
...
function stripLegacyManagedProfileBlock(content: string): string {
  const start = content.indexOf(LEGACY_DOBIUS_PROFILE_BLOCK_START)
  if (start === -1) {
    return content
  }
  const endMarker = content.indexOf(LEGACY_DOBIUS_PROFILE_BLOCK_END, start)
  const end = endMarker === -1 ? content.length : endMarker + LEGACY_DOBIUS_PROFILE_BLOCK_END.length
...
function cleanupLegacyCodexProfileHooks(): void {
  ...
  // Why: #2778 wrote Dobius hooks into a Codex profile file. Runtime CODEX_HOME
  // supersedes that representation, so remove only Dobius's marked block.
...
function removeRuntimeManagedHookTrustEntries(configPath: string): void {
  try {
    ...
    // user-approved trust entries for non-Dobius commands, so match by hash
...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  if (target === 'local' && process.platform === 'win32') {
    return [
      ...
      // the live port/token for this Dobius install; sourcing it here lets a
      // surviving PTY reach the current server even though its env points at
      // the prior Dobius's coordinates.
  ...
  return [
    ...
    // PORT/TOKEN/ENV/VERSION from the current Dobius so a surviving PTY keeps
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${DOBIUS_AGENT_HOOK_PORT}/hook/codex" \\',
...
export class CodexHookService {
  ...
  install(): AgentHookInstallStatus {
    ...
    try {
      ...
      // all old runtime [hooks.state.*] blocks would keep Dobius Codex trusted.
  ...
  async installRemote(sftp: SFTPWrapper, remoteHome: string): Promise<AgentHookInstallStatus> {
    ...
    const remoteScriptPath = `${remoteHome.replace(/\/$/, '')}/.dobius/agent-hooks/codex-hook.sh`
    try {
      ...
      // Why: SSH remotes use POSIX `.sh` hook paths even when Dobius itself is
      ...
      // Preserve non-Dobius top-level metadata while replacing the hooks tree.
  ...
  refreshRuntimeUserHooks(): AgentHookInstallStatus {
    ...
    try {
      ...
      syncSystemConfigIntoManagedCodexHome()
      // Why: this path is used when Dobius status hooks are disabled. The
      // runtime CODEX_HOME should keep user hooks, but not Dobius-managed trust.
