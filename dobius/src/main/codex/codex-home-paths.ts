...
export function getDobiusManagedCodexHomePath(): string {
  const managedHomePath = join(getDobiusUserDataPath(), 'codex-runtime-home', 'home')
  mkdirSync(managedHomePath, { recursive: true })
  return managedHomePath
}

function getDobiusUserDataPath(): string {
  ...
  if (process.platform === 'win32') {
    return join(process.env.APPDATA ?? join(homedir(), 'AppData', 'Roaming'), 'dobius')
  }
...
export function syncSystemCodexResourcesIntoManagedHome(): void {
  ...
  const managedHomePath = getDobiusManagedCodexHomePath()
...
function getResourceCopyMarkerPath(managedHomePath: string, entryName: string): string {
  return join(managedHomePath, '.dobius-resource-copies', `${entryName}.json`)
}
