...
// into 429s during long focused Dobius sessions.
...
export class RateLimitService {
  ...
  private shouldBackgroundPoll(): boolean {
    ...
    // Why: these quota fetches only power in-app UI. When Dobius is hidden,
  ...
  private shouldAllowClaudePtyFallback(
    ...
    // Why: system-default Claude is not an Dobius-managed account. Background
    // quota refresh may read existing OAuth, but must not launch Claude and
    // trigger auth/browser flows for users who never configured Claude in Dobius.
