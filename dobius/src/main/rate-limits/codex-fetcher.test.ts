...
describe('fetchCodexRateLimits', () => {
  ...
  it('runs rate-limit RPC through WSL when the Codex home is a WSL managed account', async () => {
    ...
    try {
      const resultPromise = fetchCodexRateLimits({
        codexHomePath: '\\\\wsl.localhost\\Ubuntu\\home\\alice\\.local\\share\\dobius\\account\\home'
      })
      await vi.advanceTimersByTimeAsync(1)
      await vi.advanceTimersByTimeAsync(1)
      await resultPromise
      ...
      expect(bashCommand).toContain('mkdir -p "$dobius_rate_limit_cwd"')
      expect(bashCommand).toContain('cd "$dobius_rate_limit_cwd"')
      expect(bashCommand).toContain(
        "export CODEX_HOME='/home/alice/.local/share/dobius/account/home'"
      )
