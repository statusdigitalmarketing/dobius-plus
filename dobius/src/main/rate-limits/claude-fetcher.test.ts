...
describe('fetchClaudeRateLimits', () => {
  ...
  beforeEach(() => {
    ...
    appGetPathMock.mockReturnValue('/tmp/dobius-claude-fetcher-test')
  ...
  it('does not read inactive managed credentials from unowned auth paths', async () => {
    setPlatform('linux')
    tempDir = mkdtempSync(join(tmpdir(), 'dobius-claude-fetcher-'))
    ...
    writeFileSync(join(unownedAuthPath, '.dobius-managed-claude-auth'), 'account-1\n', 'utf-8')
    writeFileSync(
  ...
  it('supplements inactive managed account OAuth usage with Fable from its usage panel', async () => {
    ...
    writeFileSync(join(ownedAuthPath, '.dobius-managed-claude-auth'), 'account-1\n', 'utf-8')
