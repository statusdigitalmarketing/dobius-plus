  ...
  try {
    ...
    const res = await net.fetch(OAUTH_USAGE_URL, {
      headers: {
        ...
        // matching the CLI user-agent keeps Dobius aligned with that contract.
...
function resolveOwnedWslClaudeManagedAuthPath(account: InactiveClaudeAccountInfo): string | null {
  ...
  if (
    !linuxPath.includes('/.local/share/dobius/claude-accounts/') ||
    !linuxPath.endsWith(`/${account.id}/auth`)
  ) {
    return null
  }
  try {
    const markerPath = path.join(account.managedAuthPath, '.dobius-managed-claude-auth')
    if (
