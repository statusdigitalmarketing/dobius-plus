...
import {
  DOBIUS_PI_AGENT_STATUS_EXTENSION_FILE,
  getPiAgentStatusExtensionSource
} from './agent-status-extension-source'
import {
  DOBIUS_PI_PREFILL_EXTENSION_FILE,
  getPiPrefillExtensionSource
} from './prefill-extension-source'
export { DOBIUS_OMP_PREFILL_ENV_VAR, DOBIUS_PI_PREFILL_ENV_VAR } from './prefill-extension-source'
import { DOBIUS_PI_EXTENSION_FILE, getPiTitlebarExtensionSource } from './titlebar-extension-source'
import {
...
// Why: old Dobius versions used per-kind overlay roots. Keep the names so
...
// shadowed one in Dobius's per-launch agent picker.
...
function withDobiusManagedExtensionMarker(source: string): string {
...
export class PiTitlebarExtensionService {
  ...
  private getPtyOverlayDir(ptyId: string, kind: PiAgentKind): string {
    // Why: old Dobius versions used PTY-scoped hashed overlays. Keep resolving
  ...
  private writeOmpFallbackStatusExtension(source: string): string | undefined {
    ...
    const fallbackPath = join(fallbackDir, DOBIUS_PI_AGENT_STATUS_EXTENSION_FILE)
  ...
  private installManagedExtensions(
    ...
    this.writeManagedExtension(
      join(extensionsDir, DOBIUS_PI_EXTENSION_FILE),
      withDobiusManagedExtensionMarker(getPiTitlebarExtensionSource())
    )
    this.writeManagedExtension(
      join(extensionsDir, DOBIUS_PI_PREFILL_EXTENSION_FILE),
      withDobiusManagedExtensionMarker(getPiPrefillExtensionSource(kind))
    )
    const statusExtensionPath = join(extensionsDir, DOBIUS_PI_AGENT_STATUS_EXTENSION_FILE)
    const statusSource = withDobiusManagedExtensionMarker(getPiAgentStatusExtensionSource(kind))
  ...
  buildPtyEnv(
    ...
    if (kind === 'omp') {
      env.DOBIUS_OMP_SOURCE_AGENT_DIR = installed.sourceAgentDir
      if (installed.statusExtensionPath) {
        env.DOBIUS_OMP_STATUS_EXTENSION = installed.statusExtensionPath
      }
    } else {
      env.DOBIUS_PI_SOURCE_AGENT_DIR = installed.sourceAgentDir
    }
    return env
  }
