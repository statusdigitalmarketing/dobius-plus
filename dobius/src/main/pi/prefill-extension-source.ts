...
export const DOBIUS_PI_PREFILL_EXTENSION_FILE = 'dobius-prefill.ts'

// Why: prefill-without-submit needs an env-var the bundled `dobius-prefill.ts`
...
const PREFILL_ENV_VAR_BY_KIND: Record<PiAgentKind, string> = {
  pi: 'DOBIUS_PI_PREFILL',
  omp: 'DOBIUS_OMP_PREFILL'
}
 ...
 *  `DOBIUS_OMP_PREFILL_ENV_VAR` instead. */
export const DOBIUS_PI_PREFILL_ENV_VAR = PREFILL_ENV_VAR_BY_KIND.pi

/** OMP's prefill env var. Mirrors `DOBIUS_PI_PREFILL_ENV_VAR` for OMP launches
 *  so renderer plans and shell-ready restore lines can stay agent-scoped. */
export const DOBIUS_OMP_PREFILL_ENV_VAR = PREFILL_ENV_VAR_BY_KIND.omp
...
export function getPiPrefillExtensionSource(kind: PiAgentKind): string {
  ...
  return [
    ...
    '    if (!process.env.DOBIUS_PANE_KEY) return',
