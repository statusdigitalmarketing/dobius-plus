...
// /hook/<kind> using the same DOBIUS_AGENT_HOOK_* + DOBIUS_PANE_KEY env that every
...
// any Dobius dep into the pi runtime.
import type { PiAgentKind } from '../../shared/pi-agent-kind'

export const DOBIUS_PI_AGENT_STATUS_EXTENSION_FILE = 'dobius-agent-status.ts'
...
export function getPiAgentStatusExtensionSource(kind: PiAgentKind = 'pi'): string {
  ...
  // so it cannot import from Dobius's main bundle. fs/http coords come from
  // the same endpoint file the OpenCode plugin reads (process.env is frozen
  // at PTY spawn, so on Dobius restart we have to re-read it from disk).
  return [
    ...
    "      console.warn('[dobius-pi-status] failed to parse endpoint file:', (err as Error).message)",
    '    }',
    '    return null',
    '  }',
    '}',
    ...
    '    })',
    '  } catch {',
    '    // Why: status reporting must never fail the pi run just because Dobius',
    '    // is unavailable or the loopback request failed (e.g. Dobius restart).',
    '  }',
    '}',
    '',
