...
vi.mock('../../resources/app-icons/dobius-watercolor.png?asset', () => ({
  default: 'watercolor-icon'
}))

vi.mock('../../resources/app-icons/dobius-watercolor.png?asset&asarUnpack', () => ({
  default: 'watercolor-icon-unpacked'
}))

vi.mock('../../resources/app-icons/dobius-blue.png?asset', () => ({
  default: 'blue-icon'
}))

vi.mock('../../resources/app-icons/dobius-blue.png?asset&asarUnpack', () => ({
...
describe('app icon selection', () => {
  ...
  it('persists a custom macOS dock icon to the app bundle for inactive Dock pins', async () => {
    ...
    persistMacDockIcon('watercolor', {
      appBundlePath: '/Applications/Dobius.app',
      execFile,
      isDevApp: false,
      platform: 'darwin'
    })
    ...
    expect(execFile).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.objectContaining({
          DOBIUS_APP_BUNDLE_PATH: '/Applications/Dobius.app',
          DOBIUS_APP_ICON_PATH: 'watercolor-icon-unpacked'
        })
      }),
      expect.any(Function)
    )
  ...
  it('clears the AppKit icon and Finder metadata when switching macOS back to classic', async () => {
    ...
    expect(execFile).toHaveBeenCalledWith(
      '/usr/bin/xattr',
      ['-d', 'com.apple.FinderInfo', '/Applications/Dobius.app'],
      expect.objectContaining({
        timeout: 10_000
      }),
      expect.any(Function)
    ...
    expect(execFile).toHaveBeenCalledWith(
      '/usr/bin/xattr',
      ['-d', 'com.apple.ResourceFork', '/Applications/Dobius.app'],
      expect.objectContaining({
        timeout: 10_000
      }),
      expect.any(Function)
  ...
  it('serializes rapid macOS dock icon persistence so the last icon request wins', async () => {
    ...
    expect(execFile).not.toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.objectContaining({
          DOBIUS_APP_ICON_PATH: 'blue-icon-unpacked'
        })
      }),
      expect.any(Function)
    )
