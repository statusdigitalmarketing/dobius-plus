...
function fetchThroughProxy(
  ...
  return new Promise((resolve, reject) => {
    // Why: *.dobius.localhost is not resolvable DNS; connect to the proxy on
...
describe('localhost worktree label proxy', () => {
  ...
  it('returns 404 for unregistered dobius.localhost labels', async () => {
    const port = await startUpstream((_request, response) => {
      response.end('ok')
    })
    const proxy = new LocalhostWorktreeLabelProxy()
    ...
    const result = await fetchThroughProxy(`http://unknown-label.dobius.localhost:${proxyPort}/`)
