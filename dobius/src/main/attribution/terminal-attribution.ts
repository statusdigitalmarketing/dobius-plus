...
import { DOBIUS_GIT_COMMIT_TRAILER } from '../../shared/dobius-attribution'
...
const DOBIUS_GH_FOOTER = `Made with [Dobius](${DOBIUS_PRODUCT_URL}) 🐋`
const SHELL_DOLLAR = '$'
const POWERSHELL_TICK = '`'
const ATTRIBUTION_ENV_KEYS = [
  'DOBIUS_ENABLE_GIT_ATTRIBUTION',
  'DOBIUS_GIT_COMMIT_TRAILER',
  'DOBIUS_GH_PR_FOOTER',
  'DOBIUS_GH_ISSUE_FOOTER',
...
export function applyTerminalAttributionEnv(
  ...
  // cannot recursively point DOBIUS_REAL_* at themselves.
  ...
  // Why: these wrappers should affect only Dobius-managed PTYs. Prepending the
  // shim directory here keeps the attribution behavior scoped to Dobius's live
  ...
  baseEnv.DOBIUS_ENABLE_GIT_ATTRIBUTION = '1'
  baseEnv.DOBIUS_GIT_COMMIT_TRAILER = DOBIUS_GIT_COMMIT_TRAILER
  baseEnv.DOBIUS_GH_PR_FOOTER = DOBIUS_GH_FOOTER
  baseEnv.DOBIUS_GH_ISSUE_FOOTER = DOBIUS_GH_FOOTER
  if (shellFamily === 'posix') {
    baseEnv.DOBIUS_ATTRIBUTION_SHIM_DIR = posixDir
  } else {
  ...
  if (platform === 'win32') {
    if (resolvedGit) {
      baseEnv.DOBIUS_REAL_GIT = resolvedGit
    }
    if (resolvedGh) {
      baseEnv.DOBIUS_REAL_GH = resolvedGh
    }
  }
}
...
function stripAttributionPathEntries(pathValue: string, pathDelimiter: string): string {
  return pathValue
    ...
    .filter((entry) => {
      ...
      return !normalized.includes('/dobius-terminal-attribution/')
    })
    .join(pathDelimiter)
}
...
clean_path() {
  ...
  for entry in "${SHELL_DOLLAR}{entries[@]}"; do
    case "$entry" in
      "$script_dir"|*/dobius-terminal-attribution/posix|*/dobius-terminal-attribution/win32|*\\dobius-terminal-attribution\\posix|*\\dobius-terminal-attribution\\win32)
        ;;
      *)
        cleaned+=("$entry")
        ;;
...
if [[ -z "$real_git" ]]; then
  echo "Dobius attribution wrapper could not locate git on PATH." >&2
  exit 127
fi
...
if [[ "\${DOBIUS_ENABLE_GIT_ATTRIBUTION:-0}" != "1" || "\${DOBIUS_ATTRIBUTION_BYPASS:-0}" == "1" ]] || ! is_commit_command "$@"; then
...
DOBIUS_ATTRIBUTION_BYPASS=1 PATH="$real_path" exec "$real_git" "${SHELL_DOLLAR}{attributed_args[@]}"
`
...
if [[ -z "$real_gh" ]]; then
  echo "Dobius attribution wrapper could not locate gh on PATH." >&2
  exit 127
fi
...
if [[ "\${DOBIUS_ENABLE_GIT_ATTRIBUTION:-0}" != "1" || "\${DOBIUS_ATTRIBUTION_BYPASS:-0}" == "1" ]]; then
  PATH="$real_path" exec "$real_gh" "$@"
fi

if [[ "\${1:-}" == "pr" && "\${2:-}" == "create" ]]; then
  footer="\${DOBIUS_GH_PR_FOOTER:-Made with [Dobius](https://github.com/statusdigitalmarketing/dobius-plus) 🐋}"
...
if [[ "\${1:-}" == "issue" && "\${2:-}" == "create" ]]; then
  footer="\${DOBIUS_GH_ISSUE_FOOTER:-Made with [Dobius](https://github.com/statusdigitalmarketing/dobius-plus) 🐋}"
  if has_passthrough_create_args "$@"; then
    PATH="$real_path" exec "$real_gh" "$@"
  fi
  if ! has_noninteractive_create_args "$@"; then
...
setlocal
if not "%DOBIUS_ENABLE_GIT_ATTRIBUTION%"=="1" goto run
if "%DOBIUS_ATTRIBUTION_BYPASS%"=="1" goto run
call :dobius_is_git_commit %*
if errorlevel 1 goto run
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0git-wrapper.ps1" %*
exit /b %ERRORLEVEL%
:run
if defined DOBIUS_REAL_GIT (
  "%DOBIUS_REAL_GIT%" %*
) else (
  echo Dobius attribution wrapper could not locate git on PATH. 1>&2
  exit /b 127
)
exit /b %ERRORLEVEL%

:dobius_is_git_commit
if "%~1"=="" exit /b 1
if /I "%~1"=="commit" exit /b 0
set "dobius_git_arg=%~1"
if /I "%dobius_git_arg%"=="-c" goto skip_two
if /I "%dobius_git_arg%"=="--config" goto skip_two
if /I "%dobius_git_arg%"=="-C" goto skip_two
if /I "%dobius_git_arg%"=="--git-dir" goto skip_two
if /I "%dobius_git_arg%"=="--work-tree" goto skip_two
if /I "%dobius_git_arg%"=="--namespace" goto skip_two
if /I "%dobius_git_arg:~0,9%"=="--config=" goto skip_one
if /I "%dobius_git_arg:~0,10%"=="--git-dir=" goto skip_one
if /I "%dobius_git_arg:~0,12%"=="--work-tree=" goto skip_one
if /I "%dobius_git_arg:~0,12%"=="--namespace=" goto skip_one
if "%dobius_git_arg:~0,1%"=="-" goto skip_one
exit /b 1
:skip_two
shift
shift
goto dobius_is_git_commit
:skip_one
shift
goto dobius_is_git_commit
`
...
exit /b %ERRORLEVEL%
:run
if defined DOBIUS_REAL_GH (
  "%DOBIUS_REAL_GH%" %*
) else (
  echo Dobius attribution wrapper could not locate gh on PATH. 1>&2
  exit /b 127
)
exit /b %ERRORLEVEL%
`
...
$env:DOBIUS_ATTRIBUTION_BYPASS = '1'
try {
...
$realGh = if ($env:DOBIUS_REAL_GH) { $env:DOBIUS_REAL_GH } else { 'gh' }
...
if ($isPrCreate) {
  ...
  if ($createdUrl) {
    ...
    $footer = if ($env:DOBIUS_GH_PR_FOOTER) { $env:DOBIUS_GH_PR_FOOTER } else { 'Made with [Dobius](https://github.com/statusdigitalmarketing/dobius-plus) 🐋' }
...
if ($isIssueCreate) {
  ...
  if ($createdUrl) {
    ...
    $footer = if ($env:DOBIUS_GH_ISSUE_FOOTER) { $env:DOBIUS_GH_ISSUE_FOOTER } else { 'Made with [Dobius](https://github.com/statusdigitalmarketing/dobius-plus) 🐋' }
    if ($null -ne $body -and $body -notmatch [Regex]::Escape($footer)) {
      $tmpFile = [System.IO.Path]::GetTempFileName()
      try {
        $trimmed = $body.TrimEnd("${POWERSHELL_TICK}r", "${POWERSHELL_TICK}n")
