...
describe('applyTerminalAttributionEnv', () => {
  ...
  function makeTmpRoot(): string {
    tmpRoot = mkdtempSync(join(tmpdir(), 'dobius-attribution-'))
    return tmpRoot
  }
  ...
  function stripInheritedAttributionPath(pathValue: string): string {
    ...
    return pathValue
      .split(pathDelimiter)
      .filter((entry) => !entry.includes('dobius-terminal-attribution'))
      .join(pathDelimiter)
  }

  function cleanAttributionEnv(env?: Record<string, string>): Record<string, string> {
    const base = { ...process.env }
    delete base.DOBIUS_ENABLE_GIT_ATTRIBUTION
    delete base.DOBIUS_GIT_COMMIT_TRAILER
    delete base.DOBIUS_GH_PR_FOOTER
    delete base.DOBIUS_GH_ISSUE_FOOTER
    delete base.DOBIUS_ATTRIBUTION_SHIM_DIR
    delete base.DOBIUS_REAL_GIT
    delete base.DOBIUS_REAL_GH
  ...
  posixSubprocessIt('does not amend HEAD when git commit --dry-run exits successfully', () => {
    ...
    mkdirSync(repo)
    runGit(repo, ['init'])
    runGit(repo, ['config', 'user.name', 'Dobius Test'])
    runGit(repo, ['config', 'user.email', 'dobius-test@example.com'])
    ...
    expect(runGit(repo, ['log', '-1', '--format=%B'])).not.toContain('Co-authored-by: Dobius')
    ...
    expect(runGit(repo, ['log', '-1', '--format=%B'])).toContain(
      'Co-authored-by: Dobius <noreply@statusdigitalmarketing.com>'
    )
  })
...
`,
      ...
      expect(readFileSync(argsPath, 'utf8')).not.toContain('Co-authored-by: Dobius')
    }
  )
...
grep -Fq 'Co-authored-by: Dobius <noreply@statusdigitalmarketing.com>' "$1"
`,
      'utf8'
    )
...
`,
    ...
    expect(readFileSync(argsPath, 'utf8')).toContain('Co-authored-by: Dobius <noreply@statusdigitalmarketing.com>')
  })
...
if [[ "$1 $2 $3 $4" == "pr view --json url" ]]; then
  printf '%s\\n' 'https://github.com/statusdigitalmarketing/dobius-plus/pull/123'
  exit 0
fi
if [[ "$1 $2" == "api repos/statusdigitalmarketing/dobius-plus/pulls/123" && "\${3:-}" == "--jq" ]]; then
  printf '%s\\n' 'Existing body'
  exit 0
fi
if [[ "$1 $2 $3 $4" == "api -X PATCH repos/statusdigitalmarketing/dobius-plus/pulls/123" ]]; then
  touch "${markerPath}"
  exit 0
fi
exit 1
...
if [[ "$1 $2" == "issue create" ]]; then
  printf '%s\\n' 'https://github.com/statusdigitalmarketing/dobius-plus/issues/456'
  exit 0
fi
if [[ "$1 $2" == "api repos/statusdigitalmarketing/dobius-plus/pulls/123" && "\${3:-}" == "--jq" ]]; then
  printf '%s\\n' 'PR body'
...
if [[ "$1 $2" == "api repos/statusdigitalmarketing/dobius-plus/issues/456" && "\${3:-}" == "--jq" ]]; then
  printf '%s\\n' 'Issue body'
  exit 0
fi
if [[ "$1 $2 $3 $4" == "api -X PATCH repos/statusdigitalmarketing/dobius-plus/pulls/123" ]]; then
...
if [[ "$1 $2 $3 $4" == "api -X PATCH repos/statusdigitalmarketing/dobius-plus/issues/456" ]]; then
...
`,
    ...
    ).toBe('https://github.com/statusdigitalmarketing/dobius-plus/pull/123\n')
    expect(
      execFileSync('gh', ['issue', 'create', '--title', 'Issue', '--body', 'Body'], {
        encoding: 'utf8',
        env: cleanAttributionEnv(attributionEnv)
      })
    ).toBe('https://github.com/statusdigitalmarketing/dobius-plus/issues/456\n')
...
`,
    ...
    expect(output).toBe('https://github.com/statusdigitalmarketing/dobius-plus/pull/123\n')
    expect(existsSync(markerPath)).toBe(false)
  })
...
`,
  ...
  it('fails open when shim files cannot be written', () => {
    ...
    expect(baseEnv.DOBIUS_ENABLE_GIT_ATTRIBUTION).toBeUndefined()
  ...
  it('does not duplicate shim directories when applied to an already-injected env', () => {
    ...
    const shimEntries = baseEnv.PATH.split(pathDelimiter).filter((entry) =>
      entry.includes('dobius-terminal-attribution')
    )
  ...
  it('puts only Windows shims on PATH for native Windows shells', () => {
    ...
    const posixDir = join(userDataPath, 'dobius-terminal-attribution', 'posix')
    const win32Dir = join(userDataPath, 'dobius-terminal-attribution', 'win32')
    const pathEntries = baseEnv.PATH.split(';')

    expect(pathEntries[0]).toBe(win32Dir)
    expect(pathEntries).not.toContain(posixDir)
    expect(baseEnv.DOBIUS_ATTRIBUTION_SHIM_DIR).toBeUndefined()
  ...
  it('keeps POSIX shims first for Windows Git Bash and WSL shells', () => {
    ...
    expect(baseEnv.DOBIUS_ATTRIBUTION_SHIM_DIR).toBe(posixDir)
  ...
  it('writes PowerShell wrappers without raw-template backslash escapes', () => {
    ...
    const shimDir = join(root, 'user-data', 'dobius-terminal-attribution', 'win32')
