...
describe('scanAiVaultSessions', () => {
  it('indexes Claude and Codex transcripts with resume commands', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dobius-ai-vault-'))
    tempRoots.push(root)
  ...
  it('indexes Codex sessions from Dobius runtime homes with resumable commands', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dobius-ai-vault-codex-runtime-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
      ...
      jsonLines([
        {
          ...
          payload: {
            ...
            cwd: '/Users/nwparker/dobius/workspaces/dobius/mem4'
          }
        },
        {
    ...
    expect(result.sessions[0]).toMatchObject({
      ...
      resumeCommand: `cd '/Users/nwparker/dobius/workspaces/dobius/mem4' && CODEX_HOME='${runtimeHome}' codex resume '019e9693-64fc-7370-9c18-7e625c595d0f'`
    })
  })

  it('indexes WSL home session roots', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dobius-ai-vault-wsl-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
    const wslHome = join(root, 'wsl', 'Ubuntu', 'home', 'ada')
    await mkdir(join(wslHome, '.claude', 'projects', 'repo'), { recursive: true })
    await mkdir(
      join(wslHome, '.local', 'share', 'dobius', 'codex-runtime-home', 'home', 'sessions'),
      {
        recursive: true
      }
    )
  ...
  it('skips hidden Codex context blocks when choosing session titles', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dobius-ai-vault-codex-hidden-context-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
  ...
  it('indexes every supported agent transcript format with native resume commands', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dobius-ai-vault-all-agents-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
  ...
  it('strips newline-heavy Grok user_query envelopes without regex matching', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'dobius-ai-vault-grok-large-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
  ...
  it('carries non-default Codex homes in copied resume commands', () => {
    expect(
      buildAiVaultResumeCommand({
        ...
        cwd: '/repo/app',
        platform: 'darwin',
        codexHome: '/Users/ada/Library/Application Support/Dobius/codex-runtime-home/home'
      })
    ).toBe(
      "cd '/repo/app' && CODEX_HOME='/Users/ada/Library/Application Support/Dobius/codex-runtime-home/home' codex resume 'session-1'"
    )
    ...
    expect(
      buildAiVaultResumeCommand({
        ...
        cwd: 'C:\\Users\\Ada Lovelace\\repo',
        platform: 'win32',
        codexHome: 'C:\\Users\\Ada\\AppData\\Roaming\\Dobius\\codex-runtime-home\\home'
      })
    ).toBe(
      'cmd /d /s /c "cd /d ""C:\\Users\\Ada Lovelace\\repo"" && set ""CODEX_HOME=C:\\Users\\Ada\\AppData\\Roaming\\Dobius\\codex-runtime-home\\home"" && codex resume ""session-1"""'
    )
  })
})
