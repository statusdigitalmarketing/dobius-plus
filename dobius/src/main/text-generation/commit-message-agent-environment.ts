...
function prepareShellConfigDirEnv(agentId: string): { ok: true; env?: NodeJS.ProcessEnv } | null {
  ...
  // Why: each kind owns a distinct DOBIUS_*_SOURCE_* shadow so a headless commit
  ...
  const sourceVar =
    agentId === 'opencode'
      ? 'DOBIUS_OPENCODE_SOURCE_CONFIG_DIR'
      : agentId === 'pi'
        ? 'DOBIUS_PI_SOURCE_AGENT_DIR'
        : agentId === 'omp'
          ? 'DOBIUS_OMP_SOURCE_AGENT_DIR'
          : undefined
  ...
  // Why: GUI-launched Dobius may not inherit shell startup exports, but these
  // vars point the headless CLI at the user's auth/config root. Nested Dobius
  // launches inherit PTY overlays, so prefer DOBIUS_*_SOURCE_* when present.
