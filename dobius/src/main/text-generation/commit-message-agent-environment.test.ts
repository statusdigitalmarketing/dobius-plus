...
function makeHome(): string {
  ...
  const dir = mkdtempSync(join(tmpdir(), 'dobius-commit-env-'))
  tempDirs.push(dir)
...
describe('prepareLocalCommitMessageAgentEnv', () => {
  ...
  it('prefers the original OpenCode config root over inherited PTY overlays', async () => {
    process.env.OPENCODE_CONFIG_DIR = '/tmp/dobius-opencode-overlay'
    process.env.DOBIUS_OPENCODE_SOURCE_CONFIG_DIR = '/Users/tester/company/opencode'
  ...
  it('prefers the original Pi agent root over inherited PTY overlays', async () => {
    process.env.PI_CODING_AGENT_DIR = '/tmp/dobius-pi-overlay'
    process.env.DOBIUS_PI_SOURCE_AGENT_DIR = '/Users/tester/.pi/agent'
  ...
  it('sets CODEX_HOME for host managed Codex accounts', async () => {
    const result = await prepareLocalCommitMessageAgentEnv('codex', {
      prepareForCodexLaunch: () =>
        'C:\\Users\\tester\\AppData\\Roaming\\Dobius\\codex-accounts\\a\\home'
    })

    expect(result).toEqual({
      ok: true,
      env: expect.objectContaining({
        CODEX_HOME: 'C:\\Users\\tester\\AppData\\Roaming\\Dobius\\codex-accounts\\a\\home'
      })
    })
  })
  ...
  it('does not pass WSL managed Codex homes to host-local commit generation', async () => {
    ...
    const result = await prepareLocalCommitMessageAgentEnv('codex', {
      prepareForCodexLaunch: () =>
        '\\\\wsl.localhost\\Ubuntu\\home\\tester\\.local\\share\\dobius\\codex-accounts\\a\\home'
    })
