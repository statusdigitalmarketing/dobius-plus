...
export class CodexAccountService {
  ...
  private async doAddAccount(target?: CodexAccountAddTarget): Promise<CodexRateLimitAccountsState> {
    ...
    try {
      ...
      if (!identity.email) {
        throw new Error('Codex login completed, but Dobius could not resolve the account email.')
      }
  ...
  private createManagedHome(
    ...
    // prove the path belongs to Dobius before deleting anything.
  ...
  private tryCreateWslManagedHome(
    ...
    const wslLinuxHomePath = `${home.replace(/\/$/, '')}/.local/share/dobius/codex-accounts/${accountId}/home`
    const markerPath = `${wslLinuxHomePath}/.dobius-managed-home`
    execFileSync(
      'wsl.exe',
      [
        '-d',
  ...
  private syncCanonicalConfigIntoManagedHome(
    ...
    // Why: Dobius account switching is meant to swap Codex credentials and quota
  ...
  private readCanonicalConfigForManagedHome(managedHomePath: string): CanonicalCodexConfig | null {
    ...
    const managedRootMarker = '/.local/share/dobius/codex-accounts/'
  ...
  private recreateExpectedHostManagedHomeForReauthentication(
    ...
    // but only at the exact Dobius-owned account path persisted for this account.
    mkdirSync(expectedManagedHomePath, { recursive: true })
    writeFileSync(join(expectedManagedHomePath, '.dobius-managed-home'), `${account.id}\n`, 'utf-8')
  ...
  private ensureExpectedWslManagedHomeForReauthentication(
    ...
    if (
      ...
      !wslInfo.linuxPath.endsWith(`/.local/share/dobius/codex-accounts/${account.id}/home`)
    ) {
      return
    }
      ...
      [
        ...
        buildEncodedWslBashCommand(
          [
            ...
            'marker="$candidate/.dobius-managed-home"',
  ...
  private assertManagedHomePath(candidatePath: string): string {
    ...
    if (wslInfo) {
      if (
        !wslInfo.linuxPath.includes('/.local/share/dobius/codex-accounts/') ||
        !wslInfo.linuxPath.endsWith('/home')
      ) {
        throw new Error('Managed WSL Codex home is outside Dobius account storage.')
      }
            ...
            [
              ...
              buildEncodedWslBashCommand(
                [
                  'set -euo pipefail',
                  `candidate=${shellQuote(wslInfo.linuxPath)}`,
                  'managed_root="${HOME%/}/.local/share/dobius/codex-accounts"',
                  'candidate_real=$(readlink -f -- "$candidate")',
                  'managed_root_real=$(readlink -f -- "$managed_root")',
                  'test -f "$candidate_real/.dobius-managed-home"',
      ...
      if (!existsSync(join(candidatePath, '.dobius-managed-home'))) {
        throw new Error('Managed Codex home is missing Dobius ownership marker.')
      }
    ...
    // mode (dobius-dev/ vs dobius/) this check also filters out production-rooted
    ...
    if (escaped) {
      throw new Error('Managed Codex home escaped Dobius account storage.')
    }

    if (!existsSync(join(canonicalCandidate, '.dobius-managed-home'))) {
      throw new Error('Managed Codex home is missing Dobius ownership marker.')
    }
        ...
        [
          ...
          buildEncodedWslBashCommand(
            [
              ...
              'test -f "$candidate_real/.dobius-managed-home"',
              'test "$(cat "$candidate_real/.dobius-managed-home")" = "$expected_marker"',
