...
function createSettings(overrides: Partial<GlobalSettings> = {}): GlobalSettings {
  ...
  return {
    ...
    terminalThemeDark: 'dobius-dark',
    terminalDividerColorDark: '#000000',
    terminalUseSeparateLightTheme: false,
    terminalThemeLight: 'dobius-light',
...
function createManagedAuth(rootDir: string, accountId: string, auth: string): string {
  ...
  writeFileSync(join(managedHomePath, '.dobius-managed-home'), `${accountId}\n`, 'utf-8')
...
describe('CodexRuntimeHomeService', () => {
  beforeEach(() => {
    vi.resetModules()
    vi.clearAllMocks()
    testState.userDataDir = mkdtempSync(join(tmpdir(), 'dobius-runtime-home-'))
    testState.fakeHomeDir = mkdtempSync(join(tmpdir(), 'dobius-codex-home-'))
    testState.previousUserDataPath = process.env.DOBIUS_USER_DATA_PATH
    process.env.DOBIUS_USER_DATA_PATH = testState.userDataDir
  ...
  afterEach(() => {
    ...
    if (testState.previousUserDataPath === undefined) {
      ...
      process.env.DOBIUS_USER_DATA_PATH = testState.previousUserDataPath
    }
  })
  ...
  it('uses the canonical Electron userData for legacy active host migration', async () => {
    const staleUserDataDir = mkdtempSync(join(tmpdir(), 'dobius-stale-runtime-home-'))
    const staleRuntimeHomePath = join(staleUserDataDir, 'codex-runtime-home', 'home')
    try {
      mkdirSync(staleRuntimeHomePath, { recursive: true })
      process.env.DOBIUS_USER_DATA_PATH = staleUserDataDir
      const legacyLaunchHomePath = join(
        testState.userDataDir,
        'codex-runtime-home',
        'launch',
      ...
      const { configureDobiusUserDataPathEnv } = await import('../startup/configure-process')
      configureDobiusUserDataPathEnv()
      const { CodexRuntimeHomeService } = await import('./runtime-home-service')
      new CodexRuntimeHomeService(store as never)

      expect(process.env.DOBIUS_USER_DATA_PATH).toBe(testState.userDataDir)
      expect(normalizeLinkTarget(readlinkSync(legacyActiveHomePath))).toBe(
        normalizeLinkTarget(getRuntimeCodexHomePath())
      )
  ...
  it('builds a valid WSL legacy active-home migration shell command', async () => {
    ...
    try {
      ...
      service.migrateLegacyWslActiveHomePointer(
        'Ubuntu',
        '\\\\wsl.localhost\\Ubuntu\\home\\alice\\.local\\share\\dobius\\codex-runtime-home\\home'
      )
      ...
      expect(shellCommand).toContain(
        "if [ ! -e '/home/alice/.local/share/dobius/codex-runtime-home/active/wsl/home' ] && [ ! -L '/home/alice/.local/share/dobius/codex-runtime-home/active/wsl/home' ]; then :"
      )
      expect(shellCommand).toContain(
        "elif [ -e '/home/alice/.local/share/dobius/codex-runtime-home/active/wsl/home' ] && [ ! -L '/home/alice/.local/share/dobius/codex-runtime-home/active/wsl/home' ]; then :"
      )
      expect(shellCommand).toContain(
        "mkdir -p '/home/alice/.local/share/dobius/codex-runtime-home/active/wsl'"
      )
      expect(shellCommand).toContain(
        "ln -s -- '/home/alice/.local/share/dobius/codex-runtime-home/home' '/home/alice/.local/share/dobius/codex-runtime-home/active/wsl/home.next-"
      )
      expect(shellCommand).toContain(
        "mv -Tf -- '/home/alice/.local/share/dobius/codex-runtime-home/active/wsl/home.next-"
      )
      expect(shellCommand).toContain(
        "' '/home/alice/.local/share/dobius/codex-runtime-home/active/wsl/home'"
      )
  ...
  it('returns the Dobius-managed runtime home for Codex launch and rate-limit preparation', async () => {
    const store = createStore(createSettings())
    const { CodexRuntimeHomeService } = await import('./runtime-home-service')
    const service = new CodexRuntimeHomeService(store as never)
      ...
      codexManagedAccounts: [
        {
          ...
          wslDistro: 'Ubuntu',
          wslLinuxHomePath: '/home/alice/.local/share/dobius/codex-accounts/account-1/home',
          providerAccountId: null,
          workspaceLabel: null,
          workspaceAccountId: null,
          createdAt: 1,
      ...
      createSettings({
        codexManagedAccounts: [
          ...
          },
          {
            ...
            managedHomeRuntime: 'wsl',
            wslDistro: 'Ubuntu',
            wslLinuxHomePath: '/home/alice/.local/share/dobius/codex-accounts/account-2/home',
      ...
      createSettings({
        codexManagedAccounts: [
          ...
          },
          {
            ...
            managedHomeRuntime: 'wsl',
            wslDistro: 'Ubuntu',
            wslLinuxHomePath: '/home/alice/.local/share/dobius/codex-accounts/wsl-account/home',
            providerAccountId: 'acct-wsl',
            workspaceLabel: null,
            workspaceAccountId: 'acct-wsl',
            createdAt: 2,
  ...
  it('uses the stable WSL runtime home for WSL system-default rate-limit fetches', async () => {
    ...
    try {
      ...
      expect(service.prepareForRateLimitFetch({ runtime: 'wsl', wslDistro: 'Ubuntu' })).toBe(
        join(wslHome, '.local', 'share', 'dobius', 'codex-runtime-home', 'home')
      )
    } finally {
      if (originalPlatform) {
        Object.defineProperty(process, 'platform', originalPlatform)
        ...
        codexManagedAccounts: [
          {
            ...
            managedHomeRuntime: 'wsl',
            wslDistro: 'Ubuntu',
            wslLinuxHomePath: '/home/alice/.local/share/dobius/codex-accounts/ubuntu/home',
          ...
          },
          {
            ...
            managedHomeRuntime: 'wsl',
            wslDistro: 'Debian',
            wslLinuxHomePath: '/home/alice/.local/share/dobius/codex-accounts/debian/home',
  ...
  it('persists runtime auth refreshes after returning to system default', async () => {
    ...
    // Codex used to refresh tokens directly in ~/.codex. With an Dobius-owned
  ...
  it('rejects runtime read-back from a different Codex identity', async () => {
    ...
    // shared runtime auth after Dobius has already selected account-1.
  ...
  it('routes runtime read-back from a different Codex identity to its matching account', async () => {
    ...
    // Dobius selected account-2. Persist the refresh to account-1, then restore
  ...
  it('restores system default when unverified runtime auth appears before deselect', async () => {
    ...
    // A stale or external process overwrites runtime with auth Dobius cannot
  ...
  it('preserves conflicting legacy session files under deterministic names', async () => {
    ...
    expect(
      readFileSync(join(runtimeSessionsDir, 'session.dobius-legacy-account-1.json'), 'utf-8')
    ).toBe('{"turns":[1,2]}')
    expect(
      readFileSync(
        join(runtimeSessionsDir, 'nested', 'session.dobius-legacy-account-1.json'),
        'utf-8'
      )
    ).toBe('{"turns":[2,3]}')
