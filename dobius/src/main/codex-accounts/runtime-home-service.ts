...
contract for Codex inside Dobius. Keeping path resolution, system-default
...
export class CodexRuntimeHomeService {
  ...
  // Why: tracks the auth.json content Dobius last wrote to the runtime CODEX_HOME.
  // Between syncs, if the file differs, Codex CLI refreshed the token — so
  // Dobius writes back the refreshed token to managed storage before overwriting.
  // On managed→system-default transition, if the file differs, an external
  // login (e.g. `codex auth login`) overwrote it — so Dobius adopts the file as
  ...
  private initializeLastSyncedState(): void {
    ...
    // transition and can restore/delete host auth that Dobius never touched.
  ...
  syncForCurrentSelection(target?: CodexAccountSelectionTarget): void {
    ...
    if (!activeAccount) {
      ...
      if (this.lastSyncedAccountId !== null) {
        ...
        if (logoutMarkerStatus.kind === 'applies') {
          ...
          // Why: Dobius-launched Codex sessions now use an Dobius-owned CODEX_HOME
          // even when no managed account is selected. Seed that runtime home
          // from the user's current system-default auth once so dev/prod Dobius
    ...
    // If we detect the runtime file differs from what Dobius last wrote, the CLI
  ...
  private readBackRefreshedTokensFromPath(
    ...
    try {
      ...
      // Why: after app restart, Dobius has no last-written baseline. Identity
  ...
  private getWslRuntimeHomePath(distro: string): string | null {
    const home = getWslHome(distro)
    return home
      ? this.joinWslPath(home, '.local', 'share', 'dobius', 'codex-runtime-home', 'home')
      : null
  }
  ...
  private runtimeAuthMatchesAccount(
    ...
    // still match the account Dobius believes is selected.
  ...
  private runtimeAuthMatchesSystemDefaultIdentity(
    ...
    // system-default identity Dobius mirrored earlier.
    if (
  ...
  private getRuntimeHomePath(): string {
    return getDobiusManagedCodexHomePath()
  }
  ...
  private getLegacyManagedHomes(): string[] {
    ...
    for (const entry of accountEntries) {
      ...
      if (existsSync(join(managedHomePath, '.dobius-managed-home'))) {
  ...
  private getPreservedLegacySessionPath(runtimeFilePath: string, accountId: string): string {
    ...
    return `${basename}.dobius-legacy-${accountId}${extension}`
  }
    ...
    try {
      ...
      if (runtimeAuth !== systemDefaultAuth) {
        ...
        if (
          ...
          // Why: system-default Codex now refreshes tokens inside Dobius's
        ...
        // external logins/logouts into Dobius's runtime home so ordinary Dobius
  ...
  private restoreSystemDefaultSnapshot(options: { detectExternalLogin: boolean }): void {
    ...
    if (options.detectExternalLogin && !existsSync(runtimeAuthPath)) {
      // Why: once Dobius owns the runtime CODEX_HOME, deleting auth.json there is
      // a local logout signal for Dobius-launched Codex sessions, not a reason to
  ...
  private clearRuntimeAuthAfterSystemDefaultLogout(runtimeAuthPath: string): void {
    // Why: when the real ~/.codex auth disappears, Dobius should treat that as an
    // external logout for unmanaged sessions, even if runtime auth had already
    // refreshed inside Dobius's CODEX_HOME.
    rmSync(runtimeAuthPath, { force: true })
    this.captureSystemDefaultSnapshot({ force: true })
    this.persistRuntimeLogoutMarker()
    this.lastWrittenAuthJson = null
