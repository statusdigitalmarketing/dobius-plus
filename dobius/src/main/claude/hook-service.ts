...
function getManagedScript(
  ...
  if (target === 'local' && process.platform === 'win32') {
    return [
      ...
      ...(options.skipWhenDevinImportsClaude
        ? [
            // Why: Devin imports .claude hooks by default. Skip Dobius's managed
  ...
  return [
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${DOBIUS_AGENT_HOOK_PORT}/hook/claude" \\',
...
export class ClaudeHookService {
  ...
  // Why: install Dobius's Claude hook settings on the remote box rather than the
