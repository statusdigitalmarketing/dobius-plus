...
describe('ClaudeHookService.install', () => {
  it('installs managed hooks into Claude settings and preserves user Bedrock settings', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'dobius-claude-hooks-'))
    ...
    try {
            ...
            Stop: [
              ...
              },
              {
                hooks: [
                  {
                    type: 'command',
                    command: '/Users/old/.dobius/agent-hooks/claude-hook.sh'
                  }
                ]
              }
            ]
      ...
      expect(
        legacyCommands.some((command: string) =>
          command.includes('/Users/old/.dobius/agent-hooks/claude-hook.sh')
        )
      ).toBe(false)
      expect(isClaudeManagedCommand(legacy.hooks.StopFailure[0].hooks[0].command)).toBe(true)
      expect(
        readFileSync(join(tmpHome, '.dobius', 'agent-hooks', CLAUDE_SCRIPT_FILE_NAME), 'utf-8')
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const tmpHome = mkdtempSync(join(tmpdir(), 'dobius claude home with spaces '))
      vi.stubEnv('HOME', tmpHome)
      vi.stubEnv('USERPROFILE', tmpHome)
      try {
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const tmpHome = mkdtempSync(join(tmpdir(), 'dobius-claude-curl-'))
      vi.stubEnv('HOME', tmpHome)
      vi.stubEnv('USERPROFILE', tmpHome)
      try {
        expect(new ClaudeHookService().install().state).toBe('installed')
        const script = readFileSync(
          join(tmpHome, '.dobius', 'agent-hooks', CLAUDE_SCRIPT_FILE_NAME),
          'utf-8'
        )
...
describe('ClaudeHookService.installRemote', () => {
  it('writes Claude settings + managed script under the remote $HOME', async () => {
    ...
    for (const event of [
      ...
      expect(cmd).toContain('/home/dev/.dobius/agent-hooks/claude-hook.sh')
      expect(cmd).toMatch(/^if \[ -x /)
    }
    // Managed script body
    const script = fs.files.get('/home/dev/.dobius/agent-hooks/claude-hook.sh')
    ...
    expect(fs.modes.get('/home/dev/.dobius/agent-hooks/claude-hook.sh')).toBe(0o755)
  })
  ...
  it('preserves user-authored hook entries while sweeping old managed entries', async () => {
              ...
              hooks: [
                {
                  type: 'command',
                  command:
                    'if [ -x /home/dev/.dobius/agent-hooks/claude-hook.sh ]; then /bin/sh /home/dev/.dobius/agent-hooks/claude-hook.sh; fi'
                }
              ]
            }
          ]
    ...
    // Original user-authored entry survives, while stale Dobius entries are
...
describe('OpenClaudeHookService-compatible install', () => {
  ...
  it('installs managed hooks into OpenClaude settings without touching Claude settings', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'dobius-openclaude-hooks-'))
    vi.stubEnv('HOME', tmpHome)
    vi.stubEnv('USERPROFILE', tmpHome)
    try {
      ...
      expect(
        readFileSync(join(tmpHome, '.dobius', 'agent-hooks', OPENCLAUDE_SCRIPT_FILE_NAME), 'utf-8')
  ...
  it('writes remote OpenClaude settings under .openclaude', async () => {
    ...
    expect(command).toContain('/home/dev/.dobius/agent-hooks/openclaude-hook.sh')
    expect(fs.files.get('/home/dev/.dobius/agent-hooks/openclaude-hook.sh')).toContain('/hook/claude')
  })
})
