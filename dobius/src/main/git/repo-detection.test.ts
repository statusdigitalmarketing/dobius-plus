...
describe('isGitRepo', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(path.join(tmpdir(), 'dobius-repo-detect-'))
  })
  ...
  it('accepts a linked worktree when git itself cannot be run', () => {
    ...
    git(realRepo, [
      '-c',
      'user.name=Dobius Test',
      '-c',
      'user.email=dobius@example.com',
      'commit',
      '--allow-empty',
      '--message',
      'initial'
  ...
  it('rejects a .git file that points at a missing gitdir when git cannot be run', () => {
    ...
    mkdirSync(fakeRepo)
    writeFileSync(path.join(fakeRepo, '.git'), 'gitdir: /missing/dobius/gitdir')
