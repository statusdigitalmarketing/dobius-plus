  ...
  it('uses a named remote that matches a URL-valued branch remote', async () => {
    gitExecFileAsyncMock.mockImplementation((args: string[]) => {
      ...
      if (args[0] === 'config' && args.includes('branch.imp/chinese-translation.remote')) {
        return Promise.resolve({ stdout: 'https://github.com/pynickle/dobius.git\n' })
      }
      ...
      if (args[0] === 'remote' && args[1] === 'get-url' && args[2] === 'origin') {
        return Promise.resolve({ stdout: 'https://github.com/statusdigitalmarketing/dobius-plus.git\n' })
      }
      ...
      if (
        args[0] === 'rev-parse' &&
        args.includes('refs/remotes/pr-pynickle-dobius/imp/chinese-translation')
      ) {
    ...
    expect(result).toEqual({
      hasUpstream: true,
      upstreamName: 'pr-pynickle-dobius/imp/chinese-translation',
      ahead: 2,
      behind: 0
    })
  })
