...
describe('git remote operations', () => {
  ...
  it('pushes to the configured upstream remote and branch', async () => {
    gitExecFileAsyncMock.mockImplementation(async (args: string[]) => {
      ...
      if (args[0] === 'config' && args.includes('branch.review/pr-1738.remote')) {
        return { stdout: 'pr-prateek-dobius\n', stderr: '' }
      }
    ...
    expect(gitExecFileAsyncMock).toHaveBeenLastCalledWith(
      ['push', '--set-upstream', 'pr-prateek-dobius', 'HEAD:prateek/fix-sidebar-agents-toggle'],
      { cwd: '/repo' }
    )
  })
  ...
  it('pushes to a URL-valued branch pushRemote when no named remote exists', async () => {
    gitExecFileAsyncMock.mockImplementation(async (args: string[]) => {
      ...
      if (args[0] === 'config' && args.includes('branch.imp/chinese-translation.pushRemote')) {
        return { stdout: 'https://github.com/pynickle/dobius.git\n', stderr: '' }
      }
      if (args[0] === 'config' && args.includes('remote.pushDefault')) {
      ...
      if (args[0] === 'remote' && args[1] === 'get-url') {
        return { stdout: 'https://github.com/statusdigitalmarketing/dobius-plus.git\n', stderr: '' }
      }
      if (args[0] === 'remote') {
        return { stdout: 'origin\n', stderr: '' }
      }
    ...
    expect(gitExecFileAsyncMock).toHaveBeenLastCalledWith(
      [
        'push',
        '--set-upstream',
        'https://github.com/pynickle/dobius.git',
  ...
  it('normalizes a URL-valued branch remote to a matching named remote before pushing', async () => {
    gitExecFileAsyncMock.mockImplementation(async (args: string[]) => {
      ...
      if (args[0] === 'remote' && args[1] === 'get-url' && args[2] === 'pr-pynickle-dobius') {
        return { stdout: 'https://github.com/pynickle/dobius.git\n', stderr: '' }
      }
      if (args[0] === 'remote') {
        return { stdout: 'origin\npr-pynickle-dobius\n', stderr: '' }
    ...
    expect(gitExecFileAsyncMock).toHaveBeenLastCalledWith(
      ['push', '--set-upstream', 'pr-pynickle-dobius', 'HEAD:imp/chinese-translation'],
      { cwd: '/repo' }
    )
  })
