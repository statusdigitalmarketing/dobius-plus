...
async function performAddWorktree(
  ...
  // every Dobius-created worktree wants the same default. True per-worktree
  ...
  //   with the "benign and idempotent" rationale above — every Dobius-created
 ...
 * for this op. The caller owns migrating Dobius's path-derived worktree identity
...
async function performRemoveWorktree(
  ...
  try {
    // Why: `git worktree remove` only detaches the filesystem entry. Dobius also
