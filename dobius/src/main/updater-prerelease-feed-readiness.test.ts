...
function buildAtomFeed(tags: string[]): string {
  return `<?xml version="1.0" encoding="UTF-8"?><feed>${tags
    .map(
      (tag) =>
        `<entry><link rel="alternate" type="text/html" href="https://github.com/statusdigitalmarketing/dobius-plus/releases/tag/${tag}"/><title>${tag}</title></entry>`
    )
    .join('')}</feed>`
}
...
function buildManifest(tag: string): string {
  ...
  return [
    `version: ${version}`,
    'files:',
    `  - url: Dobius-${version}-arm64-mac.zip`,
    '    sha512: test',
    `path: Dobius-${version}-arm64-mac.zip`
  ].join('\n')
}
...
function respondWithAtom(
  ...
  netFetchMock.mockImplementation((url: string, init?: { method?: string }) => {
    if (url === 'https://github.com/statusdigitalmarketing/dobius-plus/releases.atom') {
...
describe('fetchNewerReleaseTagsWithReadiness', () => {
  ...
  it('requires every asset referenced by the manifest files list to be reachable', async () => {
    netFetchMock.mockImplementation((url: string, init?: { method?: string }) => {
          ...
          text: () =>
            Promise.resolve(
              [
                `version: ${version}`,
                'files:',
                `  - url: Dobius-${version}-mac.zip`,
                '    sha512: test',
                `  - url: Dobius-${version}-arm64-mac.zip`,
                '    sha512: test',
                `path: Dobius-${version}-mac.zip`
              ].join('\n')
            )
        })
      }
      ...
      if (init?.method === 'HEAD') {
        return Promise.resolve({
          ok: !url.endsWith('/Dobius-1.4.28-arm64-mac.zip'),
          text: () => Promise.resolve('')
        })
      }
  ...
  it('accepts absolute manifest asset URLs without rewriting them to release asset paths', async () => {
          ...
          text: () =>
            Promise.resolve(
              [
                'version: 1.4.27',
                'files:',
                '  - url: https://downloads.example.com/Dobius-1.4.27-arm64-mac.zip',
                '    sha512: test'
              ].join('\n')
            )
        })
    ...
    expect(assetUrls).toEqual(['https://downloads.example.com/Dobius-1.4.27-arm64-mac.zip'])
  })
