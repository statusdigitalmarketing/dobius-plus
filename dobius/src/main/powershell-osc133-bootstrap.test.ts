...
describe('PowerShell OSC 133 bootstrap', () => {
  it('wraps prompt/readline without bypassing profiles or execution policy', () => {
    ...
    expect(script).toContain('DOBIUS_OPENCODE_CONFIG_DIR')
    expect(script).toContain('DOBIUS_MIMOCODE_HOME')
    expect(script).not.toContain('DOBIUS_PI_CODING_AGENT_DIR')
    expect(script).not.toContain('DOBIUS_OMP_CODING_AGENT_DIR')
    expect(script).toContain('DOBIUS_OMP_STATUS_EXTENSION')
    expect(script).toContain('function Global:omp')
    expect(script).toContain('--extension $env:DOBIUS_OMP_STATUS_EXTENSION')
    expect(script).toContain('DOBIUS_CODEX_HOME')
