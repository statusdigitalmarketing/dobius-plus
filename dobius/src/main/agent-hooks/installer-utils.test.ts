...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'dobius-installer-utils-test-'))
...
describe('createManagedCommandMatcher', () => {
  ...
  it('matches commands containing the agent-hooks/<scriptFileName> path', () => {
    expect(
      match('/bin/sh "/Users/alice/Library/Application Support/Dobius/agent-hooks/claude-hook.sh"')
    ).toBe(true)
  ...
  it('normalizes Windows backslashes so cmd-style paths still match', () => {
    expect(match('C:\\Users\\alice\\AppData\\Roaming\\Dobius\\agent-hooks\\claude-hook.sh')).toBe(
      true
    )
  })
  ...
  it('matches the guarded launcher form so wrapped commands sweep correctly', () => {
    ...
    expect(
      match(
        'if [ -x "/Users/alice/Library/Application Support/Dobius/agent-hooks/claude-hook.sh" ]; then /bin/sh "/Users/alice/Library/Application Support/Dobius/agent-hooks/claude-hook.sh"; fi'
      )
    ).toBe(true)
  })

  it('matches encoded Windows launcher commands by decoding their script path', () => {
    const command = wrapWindowsHookCommand('C:\\Users\\alice\\.dobius\\agent-hooks\\claude-hook.cmd')
    expect(match(command)).toBe(true)
  })

  it('matches the legacy per-userData script path AND the new shared ~/.dobius path', () => {
    // Why: install() must sweep old per-userData commands when migrating to
    // the shared ~/.dobius script path, or stale launchers keep failing.
    expect(
      match("/bin/sh '/Users/alice/Library/Application Support/dobius/agent-hooks/claude-hook.sh'")
    ).toBe(true)
    expect(match("/bin/sh '/Users/alice/.dobius/agent-hooks/claude-hook.sh'")).toBe(true)
  })
})
  ...
  it('removes managed direct bash/powershell/command fields', () => {
    const cleaned = removeManagedCommands(
      [
        {
          type: 'command',
          bash: '/bin/sh "/Users/alice/Dobius/agent-hooks/copilot-hook.sh"',
          timeoutSec: 5
        },
        {
          type: 'command',
          powershell: "& 'C:\\Users\\alice\\Dobius\\agent-hooks\\copilot-hook.sh'",
          timeoutSec: 5
        },
        {
          type: 'command',
...
describe('getSharedManagedScriptPath', () => {
  it("returns ~/.dobius/agent-hooks/<scriptFileName> rooted at the user's home", () => {
    expect(getSharedManagedScriptPath('claude-hook.sh')).toBe(
      join(homedir(), '.dobius', 'agent-hooks', 'claude-hook.sh')
    )
  })

  it('does not depend on Electron app.getPath, so two Dobius instances resolve to the same path', () => {
...
describe('wrapPosixHookCommand', () => {
  ...
  it('preserves spaces in the script path (Library/Application Support case)', () => {
    ...
    const cmd = wrapPosixHookCommand('/Users/a/Library/Application Support/Dobius/agent-hooks/x.sh')
    expect(cmd).toContain("'/Users/a/Library/Application Support/Dobius/agent-hooks/x.sh'")
  })
  ...
  it('can scope environment variables to the guarded script invocation', () => {
    const cmd = wrapPosixHookCommand('/does/not/exist.sh', {
      DOBIUS_COPILOT_HOOK_EVENT: 'UserPromptSubmit'
    })
    expect(cmd).toBe(
      "if [ -x '/does/not/exist.sh' ]; then DOBIUS_COPILOT_HOOK_EVENT='UserPromptSubmit' /bin/sh '/does/not/exist.sh'; fi"
    )
  })
...
describe('wrapWindowsHookCommand', () => {
  it('invokes the .cmd through an encoded PowerShell command', () => {
    const command = wrapWindowsHookCommand('C:\\Users\\alice\\.dobius\\agent-hooks\\codex-hook.cmd')
    ...
    expect(decodeWindowsHookCommand(command)).toBe(
      "& 'C:\\Users\\alice\\.dobius\\agent-hooks\\codex-hook.cmd'; exit $LASTEXITCODE"
    )
  })
  ...
  it('preserves spaces in the script path (user profile with space case)', () => {
    const cmd = wrapWindowsHookCommand('C:\\Users\\Jorge Silva\\.dobius\\agent-hooks\\codex-hook.cmd')
    expect(cmd).toMatch(qualifiedWindowsPowerShellCommand)
    expect(decodeWindowsHookCommand(cmd)).toBe(
      "& 'C:\\Users\\Jorge Silva\\.dobius\\agent-hooks\\codex-hook.cmd'; exit $LASTEXITCODE"
    )
  })

  it('keeps cmd.exe percent expansion and caret escapes out of the command line', () => {
    const cmd = wrapWindowsHookCommand('C:\\Users\\%DOBIUS_TEST%\\a^b\\codex-hook.cmd')
    expect(cmd).not.toContain('%DOBIUS_TEST%')
    expect(cmd).not.toContain('^')
    expect(decodeWindowsHookCommand(cmd)).toBe(
      "& 'C:\\Users\\%DOBIUS_TEST%\\a^b\\codex-hook.cmd'; exit $LASTEXITCODE"
    )
  })
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const scriptDir = join(tmpDir, 'home with ^ caret', '.dobius', 'agent-hooks')
      ...
      const result = spawnSync('cmd.exe', ['/d', '/c', wrapWindowsHookCommand(scriptPath)], {
        env: { ...process.env, DOBIUS_WRAP_TEST: 'expanded' }
      })
...
describe('wrapWindowsCmdHookCommand', () => {
  it('returns a bare .cmd path when cmd.exe can invoke it safely', () => {
    const scriptPath = 'C:\\Users\\alice\\.dobius\\agent-hooks\\codex-hook.cmd'
  ...
  it('falls back to the encoded launcher when cmd.exe would split or expand the path', () => {
    const scriptPath = 'C:\\Users\\Jane Doe\\%DOBIUS_TEST%\\codex-hook.cmd'
...
describe('wrapWindowsGitBashHookCommand', () => {
  it('returns a forward-slash .cmd path when Git Bash can execute it safely', () => {
    expect(
      wrapWindowsGitBashHookCommand('C:\\Users\\alice\\.dobius\\agent-hooks\\claude-hook.cmd')
    ).toBe('C:/Users/alice/.dobius/agent-hooks/claude-hook.cmd')
  })

  it('falls back to the encoded launcher when bash would split the path', () => {
    const scriptPath = 'C:\\Users\\Jane Doe\\.dobius\\agent-hooks\\claude-hook.cmd'
  ...
  it('falls back to the encoded launcher when bash metacharacters are present', () => {
    const scriptPath = 'C:\\Users\\alice & bob\\.dobius\\agent-hooks\\claude-hook.cmd'
    const command = wrapWindowsGitBashHookCommand(scriptPath)
    expect(command).toMatch(qualifiedWindowsPowerShellCommand)
...
describe('buildWindowsAgentHookPostCommand', () => {
  it('posts hook stdin through bounded curl without spawning PowerShell', () => {
    ...
    expect(command).toContain('-H "X-Dobius-Agent-Hook-Token: %DOBIUS_AGENT_HOOK_TOKEN%"')
    expect(command).toContain('--data-urlencode "paneKey=%DOBIUS_PANE_KEY%"')
...
describe('buildWindowsAgentHookCurlPostCommand', () => {
  it('posts form fields via curl.exe and reads the payload from stdin', () => {
    ...
    expect(command).toContain('http://127.0.0.1:%DOBIUS_AGENT_HOOK_PORT%/hook/codex')
    expect(command).toContain('-H "Content-Type: application/x-www-form-urlencoded"')
    expect(command).toContain('-H "X-Dobius-Agent-Hook-Token: %DOBIUS_AGENT_HOOK_TOKEN%"')
    expect(command).toContain('--data-urlencode "paneKey=%DOBIUS_PANE_KEY%"')
    expect(command).toContain('--data-urlencode "worktreeId=%DOBIUS_WORKTREE_ID%"')
