...
// Why: this module is the Dobius-main-process adapter for the shared
...
//     Dobius restart so retained dashboard rows reappear on relaunch
...
import { DOBIUS_HOOK_PROTOCOL_VERSION } from '../../shared/agent-hook-types'
import {
...
// Why: name of the on-disk cache that survives Dobius restart. Lives next to
...
export class AgentHookServer {
  ...
  private port = 0
  private token = ''
  // Why: identifies this Dobius instance so hook scripts can stamp requests and
  ...
  private normalizeHookBodyPaneKeyAlias(body: unknown): unknown {
    ...
    // DOBIUS_PANE_KEY. The reattach path proves the UUID leaf once, then this
   ...
   *  identity Dobius holds (the wire envelope carries connectionId: null and
   *  Dobius stamps the real value here). The relay has already normalized the
  ...
  async start(options?: {
    ...
    this.server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
      ...
      if (req.headers['x-dobius-agent-hook-token'] !== this.token) {
        res.writeHead(403)
        res.end()
        return
      }
  ...
  stop(): void {
    ...
    // would introduce a TOCTOU race vs. a concurrent Dobius instance.
  ...
  buildPtyEnv(): Record<string, string> {
    ...
    const env: Record<string, string> = {
      DOBIUS_AGENT_HOOK_PORT: String(this.port),
      DOBIUS_AGENT_HOOK_TOKEN: this.token,
      DOBIUS_AGENT_HOOK_ENV: this.env,
      DOBIUS_AGENT_HOOK_VERSION: DOBIUS_HOOK_PROTOCOL_VERSION
    }
    ...
    if (this.endpointFileWritten && this.endpointFilePathCache) {
      env.DOBIUS_AGENT_HOOK_ENDPOINT = this.endpointFilePathCache
    }
    return env
  }
