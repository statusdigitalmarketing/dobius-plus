...
const JSON_INSTALLERS = [
  ...
  },
  {
    agent: 'grok',
    timeout: MANAGED_HOOK_TIMEOUT_SECONDS,
    configPath: `${REMOTE_HOME}/.grok/hooks/dobius-status.json`,
    install: (sftp: SFTPWrapper) => new GrokHookService().installRemote(sftp, REMOTE_HOME)
  },
  {
    agent: 'copilot',
    timeout: 5,
    configPath: `${REMOTE_HOME}/.copilot/hooks/dobius.json`,
...
const MANAGED_HOOKS_DIR_NEEDLE = '/.dobius/agent-hooks/'

// Walk the parsed config and assert every Dobius-managed command carrier (a node
...
describe('managed agent hook timeouts', () => {
  ...
  it('writes a config-level timeout on local-only Droid hooks', () => {
    const homeDir = mkdtempSync(join(tmpdir(), 'dobius-droid-hook-timeout-'))
  ...
  it('bounds every generated POSIX curl wrapper with --connect-timeout and --max-time', async () => {
    ...
    const kimiWrapper = kimi.fs.files.get(`${REMOTE_HOME}/.dobius/agent-hooks/kimi-hook.sh`)!
  ...
  describe('dead-endpoint transport budget', () => {
      ...
      return new Promise((resolve, reject) => {
        const child = spawn('sh', [scriptPath], {
          env: {
            ...
            DOBIUS_AGENT_HOOK_TOKEN: 'test-token',
            DOBIUS_PANE_KEY: 'pane-1',
            DOBIUS_TAB_ID: 'tab-1',
    ...
    it.skipIf(!hasPosixCurl)(
      ...
      async () => {
        ...
        const wrapperBody = fs.files.get(`${REMOTE_HOME}/.dobius/agent-hooks/codex-hook.sh`)!

        tempDir = mkdtempSync(join(tmpdir(), 'dobius-hook-timeout-'))
