...
// Why: host-level backstop (seconds) for Dobius-managed status hooks. The shell
...
export function createManagedCommandMatcher(
  ...
  // entries use .sh. A platform switch should still sweep stale Dobius hooks.
  const needles = [
...
// Why: prod, dev, and parallel Dobius instances must write the same managed
...
export function getSharedManagedScriptPath(scriptFileName: string): string {
  return join(homedir(), '.dobius', 'agent-hooks', scriptFileName)
}
...
export function buildWindowsAgentHookPostCommand(source: AgentHookSource): string {
  ...
  return [
    `"%SystemRoot%\\System32\\curl.exe" -sS -X POST "http://127.0.0.1:%DOBIUS_AGENT_HOOK_PORT%/hook/${source}" ^`,
...
export function buildWindowsAgentHookCurlPostCommand(source: AgentHookSource): string {
  return [
    ...
    `"http://127.0.0.1:%DOBIUS_AGENT_HOOK_PORT%/hook/${source}"`,
    '--connect-timeout 0.5 --max-time 1.5',
    '-H "Content-Type: application/x-www-form-urlencoded"',
    '-H "X-Dobius-Agent-Hook-Token: %DOBIUS_AGENT_HOOK_TOKEN%"',
    '--data-urlencode "paneKey=%DOBIUS_PANE_KEY%"',
...
// Why: temp+rename so concurrent Dobius instances writing this shared path can't
