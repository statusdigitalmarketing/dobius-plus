...
describe('AgentHookServer listener replay', () => {
  ...
  it('suppresses local HTTP hook writes for a recently closed tab', async () => {
    ...
    try {
      ...
      const postHook = (prompt: string): Promise<Response> =>
        fetch(`http://127.0.0.1:${env.DOBIUS_AGENT_HOOK_PORT}/hook/claude`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'X-Dobius-Agent-Hook-Token': env.DOBIUS_AGENT_HOOK_TOKEN
          },
  ...
  it('hydrates cached statuses as not observed in the current runtime', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'dobius-agent-hooks-'))
  ...
  it('replays the latest retained pane status when a listener attaches after windowless events', async () => {
    ...
    try {
      const env = server.buildPtyEnv()
      expect(env.DOBIUS_AGENT_HOOK_PORT).toBeTruthy()
      expect(env.DOBIUS_AGENT_HOOK_TOKEN).toBeTruthy()

      const response = await fetch(`http://127.0.0.1:${env.DOBIUS_AGENT_HOOK_PORT}/hook/claude`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Dobius-Agent-Hook-Token': env.DOBIUS_AGENT_HOOK_TOKEN
    ...
    try {
      ...
      await expect(
        postClaudeHook({
          ...
          tool_name: 'Bash',
          tool_input: { command: 'rm -rf /tmp/dobius-subagent-repro' }
        })
      ...
      expect(server.getStatusSnapshot()).toEqual([
        expect.objectContaining({
          ...
          agentType: 'claude',
          toolName: 'Bash',
          toolInput: 'rm -rf /tmp/dobius-subagent-repro'
        })
      ])
    } finally {
      server.stop()
  ...
  it('lets Claude permission clear when approved PostToolUse matches the preceding tool use id', async () => {
    ...
    try {
      ...
      await postClaudeHook({
        hook_event_name: 'PreToolUse',
        tool_name: 'Bash',
        tool_input: { command: 'rm -rf /tmp/dobius-2824-permission-target' },
      ...
      expect(server.getStatusSnapshot()).toEqual([
        expect.objectContaining({
          ...
          agentType: 'claude',
          toolName: 'Bash',
          toolInput: 'rm -rf /tmp/dobius-2824-permission-target'
        })
      ])
    } finally {
      server.stop()
  ...
  it('does not replay cleared pane state to a newly attached listener', async () => {
    ...
    try {
      const env = server.buildPtyEnv()
      await fetch(`http://127.0.0.1:${env.DOBIUS_AGENT_HOOK_PORT}/hook/codex`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Dobius-Agent-Hook-Token': env.DOBIUS_AGENT_HOOK_TOKEN
    ...
    try {
      ...
      const postHook = async (
        ...
        const response = await fetch(
          `http://127.0.0.1:${env.DOBIUS_AGENT_HOOK_PORT}/hook/${source}`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
  ...
  it('accepts form-encoded hook posts from Unix managed scripts', async () => {
    ...
    try {
      ...
      const params = new URLSearchParams({
        ...
        env: 'production',
        version: env.DOBIUS_AGENT_HOOK_VERSION ?? '',
  ...
  it('tracks Codex agent statuses from form-encoded managed hook posts', async () => {
    ...
    try {
      ...
      const postCodexHook = async (payload: Record<string, unknown>): Promise<void> => {
        ...
        const response = await fetch(`http://127.0.0.1:${env.DOBIUS_AGENT_HOOK_PORT}/hook/codex`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
            'X-Dobius-Agent-Hook-Token': env.DOBIUS_AGENT_HOOK_TOKEN
  ...
  it('accepts Hermes plugin hook posts on /hook/hermes', async () => {
    ...
    try {
      const env = server.buildPtyEnv()
      const response = await fetch(`http://127.0.0.1:${env.DOBIUS_AGENT_HOOK_PORT}/hook/hermes`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Dobius-Agent-Hook-Token': env.DOBIUS_AGENT_HOOK_TOKEN
  ...
  it('accepts Amp plugin hook posts on /hook/amp', async () => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:${env.DOBIUS_AGENT_HOOK_PORT}/hook/amp`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Dobius-Agent-Hook-Token': env.DOBIUS_AGENT_HOOK_TOKEN
...
describe('AgentHookServer prompt-sent telemetry', () => {
  ...
  it('tracks OpenCode user MessagePart hooks once per message id', async () => {
    ...
    try {
      const env = server.buildPtyEnv()
      const response = await fetch(`http://127.0.0.1:${env.DOBIUS_AGENT_HOOK_PORT}/hook/opencode`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Dobius-Agent-Hook-Token': env.DOBIUS_AGENT_HOOK_TOKEN
      ...
      const updatedResponse = await fetch(
        `http://127.0.0.1:${env.DOBIUS_AGENT_HOOK_PORT}/hook/opencode`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
...
describe('Claude hook normalization', () => {
  ...
  describe('Stop transcript scan', () => {
    ...
    beforeEach(() => {
      tmpDir = mkdtempSync(join(tmpdir(), 'dobius-hook-test-'))
...
describe('Codex hook normalization', () => {
  ...
  it('PermissionRequest maps to waiting and surfaces the pending tool input', () => {
    // Why: Codex asks for user attention through PermissionRequest. Dobius's
...
describe('Copilot hook normalization', () => {
  ...
  it('PermissionRequest stays working and preserves tool context', () => {
    const result = _internals.normalizeHookPayload(
      ...
      buildBody({
        hook_event_name: 'PermissionRequest',
        tool_name: 'bash',
        tool_input: { command: 'rm -rf /tmp/dobius-test' }
      }),
      'production'
    )
    expect(result?.payload.state).toBe('working')
    expect(result?.payload.toolName).toBe('bash')
    expect(result?.payload.toolInput).toBe('rm -rf /tmp/dobius-test')
  })
  ...
  it('Stop reads the final assistant message from Copilot transcript events', () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'dobius-copilot-transcript-'))
  ...
  it('accepts authenticated HTTP posts on /hook/copilot', async () => {
    ...
    try {
      ...
      const listener = vi.fn()
      server.setListener(listener)
      const response = await fetch(`http://127.0.0.1:${env.DOBIUS_AGENT_HOOK_PORT}/hook/copilot`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Dobius-Agent-Hook-Token': env.DOBIUS_AGENT_HOOK_TOKEN
  ...
  it('updates Copilot Stop with final transcript text after a non-blocking retry', async () => {
    const server = new AgentHookServer()
    const tmpDir = mkdtempSync(join(tmpdir(), 'dobius-copilot-transcript-retry-'))
    const transcriptPath = join(tmpDir, 'events.jsonl')
    ...
    try {
      ...
      await fetch(`http://127.0.0.1:${env.DOBIUS_AGENT_HOOK_PORT}/hook/copilot`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Dobius-Agent-Hook-Token': env.DOBIUS_AGENT_HOOK_TOKEN
  ...
  it('updates Grok Stop with final chat-history text after a non-blocking retry', async () => {
    const server = new AgentHookServer()
    const tmpDir = mkdtempSync(join(tmpdir(), 'dobius-grok-chat-history-retry-'))
    ...
    try {
      ...
      await fetch(`http://127.0.0.1:${env.DOBIUS_AGENT_HOOK_PORT}/hook/grok`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Dobius-Agent-Hook-Token': env.DOBIUS_AGENT_HOOK_TOKEN
      ...
      const response = await fetch(`http://127.0.0.1:${env.DOBIUS_AGENT_HOOK_PORT}/hook/grok`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Dobius-Agent-Hook-Token': env.DOBIUS_AGENT_HOOK_TOKEN
...
describe('Endpoint file lifecycle', () => {
  ...
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'dobius-endpoint-'))
  })
  ...
  it('writes the endpoint file with the expected shell-sourceable shape', async () => {
    ...
    try {
      ...
      const expectedPort = server.buildPtyEnv().DOBIUS_AGENT_HOOK_PORT
      const expectedToken = server.buildPtyEnv().DOBIUS_AGENT_HOOK_TOKEN
      const prefix = process.platform === 'win32' ? 'set ' : ''
      expect(contents).toContain(`${prefix}DOBIUS_AGENT_HOOK_PORT=${expectedPort}`)
      expect(contents).toContain(`${prefix}DOBIUS_AGENT_HOOK_TOKEN=${expectedToken}`)
      expect(contents).toContain(`${prefix}DOBIUS_AGENT_HOOK_ENV=development`)
      expect(contents).toContain(`${prefix}DOBIUS_AGENT_HOOK_VERSION=1`)
    } finally {
      server.stop()
    }
  })
  ...
  it('rewrites the endpoint file with a new port after restart on the same path', async () => {
    ...
    const firstToken = server.buildPtyEnv().DOBIUS_AGENT_HOOK_TOKEN
    server.stop()

    await server.start({ env: 'production', userDataPath })
    try {
      const secondPath = server.endpointFilePath
      const secondPort = server.buildPtyEnv().DOBIUS_AGENT_HOOK_PORT
      const secondToken = server.buildPtyEnv().DOBIUS_AGENT_HOOK_TOKEN
      ...
      expect(contents).toContain(`DOBIUS_AGENT_HOOK_PORT=${secondPort}`)
      expect(contents).toContain(`DOBIUS_AGENT_HOOK_TOKEN=${secondToken}`)
      expect(contents).not.toContain(`DOBIUS_AGENT_HOOK_TOKEN=${firstToken}`)
    } finally {
      server.stop()
    }
  })
  ...
  it('leaves the endpoint file in place on stop()', async () => {
    ...
    // concurrent Dobius instance sharing userData that could rewrite the file
  ...
  it('buildPtyEnv includes DOBIUS_AGENT_HOOK_ENDPOINT when the server is running', async () => {
    const server = new AgentHookServer()
    await server.start({ env: 'production', userDataPath })
    try {
      const env = server.buildPtyEnv()
      expect(env.DOBIUS_AGENT_HOOK_ENDPOINT).toBe(server.endpointFilePath)
    } finally {
      server.stop()
    }
  })

  it('buildPtyEnv includes namespaced DOBIUS_AGENT_HOOK_ENDPOINT for development servers', async () => {
    const server = new AgentHookServer()
    await server.start({
      env: 'development',
      userDataPath,
      endpointNamespace: 'com.statusdigitalmarketing.dobius-plus.dev.test123'
    })
    try {
      const env = server.buildPtyEnv()
      expect(env.DOBIUS_AGENT_HOOK_ENDPOINT).toBe(server.endpointFilePath)
      expect(env.DOBIUS_AGENT_HOOK_ENDPOINT).toContain(
        'com.statusdigitalmarketing.dobius-plus.dev.test123'
      )
      expect(env.DOBIUS_AGENT_HOOK_PORT).toBeTruthy()
      expect(env.DOBIUS_AGENT_HOOK_TOKEN).toBeTruthy()
    } finally {
  ...
  it('keeps endpoint files separate for parallel dev namespaces', async () => {
    ...
    try {
      ...
      expect(firstServer.buildPtyEnv().DOBIUS_AGENT_HOOK_ENDPOINT).toBe(
        firstServer.endpointFilePath
      )
      expect(secondServer.buildPtyEnv().DOBIUS_AGENT_HOOK_ENDPOINT).toBe(
  ...
  it('buildPtyEnv omits DOBIUS_AGENT_HOOK_ENDPOINT when no userDataPath was provided', async () => {
  ...
  it('refuses to write the endpoint file when a value contains shell metacharacters', async () => {
    ...
    try {
      ...
      expect(server.buildPtyEnv().DOBIUS_AGENT_HOOK_ENDPOINT).toBeUndefined()
      // PORT/TOKEN still flow via PTY env — fail-open to v1 behavior.
      expect(server.buildPtyEnv().DOBIUS_AGENT_HOOK_PORT).toBeTruthy()
      expect(server.buildPtyEnv().DOBIUS_AGENT_HOOK_TOKEN).toBeTruthy()
    } finally {
      server.stop()
    }
  })
  ...
  it('endpoint file contents are re-parseable by /bin/sh', async () => {
    ...
    try {
      ...
      const out = execFileSync('/bin/sh', [
        '-c',
        `. "${filePath}" && echo "$DOBIUS_AGENT_HOOK_PORT"`
      ])
        .toString()
        .trim()
...
describe('Last-status persistence', () => {
  ...
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'dobius-laststatus-'))
  })
  ...
  async function postHookEvent(
    ...
    const env = server.buildPtyEnv()
    return fetch(`http://127.0.0.1:${env.DOBIUS_AGENT_HOOK_PORT}${path}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Dobius-Agent-Hook-Token': env.DOBIUS_AGENT_HOOK_TOKEN
