...
describe('remote hook service installers', () => {
  ...
  it('installs remote Codex hooks with matching trust entries', async () => {
    ...
    for (const eventName of [
      ...
      expect(command).toContain('/home/dev/.dobius/agent-hooks/codex-hook.sh')
      expect(command).toMatch(/^if \[ -x /)
    }
    expect(fs.files.get('/home/dev/.dobius/agent-hooks/codex-hook.sh')).toContain('#!/bin/sh')
    expect(fs.modes.get('/home/dev/.dobius/agent-hooks/codex-hook.sh')).toBe(0o755)
...
`
    ...
    for (const eventName of ['BeforeAgent', 'AfterAgent', 'AfterTool', 'BeforeTool']) {
      ...
      expect(command).toContain('/home/dev/.dobius/agent-hooks/gemini-hook.sh')
      expect(command).toMatch(/^if \[ -x /)
    }
    ...
    const antigravityConfig = JSON.parse(
      ...
      'dobius-status': Record<
        string,
        { matcher?: string; command?: string; hooks?: { command: string }[] }[]
      >
    }
    for (const eventName of ['PreInvocation', 'PostInvocation', 'Stop']) {
      const command = antigravityConfig['dobius-status'][eventName]?.[0]?.command
      expect(command).toContain('/home/dev/.dobius/agent-hooks/antigravity-hook.sh')
      expect(command).toContain(`DOBIUS_ANTIGRAVITY_EVENT='${eventName}'`)
    }
    expect(antigravityConfig['dobius-status'].PreToolUse).toBeUndefined()
    for (const eventName of ['PostToolUse']) {
      const definition = antigravityConfig['dobius-status'][eventName]?.[0]
    ...
    const ampPlugin = amp.fs.files.get('/home/dev/.config/amp/plugins/dobius-agent-status.ts')
    ...
    for (const eventName of [
      ...
      expect(definition?.command).toContain('/home/dev/.dobius/agent-hooks/cursor-hook.sh')
    ...
    for (const eventName of ['PreToolUse', 'PostToolUse', 'Stop']) {
      ...
      const command = definition?.hooks?.[0]?.command
      expect(command).toContain('/home/dev/.dobius/agent-hooks/command-code-hook.sh')
      expect(command).toMatch(/^if \[ -x /)
    }
    ...
    const grokConfig = JSON.parse(grok.fs.files.get('/home/dev/.grok/hooks/dobius-status.json')!) as {
      hooks: Record<string, { matcher?: string; hooks?: { command: string }[] }[]>
    }
    for (const eventName of [
      'SessionStart',
      ...
      const command = definition?.hooks?.[0]?.command
      expect(command).toContain('/home/dev/.dobius/agent-hooks/grok-hook.sh')
      expect(command).toMatch(/^if \[ -x /)
    }
    ...
    for (const eventName of [
      ...
      const command = definition?.hooks?.[0]?.command
      expect(command).toContain('/home/dev/.dobius/agent-hooks/devin-hook.sh')
      expect(command).toMatch(/^if \[ -x /)
    }
    ...
    expect(devin.fs.files.get('/home/dev/.dobius/agent-hooks/devin-hook.sh')).toContain('/hook/devin')
  })
  ...
  it('installs remote Kimi hooks as a managed config.toml block preserving user config', async () => {
    ...
    expect(config).toContain('/home/dev/.dobius/agent-hooks/kimi-hook.sh')
    expect(config).toMatch(/command = "if \[ -x /)
    expect(fs.files.get('/home/dev/.dobius/agent-hooks/kimi-hook.sh')).toContain('/hook/kimi')
  })
  ...
  it('does not overwrite malformed remote Devin JSONC', async () => {
    ...
    try {
      ...
      expect(fs.files.get('/home/dev/.dobius/agent-hooks/devin-hook.sh')).toBeUndefined()
    } finally {
      warn.mockRestore()
    }
  })
  ...
  it('removes stale remote Antigravity PreToolUse hooks while installing SSH hooks', async () => {
    ...
    const config = JSON.parse(fs.files.get('/home/dev/.gemini/config/hooks.json')!) as {
      'dobius-status': Record<string, { hooks?: { command: string }[] }[]>
    }
    expect(config['dobius-status'].PreToolUse).toBeUndefined()
    const postToolCommands = config['dobius-status'].PostToolUse.flatMap((definition) =>
  ...
  it('installs remote Copilot hooks under the user-level hooks directory', async () => {
    ...
    fs.files.set(
      '/home/dev/.copilot/hooks/dobius.json',
      JSON.stringify({
        version: 99,
    ...
    expect(status.state).toBe('installed')
    expect(status.configPath).toBe('/home/dev/.copilot/hooks/dobius.json')
    const config = JSON.parse(fs.files.get('/home/dev/.copilot/hooks/dobius.json')!) as {
      version: number
    ...
    for (const eventName of [
      ...
      expect(definition?.bash).toContain('/home/dev/.dobius/agent-hooks/copilot-hook.sh')
      expect(definition?.bash).toContain(`DOBIUS_COPILOT_HOOK_EVENT='${eventName}'`)
      expect(definition?.timeoutSec).toBe(5)
    }
    expect(config.disableAllHooks).toBeUndefined()
    expect(fs.files.get('/home/dev/.dobius/agent-hooks/copilot-hook.sh')).toContain('#!/bin/sh')
    expect(fs.modes.get('/home/dev/.dobius/agent-hooks/copilot-hook.sh')).toBe(0o755)
  })
  ...
  it('installs remote Hermes plugin files and enables the plugin', async () => {
    ...
    expect(fs.files.get('/home/dev/.hermes/plugins/dobius-status/plugin.yaml')).toContain(
      'pre_llm_call'
    )
    expect(fs.files.get('/home/dev/.hermes/plugins/dobius-status/__init__.py')).toContain(
      '/hook/hermes'
    )
    expect(fs.files.get('/home/dev/.hermes/config.yaml')).toContain('dobius-status')
  })

  it('does not overwrite a remote user-authored Amp plugin file', async () => {
    const { sftp, fs } = createFakeSftp({
      '/home/dev/.config/amp/plugins/dobius-agent-status.ts':
    ...
    expect(fs.files.get('/home/dev/.config/amp/plugins/dobius-agent-status.ts')).toBe(
      'export default function userPlugin() {}\n'
    )
  })
})
