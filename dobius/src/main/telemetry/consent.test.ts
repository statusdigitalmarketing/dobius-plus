...
describe('resolveConsent', () => {
  ...
  it('returns dobius_disabled when DOBIUS_TELEMETRY_DISABLED=1', () => {
    process.env.DOBIUS_TELEMETRY_DISABLED = '1'
    expect(
      resolveConsent(
        settingsWithTelemetry({
          optedIn: true,
      ...
      effective: 'disabled',
      reason: 'dobius_disabled'
    })
  })

  it('prefers do_not_track over dobius_disabled when both are set', () => {
    process.env.DO_NOT_TRACK = '1'
    process.env.DOBIUS_TELEMETRY_DISABLED = '1'
    expect(
      resolveConsent(
  ...
  it('env-var override wins over a stored pending_banner state', () => {
    ...
    ).toEqual({ effective: 'disabled', reason: 'dobius_disabled' })
  })
})
