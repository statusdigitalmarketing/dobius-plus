...
// Dobius+ fork: telemetry disconnected from dobius — never sends to us.i.posthog.com.
...
const BUILD_IDENTITY: 'stable' | 'rc' | null =
  typeof DOBIUS_BUILD_IDENTITY !== 'undefined'
    ...
    : ((globalThis as { DOBIUS_BUILD_IDENTITY?: 'stable' | 'rc' | null }).DOBIUS_BUILD_IDENTITY ?? null)
const WRITE_KEY: string | null =
  typeof DOBIUS_POSTHOG_WRITE_KEY !== 'undefined'
    ? DOBIUS_POSTHOG_WRITE_KEY
    : ((globalThis as { DOBIUS_POSTHOG_WRITE_KEY?: string | null }).DOBIUS_POSTHOG_WRITE_KEY ?? null)
 ...
 * DOBIUS_TELEMETRY_DISABLED). The SDK flag does not persist across process
...
// official stable/rc builds where CI injects `DOBIUS_BUILD_IDENTITY` and
// `DOBIUS_POSTHOG_WRITE_KEY`.
