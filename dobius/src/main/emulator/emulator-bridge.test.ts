...
import { RuntimeEmulatorCommands } from '../runtime/dobius-runtime-emulator'
...
describe('EmulatorBridge helper ownership', () => {
  ...
  it('stops the previous Dobius-managed helper when a worktree switches devices', async () => {
  ...
  it('shuts down the previous Dobius-managed device when requested', async () => {
    const bridge = new EmulatorBridge()
    bridge.registerActiveEmulator('wt-1', session('device-old'), { managed: true })
  ...
  it('only kills Dobius-managed helpers during app shutdown cleanup', async () => {
    const bridge = new EmulatorBridge()
