...
const MAC_OPEN_SHIM_DIR = join(tmpdir(), 'dobius-serve-sim-open-shim')
...
function getServeSimEnv(executable: ServeSimExecutable): NodeJS.ProcessEnv {
  ...
  if (openShimDir) {
    // Why: serve-sim needs Simulator.app attached for display/rotation, but Dobius embeds the stream.
        ...
        if (error) {
          ...
          if (/no serve-sim server|not running/i.test(message)) {
            reject(
              new EmulatorError(
                'emulator_no_active',
                'No active emulator for this worktree — use dobius emulator list/attach or open the pane'
              )
            )
            return
          }
