...
describe('ServeSimStateWatcher', () => {
  ...
  it('attaches to the serve-sim state directory when it appears after startup', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'dobius-serve-sim-watch-'))
  ...
  it('suppresses Dobius-managed sessions only while they are marked managed', async () => {
    ...
    watcher.unmarkDobiusManaged(TEST_UDID)
    watcher.ingestPtyOutput('pty-1', payload)
