...
// print JSON to the PTY. Dobius-managed attach (CLI/pane) already registers via EmulatorBridge;
...
export class ServeSimStateWatcher {
  ...
  private readonly dobiusManagedHelperKeys = new Set<string>()
  ...
  // Why: bridge calls this when Dobius CLI/pane attaches so we do not duplicate-tab on our own session.
  markDobiusManaged(info: ServeSimHelperInfo): void {
    this.dobiusManagedHelperKeys.add(helperInstanceKey(info))
  }

  unmarkDobiusManaged(deviceUdid: string): void {
    for (const key of this.dobiusManagedHelperKeys) {
      if (key.startsWith(`${deviceUdid}::`)) {
        this.dobiusManagedHelperKeys.delete(key)
      }
    }
  }
  ...
  stop(): void {
    ...
    this.dobiusManagedHelperKeys.clear()
  ...
  private emitIfExternal(
    ...
    if (this.dobiusManagedHelperKeys.has(instanceKey)) {
      return
    }
