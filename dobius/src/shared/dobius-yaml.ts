...
import type {
  DobiusDefaultTabTemplate,
  DobiusHooks,
  DobiusVmRecipe,
  DobiusVmRecipeDiagnostic
} from './types'
...
export const DOBIUS_VM_RECIPE_ID_PATTERN = /^[a-z0-9][a-z0-9._-]{0,63}$/
export const DOBIUS_VM_RECIPE_ID_RULE =
  'Use 1-64 lowercase letters, numbers, dots, underscores, or hyphens, starting with a letter or number.'

function normalizeDefaultTabs(value: unknown): DobiusDefaultTabTemplate[] {
  ...
  return value
    ...
    .filter((entry): entry is DobiusDefaultTabTemplate => entry !== null)
}

type VmRecipeParseResult = {
  recipes: DobiusVmRecipe[]
  diagnostics: DobiusVmRecipeDiagnostic[]
}

function normalizeVmRecipes(value: unknown): VmRecipeParseResult {
  const diagnostics: DobiusVmRecipeDiagnostic[] = []
  if (!Array.isArray(value)) {
  ...
  const recipes = value
    .map((entry, index) => {
      ...
      if (!DOBIUS_VM_RECIPE_ID_PATTERN.test(id)) {
        diagnostics.push({
          index,
          field: 'id',
          message: `Invalid recipe id "${id}". ${DOBIUS_VM_RECIPE_ID_RULE}`
        })
        return null
      }
      if (seenIds.has(id)) {
    ...
    .filter((entry): entry is DobiusVmRecipe => entry !== null)
  return { recipes, diagnostics }
}

/**
 * Parse the supported project defaults from `dobius.yaml`.
 */
export function parseDobiusYaml(content: string): DobiusHooks | null {
  let root: unknown
  try {
    root = parse(content)
  } catch {
