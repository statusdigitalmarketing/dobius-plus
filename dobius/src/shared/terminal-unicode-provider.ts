...
const DOBIUS_UNICODE_VERSION = 'dobius-11-zwj'
...
class DobiusUnicodeProvider implements IUnicodeVersionProvider {
  public readonly version = DOBIUS_UNICODE_VERSION
...
export function activateDobiusTerminalUnicodeProvider(terminal: XtermTerminalWithUnicodeCore): void {
  const { unicode } = terminal
  if (unicode.activeVersion === DOBIUS_UNICODE_VERSION) {
    return
  }
  ...
  if (!unicode.versions.includes(DOBIUS_UNICODE_VERSION)) {
    unicode.register(new DobiusUnicodeProvider(baseProvider))
  }
  unicode.activeVersion = DOBIUS_UNICODE_VERSION
}
