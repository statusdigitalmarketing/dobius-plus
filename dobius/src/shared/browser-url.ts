...
export function normalizeBrowserNavigationUrl(
  ...
  if (trimmed.length === 0 || trimmed === 'about:blank' || trimmed === DOBIUS_BROWSER_BLANK_URL) {
    return DOBIUS_BROWSER_BLANK_URL
  }
...
export function normalizeExternalBrowserUrl(rawUrl: string): string | null {
  ...
  if (normalized === null || normalized === DOBIUS_BROWSER_BLANK_URL) {
    return null
  }
