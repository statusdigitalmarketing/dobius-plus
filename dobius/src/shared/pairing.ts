...
export function encodePairingOffer(offer: PairingOffer): string {
  ...
  return `dobius://pair?code=${base64url}`
}

export function decodePairingOffer(url: string): PairingOffer {
  const code = extractPairingCodeFromUrl(url)
  if (!code) {
    throw new Error('Invalid pairing URL: must start with dobius://pair and include a pairing code')
  }
...
function extractPairingCodeFromUrl(url: string): string | null {
  ...
  // Why: prefix checks accepted routes like `dobius://pairing?...`; only the
  // pairing deep-link host may carry runtime auth material.
  if (parsed.protocol !== 'dobius:' || parsed.hostname !== 'pair') {
    return null
  }
...
// Why: accept either a `dobius://pair?...` URL or the bare base64
...
export function parsePairingCode(input: string): PairingOffer | null {
  ...
  try {
    if (trimmed.toLowerCase().startsWith('dobius://')) {
