...
describe('getGitCloneFailureMessage', () => {
  it('turns an existing destination into an actionable message after progress output', () => {
    expect(
      getGitCloneFailureMessage(
        [
          'Cloning into \u001b[32mdobius\u001b[0m...\r',
          "fatal: destination path 'dobius' already exists and is not an empty directory.\n"
        ].join(''),
        { clonePath: '/work/dobius' }
      )
    ).toBe(
      'Destination already exists and is not empty: /work/dobius. Choose a different parent folder, delete the existing folder, or add the existing repository instead.'
    )
  })
  ...
  it('prefers the last fatal line over a trailing fragment', () => {
    expect(
      ...
      'Destination already exists and is not empty: dobius. Choose a different parent folder, delete the existing folder, or add the existing repository instead.'
    )
  })
  ...
  it('uses the known clone path for relay destination fragments', () => {
    expect(
      getGitCloneFailureMessage('Clone failed: and the repository exists.', {
        clonePath: '/srv/dobius'
      })
    ).toBe(
      'Destination already exists and is not empty: /srv/dobius. Choose a different parent folder, delete the existing folder, or add the existing repository instead.'
    )
  })
