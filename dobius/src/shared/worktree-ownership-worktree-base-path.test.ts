...
import { buildKnownDobiusWorkspaceLayouts, classifyWorktreeOwnership } from './worktree-ownership'
...
describe('repo-specific worktree ownership layouts', () => {
  it('resolves the same relative base path from each repo root', () => {
    ...
    expect(buildKnownDobiusWorkspaceLayouts(settings, repoA)[0]).toEqual({
      path: '/projects/a/worktrees',
      nestWorkspaces: true
    })
    expect(buildKnownDobiusWorkspaceLayouts(settings, repoB)[0]).toEqual({
    ...
    expect(
      classifyWorktreeOwnership({
        ...
        knownDobiusLayouts: buildKnownDobiusWorkspaceLayouts(settings, repoA)
      })
    ).toBe('external')
    expect(
      classifyWorktreeOwnership({
        ...
        settings,
        worktree: makeWorktree('/projects/a/worktrees/repo/feature'),
        knownDobiusLayouts: buildKnownDobiusWorkspaceLayouts(settings, repoB)
      })
    ).toBe('external')
  })
  ...
  it('includes relative global layouts for SSH repos without applying absolute desktop paths', () => {
    ...
    expect(buildKnownDobiusWorkspaceLayouts(relativeSettings, repo)[0]).toEqual({
      path: '/remote/worktrees',
      nestWorkspaces: true
    })
    expect(
      buildKnownDobiusWorkspaceLayouts(absoluteSettings, repo).some(
