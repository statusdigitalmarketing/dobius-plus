...
export function parseSharedControlFrame(
  ...
  if (!sharedKey) {
    return {
      type: 'error',
      error: invalidRemoteRuntimeResponseError('Remote Dobius runtime returned a frame before E2EE.')
    }
  }
...
export function formatSharedControlCloseMessage(code: number, reason: Buffer): string {
  ...
  if (code !== 1005 && code !== 1006 && reasonText) {
    return `Remote Dobius runtime closed the connection (${code}: ${reasonText}).`
  }
  if (code !== 1005 && code !== 1006) {
    return `Remote Dobius runtime closed the connection (${code}).`
  }
  return 'Remote Dobius runtime closed the connection.'
}
