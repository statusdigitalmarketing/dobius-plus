...
// These types define the normalized status that Dobius receives from Claude,
...
export type AgentStateHistoryEntry = {
   ...
   *  (for example Claude Code `is_interrupt`) or Dobius's guarded interrupt
...
export type AgentStatusEntry = {
   ...
   *  inferred by Dobius's guarded interrupt fallback.
   ...
   *  Used only for exact CLI resume; Dobius terminal ids are not agent-session ids. */
