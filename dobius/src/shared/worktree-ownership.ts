...
export function buildKnownDobiusWorkspaceLayouts(
  ...
  const layouts: DobiusWorkspaceLayout[] = []
...
function appendWorkspaceLayouts(
  target: DobiusWorkspaceLayout[],
  source: readonly DobiusWorkspaceLayout[]
): void {
...
function buildWslWorkspaceLayouts(
  ...
  const root = `//wsl.localhost/${parsed.distro}${linuxHome}/dobius/workspaces`
...
export function classifyWorktreeOwnership(args: {
  ...
  meta?: WorktreeMeta
  settings: Pick<GlobalSettings, 'workspaceDir' | 'nestWorkspaces' | 'workspaceDirHistory'>
  knownDobiusLayouts: DobiusWorkspaceLayout[]
}): WorktreeOwnership {
  if (hasStrongDobiusMetadata(args.meta)) {
    return 'dobius-managed'
  }

  if (isUnderFlatOrUntrustedDobiusRoot(args.worktree.path, args.knownDobiusLayouts)) {
    return 'unknown-legacy'
  }

  if (canClassifyAsExternal(args.worktree.path, args.knownDobiusLayouts)) {
    // Why: a plain `git worktree add` can target Dobius's nested workspace
    // folder. Only metadata proves Dobius created it.
    return 'external'
  }
...
export function shouldShowWorktree(args: {
  ...
  if (args.ownership === 'dobius-managed') {
    return true
  }
  if (
...
function hasStrongDobiusMetadata(meta: WorktreeMeta | undefined): boolean {
  return Boolean(
    meta?.dobiusCreatedAt ||
    meta?.dobiusCreationWorkspaceLayout ||
    meta?.createdAt ||
...
function isUnderFlatOrUntrustedDobiusRoot(
  worktreePath: string,
  knownDobiusLayouts: DobiusWorkspaceLayout[]
): boolean {
  for (const layout of knownDobiusLayouts) {
...
function canClassifyAsExternal(
  ...
  if (knownDobiusLayouts.length === 0) {
    return false
  }
  for (const layout of knownDobiusLayouts) {
    const relative = relativePathInsideRoot(layout.path, worktreePath)
