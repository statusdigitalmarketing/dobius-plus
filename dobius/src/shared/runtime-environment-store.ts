...
const ENVIRONMENTS_FILE = 'dobius-environments.json'
...
export function addEnvironmentFromPairingCode(
  ...
  if (!offer) {
    throw new RuntimeEnvironmentStoreError(
      'invalid_argument',
      'Invalid pairing code. Expected a dobius://pair?... URL or bare pairing payload.'
    )
  }
...
function readEnvironmentStore(userDataPath: string): RuntimeEnvironmentStore {
  ...
  try {
    ...
    throw new RuntimeEnvironmentStoreError(
      'runtime_error',
      `Could not read Dobius environments at ${path}; the file is invalid.`
    )
  }
}
