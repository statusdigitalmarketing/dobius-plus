...
export function parseAgentHookEndpointFile(contents: string): AgentHookEndpoint {
  ...
  if (
    !values.DOBIUS_AGENT_HOOK_PORT ||
    !values.DOBIUS_AGENT_HOOK_TOKEN ||
    !values.DOBIUS_AGENT_HOOK_ENV ||
    !values.DOBIUS_AGENT_HOOK_VERSION
  ) {
    throw new Error('Agent hook endpoint file is missing required fields')
  }
  return {
    port: values.DOBIUS_AGENT_HOOK_PORT,
    token: values.DOBIUS_AGENT_HOOK_TOKEN,
    env: values.DOBIUS_AGENT_HOOK_ENV,
    version: values.DOBIUS_AGENT_HOOK_VERSION
  }
}
