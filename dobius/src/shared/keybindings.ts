...
export type TerminalShortcutPolicy = 'dobius-first' | 'terminal-first'
...
export function normalizeTerminalShortcutPolicy(
  ...
  return policy === 'terminal-first' ? 'terminal-first' : 'dobius-first'
}
...
export function keybindingIsActiveInContext(
  ...
  // Why: Dobius-first preserves existing app shortcut behavior inside terminals.
  // Terminal-first is the explicit escape hatch for shells and TUIs.
  if (normalizeTerminalShortcutPolicy(options.terminalShortcutPolicy) === 'dobius-first') {
    return true
  }
