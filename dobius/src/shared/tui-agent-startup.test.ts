...
describe('tui agent startup plans', () => {
  ...
  it('does not launch Codex with the Dobius profile when agent status hooks are enabled', () => {
    const plan = buildAgentStartupPlan({
      agent: 'codex',
      prompt: 'fix it',
      cmdOverrides: {},
  ...
  it('launches Claude without Dobius settings injection', () => {
    const plan = buildAgentStartupPlan({
      agent: 'claude',
      prompt: 'fix it',
      cmdOverrides: {},
  ...
  it('uses the Linux Dobius CLI command for Claude Agent Teams launches', () => {
    const plan = buildAgentStartupPlan({
    ...
    expect(plan?.launchCommand).toBe('dobius-ide claude-teams')
  })

  it('uses the plain dobius shim for Claude Agent Teams on Linux SSH remotes', () => {
    // Why: the SSH relay deploys the CLI shim as `dobius` (not the local-only
    // `dobius-ide` GNOME-screen-reader workaround), so a remote launch must not
    // emit `dobius-ide claude-teams` — that name is not on the remote PATH and
    ...
    expect(plan?.launchCommand).toBe('dobius claude-teams')
  })

  it('keeps the Windows dobius.cmd shim for Claude Agent Teams on SSH remotes', () => {
    // Why: the Windows remote shim is also `dobius.cmd`, matching the local
    ...
    expect(plan?.launchCommand).toBe('dobius.cmd claude-teams')
  })

  it('keeps the Linux dobius-ide wrapper for local (non-remote) Claude Agent Teams', () => {
    // Why: the `dobius-ide` rename is still required for a local Linux desktop
    // install (avoids shadowing the GNOME Dobius screen reader), so an explicit
  ...
  it('clears draft environment variables with the target shell syntax', () => {
    ...
    ).toBe('pi; Remove-Item Env:DOBIUS_PI_PREFILL -ErrorAction SilentlyContinue')
    ...
    ).toBe('pi & set "DOBIUS_PI_PREFILL="')
  })

  it('returns an OMP draft plan with DOBIUS_OMP_PREFILL (OMP-scoped, not Pi-shared)', () => {
    // Why: OMP owns its own managed prefill extension and env var.
    // dobius-prefill.ts reads DOBIUS_OMP_PREFILL for OMP launches — see
    ...
    expect(plan?.env).toEqual({ DOBIUS_OMP_PREFILL: 'fix the omp regression' })
    expect(plan?.expectedProcess).toBe('omp')
    expect(plan?.launchCommand).toBe('omp; unset DOBIUS_OMP_PREFILL')
  })
  ...
  it('excludes transient draft prompt env from launch config', () => {
    const plan = buildAgentDraftLaunchPlan({
      ...
      draft: 'prefill text',
      cmdOverrides: {},
      agentEnv: { DOBIUS_AGENT_MODE: 'managed' },
      platform: 'linux'
    })

    expect(plan?.env).toEqual({ DOBIUS_AGENT_MODE: 'managed', DOBIUS_PI_PREFILL: 'prefill text' })
    expect(plan?.launchConfig).toEqual({
      agentCommand: 'pi',
      agentArgs: '',
      agentEnv: { DOBIUS_AGENT_MODE: 'managed' }
