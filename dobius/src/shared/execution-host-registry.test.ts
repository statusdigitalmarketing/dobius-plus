...
describe('execution host registry', () => {
  ...
  it('hides runtime-owned (ephemeral VM) SSH targets from repo-derived hosts', () => {
    const hosts = buildExecutionHostRegistry({
      ...
      repos: [
        {
          connectionId: 'runtime-ssh-dobius-instance-1',
          executionHostId: 'ssh:runtime-ssh-dobius-instance-1'
        },
        { connectionId: 'repo-ssh' }
      ],
      settings: { activeRuntimeEnvironmentId: null },
      // Even if a stale label leaked in, it must still be filtered out.
      sshTargetLabels: new Map([['runtime-ssh-dobius-instance-1', 'Hidden VM']])
    })

    expect(hosts.some((h) => h.id.includes('runtime-ssh-dobius-instance-1'))).toBe(false)
          ...
          'dev-box',
          {
            status: {
              ...
              remoteControl: {
                ...
                lastError: 'Remote Dobius runtime closed the connection.'
              }
            }
          }
        ]
  ...
  it('includes runtime hosts from repo ownership but marks them disconnected without live status', () => {
    ...
    // No live status means no evidence the Dobius server is reachable, so it must
