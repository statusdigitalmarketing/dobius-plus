...
describe('pairing offer', () => {
  ...
  it('encode then decode round-trips correctly', () => {
    ...
    expect(url).toMatch(/^dobius:\/\/pair\?code=/)
  ...
  it('rejects dobius URLs outside the exact pairing route', () => {
    ...
    expect(parsePairingCode(`dobius://pairing?code=${code}`)).toBeNull()
    expect(parsePairingCode(`dobius://pair-extra?code=${code}`)).toBeNull()
    expect(() => decodePairingOffer(`dobius://pairing?code=${code}`)).toThrow('Invalid pairing URL')
  })

  it('rejects URLs without a pairing code', () => {
    expect(() => decodePairingOffer('dobius://pair')).toThrow('Invalid pairing URL')
  })

  it('decodes legacy hash URLs', () => {
    const url = encodePairingOffer(offer)
    const code = new URLSearchParams(url.slice(url.indexOf('?') + 1)).get('code')!
    expect(decodePairingOffer(`dobius://pair#${code}`)).toEqual(offer)
  })
  ...
  it('rejects payloads with missing fields', () => {
    ...
    expect(() => decodePairingOffer(`dobius://pair#${base64}`)).toThrow()
  })
...
describe('parsePairingCode', () => {
  ...
  it('parses a full dobius://pair# URL', () => {
    const url = encodePairingOffer(offer)
