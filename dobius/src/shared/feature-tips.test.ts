...
describe('feature tips', () => {
  it('orders new unseen tips before older unseen tips', () => {
    ...
    expect(tips.map((tip) => tip.id)).toEqual(['dobius-cli', 'cmd-j-palette', 'voice-dictation'])
  })

  it('skips tips the user has already seen', () => {
    const tips = getOrderedUnseenFeatureTips({
      seenTipIds: new Set<FeatureTipId>(['voice-dictation', 'dobius-cli', 'cmd-j-palette'])
    })
  ...
  it('skips tips for features the user has already interacted with', () => {
    ...
    expect(tips.map((tip) => tip.id)).toEqual(['dobius-cli', 'cmd-j-palette'])
  })
  ...
  it('normalizes persisted tip ids', () => {
    ...
    ).toEqual(['dobius-cli', 'cmd-j-palette', 'voice-dictation'])
  })
  ...
  it('describes the CLI tip as an install action with concrete workflows', () => {
    const cliTip = FEATURE_TIPS.find((tip) => tip.id === 'dobius-cli')

    expect(cliTip).toMatchObject({
      action: 'setup-cli',
      title: 'Let agents drive Dobius with the Dobius CLI',
