...
//     keyword land right after a path separator. Dobius is a cross-platform
...
export function normalizeTerminalTitle(title: string): string {
  ...
  // churn while preserving the working/idle transition Dobius keys off.
  if (isPiAgentTitle(title)) {
    const status = detectAgentStatusFromTitle(title)
    if (status === 'working') {
      return '\u280b Pi'
...
export function getAgentLabel(title: string): string | null {
  ...
  // (verified against the 2026.04.17 release) — Dobius synthesizes the same
...
// across the entire turn — it carries zero working/idle information. Dobius
...
export function detectAgentStatusFromTitle(title: string): AgentStatus | null {
  ...
  // action required") is either an Dobius-synthesized working/permission title
