...
describe('wsl login shell command helpers', () => {
  ...
  it('runs commands through the distro user login shell', () => {
    ...
    expect(command).toContain('exec "$_dobius_wsl_shell" -ilc')
  ...
  it('preserves command-scoped environment variables through the outer WSL shell', () => {
    const command = buildWslLoginShellCommand('HISTFILE=/tmp/dobius-history printf "$HISTFILE"')
    const escaped = escapeWslShCommandForWindows(command)

    expect(command).toContain('\'HISTFILE=/tmp/dobius-history printf "$HISTFILE"\'')
    expect(escaped).toContain('\\$_dobius_wsl_shell')
  ...
  it('preserves user command variables across the Windows-to-WSL argv boundary', () => {
    ...
    const command = buildWslLoginShellCommand('dobius_value=ok; printf "<%s>" "$dobius_value"')
    const escaped = escapeWslShCommandForWindows(command)
  ...
  it('starts an interactive login shell without assuming bash', () => {
    ...
    expect(command).toContain('getent passwd')
    expect(command).toContain('if [ -z "$_dobius_wsl_shell" ] || [ ! -x "$_dobius_wsl_shell" ]; then')
    expect(command).toContain('exec "$_dobius_wsl_shell" -l')
  })
})
