...
export function computerUseErrorRecoveryData(
  ...
  switch (code) {
    case 'app_not_found':
      return recoverWith(
        'Run `dobius computer list-apps --json` and retry with the exact app name or bundle ID.',
        'If the target is a website or web app such as Gmail, choose the desktop browser app/window that contains it; `dobius computer` app selectors refer to desktop apps, not website names.',
        'Do not retry the same `dobius computer ... --app <web app>` command unchanged.',
        'If the desired browser is not listed, open or focus that browser first, then retry `dobius computer list-apps --json` and `dobius computer list-windows --app <browser> --json`.'
      )
    case 'app_blocked':
      return recoverWith(
    ...
    case 'window_not_found':
      return recoverWith(
        'Run `dobius computer list-windows --app <app> --json` and target one of the listed windows.',
        'If the app is listed but no usable window is visible, retry observation once with `--restore-window`.',
        'If no window is listed, open or focus the app first; `dobius computer` does not launch closed desktop apps.'
      )
    ...
    case 'window_stale':
      return recoverWith(
        'Run `dobius computer list-windows --app <app> --json` and choose a current window selector.',
        'Then rerun `dobius computer get-app-state --app <app> --json` before acting.'
      )
    case 'provider_incompatible':
      return recoverWith(
        'Run `dobius computer capabilities --json` and verify the local provider supports the requested operation.',
        'Update Dobius or use a supported platform/provider path before retrying.'
      )
    case 'unsupported_capability':
      return recoverWith(
        'Run `dobius computer capabilities --json` and choose a supported action.',
    ...
    case 'permission_denied':
      return recoverWith(
        'Run `dobius computer permissions --json`, or `dobius computer permissions --id accessibility --json` / `--id screenshots --json` when the message names one missing permission.',
        'For remote or SSH targets, verify the command is running inside an active graphical desktop session.'
      )
    case 'element_not_found':
      return recoverWith(
        'Run `dobius computer get-app-state --app <app> --json` again and use an element index from the fresh tree.',
    ...
    case 'action_timeout':
      return recoverWith(
        'Run `dobius computer get-app-state --app <app> --json` before retrying so you know whether the UI changed.',
    ...
    case 'screenshot_failed':
      return recoverWith(
        ...
        'If the message mentions Screen Recording or screenshots permission, run `dobius computer permissions --id screenshots --json` and grant access before retrying.',
        'If the message mentions the payload cap, target a smaller/current window or use `--no-screenshot`.'
      )
    case 'accessibility_error':
      return recoverWith(
        'Run `dobius computer capabilities --json` to confirm the provider is available before retrying.',
        'If the message mentions permissions, run `dobius computer permissions --id accessibility --json` and grant access.',
