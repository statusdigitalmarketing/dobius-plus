...
import { DOBIUS_BROWSER_BLANK_URL } from './constants'
import {
  buildSearchUrl,
...
describe('browser-url helpers', () => {
  ...
  it('keeps normal web URLs and blank tabs in the allowed set', () => {
    ...
    expect(normalizeBrowserNavigationUrl('')).toBe(DOBIUS_BROWSER_BLANK_URL)
    expect(normalizeBrowserNavigationUrl('about:blank')).toBe(DOBIUS_BROWSER_BLANK_URL)
  })
  ...
  it('normalizes absolute local paths with spaces and reserved URL characters', () => {
    ...
    expect(normalizeBrowserNavigationUrl('C:\\tmp\\dobius & 100% ! ^\\index.html')).toBe(
      'file:///C:/tmp/dobius%20%26%20100%25%20!%20%5E/index.html'
    )
  })
