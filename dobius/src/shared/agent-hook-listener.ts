/* eslint-disable max-lines -- Why: this module is the canonical, transport-
   ...
   between Dobius's main process and the relay. Splitting by line count would
...
// docs/design/agent-status-over-ssh.md §3 ("relay normalizes; Dobius routes").
...
import { DOBIUS_HOOK_PROTOCOL_VERSION } from './agent-hook-types'
 ...
 *  Exported so non-HTTP ingest paths (e.g. Dobius's `ingestRemote`) can apply
 ...
 *  (last prompt, last tool snapshot, last status replay). Both Dobius's main
...
export function warnOnHookEnvOrVersionMismatch(
  ...
  if (
    version &&
    version !== DOBIUS_HOOK_PROTOCOL_VERSION &&
    !state.warnedVersions.has(version) &&
    state.warnedVersions.size < MAX_WARNED_KEYS
  ) {
    state.warnedVersions.add(version)
    console.warn(
      `[agent-hooks] received hook v${version}; server expects v${DOBIUS_HOOK_PROTOCOL_VERSION}. ` +
    ...
    if (!state.warnedEnvs.has(key) && state.warnedEnvs.size < MAX_WARNED_KEYS) {
      ...
      console.warn(
        `[agent-hooks] received ${env} hook on ${expectedEnv} server. ` +
          'Likely a stale terminal from another Dobius install.'
      )
    }
  }
}
...
export type AgentHookEventPayload = {
   ...
   *  Stamped only on the remote-ingest path (Dobius's `ingestRemote`); the
...
// those event names into Dobius's shared status states.
...
// into Dobius's shared status states while attributing the status to Kimi so the
...
export function normalizeHookPayload(
  ...
  // it null; the relay forwards null on the wire and Dobius's `ingestRemote`
...
export function writeEndpointFile(
  ...
  const valuesToWrite: [string, string][] = [
    ['DOBIUS_AGENT_HOOK_PORT', String(fields.port)],
    ['DOBIUS_AGENT_HOOK_TOKEN', fields.token],
    ['DOBIUS_AGENT_HOOK_ENV', fields.env],
    ['DOBIUS_AGENT_HOOK_VERSION', fields.version]
  ]
  ...
  try {
    ...
    // not leak the existence of this Dobius/relay install to other local users.
