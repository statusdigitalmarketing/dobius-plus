...
// the relay sends to Dobius. Consumed by `src/relay/agent-hook-server.ts` (which
...
// between Dobius's main process and the remote relay.
//
// Per the design doc:
// - The relay normalizes; Dobius routes. The envelope's `payload` field has
//   already been through `normalizeHookPayload` (which calls
//   `parseAgentStatusPayload` → `normalizeAgentStatusObject`) on the relay
//   side. Dobius's `ingestRemote` re-runs the canonical payload normalizer at
...
// - The wire `connectionId` is **always `null`**: a `connectionId` is Dobius's
//   local handle on an `ssh2` connection, not a wire identity. Dobius stamps the
...
//   POST body so Dobius's warn-once protocol diagnostics still fire. The relay
...
// with the same value so Dobius can attribute the event back to the right CLI.
...
/** Wire envelope for a single hook event flowing relay → Dobius. */
export type AgentHookRelayEnvelope = {
  source: AgentHookSource
  paneKey: string
  /** Ephemeral Dobius launch identity stamped into the PTY env for this process. */
  launchToken?: string
  tabId?: string
  worktreeId?: string
  /** Always `null` on the wire — relay does not know Dobius's local connectionId. */
  connectionId: null
  /** Preserved from the relay-side normalized hook event so Dobius can
  ...
  /** True when the relay is replaying its cache after Dobius reconnects. */
  isReplay?: boolean
  ...
  env?: string
  /** Forwarded verbatim from the agent CLI POST body. Lets Dobius's warn-once
   ...
   *  Dobius's `ingestRemote` validates it again at the SSH trust boundary. */
...
/** JSON-RPC request method Dobius issues after `--connect` reattach to ask the
...
/** JSON-RPC request method Dobius issues at session-ready to ship the
...
/** Feature-flag env var. Read once at process start by Dobius and the relay.
 *  Remote agent hooks ship as the default SSH behavior; set "0" to opt out. */
export const DOBIUS_FEATURE_REMOTE_AGENT_HOOKS_ENV = 'DOBIUS_FEATURE_REMOTE_AGENT_HOOKS' as const

export function isRemoteAgentHooksEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = env[DOBIUS_FEATURE_REMOTE_AGENT_HOOKS_ENV]
  if (raw === undefined) {
    return true
  }
