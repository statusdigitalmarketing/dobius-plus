...
describe('project host setup projection', () => {
  it('projects a legacy local repo into one project and one ready local setup', () => {
    const projection = projectHostSetupProjectionFromRepos(
      [repo({ id: 'repo-1', path: '/Users/alice/dobius', displayName: 'dobius' })],
      500
    )
  ...
  it('groups repo checkouts with the same provider identity under one project', () => {
    ...
    expect(getProjectHostSetupsForProject(projection.setups, 'github:statusdigitalmarketing/dobius-plus')).toHaveLength(
      2
    )
  })
  ...
  it('uses git remote identity as a provider identity fallback', () => {
    const projection = projectHostSetupProjectionFromRepos([
      ...
      repo({
        ...
        path: '/Users/alice/dobius/workspaces/dobius/re-enable-webgl-for-remote-runtime-terminals',
  ...
  it('ignores malformed provider identity values', () => {
    const projection = projectHostSetupProjectionFromRepos([
      repo({
        ...
        path: '/Users/alice/dobius',
        displayName: 'dobius',
        upstream: { owner: 'statusdigitalmarketing', repo: 42 } as never
      })
    ])
