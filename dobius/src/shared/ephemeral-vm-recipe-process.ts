...
function buildRecipeEnv(
  ...
  return {
    ...
    ...env,
    DOBIUS_VM_MODE: mode,
    DOBIUS_VM_INSTANCE_ID: context.instanceId ?? '',
    DOBIUS_RECIPE_ID: context.recipeId,
    DOBIUS_PROJECT_ID: context.projectId ?? '',
    DOBIUS_WORKSPACE_ID: context.workspaceId ?? '',
    DOBIUS_WORKSPACE_NAME: context.workspaceName ?? '',
    DOBIUS_REPO_PATH: context.repoPath,
    DOBIUS_REPO_URL: context.repoUrl ?? '',
    DOBIUS_REPO_BRANCH: context.branch ?? '',
    DOBIUS_REPO_REF: context.ref ?? '',
    DOBIUS_VERSION: context.dobiusVersion ?? ''
  }
}
