...
function resolveBaseCommand(args: {
  ...
  // Why: Codex status hooks live in Dobius's runtime CODEX_HOME; adding
...
function buildSleepingAgentLaunchConfig(args: {
  ...
  return {
    ...
    // durable resume snapshot is limited to Dobius-managed agent env inputs.
...
export function buildAgentStartupPlan(args: {
   ...
   * `dobius-ide` rename must be skipped for remote launches. */
  isRemote?: boolean
...
export function buildAgentResumeStartupPlan(args: {
  ...
  agentEnv?: Record<string, string> | null
  agentCommand?: string | null
  /** Why: see buildAgentStartupPlan — remote launches use the plain `dobius` shim. */
  isRemote?: boolean
}): AgentStartupPlan | null {
