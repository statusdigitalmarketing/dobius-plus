...
describe('createSequencedSetupAgentCommands', () => {
  ...
  it('wraps POSIX setup and startup commands with a matching nonce marker', () => {
    const result = createSequencedSetupAgentCommands({
      runnerScriptPath: '/repo/.git/dobius/setup-runner.sh',
    ...
    expect(result.setupCommand).toContain('bash /repo/.git/dobius/setup-runner.sh')
    ...
    expect(result.setupCommand).toContain(
      'mv -f /repo/.git/dobius/setup-runner.sh.nonce-123.done.tmp'
    )
    ...
    expect(result.startupCommand).toContain(
      'rm -f /repo/.git/dobius/setup-runner.sh.nonce-123.done /repo/.git/dobius/setup-runner.sh.nonce-123.done.tmp'
    )
  ...
  it('uses launch-specific marker paths for overlapping setup gates', () => {
    ...
    expect(first.setupCommand).toContain('/repo/.git/dobius/setup-runner.sh.first-launch.done')
    expect(first.startupCommand).toContain('/repo/.git/dobius/setup-runner.sh.first-launch.done')
    expect(second.setupCommand).toContain('/repo/.git/dobius/setup-runner.sh.second-launch.done')
    expect(second.startupCommand).toContain('/repo/.git/dobius/setup-runner.sh.second-launch.done')
    expect(first.setupCommand).not.toContain('/repo/.git/dobius/setup-runner.sh.second-launch.done')
    expect(second.setupCommand).not.toContain('/repo/.git/dobius/setup-runner.sh.first-launch.done')
  })
  ...
  it('uses the converted Linux marker path for WSL UNC runners on Windows', () => {
    const result = createSequencedSetupAgentCommands({
      runnerScriptPath:
        '\\\\wsl.localhost\\Ubuntu\\home\\jin\\repo\\.git\\worktrees\\feature\\dobius\\setup-runner.sh',
      startupCommand: 'claude',
      platform: 'windows',
      nonce: 'nonce-wsl'
    })
    ...
    expect(result.setupCommand).toContain(
      'bash /home/jin/repo/.git/worktrees/feature/dobius/setup-runner.sh'
    )
    expect(result.setupCommand).toContain(
      '/home/jin/repo/.git/worktrees/feature/dobius/setup-runner.sh.nonce-wsl.done'
    )
  ...
  it('keeps remote POSIX runners in bash even from a Windows client', () => {
    const result = createSequencedSetupAgentCommands({
      runnerScriptPath: '/remote/repo/.git/worktrees/feature/dobius/setup-runner.sh',
      startupCommand: 'claude',
      platform: 'windows',
      nonce: 'nonce-remote'
    })
    ...
    expect(result.setupCommand).toContain(
      'bash /remote/repo/.git/worktrees/feature/dobius/setup-runner.sh'
    )
  ...
  it('wraps native Windows runners in a cmd-pinned setup and startup gate', () => {
    const result = createSequencedSetupAgentCommands({
      runnerScriptPath: 'C:\\repo\\.git\\dobius\\setup-runner.cmd',
    ...
    expect(result.setupCommand).toContain('cmd.exe /c ""C:\\repo\\.git\\dobius\\setup-runner.cmd""')
    expect(result.setupCommand).toContain('echo !DOBIUS_SETUP_NONCE!:!DOBIUS_SETUP_STATUS!')
    ...
    expect(result.startupCommand).toContain('!DOBIUS_SETUP_STATUS!')
    ...
    expect(result.startupCommand).not.toContain('if ""!DOBIUS_SETUP_STATUS!""==""124""')
    expect(result.startupCommand).not.toContain('if not ""!DOBIUS_SETUP_STATUS!""==""0""')
...
function resultPathWsl(): string {
  return '\\\\wsl.localhost\\Ubuntu\\home\\jin\\repo\\.git\\worktrees\\feature\\dobius\\setup-runner.sh'
}

function makeTempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), 'dobius-setup-sequencing-'))
  TEMP_DIRS.push(dir)
  return dir
}
