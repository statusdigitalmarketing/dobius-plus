...
const UNREACHABLE = 'Could not connect to the remote Dobius runtime.'
...
describe('isTailscaleEndpoint', () => {
  ...
  it('rejects non-Tailscale hosts and the surrounding 100.x space', () => {
    ...
    expect(isTailscaleEndpoint('wss://dobius.example.com')).toBe(false)
...
describe('withRemoteRuntimeTailscaleHint', () => {
  ...
  it('leaves non-connectivity errors untouched', () => {
    const auth = 'Remote Dobius runtime rejected the pairing token.'
