...
function makeSettings(overrides: Partial<GlobalSettings> = {}): GlobalSettings {
  return {
    workspaceDir: '/dobius/workspaces',
    nestWorkspaces: true,
    workspaceDirHistory: [],
...
describe('worktree ownership classification', () => {
  it('treats explicit Dobius metadata as managed even outside the workspace root', () => {
    const repo = makeRepo()
    ...
    expect(
      classifyWorktreeOwnership({
        ...
        meta: makeMeta({ dobiusCreatedAt: 1 }),
        knownDobiusLayouts: buildKnownDobiusWorkspaceLayouts(settings, repo)
      })
    ).toBe('dobius-managed')
  })

  it('treats nested Dobius workspace paths without metadata as external', () => {
    const repo = makeRepo()
    const settings = makeSettings()
    const layouts = buildKnownDobiusWorkspaceLayouts(settings, repo)
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({ path: '/dobius/workspaces/app/feature' }),
        knownDobiusLayouts: layouts
      })
    ).toBe('external')
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({ path: '/dobius/workspaces/other/feature' }),
        knownDobiusLayouts: layouts
      })
    ).toBe('external')
  })

  it('treats explicit Dobius creation layout metadata as managed', () => {
    const repo = makeRepo()
    const settings = makeSettings()
    expect(
      classifyWorktreeOwnership({
  ...
  it('does not treat metadata-free nested workspace paths as Dobius-managed for new repos', () => {
    ...
    const detected = toDetectedWorktree({
      ...
      worktree: makeWorktree({
        path: '/dobius/workspaces/app/manual-git-worktree',
  ...
  it('does not treat generic discovery metadata on nested workspace paths as Dobius-managed', () => {
    const repo = makeRepo({ externalWorktreeVisibility: 'hide' })
    const settings = makeSettings()
    const detected = toDetectedWorktree({
      repo,
  ...
  it('treats flat workspace-root descendants as unknown legacy without strong metadata', () => {
    ...
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({ path: '/dobius/workspaces/feature' }),
        knownDobiusLayouts: buildKnownDobiusWorkspaceLayouts(settings, repo)
      })
    ).toBe('unknown-legacy')
  })
  ...
  it('keeps flat-layout history weak after switching the same root to nested mode', () => {
    ...
    const settings = makeSettings({
      nestWorkspaces: true,
      workspaceDirHistory: [{ path: '/dobius/workspaces', nestWorkspaces: false }]
    })
    expect(
      classifyWorktreeOwnership({
        repo,
  ...
  it('handles Windows drive casing and separators', () => {
    ...
    const settings = makeSettings({ workspaceDir: 'C:\\Dobius\\Workspaces' })
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({
          id: 'repo-1::C:\\DOBIUS\\WORKSPACES\\App\\Feature',
          path: 'C:\\DOBIUS\\WORKSPACES\\App\\Feature',
          isMainWorktree: false
        }),
        knownDobiusLayouts: buildKnownDobiusWorkspaceLayouts(settings, repo)
      })
