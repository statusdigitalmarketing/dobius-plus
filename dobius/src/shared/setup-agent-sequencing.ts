...
export const SETUP_AGENT_SEQUENCE_STARTUP_COMMAND_ENV = 'DOBIUS_SEQUENCED_STARTUP_COMMAND'
...
function buildWindowsSetupCommand(setupCommand: string, markerPath: string, nonce: string): string {
  return wrapCmd([
    `set "DOBIUS_SETUP_MARKER=${escapeCmdSetValue(markerPath)}"`,
    `set "DOBIUS_SETUP_NONCE=${escapeCmdSetValue(nonce)}"`,
    'del /f /q "!DOBIUS_SETUP_MARKER!" "!DOBIUS_SETUP_MARKER!.tmp" 2>nul',
    `call ${setupCommand}`,
    'set "DOBIUS_SETUP_STATUS=!ERRORLEVEL!"',
    '> "!DOBIUS_SETUP_MARKER!.tmp" echo !DOBIUS_SETUP_NONCE!:!DOBIUS_SETUP_STATUS!',
    'move /y "!DOBIUS_SETUP_MARKER!.tmp" "!DOBIUS_SETUP_MARKER!" >nul',
    'exit /b !DOBIUS_SETUP_STATUS!'
  ])
}
...
function buildWindowsStartupCommand(
  ...
  const script = [
    '$marker = $env:DOBIUS_SETUP_MARKER',
    '$tmp = $marker + ".tmp"',
    '$nonce = $env:DOBIUS_SETUP_NONCE',
