...
import { getDobiusCliCommandNameForPlatform } from './dobius-cli-command-name'
...
export type TuiAgentConfig = {
   ...
   * submitting. Today only pi uses this (via Dobius's overlay-installed
   * `dobius-prefill` extension reading `DOBIUS_PI_PREFILL`). Equivalent in
   ...
   * `›` prompt only when the composer row exists, so Dobius can paste as soon
...
export const TUI_AGENT_CONFIG: Record<TuiAgent, TuiAgentConfig> = {
  claude: {
    ...
    // See PR https://github.com/statusdigitalmarketing/dobius-plus/pull/926 for context.
    draftPromptFlag: '--prefill'
  },
  'claude-agent-teams': {
    // Why: this is a Dobius-provided launch mode, not a separate upstream
    // binary. Detection follows the Dobius CLI, while the wrapper validates the
    // real Claude binary when it starts.
    detectCmd: 'dobius',
    detectCmdAliases: ['dobius-dev'],
    launchCmd: 'dobius claude-teams',
    launchCmdByPlatform: {
      linux: `${getDobiusCliCommandNameForPlatform('linux')} claude-teams`,
      win32: `${getDobiusCliCommandNameForPlatform('win32')} claude-teams`
    },
    expectedProcess: 'claude',
  ...
  ante: {
    ...
    // once and exits), so Dobius launches the bare interactive TUI and injects
  ...
  pi: {
    ...
    // extensions list) so the paste frequently never lands. Dobius's
    // overlay installs an `dobius-prefill` pi extension (see
    ...
    draftPromptEnvVar: 'DOBIUS_PI_PREFILL'
  },
  omp: {
    detectCmd: 'omp',
    launchCmd: 'omp',
    expectedProcess: 'omp',
    promptInjectionMode: 'argv',
    draftPromptEnvVar: 'DOBIUS_OMP_PREFILL'
  },
  gemini: {
    detectCmd: 'gemini',
    launchCmd: 'gemini',
  ...
  'command-code': {
    ...
    // mirrors the preflight trust behavior Dobius applies to other first-run
  ...
  hermes: {
    ...
    // `--tui` starts the full-screen agent UI Dobius is designed to host.
  ...
  copilot: {
    ...
    // completion, which would kill the TUI session Dobius is hosting.
    // `-i/--interactive <prompt>` starts an interactive session with the
    // initial prompt pre-executed — the behavior Dobius needs.
  ...
  devin: {
    ...
    // `stdin-after-start` starts the REPL with no argv prompt; Dobius then sends
...
export function getTuiAgentLaunchCommand(
  ...
  // Why: the SSH relay shim is always named `dobius` on Unix, so the local-only
  // `dobius-ide` rename (avoids shadowing the GNOME Dobius screen reader) must not
