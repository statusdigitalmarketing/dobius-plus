...
function baseSummary(overrides: Partial<HostedReviewQueueSummary> = {}): HostedReviewQueueSummary {
  return {
    identity: { provider: 'github', host: 'github.com', owner: 'acme', repo: 'dobius', number: 42 },
...
describe('classifyHostedReview', () => {
  it('classifies mine/requested/agent/teammate', () => {
    ...
    expect(
      classifyHostedReview(baseSummary({ author: { login: 'dobius-ci' } }), {
        agentAuthorLogins: ['dobius-ci']
      }).state
    ).toBe('agent')
