...
describe('command token scanner', () => {
  ...
  it('preserves quoted command paths with spaces', () => {
    expect(getFirstCommandToken('"C:\\Program Files\\Dobius\\codex.cmd" --resume')).toBe(
      'C:\\Program Files\\Dobius\\codex.cmd'
    )
  })

  it('extracts path basenames without allocating path segment arrays', () => {
    expect(getCommandTokenPathBasename('C:\\Program Files\\Dobius\\codex.cmd')).toBe('codex.cmd')
