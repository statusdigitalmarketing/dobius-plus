...
function formatRemoteRuntimeCloseMessage(code: number, reason: Buffer): string {
  ...
  return suffixParts.length > 0
    ? `Remote Dobius runtime closed the connection (${suffixParts.join(': ')}).`
    : 'Remote Dobius runtime closed the connection.'
}
...
export async function sendRemoteRuntimeRequest<TResult>(
  ...
  return await new Promise((resolve, reject) => {
    ...
    const cleanupSocketListeners = (): void => {
      ...
      // Why: the settled one-shot no longer needs Dobius callbacks, but a ws
    ...
    function onTimeout(): void {
      finish({
        ...
        error: new RemoteRuntimeClientError(
          'runtime_timeout',
          'Timed out waiting for the remote Dobius runtime to respond.'
        )
      })
    }
    ...
    function onError(): void {
      finish({
        ...
        error: new RemoteRuntimeClientError(
          ...
          'Could not connect to the remote Dobius runtime.'
        )
      })
    }
    ...
    function onMessage(data: WebSocket.RawData, isBinary: boolean): void {
      ...
      if (isBinary) {
        finish({
          ...
          error: new RemoteRuntimeClientError(
            ...
            'Remote Dobius runtime returned an unexpected binary frame.'
          )
        })
        return
      }
      ...
      if (plaintext === null) {
        finish({
          ...
          error: new RemoteRuntimeClientError(
            'invalid_runtime_response',
            'Remote Dobius runtime returned an undecryptable frame.'
          )
        })
        return
      }
    ...
    function handleReadyFrame(frame: string): void {
      ...
      try {
        ...
        finish({
          ...
          error: new RemoteRuntimeClientError(
            'invalid_runtime_response',
            'Remote Dobius runtime returned an invalid E2EE handshake frame.'
          )
        })
        return
      }
      if (
        ...
        finish({
          ...
          error: new RemoteRuntimeClientError(
            'invalid_runtime_response',
            'Remote Dobius runtime returned an unexpected E2EE handshake frame.'
          )
        })
        return
      }
    ...
    function handleAuthenticatedFrame(plaintext: string): void {
      ...
      try {
        ...
        finish({
          ...
          error: new RemoteRuntimeClientError(
            'invalid_runtime_response',
            'Remote Dobius runtime returned an invalid E2EE auth frame.'
          )
        })
        return
      }
      ...
      if (type !== 'e2ee_authenticated') {
        ...
        finish({
          ...
          error: new RemoteRuntimeClientError(
            code,
            'Remote Dobius runtime rejected the pairing token.'
          )
        })
        return
      }
    ...
    function handleRpcFrame(plaintext: string): void {
      ...
      try {
        ...
        finish({
          ...
          error: new RemoteRuntimeClientError(
            'invalid_runtime_response',
            'Remote Dobius runtime returned an invalid response frame.'
          )
        })
        return
      }
      ...
      if (response.id !== requestId) {
        finish({
          ...
          error: new RemoteRuntimeClientError(
            'invalid_runtime_response',
            'Remote Dobius runtime returned a mismatched response id.'
          )
        })
        return
      }
...
export async function subscribeRemoteRuntimeRequest<TResult>(
  ...
  return await new Promise((resolve, reject) => {
    ...
    const cleanupSocketListeners = (): WebSocket | null => {
      ...
      socket.off('message', onMessage)
      ws = null
      // Why: startup failures detach Dobius callbacks before closing the ws,
    ...
    const timeout = setTimeout(() => {
      fail(
        new RemoteRuntimeClientError(
          'runtime_timeout',
          'Timed out waiting for the remote Dobius runtime subscription to start.'
        )
      )
    }, timeoutMs)
      ...
      if (type !== 'e2ee_authenticated') {
        const code =
          ...
          (authenticated as { error?: { code?: unknown } }).error?.code === 'unauthorized'
            ? 'unauthorized'
            : 'invalid_runtime_response'
        fail(new RemoteRuntimeClientError(code, 'Remote Dobius runtime rejected the pairing token.'))
        return
      }
      state = 'ready'
      ws?.send(
    ...
    function handleBinaryFrame(frame: Uint8Array<ArrayBufferLike>): void {
      if (state !== 'ready') {
        fail(
          new RemoteRuntimeClientError(
            'invalid_runtime_response',
            'Remote Dobius runtime returned binary data before authentication.'
          )
        )
        return
      }
      ...
      if (plaintext === null) {
        fail(
          new RemoteRuntimeClientError(
            'invalid_runtime_response',
            'Remote Dobius runtime returned an undecryptable binary frame.'
          )
        )
        return
      }
