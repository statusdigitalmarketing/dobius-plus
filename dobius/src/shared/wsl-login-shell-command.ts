...
export function buildWslLoginShellCommand(command: string): string {
  ...
  return [
    '_dobius_wsl_shell=$(getent passwd "$(id -un)" 2>/dev/null | cut -d: -f7)',
    'if [ -z "$_dobius_wsl_shell" ] || [ ! -x "$_dobius_wsl_shell" ]; then',
    '  _dobius_wsl_shell="${SHELL:-/bin/bash}"',
    'fi',
    'if [ -z "$_dobius_wsl_shell" ] || [ ! -x "$_dobius_wsl_shell" ]; then',
    '  _dobius_wsl_shell=/bin/sh',
    'fi',
    '_dobius_wsl_shell_name=$(basename "$_dobius_wsl_shell" | tr "[:upper:]" "[:lower:]")',
    'case "$_dobius_wsl_shell_name" in',
    `  sh|dash) exec "$_dobius_wsl_shell" -lc ${quotedCommand} ;;`,
    `  bash|zsh|ksh|mksh|ash) exec "$_dobius_wsl_shell" -ilc ${quotedCommand} ;;`,
