...
const EphemeralVmRecipeDobiusServerConnectionSchema = z
  .object({
    type: z.literal('dobius-server'),
...
export const EphemeralVmRecipeConnectionSchema = z.discriminatedUnion('type', [
  EphemeralVmRecipeDobiusServerConnectionSchema,
...
export function parseEphemeralVmRecipeResult(stdout: string): EphemeralVmRecipeResultParseResult {
  ...
  if (connection.type === 'dobius-server' && !parsePairingCode(connection.pairingCode)) {
    return { ok: false, error: 'Recipe result pairingCode is not a valid Dobius pairing code.' }
  }
...
export function getEphemeralVmRecipeResultPairingCode(
  ...
  return connection.type === 'dobius-server' ? connection.pairingCode : null
}
