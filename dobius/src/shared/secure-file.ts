...
// (.dobius runtime/auth/device stores) are not deleted at runtime, so this is acceptable; the
...
function bestEffortRestrictWindowsPath(targetPath: string, isDirectory: boolean): void {
  ...
  execFile(
    ...
    () => {
      ...
      // prevent Dobius from starting on Windows machines where PowerShell ACL APIs are
