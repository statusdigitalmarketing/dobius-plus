...
describe('external worktree inbox', () => {
  ...
  it('returns only hidden external worktrees outside the inbox baseline', () => {
    ...
    const detected = detectedResult([
      ...
      detectedWorktree({ id: 'dobius-managed', ownership: 'dobius-managed' })
    ])
  ...
  it('offers metadata-free nested Dobius workspace worktrees through the inbox', () => {
    const settings = makeSettings()
    const manual = toDetectedWorktree({
      repo,
      settings,
      worktree: makeGitWorktree({
        path: '/dobius/workspaces/dobius/manual-from-git',
