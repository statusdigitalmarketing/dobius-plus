...
describe('task source context', () => {
  ...
  it('keeps provider cache scopes separate by host and provider identity', () => {
    ...
    const differentRepo = getTaskSourceCacheScope({
      ...
      hostId: 'local',
      repoId: 'repo-1',
      providerIdentity: { provider: 'github', owner: 'other', repo: 'dobius' }
    })
  ...
  it('serializes provider identities for GitLab, Linear, and Jira cache scopes', () => {
    ...
    expect(
      getTaskSourceCacheScope({
        ...base,
        provider: 'gitlab',
        providerIdentity: { provider: 'gitlab', namespace: 'stably', project: 'dobius' }
      })
    ).toContain(encodeURIComponent('stably/dobius'))
    expect(
      getTaskSourceCacheScope({
        ...base,
        provider: 'linear',
