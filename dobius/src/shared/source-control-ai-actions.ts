...
export const SOURCE_CONTROL_ACTION_VARIABLE_INFO: Record<string, SourceControlActionVariableInfo> =
  {
    basePrompt: {
      description:
        'Dobius’s built-in prompt for this action, including the context Dobius knows how to gather safely.',
      example:
    ...
    firstPrompt: {
      description: 'The first user request that created the Dobius workspace.',
      example: 'Fix CI and commit the result'
    },
    assistantMessage: {
      description: 'The initial agent response, when Dobius has one available.',
