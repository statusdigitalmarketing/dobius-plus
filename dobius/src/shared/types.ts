...
export type Repo = {
  ...
  /** Controls Dobius's fork-default-branch sync offer for repos with upstream metadata. */
  forkSyncMode?: ForkSyncMode
  /** Canonical identity for the repo remote Dobius should use for provider-level grouping. */
  gitRemoteIdentity?: GitRemoteIdentity | null
  /** Controls whether worktrees Dobius did not create appear in the sidebar. */
...
export type Worktree = {
  ...
  manualOrder?: number
  lastActivityAt: number
  /** Set once when Dobius creates the worktree. Absent for worktrees discovered
  ...
  createdAt?: number
  /** Agent selected when Dobius originally created the worktree. Used only to
  ...
  baseRef?: string
  /** Remote/branch Dobius should publish review commits to when it created this worktree. */
...
export type GitPushTarget = {
  ...
  branchName: string
  remoteUrl?: string
  /** True when Dobius added this remote while preparing a fork-PR worktree. */
...
export type WorktreeMeta = {
  ...
  manualOrder?: number
  lastActivityAt: number
  /** See {@link Worktree.createdAt}. Persisted to dobius-data.json. */
  createdAt?: number
  /** See {@link Worktree.createdWithAgent}. Persisted to dobius-data.json. */
  createdWithAgent?: TuiAgent
  ...
  /** Intended create base for stale-base probes. Persisted metadata, not UI drift state. */
  baseRef?: string
  /** True when Dobius checked out a pre-existing local branch that delete must not prune. */
  preserveBranchOnDelete?: boolean
  /** See {@link Worktree.pushTarget}. Persisted so refreshed worktree lists keep the target. */
  pushTarget?: GitPushTarget
  /** Explicit marker stamped when Dobius creates the worktree. */
  dobiusCreatedAt?: number
  dobiusCreationSource?: 'desktop' | 'runtime' | 'cli' | 'ssh'
  /** Workspace layout active when Dobius created the worktree. */
  dobiusCreationWorkspaceLayout?: DobiusWorkspaceLayout
...
export type WorktreeOwnership = 'dobius-managed' | 'external' | 'unknown-legacy'
...
// existing persistence layer writes them to dobius-data.json automatically.
...
export type TerminalTab = {
   ...
   *  Why: agent CLIs overwrite the live title via OSC updates, but Dobius still
  ...
  startupCwd?: string
  /** Why: the coding-harness agent Dobius launched in this tab. Lets the tab bar
...
export type BrowserWorkspace = {
  id: string
  worktreeId: string
  /** Stable display label for the outer Dobius tab ("Browser 1", "Browser 2", …).
  ...
  // Dobius tab strip still renders one workspace entry. Mirror the active page's
...
export type GitHubWorkItemDetails = {
  // Why: main-process doesn't know Dobius's Repo.id, so this inner item omits
...
export type DobiusHooks = {
  ...
  defaultTabs?: DobiusDefaultTabTemplate[] // Terminal tabs to create once for a new worktree
  environmentRecipes?: DobiusVmRecipe[] // Project-scoped per-workspace environment recipes
  environmentRecipeDiagnostics?: DobiusVmRecipeDiagnostic[] // Non-fatal validation issues from environmentRecipes
}

export type DobiusDefaultTabTemplate = {
  title?: string
  color?: string
  command?: string
}

export type DobiusVmRecipe = {
  id: string
  name: string
  create: string
  description?: string
...
export type DobiusVmRecipeDiagnostic = {
  index: number
  field?: string
  message: string
}
...
/** All AI coding agents Dobius knows how to launch. Used for the agent picker in the new-workspace
 *  flow and for the default-agent setting. Extend this union as new agents are added. */
export type TuiAgent =
  | 'claude' // Claude Code
  | 'claude-agent-teams' // Claude Code Agent Teams via Dobius native panes
...
export type GlobalSettings = {
  ...
  workspaceDirHistory?: DobiusWorkspaceLayout[]
  ...
  /** When enabled, Dobius renames a workspace's auto-generated creature branch to
  ...
  /** App-wide color skin that recolors the whole UI chrome. 'none' (or unset)
   *  falls back to the plain dark/light theme. Values are legacy Dobius+ skin
   *  names (see APP_SKIN_NAMES in renderer lib/app-skin.ts). */
  appSkin?: string
   ...
   *  system tray instead of quitting Dobius; off keeps the default quit-on-close.
   ...
   *  need Dobius to launch terminals and scan agents in the same chosen distro
   ...
   *  tmux-compatible shim so teammate output stays on Dobius's normal PTY path. */
  ...
  /** Why: opening arbitrary links inside Dobius uses an isolated guest browser surface.
  ...
  /** Whether to show the Dobius app name in the titlebar. */
  ...
  /** Why: Dobius Mobile remains reachable from Settings; this only controls
   *  whether the top-level sidebar shortcut is shown. */
  showMobileButton?: boolean
  /** Why: Claude resumes spawn in the recorded folder and run only the resume
   *  command, with no `cd … &&` prefix. Turn off to fall back to the
   *  cd-prefixed form. Readers default to true. */
  cleanClaudeResume?: boolean
  ...
  /** Why: Dobius-first preserves fast workspace/app control from agent TUIs.
   ...
   *  the user's home directory; markdown notes use Dobius's app-owned
   ...
   *  ~/.dobius/keybindings.json; main migrates this once when present. */
   ...
   *  auth here lets Dobius prepare shared ~/.codex before the renderer hydrates,
  ...
  /** Why: Claude Code keeps conversations under one shared config root. Dobius
   ...
   *  working directory), so Dobius shows a confirmation dialog by default. Users
  ...
  /** When true, Dobius requests local awake assertions while hook-reported agents are working. */
...
export type DobiusWorkspaceLayout = {
  path: string
  nestWorkspaces: boolean
}
...
export type PersistedUIState = {
  ...
  /** Whether Dobius has already attempted to trigger the macOS notification
   ...
   *  tray, so the "Dobius is still running" notification shows only on first use. */
  ...
  /** Once the user has starred Dobius (from any entry point) we permanently
  ...
  trustedDobiusHooks?: PersistedTrustedDobiusHooks
  ...
  /** How many times each tour has auto-started. A tour that keeps getting
   *  cancelled before it paints (target flickers) would otherwise re-nag
   *  forever; a start cap retires it after a couple of impressions. */
  contextualTourStartCounts?: Partial<Record<ContextualTourId, number>>
...
export type PersistedTrustedDobiusHookEntry = {
  contentHash: string
  approvedAt: number
}

export type PersistedTrustedDobiusHookRepo = {
  all?: {
    approvedAt: number
  }
  setup?: PersistedTrustedDobiusHookEntry
  archive?: PersistedTrustedDobiusHookEntry
  issueCommand?: PersistedTrustedDobiusHookEntry
  vmRecipe?: PersistedTrustedDobiusHookEntry
}

export type PersistedTrustedDobiusHooks = Record<string, PersistedTrustedDobiusHookRepo>
...
export type AppMemory = UsageValues & {
  ...
  other: UsageValues
  /** Oldest-first memory samples (bytes) for the whole Dobius app, one per
