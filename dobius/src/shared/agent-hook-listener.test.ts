...
describe('shared agent-hook-listener', () => {
  ...
  it('normalizes Command Code hooks and reads turn text from the transcript', () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'dobius-command-code-transcript-'))
  ...
  it('reads newline-heavy Command Code transcripts without line-array splitting', () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'dobius-command-code-large-transcript-'))
    const transcriptPath = join(tmpDir, 'transcript.jsonl')
    try {
  ...
  it('reads Antigravity user requests from the transcript', () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'dobius-antigravity-prompt-'))
    const transcriptPath = join(tmpDir, 'transcript.jsonl')
    try {
      writeFileSync(
        transcriptPath,
  ...
  it('reads newline-heavy Antigravity user requests without wrapper regex matching', () => {
    ...
    const tmpDir = mkdtempSync(join(tmpdir(), 'dobius-antigravity-large-prompt-'))
    const transcriptPath = join(tmpDir, 'transcript.jsonl')
  ...
  it('keeps the cached Antigravity prompt instead of rescanning the transcript', () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'dobius-antigravity-cached-prompt-'))
    const transcriptPath = join(tmpDir, 'transcript.jsonl')
    try {
      writeFileSync(
        transcriptPath,
  ...
  it('normalizes Antigravity Stop hooks and reads final text from the transcript', () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'dobius-antigravity-transcript-'))
    const transcriptPath = join(tmpDir, 'transcript.jsonl')
    try {
      writeFileSync(
        transcriptPath,
  ...
  it('reads Grok final assistant text from chat history on Stop', () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'dobius-grok-session-'))
  ...
  it('does not let Grok sessionId escape the chat-history directory', () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'dobius-grok-session-escape-'))
    const cwd = join(tmpDir, 'workspace')
  ...
  describe('writeEndpointFile', () => {
    ...
    it('writes the endpoint file atomically with the right contents and mode', () => {
      ...
      expect(text).toContain('DOBIUS_AGENT_HOOK_PORT=12345')
      expect(text).toContain('DOBIUS_AGENT_HOOK_TOKEN=abcdef-0123')
      expect(text).toContain('DOBIUS_AGENT_HOOK_VERSION=1')
