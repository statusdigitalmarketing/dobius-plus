...
// Trimmed to a small set of values Dobius's PTY-typed-command launch architecture
// can emit:
//   - `binary_not_found` — `provider.spawn` ENOENT (the *shell* binary is
//     missing). The agent CLI being missing is invisible: Dobius spawns a
...
// to Dobius — see telemetry-plan.md §Decision: Defer per-incident error fields.
...
// `dobius_channel` is intentionally absent — it is a build-time common
// property baked in from `DOBIUS_BUILD_IDENTITY`, not a user-togglable setting.
//
...
const appStarredDobiusSchema = z
  .object({
...
const dobiusCliFeatureTipSourceSchema = z.enum(['app_open', 'manual'])
const dobiusCliFeatureTipShownSchema = z
  .object({
    source: dobiusCliFeatureTipSourceSchema,
    nth_repo_added: nthRepoAddedSchema
  })
  .strict()
const dobiusCliFeatureTipSetupClickedSchema = z
  .object({
    source: dobiusCliFeatureTipSourceSchema,
    nth_repo_added: nthRepoAddedSchema
  })
  .strict()
const dobiusCliFeatureTipSetupResultSchema = z
  .object({
    source: dobiusCliFeatureTipSourceSchema,
...
// `on_path` is misleading because Dobius's view of PATH is incomplete) from
...
export const eventSchemas = {
  ...
  app_starred_dobius: appStarredDobiusSchema,
  ...
  dobius_cli_feature_tip_shown: dobiusCliFeatureTipShownSchema,
  dobius_cli_feature_tip_setup_clicked: dobiusCliFeatureTipSetupClickedSchema,
  dobius_cli_feature_tip_setup_result: dobiusCliFeatureTipSetupResultSchema,
...
type _CohortExtendedRoster =
  ...
  | 'agent_prompt_sent'
  | 'agent_error'
  | 'dobius_cli_feature_tip_shown'
  | 'dobius_cli_feature_tip_setup_clicked'
  | 'dobius_cli_feature_tip_setup_result'
...
export const commonPropsSchema = z
  .object({
    ...
    dobius_channel: z.enum(['stable', 'rc'])
  })
  .strict()
