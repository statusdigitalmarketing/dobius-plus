...
async function seedCreatePREligibleBranch(
  ...
  return page.evaluate(async ({ createResult }) => {
    ...
    const pr = {
      ...
      state: 'open' as const,
      url: 'https://github.com/acme/dobius/pull/73',
...
test.describe('Source Control create pull request', () => {
  ...
  test('creates the pull request from the Source Control primary action', async ({ dobiusPage }) => {
    const { branch, worktreeId } = await seedCreatePREligibleBranch(dobiusPage)
    await openSourceControl(dobiusPage, worktreeId)
    await forceCreatePREligibleStatus(dobiusPage, worktreeId, branch)

    const titleInput = dobiusPage.getByRole('textbox', { name: 'Pull request title' })
    const descriptionInput = dobiusPage.getByRole('textbox', {
      name: 'Pull request description'
    })
    const createButton = getCreatePRComposerSubmitButton(dobiusPage)
    await expect(createButton).toBeVisible({ timeout: 10_000 })
    await expect(createButton).toBeEnabled()
    await expect(titleInput).toHaveValue('E2e secondary')
    await expect(dobiusPage.getByRole('textbox', { name: 'Pull request base branch' })).toHaveValue(
      'main'
    )
  ...
  test('surfaces create failures without clearing the pull request composer', async ({
    ...
    await expect(dobiusPage.getByText(failureMessage)).toBeVisible()
