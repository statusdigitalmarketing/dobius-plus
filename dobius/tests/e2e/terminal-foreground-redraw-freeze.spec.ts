...
test.describe('Terminal foreground redraw freeze repro', () => {
  test('Codex-style line rewrites request a visible row refresh', async ({ dobiusPage }) => {
    ...
    const { paneKey } = await waitForActivePaneHookDescriptor(dobiusPage)
    await waitForTerminalPtyDataInjector(dobiusPage, paneKey)
    await installActivePaneRefreshProbe(dobiusPage)
    try {
      const refreshBaseline = await readRefreshProbeCount(dobiusPage)
      await resetSchedulerDebug(dobiusPage)
      const measurement = await measureRendererDuringRewriteBurst(dobiusPage, paneKey)
      const scheduler = await readSchedulerDebug(dobiusPage)
      ...
      await expect
        .poll(async () => (await readRefreshProbeCount(dobiusPage)) - refreshBaseline, {
          timeout: 5_000,
          message: 'Codex-style terminal rewrites did not request an xterm refresh'
        })
        .toBeGreaterThan(0)
    } finally {
      await disposeActivePaneRefreshProbe(dobiusPage)
    }
  })
  ...
  test('active OpenTUI-style redraw bursts do not monopolize the renderer', async ({
    ...
    await waitForTerminalPtyDataInjector(dobiusPage, paneKey)
    await resetSchedulerDebug(dobiusPage)
    const measurement = await measureRendererDuringBurst(dobiusPage, paneKey)
    const scheduler = await readSchedulerDebug(dobiusPage)
  ...
  test('captured OpenCode/OpenTUI redraw bytes do not monopolize foreground writes', async ({
    ...
    await waitForTerminalPtyDataInjector(dobiusPage, paneKey)
    await resetSchedulerDebug(dobiusPage)
    const measurement = await measureRendererDuringFrames(dobiusPage, paneKey, frames)
    const scheduler = await readSchedulerDebug(dobiusPage)
    annotateMeasurement(testInfo, measurement, scheduler)
