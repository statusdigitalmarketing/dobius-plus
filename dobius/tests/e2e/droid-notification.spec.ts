...
test.describe('Droid notifications', () => {
  test('Codex hook completion dispatches while its worktree is inactive', async ({
    ...
    await sendToTerminal(dobiusPage, ptyId, `printf '${readyMarker}\\n'\r`)
    await waitForTerminalOutput(dobiusPage, readyMarker)

    const { paneKey, worktreeId } = await waitForActivePaneHookDescriptor(dobiusPage)
    ...
    await expect
      .poll(
        async () =>
          (await getRendererOrCachedAgentStatuses(dobiusPage)).some(
            (status) =>
    ...
    await switchToOtherExistingWorktree(dobiusPage)
    ...
    await expect
      .poll(
        async () =>
          (await getAgentStatuses(dobiusPage)).some(
            (status) =>
    ...
    await expect
      .poll(async () => isWorktreeUnread(dobiusPage, worktreeId), {
        timeout: 10_000,
  ...
  test('recognized agent title completion dispatches one task-complete notification', async ({
    ...
    await sendToTerminal(dobiusPage, ptyId, `printf '${marker}\\n'\r`)
    await waitForTerminalOutput(dobiusPage, marker)

    await emitOscTitle(dobiusPage, ptyId, 'Codex working')
    await expect
      .poll(async () => (await getRendererTitleLog(dobiusPage)).includes('Codex working'), {
        timeout: 10_000,
        message: 'Codex working title did not reach the renderer before completion'
      })
      .toBe(true)

    await emitOscTitle(dobiusPage, ptyId, 'Codex done')
    await expect
      .poll(async () => (await getRendererTitleLog(dobiusPage)).includes('Codex done'), {
        timeout: 10_000,
  ...
  test('Factory Droid needs-input native title does not dispatch a task-complete notification', async ({
    ...
    await emitOscTitle(dobiusPage, ptyId, '⠋ Droid')
    await emitOscTitle(dobiusPage, ptyId, 'Factory Droid needs input')

    await expect
      .poll(
        async () => (await getRendererTitleLog(dobiusPage)).includes('Factory Droid needs input'),
        {
          timeout: 10_000,
