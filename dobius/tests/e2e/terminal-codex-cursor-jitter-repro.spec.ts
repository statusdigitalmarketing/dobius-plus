...
test.describe('Codex terminal cursor jitter repro', () => {
  test('keeps queued-message cursor out of the Working status row in native Windows Codex @headful', async ({
    ...
    await installPtyWriteDiagnostics(dobiusPage)
    rmSync(ARTIFACT_DIR, { recursive: true, force: true })
    mkdirSync(ARTIFACT_DIR, { recursive: true })

    const shellCase = SHELL_CASES[0]!
    const { tabId, ptyId } = await prepareCodexTerminal(dobiusPage, shellCase)
    await installPtyOutputDiagnostics(dobiusPage)
    await dobiusPage.keyboard.type(CODEX_REPO_PROMPT)
    await dobiusPage.waitForTimeout(250)
    await dobiusPage.keyboard.press('Enter')
    await dobiusPage.waitForTimeout(1_000)
    if (!CODEX_WORKING_STATUS_RE.test(await getTerminalContentForTab(dobiusPage, tabId, 8_000))) {
      await dobiusPage.keyboard.press('Enter')
    }
    await dobiusPage.waitForTimeout(3_000)
    const submittedContent = await getTerminalContentForTab(dobiusPage, tabId, 8_000)
    writeFileSync(path.join(ARTIFACT_DIR, 'queued-message-after-submit.txt'), submittedContent)
    await expect
      .poll(
        async () =>
          CODEX_WORKING_STATUS_RE.test(await getTerminalContentForTab(dobiusPage, tabId, 8_000)),
        {
          timeout: 30_000,
      ...
      .toBe(true)
    await applyCursorProbeTheme(dobiusPage, tabId)
    ...
    await dobiusPage.keyboard.insertText('s')
    await expect
      .poll(
        async () =>
          (await readScreenLines(dobiusPage, tabId)).some((line) => isQueuedInputLine(line.text)),
        {
          timeout: 5_000,
    ...
    const snapshot = await readScreenSnapshot(dobiusPage, shellCase.label, tabId, ptyId)
    const rawChunks = await readPtyOutputDiagnostics(dobiusPage)
