...
test.describe('Terminal column desync repro', () => {
  test('PTY columns stay in sync with xterm across a visible resize', async ({ dobiusPage }) => {
    test.setTimeout(120_000)
    await waitForSessionReady(dobiusPage)
    await waitForActiveWorktree(dobiusPage)
    await closeRightSidebarAndFeatureTips(dobiusPage)
    await ensureTerminalVisible(dobiusPage)
    const ptyId = await settleTerminal(dobiusPage)

    // Baseline: a freshly fit terminal should agree with its PTY.
    const baseline = await readColumnSnapshot(dobiusPage, ptyId)
    expect(
      baseline.ptyCols,
    ...
    await dobiusPage.setViewportSize({ width: 760, height: 800 })
    await dobiusPage.waitForTimeout(400)
    const narrow = await readColumnSnapshot(dobiusPage, ptyId)
    expect(
      narrow.ptyCols,
      `after shrink, PTY cols (${narrow.ptyCols}) should equal xterm cols (${narrow.xtermCols})`
    ).toBe(narrow.xtermCols)

    await dobiusPage.setViewportSize({ width: 1280, height: 800 })
    await dobiusPage.waitForTimeout(400)
    const wide = await readColumnSnapshot(dobiusPage, ptyId)
    expect(
      wide.ptyCols,
  ...
  test('pty:getSize reports the size the PTY actually applied', async ({ dobiusPage }) => {
    test.setTimeout(120_000)
    await waitForSessionReady(dobiusPage)
    await waitForActiveWorktree(dobiusPage)
    await closeRightSidebarAndFeatureTips(dobiusPage)
    ...
    await dobiusPage.setViewportSize({ width: 900, height: 800 })
    await dobiusPage.waitForTimeout(500)

    const ptyCols = await readPtyCols(dobiusPage, ptyId)
    const reportedCols = await readReportedPtyCols(dobiusPage, ptyId)
    expect(
      reportedCols,
  ...
  test('PTY columns re-sync after the terminal is resized while hidden', async ({ dobiusPage }) => {
    test.setTimeout(120_000)
    await waitForSessionReady(dobiusPage)
    const homeWorktreeId = await waitForActiveWorktree(dobiusPage)
    const otherWorktreeId = (await getAllWorktreeIds(dobiusPage)).find((id) => id !== homeWorktreeId)
    ...
    await switchToWorktree(dobiusPage, otherWorktreeId)
    await waitForActiveTerminalManager(dobiusPage, 30_000)
    await dobiusPage.setViewportSize({ width: 720, height: 800 })
    await dobiusPage.waitForTimeout(500)
    ...
    const afterReturn = await readColumnSnapshot(dobiusPage, ptyId)
    expect(
      afterReturn.ptyCols,
  ...
  test('PTY columns re-sync after repeated background resizes', async ({ dobiusPage }) => {
    ...
    for (const [index, width] of widths.entries()) {
      await switchToWorktree(dobiusPage, otherWorktreeId)
      await waitForActiveTerminalManager(dobiusPage, 30_000)
      await dobiusPage.setViewportSize({ width, height: 800 })
      await dobiusPage.waitForTimeout(350)
      await switchToWorktree(dobiusPage, homeWorktreeId)
      await ensureTerminalVisible(dobiusPage)
      await waitForActiveTerminalManager(dobiusPage, 30_000)
      await dobiusPage.waitForTimeout(500)

      const snapshot = await readColumnSnapshot(dobiusPage, ptyId)
      expect(
        snapshot.ptyCols,
  ...
  test('both panes keep PTY columns synced after a vertical split reparent', async ({
    ...
    const firstPtyId = await settleTerminal(dobiusPage)

    const baseline = await readColumnSnapshot(dobiusPage, firstPtyId)
    expect(baseline.ptyCols).toBe(baseline.xtermCols)
    ...
    for (const pane of snapshot.panes) {
      ...
      const ptyCols = await readPtyCols(dobiusPage, ptyId)
      const xtermCols = await readRenderedColsForPty(dobiusPage, ptyId)
      expect(
        ptyCols,
  ...
  test('both panes stay PTY-synced when a tab MOUNTS with a split layout present', async ({
    ...
    await closeRightSidebarAndFeatureTips(dobiusPage)
    await ensureTerminalVisible(dobiusPage)
    await dobiusPage.setViewportSize({ width: 1440, height: 900 })
    await dobiusPage.waitForTimeout(300)
    await settleTerminal(dobiusPage)

    // Establish the persisted split layout once; reloads below rebuild it.
    await splitActiveTerminalPane(dobiusPage, 'vertical')
    await waitForPaneIdentitySnapshot(dobiusPage, 2)
    ...
    for (let attempt = 0; attempt < MOUNT_ATTEMPTS; attempt += 1) {
      ...
      await dobiusPage.setViewportSize({ width: 1440, height: 900 })
      await dobiusPage.reload()
      await dobiusPage.waitForFunction(() => Boolean(window.__store), null, { timeout: 30_000 })
      await waitForSessionReady(dobiusPage)
      await waitForActiveWorktree(dobiusPage)
      await closeRightSidebarAndFeatureTips(dobiusPage)
      await ensureTerminalVisible(dobiusPage)
      ...
      await dobiusPage.setViewportSize({ width: 1180, height: 800 })
      await dobiusPage.waitForTimeout(300)
  ...
  test('PTY columns stay synced when the window is resized during initial mount', async ({
    ...
    for (let attempt = 0; attempt < MOUNT_ATTEMPTS; attempt += 1) {
      ...
      const ptyId = await settleTerminal(dobiusPage)
      await dobiusPage.waitForTimeout(700)
