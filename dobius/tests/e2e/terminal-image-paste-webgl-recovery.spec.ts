...
test.describe('terminal image paste WebGL recovery @headful', () => {
  test('clears the WebGL atlas after a real image clipboard paste', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.dobius-image-paste-redraw-${marker}.mjs`)
    ...
    try {
      ...
      await waitForTerminalOutput(dobiusPage, `READY_${marker}`, 10_000)

      await forceWebgl(dobiusPage)
      ...
      await dobiusPage.locator('.xterm-helper-textarea').first().focus()
      await dobiusPage.evaluate(
        (dataUrl) => window.api.ui.writeClipboardImage(dataUrl),
        CLIPBOARD_IMAGE_DATA_URL
      )
      await dobiusPage.keyboard.press(process.platform === 'darwin' ? 'Meta+V' : 'Control+V')
      await waitForTerminalOutput(dobiusPage, `DONE_${marker}`, 10_000)

      await expect.poll(() => readAtlasResetCount(dobiusPage), { timeout: 2_000 }).toBeGreaterThan(0)
    } finally {
