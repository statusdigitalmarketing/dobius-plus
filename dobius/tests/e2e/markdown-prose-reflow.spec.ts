...
test.describe('Markdown prose reflow', () => {
  ...
  test('hard-wrapped prose reflows as one document paragraph', async ({ dobiusPage }, testInfo) => {
    let filePath: string | null = null

    try {
      filePath = await openHardWrappedFixture(dobiusPage, testInfo)
      const metrics = await getGoalParagraphMetrics(dobiusPage)
  ...
  test('deleting an inserted empty paragraph keeps hard-wrapped prose reflowing', async ({
    ...
    try {
      filePath = await openHardWrappedFixture(dobiusPage, testInfo)
      await placeCaretAtGoalParagraphEnd(dobiusPage)
  ...
  test('deleting slash text then the empty paragraph keeps hard-wrapped prose reflowing', async ({
    ...
    try {
      ...
      await placeCaretAtGoalParagraphEnd(dobiusPage)
      await dobiusPage.keyboard.press('Enter')
      await dobiusPage.keyboard.type('/')
      await dobiusPage.keyboard.press('Backspace')
      await dobiusPage.keyboard.press('Backspace')
  ...
  test('deleting the block boundary before a heading keeps hard-wrapped prose reflowing', async ({
    ...
    try {
      filePath = await openHardWrappedFixture(dobiusPage, testInfo)
      await placeCaretAtHeadingStart(dobiusPage)
      await dobiusPage.keyboard.press('Backspace')
