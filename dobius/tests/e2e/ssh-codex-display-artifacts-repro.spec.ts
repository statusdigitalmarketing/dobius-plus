...
const RUN_REAL_REMOTE_CODEX = process.env.DOBIUS_E2E_REAL_REMOTE_CODEX === '1'
const EXPECT_NO_ARTIFACTS = process.env.DOBIUS_E2E_EXPECT_NO_CODEX_ARTIFACTS === '1'
const CAPTURE_WHILE_REMOTE_TUI_RUNNING =
  process.env.DOBIUS_E2E_CAPTURE_WHILE_REMOTE_TUI_RUNNING === '1'
const HIDE_UNTIL_REMOTE_TUI_DONE = process.env.DOBIUS_E2E_HIDE_UNTIL_REMOTE_TUI_DONE === '1'
const CAPTURE_SCROLLBACK_ARTIFACT_REGION =
  process.env.DOBIUS_E2E_CAPTURE_SCROLLBACK_ARTIFACT_REGION === '1'
const FORCE_SSH_RECONNECT_DURING_TUI = process.env.DOBIUS_E2E_FORCE_SSH_RECONNECT_DURING_TUI === '1'
const KEEP_SSH_REPRO_TARGET = process.env.DOBIUS_E2E_KEEP_SSH_REPRO_TARGET === '1'

test.describe('Remote SSH Codex display artifacts repro', () => {
  test.skip(!RUN_DOCKER_SSH, 'Set DOBIUS_E2E_SSH_DOCKER=1 to run Docker-backed SSH repro.')
  ...
  test('does not leave duplicated Codex status output after SSH replay', async ({
    ...
    try {
      ...
      await ensureTerminalVisible(dobiusPage, 45_000)
      await waitForActiveTerminalManager(dobiusPage, 60_000)
      await enableRiskyTerminalRendererPath(dobiusPage)
      await installPtyReplayProbe(dobiusPage)

      const ptyId = await waitForActivePanePtyId(dobiusPage, 60_000)
      const doneMarker = RUN_REAL_REMOTE_CODEX
        ? `DOBIUS_REAL_REMOTE_CODEX_DONE_${Date.now()}`
        : REMOTE_TUI_DONE
      const cleanMarker = RUN_REAL_REMOTE_CODEX
        ? `DOBIUS_REAL_REMOTE_CODEX_CLEAN_${Date.now()}`
        : doneMarker
      ...
      await dobiusPage.waitForTimeout(1_200)
      if (FORCE_SSH_RECONNECT_DURING_TUI) {
        dropDockerSshClientSessions(target)
        await waitForDockerRemoteReconnected(dobiusPage, remote.targetId)
        await dobiusPage.waitForTimeout(2_000)
      }
      await (RUN_REAL_REMOTE_CODEX
        ? (async () => {
            await stressRestoreRemoteTerminalDuringCodex(dobiusPage, remote.worktreeId)
            await waitForRealRemoteCodexCompletion(dobiusPage, doneMarker)
          })()
        : (async () => {
            if (CAPTURE_WHILE_REMOTE_TUI_RUNNING) {
              await dobiusPage.waitForTimeout(10_000)
            } else {
              await switchToNonRemoteWorktree(dobiusPage, remote.worktreeId)
              await (HIDE_UNTIL_REMOTE_TUI_DONE
                ? waitForRemoteFixtureCleanFinalInHiddenPane(dobiusPage, remote.worktreeId)
                : dobiusPage.waitForTimeout(10_000))
            }
            if (CAPTURE_WHILE_REMOTE_TUI_RUNNING) {
              await dobiusPage.waitForTimeout(900)
              return
            }
            await switchToWorktree(dobiusPage, remote.worktreeId)
            await ensureTerminalVisible(dobiusPage, 45_000)
            await waitForActiveTerminalManager(dobiusPage, 60_000)
      ...
      if (CAPTURE_SCROLLBACK_ARTIFACT_REGION) {
        await scrollActiveTerminalToArtifactHistory(dobiusPage)
      }

      const { analysis, screenshot } = await captureGraySlabAnalysis(dobiusPage)
      analysis.replayDebug = await readReplayProbeSnapshot(dobiusPage)
      analysis.duplicateStatusRows = await readDuplicateStatusRows(dobiusPage)
      ...
      const resetEvidence = await resetWebglAndCaptureGraySlabAnalysis(dobiusPage)
      resetEvidence.analysis.replayDebug = await readReplayProbeSnapshot(dobiusPage)
      resetEvidence.analysis.duplicateStatusRows = await readDuplicateStatusRows(dobiusPage)
      ...
      if (RUN_REAL_REMOTE_CODEX) {
        await clearRemoteTerminalAfterCodex(dobiusPage, ptyId, cleanMarker)
      }
    } finally {
