...
test('mobile subscribe mounts overlay; collapse → chip; Take back dismisses', async ({
  ...
  const overlay = dobiusPage.locator('.mobile-driver-banner')
  await expect(overlay).toHaveCount(0)

  // Fire the IPC events main emits when a mobile client subscribes in 'auto'
  // mode (handleMobileSubscribe in src/main/runtime/dobius-runtime.ts). The
  ...
  await expectExpandedOverlayLeavesPaneReadable(dobiusPage, ptyId)
  ...
  await captureAttachment(dobiusPage, testInfo, 'overlay-loud.png')
  ...
  await expectChipIsCompactInPane(dobiusPage, ptyId)

  await captureAttachment(dobiusPage, testInfo, 'overlay-collapsed.png')
...
test('held phone-fit state mounts restore overlay without collapse', async ({
  ...
  await captureAttachment(dobiusPage, testInfo, 'overlay-held-fit.png')
...
test('restore this terminal refits the active restored pane', async ({ dobiusPage, electronApp }) => {
  await waitForSessionReady(dobiusPage)
  await waitForActiveWorktree(dobiusPage)
  await ensureTerminalVisible(dobiusPage)
  await waitForActiveTerminalManager(dobiusPage)
  ...
  await expect(dobiusPage.locator('.mobile-driver-banner')).toHaveCount(1, { timeout: 15_000 })
  await expect
    .poll(() => getPaneTerminalCols(dobiusPage, ptyId), {
...
test('restore all refits non-focused restored terminal panes', async ({
  ...
  const ptyIds = await waitForVisiblePanePtyIds(dobiusPage, 2)
  const focusPtyId = await waitForActivePanePtyId(dobiusPage)
  ...
  await expect(dobiusPage.locator('.mobile-driver-banner')).toHaveCount(2, { timeout: 15_000 })

  await forcePaneToOneColumn(dobiusPage, inactivePtyId)
  await expect
    .poll(() => getPaneTerminalCols(dobiusPage, inactivePtyId), {
...
test('restore all recovers a hidden workspace held at narrow terminal geometry', async ({
  ...
  await ensureTerminalVisible(dobiusPage)
  await waitForActiveTerminalManager(dobiusPage)
  const hiddenWorkspacePtyId = await waitForActivePanePtyId(dobiusPage)
  await sendHeldPhoneFitIpc(electronApp, { ptyId: hiddenWorkspacePtyId, cols: 45, rows: 20 })
  await expect(dobiusPage.locator('.mobile-driver-banner')).toHaveCount(1, { timeout: 15_000 })

  await forcePaneToOneColumnAndSwitchWorktree(dobiusPage, hiddenWorkspacePtyId, secondWorktreeId)
  await expect
    .poll(() => getActiveWorktreeId(dobiusPage), {
      timeout: 5_000,
      message: 'second worktree should become active before restore-all'
    })
    .toBe(secondWorktreeId)
  await expect
    .poll(() => getPaneTerminalCols(dobiusPage, hiddenWorkspacePtyId), {
      message: 'test harness should hold workspace 1 in the bad narrow state'
    })
    .toBeLessThanOrEqual(2)
  await ensureTerminalVisible(dobiusPage)
  await waitForActiveTerminalManager(dobiusPage)
  const activeWorkspacePtyId = await waitForActivePanePtyId(dobiusPage)
  await installRestoreTerminalFitAutoRestoreRecorder(electronApp)
  await sendHeldPhoneFitIpc(electronApp, { ptyId: activeWorkspacePtyId, cols: 45, rows: 20 })
  await expect(
    dobiusPage.locator(`[data-pty-id="${activeWorkspacePtyId}"] .mobile-driver-banner`)
