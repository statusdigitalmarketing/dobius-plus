...
async function createProjectHeaderSortFixture(): Promise<string[]> {
  ...
  const root = realpathSync(await mkdtemp(path.join(os.tmpdir(), 'dobius-e2e-project-sort-')))
  tempRoots.push(root)
...
test.describe('Project Group manual sorting', () => {
  test('dragging a project header body reorders the visible project headers', async ({
    ...
    const projects = await seedProjectHeaderSortScenario(dobiusPage, repoPaths)

    await expect
      .poll(() => getProjectHeaderOrder(dobiusPage, projects), {
        timeout: 12_000,
  ...
  test('dragging a duplicate-ranked Project Group header reorders the visible headers', async ({
    ...
    await waitForSessionReady(dobiusPage)
    const groups = await seedDuplicateTabOrderProjectGroups(dobiusPage)

    await expect
      .poll(() => getProjectGroupHeaderOrder(dobiusPage, groups), {
        timeout: 12_000,
