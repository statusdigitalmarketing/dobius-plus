...
function largePastePayload(runId: string): string {
  return `DOBIUS_LARGE_PASTE_${runId}_0123456789abcdef`.repeat(4096)
}
...
test.describe('large terminal paste responsiveness', () => {
  test('chunked keyboard paste keeps the renderer responsive while PTY writes are pending', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.dobius-large-paste-${runId}.cjs`)
    ...
    try {
      ...
      scriptStarted = true
      await waitForTerminalOutput(dobiusPage, `LARGE_PASTE_READY_${runId}`, 10_000)
      ...
      await installRendererHeartbeat(dobiusPage)
      await focusActiveTerminalInput(dobiusPage)

      const pasteKey = dobiusPage.keyboard.press(keyboardPasteChord())
      await expect
        .poll(
          async () =>
      ...
      const heartbeatBefore = await readRendererHeartbeat(dobiusPage)
      await dobiusPage.waitForTimeout(150)
      const heartbeatAfter = await readRendererHeartbeat(dobiusPage)
      expect(heartbeatAfter).toBeGreaterThan(heartbeatBefore)

      await pasteKey
      await waitForTerminalOutput(dobiusPage, doneLine, 20_000, 12_000)
      ...
      await stopRendererHeartbeat(dobiusPage).catch(() => undefined)
      if (scriptStarted) {
