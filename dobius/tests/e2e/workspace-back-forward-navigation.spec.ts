...
test.describe('Workspace Back/Forward Navigation', () => {
  ...
  test('buttons are hidden outside the terminal view', async ({ dobiusPage }) => {
    await expect(await getBackButton(dobiusPage)).toBeVisible()
    await expect(await getForwardButton(dobiusPage)).toBeVisible()
    ...
    await expect(await getBackButton(dobiusPage)).toHaveCount(0)
    await expect(await getForwardButton(dobiusPage)).toHaveCount(0)
  ...
  test('both buttons disabled at cold start with a single history entry', async ({ dobiusPage }) => {
    ...
    const activeId = await getActiveWorktreeId(dobiusPage)
    expect(activeId).not.toBeNull()

    await resetNavHistory(dobiusPage)
    await seedVisit(dobiusPage, activeId!)

    const back = await getBackButton(dobiusPage)
    const forward = await getForwardButton(dobiusPage)
    await expect(back).toBeDisabled()
    await expect(forward).toBeDisabled()
  })

  test('clicking Back and Forward walks the history stack', async ({ dobiusPage }) => {
    const worktreeIds = await getAllWorktreeIds(dobiusPage)
    ...
    await resetNavHistory(dobiusPage)
    await seedVisit(dobiusPage, primaryId)
    await seedVisit(dobiusPage, secondaryId)
    ...
    const primaryRow = worktreeRow(dobiusPage, primaryId)
    const secondaryRow = worktreeRow(dobiusPage, secondaryId)

    await back.click()
    await expect
      .poll(async () => getActiveWorktreeId(dobiusPage), {
  ...
  test('re-activating the current worktree is a no-op (dedupe)', async ({ dobiusPage }) => {
    const activeId = await getActiveWorktreeId(dobiusPage)
    expect(activeId).not.toBeNull()
    ...
    const snapshot = await getNavHistorySnapshot(dobiusPage)
    expect(snapshot.history).toEqual([activeId])
    expect(snapshot.index).toBe(0)
    await expect(await getBackButton(dobiusPage)).toBeDisabled()
  })

  test('new navigation after going back truncates the forward stack', async ({ dobiusPage }) => {
    const worktreeIds = await getAllWorktreeIds(dobiusPage)
    ...
    await seedVisit(dobiusPage, primaryId)
    await seedVisit(dobiusPage, secondaryId)
    await (await getBackButton(dobiusPage)).click()
    await expect.poll(() => getActiveWorktreeId(dobiusPage)).toBe(primaryId)

    // Forward button is live — a forward entry exists.
    await expect(await getForwardButton(dobiusPage)).toBeEnabled()
    ...
    await expect(await getForwardButton(dobiusPage)).toBeDisabled()
  })

  test(`${isMac ? 'Cmd' : 'Ctrl'}+Alt+Left/Right shortcuts walk history`, async ({ dobiusPage }) => {
    const worktreeIds = await getAllWorktreeIds(dobiusPage)
    ...
    await dobiusPage.keyboard.press(`${mod}+Alt+ArrowLeft`)
    await expect
      .poll(async () => getActiveWorktreeId(dobiusPage), {
    ...
    await dobiusPage.keyboard.press(`${mod}+Alt+ArrowRight`)
    await expect
      .poll(async () => getActiveWorktreeId(dobiusPage), {
  ...
  test('shortcut is a no-op in settings view', async ({ dobiusPage }) => {
    const worktreeIds = await getAllWorktreeIds(dobiusPage)
    ...
    await expect
      .poll(async () => dobiusPage.evaluate(() => window.__store!.getState().activeView))
      .toBe('settings')

    const idBefore = await getActiveWorktreeId(dobiusPage)
    await dobiusPage.evaluate(() => document.body.focus())
    await dobiusPage.keyboard.press(`${mod}+Alt+ArrowLeft`)

    // Give any erroneous nav a beat to land, then assert the active worktree
    // and the slice index both stayed put.
    await dobiusPage.waitForTimeout(150)
    expect(await getActiveWorktreeId(dobiusPage)).toBe(idBefore)
    const snapshot = await getNavHistorySnapshot(dobiusPage)
    expect(snapshot.index).toBe(1)
  })
})
