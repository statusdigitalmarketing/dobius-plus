/**
 * E2E tests for the "Create Workspace" flow in Dobius.
 *
...
test.describe('Create Workspace', () => {
  ...
  test('creates a worktree through the composer UI and activates it', async ({ dobiusPage }) => {
    const worktreeIdBefore = await getActiveWorktreeId(dobiusPage)
    ...
    dobiusPage.on('console', onConsole)
    ...
    try {
      ...
      await dobiusPage.getByRole('button', { name: 'New workspace', exact: true }).click()

      const dialog = dobiusPage.getByRole('dialog', { name: /Create (Workspace|Worktree)/i })
      ...
      await expect
        .poll(async () => worktreeExists(dobiusPage, workspaceName), {
          timeout: 10_000,
      ...
      await expect
        .poll(
          async () => {
            const id = await getActiveWorktreeId(dobiusPage)
      ...
      dobiusPage.off('console', onConsole)
  ...
  test('shows a failed workspace entry when worktree creation fails', async ({ dobiusPage }) => {
    ...
    try {
      ...
      const failedWorkspace = dobiusPage.getByRole('button', {
        name: new RegExp(`${workspaceName} No base branch found`)
      })
      await expect(failedWorkspace).toBeVisible()
      await expect(dobiusPage.getByText('Couldn’t create worktree')).toBeVisible()
      await expect(failedWorkspace).toContainText('No base branch found')
      await expect(dobiusPage.getByRole('button', { name: 'Retry' })).toBeVisible()
    } finally {
      await dobiusPage
        .evaluate(() => {
          ;(
  ...
  test('reuses a resolved pasted GitHub URL when quick create submits', async ({
    ...
    const url = 'https://github.com/statusdigitalmarketing/dobius-plus/pull/2049'
    ...
    try {
      ...
      await expect(dialog).toBeHidden({ timeout: 15_000 })
      await expect(dobiusPage.getByRole('option', { name: linkedWorkspacePattern })).toBeVisible({
        timeout: 10_000
      })
      await expect(dobiusPage.getByRole('option', { name: url })).toHaveCount(0)
      await expect(dobiusPage.getByText('Linked PR #2049')).toBeVisible()
      await expect
        .poll(() =>
          electronApp.evaluate(() => {
            const counters = globalThis as unknown as {
  ...
  test('names the workspace after the PR title when the pasted URL suggestion is selected', async ({
    ...
    const url = 'https://github.com/statusdigitalmarketing/dobius-plus/pull/2050'
    const linkedWorkspacePattern = new RegExp(title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))
    ...
    try {
      ...
      const suggestion = dobiusPage.getByRole('option', { name: linkedWorkspacePattern })
      ...
      await expect(dobiusPage.getByRole('option', { name: /https-github/i })).toHaveCount(0)
      await expect(dobiusPage.getByText('Linked PR #2050')).toBeVisible()
    } finally {
      await dobiusPage
        .evaluate(() => {
          window.__store?.getState().closeModal()
