...
test.describe('terminal document visibility WebGL recovery @headful', () => {
  test('clears the WebGL atlas and keeps terminal text painted after document visibility resumes', async ({
    ...
    const webglActive = await forceWebgl(dobiusPage)
    test.skip(!webglActive, 'WebGL was not active in this headful environment')

    await writeStableTerminalContent(dobiusPage)
    expect(await patchAtlasCounter(dobiusPage)).toBe(true)
    expect(await countPatchedWebglAddons(dobiusPage)).toBeGreaterThanOrEqual(2)
    const baseline = await terminalScreenshots(dobiusPage)
    ...
    try {
      ...
      await resetAtlasResetCount(dobiusPage)
      ...
      if (browserWindowVisibilityWorked) {
        await expect
          .poll(() => readAtlasResetCount(dobiusPage), {
            timeout: 2_000,
        ...
        await resetAtlasResetCount(dobiusPage)
        await dispatchDocumentVisibilityCycle(dobiusPage)
        await expect
          .poll(() => readAtlasResetCount(dobiusPage), {
            timeout: 2_000,
      ...
      await waitForTerminalPaint(dobiusPage)

      const afterResume = await terminalScreenshots(dobiusPage)
