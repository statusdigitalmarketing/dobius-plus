...
test.describe('Terminal inactive cursor rendering', () => {
  ...
  test('keeps an unfocused prompt cursor rendered as one block outline', async ({ dobiusPage }) => {
    await splitActiveTerminalPane(dobiusPage, 'vertical')
    await waitForPaneCount(dobiusPage, 2)
    await placeInactiveCursorAtPrompt(dobiusPage)

    const fixedBehavior = await renderInactiveCursor(dobiusPage)
    ...
    const oldBehavior = await renderInactiveCursor(dobiusPage, 'outline')
