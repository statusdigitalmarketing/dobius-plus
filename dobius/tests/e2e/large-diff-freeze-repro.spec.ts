...
async function addAndActivateRepo(dobiusPage: Page, repoPath: string): Promise<string> {
  const repoId = await dobiusPage.evaluate(async (pathToRepo: string) => {
  ...
  await expect
    .poll(
      () =>
        dobiusPage.evaluate(async (targetRepoId: string) => {
          const store = window.__store
          if (!store) {
            return 0
          }
  ...
  const worktreeId = await dobiusPage.evaluate(
...
test.describe('Large diff freeze repro', () => {
  ...
  test('opening a large single-file diff keeps the renderer responsive', async ({ dobiusPage }) => {
    await waitForSessionReady(dobiusPage)
    const fixture = createIsolatedLargeDiffRepo()
    const lineCount = Number(process.env.DOBIUS_LARGE_DIFF_REPRO_LINES ?? '60000')
    if (!Number.isFinite(lineCount) || lineCount < 0) {
      throw new Error(
        `Invalid DOBIUS_LARGE_DIFF_REPRO_LINES: ${process.env.DOBIUS_LARGE_DIFF_REPRO_LINES}`
      )
    }
    ...
    try {
      ...
      const measurement = await dobiusPage.evaluate(
