...
async function createShallowPriorityTruncationFixture(): Promise<{
  ...
  const parentPath = realpathSync(
    await mkdtemp(path.join(os.tmpdir(), 'dobius-e2e-shallow-priority-'))
  )
...
async function createCancellableScanFixture(): Promise<{
  ...
  const parentPath = realpathSync(
    await mkdtemp(path.join(os.tmpdir(), 'dobius-e2e-cancellable-scan-'))
  )
  tempRoots.push(parentPath)
...
test('prioritizes shallow sibling repositories in a bounded nested scan', async ({
  ...
  await dobiusPage
    ...
    .first()
    .click()
  const dialog = dobiusPage.getByRole('dialog', { name: /Add a project/i })
  await expect(dialog).toBeVisible()
  await dialog.getByRole('button', { name: /Browse folder/i }).click()

  const importDialog = dobiusPage.getByRole('dialog', {
  ...
  await expect(dobiusPage.getByText(fixture.groupName)).toBeVisible()
  await expect(dobiusPage.getByText('z-web-client').first()).toBeVisible()
})
