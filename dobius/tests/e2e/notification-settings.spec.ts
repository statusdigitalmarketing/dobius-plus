...
test.describe('Notification settings', () => {
  ...
  test('can be toggled from settings and disables child controls', async ({ dobiusPage }) => {
    await openNotificationSettings(dobiusPage)

    const notificationsSection = dobiusPage.locator('[data-settings-section="notifications"]')
    ...
    await expect
      .poll(async () => (await getSettings(dobiusPage)).notifications.agentTaskComplete, {
        timeout: 5_000,
    ...
    await expect
      .poll(async () => (await getSettings(dobiusPage)).notifications.enabled, {
        timeout: 5_000,
    ...
    await expect
      .poll(async () => {
        const settings = await getSettings(dobiusPage)
        return {
