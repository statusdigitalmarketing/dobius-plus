...
test.describe('Terminal tab switch SIGWINCH restore', () => {
  test('keeps an alternate-screen Codex viewport after hidden snapshot replay', async ({
    ...
    await sendToTerminal(dobiusPage, paneIdentity.ptyId, 'ARM_SIGWINCH_PROBE\n')
    await expect
      .poll(() => getTerminalContent(dobiusPage, 8_000), {
        timeout: 10_000,
    ...
    const armedContentAfterSettle = await getTerminalContent(dobiusPage, 8_000)
    ...
    await expect
      .poll(
        async () => {
          const content = await getTerminalContent(dobiusPage, 8_000)
      ...
      .toBe('snapshot')
    await dobiusPage.waitForTimeout(1_200)
    const contentAfterSettle = await getTerminalContent(dobiusPage, 8_000)
