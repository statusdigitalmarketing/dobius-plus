...
test.describe('Terminal attention', () => {
  ...
  test('background terminal attention marks a tab unread and clears on focus', async ({
    ...
    const secondTabId = await createTerminalTab(dobiusPage)
    await waitForActiveTerminalManager(dobiusPage, 30_000)
    ...
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(dobiusPage)).includes(secondTabId), {
        timeout: 10_000,
        message: 'Background tab did not become unread after BEL'
      })
      .toBe(true)

    const secondTabBell = dobiusPage
      .locator(
  ...
  test('a BEL on the focused tab raises, then clears on click', async ({ dobiusPage }) => {
    await waitForSessionReady(dobiusPage)
    await waitForActiveWorktree(dobiusPage)
    await ensureTerminalVisible(dobiusPage)
    await waitForActiveTerminalManager(dobiusPage, 30_000)

    const activeTabId = await getActiveTabId(dobiusPage)
    if (!activeTabId) {
      throw new Error('Expected an active terminal tab')
    }
    const activePtyId = await waitForActivePanePtyId(dobiusPage)
    await installRendererTitleLog(dobiusPage)
    ...
    expect((await getUnreadTerminalTabIds(dobiusPage)).includes(activeTabId)).toBe(true)
    const activeTabBell = dobiusPage
      .locator(
    ...
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(dobiusPage)).includes(activeTabId), {
        timeout: 5_000,
  ...
  test('a BEL on the focused tab raises, then clears on plain Escape', async ({ dobiusPage }) => {
    await waitForSessionReady(dobiusPage)
    await waitForActiveWorktree(dobiusPage)
    await ensureTerminalVisible(dobiusPage)
    await waitForActiveTerminalManager(dobiusPage, 30_000)
    ...
    const activePaneKey = await getActivePaneKey(dobiusPage, activeTabId)
    const activePtyId = await waitForActivePanePtyId(dobiusPage)
    await installRendererTitleLog(dobiusPage)
    ...
    await expect
      .poll(async () => (await getUnreadTerminalPaneKeys(dobiusPage)).includes(activePaneKey), {
        timeout: 5_000,
    ...
    await focusActiveXterm(dobiusPage, activeTabId)
  ...
  // after restarting Dobius with a Claude Code session open, clicking between
  ...
  test('mode bits replayed into xterm do not leak focus escapes to the shell', async ({
    ...
    try {
      ...
      // shell-startup BELs.
      const emittedFromXterm = await dobiusPage.evaluate(
        () =>
