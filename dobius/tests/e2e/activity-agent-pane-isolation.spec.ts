...
test.describe('Activity Agent Pane Isolation', () => {
  test.beforeEach(async ({ dobiusPage }) => {
    ...
    await enableActivityAgentsView(dobiusPage)
  ...
  test('selecting agent rows isolates the matching split pane by stable leaf id', async ({
    ...
    const [first, second] = await seedActivityThreadsForSplitPanes(dobiusPage, snapshot)

    await agentsSidebarButton(dobiusPage).click()
    await expect(dobiusPage.getByText(first.prompt)).toBeVisible()
    await expect(dobiusPage.getByText(second.prompt)).toBeVisible()

    await dobiusPage.getByRole('button').filter({ hasText: first.prompt }).first().click()
    await expect
      .poll(async () => readActivityPaneVisibility(dobiusPage), {
        timeout: 10_000,
    ...
    await dobiusPage.getByRole('button').filter({ hasText: second.prompt }).first().click()
    await expect
      .poll(async () => readActivityPaneVisibility(dobiusPage), {
        timeout: 10_000,
  ...
  test('acknowledged stable pane keys clear the Agents unread badge', async ({ dobiusPage }) => {
    await splitActiveTerminalPane(dobiusPage, 'vertical')
    await waitForPaneCount(dobiusPage, 2)
    const snapshot = await waitForPaneIdentitySnapshot(dobiusPage, 2)
    ...
    await expect(agentsSidebarButton(dobiusPage)).toHaveAccessibleName(/^Agents\s+1$/)

    await dobiusPage.evaluate((paneKey) => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
      }
    ...
    await expect(agentsSidebarButton(dobiusPage)).toHaveAccessibleName(/^Agents$/)
    await expect(dobiusPage.getByRole('button', { name: /^Agents\s+1$/ })).toHaveCount(0)
  })

  test('workspace card agent rows focus the matching terminal split pane', async ({ dobiusPage }) => {
    await splitActiveTerminalPane(dobiusPage, 'vertical')
    await waitForPaneCount(dobiusPage, 2)
    const snapshot = await waitForPaneIdentitySnapshot(dobiusPage, 2)
    const [first, second] = await seedActivityThreadsForSplitPanes(dobiusPage, snapshot)

    await enableInlineAgentCards(dobiusPage)

    await clickWorkspaceCardAgentRow(dobiusPage, first.prompt)
    await expect
      .poll(async () => readActivePaneSelection(dobiusPage), {
        timeout: 10_000,
    ...
    await clickWorkspaceCardAgentRow(dobiusPage, second.prompt)
    await expect
      .poll(async () => readActivePaneSelection(dobiusPage), {
        timeout: 10_000,
  ...
  test('workspace card agent rows reveal terminal logs from a non-terminal surface', async ({
    ...
    await waitForPaneCount(dobiusPage, 2)
    const snapshot = await waitForPaneIdentitySnapshot(dobiusPage, 2)
    const [first] = await seedActivityThreadsForSplitPanes(dobiusPage, snapshot)

    await enableInlineAgentCards(dobiusPage)
    await expect(terminalPaneForLeaf(dobiusPage, first.leafId)).toBeVisible()
    ...
    await expect(await clickFileInExplorer(dobiusPage, ['README.md'])).toBe('README.md')
    await expect
      .poll(() => readActivePaneSelection(dobiusPage))
      .toMatchObject({
        activeTabType: 'editor',
        activeTabId: snapshot.tabId
      })
    await expect(terminalPaneForLeaf(dobiusPage, first.leafId)).toBeHidden()
  ...
  test('workspace card agent rows focus the matching split-group terminal pane', async ({
    ...
    await splitActiveTerminalPane(dobiusPage, 'vertical')
    await waitForPaneCount(dobiusPage, 2)
    const firstGroupSnapshot = await waitForPaneIdentitySnapshot(dobiusPage, 2)
    const [first, second] = await seedActivityThreadsForSplitPanes(dobiusPage, firstGroupSnapshot)

    const splitGroup = await createTerminalInNewSplitGroup(dobiusPage)
    await waitForActiveTerminalManager(dobiusPage, 30_000)
    await waitForPaneCount(dobiusPage, 1, 30_000)
    const secondGroupSnapshot = await waitForPaneIdentitySnapshot(dobiusPage, 1)
    ...
    await clickWorkspaceCardAgentRow(dobiusPage, splitGroupThread.prompt)
    await expect
      .poll(async () => readActivePaneSelection(dobiusPage), {
        timeout: 10_000,
