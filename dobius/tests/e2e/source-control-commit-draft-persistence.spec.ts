...
test.describe('Source Control commit draft persistence', () => {
  test('preserves a typed draft when the sidebar tab remounts', async ({
    ...
    try {
      ...
      await openSourceControlForWorktree(dobiusPage, testRepoPath, firstWorktree.worktreePath)

      const textarea = dobiusPage.getByRole('textbox', { name: 'Commit message' })
      ...
      await expect
        .poll(
          async () => dobiusPage.evaluate(() => window.__store?.getState().rightSidebarTab ?? null),
          { timeout: 5_000 }
        )
        .toBe('explorer')
      ...
      await openSourceControlForWorktree(dobiusPage, testRepoPath, secondWorktree.worktreePath)
      await expect(textarea).toBeVisible({ timeout: 10_000 })
