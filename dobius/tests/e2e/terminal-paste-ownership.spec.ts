...
async function installClipboardReadTerminalBlurRepro(app: ElectronApplication): Promise<void> {
  await app.evaluate(({ BrowserWindow, ipcMain }) => {
    ...
    const global = globalThis as unknown as {
      __dobiusOriginalClipboardReadTextHandler?: ClipboardReadHandler
    }
    ...
    if (!invokeHandlers || !handler || global.__dobiusOriginalClipboardReadTextHandler) {
      return
    }
    global.__dobiusOriginalClipboardReadTextHandler = handler
    invokeHandlers.set('clipboard:readText', async (event, ...args) => {
      ...
      return global.__dobiusOriginalClipboardReadTextHandler!(event, ...args)
    })
  })
}
...
async function restoreClipboardReadTerminalBlurRepro(app: ElectronApplication): Promise<void> {
  await app.evaluate(({ ipcMain }) => {
    ...
    if (invokeHandlers && global.__dobiusOriginalClipboardReadTextHandler) {
      invokeHandlers.set('clipboard:readText', global.__dobiusOriginalClipboardReadTextHandler)
      delete global.__dobiusOriginalClipboardReadTextHandler
    }
  })
}
...
test.describe('terminal paste ownership', () => {
  test('keyboard paste shortcuts send clipboard text to the focused terminal exactly once', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.dobius-paste-ownership-${runId}.mjs`)
    ...
    try {
      ...
      for (const [index, chord] of keyboardPasteChords().entries()) {
        const payload = `DOBIUS_E2E_PASTE_${runId}_${index}`
        ...
        await dobiusPage.keyboard.press(chord)
  ...
  test('keyboard paste survives transient terminal blur during clipboard read', async ({
    ...
    const ptyId = await waitForActivePanePtyId(dobiusPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.dobius-paste-blur-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false
    ...
    try {
      ...
      const payload = `DOBIUS_E2E_TRANSIENT_BLUR_PASTE_${runId}`
      const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
      await clearTerminalPtyWriteLog(electronApp)
      await dobiusPage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(dobiusPage)
      ...
      await dobiusPage.keyboard.press(keyboardPasteChords()[0])
      await waitForTerminalOutput(dobiusPage, encodedPayload, 10_000, 12_000)
  ...
  test('terminal context-menu Paste sends clipboard text exactly once', async ({
    ...
    const ptyId = await waitForActivePanePtyId(dobiusPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.dobius-paste-context-menu-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false
    ...
    try {
      ...
      const payload = `DOBIUS_E2E_CONTEXT_MENU_PASTE_${runId}`
      const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
      await clearTerminalPtyWriteLog(electronApp)
      await dobiusPage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(dobiusPage)
      ...
      await dobiusPage.getByRole('menuitem', { name: /Paste/ }).click()
      await waitForTerminalOutput(dobiusPage, encodedPayload, 10_000, 12_000)
  ...
  test('Windows multiline keyboard paste preserves terminal content with one PTY owner', async ({
    ...
    const ptyId = await waitForActivePanePtyId(dobiusPage)
    const runId = randomUUID()
    const sentinel = `DOBIUS_E2E_MULTILINE_DONE_${runId}`
    const payload = [
      `DOBIUS_E2E_MULTILINE_${runId}`,
    ...
    const scriptPath = path.join(testRepoPath, `.dobius-paste-multiline-${runId}.mjs`)
  ...
  test('Windows right-click paste sends clipboard text to the focused terminal exactly once', async ({
    ...
    const ptyId = await waitForActivePanePtyId(dobiusPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.dobius-paste-right-click-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false
    ...
    try {
      ...
      const payload = `DOBIUS_E2E_RIGHT_CLICK_PASTE_${runId}`
      const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
      await clearTerminalPtyWriteLog(electronApp)
      await dobiusPage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(dobiusPage)

      await rightClickActiveTerminalSurface(dobiusPage)
      await waitForTerminalOutput(dobiusPage, encodedPayload, 10_000, 12_000)
