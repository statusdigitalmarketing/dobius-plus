 ...
 * - PR #677 (`return to Dobius landing screen after closing last terminal`)
...
test.describe('Tab Close Navigation', () => {
   ...
   */
  test('closing the active editor tab activates its visual neighbor', async ({ dobiusPage }) => {
    const worktreeId = await waitForActiveWorktree(dobiusPage)

    const fileIds = await openSeededEditorTabs(dobiusPage, worktreeId, [
      'package.json',
      'README.md',
      'tsconfig.json'
    ])
    ...
    await setActiveFile(dobiusPage, fileIds[1])
    await expect.poll(async () => getActiveFileId(dobiusPage), { timeout: 3_000 }).toBe(fileIds[1])

    await closeFile(dobiusPage, fileIds[1])
    ...
    await expect
      .poll(async () => getActiveFileId(dobiusPage), {
        timeout: 5_000,
   ...
   */
  test('closing the active diff tab activates a still-open neighbor', async ({ dobiusPage }) => {
    const worktreeId = await waitForActiveWorktree(dobiusPage)

    // Seed two editor tabs + one diff tab in the same worktree.
    const editorIds = await openSeededEditorTabs(dobiusPage, worktreeId, [
      'package.json',
      'README.md'
    ])
    expect(editorIds.length).toBe(2)

    const diffId = await dobiusPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
        return null
      }
    ...
    await expect.poll(async () => getActiveFileId(dobiusPage), { timeout: 3_000 }).toBe(diffId)

    await closeFile(dobiusPage, diffId!)
   ...
   */
  test('closing the last visible surface returns the app to Landing', async ({ dobiusPage }) => {
    const worktreeId = await waitForActiveWorktree(dobiusPage)
    ...
    const editorIds = await openSeededEditorTabs(dobiusPage, worktreeId, ['package.json'])
    expect(editorIds.length).toBe(1)

    await setActiveFile(dobiusPage, editorIds[0])
    await expect.poll(async () => getActiveFileId(dobiusPage), { timeout: 3_000 }).toBe(editorIds[0])
    ...
    await closeFile(dobiusPage, editorIds[0])
