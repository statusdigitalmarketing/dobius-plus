...
test.describe('Windows project runtime smoke', () => {
  test('keeps a Windows-host project and WSL project available side by side', async ({
    ...
    const wsl = await dobiusPage.evaluate(async () => ({
    ...
    const wslRepoPath = await createGitRepo('dobius-e2e-project-runtime-', 'wsl-runtime-project')

    const smoke = await dobiusPage.evaluate(
    ...
    await openRepoSettings(dobiusPage, smoke.hostRepoId)
    const hostSection = dobiusPage.locator(`[data-settings-section="repo-${smoke.hostRepoId}"]`)
    ...
    await chooseProjectRuntime(dobiusPage, smoke.hostRepoId, 'WSL')
    await expect(
      hostSection.getByText(`This project runs in ${smoke.wslDistro} via WSL.`)
    ).toBeVisible()
    const hostAfterWslUiSwitch = await dobiusPage.evaluate((hostRepoId) => {
      const state = window.__store!.getState()
      const hostProject = state.projects.find((project) =>
    ...
    await chooseProjectRuntime(dobiusPage, smoke.hostRepoId, 'Windows')
    await expect(hostSection.getByText('This project runs on Windows.')).toBeVisible()
    const hostAfterWindowsUiSwitch = await dobiusPage.evaluate((hostRepoId) => {
      const state = window.__store!.getState()
      const hostProject = state.projects.find((project) =>
        project.sourceRepoIds.includes(hostRepoId)
      )
    ...
    await openRepoSettings(dobiusPage, smoke.wslRepoId)
    const wslSection = dobiusPage.locator(`[data-settings-section="repo-${smoke.wslRepoId}"]`)
    ...
    await dobiusPage.evaluate(async (repoId) => {
