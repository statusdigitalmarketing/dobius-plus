...
const RUN_LOCALHOST_SSH = process.env.DOBIUS_E2E_SSH_LOCALHOST === '1'
const RUN_REMOTE_HOOKS =
  process.env.DOBIUS_FEATURE_REMOTE_AGENT_HOOKS === undefined ||
  (process.env.DOBIUS_FEATURE_REMOTE_AGENT_HOOKS.trim() !== '' &&
    process.env.DOBIUS_FEATURE_REMOTE_AGENT_HOOKS.trim() !== '0')
...
function parsePort(value: string | undefined): number {
  ...
  throw new Error(`Invalid DOBIUS_E2E_SSH_PORT: ${value}`)
}

function currentUsername(): string {
  return (
    process.env.DOBIUS_E2E_SSH_USER ??
    process.env.USER ??
...
function readLocalhostSshTarget(): LocalhostSshTarget {
  const configHost = process.env.DOBIUS_E2E_SSH_CONFIG_HOST?.trim()
  const host = process.env.DOBIUS_E2E_SSH_HOST?.trim() ?? (configHost ? '' : '127.0.0.1')
  const identityFile = process.env.DOBIUS_E2E_SSH_IDENTITY_FILE?.trim()

  return {
    label: `Localhost SSH E2E ${Date.now()}`,
    host,
    port: parsePort(process.env.DOBIUS_E2E_SSH_PORT),
...
function marker(name: string): string {
  return `__DOBIUS_${name}_${Date.now()}__`
}
...
async function postCodexHook(
  ...
  await execInTerminal(
    ...
    [
      ...
      '  echo __DOBIUS_AGENT_HOOK_ENV_MISSING__',
      'else',
      `  hook_payload=${shellQuote(JSON.stringify(payload))}`,
      '  (',
      '    sleep 0.1',
      '    if curl -sS -X POST "http://127.0.0.1:${DOBIUS_AGENT_HOOK_PORT}/hook/codex" \\',
...
test.describe('Localhost SSH', () => {
  test.skip(
    !RUN_LOCALHOST_SSH,
    'Set DOBIUS_E2E_SSH_LOCALHOST=1 to run this local-machine-only SSH E2E test.'
  )
  test.skip(
    !RUN_REMOTE_HOOKS,
    'Unset DOBIUS_FEATURE_REMOTE_AGENT_HOOKS or set it to 1 so remote PTYs keep pane identity and forward hook events.'
  )
  ...
  test('routes a terminal and agent-hook status over localhost SSH', async ({
    ...
    const remote = await dobiusPage.evaluate(
    ...
    await ensureTerminalVisible(dobiusPage, 30_000)
    await waitForActiveTerminalManager(dobiusPage, 45_000)
    const ptyId = await waitForActivePanePtyId(dobiusPage, 45_000)
    const paneKey = await dobiusPage.evaluate(() => {
      const store = window.__store
      if (!store) {
        throw new Error('Store unavailable')
      }
    ...
    await execInTerminal(dobiusPage, ptyId, emitMarkerCommand(terminalMarker))
    await waitForTerminalOutput(dobiusPage, terminalMarker, 20_000)
    ...
    await execInTerminal(
      ...
      ptyId,
      [
        `if [ "$DOBIUS_PANE_KEY" = ${shellQuote(paneKey)} ] && [ -n "$DOBIUS_AGENT_HOOK_PORT" ] && [ -n "$DOBIUS_AGENT_HOOK_TOKEN" ] && /bin/sh -c 'test -n "$DOBIUS_PANE_KEY" && test -n "$DOBIUS_AGENT_HOOK_PORT" && test -n "$DOBIUS_AGENT_HOOK_TOKEN"'; then`,
        `  ${emitMarkerCommand(envMarker)}`,
        'else',
        '  token_state=${DOBIUS_AGENT_HOOK_TOKEN:+set}',
        `  printf '%s pane=%s port=%s token=%s endpoint=%s\\n' ${shellQuote(envFailedMarker)} "$DOBIUS_PANE_KEY" "$DOBIUS_AGENT_HOOK_PORT" "$token_state" "$DOBIUS_AGENT_HOOK_ENDPOINT"`,
        'fi'
      ].join('\n')
    )
    await waitForTerminalOutput(dobiusPage, envMarker, 20_000)
    ...
    await execInTerminal(
      ...
      ptyId,
      [
        'opencode_status_file="$OPENCODE_CONFIG_DIR/plugins/dobius-opencode-status.js"',
        'pi_status_file="$HOME/.pi/agent/extensions/dobius-agent-status.ts"',
    ...
    await waitForTerminalOutput(dobiusPage, pluginOverlayMarker, 20_000)

    const prompt = `dobius ssh e2e prompt ${Date.now()}`
    await postCodexHook(
      dobiusPage,
      ptyId,
    ...
    const ctrlPrompt = `dobius ssh ctrl-c interrupt ${Date.now()}`
    await postCodexHook(
      dobiusPage,
      ptyId,
    ...
    await focusTerminal(dobiusPage)
    await dobiusPage.keyboard.press('Control+C')
    await dobiusPage.waitForTimeout(750)
    expect(
      await dobiusPage.evaluate(
        ({ paneKey, prompt, targetId, worktreeId }) => {
          const state = window.__store?.getState()
    ...
    const escapePrompt = `dobius ssh escape interrupt ${Date.now()}`
    await postCodexHook(
      dobiusPage,
      ptyId,
