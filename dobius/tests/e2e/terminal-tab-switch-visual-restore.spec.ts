...
test.describe('Terminal tab switch visual restore', () => {
  ...
  test('keeps full-width geometry after switching away and back', async ({
    ...
    const { firstTabId, secondTabId } = await ensureTwoTerminalTabs(dobiusPage)
    await forceWebglOnActiveTab(dobiusPage)

    const runId = `${Date.now()}`
    const marker = `${TAB_SWITCH_MARKER_PREFIX}_${runId}`
    const firstPtyId = await waitForPanePtyIdOnTab(dobiusPage, firstTabId)
    await writeStaticTabContent(dobiusPage, firstTabId, marker, TAB_A_GLYPH_ROW)

    const baseline = await readTabTerminalGeometry(dobiusPage, firstTabId, runId)
    ...
    await resetTerminalOutputSchedulerDebug(dobiusPage)
    await startHiddenPtyOutputBurst(dobiusPage, firstPtyId, runId)

    for (let cycle = 0; cycle < 12; cycle += 1) {
      await activateTerminalTab(dobiusPage, secondTabId)
      await injectHiddenStreamingBurst(dobiusPage, firstTabId, runId)
      // Why: rapid back-to-back switches mirror the user's leave/return pattern
      // and race the overlay's rAF/50ms refit retries.
      await activateTerminalTab(dobiusPage, firstTabId)
      if (cycle % 3 === 0) {
        await activateTerminalTab(dobiusPage, secondTabId)
        await activateTerminalTab(dobiusPage, firstTabId)
      }
      ...
      const immediate = await readTabTerminalGeometry(dobiusPage, firstTabId, runId)
      ...
      await dobiusPage.waitForTimeout(60)
      const settled = await readTabTerminalGeometry(dobiusPage, firstTabId, runId)
    ...
    const schedulerActivity = await waitForHiddenOutputSchedulerActivity(dobiusPage)
  ...
  test('keeps geometry after hidden alt-screen TUI redraws during tab switches', async ({
    ...
    for (let cycle = 0; cycle < 6; cycle += 1) {
      ...
      const geometry = await readTabTerminalGeometry(dobiusPage, firstTabId, `${runId}_ALT`)
  ...
  test('restores skipped hidden agent output on light tab resume', async ({ dobiusPage }) => {
    await waitForSessionReady(dobiusPage)
    await waitForActiveWorktree(dobiusPage)
    await ensureTerminalVisible(dobiusPage)
    await waitForActiveTerminalManager(dobiusPage, 30_000)

    const shellTabId = (await getActiveTabId(dobiusPage))!
    const agentTabId = await createCodexMarkedTerminalTab(dobiusPage)
    await waitForActiveTerminalManager(dobiusPage, 30_000)
    await waitForPanePtyIdOnTab(dobiusPage, agentTabId)
    const paneIdentity = await readPaneIdentityOnTab(dobiusPage, agentTabId)
    const paneKey = `${agentTabId}:${paneIdentity.leafId}`

    await activateTerminalTab(dobiusPage, shellTabId)
    const runId = `${Date.now()}`
    ...
    await resetHiddenOutputDebug(dobiusPage)
    await injectPaneData(dobiusPage, paneKey, hiddenFrame, {
      seq: hiddenFrame.length,
      rawLength: hiddenFrame.length
    })

    await expect
      .poll(async () => (await readHiddenOutputDebug(dobiusPage))?.hiddenRendererSkipCount ?? 0, {
        timeout: 5_000,
        message: 'Codex-marked hidden output did not take the skipped renderer path'
      })
      .toBeGreaterThan(0)
    await setHiddenSnapshotOverride(dobiusPage, paneIdentity.ptyId, {
    ...
    await activateTerminalTab(dobiusPage, agentTabId)

    await expect
      .poll(() => getTerminalContent(dobiusPage, 8_000), {
        timeout: 10_000,
  ...
  test('restores skipped hidden Grok output on light tab resume', async ({ dobiusPage }) => {
    await waitForSessionReady(dobiusPage)
    await waitForActiveWorktree(dobiusPage)
    await ensureTerminalVisible(dobiusPage)
    await waitForActiveTerminalManager(dobiusPage, 30_000)
    ...
    const shellTabId = (await getActiveTabId(dobiusPage))!
    const grokTabId = await createGrokMarkedTerminalTab(dobiusPage)
    await waitForActiveTerminalManager(dobiusPage, 30_000)
    await waitForPanePtyIdOnTab(dobiusPage, grokTabId)
    const paneIdentity = await readPaneIdentityOnTab(dobiusPage, grokTabId)
    ...
    await activateTerminalTab(dobiusPage, grokTabId)
  ...
  test('keeps returned tab glyphs intact across tab switches', async ({ dobiusPage }, testInfo) => {
    ...
    await forceWebglOnActiveTab(dobiusPage)
    await activateTerminalTab(dobiusPage, firstTabId)
    const firstWebgl = await waitForWebglOnTab(dobiusPage, firstTabId)
    await activateTerminalTab(dobiusPage, secondTabId)
    await dobiusPage.evaluate((id) => {
      window.__paneManagers?.get(id)?.setTerminalGpuAcceleration?.('on')
    }, secondTabId)
    const secondWebgl = await waitForWebglOnTab(dobiusPage, secondTabId)
    ...
    const firstPtyId = await waitForPanePtyIdOnTab(dobiusPage, firstTabId)
    const secondPtyId = await waitForPanePtyIdOnTab(dobiusPage, secondTabId)
    await sendToTerminal(dobiusPage, firstPtyId, SILENT_FOREGROUND_COMMAND)
    await sendToTerminal(dobiusPage, secondPtyId, SILENT_FOREGROUND_COMMAND)
    ...
    await writeStaticTabContent(dobiusPage, firstTabId, markerA, TAB_A_GLYPH_ROW)
    await activateTerminalTab(dobiusPage, secondTabId)
    await writeStaticTabContent(dobiusPage, secondTabId, markerB, TAB_B_GLYPH_ROW)

    await activateTerminalTab(dobiusPage, firstTabId)
    await resetAtlasOnTab(dobiusPage, firstTabId)
    await dobiusPage.waitForTimeout(800)
    const baseline = await captureStableTabScreenshot(dobiusPage, firstTabId)
    ...
    for (let cycle = 0; cycle < 8; cycle += 1) {
      ...
      const afterReturn = await captureStableTabScreenshot(dobiusPage, firstTabId)
