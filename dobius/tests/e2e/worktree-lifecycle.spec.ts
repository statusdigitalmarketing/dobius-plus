...
test.describe('Worktree Lifecycle', () => {
  ...
  test('removing a worktree clears its tabs, open files, and browser tabs', async ({
    ...
    const originalWorktreeId = await waitForActiveWorktree(dobiusPage)

    createdWorktreeId = await createIsolatedWorktree(dobiusPage)
    const newWorktreeId = createdWorktreeId
    await switchToWorktree(dobiusPage, newWorktreeId)
    await expect
    ...
    await dobiusPage.evaluate((worktreeId) => {
      const store = window.__store
      if (!store) {
        return
      }
    ...
    await openFileExplorer(dobiusPage)
    await clickFileInExplorer(dobiusPage, ['README.md', 'package.json'])

    // Baseline: the new worktree now has tabs/browser tabs/open files.
    expect((await getWorktreeTabs(dobiusPage, newWorktreeId)).length).toBeGreaterThan(0)
    expect((await getBrowserTabs(dobiusPage, newWorktreeId)).length).toBeGreaterThan(0)
    expect((await getOpenFiles(dobiusPage, newWorktreeId)).length).toBeGreaterThan(0)
    ...
    // is protecting.
    await switchToWorktree(dobiusPage, originalWorktreeId)
    await expect
      .poll(async () => getActiveWorktreeId(dobiusPage), { timeout: 10_000 })
      .toBe(originalWorktreeId)

    const result = await removeWorktreeViaStore(dobiusPage, newWorktreeId)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(dobiusPage, newWorktreeId)).length, {
        timeout: 10_000,
        message: 'tabsByWorktree still holds entries for the removed worktree'
      })
      .toBe(0)
    await expect
      .poll(async () => (await getBrowserTabs(dobiusPage, newWorktreeId)).length, { timeout: 5_000 })
      .toBe(0)
    await expect
      .poll(async () => (await getOpenFiles(dobiusPage, newWorktreeId)).length, { timeout: 5_000 })
      .toBe(0)

    const allIds = await getAllWorktreeIds(dobiusPage)
  ...
  test('switching worktrees preserves per-worktree state across a round-trip', async ({
    ...
    const originalState = await dobiusPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
    ...
    const afterRoundTrip = await dobiusPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
   ...
   */
  test('terminal tabs stay scoped to the worktree that created them', async ({ dobiusPage }) => {
    const allIds = await getAllWorktreeIds(dobiusPage)
    expect(
      allIds.length,
      'fixture should provide primary + e2e-secondary worktrees'
    ...
    const worktreeA = await waitForActiveWorktree(dobiusPage)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(dobiusPage, worktreeA)).length, { timeout: 5_000 })
      .toBeGreaterThanOrEqual(2)

    // Switch to B and create a tab there too.
    await switchToWorktree(dobiusPage, worktreeB)
    await expect
      .poll(async () => getActiveWorktreeId(dobiusPage), { timeout: 10_000 })
      .toBe(worktreeB)
    await ensureTerminalVisible(dobiusPage)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(dobiusPage, worktreeB)).length, { timeout: 5_000 })
      .toBeGreaterThanOrEqual(2)

    const tabsA = await getWorktreeTabs(dobiusPage, worktreeA)
    const tabsB = await getWorktreeTabs(dobiusPage, worktreeB)
