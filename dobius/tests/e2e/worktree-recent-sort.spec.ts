...
test.describe('Worktree Recent Sort', () => {
  ...
  function createFolderFixture(): string {
    const dir = mkdtempSync(path.join(os.tmpdir(), 'dobius-e2e-folder-'))
  ...
  test.afterEach(() => {
    ...
    // matches the mkdtempSync/rmSync pairing used in helpers/dobius-app.ts
    // and helpers/dobius-restart.ts.
  ...
  test('stamps lastActivityAt on a newly-added folder repo so it sorts to the top of Recent', async ({
    ...
    const repoId = await addFolderRepo(dobiusPage, folderPath)
    const lastActivityAt = await readFolderWorktreeLastActivity(dobiusPage, repoId)
  ...
  test('leaves lastActivityAt stable across repeated list refreshes', async ({ dobiusPage }) => {
    ...
    const first = await readFolderWorktreeLastActivity(dobiusPage, repoId)

    await dobiusPage.evaluate(async (id) => {
      await window.__store?.getState().fetchWorktrees(id)
      await window.__store?.getState().fetchWorktrees(id)
    }, repoId)

    const second = await readFolderWorktreeLastActivity(dobiusPage, repoId)
