...
test.describe('Terminal output scheduler', () => {
  test('background tab output bursts use the shared drain while the active tab renders', async ({
    ...
    const ptyIdsByTabId: Record<string, string> = {
      [firstTabId]: await waitForTabPtyId(dobiusPage, firstTabId)
    }

    while (tabIds.length < TAB_COUNT) {
      const tabId = await createTerminalTab(dobiusPage)
      await waitForActiveTerminalManager(dobiusPage, 30_000)
      tabIds.push(tabId)
      ptyIdsByTabId[tabId] = await waitForTabPtyId(dobiusPage, tabId)
    }

    await tabLocator(dobiusPage, firstTabId).click()
    await expect
    ...
    await resetSchedulerDebug(dobiusPage)
    ...
    await sendPtyCommands(dobiusPage, [
      {
    ...
    await expect
      .poll(async () => (await getTerminalContent(dobiusPage)).includes(foregroundMarker), {
        timeout: 5_000,
    ...
    await tabLocator(dobiusPage, firstBackgroundTabId).click()
    await expect
      .poll(() => getDomActiveTabId(dobiusPage), {
        timeout: 5_000,
    ...
    await expect
      .poll(async () => (await getTerminalContent(dobiusPage)).includes(firstBackground.marker), {
        timeout: 5_000,
  ...
  test('visible bulk output uses the high-priority drain instead of synchronous xterm writes', async ({
    ...
    const activeTabId = await createTerminalTab(dobiusPage)
    if (!activeTabId) {
      throw new Error('Expected a fresh terminal tab')
    }
    const ptyId = await waitForTabPtyId(dobiusPage, activeTabId)
    await resetSchedulerDebug(dobiusPage)
    ...
    await sendPtyCommands(dobiusPage, [{ ptyId, command: floodCommand }])

    await expect
      .poll(async () => (await getTerminalContent(dobiusPage, 12_000)).includes(marker), {
        timeout: 30_000,
  ...
  test('hidden overflow restores from main-owned terminal state when the tab becomes visible', async ({
    ...
    const foregroundTabId = await getActiveTabId(dobiusPage)
    if (!foregroundTabId) {
      throw new Error('Expected an initial terminal tab')
    }
    const hiddenTabId = await createTerminalTab(dobiusPage)
    await waitForActiveTerminalManager(dobiusPage, 30_000)
    const hiddenPtyId = await waitForTabPtyId(dobiusPage, hiddenTabId)

    await tabLocator(dobiusPage, foregroundTabId).click()
    await expect
      .poll(() => getDomActiveTabId(dobiusPage), {
        timeout: 5_000,
    ...
    await sendPtyCommands(dobiusPage, [{ ptyId: hiddenPtyId, command: floodCommand }])

    await expect
      .poll(async () => mainSnapshotContains(dobiusPage, hiddenPtyId, marker), {
        timeout: 30_000,
        message: 'Main-owned terminal snapshot did not capture the hidden flood marker'
      })
      .toBe(true)

    await tabLocator(dobiusPage, hiddenTabId).click()
    await expect
      .poll(() => getDomActiveTabId(dobiusPage), {
        timeout: 5_000,
    ...
    expect(await getTerminalContent(dobiusPage)).not.toContain('Dobius skipped hidden terminal output')
  })
})
