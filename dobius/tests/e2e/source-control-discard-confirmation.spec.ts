...
async function seedUntrackedFile(page: Page): Promise<SeededUntrackedFile> {
  return page.evaluate(async () => {
    ...
    const fileName = `dobius-discard-confirm-${Date.now()}.txt`
...
test.describe('Source Control discard confirmation', () => {
  ...
  test('deletes an untracked file without confirmation', async ({ dobiusPage }) => {
    const seededFile = await seedUntrackedFile(dobiusPage)
    await openSourceControl(dobiusPage)

    const row = dobiusPage
      .locator('[data-testid="source-control-entry"]')
      .filter({ hasText: seededFile.fileName })
    await expect(row).toBeVisible()

    await deleteUntrackedFileFromRow(row)
    await confirmPendingDelete(dobiusPage)

    await expect(
      dobiusPage.getByRole('dialog', { name: `Delete "${seededFile.fileName}"?` })
    ).toHaveCount(0)
    await expect(row).toHaveCount(0, { timeout: 10_000 })

    await refreshGitStatus(dobiusPage)
    await expect(
      dobiusPage.locator('[data-testid="source-control-entry"]').filter({
