...
const TOGGLE_EVENT = 'dobius-toggle-floating-terminal'
...
test.describe('floating workspace reopen WebGL recovery @headful', () => {
  test('reopening the floating workspace recovers a corrupted glyph atlas', async ({
    ...
    const shots = await setUpCorruptedFloatingTerminal(dobiusPage, 'REOPEN')
    ...
    expect(await instrumentRecoveryCounters(dobiusPage)).toBe(true)

    await toggleFloatingPanel(dobiusPage, false)
    ...
    const webglAttachedWhileClosed = await dobiusPage.evaluate((worktreeId) => {
      const state = window.__store?.getState()
      const tab = (state?.tabsByWorktree?.[worktreeId] ?? [])[0]
      const manager = tab ? window.__paneManagers?.get(tab.id) : null
      const diagnostics = manager?.getRenderingDiagnostics?.() ?? []
    ...
    await toggleFloatingPanel(dobiusPage, true)
    await settleRecoveryWindows(dobiusPage)

    const counters = await readRecoveryCounters(dobiusPage)
    const afterReopen = await screenshotFloatingTerminal(dobiusPage)
  ...
  test('window focus regain recovers the corrupted atlas (harness control)', async ({
    ...
    const shots = await setUpCorruptedFloatingTerminal(dobiusPage, 'CONTROL')
    test.skip(!shots, 'WebGL was not active or atlas corruption could not be injected')
    const { baseline, corrupted } = shots!
    expect(corrupted.equals(baseline)).toBe(false)
    ...
    const afterFocus = await screenshotFloatingTerminal(dobiusPage)
