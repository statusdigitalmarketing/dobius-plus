...
test.describe('Tab Rename (Inline)', () => {
  ...
  test('double-clicking a tab opens an inline rename input and Enter commits', async ({
    ...
    const tabLocator = tabLocatorByTitle(dobiusPage, originalTitle)
    ...
    await expect
      .poll(async () => getActiveCustomTitle(dobiusPage, worktreeId), { timeout: 3_000 })
      .toBe('My Custom Title')
    await expect(renameInput).toBeHidden()
    await expect(tabLocatorByTitle(dobiusPage, 'My Custom Title')).toBeVisible()
  })
  ...
  test('context-menu Change Title opens a focused select-all rename input', async ({
    ...
    await tabLocatorByTitle(dobiusPage, originalTitle).click({ button: 'right' })
    await dobiusPage.getByRole('menuitem', { name: 'Change Title', exact: true }).click()
    ...
    await expect(tabLocatorByTitle(dobiusPage, 'Context Menu Title')).toBeVisible()
  })

  test('Escape during inline rename discards the edit', async ({ dobiusPage }) => {
    const worktreeId = (await getActiveWorktreeId(dobiusPage))!
    const originalTitle = await getActiveTabTitle(dobiusPage, worktreeId)
  ...
  test('renaming to an empty string resets the tab to its default title', async ({ dobiusPage }) => {
    const worktreeId = (await getActiveWorktreeId(dobiusPage))!
    ...
    const defaultTitle = await getActiveTabTitle(dobiusPage, worktreeId)
    ...
    const tabLocator = tabLocatorByTitle(dobiusPage, 'Seeded Custom')
    await tabLocator.dblclick()
    ...
    await expect(tabLocatorByTitle(dobiusPage, defaultTitle)).toBeVisible()
    await expect
      .poll(async () => getActiveCustomTitle(dobiusPage, worktreeId), { timeout: 3_000 })
      .toBe(null)
  })

  test('clicking away (blur) commits the rename', async ({ dobiusPage }) => {
    const worktreeId = (await getActiveWorktreeId(dobiusPage))!
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(dobiusPage, worktreeId)).length, { timeout: 3_000 })
      .toBeGreaterThanOrEqual(2)

    const tabs = await getWorktreeTabs(dobiusPage, worktreeId)
    const activeId = await getActiveTabId(dobiusPage)
    const activeTab = tabs.find((t) => t.id === activeId)!
    const otherTab = tabs.find((t) => t.id !== activeId)!

    const tabLocator = tabLocatorByTitle(dobiusPage, activeTab.title!)
    await tabLocator.dblclick()
    ...
    await tabLocatorByTitle(dobiusPage, otherTab.title!).click()

    await expect(renameInput).toBeHidden()
    await expect(tabLocatorByTitle(dobiusPage, 'Committed By Blur')).toBeVisible()
    expect(
      await dobiusPage.evaluate(
  ...
  test('rename input stays at a usable width when many tabs are open', async ({ dobiusPage }) => {
    const worktreeId = (await getActiveWorktreeId(dobiusPage))!
    const targetTabId = await getActiveTabId(dobiusPage)
    ...
    const tabLocator = tabLocatorByTitle(dobiusPage, targetTitle)
  ...
  test('middle-clicking inside the rename input does not close the tab', async ({ dobiusPage }) => {
    const worktreeId = (await getActiveWorktreeId(dobiusPage))!
    const tabsBefore = (await getWorktreeTabs(dobiusPage, worktreeId)).length
    const originalTitle = await getActiveTabTitle(dobiusPage, worktreeId)
    ...
    await expect(renameInput).toBeVisible()
    await expect(tabLocatorByTitle(dobiusPage, originalTitle)).toBeVisible()
    expect((await getWorktreeTabs(dobiusPage, worktreeId)).length).toBe(tabsBefore)
  })
})
