...
test.describe('Diff note delete', () => {
  ...
  test('clicking the trash button removes the saved note', async ({ dobiusPage }) => {
    ...
    const deleteButton = dobiusPage.getByTitle('Delete note').first()
    ...
    await expect(dobiusPage.getByTitle('Delete note')).toHaveCount(0, {
      timeout: 5_000
    })
  })
})
