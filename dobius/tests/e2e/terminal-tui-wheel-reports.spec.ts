...
test.describe('terminal TUI wheel reports', () => {
  test('notched mouse wheel ticks produce immediate mouse-reporting TUI scroll reports', async ({
    ...
    const samples = await probeSmallMouseWheelReports(dobiusPage, 4)
  ...
  test('fullscreen mouse-reporting TUI scroll distance follows wheel magnitude @headful', async ({
    ...
    await expect
      ...
      .toBe(true)
    await dobiusPage.waitForTimeout(1200)
    ...
    await execInTerminal(dobiusPage, ptyId, `node ${JSON.stringify(VISIBLE_TUI_FIXTURE_PATH)}`)

    await expect
      .poll(() => readVisibleTuiOffset(dobiusPage), {
        timeout: 10_000,
        message: 'visible fullscreen TUI did not render numbered rows'
      })
      .toBe(0)

    await dispatchTuiWheel(dobiusPage, {
      deltaY: 10,
    ...
    const cellHeight = await dobiusPage.evaluate(() => {
      const state = window.__store?.getState()
      const worktreeId = state?.activeWorktreeId
      const tabId =
        state?.activeTabType === 'terminal'
  ...
  test('TUI scroll setting scales notched mouse wheel reports', async ({ dobiusPage }) => {
    await waitForSessionReady(dobiusPage)
    await waitForActiveWorktree(dobiusPage)
    await ensureTerminalVisible(dobiusPage)
    await waitForActiveTerminalManager(dobiusPage, 30_000)
    ...
    const slow = await probeTimedSmallMouseWheelReports(dobiusPage, {
      drainWaitMs: 120,
      intervalMs: 220,
      ticks: 5
    })
    await dobiusPage.waitForTimeout(220)
    const paced = await probeTimedSmallMouseWheelReports(dobiusPage, {
      drainWaitMs: 220,
      intervalMs: 80,
      ticks: 5
    })
