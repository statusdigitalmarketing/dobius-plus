...
const RUN_REPRO = process.env.DOBIUS_E2E_CODEX_SKILL_PREVIEW_REPRO === '1'
const EXPECT_NO_ARTIFACTS = process.env.DOBIUS_E2E_EXPECT_NO_CODEX_SKILL_PREVIEW_ARTIFACTS === '1'
const FULLSCREEN_MIN_SIZE = { width: 1200, height: 760 }
const MIN_REPRO_DIFF_RATIO = 0.006
const MAX_CLEAN_DIFF_RATIO = 0.0015
const DOBIUS_REPO_PATH = realpathSync(process.cwd())
...
const CODEX_SKILL_PREVIEW_RE = /Press enter to insert|esc to close|electron|dobius-cli|dobius-emulator/i
const SETUP_PANE_ACTIVITY_RE = /install-dobius-skills|pnpm|Progress:|Packages:|Lockfile/i
...
async function addRealDobiusRepo(page: Page, repoPath: string): Promise<string> {
  return page.evaluate(async (repoPath) => {
    ...
    if (!repo) {
      throw new Error(`Real Dobius repo did not load: ${repoPath}`)
    }
    ...
    if (!worktree) {
      throw new Error(`Real Dobius worktree did not load: ${repoPath}`)
    }
...
async function createWorkspaceThroughComposer(page: Page, workspaceName: string): Promise<string> {
  ...
  await expect
    .poll(
      ...
      {
        timeout: 60_000,
        message: `Workspace ${workspaceName} did not appear in the real Dobius repo`
      }
    )
    .not.toBeNull()
  ...
  await expect
    .poll(() => getActiveWorktreeId(page), {
      timeout: 30_000,
      message: 'Created real Dobius workspace did not become active'
    })
    .toBe(createdId)
...
test.describe('Codex skill preview terminal artifact repro @headful', () => {
  ...
  test.skip(!RUN_REPRO, 'Set DOBIUS_E2E_CODEX_SKILL_PREVIEW_REPRO=1 to run this repro.')
  ...
  test('captures the real Dobius repo setup-split Codex skill preview overpaint before any click', async ({
    electronApp,
    dobiusPage
  }, testInfo) => {
    ...
    await setStableFullscreenWindow(electronApp, dobiusPage)
    await waitForSessionReady(dobiusPage)
    await addRealDobiusRepo(dobiusPage, DOBIUS_REPO_PATH)
    ...
    const worktreeId = await createWorkspaceThroughComposer(dobiusPage, workspaceName)
    createdWorktreeIds.push(worktreeId)

    await ensureTerminalVisible(dobiusPage, 30_000)
    await waitForActiveTerminalManager(dobiusPage, 30_000)
    await waitForPaneCount(dobiusPage, 2, 60_000)
    await waitForPaneIdentitySnapshot(dobiusPage, 2)
    const webglActive = await forceTerminalWebgl(dobiusPage)
    test.skip(!webglActive, 'Codex skill preview artifact repro needs WebGL rendering')

    const leftPane = await focusLeftTerminalPane(dobiusPage)
    const rightPane = await getRightTerminalPane(dobiusPage)
    ...
    await dismissCodexPromptsIfPresent(dobiusPage, leftPane)
    await waitForPaneContent(dobiusPage, leftPane.tabId, leftPane.ptyId, CODEX_READY_RE, 60_000)
    await waitForPaneContent(
      dobiusPage,
      leftPane.tabId,
      leftPane.ptyId,
    ...
    await dobiusPage.waitForTimeout(CODEX_READY_SETTLE_MS)
    await waitForPaneVisibleContentChanges(dobiusPage, rightPane, 1, 8_000)

    await focusLeftTerminalPane(dobiusPage)
    await dobiusPage.keyboard.type('test $e', { delay: 70 })
    await waitForPaneContent(
      dobiusPage,
      leftPane.tabId,
      leftPane.ptyId,
    ...
    const evidence = await captureClickEvidence(dobiusPage, leftPane, testInfo)
