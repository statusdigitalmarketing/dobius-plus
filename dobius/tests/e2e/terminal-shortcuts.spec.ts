...
async function dispatchCtrlCToActiveTerminalTextarea(
  ...
  return page.evaluate((dispatchOptions) => {
    ...
    // synthetic DOM events still exercise Dobius's installed xterm boundary.
...
test.describe('Terminal Shortcuts', () => {
  ...
  test('Shift+Enter writes the platform newline chord for terminal TUIs', async ({
    ...
    await waitForActivePanePtyId(dobiusPage)
  ...
  test('Ctrl+Enter writes the kitty modified-enter chord for terminal TUIs', async ({
    ...
    await pressAndExpectWrite(dobiusPage, electronApp, 'Control+Enter', '\x1b[13;5u')
  })
  ...
  test('plain Ctrl+C sends ETX under kitty keyboard reporting', async ({
    ...
    await installMainProcessPtyWriteSpy(electronApp)
    await waitForActivePanePtyId(dobiusPage)
    await enableKittyKeyboardReporting(dobiusPage, 31)
    await clearPtyWriteLog(electronApp)
    await focusActiveTerminal(dobiusPage)
    await dobiusPage.keyboard.down('Control')
    await dobiusPage.keyboard.up('Control')
    expect((await getPtyWrites(electronApp)).join('')).toBe('')
    await clearPtyWriteLog(electronApp)

    expect(await dispatchCtrlCToActiveTerminalTextarea(dobiusPage, { keyupCtrlKey: false })).toEqual({
      keydownDefaultPrevented: false,
    ...
    await expect
      .poll(async () => await getKittyKeyboardFlags(dobiusPage), {
        timeout: 5_000,
    ...
    await clearPtyWriteLog(electronApp)
    await focusActiveTerminal(dobiusPage)
    await dobiusPage.keyboard.type('x')
    await expect
  ...
  test('@headful Codex-like background output stays visible without disabling WebGL in auto mode', async ({
    ...
    const webglActive = await dobiusPage
      .waitForFunction(
        () => {
          const state = window.__store?.getState()
          const worktreeId = state?.activeWorktreeId
    ...
    await execInTerminal(dobiusPage, ptyId, `printf '\\033[48;2;52;52;52m  ${marker}  \\033[0m\\n'`)
    await waitForTerminalOutput(dobiusPage, marker)

    await expect
      .poll(
        () =>
          dobiusPage.evaluate((expectedMarker) => {
            const state = window.__store?.getState()
            const worktreeId = state?.activeWorktreeId
            const tabId =
              state?.activeTabType === 'terminal'
  ...
  test('floating terminal owns tab switch shortcuts while focused', async ({ dobiusPage }) => {
    const scenario = await seedFloatingTerminalTabSwitchScenario(dobiusPage)
    await dobiusPage.evaluate(async () => {
      ...
      if (!document.querySelector('[data-floating-terminal-panel][aria-hidden="false"]')) {
        window.dispatchEvent(new CustomEvent('dobius-toggle-floating-terminal'))
      }
    })
    await expect(
      dobiusPage.locator('[data-floating-terminal-panel][aria-hidden="false"]')
    ).toBeVisible()
    await focusFloatingTerminal(dobiusPage)

    await dobiusPage.keyboard.press(`${mod}+Shift+BracketRight`)
    await expect
      .poll(() => getActiveFloatingTerminalTabId(dobiusPage), {
        timeout: 5_000,
        message: 'floating terminal did not switch to the next tab'
      })
      .toBe(scenario.floatingSecondTabId)
    await expect
      .poll(() => getActiveBackgroundTerminalTabId(dobiusPage), {
        timeout: 1_000,
    ...
    await expect(getActiveBackgroundTerminalTabId(dobiusPage)).resolves.toBe(
  ...
  test('all terminal chords reach the PTY or fire their action', async ({
    ...
    await pressAndExpectWrite(dobiusPage, electronApp, 'Alt+ArrowLeft', '\x1bb')
    await pressAndExpectWrite(dobiusPage, electronApp, 'Alt+ArrowRight', '\x1bf')
    ...
    if (!isMac) {
      await pressAndExpectWrite(dobiusPage, electronApp, 'Control+ArrowLeft', '\x1bb')
      await pressAndExpectWrite(dobiusPage, electronApp, 'Control+ArrowRight', '\x1bf')
    }

    // Alt+Backspace → Esc+DEL (readline backward-kill-word).
    await pressAndExpectWrite(dobiusPage, electronApp, 'Alt+Backspace', '\x1b\x7f')

    // Ctrl+Backspace → \x17 (unix-word-rubout).
    await pressAndExpectWrite(dobiusPage, electronApp, 'Control+Backspace', '\x17')
    ...
    if (isMac) {
      ...
      await pressAndExpectWrite(dobiusPage, electronApp, 'Meta+ArrowLeft', '\x01')
      await pressAndExpectWrite(dobiusPage, electronApp, 'Meta+ArrowRight', '\x05')

      // Cmd+Backspace → Ctrl+U (kill line). Cmd+Delete → Ctrl+K (kill to EOL).
      await pressAndExpectWrite(dobiusPage, electronApp, 'Meta+Backspace', '\x15')
      await pressAndExpectWrite(dobiusPage, electronApp, 'Meta+Delete', '\x0b')
    }
    ...
    await focusActiveTerminal(dobiusPage)
    await dobiusPage.keyboard.press(`${mod}+k`)
    await expect
      .poll(async () => (await getTerminalContent(dobiusPage)).includes(marker), {
        timeout: 5_000,
    ...
    const panesBeforeSplit = await countVisibleTerminalPanes(dobiusPage)
    await focusActiveTerminal(dobiusPage)
    await dobiusPage.keyboard.press(splitVerticalChord)
    await waitForPaneCount(dobiusPage, panesBeforeSplit + 1)

    // Cmd/Ctrl+] and Cmd/Ctrl+[ cycle focus (no pane-count change).
    await focusActiveTerminal(dobiusPage)
    await dobiusPage.keyboard.press(`${mod}+BracketRight`)
    await focusActiveTerminal(dobiusPage)
    await dobiusPage.keyboard.press(`${mod}+BracketLeft`)
    expect(await countVisibleTerminalPanes(dobiusPage)).toBe(panesBeforeSplit + 1)
    ...
    await focusActiveTerminal(dobiusPage)
    await dobiusPage.keyboard.press(`${mod}+Shift+Enter`)
    await expect
    ...
    await focusActiveTerminal(dobiusPage)
    await dobiusPage.keyboard.press(`${mod}+w`)
    await confirmCloseDialogIfShown(dobiusPage)
    await waitForPaneCount(dobiusPage, panesBeforeSplit)

    // Split horizontally (chord varies by platform — see splitHorizontalChord).
    const panesBeforeHSplit = await countVisibleTerminalPanes(dobiusPage)
    await focusActiveTerminal(dobiusPage)
    await dobiusPage.keyboard.press(splitHorizontalChord)
    await waitForPaneCount(dobiusPage, panesBeforeHSplit + 1)
    await focusActiveTerminal(dobiusPage)
    await dobiusPage.keyboard.press(`${mod}+w`)
    await confirmCloseDialogIfShown(dobiusPage)
    await waitForPaneCount(dobiusPage, panesBeforeHSplit)

    // Cmd/Ctrl+F toggles the search overlay.
    await focusActiveTerminal(dobiusPage)
    await dobiusPage.keyboard.press(`${mod}+f`)
    const searchInput = dobiusPage.locator('[data-terminal-search-root] input').first()
    ...
    await expect(dobiusPage.locator('[data-terminal-search-root]').first()).toBeHidden({
      timeout: 3_000
    })
  })
  ...
  test('Cmd+Up/Down scrolls terminal viewport without writing to the PTY on macOS', async ({
    ...
    await execInTerminal(dobiusPage, ptyId, `for i in {1..120}; do echo ${marker}_$i; done`)
    await waitForTerminalOutput(dobiusPage, `${marker}_120`)

    await expect
      .poll(
        async () => {
          const viewport = await getActiveTerminalViewport(dobiusPage)
    ...
    await clearPtyWriteLog(electronApp)
    await focusActiveTerminal(dobiusPage)
    await dobiusPage.keyboard.press('Meta+ArrowUp')
    await expect
      .poll(async () => getActiveTerminalViewport(dobiusPage), {
        timeout: 5_000,
    ...
    await focusActiveTerminal(dobiusPage)
    await dobiusPage.keyboard.press('Meta+ArrowDown')
    await expect
      .poll(
        async () => {
          const viewport = await getActiveTerminalViewport(dobiusPage)
  ...
  test('Shift with Russian layout text reaches the PTY as Cyrillic under kitty keyboard reporting', async ({
    ...
    const dispatch = await pressShiftedRussianLayoutKey(dobiusPage)
