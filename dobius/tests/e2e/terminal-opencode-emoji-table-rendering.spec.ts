...
const EMOJI_TABLE_MARKER = 'DOBIUS_EMOJI_TABLE_RENDER_DONE'
...
test.describe('OpenCode emoji table terminal rendering', () => {
  test('keeps emoji table output visually sane and restores the cursor', async ({
    ...
    await waitForPtyShellEcho(dobiusPage, ptyId, 20_000)
    const runId = randomUUID()
    const marker = `${EMOJI_TABLE_MARKER}_${runId}`
    const scriptPath = path.join(testRepoPath, `.dobius-opencode-emoji-table-${runId}.mjs`)
    writeFileSync(scriptPath, emojiTableScript(marker))
    try {
      await sendToTerminal(dobiusPage, ptyId, `${nodeTerminalCommand([scriptPath])}\r`)
      await waitForTerminalOutput(dobiusPage, marker, 10_000)
      await dobiusPage.waitForTimeout(250)
      await forceCursorProbeTheme(dobiusPage)
      await dobiusPage.waitForTimeout(50)

      const renderState = await readActiveTerminalRenderState(dobiusPage)
      const blinkSamples = await sampleCursorBlink(dobiusPage)
  ...
  test('local real OpenCode demo keeps table rendering and cursor visible', async ({
    ...
    test.skip(
      process.env.DOBIUS_E2E_REAL_OPENCODE !== '1',
      'Set DOBIUS_E2E_REAL_OPENCODE=1 to exercise the locally installed OpenCode TUI'
    )
    ...
    try {
      await waitForTerminalOutput(dobiusPage, 'Give me markdown table', 15_000)
      await waitForTerminalOutput(dobiusPage, 'Emoji', 60_000)
      await waitForTerminalOutput(dobiusPage, 'Alice', 60_000)
      await dobiusPage.waitForTimeout(1_500)

      await testInfo.attach('real-opencode-demo-table', {
        body: await dobiusPage.screenshot({ fullPage: true }),
