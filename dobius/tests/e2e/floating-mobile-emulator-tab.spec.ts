...
test('floating Mobile Emulator tab renders content and closes from the tab strip', async ({
  ...
  const tab = await seedFloatingSimulatorTab(dobiusPage)
  await openFloatingPanelIfNeeded(dobiusPage)

  const openPanel = dobiusPage.locator(OPEN_PANEL_SELECTOR).first()
  ...
  await attachPanelScreenshot(dobiusPage, testInfo)
