...
>({
  ...
  const hiddenPanes = await deps.ensureActiveWorktreePaneLoad(dobiusPage, hiddenPaneCount)

  const runId = randomUUID()
  const typingScriptPath = path.join(
    testRepoPath,
    `.dobius-opencode-hidden-pressure-typing-${runId}.mjs`
  )
  const pressureScriptPath = path.join(
    testRepoPath,
    `.dobius-opencode-hidden-pressure-load-${runId}.mjs`
  )
  ...
  try {
    ...
    await switchToTypingWorkspace(dobiusPage, firstWorktreeId)
    ...
    const debug = await deps.readTerminalPtyOutputDebug(dobiusPage)
    ...
    deps.annotateTypingMeasurement(
      ...
      measurement,
      debug,
      await deps.readTerminalOutputSchedulerDebug(dobiusPage),
      mainPressure,
      ackGate
    )
...
async function measureHiddenOutputRestoreLatency(
  ...
  await expect
    .poll(() => getTerminalContent(dobiusPage, 20_000), {
      timeout: 20_000,
...
async function switchToTypingWorkspace(dobiusPage: Page, worktreeId: string): Promise<void> {
  await switchToWorktree(dobiusPage, worktreeId)
  await expect.poll(() => getActiveWorktreeId(dobiusPage), { timeout: 10_000 }).toBe(worktreeId)
...
>({
  ...
  await deps.releaseTerminalAckGate(dobiusPage)
  await switchToWorktree(dobiusPage, firstWorktreeId).catch(() => undefined)
  await waitForActivePanePtyId(dobiusPage)
    .then((ptyId) => sendToTerminal(dobiusPage, ptyId, '\x03'))
    .catch(() => undefined)
  await switchToWorktree(dobiusPage, secondWorktreeId).catch(() => undefined)
  await Promise.all(
    hiddenPanes.map((pane) => sendToTerminal(dobiusPage, pane.ptyId, '\x03').catch(() => undefined))
  )
