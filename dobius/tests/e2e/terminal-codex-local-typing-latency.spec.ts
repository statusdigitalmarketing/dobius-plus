...
test.describe('local Codex terminal typing latency', () => {
  test('keeps Codex prompt typing responsive @local-real-codex', async ({ dobiusPage }, testInfo) => {
    test.skip(
      process.env.DOBIUS_E2E_REAL_CODEX !== '1',
      'Set DOBIUS_E2E_REAL_CODEX=1 to exercise the locally installed Codex TUI'
    )
    ...
    try {
      await sendToTerminal(dobiusPage, ptyId, launchCommand)
      await dismissCodexPromptsIfPresent(dobiusPage)
      await waitForCodexReady(dobiusPage)
      ...
      const prompt = `dobius_codex_latency_${runId}`
      const latencies: number[] = []
      let typed = ''
      for (const char of prompt) {
        typed += char
        const start = performance.now()
        await dobiusPage.keyboard.type(char)
        await waitForPromptText(dobiusPage, typed)
