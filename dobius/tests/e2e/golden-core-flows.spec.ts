...
const REPO_STEP_HEADING = /Point Dobius at some code/i
...
test.describe('Existing-user golden core flow', () => {
  test('adds project, creates workspace, opens a terminal tab, and splits a pane', async ({
    ...
    const repoPath = await createGitRepo('dobius-e2e-golden-existing-', 'golden-existing-project')

    await addProjectFromSidebar(dobiusPage, electronApp, repoPath)
    const workspaceName = `golden-existing-${Date.now()}`
    await createWorkspace(dobiusPage, workspaceName)
    await expectActiveWorkspaceBelongsToRepo(dobiusPage, workspaceName, repoPath)
    await ensureTerminalVisible(dobiusPage)
    await expectTerminalSurface(dobiusPage)
    await waitForTerminalPaneManager(dobiusPage)

    await createTerminalTabThroughMenu(dobiusPage)
    await splitTerminalPaneAndAssertIdentity(dobiusPage)
  })
})
...
test.describe('New-user golden core flow', () => {
  ...
  test('completes onboarding, adds a project, and follows the workspace tour handoff', async ({
    ...
    await selectCodexAgent(dobiusPage)
    await continueOnboarding(dobiusPage)
    await expect(dobiusPage.getByRole('heading', { name: /Make it feel like home/i })).toBeVisible()
    await chooseOppositeTheme(dobiusPage)
    await continueOnboarding(dobiusPage)
    await continueThroughOptionalSetupToNotifications(dobiusPage)
    await expect(dobiusPage.getByRole('button', { name: /Send Test Notification/i })).toBeVisible()
    await chooseNotificationSound(dobiusPage)
    await continueFromNotificationsToRepo(dobiusPage)

    const repoPath = await createGitRepo('dobius-e2e-golden-new-', 'golden-new-project')
    await chooseFolderInNativeDialog(electronApp, repoPath)
    await dobiusPage
      .getByRole('button', { name: /Browse for a folder|Open a folder|Browse folder/i })
      .click()
    await expect(dobiusPage.getByRole('heading', { name: REPO_STEP_HEADING })).toHaveCount(0, {
      timeout: 30_000
    })
    await waitForRepoLoaded(dobiusPage, repoPath)
    await expectProjectVisible(dobiusPage, repoPath)
    await waitForActiveWorktree(dobiusPage)
    await ensureTerminalVisible(dobiusPage)
    await expectTerminalSurface(dobiusPage)
    await waitForTerminalPaneManager(dobiusPage)

    await requestAgentSessionsTour(dobiusPage)
    const paneCountBeforeTourSplit = await countVisibleTerminalPanes(dobiusPage)
    await dobiusPage.getByRole('button', { name: /^Split terminal$/ }).click()
    await waitForPaneCount(dobiusPage, paneCountBeforeTourSplit + 1)
    await waitForPaneIdentitySnapshot(dobiusPage, paneCountBeforeTourSplit + 1)

    await expect(
      dobiusPage.getByRole('dialog', { name: /Start another task in parallel/i })
    ).toBeVisible()
    const createControl = dobiusPage
    ...
    await completeWorkspaceCreationTour(dobiusPage, workspaceName)
    await expectActiveWorkspaceBelongsToRepo(dobiusPage, workspaceName, repoPath)
  })
})
