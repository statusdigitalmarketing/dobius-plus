...
test.describe('Browser Tab', () => {
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('creating a browser tab adds it and activates browser view', async ({ dobiusPage }) => {
    const worktreeId = (await getActiveWorktreeId(dobiusPage))!
    const browserTabsBefore = await getBrowserTabs(dobiusPage, worktreeId)

    await createBrowserTab(dobiusPage, worktreeId)

    // Wait for the browser tab to appear in the store
    await expect
      .poll(async () => (await getBrowserTabs(dobiusPage, worktreeId)).length, { timeout: 5_000 })
      .toBe(browserTabsBefore.length + 1)

    // The active tab type should switch to 'browser'
    await expect.poll(async () => getActiveTabType(dobiusPage), { timeout: 3_000 }).toBe('browser')
  })
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('browser tab is created and active in the store', async ({ dobiusPage }) => {
    const worktreeId = (await getActiveWorktreeId(dobiusPage))!

    await createBrowserTab(dobiusPage, worktreeId)
    await expect.poll(async () => getActiveTabType(dobiusPage), { timeout: 5_000 }).toBe('browser')

    // Verify the browser tab exists in the store
    const browserTabs = await getBrowserTabs(dobiusPage, worktreeId)
    ...
    const activeBrowserTabId = await dobiusPage.evaluate(() => {
      const store = window.__store
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('browser tab retains state when switching to terminal and back', async ({ dobiusPage }) => {
    const worktreeId = (await getActiveWorktreeId(dobiusPage))!
    ...
    await switchToTerminalTab(dobiusPage, worktreeId)
    await expect.poll(async () => getActiveTabType(dobiusPage), { timeout: 3_000 }).toBe('terminal')

    // Switch back to browser tab
    await switchToBrowserTab(dobiusPage, worktreeId, browserTabId!)
    await expect.poll(async () => getActiveTabType(dobiusPage), { timeout: 3_000 }).toBe('browser')

    // The browser tab should still exist with the same ID
    const browserTabsAfter = await getBrowserTabs(dobiusPage, worktreeId)
  ...
  test('browser webview form state survives switching between browser tabs', async ({
    ...
    try {
      ...
      await writeBrowserInputValue(dobiusPage, firstTab!.id, 'first typed value')
      ...
      await writeBrowserInputValue(dobiusPage, secondTab!.id, 'second typed value')
      ...
      await switchToBrowserTab(dobiusPage, worktreeId, firstTab!.id)
      await expect
        .poll(async () => readBrowserInputValue(dobiusPage, firstTab!.id), { timeout: 5_000 })
        .toBe('first typed value')

      await switchToBrowserTab(dobiusPage, worktreeId, secondTab!.id)
      await expect
        .poll(async () => readBrowserInputValue(dobiusPage, secondTab!.id), { timeout: 5_000 })
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('browser tab retains state when switching worktrees and back', async ({ dobiusPage }) => {
