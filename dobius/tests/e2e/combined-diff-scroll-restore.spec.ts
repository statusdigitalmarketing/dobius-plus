...
function createCombinedDiffScrollRepo(): CombinedDiffScrollRepo {
  const repoPath = realpathSync(mkdtempSync(path.join(os.tmpdir(), 'dobius-combined-diff-scroll-')))
...
test.describe('Combined diff scroll restore', () => {
  ...
  test('keeps the visible section anchored after switching tabs', async ({ dobiusPage }) => {
    await waitForSessionReady(dobiusPage)
    const fixture = createCombinedDiffScrollRepo()

    try {
      const worktreeId = await addAndActivateRepo(dobiusPage, fixture.repoPath)
      const diffTabId = await openCombinedDiff(dobiusPage, worktreeId, fixture.repoPath)
      await expect(dobiusPage.locator('.combined-diff-scroll-container')).toBeVisible()
      await expect(dobiusPage.getByText(`${FILE_COUNT} changed files`)).toBeVisible()

      await scrollCombinedDiffDeep(dobiusPage)
      await waitForStableViewportAnchor(dobiusPage)
      const activeScrollSamples = await wheelCombinedDiffDown(dobiusPage)
      ...
      const beforeSwitch = await waitForStableViewportAnchor(dobiusPage)
      ...
      await expect(dobiusPage.locator('.combined-diff-scroll-container')).toHaveCount(0)

      await dobiusPage.locator(`[data-tab-id="${diffTabId}"]`).click({ force: true })
      await expect(dobiusPage.locator('.combined-diff-scroll-container')).toBeVisible()
      const afterSwitch = await waitForStableViewportAnchor(dobiusPage)

      expect(afterSwitch.key).toBe(beforeSwitch.key)
      expect(Math.abs(afterSwitch.top - beforeSwitch.top)).toBeLessThan(80)

      await clickVisibleDiffLine(dobiusPage)
      const afterLineClick = await waitForStableViewportAnchor(dobiusPage)
