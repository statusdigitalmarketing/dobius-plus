...
test.describe('Setup guide sidebar entry', () => {
  ...
  test('does not flash while completed setup waits for capability readiness', async ({
    ...
    await waitForSessionReady(dobiusPage)
    await seedCompletedSetupExceptCapabilityReadiness(dobiusPage)

    await expect
      .poll(async () => getStoreState<boolean>(dobiusPage, 'setupGuideSidebarDismissed'), {
        timeout: 5_000
      })
      .toBe(false)
    await expect(dobiusPage.getByText(CHECKLIST_TEXT)).toHaveCount(0)

    await startSetupGuideFlashMonitor(dobiusPage)

    await setActiveViewForFlashProbe(dobiusPage, 'tasks')
    await expect
      .poll(async () => getStoreState<string>(dobiusPage, 'activeView'), { timeout: 5_000 })
      .toBe('tasks')
    await dobiusPage.waitForTimeout(500)

    await setActiveViewForFlashProbe(dobiusPage, 'automations')
    await expect
      .poll(async () => getStoreState<string>(dobiusPage, 'activeView'), { timeout: 5_000 })
      .toBe('automations')
    await dobiusPage.waitForTimeout(500)

    await setActiveViewForFlashProbe(dobiusPage, 'mobile')
    await expect
      .poll(async () => getStoreState<string>(dobiusPage, 'activeView'), { timeout: 5_000 })
      .toBe('mobile')
    await dobiusPage.waitForTimeout(500)

    const flashSamples = await stopSetupGuideFlashMonitor(dobiusPage)
    ...
    await dobiusPage.evaluate(() => {
      window.dispatchEvent(new CustomEvent('dobius:installed-agent-skills-changed'))
    })
  })
})
...
async function installBlockedCompletedCapabilityFakes(
  ...
  await evaluateInElectronMainWithNavigationRetry(electronApp, ({ ipcMain }) => {
    ...
    const makeSkill = (name: string, id: string): SkillDiscoveryResult['skills'][number] => ({
      ...
      rootPath: '/tmp/dobius-e2e-skills',
      directoryPath: `/tmp/dobius-e2e-skills/${name}`,
      skillFilePath: `/tmp/dobius-e2e-skills/${name}/SKILL.md`,
      installed: true,
      fileCount: 1,
      updatedAt: 1
    })
    ...
    ipcMain.handle('skills:discover', async (): Promise<SkillDiscoveryResult> => {
      ...
      return {
        skills: [
          makeSkill('dobius-cli', 'e2e-dobius-cli'),
