...
const SAME_WORKSPACE_PANES = readPositiveInt(
  'DOBIUS_E2E_OPENCODE_SAME_WORKSPACE_PANES',
  DEFAULT_SAME_WORKSPACE_PANES
)
const CROSS_WORKSPACE_PANES_PER_WORKTREE = readPositiveInt(
  'DOBIUS_E2E_OPENCODE_CROSS_WORKSPACE_PANES',
  DEFAULT_CROSS_WORKSPACE_PANES_PER_WORKTREE
)
const PRESSURE_BACKGROUND_PANES = readPositiveInt(
  'DOBIUS_E2E_OPENCODE_PRESSURE_BACKGROUND_PANES',
  DEFAULT_PRESSURE_BACKGROUND_PANES
)
const PRESSURE_OUTPUT_CHARS = readPositiveInt(
  'DOBIUS_E2E_OPENCODE_PRESSURE_OUTPUT_CHARS',
  DEFAULT_PRESSURE_OUTPUT_CHARS
)
const HIDDEN_PRESSURE_PANES = readPositiveInt(
  'DOBIUS_E2E_OPENCODE_HIDDEN_PRESSURE_PANES',
  DEFAULT_HIDDEN_PRESSURE_PANES
)
const FRAME_COUNT = readPositiveInt('DOBIUS_E2E_OPENCODE_FRAME_COUNT', DEFAULT_FRAME_COUNT)
const FRAME_INTERVAL_MS = readPositiveInt(
  'DOBIUS_E2E_OPENCODE_FRAME_INTERVAL_MS',
  DEFAULT_FRAME_INTERVAL_MS
)
const SCALE_SAME_WORKSPACE_PANES = readPositiveIntList('DOBIUS_E2E_OPENCODE_SCALE_PANES')
const SCALE_CROSS_WORKSPACE_PANES = readPositiveIntList(
  'DOBIUS_E2E_OPENCODE_SCALE_CROSS_WORKSPACE_PANES'
)
const SCALE_PRESSURE_PANES = readPositiveIntList('DOBIUS_E2E_OPENCODE_SCALE_PRESSURE_PANES')
const SCALE_HIDDEN_PRESSURE_PANES = readPositiveIntList(
  'DOBIUS_E2E_OPENCODE_SCALE_HIDDEN_PRESSURE_PANES'
)
...
async function measureCrossWorkspaceTypingDuringHiddenLoad({
  ...
  const hiddenPanes = await ensureActiveWorktreePaneLoad(dobiusPage, hiddenPaneCount)

  await switchToWorktree(dobiusPage, firstWorktreeId)
  await expect.poll(() => getActiveWorktreeId(dobiusPage), { timeout: 10_000 }).toBe(firstWorktreeId)
  await ensureTerminalVisible(dobiusPage)
  await waitForActiveTerminalManager(dobiusPage, 30_000)
  const typingPtyId = await waitForActivePanePtyId(dobiusPage)

  const runId = randomUUID()
  const scriptPath = path.join(testRepoPath, `.dobius-opencode-cross-${hiddenPaneCount}-${runId}.mjs`)
  writeInteractivePromptScript(scriptPath, runId)
  await resetTerminalPtyOutputDebug(dobiusPage)
  ...
  try {
    const measurement = await measureTypingDuringLoad(dobiusPage, scriptPath, typingPtyId, runId)
    const debug = await readTerminalPtyOutputDebug(dobiusPage)
    const scheduler = await readTerminalOutputSchedulerDebug(dobiusPage)
    const mainPressure = await readMainPtyPressureDebug(dobiusPage)
    ...
    await load.stop()
    await sendToTerminal(dobiusPage, typingPtyId, '\x03').catch(() => undefined)
...
test.describe('Artificial OpenCode terminal load', () => {
  ...
  test('measures baseline typing responsiveness with one active terminal', async ({
    ...
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.dobius-opencode-baseline-typing-${runId}.mjs`)
    writeInteractivePromptScript(scriptPath, runId)
    await resetTerminalPtyOutputDebug(dobiusPage)
    try {
      const measurement = await measureTypingDuringLoad(dobiusPage, scriptPath, typingPtyId, runId)
  ...
  test('keeps typing responsive while same-workspace panes redraw simultaneously', async ({
    ...
    await waitForSessionReady(dobiusPage)
    await waitForActiveWorktree(dobiusPage)
    const panes = await ensureActiveWorktreePaneLoad(dobiusPage, SAME_WORKSPACE_PANES)
    const [typingPane, ...loadPanes] = panes
    await focusPane(dobiusPage, typingPane.paneKey)

    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.dobius-opencode-typing-${runId}.mjs`)
    writeInteractivePromptScript(scriptPath, runId)
    await resetTerminalPtyOutputDebug(dobiusPage)
    const load = await startSyntheticOpenCodeInjection({
      frameCount: FRAME_COUNT,
    ...
    try {
      ...
      annotateTypingMeasurement(
        ...
        panes.length,
        measurement,
        await readTerminalPtyOutputDebug(dobiusPage),
        await readTerminalOutputSchedulerDebug(dobiusPage),
        await readMainPtyPressureDebug(dobiusPage)
      )
      expect(measurement.medianLatencyMs).toBeLessThan(MAX_MEDIAN_KEY_LATENCY_MS)
      expect(measurement.worstLatencyMs).toBeLessThan(MAX_WORST_KEY_LATENCY_MS)
      expect(measurement.maxTimerDriftMs).toBeLessThan(MAX_TIMER_DRIFT_MS)
  ...
  for (const paneCount of SCALE_SAME_WORKSPACE_PANES) {
    test(`keeps typing responsive at ${paneCount} same-workspace OpenCode panes`, async ({
      ...
      await waitForSessionReady(dobiusPage)
      await waitForActiveWorktree(dobiusPage)
      const panes = await ensureActiveWorktreePaneLoad(dobiusPage, paneCount)
      const [typingPane, ...loadPanes] = panes
      await focusPane(dobiusPage, typingPane.paneKey)

      const runId = randomUUID()
      const scriptPath = path.join(testRepoPath, `.dobius-opencode-scale-${paneCount}-${runId}.mjs`)
      writeInteractivePromptScript(scriptPath, runId)
      await resetTerminalPtyOutputDebug(dobiusPage)
      const load = await startSyntheticOpenCodeInjection({
        frameCount: FRAME_COUNT,
