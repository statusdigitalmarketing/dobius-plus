...
test.describe('Docker SSH relay perf', () => {
  test.skip(!RUN_DOCKER_SSH, 'Set DOBIUS_E2E_SSH_DOCKER=1 to run Docker-backed SSH relay perf.')
  ...
  test('keeps remote typing responsive while the Linux relay streams TUI output', async ({
    ...
    try {
      ...
      await connectDockerRemote(dobiusPage, target)
      ...
      await execInTerminal(dobiusPage, ptyId, `node -e ${shellQuote(remoteTypingLoadScript(runId))}`)
      await waitForTerminalOutput(dobiusPage, `REMOTE_TUI_READY_${runId}`, 30_000, 80_000)
      const measurement = await measureRemoteTyping(dobiusPage, ptyId, runId)
      ...
      await stopRemoteLoad(dobiusPage, ptyId)
    } finally {
  ...
  test('keeps an SSH workspace terminal usable after disconnect and reconnect', async ({
    ...
    try {
      ...
      await ensureTerminalVisible(dobiusPage, 45_000)
      await waitForActiveTerminalManager(dobiusPage, 60_000)
      const beforePtyId = await waitForActivePanePtyId(dobiusPage, 60_000)
      const beforeMarker = `SSH_RECONNECT_BEFORE_${Date.now()}`
      await execInTerminal(dobiusPage, beforePtyId, `printf ${shellQuote(beforeMarker)}`)
      await waitForTerminalOutput(dobiusPage, beforeMarker, 20_000, 60_000)

      await reconnectDockerTarget(dobiusPage, remote.targetId)
      await ensureTerminalVisible(dobiusPage, 45_000)
      await waitForActiveTerminalManager(dobiusPage, 60_000)
      const afterPtyId = await waitForActivePanePtyId(dobiusPage, 60_000)
      const afterMarker = `SSH_RECONNECT_AFTER_${Date.now()}`
      await execInTerminal(dobiusPage, afterPtyId, `printf ${shellQuote(afterMarker)}`)
      await waitForTerminalOutput(dobiusPage, afterMarker, 20_000, 60_000)
