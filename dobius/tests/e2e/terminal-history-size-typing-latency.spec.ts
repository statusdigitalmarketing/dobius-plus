...
test.describe('Terminal typing latency vs scrollback history size', () => {
  test('typing stays responsive as terminal history grows', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.dobius-history-benchmark-${runId}.mjs`)
    ...
    try {
      ...
      commandSent = true
      await waitForRecentTerminalMarker(dobiusPage, `HIST_READY_${runId}`, 10_000)
      await focusActiveTerminalInput(dobiusPage)

      const phases: PhaseLatency[] = []
      let seq = 0

      const baseline = await measureTypingLatency(dobiusPage, runId, 'empty history', seq)
