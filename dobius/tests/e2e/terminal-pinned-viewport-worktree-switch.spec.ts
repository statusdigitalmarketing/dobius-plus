...
import { expect, test } from './helpers/dobius-app'
import {
...
test.describe('Terminal pinned viewport worktree switch', () => {
  test('does not jump or flash when returning to a viewport pinned just above bottom', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.dobius-pinned-viewport-${runId}.mjs`)
    ...
    try {
      ...
      const pinned = await pinActiveTerminalNearBottom(dobiusPage)
      ...
      const samplesPromise = sampleTerminalViewportDuringReturn(dobiusPage, pinned.tabId, 450)
