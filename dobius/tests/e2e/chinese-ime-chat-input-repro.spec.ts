...
function emitState(reason) {
  ...
  process.stdout.write('\\x1b]1337;DobiusImeState=' + Buffer.from(JSON.stringify({
    reason,
    model,
    cursor,
    submitted
...
async function installImeEventProbe(page: Page): Promise<void> {
  await page.evaluate(() => {
    const targetWindow = window as unknown as { __dobiusImeEventLog?: ImeEventLogEntry[] }
    targetWindow.__dobiusImeEventLog = []
    ...
    const record = (event: Event): void => {
      ...
      targetWindow.__dobiusImeEventLog!.push({
        type: event.type,
        at: performance.now(),
...
async function readImeEventLog(page: Page): Promise<ImeEventLogEntry[]> {
  return page.evaluate(() => {
    const targetWindow = window as unknown as { __dobiusImeEventLog?: ImeEventLogEntry[] }
    return targetWindow.__dobiusImeEventLog ?? []
  })
}
...
test.describe('Chinese IME terminal chat input repro', () => {
  test('keeps composed Chinese text, cursor movement, and Backspace stable in the agent input surface', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.dobius-chinese-ime-harness-${runId}.cjs`)
    writeFileSync(scriptPath, terminalImeHarnessScript(runId))
    const session = await dobiusPage.context().newCDPSession(dobiusPage)

    try {
      await sendToTerminal(dobiusPage, ptyId, `node ${JSON.stringify(scriptPath)}\r`)
      await waitForTerminalOutput(dobiusPage, `IME_HARNESS_READY_${runId}`, 10_000, 20_000)
      await focusActiveTerminalInput(dobiusPage)
      await installImeEventProbe(dobiusPage)

      await dispatchImeProcessKey(session, 'KeyN')
      await composeAndCommitChineseText(session, dobiusPage, ['n', 'ni', '你', '你好'], '你好')
      await waitForLivePrompt(dobiusPage, '你好')
      await attachImeEvidence(dobiusPage, testInfo, 'after-compose-hello')
      await dobiusPage.keyboard.press('Enter')
      await expect
        .poll(async () => (await readPromptState(dobiusPage))?.submitted.at(-1) ?? null, {
          timeout: 5_000,
      ...
      await waitForLivePrompt(dobiusPage, '一二三四五六七八九十')
      for (let index = 0; index < 5; index += 1) {
        await dobiusPage.keyboard.press('ArrowLeft')
      }
      await dispatchImeProcessKey(session, 'KeyZ')
      await composeAndCommitChineseText(session, dobiusPage, ['z', 'zh', '中'], '中')
      await waitForLivePrompt(dobiusPage, '一二三四五中六七八九十')
      await attachImeEvidence(dobiusPage, testInfo, 'after-middle-insert')
      ...
      await dobiusPage.keyboard.press('Backspace')
      await waitForLivePrompt(dobiusPage, '一二三四五六七八九十')
      await attachImeEvidence(dobiusPage, testInfo, 'after-single-backspace')
      ...
      const log = await readImeEventLog(dobiusPage)
      expect(
      ...
      await attachImeEvidence(dobiusPage, testInfo, 'final-ime-evidence').catch(() => undefined)
  ...
  test('keeps composed Chinese text stable in the real Codex TUI input @real-codex-ime', async ({
    ...
    test.skip(
      process.env.DOBIUS_E2E_REAL_CODEX_IME !== '1',
      'Set DOBIUS_E2E_REAL_CODEX_IME=1 to exercise the locally installed Codex TUI'
    )
    ...
    try {
      await launchCodexTui(dobiusPage, ptyId)
      await installImeEventProbe(dobiusPage)

      await dispatchImeProcessKey(session, 'KeyN')
      await composeAndCommitChineseText(session, dobiusPage, ['n', 'ni', '你', '你好'], '你好')
      await waitForCleanTerminalText(dobiusPage, /你好/, 'Codex input did not show composed Chinese')
      await attachImeEvidence(dobiusPage, testInfo, 'codex-after-compose-hello', {
        cleanTerminal: stripTerminalControls(await getTerminalContent(dobiusPage, 20_000))
      })
      ...
      await attachImeEvidence(dobiusPage, testInfo, 'codex-after-composition-backspace', {
        cleanTerminal: stripTerminalControls(await getTerminalContent(dobiusPage, 20_000))
      })

      const cleanTerminal = stripTerminalControls(await getTerminalContent(dobiusPage, 20_000))
      expect(
        cleanTerminal,
      ...
      await attachImeEvidence(dobiusPage, testInfo, 'codex-final-ime-evidence', {
        cleanTerminal: stripTerminalControls(await getTerminalContent(dobiusPage, 20_000))
      }).catch(() => undefined)
      await session.detach().catch(() => undefined)
      await sendToTerminal(dobiusPage, ptyId, '\x03/quit\r').catch(() => undefined)
    }
  })
})
