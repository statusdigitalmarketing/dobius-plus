...
test.describe('Worktree Lineage', () => {
  ...
  test('renders existing child lineage in the sidebar', async ({ dobiusPage }) => {
    const { parentId, childId } = await seedLineageScenario(dobiusPage)
    const parentRow = worktreeOption(dobiusPage, parentId)
    const childRow = worktreeOption(dobiusPage, childId)
    ...
    const positions = await dobiusPage.evaluate(
      ({ parentId, childId }) => {
    ...
    await dobiusPage.evaluate(async (childId) => {
  ...
  test('updates nested child preview status when the child terminal sleeps', async ({
    ...
    const childTabId = await seedWorkspaceLiveTerminal(dobiusPage, childId)
    await expect(childRow).toContainText('Active')

    await markWorkspaceTerminalSlept(dobiusPage, { worktreeId: childId, tabId: childTabId })
  ...
  test('shows parent and child agent rows while the parent workspace is active', async ({
    ...
    const parentAgentPrompt = await seedWorkspaceAgentStatus(dobiusPage, parentId, 'PARENT')
    const childAgentPrompt = await seedWorkspaceAgentStatus(dobiusPage, childId, 'CHILD')
