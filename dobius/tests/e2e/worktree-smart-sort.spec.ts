...
test.describe('Worktree Smart Sort', () => {
  ...
  test('renders attention-needed worktrees above finished agents in Smart mode', async ({
    ...
    const scenario = await seedSmartSortScenario(dobiusPage)
    const { blockedId, doneId } = scenario

    await expect
      .poll(() => getSmartSortScenarioReadiness(dobiusPage, scenario), {
        timeout: 8_000,
    ...
    await expect
      .poll(async () => (await getVisibleWorktreeIdsByTop(dobiusPage)).slice(0, 2), {
        timeout: 12_000,
        message: 'Smart sort did not promote the blocked worktree in the visible sidebar'
      })
      .toEqual([blockedId, doneId])

    await expect(worktreeRow(dobiusPage, blockedId)).toBeVisible()
    await expect(worktreeRow(dobiusPage, doneId)).toBeVisible()
  })
})
