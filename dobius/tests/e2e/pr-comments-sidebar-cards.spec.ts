...
test.describe('PR comments sidebar cards view', () => {
  ...
  test('groups open, conversation, and resolved comments in cards layout', async ({ dobiusPage }) => {
    const { worktreeId } = await seedPRCommentsSidebarFixture(dobiusPage)
    await openChecks(dobiusPage, worktreeId)

    const commentsSection = dobiusPage.getByText('Comments', { exact: true })
    await expect(commentsSection).toBeVisible({ timeout: 10_000 })

    await expect(dobiusPage.getByText('Needs review · 1')).toBeVisible()
    await expect(dobiusPage.getByText('Please update this handler before merge.')).toBeVisible()
    await expect(dobiusPage.getByText('alice')).toBeVisible()
    await expect(dobiusPage.getByText('Open', { exact: true })).toBeVisible()
    await expect(dobiusPage.getByText('LGTM on the overall approach.')).toBeVisible()

    const openThreadCard = dobiusPage.getByTestId('pr-comment-group').filter({
      hasText: 'Please update this handler before merge.'
    })
    const conversationCard = dobiusPage.getByTestId('pr-comment-group').filter({
    ...
    const resolvedTrigger = dobiusPage.getByRole('button', { name: 'Resolved · 1' })
    await expect(resolvedTrigger).toBeVisible()
    await expect(dobiusPage.getByText('Already fixed upstream.')).toBeHidden()

    await resolvedTrigger.click()
    await expect(dobiusPage.getByText('Already fixed upstream.')).toBeVisible()
    await expect(dobiusPage.getByText('Resolved', { exact: true })).toBeVisible()
    await expect(
      dobiusPage
    ...
    await expect(dobiusPage.getByRole('button', { name: /^Add$/ })).toHaveCount(0)
  })

  test('can switch from grouped to chronological timeline order', async ({ dobiusPage }) => {
    const { worktreeId } = await seedPRCommentsSidebarFixture(dobiusPage)
    await openChecks(dobiusPage, worktreeId)

    await expect(dobiusPage.getByText('Needs review · 1')).toBeVisible({ timeout: 10_000 })
    await dobiusPage.getByRole('button', { name: 'Comment display options' }).click()
    await dobiusPage.getByRole('menuitemradio', { name: 'Timeline' }).click()

    await expect(dobiusPage.getByText('Needs review · 1')).toHaveCount(0)
    await expect(dobiusPage.getByText('Already fixed upstream.')).toBeVisible()

    const comments = [
      dobiusPage.getByText('Already fixed upstream.'),
      dobiusPage.getByText('Please update this handler before merge.'),
      dobiusPage.getByText('LGTM on the overall approach.')
    ]
  ...
  test('queues an open thread for the agent from the visible row action and menu fallback', async ({
    ...
    await expect(
      dobiusPage.getByRole('button', { name: 'Send 1 queued comments to AI' })
    ).toBeVisible()
    await expect(dobiusPage.getByText('Queued', { exact: true })).toBeVisible()

    await dobiusPage.getByRole('button', { name: 'Clear queued comments' }).click()
    await expect(
      dobiusPage.getByRole('button', { name: 'Send 1 queued comments to AI' })
    ).toBeHidden()
    await openThreadCard.hover()
    ...
    const queueMenuItem = dobiusPage.getByRole('menuitem', { name: 'Queue for agent' })
    ...
    const queuedCard = dobiusPage.getByTestId('pr-comment-group').filter({
      hasText: 'Please update this handler before merge.'
    })
    const queuedCardBox = await queuedCard.boundingBox()
    const checkboxBox = await dobiusPage
  ...
  test('keeps open card content aligned while the row menu is open', async ({ dobiusPage }) => {
    const { worktreeId } = await seedPRCommentsSidebarFixture(dobiusPage)
    await openChecks(dobiusPage, worktreeId)
    ...
    await actionsMenu.evaluate((element) => (element as HTMLElement).focus())
    await actionsMenu.press('Enter')
    await expect(dobiusPage.getByRole('menuitem', { name: 'Queue for agent' })).toBeVisible()
