...
test.describe('Terminal typing latency', () => {
  test('interactive prompt echoes typed keys without visible lag', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.dobius-typing-benchmark-${runId}.mjs`)
    ...
    try {
      ...
      commandSent = true
      await waitForTerminalOutput(dobiusPage, `TYPING_READY_${runId}`, 10_000)
      ...
      for (const [index, char] of [...KEY_LATENCY_SAMPLES].entries()) {
        ...
        await waitForMarkerLatency(dobiusPage, marker, MAX_WORST_KEY_LATENCY_MS)
