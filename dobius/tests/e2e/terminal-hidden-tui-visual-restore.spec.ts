...
test.describe('Hidden terminal TUI visual restore', () => {
  test('restores hidden full-screen TUI output without visible corruption', async ({
    ...
    const hiddenSnapshot = await waitForPaneIdentitySnapshot(dobiusPage, 1)
    ...
    const scriptPath = path.join(testRepoPath, `.dobius-hidden-tui-visual-${runId}.mjs`)
    writeHiddenFrameScript(scriptPath, runId)
    await resetHiddenDebug(dobiusPage)
    await writeHiddenFrames(dobiusPage, hiddenPane.ptyId, scriptPath)

    await expect
      .poll(async () => (await readHiddenDebug(dobiusPage))?.hiddenRendererSkipCount ?? 0, {
        timeout: 10_000,
    ...
    await expect
      .poll(() => getTerminalContent(dobiusPage, 12_000), {
        timeout: 10_000,
        message: 'hidden TUI final frame did not restore when the workspace became visible'
      })
      .toContain(finalMarker)

    const content = await getTerminalContent(dobiusPage, 12_000)
    expect(content).toContain(`Frame 024`)
    expect(content).toContain('╭')
    expect(content).toContain('├')
    expect(content).toContain('█')
    expect(content).not.toContain('Dobius skipped hidden terminal output')
    await expect
      .poll(() => readTuiCursorState(dobiusPage), {
        timeout: 5_000,
  ...
  test('keeps newer live output correct after hidden output stayed live', async ({
    ...
    await ensureTerminalVisible(dobiusPage)
    await waitForActiveTerminalManager(dobiusPage, 30_000)
    await injectPaneData(dobiusPage, paneKey, liveFrame, {
  ...
  test('keeps hidden terminal side effects live while hidden output stays live', async ({
    ...
    await resetHiddenDebug(dobiusPage)
    await writeHiddenSideEffectBurst(dobiusPage, hiddenPane.ptyId, hiddenTitle, marker)
    ...
    await expect
      ...
      .toBe(0)
    await expect
      .poll(() => getRuntimePaneTitle(dobiusPage, hiddenSnapshot.tabId, hiddenPane.numericPaneId), {
        timeout: 10_000,
        message: 'hidden OSC title did not update renderer-visible model state'
      })
      .toBe(hiddenTitle)
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(dobiusPage)).includes(hiddenSnapshot.tabId), {
        timeout: 10_000,
        message: 'hidden BEL did not mark the hidden terminal tab unread'
      })
      .toBe(true)
    await expect
      .poll(() => readMainSnapshotSource(dobiusPage, hiddenPane.ptyId!), {
        timeout: 10_000,
