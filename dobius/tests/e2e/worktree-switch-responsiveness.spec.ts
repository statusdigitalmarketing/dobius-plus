...
test.describe('Worktree switch responsiveness', () => {
  ...
  test('updates the selected workspace in the same click task when changing back', async ({
    ...
    const [firstWorktreeId, secondWorktreeId] = await prepareSidebarForSwitchTest(dobiusPage)
    const firstRow = worktreeRow(dobiusPage, firstWorktreeId)
    const secondRow = worktreeRow(dobiusPage, secondWorktreeId)

    await expect(firstRow).toBeVisible()
    await expect(secondRow).toBeVisible()
    await expect(firstRow).toHaveAttribute('aria-current', 'page')
    await expect(dobiusPage.locator('[data-rendered-active-worktree-id]')).toHaveAttribute(
      'data-rendered-active-worktree-id',
      firstWorktreeId
    )

    const result = await dobiusPage.evaluate(
