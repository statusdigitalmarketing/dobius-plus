...
test.describe('Repository Display Name IME composition', () => {
  test('keeps Hangul syllables composed while typing slowly', async ({ dobiusPage }) => {
    await waitForSessionReady(dobiusPage)

    const repos = await getStoreState<Repo[]>(dobiusPage, 'repos')
    expect(repos.length).toBeGreaterThan(0)
    const repo = repos[0]

    await openRepoSettings(dobiusPage, repo.id)

    const repoSection = dobiusPage.locator(`[data-settings-section="repo-${repo.id}"]`)
    ...
    await typeHangulGanadaSlowly(session, dobiusPage, displayNameInput)
    ...
    await expect
      .poll(
        async () => {
          const current = await getStoreState<Repo[]>(dobiusPage, 'repos')
