...
test.describe('Tasks page', () => {
  ...
  test('opening the tasks view renders the tasks UI', async ({ dobiusPage }) => {
    await openTasksPage(dobiusPage)

    await expect
      .poll(async () => getStoreState<string>(dobiusPage, 'activeView'), { timeout: 5_000 })
      .toBe('tasks')

    await expect(dobiusPage.getByRole('button', { name: 'Close tasks' })).toBeVisible({
      timeout: 10_000
    })
    ...
    await expect
      .poll(
        async () => {
          renderedSources = await getRenderedTaskSources(dobiusPage)
    ...
    if (renderedSources.some((source) => source.source === 'github' && source.active)) {
      await expect(dobiusPage.getByRole('button', { name: 'Issues', exact: true })).toBeVisible()
      await expect(dobiusPage.getByRole('button', { name: 'PRs', exact: true })).toBeVisible()
      await expect(dobiusPage.getByRole('button', { name: 'Projects', exact: true })).toBeVisible()
      await expect(dobiusPage.getByPlaceholder(/Search GitHub (issues|PRs)/i)).toBeVisible()
    }
  })

  test('closing the tasks page returns to the previous view', async ({ dobiusPage }) => {
    const previousView = await getStoreState<string>(dobiusPage, 'activeView')
    ...
    await dobiusPage.getByRole('button', { name: 'Close tasks' }).click()
    ...
    await expect(dobiusPage.getByRole('button', { name: 'Close tasks' })).toHaveCount(0)
    if (previousView === 'terminal') {
      await expect(dobiusPage.locator('.xterm').first()).toBeVisible({ timeout: 5_000 })
    }
  })
})
