...
import { expect } from './helpers/dobius-app'
import {
...
export async function installPtyReplayProbe(page: Page): Promise<void> {
  await page.evaluate(() => {
    ...
    const holder = window as unknown as {
      __dobiusSshCodexReplayProbe?: {
        payloads: { id: string; length: number; preview: string }[]
        dispose: () => void
      }
    }
    holder.__dobiusSshCodexReplayProbe?.dispose()
    ...
    holder.__dobiusSshCodexReplayProbe = { payloads, dispose }
  })
}
