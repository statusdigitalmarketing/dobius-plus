...
async function createCloneFixture(): Promise<{
  ...
  const rootPath = realpathSync(
    await mkdtemp(path.join(os.tmpdir(), 'dobius-e2e-add-project-clone-'))
  )
...
async function createLinkedWorktreeFixture(): Promise<{
  ...
  const rootPath = realpathSync(
    await mkdtemp(path.join(os.tmpdir(), 'dobius-e2e-add-project-linked-'))
  )
  tempRoots.push(rootPath)
...
test.describe('Add project default checkout', () => {
  test('clones a repo and opens the default checkout without the setup-choice modal', async ({
    ...
    await dobiusPage
      ...
      .first()
      .click()
    const addDialog = dobiusPage.getByRole('dialog', { name: /Add a project/i })
    await expect(addDialog).toBeVisible()
    await addDialog.getByRole('button', { name: /Clone from URL/i }).click()

    const cloneDialog = dobiusPage.getByRole('dialog', { name: /Clone from URL/i })
    ...
    await expect(dobiusPage.getByRole('dialog', { name: /Repo added/i })).toBeHidden()
    await expect(dobiusPage.getByText('Use existing worktrees')).toBeHidden()
    await expect(dobiusPage.getByText('Create a new worktree')).toBeHidden()

    await expect
      .poll(
        () =>
          dobiusPage.evaluate((cloneName) => {
  ...
  test('reveals sibling git worktrees before opening the default checkout', async ({
    ...
    await dobiusPage.evaluate((folderPath) => {
      window.__store?.getState().openModal('confirm-add-project-from-folder', { folderPath })
    }, fixture.mainPath)
    const addProjectDialog = dobiusPage.getByRole('dialog', { name: /^Add Project$/i })
