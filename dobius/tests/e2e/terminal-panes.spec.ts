...
test.describe('Terminal Panes', () => {
  test.beforeEach(async ({ dobiusPage }) => {
    ...
    const hasPaneManager = await waitForActiveTerminalManager(dobiusPage, 30_000)
      .then(() => true)
      .catch(() => false)
    test.skip(
      !hasPaneManager,
    ...
    await waitForPaneCount(dobiusPage, 1, 30_000)
  })
   ...
   * - terminal panes can be split
   */
  test('can split terminal pane right', async ({ dobiusPage }) => {
    const paneCountBefore = await countVisibleTerminalPanes(dobiusPage)

    await splitActiveTerminalPane(dobiusPage, 'vertical')
    await waitForPaneCount(dobiusPage, paneCountBefore + 1)

    const paneCountAfter = await countVisibleTerminalPanes(dobiusPage)
   ...
   * - terminal panes can be split
   */
  test('can split terminal pane down', async ({ dobiusPage }) => {
    const paneCountBefore = await countVisibleTerminalPanes(dobiusPage)

    await splitActiveTerminalPane(dobiusPage, 'horizontal')
    await waitForPaneCount(dobiusPage, paneCountBefore + 1)
  ...
  test('split panes persist PTY bindings by stable UUID leaf id', async ({ dobiusPage }) => {
    const paneCountBefore = await countVisibleTerminalPanes(dobiusPage)

    await splitActiveTerminalPane(dobiusPage, 'vertical')
    await waitForPaneCount(dobiusPage, paneCountBefore + 1)

    const snapshot = await waitForPaneIdentitySnapshot(dobiusPage, paneCountBefore + 1)
  ...
  test('terminal process receives DOBIUS_PANE_KEY with the active UUID leaf id', async ({
    dobiusPage
  }) => {
    const snapshot = await waitForPaneIdentitySnapshot(dobiusPage, 1)
    ...
    const ptyId = await discoverActivePtyId(dobiusPage)
    const marker = `DOBIUS_PANE_KEY_E2E_${Date.now()}`

    await execInTerminal(dobiusPage, ptyId, `printf '${marker}=%s\\n' "$DOBIUS_PANE_KEY"`)
    await waitForTerminalOutput(dobiusPage, `${marker}=${expectedPaneKey}`)

    expect(activeLeafId).toMatch(UUID_RE)
  })

  test('terminal context menu copies the stable pane ID', async ({ dobiusPage }) => {
    const snapshot = await waitForPaneIdentitySnapshot(dobiusPage, 1)
    ...
    await openTerminalContextMenu(dobiusPage)
    await dobiusPage.getByText('Copy Pane ID', { exact: true }).click()

    await expect
      .poll(() => dobiusPage.evaluate(() => window.api.ui.readClipboardText()), { timeout: 3_000 })
      .toBe(expectedPaneKey)
    await expect(dobiusPage.getByText('Pane ID copied', { exact: true })).toBeVisible()
    expect(leafId).toMatch(UUID_RE)
  })

  test('first Set Title from terminal context menu stays open for typing', async ({ dobiusPage }) => {
    const title = `First menu title ${Date.now()}`

    await openTerminalContextMenu(dobiusPage)
    await dobiusPage.getByText('Set Title…', { exact: true }).click()

    const titleInput = dobiusPage.locator('.pane-title-input').first()
    await expect(titleInput).toBeVisible()
    ...
    await expect(titleInput).toHaveCount(0)
    await expect(dobiusPage.locator('.pane-title-text', { hasText: title })).toHaveCount(1)
  })

  test('Set Title editor renders in Dobius overlay while terminal reserves title space', async ({
    dobiusPage
  }) => {
    const title = `Reserved overlay title ${Date.now()}`
    const terminalBoxBefore = await readVisibleXtermContainerBox(dobiusPage)

    await openTerminalContextMenu(dobiusPage)
    await dobiusPage.getByText('Set Title…', { exact: true }).click()

    const titleInput = dobiusPage.locator('.pane-title-overlay-layer .pane-title-input').first()
    await expect(titleInput).toBeVisible()
    await expect(titleInput).toBeFocused()
    await expect(dobiusPage.getByText('Set Title…', { exact: true })).toBeHidden()
    await expect(dobiusPage.locator('.pane .pane-title-input')).toHaveCount(0)
    await expect(dobiusPage.locator('.pane[data-has-title]')).toHaveCount(1)
    await expect
      .poll(() =>
        dobiusPage
    ...
    const terminalBoxEditing = await readVisibleXtermContainerBox(dobiusPage)
    expectTerminalToReserveTitleSpace(terminalBoxEditing, terminalBoxBefore)

    await titleInput.fill(title)
    await titleInput.press('Enter')
    await expect(dobiusPage.locator('.pane-title-text', { hasText: title })).toBeVisible()
    await expect(dobiusPage.locator('.pane[data-has-title]')).toHaveCount(1)
    expectTerminalToReserveTitleSpace(
      await readVisibleXtermContainerBox(dobiusPage),
      terminalBoxBefore
    )
  })

  test('Set Title context menu opens from the title overlay strip', async ({ dobiusPage }) => {
    const title = `Overlay menu title ${Date.now()}`
    const updatedTitle = `Overlay menu updated ${Date.now()}`

    await setPaneTitleFromTerminalMenu(dobiusPage, title)
    await openPaneTitleContextMenu(dobiusPage, title)
    await dobiusPage.getByText('Set Title…', { exact: true }).click()
    ...
    await expect(dobiusPage.locator('.pane-title-text', { hasText: updatedTitle })).toHaveCount(1)
    await expect(dobiusPage.locator('.pane-title-text', { hasText: title })).toHaveCount(0)
  })

  test('Set Title strip activates its pane and accepts file-path drops', async ({ dobiusPage }) => {
    const title = `Drop target title ${Date.now()}`
    const droppedPath = `/tmp/title-drop-${Date.now()}.txt`

    await setPaneTitleFromTerminalMenu(dobiusPage, title)
    const initialSnapshot = await waitForPaneIdentitySnapshot(dobiusPage, 1)
    ...
    await splitActiveTerminalPane(dobiusPage, 'vertical')
    await waitForPaneCount(dobiusPage, 2)
    const splitSnapshot = await waitForPaneIdentitySnapshot(dobiusPage, 2)
    ...
    await expect
      .poll(async () => (await readPaneIdentitySnapshot(dobiusPage))?.activeLeafId ?? null)
      .toBe(otherPane.leafId)

    const titleBar = dobiusPage.locator('.pane-title-bar', { hasText: title }).first()
    ...
    await titleBar.evaluate((element, path) => {
      ...
      dataTransfer.setData('text/x-dobius-file-path', path)
    ...
    await expect
      ...
      .toBe(titledLeafId)
    await expect
      .poll(async () => (await getTerminalContent(dobiusPage)).includes(droppedPath), {
        timeout: 5_000,
  ...
  test('Set Title overlay follows its pane after same-count pane move', async ({ dobiusPage }) => {
    ...
    await splitActiveTerminalPane(dobiusPage, 'vertical')
    await waitForPaneCount(dobiusPage, 2)
    const beforeMove = await waitForPaneIdentitySnapshot(dobiusPage, 2)
    ...
    const beforeOrder = await readTerminalPaneDomLeafOrder(dobiusPage)

    await expectPaneTitleAttachedToLeaf(dobiusPage, title, titledLeafId)
    await moveTerminalPaneByLeafId(dobiusPage, titledLeafId, target.leafId, 'right')

    await expect
      .poll(async () => readTerminalPaneDomLeafOrder(dobiusPage), {
        timeout: 10_000,
  ...
  test('Set Title keeps the pane drag handle available over the title strip', async ({
    ...
    const titleTopHit = await dobiusPage.evaluate(
      ({ title, titledLeafId }) => {
    ...
    await dobiusPage.locator('.pane-title-bar', { hasText: title }).click({
      position: { x: 20, y: 18 }
    })
    await expect(dobiusPage.locator('.pane-title-input')).toBeVisible()
  })

  test('@headful Set Title pane can be dragged from the title strip', async ({ dobiusPage }) => {
    ...
    await splitActiveTerminalPane(dobiusPage, 'vertical')
    await waitForPaneCount(dobiusPage, 2)
    const beforeDrag = await waitForPaneIdentitySnapshot(dobiusPage, 2)
    ...
    const titleDragHandle = dobiusPage
    ...
    const targetBox = await dobiusPage.locator(`.pane[data-leaf-id="${target.leafId}"]`).boundingBox()
    ...
    await dobiusPage.mouse.move(sourceBox!.x + sourceBox!.width / 2, sourceBox!.y + 4)
    await dobiusPage.mouse.down()
    await dobiusPage.mouse.move(targetDropX, targetBox!.y + targetBox!.height / 2, {
      steps: 20
    })
    ...
    await expect
      ...
      .not.toEqual(beforeOrder)
    const afterDrag = await waitForPaneIdentitySnapshot(dobiusPage, 2)
  ...
  test('Set Title input stays open when clicked in a split terminal', async ({ dobiusPage }) => {
    await splitActiveTerminalPane(dobiusPage, 'vertical')
    await waitForPaneCount(dobiusPage, 2)
    await splitActiveTerminalPane(dobiusPage, 'horizontal')
    await waitForPaneCount(dobiusPage, 3)
  ...
  test('Set Title survives an early blur during first focus handoff', async ({ dobiusPage }) => {
    await openTerminalContextMenu(dobiusPage)
    await dobiusPage.evaluate(() => {
  ...
  test('Set Title survives delayed terminal focus handoffs', async ({ dobiusPage }) => {
    await openTerminalContextMenu(dobiusPage)
    await installDelayedTerminalFocusSteals(dobiusPage, [50, 150, 300])
    await dobiusPage.getByText('Set Title…', { exact: true }).click()

    const titleInput = dobiusPage.locator('.pane-title-input').first()
    await expect(titleInput).toBeVisible()
    await expect(titleInput).toBeFocused()
    await dobiusPage.waitForTimeout(600)
    await expect(titleInput).toBeVisible()
    await expect(titleInput).toBeFocused()
  })
  ...
  test('Set Title preserves draft text across terminal focus steals', async ({ dobiusPage }) => {
  ...
  test('Set Title does not submit when synthetic focus restore fails', async ({ dobiusPage }) => {
    ...
    await expect(titleInput).toBeVisible()
    await expect(titleInput).toHaveValue(draftTitle)
    await expect(dobiusPage.locator('.pane-title-text', { hasText: draftTitle })).toHaveCount(0)
  })
  ...
  test('Set Title still commits by blur after synthetic terminal focus steals', async ({
    ...
    await openTerminalContextMenu(dobiusPage)
    await installDelayedTerminalFocusSteals(dobiusPage, [50, 150])
    await dobiusPage.getByText('Set Title…', { exact: true }).click()

    const titleInput = dobiusPage.locator('.pane-title-input').first()
    await expect(titleInput).toBeVisible()
    await expect(titleInput).toBeFocused()
    await dobiusPage.waitForTimeout(300)
    await titleInput.fill(title)
    await dobiusPage
      .locator('.xterm:visible')
      .first()
  ...
  test('Set Title commits when tabbing away from the title input', async ({ dobiusPage }) => {
  ...
  test('Set Title overlay hides with its inactive terminal tab', async ({ dobiusPage }) => {
    ...
    await pressShortcut(dobiusPage, 't')
    await expect
      .poll(async () => (await getWorktreeTabs(dobiusPage, worktreeId)).length, { timeout: 5_000 })
      .toBeGreaterThanOrEqual(2)
    await expect(dobiusPage.locator('.pane-title-text', { hasText: title })).toBeHidden()

    await pressShortcut(dobiusPage, 'BracketLeft', { shift: true })
    await expect(dobiusPage.locator('.pane-title-text', { hasText: title })).toBeVisible()
  })

  test('Set Title still commits by blur after focus settles', async ({ dobiusPage }) => {
  ...
  test('Always-on pane header split button hover stays transparent', async ({ dobiusPage }) => {
    const splitButton = dobiusPage.getByRole('button', { name: 'Split Terminal Right' })
  ...
  test('Set Title stays pane-local during agent title churn', async ({ dobiusPage }) => {
    const worktreeId = (await getActiveWorktreeId(dobiusPage))!
    const tabId = (await getActiveTabId(dobiusPage))!
    ...
    await setPaneTitleFromTerminalMenu(dobiusPage, paneTitle)
    await expect(dobiusPage.locator('.pane-title-text', { hasText: paneTitle })).toBeVisible()
    await expectTabCustomTitle(dobiusPage, worktreeId, tabId, null)

    await dobiusPage.getByRole('button', { name: `Edit pane title: ${paneTitle}` }).focus()
    await dobiusPage.keyboard.press('Enter')
    const paneTitleInput = dobiusPage.getByRole('textbox', { name: 'Pane title' })
    ...
    // Why: active agents continuously write OSC titles. Set Title is Dobius's
    // pane-local overlay and must remain visible while the tab runtime title
    // continues to follow the active PTY.
    await expect(dobiusPage.locator('.pane-title-text', { hasText: paneTitle })).toBeVisible()
    await expect(
      dobiusPage.locator(`[data-testid="sortable-tab"][data-tab-id="${tabId}"]`)
    ).toHaveAttribute('data-tab-title', runtimeTitle)
    await expectTabCustomTitle(dobiusPage, worktreeId, tabId, null)

    await setPaneTitleFromTerminalMenu(dobiusPage, '')
    await expect(dobiusPage.locator('.pane-title-text', { hasText: paneTitle })).toBeHidden()
    await expectSavedLayoutNotToContainTitle(dobiusPage, tabId, paneTitle)

    await setPaneTitleFromTerminalMenu(dobiusPage, removeButtonTitle)
    await setPaneTitleFromTerminalMenu(dobiusPage, '')
    await expect(dobiusPage.locator('.pane-title-text', { hasText: removeButtonTitle })).toBeHidden()
    await expectSavedLayoutNotToContainTitle(dobiusPage, tabId, removeButtonTitle)

    await setPaneTitleFromTerminalMenu(dobiusPage, splitTitle)
    await expectTabCustomTitle(dobiusPage, worktreeId, tabId, null)

    await splitActiveTerminalPane(dobiusPage, 'vertical')
    await waitForPaneCount(dobiusPage, 2)
    await expect(dobiusPage.locator('.pane-title-text', { hasText: splitTitle })).toBeVisible()
  ...
  test('closing a split pane prunes its leaf-keyed PTY binding without remapping siblings', async ({
    ...
    const beforeClose = await waitForPaneIdentitySnapshot(dobiusPage, 3)
    ...
    await closeActiveTerminalPane(dobiusPage)
    await waitForPaneCount(dobiusPage, 2)

    const afterClose = await waitForPaneIdentitySnapshot(dobiusPage, 2)
  ...
  test('closing and remaking right/down splits keeps surviving leaf-keyed bindings stable', async ({
    ...
    const afterRemake = await waitForPaneIdentitySnapshot(dobiusPage, 3)
  ...
  test('moving panes through the drag-drop handler preserves leaf-keyed PTY bindings', async ({
    ...
    const beforeMove = await waitForPaneIdentitySnapshot(dobiusPage, 3)
    const beforeOrder = await readTerminalPaneDomLeafOrder(dobiusPage)
    ...
    await moveTerminalPaneByLeafId(dobiusPage, source.leafId, target.leafId, 'left')
    ...
    const afterMove = await waitForPaneIdentitySnapshot(dobiusPage, 3)
   ...
   * - terminal panes retain state when switching tabs and when you make / close a pane / switch worktrees
   */
  test('terminal pane retains content when switching tabs and back', async ({ dobiusPage }) => {
    // Write a unique marker to the current terminal
    const ptyId = await discoverActivePtyId(dobiusPage)
    const marker = `RETAIN_TEST_${Date.now()}`
    await execInTerminal(dobiusPage, ptyId, `echo ${marker}`)
    await waitForTerminalOutput(dobiusPage, marker)
    ...
    await expect
      .poll(async () => (await getTerminalContent(dobiusPage)).includes(marker), { timeout: 5_000 })
      .toBe(true)

    // Clean up the extra tab
    await pressShortcut(dobiusPage, 'BracketRight', { shift: true })
    await pressShortcut(dobiusPage, 'w')
  })
   ...
   * - terminal panes retain state when switching tabs and when you make / close a pane / switch worktrees
   */
  test('terminal pane retains content when splitting and closing a pane', async ({ dobiusPage }) => {
    // Write a unique marker to the current terminal
    const ptyId = await discoverActivePtyId(dobiusPage)
    ...
    const panesBefore = await countVisibleTerminalPanes(dobiusPage)

    // Split the terminal right
    await splitActiveTerminalPane(dobiusPage, 'vertical')
    await waitForPaneCount(dobiusPage, panesBefore + 1)

    await focusLastTerminalPane(dobiusPage)
    await closeActiveTerminalPane(dobiusPage)
    await waitForPaneCount(dobiusPage, panesBefore)
   ...
   * - terminal panes retain state when switching tabs and when you make / close a pane / switch worktrees
   */
  test('terminal pane retains content when switching worktrees and back', async ({ dobiusPage }) => {
    const allWorktreeIds = await getAllWorktreeIds(dobiusPage)
    ...
    const otherId = await switchToOtherWorktree(dobiusPage, worktreeId)
    expect(otherId).not.toBeNull()
    await expect.poll(async () => getActiveWorktreeId(dobiusPage), { timeout: 5_000 }).toBe(otherId)

    // Switch back to the original worktree
    await switchToWorktree(dobiusPage, worktreeId)
    await expect
      .poll(async () => getActiveWorktreeId(dobiusPage), { timeout: 5_000 })
      .toBe(worktreeId)
    ...
    await expect
      .poll(async () => (await getTerminalContent(dobiusPage)).includes(marker), { timeout: 20_000 })
      .toBe(true)
  })
   ...
   * - resizing terminal panes works
   */
  test('shows a pane divider after splitting', async ({ dobiusPage }) => {
    ...
    await expect(dobiusPage.locator('.pane-divider.is-vertical').first()).toBeVisible({
      timeout: 3_000
    })
  })
   ...
   *   DOBIUS_E2E_HEADFUL=1 pnpm run test:e2e
   */
  test('@headful can resize terminal panes by real mouse drag', async ({ dobiusPage }) => {
    ...
    const paneWidthsBefore = await dobiusPage.evaluate(() => {
    ...
    const divider = dobiusPage.locator('.pane-divider.is-vertical').first()
    ...
    await dobiusPage.mouse.down()
    await dobiusPage.mouse.move(startX + 150, startY, { steps: 20 })
    await dobiusPage.mouse.up()
    ...
    await expect
      .poll(
        async () => {
          const widthsAfter = await dobiusPage.evaluate(() => {
            const xterms = document.querySelectorAll('.xterm')
            return Array.from(xterms)
              .filter((x) => (x as HTMLElement).offsetParent !== null)
              .map((x) => (x as HTMLElement).getBoundingClientRect().width)
  ...
  test('@headful resizing split panes forwards only the settled PTY size', async ({ dobiusPage }) => {
    await splitActiveTerminalPane(dobiusPage, 'vertical')
    const snapshot = await waitForPaneIdentitySnapshot(dobiusPage, 2)
    ...
    await expect
      .poll(
        async () =>
          (await readVisiblePaneContents(dobiusPage)).every((content) =>
    ...
    await dobiusPage.mouse.move(startX, startY)
    await dobiusPage.mouse.down()
    await dobiusPage.mouse.move(startX - 350, startY, { steps: 40 })
    await dobiusPage.mouse.move(startX + 250, startY, { steps: 40 })
    await dobiusPage.mouse.up()
    await dobiusPage.waitForTimeout(500)

    const paneContents = await readVisiblePaneContents(dobiusPage)
  ...
  test('@headful dragging terminal panes around preserves leaf-keyed PTY bindings', async ({
    ...
    const beforeDrag = await waitForPaneIdentitySnapshot(dobiusPage, 3)
    const beforeOrder = await readTerminalPaneDomLeafOrder(dobiusPage)
    ...
    const sourceHandle = dobiusPage.locator(
    ...
    await dobiusPage.mouse.move(sourceBox!.x + sourceBox!.width / 2, sourceBox!.y + 4)
    await dobiusPage.mouse.down()
    await dobiusPage.mouse.move(targetBox!.x + 8, targetBox!.y + targetBox!.height / 2, {
      steps: 20
    })
    await dobiusPage.mouse.up()
    ...
    const afterDrag = await waitForPaneIdentitySnapshot(dobiusPage, 3)
    expect(afterDrag.panes.map((pane) => pane.leafId).sort()).toEqual(
      beforeDrag.panes.map((pane) => pane.leafId).sort()
    )
   ...
   * - closing panes works
   */
  test('closing a split pane removes it and remaining pane fills space', async ({ dobiusPage }) => {
    const panesBefore = await countVisibleTerminalPanes(dobiusPage)
    ...
    const panesAfterSplit = await countVisibleTerminalPanes(dobiusPage)
    expect(panesAfterSplit).toBeGreaterThanOrEqual(2)

    await closeActiveTerminalPane(dobiusPage)
    await waitForPaneCount(dobiusPage, panesAfterSplit - 1)

    // The remaining pane should fill the available space
    const paneWidth = await dobiusPage.evaluate(() => {
      const xterms = document.querySelectorAll('.xterm')
