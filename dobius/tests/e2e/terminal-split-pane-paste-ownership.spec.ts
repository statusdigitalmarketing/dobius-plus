...
test.describe('split terminal pane paste ownership', () => {
  test('keyboard paste writes only to the active split pane PTY', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.dobius-split-paste-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false

    try {
      await sendToTerminal(dobiusPage, activePane.ptyId, `node ${JSON.stringify(scriptPath)}\r`)
      scriptStarted = true
      await waitForTerminalOutput(dobiusPage, `SPLIT_PASTE_READY_${runId}`, 10_000)

      const payload = `DOBIUS_E2E_SPLIT_PASTE_${runId}`
      ...
      await dobiusPage.keyboard.press(keyboardPasteChord())
      ...
      if (scriptStarted) {
        await sendToTerminal(dobiusPage, activePane.ptyId, '\x03').catch(() => undefined)
      }
  ...
  test('internal file drop writes only to the pane under the drop target', async ({
    ...
    await dobiusPage.evaluate(
      ({ leafId, pathValue }) => {
        ...
        dataTransfer.setData('text/x-dobius-file-path', pathValue)
