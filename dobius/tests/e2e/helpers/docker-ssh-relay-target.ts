...
export const DOCKER_SSH_RELAY_REMOTE_REPO_PATH = '/tmp/dobius-docker-relay-perf-repo'
...
const CONTAINER_IMAGE = process.env.DOBIUS_E2E_SSH_DOCKER_IMAGE ?? 'node:22-bookworm'
...
function seedRemoteRepo(target: DockerSshRelayTarget): void {
  dockerExec(
    ...
    [
      ...
      'git config user.name "Dobius Docker SSH E2E"',
...
export function startDockerSshRelayTarget(testInfo: TestInfo): DockerSshRelayTarget {
  const tempDir = mkdtempSync(path.join(os.tmpdir(), 'dobius-ssh-docker-'))
  ...
  const containerName = `dobius-ssh-e2e-${testInfo.workerIndex}-${Date.now()}`
    ...
    run(
      ...
      [
        ...
        [
          ...
          'git config --global user.name "Dobius Docker SSH E2E"',
