/**
 * Shared Electron fixture for Dobius E2E tests.
 *
...
import { getDobiusElectronLaunchArgs } from './electron-launch-args'
import { getE2ECompletedOnboardingProfile } from './e2e-completed-onboarding-profile'

type DobiusTestFixtures = {
  ...
  // developer's shell or already-running Dobius instance.
  launchEnv: NodeJS.ProcessEnv
}

type DobiusWorkerFixtures = {
  /** Absolute path to the test git repo created by globalSetup. */
  testRepoPath: string
}

// Why: parse + warn at module scope so a bad DOBIUS_E2E_SLOWMO_MS value logs once
// per worker instead of once per test (otherwise hundreds of lines per CI run).
const DOBIUS_E2E_SLOWMO_MS_RAW = process.env.DOBIUS_E2E_SLOWMO_MS
const DOBIUS_E2E_SLOWMO_MS = ((): number => {
  if (DOBIUS_E2E_SLOWMO_MS_RAW === undefined) {
    return 0
  }
  const parsed = Number(DOBIUS_E2E_SLOWMO_MS_RAW)
  if (!Number.isFinite(parsed)) {
    console.warn(
      `[dobius-e2e] DOBIUS_E2E_SLOWMO_MS="${DOBIUS_E2E_SLOWMO_MS_RAW}" is not a number; ignoring (using 0).`
    )
    return 0
  }
...
function shouldLaunchHeadful(testInfo: TestInfo): boolean {
  // Why: DOBIUS_E2E_FORCE_HEADFUL lets a developer watch any spec in a real
  // window without retagging it `@headful` or switching projects.
  if (process.env.DOBIUS_E2E_FORCE_HEADFUL === '1') {
    return true
  }
  return testInfo.project.metadata.dobiusHeadful === true
}

function forwardElectronProcessLogs(app: ElectronApplication, testInfo: TestInfo): void {
  if (process.env.DOBIUS_E2E_FORWARD_APP_LOGS !== '1') {
    return
  }
...
function createSeededTestRepo(): string {
  ...
  const testRepoDir = realpathSync(mkdtempSync(path.join(os.tmpdir(), 'dobius-e2e-repo-')))
  ...
  const worktreeDir = path.join(testRepoDir, '..', `dobius-e2e-worktree-${randomUUID()}`)
...
/**
 * Extended Playwright test with Dobius-specific fixtures.
 *
 * `dobiusPage` — the main Dobius renderer window.
 *
 ...
 */
export const test = base.extend<DobiusTestFixtures, DobiusWorkerFixtures>({
  ...
  electronApp: async ({ dismissOnboarding, launchEnv }, provideFixture, testInfo) => {
    ...
    const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'dobius-e2e-userdata-'))
    ...
    void _unused
    // Why: DOBIUS_E2E_SLOWMO_MS adds a pause between every Playwright action so a
    // developer running with DOBIUS_E2E_FORCE_HEADFUL=1 can actually watch what
    // the test does. Defaults to 0 (no slowdown) for normal runs.
    const slowMo = DOBIUS_E2E_SLOWMO_MS
    // Why: DOBIUS_E2E_RECORD_VIDEO=1 captures a webm of the renderer so a
    ...
    const recordVideoDir = process.env.DOBIUS_E2E_RECORD_VIDEO === '1' ? testInfo.outputDir : null
    if (recordVideoDir) {
      mkdirSync(recordVideoDir, { recursive: true })
    }
    const app = await electron.launch({
      args: getDobiusElectronLaunchArgs(mainPath, headful),
      ...(slowMo > 0 ? { slowMo } : {}),
      ...(recordVideoDir ? { recordVideo: { dir: recordVideoDir } } : {}),
      // Why: keep NODE_ENV=development so window.__store is exposed and
      // dev-only helpers activate. DOBIUS_E2E_USER_DATA_DIR overrides the usual
      // shared dev profile so every spec gets a clean persistence root.
      // Why: DOBIUS_E2E_HEADLESS suppresses mainWindow.show() so the app
      // window stays hidden during test runs, avoiding focus stealing and
      // screen clutter. Playwright interacts via CDP regardless.
      // Why: DOBIUS_E2E_HEADLESS suppresses mainWindow.show() for CI/headless
      // runs. DOBIUS_E2E_HEADFUL overrides this for tests that need a visible
      ...
      env: {
        ...
        DOBIUS_E2E_USER_DATA_DIR: userDataDir,
        ...((process.env.DOBIUS_E2E_SSH_LOCALHOST === '1' ||
          process.env.DOBIUS_E2E_SSH_DOCKER === '1') &&
        !cleanEnv.DOBIUS_RELAY_PATH
          ? { DOBIUS_RELAY_PATH: path.join(process.cwd(), 'out', 'relay') }
          : {}),
        ...(headful ? { DOBIUS_E2E_HEADFUL: '1' } : { DOBIUS_E2E_HEADLESS: '1' })
      }
    })
  ...
  dobiusPage: async ({ sharedPage }, provideFixture) => {
