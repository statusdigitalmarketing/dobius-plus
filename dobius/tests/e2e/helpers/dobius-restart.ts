 ...
 *
 * Why: the default `dobiusPage` fixture creates a fresh `userDataDir` per test
...
export function createRestartSession(testInfo: TestInfo): RestartSession {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'dobius-e2e-restart-'))
  ...
  const launch = async (): Promise<LaunchedDobius> => {
...
export async function attachRepoAndOpenTerminal(page: Page, repoPath: string): Promise<string> {
  ...
  const repoId = await page.evaluate(async (repoPath) => {
    ...
    // worktree is external to Dobius's workspace root after the visibility rollout.
