...
test.describe('Windows terminal shell paste ownership', () => {
  ...
  test('PowerShell default terminal keyboard paste preserves exact content with one PTY owner', async ({
    ...
    await createWindowsDefaultShellTerminalTab(dobiusPage, 'powershell.exe')
    await waitForActiveTerminalManager(dobiusPage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(dobiusPage)
    const runId = randomUUID()
    const sentinel = `DOBIUS_E2E_POWERSHELL_DONE_${runId}`
    const powershellEscape = '`'
    const payload = [
      `DOBIUS_E2E_POWERSHELL_PASTE_${runId}`,
      `PowerShell metacharacters: ${powershellEscape} $ " ' ; | & < > @ { } ( )`,
      'quoted Windows path: C:\\Program Files\\Dobius Test\\file name.txt',
      'cmd metacharacters preserved as text: %PATH% !PROMPT! ^ & | < >',
      'Unicode: café 你好 مرحبا 😀',
      `mixed-newline-before\r\nlf-line\ncrlf-line\r\n${sentinel}`
    ].join('\n')
    const scriptPath = path.join(testRepoPath, `.dobius-paste-powershell-shell-${runId}.mjs`)
    ...
    try {
      ...
      scriptStarted = true
      await waitForTerminalOutput(dobiusPage, `PASTE_READY_${runId}`, 10_000)
      ...
      await dobiusPage.keyboard.press('Control+V')
      await waitForTerminalOutput(dobiusPage, `PASTE_COMPLETE_${runId}:MATCH`, 10_000, 12_000)
  ...
  test('cmd.exe default terminal keyboard paste preserves exact content with one PTY owner', async ({
    ...
    await waitForActiveWorktree(dobiusPage)
    await ensureTerminalVisible(dobiusPage)
    await createWindowsDefaultShellTerminalTab(dobiusPage, 'cmd.exe')
    await waitForActiveTerminalManager(dobiusPage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(dobiusPage)
    const runId = randomUUID()
    const sentinel = `DOBIUS_E2E_CMD_DONE_${runId}`
    const payload = [
      `DOBIUS_E2E_CMD_PASTE_${runId}`,
      'cmd metacharacters: %PATH% !PROMPT! ^ & | < >',
      'quoted Windows path: C:\\Program Files\\Dobius Test\\file name.txt',
      'PowerShell metacharacters: ` $ " \' ; @ { } ( )',
      `mixed-newline-before\r\nlf-line\ncrlf-line\r\n${sentinel}`
    ].join('\n')
    const scriptPath = path.join(testRepoPath, `.dobius-paste-cmd-shell-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
  ...
  test('Git Bash default terminal keyboard paste preserves POSIX-shaped content with one PTY owner', async ({
    ...
    await skipWhenGitBashUnavailable(dobiusPage)

    await waitForSessionReady(dobiusPage)
    await waitForActiveWorktree(dobiusPage)
    await ensureTerminalVisible(dobiusPage)
    await createWindowsDefaultShellTerminalTab(dobiusPage, WINDOWS_GIT_BASH_SHELL)
    await waitForActiveTerminalManager(dobiusPage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(dobiusPage)
    const runId = randomUUID()
    const sentinel = `DOBIUS_E2E_GIT_BASH_DONE_${runId}`
    const payload = [
      `DOBIUS_E2E_GIT_BASH_PASTE_${runId}`,
    ...
    const scriptPath = path.join(testRepoPath, `.dobius-paste-git-bash-shell-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
  ...
  test('WSL terminal keyboard paste preserves Linux shell content with one PTY owner', async ({
    ...
    await waitForActiveWorktree(dobiusPage)
    await ensureTerminalVisible(dobiusPage)
    const wslDistro = await configureActiveProjectWslRuntime(dobiusPage)
    test.skip(!wslDistro, 'No WSL distro is available on this Windows host')
    await createWindowsProjectRuntimeTerminalTab(dobiusPage, 'wsl.exe')
    await waitForActiveTerminalManager(dobiusPage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(dobiusPage)
    const runId = randomUUID()
    const sentinel = `DOBIUS_E2E_WSL_DONE_${runId}`
    const payload = [
      `DOBIUS_E2E_WSL_PASTE_${runId}`,
      'POSIX shell metacharacters: $ ` " \' ; | & < > * ? [ ] ( )',
    ...
    const scriptPath = path.join(testRepoPath, `.dobius-paste-wsl-shell-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
  ...
  test('existing WSL terminal keeps paste runtime after default shell changes', async ({
    ...
    const wslDistro = await configureActiveProjectWslRuntime(dobiusPage)
    test.skip(!wslDistro, 'No WSL distro is available on this Windows host')
    const tabId = await createWindowsProjectRuntimeTerminalTab(dobiusPage, 'wsl.exe')
    await updateWindowsDefaultShellSetting(dobiusPage, 'cmd.exe')
    await expect(
      dobiusPage.locator(`[data-testid="sortable-tab"][data-tab-id="${tabId}"] [data-shell-icon]`)
    ...
    const ptyId = await waitForActivePanePtyId(dobiusPage)
    const runId = randomUUID()
    const sentinel = `DOBIUS_E2E_WSL_RETENTION_DONE_${runId}`
    const payload = [
      `DOBIUS_E2E_WSL_RETENTION_PASTE_${runId}`,
    ...
    const scriptPath = path.join(testRepoPath, `.dobius-paste-wsl-retention-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
