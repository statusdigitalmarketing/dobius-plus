...
test.describe('Codex hidden startup composer background', () => {
  test('restores the input background when a Codex worktree first becomes visible', async ({
    ...
    const hiddenTabId = await dobiusPage.evaluate(
    ...
    const hiddenPtyId = await waitForHiddenTabPtyId(dobiusPage, hiddenTabId)
    await expect
      .poll(() => mainSnapshotContains(dobiusPage, hiddenPtyId, marker), {
        timeout: 20_000,
    ...
    await expect
      .poll(
        async () => {
          try {
            const nextTarget = await readCodexStartupBackgroundTarget(dobiusPage, marker)
            target = nextTarget
    ...
    const visibleBackgroundPixels = await countVisibleBackgroundPixels(dobiusPage, target)
