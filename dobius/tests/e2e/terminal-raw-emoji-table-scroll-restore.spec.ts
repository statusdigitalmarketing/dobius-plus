...
test.describe('Terminal raw emoji table scroll restore repro', () => {
  ...
  test('uses WebGL by default for ordinary terminal output when available @terminal-rendering-golden', async ({
    ...
    const marker = `DOBIUS_AUTO_WEBGL_SMOKE_${randomUUID()}`

    await sendToTerminal(dobiusPage, ptyId, `printf ${JSON.stringify(`${marker}\\n`)}\r`)
    await waitForTerminalOutput(dobiusPage, marker, 10_000)

    const expectedWebgl = await expectAutoWebgl(dobiusPage)
    ...
    let diagnostics = await readTerminalRenderDiagnostics(dobiusPage)
    await expect
      .poll(
        async () => {
          diagnostics = await readTerminalRenderDiagnostics(dobiusPage)
  ...
  test('keeps raw emoji box table aligned after restore and scroll @terminal-rendering-golden', async ({
    ...
    await ensureTerminalVisible(dobiusPage)
    await waitForActiveTerminalManager(dobiusPage, 30_000)
    await setWideRenderedTableViewport(dobiusPage)
    await waitForActiveTerminalColumns(dobiusPage, RAW_EMOJI_BOX_TABLE_WIDTH)
    const ptyId = await waitForActivePanePtyId(dobiusPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.dobius-raw-emoji-fixture-table-${runId}.mjs`)
    ...
    try {
      ...
      await waitForTerminalOutput(dobiusPage, completionMarker, 20_000, 30_000)
      await expect
      ...
      await scrollActiveTerminalToText(dobiusPage, 'Singer')
      await closeFeatureTips(dobiusPage)
      const expectedWebgl = await expectAutoWebgl(dobiusPage)
      ...
      const singerGeometry = await readVisibleSingerRowGeometry(dobiusPage)
