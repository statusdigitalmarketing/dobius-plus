...
test.describe('Reveal active workspace button', () => {
  ...
  test('reveals the current workspace when it is clipped in the production sidebar', async ({
    ...
    await prepareSidebarForScrollTest(dobiusPage)

    const renderedOptions = dobiusPage.locator('[data-worktree-sidebar] [role="option"]')
    ...
    const targetRow = dobiusPage
      .locator(`[data-worktree-sidebar] [data-worktree-id=${JSON.stringify(targetId)}]`)
      .first()
    const revealButton = dobiusPage.getByRole('button', { name: 'Reveal active workspace' })

    await renderedOptions.last().click()
    await expect(targetRow).toHaveAttribute('aria-current', 'page')
    await expectNoRevealHighlightDuring(dobiusPage, targetId, 400)
    await expect(revealButton).toBeVisible()
    await expect(revealButton).toBeEnabled()
    await forceCurrentWorkspaceClipped(dobiusPage, targetId)
  ...
  test('reveals the current workspace when it starts outside the virtualized window', async ({
    ...
    const targetId = await dobiusPage.evaluate(() => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
      }
    ...
    const scroller = dobiusPage.locator('[data-worktree-sidebar]')
    await expect
      .poll(
        () =>
          dobiusPage.evaluate(() => {
  ...
  test('uses the active workspace key when the legacy active worktree id is not set', async ({
    ...
    const folderWorktreeId = await dobiusPage.evaluate(() => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
      }
