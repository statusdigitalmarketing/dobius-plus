...
async function installPowerSaveBlockerProbe(electronApp: ElectronApplication): Promise<void> {
  await electronApp.evaluate(({ powerSaveBlocker }) => {
    ...
    if (root.__dobiusAwakePowerProbe) {
      root.__dobiusAwakePowerProbe.starts = []
      root.__dobiusAwakePowerProbe.stops = []
      return
    }
    ...
    powerSaveBlocker.start = ((type) => {
      ...
      root.__dobiusAwakePowerProbe?.starts.push({ type, id })
      return id
    }) as typeof powerSaveBlocker.start

    powerSaveBlocker.stop = ((id) => {
      root.__dobiusAwakePowerProbe?.stops.push({ id })
      originalStop(id)
...
async function postCodexHookEvent(
  ...
  const response = await fetch(`http://127.0.0.1:${endpoint.port}/hook/codex`, {
    ...
    headers: {
      ...
      'X-Dobius-Agent-Hook-Token': endpoint.token
    },
    body: JSON.stringify({
...
test.describe('Agent awake setting', () => {
  ...
  test('can be toggled from Agents settings and persists through IPC', async ({ dobiusPage }) => {
    await openSettings(dobiusPage)
    await dismissTransientAnnouncement(dobiusPage)
    await dobiusPage.getByPlaceholder('Search settings').fill('awake')

    await expect(
      dobiusPage.getByText('Keep computer awake while agents are working').first()
    ).toBeVisible()

    const keepAwakeSwitch = dobiusPage.getByRole('switch', {
    ...
    await expect
      .poll(async () => (await getSettings(dobiusPage)).keepComputerAwakeWhileAgentsRun, {
        timeout: 5_000,
  ...
  test('keeps the OS awake only while a hook-reported agent is working', async ({
    ...
    await setKeepAwake(dobiusPage, true)
