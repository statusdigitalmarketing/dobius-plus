...
import { test as base, expect } from './helpers/dobius-app'
import { waitForSessionReady } from './helpers/store'

const fakeGhDir = mkdtempSync(path.join(os.tmpdir(), 'dobius-e2e-fake-gh-'))
  ...
  launchEnv: [
    {
      ...
      DOBIUS_GH_EXEC_TIMEOUT_MS: '1000'
    },
    { option: true }
  ]
})
...
test('GitHub Tasks drawer recovers when gh stalls on issue details', async ({
  ...
  const { repoId } = await dobiusPage.evaluate((repoPath) => {
