...
test.describe('Setup script import prompt', () => {
  ...
  test('imports Superset local overlays through the prompt UI', async ({ dobiusPage }, testInfo) => {
    const repoPath = createSupersetSetupRepo(testInfo.outputPath('superset-setup-repo'))
    const repoId = await addAndActivateRepo(dobiusPage, repoPath)
    ...
    await dobiusPage.getByRole('button', { name: 'Save local setup' }).click()

    await expect(
      dobiusPage.getByText('2 unsupported fields skipped. Saved the setup command.')
    ).toBeVisible()

    const localCommands = await openImportedSetupSettingsFromToast(dobiusPage, repoId)
  ...
  test('imports cmux setup commands through the prompt UI', async ({ dobiusPage }, testInfo) => {
    ...
    await expect(
      dobiusPage.getByRole('button', { name: "project's settings", exact: true })
    ).toBeVisible()

    const repoSettings = await openRepoSettings(dobiusPage, repoId)
