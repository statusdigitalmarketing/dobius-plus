# Design: Manual network address entry for Dobius desktop mobile pairing
...
3. Should `CommandInput` accept paste of a multi-line string (e.g. user pastes `dobius://pair?code=…`)? Recommend: no special handling; the existing trim+validate treats it as an invalid address and shows the error. Pair-URL paste remains the path through `pair-scan.tsx`.
