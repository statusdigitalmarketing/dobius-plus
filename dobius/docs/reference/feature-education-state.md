...
Dobius uses local persisted UI state to decide which education surfaces should appear for a user. Keep these concepts separate; they answer different questions and should not be collapsed into one flag.
...
`featureTipsSeenIds` answers: "Has Dobius already surfaced this tip?"
...
`contextualToursSeenIds` answers: "Has Dobius already surfaced this contextual tour?"
...
Missing means Dobius has not classified the profile yet. On first hydrated launch after this change, the renderer sets it once:
