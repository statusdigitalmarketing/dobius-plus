...
This note tracks the investigation into terminal input lag that appears after switching workspaces or terminals in a heavy packaged Dobius profile.

The user-visible symptom is that typing after switching back to a workspace can feel delayed for about one second. Text may appear all at once after the delay. The issue reproduced in the user's main packaged Dobius app, but not reliably in lighter dev profiles.

## Current Reproduction Setup

- Main packaged Dobius app is used for the meaningful repro.
- The main profile had roughly 150 terminals when the lag reproduced.
- The current worktree under test is `/Users/jinwoohong/dobius/workspaces/dobius/osprey`.
...
```sh
DOBIUS_PROBE_RUNS=8 DOBIUS_PROBE_FOCUS_EACH_RUN=0 DOBIUS_PROBE_TYPE_MODE=terminal-send DOBIUS_PROBE_RECEIPT_MODE=file node .tmp/terminal-main-app-typing-lag/probe.mjs
DOBIUS_PROBE_RUNS=8 DOBIUS_PROBE_FOCUS_EACH_RUN=0 DOBIUS_PROBE_TYPE_MODE=daemon-direct-request DOBIUS_PROBE_RECEIPT_MODE=file node .tmp/terminal-main-app-typing-lag/probe.mjs
DOBIUS_PROBE_RUNS=8 DOBIUS_PROBE_SKIP_SWITCH=1 DOBIUS_PROBE_FOCUS_EACH_RUN=0 DOBIUS_PROBE_TYPE_MODE=daemon-direct-request DOBIUS_PROBE_RECEIPT_MODE=file node .tmp/terminal-main-app-typing-lag/probe.mjs
DOBIUS_PROBE_RUNS=4 DOBIUS_PROBE_FOCUS_EACH_RUN=0 DOBIUS_PROBE_SKIP_FOCUS=1 DOBIUS_PROBE_SAMPLE_RENDERER=0 DOBIUS_PROBE_ALT_TERMINAL=term_4d7a0b50-e9ae-420e-ac5d-7ec12cbfa408 DOBIUS_PROBE_TYPE_MODE=daemon-direct-request DOBIUS_PROBE_RECEIPT_MODE=file node .tmp/terminal-main-app-typing-lag/probe.mjs
```
...
A direct daemon client sent `ping` requests every roughly 50 ms while the harness issued `dobius terminal switch` away and back.
...
- Adding `DOBIUS_PROBE_AFTER_SWITCH_AWAY_MS=1500` before switching back made direct daemon writes fast again.
- This indicates expensive work starts when Dobius switches into or resumes the output-rich workspace, then spills into the immediate switch-back/write window.
...
- It is probably not queueing only on Dobius's normal daemon client socket. The direct-daemon harness uses a separate socket and still sees the delay.
...
- Runtime terminal send/focus/write: `src/main/runtime/dobius-runtime.ts`
- Runtime PTY data handling: `onPtyData`, `trackHeadlessTerminalData`, hidden-output serialization in `src/main/runtime/dobius-runtime.ts`
...
- Do not kill or restart the user's main packaged Dobius app or daemon without explicit approval.
