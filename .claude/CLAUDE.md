...
## Workspace and install target

- The active Dobius+ workspace is `/Users/bayou/Projects (Code)/dobius-plus`.
- Make app changes in this workspace, including files under `dobius/`. Do not jump to `~/DobiusPlus` or the sibling folders `dobius-plus-redesign/`, `dobius-shell-rebrand/`, or `dobius-parallel-worktrees/` unless the user explicitly redirects you.
- The installed app target is the existing `/Applications/Dobius+.app`. Build/install this workspace into that app; do not create or install a side app from another checkout.
- Workflow is local-first: build, test, and install locally until the app is in a good working state. Push to `statusdigitalmarketing/dobius-plus` and open a PR only at a stable checkpoint.
- Before any install, verify `git rev-parse --show-toplevel` reports `/Users/bayou/Projects (Code)/dobius-plus`.
